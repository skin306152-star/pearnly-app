...
- **代码规模快照**(实时跑 `scripts/refactor_progress.py`):home.js **5737**→<200 · app.py **1730**→<500 · db.py **819**→<500 · home.css **0** ✅ · home.html **4410**→<1000 · 综合 **~80%**。
- **模块/测试**:src/home **38**(+line-email-modal/change-password/session-heartbeat WB) · *_routes **36** · services **106**(+playwright_bootstrap +background_loops +static_assets +erp/auto_push +startup WA)· unit **1666** · integration **47**(WC-D3 +signup +VAT excel +bank-recon +导入器映射 +OCR recognize 守门 契约)· E2E **17** · home.js silent **0**。
