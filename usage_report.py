...
def _group_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        if uid not in groups:
            groups[uid] = {
                "user_label": r.get("user_name") or (r.get("user_email") or "").split("@")[0] or "—",
...
def _build_paragraph_text(s: str, bold: bool = False) -> str:
    ...
    if not s:
        return ''
    cjk_font = 'PR-CJK' if _HAS_CJK else ('PR-Thai' if _HAS_THAI else ('Helvetica-Bold' if bold else 'Helvetica'))
...
def build_pdf(
    *,
    ...
    title_style = ParagraphStyle('title', fontName=bold_font, fontSize=16, alignment=TA_CENTER, leading=22, spaceAfter=2)
    sub_style = ParagraphStyle('sub', fontName=base_font, fontSize=10, alignment=TA_CENTER, leading=14, textColor=colors.HexColor('#475569'))
    user_style = ParagraphStyle('user', fontName=bold_font, fontSize=11.5, leading=15, spaceBefore=10, spaceAfter=3, textColor=colors.HexColor('#0f172a'))
    cell_left = ParagraphStyle('cell-left', fontName=base_font, fontSize=9, leading=12, alignment=TA_LEFT)
    cell_right = ParagraphStyle('cell-right', fontName=base_font, fontSize=9, leading=12, alignment=TA_RIGHT)
    cell_header = ParagraphStyle('cell-hd', fontName=bold_font, fontSize=9.5, leading=12, alignment=TA_LEFT, textColor=colors.HexColor('#1f2937'))
    cell_header_r = ParagraphStyle('cell-hd-r', fontName=bold_font, fontSize=9.5, leading=12, alignment=TA_RIGHT, textColor=colors.HexColor('#1f2937'))
    cell_subtotal = ParagraphStyle('cell-st', fontName=bold_font, fontSize=9.5, leading=12, alignment=TA_RIGHT, textColor=colors.HexColor('#9a3412'))
    cell_total = ParagraphStyle('cell-tot', fontName=bold_font, fontSize=11, leading=14, alignment=TA_RIGHT, textColor=colors.HexColor('#7c2d12'))
    empty_style = ParagraphStyle('empty', fontName=base_font, fontSize=11, alignment=TA_CENTER, leading=14, textColor=colors.HexColor('#94a3b8'))
    ...
    *,
    ...
    if not groups:
        ...
        for g in groups:
            ...
            for r in g["rows"]:
                date_str = (r.get("date") or "")[:10]
                c = ws.cell(row=row, column=1, value=date_str); c.border = border; c.alignment = left
                c = ws.cell(row=row, column=2, value=r.get("filename") or "—"); c.border = border; c.alignment = left
                c = ws.cell(row=row, column=3, value=int(r.get("pages") or 0)); c.border = border; c.alignment = right
