...
class RegroupActionsTests(unittest.TestCase):
    def test_projects_original_name_and_kinds(self):
        ...
        (got,) = machine_actions.regroup_actions([_regroup_event("i1")], items)
        self.assertEqual(got["name"], "IMG_2497.jpg")
        self.assertEqual((got["from_kind"], got["to_kind"]), ("non_tax", "bank_statement"))
        self.assertEqual(got["reason"], "statement_sequence")
    ...
    def test_display_name_strips_storage_prefix(self):
        items = [{"id": "i1", "file_ref": "/sealed/3f2a1b9c8d7e__IMG_2497.jpg"}]
        (got,) = machine_actions.regroup_actions([_regroup_event("i1")], items)
        self.assertEqual(got["name"], "IMG_2497.jpg")

    def test_repeated_regroup_of_same_item_yields_one_row(self):
        # latest-wins:同件重复改判只呈现最后一次,与其余读侧投影同语义。
        first = _regroup_event("i1")
        second = _regroup_event("i1", from_kind="duplicate")
        second["id"] = 9
        out = machine_actions.regroup_actions([first, second], [])
        self.assertEqual(len(out), 1)
        self.assertEqual(out[0]["from_kind"], "duplicate")
...
class BankRowCorrectionsTests(unittest.TestCase):
    ...
    def test_same_item_parsed_twice_counts_once(self):
        events = [
            _parsed_event("b1", [_row(amount_autocorrected=True)], event_id=1),
            _parsed_event("b1", [_row(amount_autocorrected=True)], event_id=2),
        ]
        out = machine_actions.bank_row_corrections(events)
        self.assertEqual(len(out), 1)
        self.assertEqual(out[0]["amount_rows"], 1)
