...
## 🎯 状态卡（2026-06-26 · **B8 RLS · wave4 erp 映射表 + erp_endpoints/push_logs(JOIN 富化) + import_template + email_ingest 全 enroll 上线 + /simplify 收口** · 已 push（origin 含 email_ingest+/simplify·见 §7.10 git 事故注） · CI 绿 + prod 金丝雀 PASS · companion 未动 · 交接 `docs/refactor/b8-rls-HANDOFF-2026-06-25.md` §7.7/§7.8/§7.9/§7.10）

- **★ wave4 第四棒 email_ingest 三表 enroll + /simplify 收口 上线 prod 验过**(`74065156`+/simplify 收口)：accounts/logs 纯 user(`apply_user_rls`)、seen_uids 经父表 `apply_user_via_parent_rls`。**/simplify 把三处 legacy enroll 统一到 `ensure_bank_recon_rls` 范式**——独立 `ensure_erp_push_rls`/`ensure_email_ingest_rls`(各自事务·进 boot_ensures·不再藏进 retry-ALTER 函数/不再 startup 第二套 loop)。金丝雀:5 表 rls=on policies=1。集成 10 例 + 4973 单测 + 三道 per-commit 闸绿。**★git 事故**(详见 §7.10):另一窗口在我 commit→amend 间隙 push,致 `--amend` 改错 commit→origin message 错位但**代码 100% 正确已部署**;master 禁强推无法补 message,已 `reset --soft origin/master` 采纳+docs 收尾让最新 CI 转绿。**教训:共享树别用 `--amend`,commit 后立即 push,RATCHET-EXEMPT 逐文件列。** **wave4 剩零暴露孤儿**(erp_oauth_*/mrerp_credentials/erp_connectors/excel_templates·无访问点·范式已定照搬即可·低优先)。
- **★ wave4 第三棒 import_template_mappings enroll 上线 prod 验过**(`05dfd10b`):纯 tenant·`apply_tenant_rls` 落 `template_store.ensure_table`(startup 跑)·4 CRUD 穿 tenant。金丝雀真租户 758bea47 自见 2/假 0。集成 3 例 + 4970 单测 + CI 6 闸绿(`28237294792`)。
