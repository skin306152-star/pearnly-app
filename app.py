...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.35.0.9 · credits 全站统一 + 首页余额卡 + 升级 modal 彻底下线:\n• 全部 15 个老用户(trial/free/monthly/yearly/lifetime)统一迁到 plan='credits' · 全员钱包计费\n• 首页 KPI 第二排恢复:账户余额卡(老板看到 ฿X.XX · <50 红字加充值链接 · 豁免账号显示 ∞)+ 本月用量卡(分级 ฿1.50 / ฿0.75)\n• 升级 modal 整套(顶部横幅 / showUpgradeModal / openUpgradeModal / data-upgrade-cta 点击绑定)永久下线",
            "th": "v118.35.0.9 · credits ทุกบัญชี + การ์ดยอดเงินหน้าแรก + ปิดโมดอลอัปเกรดถาวร:\n• ผู้ใช้เก่าทั้งหมด 15 บัญชี (trial/free/monthly/yearly/lifetime) ย้ายไป plan='credits' · ใช้กระเป๋าเงินทุกคน\n• แถวที่สองของ KPI หน้าแรก: ยอดคงเหลือ (เจ้าของเห็น ฿X.XX · ต่ำกว่า 50 แดง · ยกเว้นแสดง ∞) + การใช้งานเดือนนี้ (฿1.50 หรือ ฿0.75 ตามขั้น)\n• โมดอลอัปเกรดทั้งชุด (แบนเนอร์ / ฟังก์ชัน / event binding) ปิดถาวร",
            "en": "v118.35.0.9 · credits unified + homepage balance card + upgrade modal fully retired:\n• All 15 legacy users (trial/free/monthly/yearly/lifetime) migrated to plan='credits' · everyone on wallet billing\n• Homepage KPI row 2 restored: account balance card (owners see ฿X.XX · <50 red with top-up link · exempt shows ∞) + usage card (tiered ฿1.50 or ฿0.75)\n• Upgrade modal infrastructure (banner / showUpgradeModal / openUpgradeModal / data-upgrade-cta bindings) permanently retired",
            "ja": "v118.35.0.9 · credits 統一 + ホーム残高カード + アップグレードモーダル完全廃止:\n• 15 名の旧ユーザー全員(trial/free/monthly/yearly/lifetime)を plan='credits' へ移行 · 全員ウォレット課金\n• ホーム KPI 2 行目を復活:残高カード(オーナーは ฿X.XX · 50 未満は赤+チャージリンク · 免除は ∞)+ 利用量カード(段階 ฿1.50 / ฿0.75)\n• アップグレードモーダル一式(バナー / 関数 / クリックバインド)を永久廃止",
