...
> **当前数字(master 全绿·工作树干净)**:home.js **6191**(第四十二会话:抽 admin→整删老 admin 布局→删死套餐前端 IIFE · 8941→6191 · -2750)· app.py 4839 · db.py 3371 · home.css 0 · home.html **6428** · src/home 模块 **35** 个 · 单测 **1516** · E2E **10**。`gh.exe` 路径 `C:\Program Files\GitHub CLI\gh.exe` · repo `skin306152-star/pearnly-app`。
> **🅱 计费模型已变(重要)**:产品现在**只有「充值+按量扣费」** · 老月订/年订/体验(trial)/免费(free)套餐**全下线** → home.js+后端大量老套餐 UI/PLG 试用闸/升级弹窗 = **死码可清**(I2)。
> **下一个任务 = 计费迁移收尾 step2b(后端老 plan 配置清理 · 选做 · 低优先)**:step1(`_get_plan`→credits `101824d`)+ step2a(删死的 v109.3 套餐前端 IIFE -762 行 `1f7d8b8`)均已上线验毕(横幅全隐 · E2E 10/10 · CI 绿)。**剩 step2b = 删后端 `PLAN_CONFIG` 老档(trial/monthly/yearly/lifetime)+ `LEGACY_PLAN_MAP`**。这些现在是**无害死配置**(`_get_plan` 恒返 credits/admin 已覆盖 · 功能权限早扁平化 · OCR 只看 credits 余额),但删它**牵连 3 处需逐一核**:① `get_plan_features` 的 fallback `PLAN_CONFIG["trial"]`(默认参表达式 · 删 trial 会 KeyError → 须先改 `["credits"]`)· 且 `get_plan_features` 喂 `/api/me/plan` → getMaxFiles **上传上限**,改错会让上传上限失常。② signup 写 `plan=invite_plan`(credits 默认/pro·firm 邀请码)· 现被 `_get_plan` 覆盖成 vestigial,可简化为恒 credits,但改的是注册写库值。③ app.py ~10 处直读 `user["plan"]`(1316/1450-1511/1751/2791…)在旧 quota 死路径里 · 删 PLAN_CONFIG 档前要确认它们确死。**做法**:Zihao 在场专注一刀 · 先改 get_plan_features fallback→credits + 删 PLAN_CONFIG 老档 + 删 LEGACY_PLAN_MAP(改 signup 引用)· pytest 守门 + 真账号验**上传上限 + 注册 + 充值**。**不急**(纯无害死配置 · 已不影响任何用户)。
...
> ════════════════════════════════════════════════════════════
> **【第四十二会话 · 交接 · 2026-05-27】整删老 admin 布局(-4566)+ 计费迁移收尾 step1+2a(`_get_plan`→credits · 删死套餐前端 IIFE -762)· home.js 8941→6191(-2750)· home.html 6658→6428 · 全程双账号实测 10/10 + CI 全绿 · 剩 step2b 后端老 plan 配置(无害死配置·选做)**
> ════════════════════════════════════════════════════════════
