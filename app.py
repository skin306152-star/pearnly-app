...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.15 · MR.ERP 登录表单 timing 加固:\n• 之前测试连接偶尔成功偶尔报「login form missing txtusers/txtpasswords inputs」· 不是配置问题 · 是网页爬虫 timing\n• 根因:导航到 /login/login.php 之后立刻 page.locator(...).count() · 表单还没渲染就 0\n• 修(4 层):\n  - 找 input 前 wait_for_selector 15s budget · 主选择器 + 4 个 fallback(input[id*='user']等)· 网站结构小改也接得住\n  - 找不到时:detect_maintenance_page() 检查 body 文字(ปิดปรับปรุง / under maintenance 等)· 是维护就报清晰错误\n  - 不是维护:page.reload() 再 wait 15s\n  - 仍找不到:存截图到 /tmp/mrerp_login_fail_<ts>.png · 路径塞进 raw_error · /api/version.last_500 看得到\n• test-connection 外层 retry_attempts 1→2 · 配合内层 retry 给一次测试两层兜底\n• 加守门测试 LoginFormRetryTests · 起本地 HTTP server 拖 3s 才返表单 HTML · 验证 wait_for_selector 真起作用\n\n刷浏览器测试连接 3 次 · 应该至少 2 次返 companies",
            "en": "v118.34.15 · MR.ERP login form timing hardening:\n• Test-connection succeeded once then started failing intermittently with 'login form missing txtusers/txtpasswords inputs'. Not a config issue — the scraper raced the page render.\n• Root cause: after navigating to /login/login.php, the code immediately called page.locator('input[name=txtusers]').count(). Slow networks or mid-deploy MR.ERP returned a body without the form HTML yet, count==0, raised technical error.\n• Four-layer fix:\n  - Before lookup, wait_for_selector with 15 s budget. Primary selector plus 4 fallbacks (input[id*='user'], input[type='text']:visible, etc.) so a minor MR.ERP markup change doesn't break us.\n  - On timeout: detect_maintenance_page() scans body for Thai + English maintenance markers (ปิดปรับปรุง / under maintenance / 503 service unavailable). If matched, raises 'MR.ERP under maintenance' specifically.\n  - Otherwise: page.reload() + another 15 s wait_for_selector budget.\n  - Still nothing: full-page screenshot to /tmp/mrerp_login_fail_<ts>.png; path embedded into the MRERPTechnicalError message → wizard's raw_error → /api/version.last_500 surfaces it for the operator.\n• Bumped test-connection's outer retry_attempts 1→2 with 2 s delay between, layered with the new inner reload-retry so one click gets two full reload cycles before giving up.\n• Added LoginFormRetryTests — spins a local HTTP server that delays the login HTML by 3 s. Confirms the wait_for_selector path survives without raising 'login form missing'.\n\nClick test-connection a few times — expect stable success (companies) or a clean ERR_AUTH if credentials are wrong.",
