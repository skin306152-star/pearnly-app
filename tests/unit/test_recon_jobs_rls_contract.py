# -*- coding: utf-8 -*-
"""B8 RLS 契约:recon_jobs store(REFACTOR-B8 wave2)。

recon_jobs 是 tenant_or_user 模板的队列表。约束:
- 用户面 enqueue / get 必须把 tenant_id + user_id 注入 get_cursor_rls(无 bypass),
  否则启用 RLS 后用户读不到自己的任务 / INSERT 被 WITH CHECK 拒。
- worker 队列操作(claim_next / update_progress / finish / set_* / fail / reclaim_stale / gc_old)
...
from services.recon_jobs import store
...
def _no_bare_get_cursor():
    return mock.patch(
        "services.recon_jobs.store.get_cursor",
        side_effect=AssertionError("must use get_cursor_rls"),
    )


class ReconJobsUserFacingRls(unittest.TestCase):
    def test_enqueue_threads_tenant_and_user(self):
        calls, fake = _capture()
        with mock.patch("services.recon_jobs.store.get_cursor_rls", fake), _no_bare_get_cursor():
            store.enqueue("bank", "u1", "ten-1", workspace_client_id=5)
...
class ReconJobsWorkerBypass(unittest.TestCase):
    ...
    def test_finish(self):
        self._assert_bypass(store.finish, "job-1", "bank", "r1")

    def test_set_needs_review(self):
        self._assert_bypass(store.set_needs_review, "job-1", {})

    def test_set_needs_mapping(self):
        self._assert_bypass(store.set_needs_mapping, "job-1", {})
