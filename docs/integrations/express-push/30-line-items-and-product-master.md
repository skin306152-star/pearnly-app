...
#### REFNUM 不是可选装饰(2026-07-31 · 17 套真账镜像 + 报表模板取证)

填充率(镜像 EXP67/68/69 · STCRD 活行 59 799 · 表结构 17/17 都是 45 列、`REFNUM C(15)`):
采购单别 RR 18 168/18 533 = 98.0%、HP 147/147 = 100%,且与 APTRN 单头 REFNUM 同值;
销项单别 IV 0/9 266、HS 0/5 492、HL 0/3 076 全空。**所以采购填、销项留空是照抄 Express
自己的写法**,不是"都填上更整齐"。

取数的是随程序发的库存卡报表(`eng/` `thai/` 下 `STCRD*.RWT` / `STK-CARD.RWT` /
`STOCK001.RWT`,镜像里 39 个模板共 117 处绑定 `STCRD->REFNUM`),单据号列的表达式是:

```
DOC = IIF(STCRD->REFNUM # '  ', STCRD->REFNUM, STCRD->DOCNUM - '-' - STCRD->SEQNUM)
```

这一列空着,库存卡上我们写进去的那一行就只剩内部单号,而同一张报表上所有原生行显示的是
供应商票号 —— 会计要对票就得逐张单点开看。
...
- **拿 APBAL 月桶反推「单据是不是都记对了」之前,先把没有 APMAS 主档的 SUPCOD 剔掉。** 真账里存在**没有主档的余额行**:镜像 17 套 / 8 746 个 APBAL 主键码里有 1 个(69ASIAMI + 68ASIAMI 同一行,`ซ007`)—— APMAS 一行都没有(含软删)、APTRN 一张单都没有,但 19 个桶有钱(RR 10 个月 + HP 2 个月 + PS 付款,且 RR 本年合计 66 189.52 == PS 合计,说明那家供应商的往来是完整且已结清的)。它是**客户侧的数据残骸**(主档与单据被删掉、余额行留下:该书 `ซ001..ซ010` 里独缺 `ซ007`,前后两档建档日期 2012 与 2023),不是我们的模型漏了付款冲销或跨年结转 —— 跨年结转会落 `BEGBAL`,不会散进 12 个月桶。不剔掉它,交叉验证脚本会把它报成「我们对不上」。
