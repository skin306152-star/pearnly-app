...
超管两种发放方式:
  ...
  ② 账号不存在 → 任意用户名直接建号(自由邀请制,不强制邮箱;是邮箱则顺手落
    users.email)。密码可超管自定义或留空随机,只在本次响应回显
    (库里只存 bcrypt,不落日志/审计 details),建完自动进名单。

建号(invite)和重置(reset-password)都可选传 password:超管指定就原样用(超管口
不设强度闸 · 2026-07-16 拍板,面向转交客户的临时凭据,客户可自行改密),留空落回
随机一次性密码 —— 见 _resolve_password。
...
from services.auth.entrance_store import AI, grant_entrance_safe
...
async def pearnly_ai_invite(request: Request, body: InviteBody):
    ...
    if existing:
        ...
        grant_entrance_safe(
            AI, existing.get("tenant_id"), str(admin.get("id")) if admin else None, context="ai"
        )
        _log_op(
            request,
            admin,
    ...
    grant_entrance_safe(AI, tenant_id, str(admin.get("id")) if admin else None, context="ai")
    _log_op(
        request,
        admin,
