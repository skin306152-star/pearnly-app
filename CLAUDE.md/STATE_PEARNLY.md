...
## 🎯 状态卡(2026-06-01 · **整顿主线 C3 进行中** · R15~R18 抽 6 个 modal/drawer · home.html 1495→1247 上线 prod 11850057)

- **模式**:整顿封锁期 · **整顿主线 C3 进行中**(Zihao 2026-06-01 拍板:DMS 全链路收尾闭环 → 回整顿)。**当前 task = C3 拆 home.html → <1000**:本窗口 R15~R18 后 **1247**(还差 ~247)。下个候选:`billing-modal-mask`~38〔⚠️**全仓 grep 零 JS 消费者(只 i18n+markup)= credits 取代 BYO-key 后的死 markup**·应直接删而非抽·涉历史判断留 Zihao 拍/或确认后删〕/ `email-modal`~155〔牵涉 `line-email-modal.js` 🔴高敏·跳过〕/ `confirm-modal`+`pearnly-confirm-modal`(showConfirm 通用·可能 eval 早绑·谨慎)/ page-ocr(~150·识别主页高耦合·留最后)。**app shell(顶栏~128/侧栏~143)boot 关键勿轻动**。
...
- **C3 进度封存**:home.html 1730→**1247**(R13 `#page-history` + R14 `#onboarding-modal` + **R15 `#bank-cand-drawer`→`bank-cand-drawer-html.js` `277b377`(-22)** + **R16 `#client-modal-mask`+`#wsclient-modal-mask`→`modal-client-edit.js` `bf42884`(-98)** + **R17 `#camera-tips-modal`+`#bank-client-picker-modal`→2 模块 `d773424`(-51)** + **R18 `#endpoint-modal`→`endpoint-modal-html.js` `6264f2a`(-77·⚠️erp-integration 无守卫 IIFE·import 严置其前)**·均字节等价〔脚本提取+校验模板===git HEAD inner〕+prod E2E 验〔R16 真浏览器开弹窗·R17/R18 查空壳 wbInjected+子元素在场+IIFE 未崩〕)。范式见记忆 [[wb-src-home-c3-split-playbook]](留空壳 + `*-html.js`/`modal-*.js` eval 期注入 inner + 子树初译 + import 置消费模块前;消费模块 load()/_bindOnce/DOMContentLoaded 守卫绑定→eval 注入恒早于首次绑定·非竞态)。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。
...
- **最后 commit**:`6264f2a`(C3 R18 抽 ERP 端点配置弹窗 inner→运行期注入 · REFACTOR-WB-C3 · bump `?v=` 11850056→11850057)· `git log --oneline -8` 查最新。**均已 push 上线·prod 11850057 健康**。**整顿轮 cache-bust:仍 bump `?v=` 保缓存正确,但不重写 release_notes(Zihao 2026-06-01:纯重构忽视新版本横幅·见记忆 [[refactor-skip-version-banner]])**。home.js 拆迁全收官 33768→0(范式 [[home-js-batch-split-bridge]])。
