...
_TAX_NO_NUMBERS = {
    "zh": "{client} · {period}:还没算到税额(工单在{status},当前 {step})。等这一步跑完再问。",
    "th": "{client} · งวด {period}: ยังคำนวณภาษีไม่ถึง (งานอยู่ในสถานะ{status} ขั้น {step}) รอขั้นนี้เสร็จแล้วถามใหม่",
}
...
_PUSH_BAD = {
    "zh": "推过 {n} 次,最后一次没成({status}{code})。去「推送日志」看这条的详情。",
    "th": "ส่งไปแล้ว {n} ครั้ง ครั้งล่าสุดไม่สำเร็จ ({status}{code}) ดูรายละเอียดที่「บันทึกการส่ง」",
}
_PUSH_KIND_HINT = {
