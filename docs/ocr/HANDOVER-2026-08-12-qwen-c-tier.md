...
> **2026-08-12 午间清账**:下面账本 22 条中 A1/A2、B3-B6、C7-C22 全部落地
> (A1 金标复测两轮 vat/total 零漂 + ABB 全对 + 贷记单合成加考 2/2 → qwen 已移出
> PARTIAL_MODES,全局键解锁,真切全局待 Zihao 后台拍板;C12/18/19/20 四条见当日
> FE-2 提交)。入口普查结论:登录态入口全部吃灰度;LINE webhook/独立 worker 无登录态
> 回落全局档 = 设计内,全局切 qwen 后自然覆盖。本文其余内容保留作历史语境。
