# -*- coding: utf-8 -*-
"""
Pearnly · ERP 推送 API 路由聚合 + 连接测试/列表(向导)路由(REFACTOR-B1 抽出 · R18 再拆)

本模块 = 连接测试 / 端点健康检查 / 客户·商品列表(向导 Step-3)+ listing 缓存,
并聚合(include_router)端点 CRUD(erp_endpoints_routes)与推送/日志/重试/批量
(erp_push_log_routes)两个子路由 → 对外仍是单一 `router`,app.py 的
`app.include_router(erp_router)` 不变。
...
依赖:db.* / erp_push as _erp / auth.get_current_user_from_request /
      route_helpers(_record_500 等)/ erp_routes_access._check_push_access。
"""
...
# 契约保留:erp_routes._record_500 is route_helpers._record_500(connection 组不直接用)
...
# ============================================================
# REFACTOR-WA-B1 R18 · 聚合端点 CRUD + 推送/日志子路由(app.py 单一 include 不变)·
# 同时 re-export 各组 model / helper 回 erp_routes 命名空间(契约/调用点零改)。
# ============================================================
from erp_endpoints_routes import (  # noqa: E402,F401
...
from erp_push_log_routes import (  # noqa: E402,F401
