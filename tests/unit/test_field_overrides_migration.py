...
class FieldOverridesMigrationContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.service_path = os.path.join(ROOT, "services", "db_migrations", "field_overrides.py")
    ...
    def test_upgrade_adds_4_columns(self):
        ...
        if m_tables:
            ...
            for table in self.EXPECTED_TABLES:
                self.assertIn(table, tables_in_list, f"_TABLES list must include {table}")
        else:
            ...
            for table in self.EXPECTED_TABLES:
                self.assertIn(table, m.group(1), f"upgrade() must add field_overrides to {table}")
    ...
    def test_ensure_function_defined_with_all_4_tables(self):
        ...
        self.assertIn(
            "def ensure_field_overrides_columns",
            self.service_src,
