...
2. **wave4**:~~`erp_*_mappings`~~ ✅(§7.7)、~~`erp_endpoints`/`erp_push_logs`~~ ✅(§7.8·JOIN 富化难点已解)、~~`import_template_mappings`~~ ✅(§7.9 `05dfd10b`)。剩 **legacy-无-DDL-钩子孤儿**(同类·都 0 暴露·守卫已 DISABLE·proper enroll 需先决定重建 creator 或纯 DB 层处理):`erp_oauth_*`/`mrerp_credentials`/`erp_connectors`(代码树无 CREATE 也无访问点)、`email_ingest_accounts`/`email_ingest_logs`/`email_ingest_seen_uids`(有 store 但无 CREATE 钩子·加新 ensure_ 触 new_debt;还有 worker 跨租户路径 + seen_uids 仅 account_id 需 via-parent;prod 0 行)、`excel_templates`(无任何应用读写访问点·只超管 bypass 级联删)。
