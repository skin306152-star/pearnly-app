...
| 2 home.js | 🟡 抽取中 | home.js **22970 行** | home.js **11898 行** | 第三十八会话(2026-05-27)E2E 网绿后**开抽 §13🟢 表 · 25 块已抽**(每块独立 commit / 每轮一 commit · 每轮真站点 E2E 10/10 全绿 · CI 绿):**R1 串行**(逐块验配方)recon-center `4fa262e` / assign-clients `e2a9d43` / access-log `6ff538b` / notifications `cad5f1b`(-979);**R2 并行 agent**(转批量)recon-batch `f3d4cbc` / welcome-wizard `c9fb2a0` / ai-balance `ea71f7f` / archive-settings `10cb6d6`(-1160);**R3 并行 agent ×5**:big-batch-progress `696136c` / erp-xero `a6d5652` / report-templates `c603834` / folder-watcher `eff5f79` / email-ingest `3bd7fbc`(-2317)。第三十九会话 **R4 并行 5 块**:bank-recon / clients / exceptions / topbar-avatar / recon-collapse(`b9f6e1f` · -4030);**R5 并行 3 块**:recon-subtab-settings / excel-formula-recon / gl-vat-recon(`962105e` · -1554 · 含守门测试 test_brv2 跟随代码搬家更新)。**第四十会话 R6 并行 4 块**(ERP 尾区干净单 IIFE):erp-mappings / unified-push / erp-map-advanced / erp-onboard(`8358366` · -1031 · 12929→11898)· 守门全绿(pytest 1131)· 生产 E2E 10/10。共 **-11072 行**(22970→11898 · 25 块)。配方见 §13。**继续抽 §13🟢 余下块** |
| 3 css/html | 🟡 css 拆解中(C2) | css **16673 行** | css **15365 行** | 第三十九会话(2026-05-27)**home.css 拆解窗口**(REFACTOR-C2 · 与 home.js 窗口并行碰不同文件)。安全模型 = 自顶向下 peel 区块到 `static/home-*.css` + `home.html` 按序加 `<link>`(新文件→home.css 末)+ **杀手级字节校验**(按 link 序拼接 ≡ 基线 `home.css` · 0 差异 = 级联零变化)。**R1**(`98f184c`):`home-01-base.css`(设计令牌 `:root`+reset+顶栏/brand · 含 BOM)· 校验 PASS · 已 push + 生产实测服务正确(新文件 200 · home.css 尾巴生效非缓存)。**R2 抽 4 块**:`home-02-switcher`(客户/workspace 切换器)/ `home-03-layout`(侧栏+主区+卡片)/ `home-04-results`(按钮+提示条+4格统计+结果表格)/ `home-05-overlays`(抽屉+模态+套餐对比+占位+Toast)· 6 parts PASS。**R3 抽 5 块**:`home-06-responsive-rd`(响应式+RD税务按钮/对比弹窗)/ `home-07-history-automation`(识别结果统计条+历史页+自动化页+模态对话框)/ `home-08-pagehead-toast`(页头卡片/清爽版+Toast+内联错误+抽屉归档名)/ `home-09-badges-locks`(Plus角标+新视觉+锁样式)/ `home-10-push-logs`(推送日志区/增强+ERP凭证弹窗)· 校验 PASS 482214 bytes byte-identical(**11 parts**)。**累计抽 10 块 · home.css 16673→13433 行(-3240)**。配方:`node css_multipeel.mjs` 单遍多切(原文件行坐标 · 保 CRLF/BOM · 内置 recombine 自检)→ `css_verify.mjs` 按 home.html link 序拼接 ≡ 基线。**下一窗口续抽:re-grep home.css 顶部区块 → 选边界 → multipeel → home.html 按序插 link(`?v=` +1)→ 杀手级校验必 PASS → commit `· REFACTOR-C2` → 单独 push。基线金标准 `$TEMP/home_css_orig.css` 不可变(若重开窗口须重存:工作树 home.css 已是尾巴,基线得从某早期 commit `git show <旧commit>:home.css` 取并核对 482214 bytes)。** |
| 4 i18n/文档/抛光 | ⚪ | — | — | — |
...
>
> **⚠️ 进度更新(第三十八/九/四十会话 · 2026-05-27)**:已抽 **25 块** · home.js **22970 → 11898 行**(累计 **-11072**)· R1-R3 抽 13 块,**R4 并行 5 块**:bank-recon / clients / exceptions / topbar-avatar / recon-collapse(`b9f6e1f`),**R5 并行 3 块**:recon-subtab-settings / excel-formula-recon / gl-vat-recon(`962105e`),**R6 并行 4 块**(ERP 尾区):erp-mappings(11805-12280)/ unified-push(统一推送按钮·12285-12684)/ erp-map-advanced(高级 toggle·12693-12740)/ erp-onboard(对接引导·12749-12855)(`8358366` · -1031)· 均守门全绿(pytest 1131)· 生产 E2E 10/10。**②表行号已严重漂移 · 抽前必 re-grep**。
> **R6 后当前 home.js IIFE 地图(11898 行 · 仅供起点 · 必 re-grep)**:🔴plans-plg-line ~8645-11334 / 🔴password ~11342-11598 / 🔴line-email-modal ~11629-11753 / **🔴session-heartbeat ~11838(`_sessionCheck` · 末尾)**;`window.pearnlyConfirm`(L11761)= 共享工具留 home.js 别动。
> **剩 safe 候选(第四十会话已验证边界 · 下窗口可直接抽)**:① **recon-job-poll(6765-6830 · 定义 `window._reconPollJob`/`_reconProgressText`)+ bank-recon-v2(6832-8115 · ~1284行 · 用 `window._reconPollJob` · 暴露 `_loadBankReconV2Panel`/`_bankReconV2Init`/`_brv2LoadHistory`,被 recon-center/recon-batch 经 `window.` 运行期调)** —— 这对只经 `window.` 耦合、运行期取用、顺序无关,**可一并抽两个 module**(已派 agent 验证边界干净 · recon-job-poll.js 已建过 · 本轮停手已删,下窗口重派或直接抽)。② **admin-misc(8364-8632 · 两 IIFE:顶栏「管理」下拉 8368-8402 + 成本面板 8408-8632)** —— ⚠️ 成本面板暴露 `window.loadAdminCostPage`,被 `ai-balance.js`(已抽)用 `setTimeout(200ms)` 兜底包裹 → **admin-misc 的 import 须排在 main.js 的 ai-balance import 之前**(确定性,不靠 200ms 兜底)。(admin-misc.js 本轮已建过、停手已删 · agent 报告 8566 行模板字串去过一个行尾空格 → 下窗口重抽时核对或用 prettier 抹平。)③ **erp-endpoints-logs 区(5464-6760)= 顶层函数群 + 内嵌 IIFE(`_bindErpBatchButtonsDirect` 5885 / `initAutomationPage` 6642)** · 夹在 top-level 函数群里 · 归"较难/地板"。④ **⚠️ 8120-8362 = LINE Bot 绑定面板**(`_codeTimer`/`_pollTimer` 绑定码轮询 · 账号绑定相关)→ **当敏感跳过**。⑤ early 区 banner 分隔函数群 / team(含员工改密 borderline)· 较难。
