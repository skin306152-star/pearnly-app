...
_I18N_EXPORT: Dict[str, Dict[str, str]] = {
    ...
    "col_source_stmt": {"th": "ไฟล์บัญชี", "en": "Stmt File", "zh": "对账单文件", "ja": "明細ファイル"},
    ...
    "lbl_gl_debit_only": {"th": "GL เดบิตเท่านั้น (ไม่มีในบัญชีธนาคาร)", "en": "GL Debit Only (not in stmt)", "zh": "仅 GL 有借方（对账单中缺失）", "ja": "GLのみ借方"},
    "lbl_gl_credit_only": {"th": "GL เครดิตเท่านั้น (ไม่มีในบัญชีธนาคาร)", "en": "GL Credit Only (not in stmt)", "zh": "仅 GL 有贷方（对账单中缺失）", "ja": "GLのみ貸方"},
    "lbl_stmt_wd_only": {"th": "รายการถอนใน Statement เท่านั้น (ไม่มีใน GL)", "en": "Stmt Withdrawal Only (not in GL)", "zh": "仅对账单有提款（GL 中缺失）", "ja": "明細のみ出金"},
    "lbl_stmt_dep_only": {"th": "รายการฝากใน Statement เท่านั้น (ไม่มีใน GL)", "en": "Stmt Deposit Only (not in GL)", "zh": "仅对账单有存款（GL 中缺失）", "ja": "明細のみ入金"},
    ...
    # Status labels
    "st_gl_debit_only": {"th": "GL เดบิตเท่านั้น", "en": "GL Debit Only", "zh": "仅 GL 借方", "ja": "GLのみ借方"},
    "st_gl_credit_only": {"th": "GL เครดิตเท่านั้น", "en": "GL Credit Only", "zh": "仅 GL 贷方", "ja": "GLのみ貸方"},
    "st_stmt_wd_only": {"th": "รายการถอนเท่านั้น", "en": "Stmt Withdrawal Only", "zh": "仅对账单提款", "ja": "明細のみ出金"},
    "st_stmt_dep_only": {"th": "รายการฝากเท่านั้น", "en": "Stmt Deposit Only", "zh": "仅对账单存款", "ja": "明細のみ入金"},
    ...
    "fi_stmt_type":     {"th": "Statement ธนาคาร", "en": "Bank Stmt", "zh": "银行对账单", "ja": "銀行明細"},
...
_USAGE_BLOCKS: Dict[str, List[Tuple[str, bool]]] = {
    ...
    "en": [
        ...
        ("• 'Match Results'        Matched + Unmatched GL + Unmatched Stmt, distinguished by Status column", False),
    ...
    "ja": [
        ...
        ("", False),
        ("シート構成 (全 4 シート):", True),
        ("• 「サマリー」               本シート · ファイル情報、照合公式、件数、使い方", False),
