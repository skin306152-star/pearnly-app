...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "v118.33.13.1 更新:\n• 修复 Excel 汇总区列宽冲突导致的数字溢出（##）问题，6 张卡片 + 5 个运算符正确对齐\n• 修复期初余额行（ยอดยกมา 等）被误判为余额校验失败的问题，开户行不再参与校验\n• 新增 OCR 缓存：相同 PDF 重复上传秒级返回（不再走 Gemini）\n• 切换至更快的 gemini-2.5-flash-lite 模型，单页提速约 30-50%\n• 使用说明 sheet 去掉了第三方引擎名称，统一为「AI OCR」",
            "en": "v118.33.13.1 updates:\n• Fix Excel Summary column-width conflict causing ## overflow; cards + operators now properly aligned\n• Fix opening-balance rows (ยอดยกมา etc.) being wrongly flagged as balance-check failures; opening rows are now excluded from arithmetic verification\n• New OCR result cache: re-uploading the same PDF returns instantly (no Gemini call)\n• Switched to faster gemini-2.5-flash-lite model — ~30-50% faster per page\n• Usage sheet no longer mentions the specific third-party engine name; uses 'AI OCR' generically",
            "th": "v118.33.13.1 อัปเดต:\n• แก้ปัญหาความกว้างคอลัมน์ในชีต 'สรุป' ของ Excel ที่ทำให้ตัวเลขขึ้น ## การ์ดและเครื่องหมายจัดเรียงถูกต้องแล้ว\n• แก้แถวยอดยกมาถูกแจ้งว่าตรวจยอดไม่ผ่าน บรรทัดยอดยกมาจะไม่เข้าสู่การตรวจสอบทางคณิตศาสตร์อีกต่อไป\n• เพิ่มแคชผล OCR: หากอัปโหลด PDF เดียวกันซ้ำ จะคืนค่าทันที (ไม่เรียก AI ใหม่)\n• เปลี่ยนเป็นโมเดล gemini-2.5-flash-lite ที่เร็วกว่า — เร็วขึ้นประมาณ 30-50% ต่อหน้า\n• ชีตวิธีใช้งานเปลี่ยนชื่อเอนจินจาก Gemini เป็น 'AI OCR' โดยทั่วไป",
            "ja": "v118.33.13.1 更新:\n• Excelサマリーシートの列幅衝突を修正、数字の##溢れを解消、カードと演算子が正しく整列\n• 期首残高行(ยอดยกมา 等)が残高検証エラーと誤判定される問題を修正、開設残高行は検証対象から除外\n• OCR結果キャッシュ追加: 同一PDFの再アップロードは即時返却(API再呼出なし)\n• より高速な gemini-2.5-flash-lite モデルへ切替 — 1ページあたり約30-50%高速化\n• 使い方シートで具体的なエンジン名を削除、汎用的に「AI OCR」表記に統一"
        }
    }
