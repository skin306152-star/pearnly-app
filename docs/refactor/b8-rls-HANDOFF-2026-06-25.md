...
2. **wave4**:~~`erp_*_mappings`~~ ✅(§7.7 `3f337517`)、~~`erp_endpoints`/`erp_push_logs`~~ ✅(§7.8 `f4447662`·含 JOIN 富化难点已解)。剩 `erp_oauth_*`、`email_ingest_*`、`import_template_*`。`erp_oauth_*`/`mrerp_credentials`/`erp_connectors` 当前代码树无 CREATE 也无访问点(纯 prod 孤儿,需先决定是否重建 creator 或纯 DB 层 disable)。
