# -*- coding: utf-8 -*-
"""admin_daily_routes 契约(daily_finance 闸发放侧 · 照 pearnly-ai 范式)。

锁定:4 路由 path+method 契约;app include_router 挂上;全路由复用
route_helpers._require_super_admin 单一来源(非超管一律 403)。业务层重点钉死
tenant-first 判据(core/feature_flags.daily_enabled_for 同一口径:有 tenant_id 写
tenant_id,没有才退回 user_id——写反了闸永远判不中且查不出根因)、一次性密码只在
响应回显一次(不进 _log_op details)。

reset-password 额外钉死:仅限 allowlist 名单内 subject(不复活已被砍掉的通用
超管改密);自定义密码原样生效(超管口不设强度闸),留空落回随机一次性密码。
"""
...
from routes import admin_daily_routes
from routes.admin_daily_routes import router
...
class RoutesContractTests(unittest.TestCase):
    def test_router_registers_expected_routes(self):
        ...
        self.assertEqual(
            got,
            {
                ("GET", "/api/admin/daily/overview"),
                ("POST", "/api/admin/daily/invite"),
                ("POST", "/api/admin/daily/revoke"),
                ("POST", "/api/admin/daily/reset-password"),
            },
        )
    ...
    def test_app_includes_router(self):
        ...
        self.assertIn("/api/admin/daily/overview", paths)
        self.assertIn("/api/admin/daily/invite", paths)
        self.assertIn("/api/admin/daily/revoke", paths)
        self.assertIn("/api/admin/daily/reset-password", paths)

    def test_super_admin_guard_single_source(self):
        self.assertIs(
            admin_daily_routes._require_super_admin,
            route_helpers._require_super_admin,
        )

    def test_grant_entrance_uses_daily_entrance(self):
        """发放侧 grant 的是 DAILY 入口(daily_enabled_for 登录准入的同源判据)。"""
        from services.auth.entrance_store import DAILY

        self.assertEqual(DAILY, "daily")
        self.assertIs(admin_daily_routes.DAILY, DAILY)
        self.assertEqual(admin_daily_routes.DAILY_KEY, "daily_finance")
...
class GuardEnforcedTests(unittest.TestCase):
    ...
    def test_invite_non_super_403(self):
        with self._as_non_super():
            r = self.client.post("/api/admin/daily/invite", json={"username_or_email": "x@y.com"})
        self.assertEqual(r.status_code, 403)

    def test_revoke_non_super_403(self):
        with self._as_non_super():
            r = self.client.post("/api/admin/daily/revoke", json={"subject_id": "t1"})
        self.assertEqual(r.status_code, 403)

    def test_reset_password_non_super_403(self):
        with self._as_non_super():
            r = self.client.post("/api/admin/daily/reset-password", json={"subject_id": "t1"})
        self.assertEqual(r.status_code, 403)
...
class InviteExistingUserTests(unittest.TestCase):
    ...
    def test_invite_existing_user_with_tenant_writes_tenant_id(self):
        ...
        m_add.assert_called_once_with("daily_finance", "tenant-9")
        self.assertEqual(m_grant.call_args.args[0], "daily")
    ...
    def test_invite_existing_user_without_tenant_falls_back_to_user_id(self):
        ...
        with (
            ...
            r = self.client.post("/api/admin/daily/invite", json={"username_or_email": "solo"})
        self.assertEqual(r.status_code, 200)
        body = r.json()
...
class InviteCreateAccountTests(unittest.TestCase):
    ...
    def test_invite_unknown_email_creates_account_and_reveals_password_once(self):
        ...
        m_add.assert_called_once_with("daily_finance", "new-tenant")
        details_str = str(m_log.call_args.kwargs.get("details") or "")
    ...
    def test_invite_unknown_plain_username_creates_account(self):
        with (
            ...
            mock.patch.object(admin_daily_routes.db, "get_cursor") as m_cur,
            mock.patch.object(admin_daily_routes, "grant_entrance_safe"),
            mock.patch.object(admin_daily_routes, "_log_op"),
        ):
            r = self.client.post(
                "/api/admin/daily/invite", json={"username_or_email": "plainusername"}
            )
        self.assertEqual(r.status_code, 200)
        body = r.json()
        self.assertTrue(body["created_account"])
    ...
    def test_invite_username_exists_race_returns_409(self):
        with (
            ...
            r = self.client.post(
                "/api/admin/daily/invite", json={"username_or_email": "race@example.com"}
            )
...
class RevokeTests(unittest.TestCase):
    ...
    def test_revoke_missing_subject_400(self):
        r = self.client.post("/api/admin/daily/revoke", json={"subject_id": "  "})
        self.assertEqual(r.status_code, 400)


class ResetPasswordTests(unittest.TestCase):
    """/daily 邀请账号密码重置 —— 严格闸在 allowlist 名单内,不是通用改密能力复活。"""
    ...
    def test_reset_password_user_subject_without_tenant_200(self):
        ...
        with (
            ...
            mock.patch.object(admin_daily_routes.db, "reset_user_password", return_value=True),
            mock.patch.object(admin_daily_routes, "_log_op"),
        ):
            r = self.client.post(
                "/api/admin/daily/reset-password", json={"subject_id": "user-orphan"}
            )
        self.assertEqual(r.status_code, 200)
    ...
    def test_reset_password_custom_password_passed_through(self):
        ...
        with (
            ...
            r = self.client.post(
                "/api/admin/daily/reset-password",
