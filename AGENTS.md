...
> **下个 task = 模块化深化继续拆 >500**(✅`bank_recon_v2` 6745→422 + ✅`gl_vat_reconciler` 1423→72 + ✅`recon_routes` 2000→460(含计费 3-way) + ✅`vat_excel_export` 1960→55(含 build_excel 624 单函数分解·cell-等价验证)收官 + ✅`bank_recon_excel` 1397→380 + ✅`mrerp_xlsx_generator` 1336→381(拆 5 leaf·monkeypatch 双目标:函数+常量)收官 / `services/erp/mrerp_customer_sync`1324 → `report_engine`1026 → ERP 周边 → 报表 → 前端 `src/home/*`)。**完整清单见 STATE 状态卡**。**目录重组(86 root .py→包)= 最后做**(文件全 <500 后)。
