...
# 相对期间同义词。会计口语里「期」与「月」是一回事(月度义务一期一月),两套说法都得认:
# 首版只认「这个月/เดือนนี้」,管家自带的四个 chips 有两个说的是「本期/งวดนี้」,解不出就被
# 当成没给期间去追问「哪一期?」—— 点了按钮反被反问,是死路。英文不收 "this period":
# 判卷金标把它列为「解不出」的反证题(期间要么明确要么诚实认不会,不靠英文泛化猜)。
_LAST_MONTH_WORDS = (
    "เดือนที่แล้ว",
    "งวดที่แล้ว",
    "上个月",
