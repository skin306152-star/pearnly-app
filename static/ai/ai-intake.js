...
(function () {
    ...
    function openForm() {
        S.formOpen = true;
        if (!S.formLocked) S.formErr = false;
        if (!S.formLocked) S.formErrKey = null;
        render();
    }
    ...
    function submitForm() {
        if (S.submitting || S.formLocked) return;
        ...
        if (!payload) {
            S.formErr = true;
            S.formErrKey = 'intake_form_invalid';
            render();
            return;
        }
        var session = S;
        ...
        S.api
            ...
            .catch(function (err) {
                ...
                S.submitting = false;
                S.formErr = true;
                S.formErrKey = AI.intakeRender.resolveIntakeErrKey(err);
                S.formLocked =
                    S.formErrKey === 'intake_err_entrance' ||
                    S.formErrKey === 'intake_err_forbidden';
                if (S.formErrKey === 'intake_err_session') {
                    AI.expireSession(S.formErrKey);
                    return;
                }
                render();
            });
