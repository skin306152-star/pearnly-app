// ============================================================
// REFACTOR-WB-C3 (2026-05-29) · page-exceptions 静态骨架从 home.html 抽出 · 运行期模板注入
//
// home.html 的 <section id="page-exceptions"> 现为空壳 · 本模块(Vite bundle · defer)eval 时把骨架
// innerHTML 注入回去。注入早于任何路由(用户点「异常栏」远晚于模块加载)· 且 home.js / exceptions.js
// 均不在 parse/eval 期直接 getElementById 异常栏元素(已核 · 全部走 loadExceptionsPage 路由期 / 事件委托)。
//
// i18n:boot applyLang(home.js · 同步 · 先于本模块)只译 boot 时在场的 [data-i18n] · 注入内容会漏初译 →
// 注入后立即对子树补译(读 window.I18N / window._currentLang · 逻辑镜像 applyLang)。后续切语言由 applyLang
// 全文 querySelectorAll 扫描覆盖(注入内容已在 DOM)。verbatim 搬迁骨架 · 0 改其结构。
// ============================================================
(function () {
    'use strict';
    const sec = document.getElementById('page-exceptions');
...
`;
    ...
    // 注入子树初译(boot applyLang 已过 · 补译当前语言;切语言由 applyLang 全文扫描覆盖)
    try {
