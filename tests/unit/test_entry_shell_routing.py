...
①module-nav.ts 收服务器权威 entry + 保留 localStorage 回落 + pos 模块护栏 ②login-url.ts
entry 优先判据 ③pos-login.html 写 'pos' 记号 ④主站登录源(landing.js)写 'main' 记号。
"""
...
class ModuleNavEntryRoutingTests(unittest.TestCase):
    def test_uses_server_authoritative_entry(self):
        # ★ 各是各的:壳的权威 entry 来自服务器(/api/me/modules 的 data.entry),不再猜
        # localStorage。apply 收第三参 entry,window._entry 以它为主(串壳根治的核心)。
    ...
    def test_reads_pearnly_entry_from_localstorage(self):
        # localStorage 记号降级为回落/pre-auth 提示(旧后端/旧 token 缺 data.entry 时兜底)。
        text = _read("src/home/module-nav.ts")
