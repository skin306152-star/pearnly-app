...
> **当前数字(master 全绿·工作树干净)**:home.js **6955**(第四十二会话抽 admin 后整删老 admin 布局 · 8941→6955)· app.py 4839 · db.py 3371 · home.css 0 · home.html **6428**(删 admin 两段 · 6658→6428)· src/home 模块 **35** 个 · 单测 **1516** · E2E **10**。`gh.exe` 路径 `C:\Program Files\GitHub CLI\gh.exe` · repo `skin306152-star/pearnly-app`。
> **🅱 计费模型已变(重要)**:产品现在**只有「充值+按量扣费」** · 老月订/年订/体验(trial)/免费(free)套餐**全下线** → home.js+后端大量老套餐 UI/PLG 试用闸/升级弹窗 = **死码可清**(I2)。
> **下一个任务 · ⚠️ 前半套餐代码「暂不能删」的铁证(第四十二会话查实)**:大 IIFE 现**只剩前半**(home.js · 套餐/试用横幅 + LINE modal + `window.fetch` 429 拦截 + `loadPlan` 60s 轮询)。交接曾说「计费改只剩充值 → 前半大半死码」——**经查这是 aspirational,后端迁移没做完,前半其实是活码**:① `/api/me/plan` **仍存在且活**(`auth_signup.py:1539` · 返 plan/trial_expires_at/plan_expires_at)② 后端仍满是 trial/free/plan 逻辑(`auth_signup.py:1316/1511`、`app.py:1511/1602`)③ `renderTrialBanner` 对 plan∈{trial,monthly,yearly} 真显 banner · trial/free 用户会看到。**所以删前半 = 清错有用的东西(违 Zihao 铁令)→ 没删,正确。** 真正要清套餐死码,得**前后端一起做**:先确认后端真把 trial/free/月订/年订 下线(改 `/api/me/plan` + `_get_plan` + 计费逻辑),再同步删前端 banner/plan 代码。**这是一个独立的「计费模型迁移收尾」任务,不是纯前端 dead-code,需 Zihao 拍板后端迁移范围。** 唯一明确死的小块:429 拦截里 `detail.code.startsWith('v109.')` 分支(OCR 超额现返 `insufficient_balance` 非 v109. 码 · `app.py:2050`)——但它和 LINE modal 缠在 fetch hijack 里,单拆收益小风险中,留迁移收尾时一并处理。
...
> ════════════════════════════════════════════════════════════
> **【第四十二会话 · 交接 · 2026-05-27】抽+整删老 home.html admin 布局死码(用户管理+成本+顶栏下拉)· home.js 8941→6955 · home.html 6658→6428 · 删 3 模块+1css(-4566 行)· 双账号实测 + CI 绿 · 前半套餐查实「活码暂不删」**
> ════════════════════════════════════════════════════════════
...
> - **⛔ Part B(前半套餐代码)查实「暂不能删」**:见上方「下一个任务」铁证 —— `/api/me/plan` 活 + 后端 trial/free/plan 逻辑全在 → 前半是活码不是死码,删了会清错有用的东西。**正确做法 = 等后端「计费迁移收尾」(下线 trial/free)再前后端一起删,需 Zihao 拍板。** 本会话据此**停手未删前半**(符合「不清错有用」)。
>
> ════════════════════════════════════════════════════════════
