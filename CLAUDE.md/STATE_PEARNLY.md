...
## 🎯 状态卡（2026-07-01 · **对话 Agent M1（WP1~WP5）全线收官 · 灰度上线 skin canary · 待 Zihao 拍板放量**）

> 本卡是当下唯一可信的"现在在哪"。数字只信 `python scripts/refactor_progress.py`;历史往【本段以下】追加,别改这段以上。

- **★ 当前主线 · 对话 Agent M1（WP1~WP5 全 ✅ · master `08d262bf` · prod 200）**:通用大模型套一层"只懂 Pearnly、参数卡住按真实账况反问"的壳,LINE 用大白话记账/查询。WP1 插座核心(`962008fb`·`services/agent/` 9 文件 + 5 只读工具)/ WP2 钥匙闸(`260f5ffb`·`services/platform_settings`+`core/feature_flags`·默认关 fail-closed)/ WP3 对话文案(`32888fcc`·agent.* 29 键四语)/ WP4 只读工具(并入 WP1)/ WP5 接 LINE(`83df5040`·钥匙闸分流 + 能力只增不减 · 异常即 defer 回旧路)。**灰度**:enabled=True · rollout=allowlist=[skin] · 真机验收通过(skin 查余额返真实 58.02฿ · 泰文)。铁律:能力只增不减 / 参数编造值拦死 / 钥匙闸 fail-closed 一秒回退无感。权威文档 `docs/agent/MASTER-PLAN.md`。
...
- **✅ 2026-07-01 补:M2 小债已收口上线**(`ac7e65a8`·prod `11851120`):三键回填富版(`history`=列表预览+真实前 3 条 / `history_summary`=本月单据数·合计฿·分类分布 / `usage`=页数+本月单据数)· 新数据层 `services/ocr_history/agent_overview.month_overview`(本月 Asia/Bangkok 聚合·可见性抽 `list_status.owner_visibility_where` 单一事实源·走 RLS)· agent_i18n↔i18n-data.js 两侧模板对齐(key parity 守)· "几张"路由靠 manifest 描述+brain 消歧规则修正(**LLM 路由效果待 skin 真机确认**)· 全量 5265 单测绿。**注:这只是里程碑表标的"小债"。真 M2「换脑」分两半:**
  - **① 换 qwen3.5 便宜脑(省钱半边)· 底座已上线**(`16b83bc8`):网关 `transport` 加 per-call `backend=` 覆盖 + `brain` 读 `AGENT_BRAIN_BACKEND`(默认关=现状 Gemini·零变化)。**做到了"只切 Agent 路由、OCR 仍 Gemini"**(现全局 `OCR_LLM_BACKEND` 共用做不到分开)。fail-safe:端点不可用→defer 回旧路。**★仍缺 qwen 端点(L40S 已删)→ Zihao 拍板:自托管 GPU vs 托管 OpenAI 兼容 API(qwen3.5-35B-A3B 已 Phase0 验 18/18)· 立起来 + 设 env + skin 金丝雀验路由质量才真省到钱。**
  - **② 工具大脑替旧 dispatcher(记账/撤销/改错入工具)· 未做**:碰 LINE 记账主路径 + 与 M3 写工具重叠 → 建议随 M3 一起做,单独报方案。
