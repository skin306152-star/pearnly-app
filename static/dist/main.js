...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){const e=document.getElementById("archive-token-modal");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        ...
        `}function D(){const I=document.getElementById("btn-email-bind");I&&I.addEventListener("click",()=>m("new"));const P=document.getElementById("btn-email-edit");P&&P.addEventListener("click",()=>m("edit"));const U=document.getElementById("btn-email-unbind");U&&U.addEventListener("click",T);const G=document.getElementById("btn-email-trigger");G&&G.addEventListener("click",_);const N=document.getElementById("email-enabled-toggle");N&&N.addEventListener("change",g);const Q=document.getElementById("email-modal-close");Q&&Q.addEventListener("click",c);const oe=document.getElementById("btn-email-modal-cancel");oe&&oe.addEventListener("click",c);const M=document.getElementById("btn-email-modal-test");M&&M.addEventListener("click",L);const x=document.getElementById("btn-email-modal-save");x&&x.addEventListener("click",A);const b=document.getElementById("email-preset");b&&b.addEventListener("change",Y=>u(Y.target.value));const q=document.getElementById("email-address");q&&!q.dataset.autoBound&&(q.dataset.autoBound="1",q.addEventListener("blur",Y=>w((Y.target.value||"").trim())),q.addEventListener("input",Y=>{const Z=(Y.target.value||"").trim();Z.includes("@")&&Z.split("@")[1].includes(".")&&w(Z)}));const z=document.getElementById("btn-email-refresh-logs");z&&z.addEventListener("click",()=>{z.classList.add("spinning"),setTimeout(()=>z.classList.remove("spinning"),600),B()})}D(),window._loadEmailIngestPanel=i,window._rerenderEmailIngest=function(){if(!o)return;r();const I=document.getElementById("email-logs-section");e&&I&&I.open&&B()};let R=null;window._startEmailLogAutoRefresh=function(){R||(R=setInterval(()=>{e&&o&&B()},3e4))},window._stopEmailLogAutoRefresh=function(){R&&(clearInterval(R),R=null)}})();(function(){let e=[],n=null,a=[],o="all",s=null,i=!1;async function r(){if(i){D();return}i=!0,p(),await l(),D()}function p(){const y=document.getElementById("bank-file-input");y&&!y._bound&&(y._bound=!0,y.addEventListener("change",w));const F=document.getElementById("btn-bank-queue-clear-done");F&&!F._bound&&(F._bound=!0,F.addEventListener("click",L));const V=document.getElementById("btn-bank-back");V&&!V._bound&&(V._bound=!0,V.addEventListener("click",()=>{n=null,a=[],Q()}));const W=document.getElementById("btn-bank-delete");W&&!W._bound&&(W._bound=!0,W.addEventListener("click",g));const X=document.getElementById("btn-bank-run-match");X&&!X._bound&&(X._bound=!0,X.addEventListener("click",_)),document.querySelectorAll(".bank-filter-btn").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",()=>{o=fe.dataset.bankFilter||"all",document.querySelectorAll(".bank-filter-btn").forEach(me=>{me.classList.toggle("active",me===fe)}),Z()}))}),document.querySelectorAll("[data-bank-cand-close]").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",ne))});const de=document.getElementById("btn-bank-cand-pane-close");de&&!de._bound&&(de._bound=!0,de.addEventListener("click",ne));const ie=document.getElementById("btn-bank-cand-ignore");ie&&!ie._bound&&(ie._bound=!0,ie.addEventListener("click",B));const ce=document.getElementById("btn-bank-cand-ignore-pane");ce&&!ce._bound&&(ce._bound=!0,ce.addEventListener("click",B));const ae=document.getElementById("bank-client-badge");ae&&!ae._bound&&(ae._bound=!0,ae.addEventListener("click",x)),document.querySelectorAll("[data-bank-client-picker-close]").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",b))});const pe=document.getElementById("btn-bank-client-picker-save");pe&&!pe._bound&&(pe._bound=!0,pe.addEventListener("click",z)),document.querySelectorAll(".bank-sessions-chip").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",()=>{R=fe.dataset.sessFilter||"all",document.querySelectorAll(".bank-sessions-chip").forEach(me=>{me.classList.toggle("active",me===fe)}),I()}))})}async function l(){try{const y=await fetch("/api/bank-recon/sessions?limit=30",{headers:{Authorization:"Bearer "+token}});if(!y.ok)throw new Error("sessions:"+y.status);e=await y.json(),I()}catch(y){console.warn("[bank-recon] loadSessions failed",y),e=[],I()}}async function d(y){try{const F="/api/bank-recon/sessions/"+encodeURIComponent(y)+(o!=="all"?"?filter="+o:""),V=await fetch(F,{headers:{Authorization:"Bearer "+token}});if(!V.ok)throw new Error("detail:"+V.status);const W=await V.json();n=W.session,a=W.transactions||[],N()}catch(F){console.warn("[bank-recon] loadSessionDetail failed",F),showToast(t("bank-load-failed"),"error")}}let m=[],c=0;const u=3;function f(){return c+=1,"q"+c+"_"+Date.now()}async function w(y){const F=Array.from(y.target.files||[]);if(y.target.value="",F.length!==0){for(const V of F){const W={id:f(),file:V,name:V.name,size:V.size,status:"pending",progress:0,error_code:null,tx_count:0,session_id:null};V.name.toLowerCase().endsWith(".pdf")?V.size>20*1024*1024&&(W.status="failed",W.error_code="bank_recon.file_too_large"):(W.status="failed",W.error_code="bank_recon.only_pdf"),m.push(W)}C(),E(),A()}}function C(){const y=document.getElementById("bank-upload-queue");y&&(y.style.display=""),se(),ue()}function E(){const y=document.getElementById("bank-upload-queue-list"),F=document.getElementById("bank-upload-queue-summary");if(!y)return;if(m.length===0){y.innerHTML="",F&&(F.textContent="");const ie=document.getElementById("bank-upload-queue");ie&&(ie.style.display="none");return}let V=0,W=0,X=0,de=0;for(const ie of m)ie.status==="ok"?V++:ie.status==="failed"?W++:ie.status==="uploading"||ie.status==="parsing"?X++:de++;F&&(F.textContent=t("bank-queue-summary").replace("{ok}",V).replace("{run}",X).replace("{wait}",de).replace("{fail}",W)),y.innerHTML=m.map(v).join(""),y.querySelectorAll("[data-q-act]").forEach(ie=>{const ce=ie.dataset.qAct,ae=ie.dataset.qId;ie.addEventListener("click",()=>{ce==="retry"&&h(ae),ce==="remove"&&S(ae)})})}function v(y){const F=(y.size/1024).toFixed(0)+" KB";let V="",W="";if(y.status==="pending")V='<span class="bq-stat bq-wait">'+t("bank-queue-status-wait")+"</span>",W='<button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>';else if(y.status==="uploading")V='<span class="bq-stat bq-run">'+t("bank-queue-status-uploading")+'</span><div class="bq-bar"><div class="bq-bar-fill" style="width:'+(y.progress||0)+'%"></div></div>';else if(y.status==="parsing")V='<span class="bq-stat bq-run">'+t("bank-queue-status-parsing")+'</span><div class="bq-bar"><div class="bq-bar-fill bq-bar-indet"></div></div>';else if(y.status==="ok")V='<span class="bq-stat bq-ok">'+t("bank-queue-status-ok").replace("{n}",y.tx_count||0)+"</span>",W='<button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>';else if(y.status==="failed"){const X=k(y.error_code||"unknown");V='<span class="bq-stat bq-fail" title="'+O(X)+'">'+O(X)+"</span>",W='<button data-q-act="retry" data-q-id="'+O(y.id)+'" class="bq-act bq-act-retry">'+t("bank-queue-retry")+'</button><button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>'}return'<div class="bq-row" data-q-row="'+O(y.id)+'"><div class="bq-name" title="'+O(y.name)+'">'+O(y.name)+'</div><div class="bq-size">'+F+'</div><div class="bq-status">'+V+'</div><div class="bq-actions">'+W+"</div></div>"}function h(y){const F=m.find(V=>V.id===y);F&&(F.status="pending",F.error_code=null,F.progress=0,E(),A())}function S(y){const F=m.findIndex(W=>W.id===y);if(F<0)return;const V=m[F];V.status==="uploading"||V.status==="parsing"||(m.splice(F,1),E())}function L(){m=m.filter(y=>y.status!=="ok"),E()}async function A(){for(;;){if(m.filter(V=>V.status==="uploading"||V.status==="parsing").length>=u)return;const F=m.find(V=>V.status==="pending");if(!F){m.every(V=>V.status==="ok"||V.status==="failed")&&(await l(),typeof window.loadReconcilePage=="function"&&window.loadReconcilePage());return}T(F).then(()=>A())}}async function T(y){y.status="uploading",y.progress=0,E();try{const F=new FormData;F.append("file",y.file,y.name);const V=await new Promise((X,de)=>{const ie=new XMLHttpRequest;ie.open("POST","/api/bank-recon/upload"),ie.setRequestHeader("Authorization","Bearer "+token),ie.upload.onprogress=ce=>{ce.lengthComputable&&(y.progress=Math.min(99,Math.round(ce.loaded/ce.total*100)),E())},ie.upload.onload=()=>{y.status="parsing",E()},ie.onload=()=>{ie.status>=200&&ie.status<300?X({status:ie.status,text:ie.responseText}):X({status:ie.status,text:ie.responseText})},ie.onerror=()=>de(new Error("network")),ie.send(F)});let W={};try{W=JSON.parse(V.text||"{}")}catch{W={}}if(V.status>=400){y.status="failed",y.error_code=W&&W.detail||"unknown",E();return}if(W.parse_status==="parse_failed"){y.status="failed",y.error_code=W.error==="scanned_pdf_not_yet"?"bank_recon.scanned":"bank_recon.no_tx",E();return}y.status="ok",y.tx_count=W.tx_count||0,y.session_id=W.session_id||null,E()}catch(F){console.warn("[bank-recon] upload failed",F),y.status="failed",y.error_code="network",E()}}async function _(){if(!n)return;const y=document.getElementById("btn-bank-run-match"),F=y.innerHTML;y.disabled=!0,y.innerHTML="<span>"+t("bank-matching")+"</span>";try{const V=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id)+"/match",{method:"POST",headers:{Authorization:"Bearer "+token}});if(!V.ok)throw new Error("match:"+V.status);const W=await V.json();showToast(t("bank-match-done").replace("{matched}",W.matched).replace("{suggested}",W.suggested).replace("{unmatched}",W.unmatched),"success"),await d(n.id),await l()}catch(V){console.warn("[bank-recon] match failed",V),showToast(t("bank-match-failed"),"error")}finally{y.disabled=!1,y.innerHTML=F}}async function g(){if(!(!n||!await showConfirm(t("bank-delete-confirm"),{danger:!0})))try{const F=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id),{method:"DELETE",headers:{Authorization:"Bearer "+token}});if(!F.ok)throw new Error("delete:"+F.status);showToast(t("bank-deleted"),"success"),n=null,a=[],Q(),await l()}catch(F){console.warn("[bank-recon] delete failed",F),showToast(t("bank-delete-failed"),"error")}}async function B(){if(s)try{const y=await fetch("/api/bank-recon/tx/"+encodeURIComponent(s.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"ignored"})});if(!y.ok)throw new Error("ignore:"+y.status);ne(),await d(n.id)}catch{showToast(t("bank-action-failed"),"error")}}async function H(y){if(s)try{const F=await fetch("/api/bank-recon/tx/"+encodeURIComponent(s.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"matched",history_id:y})});if(!F.ok)throw new Error("pick:"+F.status);showToast(t("bank-matched-ok"),"success"),ne(),await d(n.id)}catch{showToast(t("bank-action-failed"),"error")}}function D(){const y=document.getElementById("bank-status-summary");if(!y)return;if(e.length===0){y.textContent=t("bank-pill-none");return}let V=0;for(const W of e)W.parse_status==="parsed"&&(W.unmatched_count||0)>0&&V++;y.textContent=V>0?t("bank-pill-pending").replace("{n}",V):t("bank-pill-ok")}let R="all";function I(){const y=document.getElementById("bank-sessions-list");if(!y)return;let F=e||[];if(R==="parsed"?F=F.filter(V=>V.parse_status==="parsed"):R==="failed"&&(F=F.filter(V=>V.parse_status==="parse_failed")),!e||e.length===0){y.innerHTML='<div class="bank-empty" data-i18n="bank-sessions-empty">'+t("bank-sessions-empty")+"</div>";return}if(F.length===0){y.innerHTML='<div class="bank-empty">'+t("bank-sess-filter-empty")+"</div>";return}y.innerHTML=F.map(V=>U(V)).join(""),y.querySelectorAll(".bank-session-row").forEach(V=>{V.addEventListener("click",W=>{W.target.closest(".bank-session-trash")||d(V.dataset.sessionId)})}),y.querySelectorAll(".bank-session-trash").forEach(V=>{V.addEventListener("click",W=>{W.stopPropagation();const X=V.dataset.sessionId,de=V.dataset.sessionName||"";P(X,de)})})}async function P(y,F){if(!y)return;const V=(t("bank-session-delete-confirm")||"确定删除这条对账记录吗?").replace("{name}",F||"");if(await showConfirm(V,{danger:!0}))try{const X=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(y),{method:"DELETE",headers:{Authorization:"Bearer "+token}});if(!X.ok)throw new Error("delete:"+X.status);showToast(t("bank-deleted"),"success"),n&&n.id===y&&(n=null,a=[],Q()),await l(),typeof window.loadReconcilePage=="function"&&window.loadReconcilePage()}catch(X){console.warn("[bank-recon] delete failed",X),showToast(t("bank-delete-failed"),"error")}}window._deleteBankSession=P;function U(y){const F=(y.bank_code||"OTHER").toUpperCase(),V=J(y.period_start,y.period_end),W=y.account_last4?"···"+y.account_last4:"",X=G(y),de=j(y.created_at);return`
            <div class="bank-session-row" data-session-id="${O(y.id)}">
                <div class="bank-session-bank bk-${O(F)}">${O(F)}</div>
        ...
        `}function G(y){if(y.parse_status==="parse_failed")return`<span class="bank-session-count cnt-failed">${t("bank-count-parse-failed")}</span>`;if(y.parse_status!=="parsed")return`<span class="bank-session-count">${t("bank-count-parsing")}</span>`;const F=y.tx_count||0,V=y.matched_count||0,W=y.unmatched_count||0,X=[`<span class="bank-session-count">${F} ${t("bank-count-tx")}</span>`];return V>0&&X.push(`<span class="bank-session-count cnt-matched">${V} ${t("bank-count-matched")}</span>`),W>0&&X.push(`<span class="bank-session-count cnt-unmatched">${W} ${t("bank-count-unmatched")}</span>`),X.join("")}function N(){document.getElementById("bank-detail").style.display="",document.querySelector(".bank-sessions-section").style.display="none",Y(),Z(),oe()}function Q(){document.getElementById("bank-detail").style.display="none",document.querySelector(".bank-sessions-section").style.display="";const y=document.getElementById("bank-detail-body");y&&y.classList.remove("has-pane"),s=null}function oe(){const y=document.getElementById("bank-client-badge");if(!y||!n)return;const F=n.client_id,V=document.getElementById("bank-client-badge-dot"),W=document.getElementById("bank-client-badge-name"),X=document.getElementById("bank-client-badge-caret"),de=typeof _userInfo<"u"?_userInfo:null,ie=!(de&&de.role==="member");if(F!=null){const ce=(window._clientsCache||[]).find(ae=>Number(ae.id)===Number(F));y.classList.remove("is-empty"),V&&(V.style.background=ce&&ce.color||"#111111"),W&&(W.textContent=ce&&(ce.short_name||ce.name)||"#"+F)}else y.classList.add("is-empty"),V&&(V.style.background=""),W&&(W.textContent=t("bank-client-none"));ie?(y.classList.remove("is-readonly"),y.disabled=!1,X&&(X.style.display="")):(y.classList.add("is-readonly"),y.disabled=!0,X&&(X.style.display="none")),y.style.display=""}let M=null;function x(){if(!n)return;const y=typeof _userInfo<"u"?_userInfo:null;if(!!(y&&y.role==="member"))return;M=n.client_id!=null?Number(n.client_id):null,q();const V=document.getElementById("bank-client-picker-modal");V&&(V.style.display="")}function b(){const y=document.getElementById("bank-client-picker-modal");y&&(y.style.display="none"),M=null}function q(){const y=document.getElementById("bank-client-picker-list");if(!y)return;const F=(window._clientsCache||[]).filter(W=>W&&(W.is_active===!0||W.is_active===void 0)),V=[];V.push('<div class="bank-client-picker-row is-none'+(M==null?" is-selected":"")+'" data-cid=""><span class="bank-cp-dot"></span><span>'+O(t("bank-client-picker-none"))+"</span></div>"),F.forEach(W=>{const X=Number(W.id)===Number(M)?" is-selected":"";V.push('<div class="bank-client-picker-row'+X+'" data-cid="'+O(W.id)+'"><span class="bank-cp-dot" style="background:'+O(W.color||"#111111")+'"></span><span>'+O(W.short_name||W.name||"#"+W.id)+"</span></div>")}),y.innerHTML=V.join(""),y.querySelectorAll(".bank-client-picker-row").forEach(W=>{W.addEventListener("click",()=>{const X=W.dataset.cid;M=X?Number(X):null,q()})})}async function z(){if(n)try{const y=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id)+"/client",{method:"PATCH",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({client_id:M})});if(!y.ok)throw new Error("client:"+y.status);n.client_id=M,oe(),showToast(t("bank-client-changed"),"success"),b();try{await l()}catch{}}catch(y){console.warn("[bank-recon] save client failed",y),showToast(t("bank-client-change-failed"),"error")}}function Y(){if(!n)return;const y=n;document.getElementById("bank-detail-title").textContent=(y.bank_code||"-")+(y.account_last4?" ···"+y.account_last4:"")+" · "+(y.source_filename||""),document.getElementById("bank-meta-period").textContent=J(y.period_start,y.period_end)||"-",document.getElementById("bank-meta-opening").textContent=$(y.opening_balance),document.getElementById("bank-meta-inflow").textContent="+"+$(y.total_inflow),document.getElementById("bank-meta-outflow").textContent="-"+$(y.total_outflow),document.getElementById("bank-meta-closing").textContent=$(y.closing_balance);const F=a||[],V=F.length;let W=0,X=0,de=0;for(const ie of F){const ce=ie.match_status||"unmatched";ce==="matched"?W++:ce==="suggested"?X++:de++}document.getElementById("bank-stat-total").textContent=V,document.getElementById("bank-stat-matched").textContent=W,document.getElementById("bank-stat-suggested").textContent=X,document.getElementById("bank-stat-unmatched").textContent=de}function Z(){const y=document.getElementById("bank-tx-tbody");if(!y)return;let F=a||[];if(o!=="all"&&(F=F.filter(V=>o==="matched"?V.match_status==="matched":o==="suggested"?V.match_status==="suggested":o==="unmatched"?V.match_status==="unmatched"||V.match_status==="ignored":!0)),F.length===0){y.innerHTML=`<tr><td colspan="4" class="bank-empty">${t("bank-tx-empty")}</td></tr>`;return}if(y.innerHTML=F.map(V=>le(V)).join(""),y.querySelectorAll("tr[data-tx-id]").forEach(V=>{V.addEventListener("click",()=>{const W=V.dataset.txId,X=a.find(de=>de.id===W);X&&(y.querySelectorAll("tr.is-selected").forEach(de=>de.classList.remove("is-selected")),V.classList.add("is-selected"),re(X))})}),s){const V=y.querySelector('tr[data-tx-id="'+s.id+'"]');V&&V.classList.add("is-selected")}}function le(y){const F=y.direction==="OUT",V=F?"-":"+",W=F?"bank-out":"bank-in",X=y.match_status||"unmatched",de=t("bank-match-"+X)||X,ie=j(y.tx_date),ce=y.channel?`<span class="bank-tx-channel">${O(y.channel)}</span>`:"";return`
            <tr data-tx-id="${O(y.id)}">
                <td class="bank-tx-date">${O(ie)}</td>
                <td class="bank-tx-desc">${ce}${O(y.description||"-")}</td>
                <td class="bank-td-amount ${W}">${V}${$(y.amount)}</td>
                <td><span class="bank-tx-match mt-${X}">${O(de)}</span></td>
            </tr>
        `}async function re(y){s=y;const F=document.getElementById("bank-detail-body");F&&F.classList.add("has-pane");const V=document.getElementById("bank-cand-pane-title"),W=document.getElementById("bank-cand-pane-sub"),X=document.getElementById("bank-cand-pane-foot");if(V&&(V.textContent=t("bank-cand-pane-current")),W){const ie=y.direction==="OUT"?"-":"+",ce=y.direction==="OUT"?"bank-out":"bank-in";W.innerHTML=`${O(j(y.tx_date))}
                <span style="margin:0 6px;color:#D1D5DB">·</span>
                <span>${O(y.description||"-")}</span>
                <span style="margin:0 6px;color:#D1D5DB">·</span>
                <strong class="${ce}">${ie}${$(y.amount)}</strong>`}X&&(X.style.display="");const de=document.getElementById("bank-cand-pane-body");if(de){de.innerHTML=`<div class="bank-empty">${t("bank-cand-loading")}</div>`;try{const ie=await fetch("/api/bank-recon/tx/"+encodeURIComponent(y.id)+"/candidates",{headers:{Authorization:"Bearer "+token}});if(!ie.ok)throw new Error("cands:"+ie.status);const ce=await ie.json();te(y,ce.candidates||[])}catch{de.innerHTML=`<div class="bank-empty">${t("bank-cand-load-failed")}</div>`}}}function K(y){const F=Number(y||0);let V="score-low";return F>=85?V="score-high":F>=60&&(V="score-mid"),'<span class="bank-cand-score '+V+'">'+F.toFixed(0)+" "+t("bank-cand-score-unit")+"</span>"}function ee(y,F,V){const W=F.history_id,X=F.invoice_no||"-",de=F.vendor||"-",ie=F.amount_total!==null&&F.amount_total!==void 0?$(F.amount_total):"-",ce=F.invoice_date?j(F.invoice_date):"-",ae=F.filename||"",pe=!!V&&y.matched_history_id===W,fe="bank-cand-card"+(F.is_auto_picked?" is-auto":"")+(pe?" is-picked":"");let me="";return pe?me='<button class="btn btn-ghost btn-small" data-act="unmatch"><span>'+t("bank-cand-unmatch")+"</span></button>":me='<button class="btn btn-primary btn-small" data-act="pick" data-hid="'+O(W)+'"><span>'+t(F.is_auto_picked?"bank-cand-confirm":"bank-cand-pick-this")+"</span></button>",'<div class="'+fe+'" data-hid="'+O(W)+'"><div class="bank-cand-card-head"><div class="bank-cand-card-vendor">'+O(de)+"</div>"+K(F.score)+'</div><div class="bank-cand-card-fields"><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-invoice-no")+"</span> "+O(X)+'</span><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-amount")+"</span> "+ie+'</span><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-date")+"</span> "+O(ce)+"</span></div>"+(ae?'<div class="bank-cand-card-file" title="'+O(ae)+'">'+O(ae)+"</div>":"")+(F.reason?'<div class="bank-cand-card-reason">'+O(F.reason)+"</div>":"")+'<div class="bank-cand-card-actions">'+me+"</div></div>"}function te(y,F){const V=document.getElementById("bank-cand-pane-body");if(!V)return;const W=F||[];let X="";if(y.match_status==="matched")X='<div class="bank-cand-hint hint-matched">'+t("bank-cand-hint-matched").replace("{n}",W.length)+"</div>";else if(y.match_status==="suggested")X='<div class="bank-cand-hint hint-suggested">'+t("bank-cand-hint-suggested").replace("{n}",W.length)+"</div>";else if(W.length>0)X='<div class="bank-cand-hint hint-low">'+t("bank-cand-hint-low").replace("{n}",W.length)+"</div>";else{V.innerHTML='<div class="bank-empty">'+t("bank-cand-no-match-detail")+"</div>";return}const de=y.match_status==="matched",ie=W.map(ce=>ee(y,ce,de)).join("");V.innerHTML=X+'<div class="bank-cand-list">'+ie+"</div>",V.querySelectorAll('[data-act="pick"]').forEach(ce=>{ce.addEventListener("click",()=>{H(ce.dataset.hid)})}),V.querySelectorAll('[data-act="unmatch"]').forEach(ce=>{ce.addEventListener("click",async()=>{try{await fetch("/api/bank-recon/tx/"+encodeURIComponent(y.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"unmatched"})}),ne(),await d(n.id)}catch{showToast(t("bank-action-failed"),"error")}})})}function ne(){const y=document.getElementById("bank-detail-body");y&&y.classList.remove("has-pane");const F=document.getElementById("bank-cand-pane-title"),V=document.getElementById("bank-cand-pane-sub"),W=document.getElementById("bank-cand-pane-body"),X=document.getElementById("bank-cand-pane-foot");F&&(F.textContent=t("bank-cand-pane-empty-title")),V&&(V.textContent=t("bank-cand-pane-empty-sub")),X&&(X.style.display="none"),W&&(W.innerHTML='<div class="bank-cand-pane-empty"><svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="14" y="10" width="36" height="44" rx="3"/><path d="M22 22h20M22 30h20M22 38h12"/></svg><div>'+t("bank-cand-pane-empty-hint")+"</div></div>");const de=document.getElementById("bank-tx-tbody");de&&de.querySelectorAll("tr.is-selected").forEach(ie=>ie.classList.remove("is-selected")),s=null}function se(y){const F=document.getElementById("bank-upload-progress");F&&(F.style.display="none")}function ue(){const y=document.getElementById("bank-upload-error");y&&(y.style.display="none")}function k(y){return{"bank_recon.only_pdf":t("bank-err-only-pdf"),"bank_recon.empty_file":t("bank-err-empty"),"bank_recon.file_too_large":t("bank-err-too-large"),"bank_recon.save_failed":t("bank-err-server"),"bank_recon.scanned":t("bank-err-scanned"),"bank_recon.no_tx":t("bank-err-no-tx"),network:t("bank-err-network")}[y]||t("bank-err-unknown")+" ("+y+")"}function $(y){if(y==null)return"-";const F=Number(y);return isNaN(F)?"-":F.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function j(y){if(!y)return"-";const F=String(y);return F.length>=10?F.slice(0,10):F}function J(y,F){return!y&&!F?"":(j(y)||"?")+" ~ "+(j(F)||"?")}function O(y){return y==null?"":String(y).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}window._loadBankReconPanel=r,window._rerenderBankRecon=function(){if(currentRoute==="automation"){if(I(),n&&(Y(),Z(),oe(),!s)){const y=document.getElementById("bank-cand-pane-title"),F=document.getElementById("bank-cand-pane-sub");y&&(y.textContent=t("bank-cand-pane-empty-title")),F&&(F.textContent=t("bank-cand-pane-empty-sub"))}E()}},typeof window.subscribeI18n=="function"&&window.subscribeI18n("bank-recon",window._rerenderBankRecon),window._openBankSession=async function(y){y&&(i||await r(),await d(y))}})();(function(){const e=document.getElementById("page-clients");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        <div class="page-head">
            <div class="page-head-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function n(M,x){let b=t(M)||M;if(x)for(const q in x)b=b.replace(new RegExp("\\{"+q+"\\}","g"),String(x[q]));return b}async function a(){try{const M=e.currentClient||"",x="/api/exceptions/stats?status=pending"+(M?"&client_id="+encodeURIComponent(M):""),b=await fetch(x,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!b.ok)return;const q=await b.json(),z=document.getElementById("nav-exc-badge");if(!z)return;const Y=parseInt(q.pending||0,10);Y>0?(z.textContent=Y>99?"99+":String(Y),z.style.display=""):z.style.display="none"}catch{}}function o(M){return M==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        ...
        </svg>`}function i(M){if(M==null)return"—";const x=parseFloat(M);return isNaN(x)?"—":"฿ "+x.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})}function r(M){return M?M.slice(0,10):"—"}function p(M){document.getElementById("exc-kpi-pending").textContent=M.pending||0,document.getElementById("exc-kpi-high").textContent=M.high_severity||0,document.getElementById("exc-kpi-resolved").textContent=M.resolved||0,document.getElementById("exc-kpi-learned").textContent=M.learned_rules||0;const x=document.getElementById("exc-status-tab-count-pending"),b=document.getElementById("exc-status-tab-count-resolved"),q=document.getElementById("exc-status-tab-count-ignored");x&&(x.textContent=M.pending||0),b&&(b.textContent=M.resolved||0),q&&(q.textContent=M.ignored||0),document.querySelectorAll("#exc-status-tabs .exc-status-tab").forEach(Y=>{Y.classList.toggle("active",Y.dataset.status===(e.currentStatus||"pending"))})}function l(M){const x=document.getElementById("exc-chips");if(!x)return;const b=M.by_rule||{},q=["confidence_low","duplicate","amount_missing","math_mismatch","tax_id_format_invalid"];let Y=`<button class="exc-chip ${!e.currentRule?"active":""}" data-rule="">
            ...
            `}).join(""),x.querySelectorAll(".exc-row").forEach(z=>{z.addEventListener("click",Y=>{if(Y.target.closest(".exc-row-check"))return;const Z=z.dataset.excId;Z&&L(parseInt(Z,10))})}),x.querySelectorAll(".exc-row-check").forEach(z=>{z.addEventListener("click",Y=>{Y.stopPropagation();const Z=parseInt(z.dataset.checkId,10);Z&&(e.selectedIds.has(Z)?(e.selectedIds.delete(Z),z.classList.remove("checked"),z.closest(".exc-row").classList.remove("selected")):(e.selectedIds.add(Z),z.classList.add("checked"),z.closest(".exc-row").classList.add("selected")),m())})}),m(),c()}function m(){const M=document.getElementById("exc-batch-bar"),x=document.getElementById("exc-batch-count");if(!M||!x)return;const b=e.selectedIds.size;b===0?M.style.display="none":(M.style.display="",x.textContent=n("exc-batch-count",{n:b}))}function c(){const M=document.getElementById("exc-list-foot"),x=document.getElementById("exc-list-count"),b=document.getElementById("exc-loadmore");if(!M||!x||!b)return;const q=e.listCache.length;if(q===0){M.style.display="none";return}M.style.display="";let z=q;const Y=e.statsCache;Y&&(e.currentRule?z=(Y.by_rule||{})[e.currentRule]||q:z=Y.pending||q),e.total=z,x.textContent=n("exc-list-count",{shown:q,total:z});const Z=q<z&&q<500;b.style.display=Z?"":"none"}async function u(){try{if(navigator.onLine===!1)throw new Error("offline");const M=e.currentClient||"",x=e.currentStatus||"pending",b=new URLSearchParams;b.set("status",x),M&&b.set("client_id",M);const q="/api/exceptions/stats?"+b.toString(),z=await fetch(q,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!z.ok)throw new Error("http "+z.status);const Y=await z.json();return e.statsCache=Y,p(Y),l(Y),Y}catch(M){return console.warn("loadExceptionsStats fail",M),null}}function f(M){const x=document.getElementById("exc-list");if(!x)return;const b=`<svg class="exc-error-icon" viewBox="0 0 36 36" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="18" cy="18" r="14"/>
            <line x1="18" y1="11" x2="18" y2="19"/>
            <circle cx="18" cy="23" r="0.8" fill="currentColor"/>
        </svg>`,q=M?t("exc-offline"):t("exc-error-retry-title"),z=M?"":t("exc-error-retry-desc");x.innerHTML=`
            <div class="exc-error">
                ${b}
            ...
            </div>`;const Y=document.getElementById("exc-retry-btn");Y&&Y.addEventListener("click",()=>window.loadExceptionsPage&&window.loadExceptionsPage())}async function w(M){M=M||{};const x=!!M.append,b=document.getElementById("exc-list");!x&&b&&e.listCache.length===0&&(b.innerHTML=`<div class="exc-loading">${escapeHtml(t("exc-loading"))}</div>`);const q=new URLSearchParams;q.set("status",e.currentStatus||"pending"),e.currentRule&&q.set("rule_code",e.currentRule),e.currentClient&&q.set("client_id",e.currentClient);const z=x?e.listCache.length:0;q.set("limit",String(e.pageSize)),q.set("offset",String(z));try{if(navigator.onLine===!1)throw new Error("offline");const Y=await fetch("/api/exceptions/list?"+q.toString(),{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!Y.ok)throw new Error("http "+Y.status);const le=(await Y.json()).items||[];x?e.listCache=e.listCache.concat(le):(e.listCache=le,e.selectedIds.clear()),e.loadFailed=!1,d(e.listCache),e.statsCache&&l(e.statsCache)}catch(Y){console.warn("loadExceptionsList fail",Y),e.loadFailed=!0;const Z=navigator.onLine===!1||String(Y.message||"").includes("offline");x?showToast(t("exc-toast-load-fail"),"error"):(f(Z),showToast(Z?t("exc-offline"):t("exc-toast-load-fail"),"error"))}}async function C(){if(!e.loading&&!(e.listCache.length>=500)){e.loading=!0;try{await w({append:!0})}finally{e.loading=!1}}}function E(){const M=document.getElementById("exc-client-filter");if(!M)return;const x=window._clientsCache||[],b=e.currentClient||"",q=typeof t=="function"?t("history-client-all"):"全部客户";M.innerHTML=`<option value="">${escapeHtml(q)}</option>`+x.map(z=>`<option value="${z.id}">${escapeHtml(z.name)}</option>`).join(""),M.value=b}window.loadExceptionsPage=async function(){if(!e.loading){e.loading=!0;try{E(),typeof window.loadErpExceptions=="function"&&window.loadErpExceptions(),await u(),await w()}finally{e.loading=!1}}},window.refreshExcBadge=a,window._refreshExcClientFilter=E,window._excState=e,window._rerenderExceptions=function(){try{E()}catch{}e.statsCache&&(p(e.statsCache),l(e.statsCache)),e.listCache&&e.listCache.length&&d(e.listCache);try{window._erpExcState&&window._erpExcState.items&&window._erpExcState.items.length&&typeof window._rerenderErpExceptions=="function"&&window._rerenderErpExceptions()}catch{}v.openExcId&&H()};let v={openExcId:null,excRow:null,history:null,loading:!1,pdfUrl:null,pdfStatus:"idle",editing:!1,editFields:null};function h(){if(v.pdfUrl){try{URL.revokeObjectURL(v.pdfUrl)}catch{}v.pdfUrl=null}v.pdfStatus="idle"}async function S(M,x){v.pdfStatus="loading",H();try{const b=await fetch("/api/history/"+encodeURIComponent(M)+"/pdf",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(v.openExcId!==x)return;if(b.status===404){v.pdfStatus="empty",H();return}if(!b.ok)throw new Error("http "+b.status);const q=await b.blob();if(v.openExcId!==x)return;h(),v.pdfUrl=URL.createObjectURL(q),v.pdfStatus="ready",H()}catch(b){if(v.openExcId!==x)return;console.warn("loadDrawerPdf fail",b),v.pdfStatus="error",H()}}function L(M){const x=(e.listCache||[]).find(b=>b.id===M);if(!x){showToast(t("exc-drawer-error"),"error");return}e.listScrollY=window.scrollY||document.documentElement.scrollTop||0,h(),v.editing=!1,v.editFields=null,v.openExcId=M,v.excRow=x,v.history=null,document.getElementById("exc-drawer-mask").classList.add("show"),document.getElementById("exc-drawer").classList.add("show"),H(),T(x.history_id),S(x.history_id,M)}function A(){h(),v.editing=!1,v.editFields=null,v.openExcId=null,v.excRow=null,v.history=null,document.getElementById("exc-drawer-mask").classList.remove("show"),document.getElementById("exc-drawer").classList.remove("show");const M=e.listScrollY||0;M>0&&requestAnimationFrame(()=>window.scrollTo(0,M))}async function T(M){try{const x=await fetch("/api/history/"+encodeURIComponent(M),{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);v.history=await x.json()}catch(x){console.warn("loadHistoryDetail fail",x),v.history={_err:!0}}v.excRow&&H()}function _(M){if(!M||!M.pages)return{};const x=M.pages,b=x.find(q=>!q.is_duplicate&&!q.is_copy)||x[0];return b&&b.fields||{}}function g(M){if(M==null)return"—";const x=typeof M=="number"?M:parseFloat(String(M).replace(/,/g,""));return isNaN(x)?escapeHtml(String(M)):"฿ "+x.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})}function B(M,x){if(x=x||{},M==="math_mismatch")return`
            ...
            `;if(M==="duplicate"){const b=x.level==="exact"?t("exc-detail-dup-level-exact"):t("exc-detail-dup-level-likely");return`
            ...
            `:M==="amount_missing"?`<div class="exc-why-detail-row" style="justify-content:center;color:var(--danger);"><span>${escapeHtml(t("exc-detail-missing"))}</span></div>`:`<div class="exc-why-detail-row"><span style="font-size:11px;">${escapeHtml(JSON.stringify(x))}</span></div>`}function H(){const M=v.excRow;if(!M)return;const x=M.seller_name&&M.seller_name.trim()?M.seller_name:t("exc-no-seller"),b=M.filename||"—";document.getElementById("exc-drawer-title").textContent=b;const q="exc-status-"+(M.status||"pending"),z=t(q)||M.status,Y="s-"+(M.status||"pending"),Z=(M.invoice_date||M.created_at||"").slice(0,10);document.getElementById("exc-drawer-sub").innerHTML=`
        ...
        `;const le=M.severity||"medium",re=t("exc-rule-"+M.rule_code)||M.rule_code,K=B(M.rule_code,M.detail||{}),ee=_(v.history),te=v.history===null,ne=v.history&&v.history._err,se=new Set;M.rule_code==="math_mismatch"?(se.add("subtotal"),se.add("vat"),se.add("total_amount")):M.rule_code==="tax_id_format_invalid"?se.add("seller_tax"):M.rule_code==="amount_missing"&&(se.add("total_amount"),se.add("invoice_number"));const ue=!!v.editing,k=v.editFields||{},$=(ae,pe,fe)=>{if(te)return`<div class="exc-field-row"><label>${escapeHtml(t(pe))}</label><span class="val empty">…</span></div>`;const me=ue?k[ae]!==void 0?k[ae]:ee[ae]!==void 0&&ee[ae]!==null?ee[ae]:"":ee[ae],ge=se.has(ae)?"flagged":"";if(ue){const Oe=fe?"number":"text",Ce=fe?' step="0.01" inputmode="decimal"':"",He=me==null?"":String(me).replace(/"/g,"&quot;");return`<div class="exc-field-row ${ge} editing">
        ...
        `;const O=document.getElementById("exc-fld-edit");O&&O.addEventListener("click",()=>{v.editing=!0,v.editFields={..._(v.history)},H()});const y=document.getElementById("exc-fld-cancel");y&&y.addEventListener("click",()=>{v.editing=!1,v.editFields=null,H()});const F=document.getElementById("exc-fld-save");F&&F.addEventListener("click",()=>D()),document.querySelectorAll(".exc-field-input").forEach(ae=>{ae.addEventListener("input",()=>{v.editFields||(v.editFields={}),v.editFields[ae.dataset.editKey]=ae.value})});const W=document.getElementById("exc-pdf-retry");W&&v.openExcId&&W.addEventListener("click",()=>{v.excRow&&S(v.excRow.history_id,v.openExcId)});const X=M.status==="pending",de=!!(M.seller_name&&M.seller_name.trim()),ie=document.getElementById("exc-btn-resolve"),ce=document.getElementById("exc-btn-ignore");ie.disabled=!X,ce.disabled=!X||!de,ce.title=de?t("exc-ignore-hint"):t("exc-ignore-no-seller")}async function D(){if(!v.openExcId||!v.history||!v.history.pages||v.loading)return;v.loading=!0;const M=showToast(t("exc-fld-saving"),"loading",0);try{const x=JSON.parse(JSON.stringify(v.history.pages||[]));let b=x.findIndex(re=>!re.is_duplicate&&!re.is_copy);b<0&&(b=0),x[b]||(x[b]={fields:{}});const q=x[b].fields||{},z=v.editFields||{},Y=new Set(["subtotal","vat","total_amount"]),Z={...q};for(const re in z){let K=z[re];if((K===""||K===void 0)&&(K=null),Y.has(re)&&K!==null){const ee=parseFloat(K);K=isNaN(ee)?null:ee}Z[re]=K}x[b].fields=Z;const le=await fetch("/api/history/"+encodeURIComponent(v.history.id),{method:"PUT",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({pages:x})});if(!le.ok)throw new Error("http "+le.status);M(),showToast(t("exc-fld-save-ok"),"success"),A(),await u(),await w(),a()}catch(x){M(),console.warn("save fields fail",x),showToast(t("exc-fld-save-fail"),"error")}finally{v.loading=!1}}async function R(){if(!(!v.openExcId||v.loading)){v.loading=!0;try{const M=await fetch("/api/exceptions/"+v.openExcId+"/resolve",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!M.ok)throw new Error("http "+M.status);showToast(t("exc-toast-resolved"),"success"),A(),await u(),await w(),a()}catch(M){console.warn("resolve fail",M),showToast(t("exc-toast-action-fail"),"error")}finally{v.loading=!1}}}async function I(){if(!(!v.openExcId||v.loading)){v.loading=!0;try{const M=await fetch("/api/exceptions/"+v.openExcId+"/ignore",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!M.ok)throw new Error("http "+M.status);showToast(t("exc-toast-ignored"),"success"),A(),await u(),await w(),a()}catch(M){console.warn("ignore fail",M),showToast(t("exc-toast-action-fail"),"error")}finally{v.loading=!1}}}let P=!1;async function U(){if(P)return;const M=Array.from(e.selectedIds);if(M.length===0||!await showConfirm(n("exc-batch-confirm-resolve",{n:M.length})))return;P=!0;const b=showToast(n("exc-batch-count",{n:M.length})+" …","loading",0);try{const q=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:M,action:"resolve"})});if(!q.ok)throw new Error("http "+q.status);const z=await q.json();b(),showToast(n("exc-toast-batch-resolved",{n:z.processed||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(q){b(),console.warn("batch resolve fail",q),showToast(t("exc-toast-batch-fail"),"error")}finally{P=!1}}async function G(){if(P)return;const M=Array.from(e.selectedIds);if(M.length===0||!await showConfirm(n("exc-batch-confirm-ignore",{n:M.length}),{danger:!1}))return;P=!0;const b=showToast(n("exc-batch-count",{n:M.length})+" …","loading",0);try{const q=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:M,action:"ignore"})});if(!q.ok)throw new Error("http "+q.status);const z=await q.json();b(),showToast(n("exc-toast-batch-ignored",{n:z.processed||0,wl:z.whitelist_added||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(q){b(),console.warn("batch ignore fail",q),showToast(t("exc-toast-batch-fail"),"error")}finally{P=!1}}function N(){e.selectedIds.clear(),d(e.listCache)}document.addEventListener("click",M=>{M.target.closest("#exc-drawer-close")&&A(),M.target.closest("#exc-drawer-mask")&&A(),M.target.closest("#exc-btn-resolve")&&R(),M.target.closest("#exc-btn-ignore")&&I(),M.target.closest("#exc-batch-resolve")&&U(),M.target.closest("#exc-batch-ignore")&&G(),M.target.closest("#exc-batch-clear")&&N(),M.target.closest("#exc-loadmore")&&C()}),document.addEventListener("keydown",M=>{M.key==="Escape"&&v.openExcId&&A()}),document.addEventListener("click",M=>{M.target.closest("#btn-exc-refresh")&&(typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a())}),document.addEventListener("change",M=>{if(!M.target.closest("#exc-client-filter"))return;const x=M.target;e.currentClient=x.value||"",e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a()}),document.addEventListener("click",M=>{const x=M.target.closest("#exc-status-tabs .exc-status-tab");if(!x)return;const b=x.dataset.status||"pending";b!==e.currentStatus&&(e.currentStatus=b,e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage())}),window.addEventListener("online",()=>{e.loadFailed&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage&&window.loadExceptionsPage()}),setTimeout(a,1500),setInterval(a,6e4);function Q(M){if(!M)return"—";try{const x=new Date(M),b=q=>String(q).padStart(2,"0");return`${x.getFullYear()}-${b(x.getMonth()+1)}-${b(x.getDate())} ${b(x.getHours())}:${b(x.getMinutes())}`}catch{return M.slice(0,16).replace("T"," ")}}async function oe(){const M=document.getElementById("learned-list");if(M){M.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-loading"))}</div>`;try{const x=await fetch("/api/exception-whitelist",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);const q=(await x.json()).items||[];if(q.length===0){M.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-empty"))}</div>`;return}const z=`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                <path d="M3 4h8M5.5 4V2.5h3V4M4 4l0.6 8.5h4.8L10 4"/>
            </svg>`;M.innerHTML=q.map(Y=>{const Z=t("exc-rule-"+Y.rule_code)||Y.rule_code;return`
                ...
                `}).join("")}catch(x){console.warn("loadLearnedRules fail",x),M.innerHTML=`<div class="learned-empty">${escapeHtml(t("exc-toast-load-fail"))}</div>`}}}window.loadLearnedRules=oe,document.addEventListener("click",async M=>{const x=M.target.closest("[data-del-wl]");if(!x)return;const b=parseInt(x.dataset.delWl,10);if(!b)return;const q=x.closest(".learned-row"),z=q&&q.querySelector(".learned-seller"),Y=z?z.textContent.trim():"",Z=t("set-learned-del-confirm").replace("{seller}",Y);if(await showConfirm(Z,{danger:!0}))try{const re=await fetch("/api/exception-whitelist/"+b,{method:"DELETE",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!re.ok)throw new Error("http "+re.status);showToast(t("set-learned-del-ok"),"success"),oe(),typeof window.loadExceptionsPage=="function"&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage()}catch(re){console.warn("delete whitelist fail",re),showToast(t("set-learned-del-fail"),"error")}})})();(function(){let e={items:[],q:"",cat:"",adapter:"",selected:new Set,total:0,categories:{},pageSize:30,loading:!1,focusSearch:!1,searchCaret:0},n=null;function a(){return localStorage.getItem("mrpilot_token")||""}function o(c){const u=typeof currentLang=="string"&&currentLang||window._currentLang||"th",f=c.error_friendly&&c.error_friendly[u];if(f)return f;if(typeof humanizeError=="function"&&c.error_msg)try{return humanizeError(c.error_msg)}catch{}return t("erp-exc-reason-"+(c.category||"other"))}function s(){const c=document.getElementById("erp-exc-batch");if(!c)return;const u=e.selected.size;c.hidden=u===0;const f=c.querySelector(".erp-exc-batch-count");f&&(f.textContent=String(u))}function i(){const c=document.getElementById("erp-exc-block");if(!c)return;const u=e;if(!(u.total>0||!!u.q||!!u.cat)){c.hidden=!0,c.innerHTML="";return}c.hidden=!1;const w=u.categories||{},C=Object.keys(w).reduce((N,Q)=>N+w[Q],0);let E=`<button class="erp-exc-chip ${u.cat===""?"active":""}" data-erpexc-cat=""><span>${escapeHtml(t("erp-exc-cat-all"))}</span><span class="erp-exc-chip-count">${C}</span></button>`;Object.keys(w).forEach(N=>{E+=`<button class="erp-exc-chip ${u.cat===N?"active":""}" data-erpexc-cat="${escapeHtml(N)}"><span>${escapeHtml(t("erp-exc-cat-"+N))}</span><span class="erp-exc-chip-count">${w[N]}</span></button>`});const v=u.items||[],h=v.length>0&&v.every(N=>u.selected.has(N.id)),S=v.map(N=>{const Q=N.state==="needs_action"?"needs":N.state==="retrying"?"retry":"fail",oe=t("erp-exc-state-"+(N.state||"failed")),M=o(N),x=u.selected.has(N.id)?"checked":"",b=N.push_type==="id_card",q=b?`<span class="erp-exc-type-idcard">${escapeHtml(t("erp-log-type-idcard"))}</span> `:"",z=b?`<span class="ex-inv" title="${escapeHtml(t("erp-log-col-booking"))}">${q}${escapeHtml(N.invoice_no||"—")}</span>`:`<span class="ex-inv" title="${escapeHtml(N.invoice_no||"")}">${escapeHtml(N.invoice_no||"—")}</span>`,Y=b?`<span class="ex-seller" title="${escapeHtml(t("erp-log-col-customer"))}">${escapeHtml(N.seller_name||"—")}</span>`:`<span class="ex-seller" title="${escapeHtml(N.seller_name||"")}">${escapeHtml(N.seller_name||"—")}</span>`,Z=b?`<span class="ex-buyer" title="${escapeHtml(t("erp-log-col-idcard"))}">${N.id_card_tail?"••••"+escapeHtml(N.id_card_tail):"—"}</span>`:`<span class="ex-buyer" title="${escapeHtml(N.ocr_buyer_name||"")}">${escapeHtml(N.ocr_buyer_name||"—")}</span>`;return`<div class="erp-exc-row" data-erpexc-id="${escapeHtml(N.id)}">
                <span class="ex-cb"><input type="checkbox" class="erp-exc-cb" data-erpexc-cb="${escapeHtml(N.id)}" ${x}></span>
                ${z}
                ${Y}
                ${Z}
                <span class="ex-state"><span class="erp-exc-state ${Q}">${escapeHtml(oe)}</span></span>
                <span class="ex-reason" title="${escapeHtml(M)}">${escapeHtml(M)}${N.error_code?` <span class="erp-exc-code">${escapeHtml(N.error_code)}</span>`:""}</span>
                <span class="ex-act"><button class="btn btn-sm btn-secondary" type="button" data-erpexc-retry="${escapeHtml(N.id)}">${escapeHtml(t("erp-exc-retry"))}</button></span>
            </div>`}).join(""),L=v.length===0?`<div class="erp-exc-empty">${escapeHtml(t("erp-exc-empty"))}</div>`:"",A=v.length<u.total?`<button class="erp-exc-more" type="button" id="erp-exc-more">${escapeHtml(t("erp-exc-load-more"))} (${v.length}/${u.total})</button>`:u.total>0?`<div class="erp-exc-count">${escapeHtml(t("erp-exc-shown",{n:v.length,total:u.total}))}</div>`:"",T=u.adapter==="mrerp_dms",_=Array.isArray(window._erpEndpoints)?window._erpEndpoints:[],g=new Set;let B=`<option value="">${escapeHtml(t("erp-logs-erp-all"))}</option>`;_.forEach(N=>{const Q=(N&&N.adapter||"").toLowerCase();!Q||g.has(Q)||(g.add(Q),B+=`<option value="${escapeHtml(Q)}"${Q===u.adapter?" selected":""}>${escapeHtml(N&&N.name||Q)}</option>`)});const H=T?t("erp-log-col-booking"):t("erp-exc-f-invoice"),D=T?t("erp-log-col-customer"):t("erp-exc-f-seller"),R=T?t("erp-log-col-idcard"):t("erp-exc-f-buyer");c.innerHTML=`
            ...
            <div class="erp-exc-modal" role="dialog" aria-modal="true">
                ...
                <td>${J}</td>
                <td>${O}</td>
                <td><span class="badge ${ne[F]||"badge-gray"}">${te[F]||F}</span></td>
                <td>${y}</td>
        ...
        </div>`;K&&(K.innerHTML=p.map(($,j)=>se($,j,"inv")).join("")||'<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>'),ee&&(ee.innerHTML=l.map(($,j)=>se($,j,"rep")).join("")||'<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>'),document.querySelectorAll(".vex-fi-x").forEach($=>{$.addEventListener("click",j=>{const J=$.dataset.vexKind,O=parseInt($.dataset.vexIdx,10);J==="inv"?R(O):I(O)})});const ue=p.length>0&&l.length>0;o("vex-build").disabled=!ue||d;const k=o("vex-action-info");k&&(!p.length||!l.length?(k.textContent=t("vex-need-both")||"需要至少 1 张发票 + 1 份 VAT 报告",k.className="vex-action-info muted"):(k.textContent=(t("vex-ready")||"已就绪 · {a} 张发票 · {b} 份报告").replace("{a}",p.length).replace("{b}",l.length),k.className="vex-action-info ok")),Q()}const U='<svg class="vex-pp-fi-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6B7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>',G='<svg class="vex-pp-fi-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#111111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>',N='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';function Q(){const K=o("vex-preview-panel");if(!K||K.style.display==="none")return;oe("inv"),oe("rep");const ee=o("vex-pp-guide");ee&&(ee.style.display=p.length>100?"flex":"none")}function oe(K){const ee=o(K==="inv"?"vex-pp-invoice-col":"vex-pp-report-col");if(!ee)return;const te=K==="inv"?p:l,ne=K==="inv"?f:w,se=t(K==="inv"?"vex-preview-invoice":"vex-preview-report")||(K==="inv"?"销售发票":"VAT 报告"),ue=i(t("vex-preview-search")||"搜索文件名..."),k=i(t("vex-preview-clear-all")||"全清");ee.innerHTML=`
            ...
            </table>`,$.style.display=""}async function b(k){const $=localStorage.getItem("mrpilot_token")||"",j=window._currentLang||"zh";try{const J=await fetch("/api/recon/bank-v2/"+k+"/export?lang="+j,{headers:{Authorization:"Bearer "+$}});if(!J.ok){const de=await J.json().catch(()=>({}));window.showToast&&window.showToast(de.detail||"Export failed","error");return}const O=await J.blob(),F=(J.headers.get("content-disposition")||"").match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/),V=F?F[1].replace(/['"]/g,""):"reconciliation.xlsx",W=URL.createObjectURL(O),X=document.createElement("a");X.href=W,X.download=V,document.body.appendChild(X),X.click(),document.body.removeChild(X),URL.revokeObjectURL(W)}catch(J){window.showToast&&window.showToast("Export error: "+J.message,"error")}}function q(k,$){const j=l("brv2-summary-collapse");let J=l("brv2-warnings");const O=window._currentLang||"zh",y={zh:"⏭ 已跳过无法识别的文件:",th:"⏭ ข้ามไฟล์ที่อ่านไม่ได้:",en:"⏭ Skipped unreadable file:",ja:"⏭ 読み取れないファイルをスキップ:"}[O]||"⏭ ",F=[];if(($||[]).forEach(V=>F.push(y+" "+V)),(k||[]).forEach(V=>F.push(V)),!F.length){J&&(J.style.display="none");return}if(!J)if(J=document.createElement("div"),J.id="brv2-warnings",j&&j.parentNode)j.parentNode.insertBefore(J,j);else return;J.style.cssText="display:block;margin:10px 0;padding:10px 14px;background:#FEF3C7;border:1px solid #FCD34D;border-radius:8px;color:#92400E;font-size:13px;line-height:1.6",J.innerHTML=F.map(V=>"<div>"+c(V)+"</div>").join("")}function z(k){x(k),q(k.warnings||[],k.skipped_files||[]),!k.ok&&k.error&&window.showToast&&window.showToast(k.error,"error");const $=k.stats||{},j=k.summary||{},J=$.matched||0,O=($.gl_debit_only||0)+($.gl_credit_only||0),y=($.stmt_withdrawal_only||0)+($.stmt_deposit_only||0),F=Number(j.formula_diff||0),V=Math.abs(F)<.05;l("brv2-kpi-matched")&&(l("brv2-kpi-matched").textContent=J),l("brv2-kpi-diff")&&(l("brv2-kpi-diff").textContent=d(F)),l("brv2-kpi-unmatched")&&(l("brv2-kpi-unmatched").textContent=O+y);const W=l("brv2-kpi-diff-icon");W&&(W.style.background=V?"#d1fae5":"#fee2e2",W.style.color=V?"#065f46":"#b91c1c");const X=l("brv2-formula-sub");if(X){const pe=window._currentLang||"zh";X.textContent=V?{zh:"✓ 平衡",th:"✓ สมดุล",en:"✓ Balanced",ja:"✓ 一致"}[pe]||"✓ 平衡":({zh:"差 ",th:"ต่าง ",en:"Diff ",ja:"差 "}[pe]||"差 ")+d(F)}const de=l("brv2-detail-sub");if(de){const pe=window._currentLang||"zh",fe={zh:"共 {n} 行",th:"ทั้งหมด {n} แถว",en:"{n} rows",ja:"計 {n} 行"}[pe]||"共 {n} 行";de.textContent=fe.replace("{n}",i.length)}function ie(pe,fe,me){const ge=l(pe);ge&&(ge.textContent=(me&&fe>0?"(":"")+d(me?-fe:fe)+(me&&fe>0?")":""))}ie("brf-gl-close",j.gl_closing||0),ie("brf-open-diff",j.opening_diff||0),ie("brf-gl-debit-only",j.gl_debit_only_amount||0,!0),ie("brf-gl-credit-only",j.gl_credit_only_amount||0),ie("brf-stmt-wd-only",j.stmt_withdrawal_only_amount||0,!0),ie("brf-stmt-dep-only",j.stmt_deposit_only_amount||0),ie("brf-calc-close",j.formula_stmt_closing||0),ie("brf-stmt-close",j.stmt_closing||0),l("brf-diff")&&(l("brf-diff").textContent=d(F));const ce=l("brv2-fcell-diff");ce&&ce.classList.toggle("brv2-fcell-diff-ok",V);const ae=l("brv2-export-btn");ae&&(ae.onclick=()=>{o&&b(o.task_id)}),R(j),M(!0),Y()}function Y(){const k=l("brv2-tbody");if(!k)return;const $=i.filter(y=>s==="all"?!0:s==="matched"?y.match_status==="matched":s==="gl_only"?y.match_status.startsWith("gl_"):s==="stmt_only"?y.match_status.startsWith("stmt_"):!0);if($.length===0){const y={zh:"无记录",th:"ไม่มีรายการ",en:"No rows",ja:"行なし"}[window._currentLang||"zh"]||"无记录";k.innerHTML=`<tr><td colspan="10" style="text-align:center;padding:20px;color:var(--ink-3)">${y}</td></tr>`;return}const j=window._currentLang||"zh",J={zh:"OCR 余额验证未通过 · 上一行余额 ± 金额 ≠ 本行余额，请核对原 PDF",th:"การตรวจสอบยอดคงเหลือไม่ผ่าน · ยอดก่อนหน้า ± จำนวน ≠ ยอดบรรทัดนี้ โปรดตรวจสอบ PDF ต้นฉบับ",en:"Balance check FAILED · prev_balance ± amount ≠ this row balance — verify against the original PDF",ja:"残高検証エラー · 前残高 ± 金額 ≠ この行残高 — 元のPDFと照合してください"}[j],O={zh:"OCR 低置信度 · 数字模糊或难以辨认，请核对原 PDF",th:"OCR ความมั่นใจต่ำ · ตัวเลขเบลอหรืออ่านยาก โปรดตรวจสอบ PDF ต้นฉบับ",en:"OCR low confidence · digit was blurry or hard to read — verify against the original PDF",ja:"OCR信頼度低 · 数字がぼやけている — 元のPDFと照合してください"}[j];k.innerHTML=$.map(y=>{const F=y.match_status,V=y.match_layer;let W="",X="";F==="matched"?(V===1&&(W="matched",X='<span class="brv2-status-badge brv2-badge-matched">L1 ✓</span>'),V===2&&(W="matched-l2",X='<span class="brv2-status-badge brv2-badge-matched-l2">L2 ~</span>'),V===3&&(W="matched-l3",X='<span class="brv2-status-badge brv2-badge-matched-l3">L3 ?</span>')):F==="gl_debit_only"||F==="gl_credit_only"?(W="gl-only",X='<span class="brv2-status-badge brv2-badge-gl-only">GL</span>'):(W="stmt-only",X=`<span class="brv2-status-badge brv2-badge-stmt-only">${{zh:"账单",th:"บัญชี",en:"Stmt",ja:"明細"}[j]||"账单"}</span>`);let de="";return y.stmt_balance_ok===!1&&(de+=`<span class="brv2-ocr-warn brv2-ocr-warn-bal" title="${c(J)}">⚠</span>`,W+=" brv2-row-warn"),y.stmt_confidence==="low"&&(de+=`<span class="brv2-ocr-warn brv2-ocr-warn-conf" title="${c(O)}">◌</span>`,W.includes("brv2-row-warn")||(W+=" brv2-row-warn-soft")),`<tr class="${W.trim()}">
              <td>${X}${de}</td>
