...
formatDate 是全站唯一日期出口(显示/表格/CSV/PDF 全走它),用户可见历法固定佛历 พ.ศ.(公历年 +543)。
这里用 esbuild 把 TS 转出后在 node 内跑,断言:佛历、各样式、空/非法输入。
...
const { formatDate, formatDateTime, setCalendar, getCalendar } = mod.exports;
...
eq(formatDate(new Date(2026, 5, 6), { calendar: 'gregorian' }), '2569-06-06', 'Date input');
console.log('OK');
"""
