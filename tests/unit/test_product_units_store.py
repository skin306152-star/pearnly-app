...
class UnitsIsolationTests(unittest.TestCase):
    def test_every_statement_scopes_to_tenant_and_product(self):
        for fn in (
            lambda c: dal.list_units(c, tenant_id="t-1", product_id="p-1"),
            lambda c: dal.get_unit(c, tenant_id="t-1", product_id="p-1", unit_id="u1"),
            lambda c: dal.update_unit(
                c, tenant_id="t-1", product_id="p-1", unit_id="u1", fields={"barcode": "x"}
            ),
            lambda c: dal.delete_unit(c, tenant_id="t-1", product_id="p-1", unit_id="u1"),
        ):
...
class CreateUnitTests(unittest.TestCase):
    def test_insert_parameterized_and_decimal(self):
        ...
        self.assertEqual(cur.last_params[1], "p-1")
...
class UpdateDeleteTests(unittest.TestCase):
    def test_update_no_fields_returns_current(self):
        cur = FakeCursor(fetchone=ROW)
        dal.update_unit(cur, tenant_id="t-1", product_id="p-1", unit_id="u1", fields={})
    ...
    def test_delete_is_hard_delete(self):
        ...
        ok = dal.delete_unit(cur, tenant_id="t-1", product_id="p-1", unit_id="u1")
        self.assertTrue(ok)
