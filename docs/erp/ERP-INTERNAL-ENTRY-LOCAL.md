...
## 2026-09-16 原票日期、金额及收付款修复候选
- ERP OCR保留原票日期，限定读取完整票面年份；记录明确实收金额，付款方式本身不当作已收。金额采用Decimal，原票含税总额不重复加税，折扣同步正式销售单。
- LINE手动编辑器可选择当前用户授权账套；切换清除旧商品关联。无买方散客票号也防重，已完成卡片明确回复状态。Cowork保持原识别行为。
- 本地真实OCR及浏览器保存：采购1125、税73.60、实付1125、45件/成本1051.40、30/06/2568；销售3806、税248.99、折扣200、实收3806、13件、24/05/2569。正式单与网页及收发存逐项回读，两个账套隔离。证据在忽略目录 `.local/incident-20260916/PAYMENT_VERIFICATION.md`。
- 定向134测试及11subtests、后续43项OCR/日期测试通过；真实浏览器确认侧栏和采购页面均为“采购记录”。销售月份去掉时代前缀、保留佛历年份。
- 用户最新明确：线上历史错单保持原样，不更正采购、不作废销售、不冲回历史库存。之前纠正提案取消。
- 限制：OCR公司名称仍有误读，税号匹配与人工编辑仍必要；本地模拟器接真实后端/数据库，不代表LINE手机授权与唤起已验收。完整发布闸与部署身份待本次发布补记。

### 本候选正式发布完成
- 发布 `c5fcee41785d9d1ca6117e700da6aa3ae4008df9`，Manual CD35095646269成功；Web/Worker c5fcee41785d-s3 Ready各100%，同digest85aa9fe0b698。完整身份见部署账本。
- 1196模块/6分片及完整pre-push通过；正式运行SHA与4个页面资源逐字节核对通过。本地浏览器采购侧栏/标题“采购记录”，销售月份“2569年5月”。
- 旧单纠正取消，未更改历史采购、销售及库存影响；不声称OCR名称或LINE真机全部通过。盘点页仅词典缓存引用变化，未重跑扫码E2E，已有台账按当日记载边界。
