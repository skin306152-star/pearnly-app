...
def inv(num, fname, title, expected, buyer, items, inv_no, date, payment):
    return g.Invoice(
        num, fname, "adversarial", title, expected, "推送(商品落库压测)",
        OWN, buyer, items, inv_no, date, payment=payment,
    )
...
def build():
    ...
    S.append(inv(
        "A1", "A1_repeat_and_variants", "重复+近似变体",
        "短名精确同名应复用昨日PN;长名/带前缀变体会另建", BUYERS["garage"],
        [
            ...
            I("PERFORMA NGV 10W-40 ( 10 / 1 L )", 2, 185),      # 空格变体 → _norm 归一应复用 PN00020
            I("performa ngv 10w-40 (10/1l)", 1, 185),           # 大小写变体 → 应复用 PN00020
            I("ผ้าเบรกหน้า", 4, 320),                           # = PN00029 → 应复用
            I("ผ้าเบรคหน้า", 4, 320),                           # ค/ก 拼写差异 → 视为不同,另建
        ],
        "HS6901-101", (2025, 12, 24), "เงินสด",
    ))

    # A2 — 超 40 字符长名 + 共享前 40 碰撞
    common = "ชุดซ่อมระบบหัวฉีดคอมมอนเรลดีเซลพร้อมอะไหล่แท้เบอร์"  # 长(>40)公共前缀
    S.append(inv(
        "A2", "A2_long_name_truncation", "超长品名截断碰撞",
        "前40字符相同的两个长名→STKDES截断后同名(应暴露截断去重缺陷)", BUYERS["fleet"],
        [
            I(common + " A-001", 1, 4500),                      # 截断后 = common[:40]
            I(common + " B-002", 1, 4800),                      # 截断后同上 → 商品库两条同名不同码
            I("น้ำมันเครื่องสังเคราะห์แท้ 100% เกรดส่งออกสำหรับเครื่องยนต์ดีเซลคอมมอนเรล", 2, 1250),  # 单条超长
            I("ค่าบริการตรวจเช็คและบำรุงรักษาเครื่องยนต์ตามระยะ 40000 กิโลเมตร", 1, 3500),  # 长服务名
            I("แบตเตอรี่รถยนต์ 12V 70Ah", 2, 2850),             # = PN00032 → 应复用
            I("ไส้กรองโซลินอยด์ Bosch 0445110xxx", 1, 1900),    # 型号/特殊字符
        ],
        "HS6901-102", (2025, 12, 25), "เงินเชื่อ",
    ))

    # A3 — 特殊字符/数字/型号密集(测字段宽度与编码)
    S.append(inv(
        "A3", "A3_special_chars_models", "特殊字符型号密集",
        "斜杠/×/#/% 与型号码不应破坏建档", BUYERS["garage"],
        [
        ...
        "HS6901-103", (2025, 12, 26), "เงินสด",
    ))

    # A4 — 中泰英混排 + 长名 + 服务行(贴近真实杂单)
    S.append(inv(
        "A4", "A4_mixed_lang_services", "中泰英混排+服务",
        "多语言/长服务名应原样落库(截断到40)", BUYERS["taxi"],
        [
            ...
            I("น้ำมันเครื่อง PTT DYNAMIC PREMIER (10/1L)", 5, 215),  # >40 → 测是否再重复(对照昨日 PN00031/36)
            I("จารบีอเนกประสงค์", 6, 85),                       # = PN00030(注:昨日误建成 STKTYP=0)
            I("ค่าแรงช่าง (ชั่วโมง)", 8, 250),
        ],
        "HS6901-104", (2025, 12, 27), "เงินเชื่อ",
    ))
    ...
    S.append(inv(
        "A5", "A5_bulk_14_lines", "大批量14行",
        "多行(部分复用+多新建)·测多次ensure_item与STCRD多行", BUYERS["fleet"],
        items, "HS6901-105", (2025, 12, 28), "เงินสด",
    ))
    return S
...
def _write_readme(scen):
    ...
    existing = {
        "performangv10w-40(10/1l)": "PN00020", "น้ำกลั่น": "PN00019",
        "pttdynamicpremier(10/1)": "PN00027", "automat(10/1l)": "PN00028",
        "ผ้าเบรกหน้า": "PN00029", "แบตเตอรี่รถยนต์12v70ah": "PN00032",
        "pttv-120d#40(10/1l)": "PN00025", "v-120b#40(10/1l)": "PN00026",
        "performasemisyn(6/4l)": "PN00024", "หัวเชื้อดีเซล(24/0.17)": "PN00023",
        "น้ำยาลดความร้อน": "PN00022", "brakeextra(12/0.5l)": "PN00021",
