...
## 🎯 状态卡(2026-06-01 · **整顿主线 C3 进行中** · R15~R19 抽 6 modal + 删 1 死 modal · home.html 1495→1201 上线 prod 11850057)

- **模式**:整顿封锁期 · **整顿主线 C3 进行中**(Zihao 2026-06-01 拍板:DMS 全链路收尾闭环 → 回整顿)。**当前 task = C3 拆 home.html → <1000**:本窗口 R15~R19 后 **1201**(还差 ~201)。下个候选:`email-modal`~155〔牵涉 `line-email-modal.js` 🔴高敏·跳过〕/ `confirm-modal`+`pearnly-confirm-modal`(showConfirm 通用·**eval 早绑可能性高·先查绑定时机**)/ page-ocr(~150·识别主页高耦合·留最后)/ 其余零散 on-demand modal。**app shell(顶栏~128/侧栏~143)boot 关键勿轻动**。孤儿 i18n key(billing-modal-title/help/google-open)R19 删 markup 后留存·无害·可日后清(需 bump i18n-data.js?v=)。
...
- **C3 进度封存**:home.html 1730→**1201**(R13 `#page-history` + R14 `#onboarding-modal` + R15 `#bank-cand-drawer` `277b377`(-22) + R16 `#client-modal-mask`+`#wsclient-modal-mask`→`modal-client-edit.js` `bf42884`(-98) + R17 `#camera-tips-modal`+`#bank-client-picker-modal` `d773424`(-51) + R18 `#endpoint-modal`→`endpoint-modal-html.js` `6264f2a`(-77·⚠️erp-integration 无守卫 IIFE·import 严置其前) + **R19 删死 markup `#billing-modal-mask`(BYO-key 计费遗留·全仓零消费者) `83099ee`(-46)**·抽出均字节等价〔脚本提取+校验模板===git HEAD inner〕+prod E2E 验)。**R19 纯删 home.html=不动 bundle·免 build/免 bump ?v=**(home.html 实时下发)。范式见记忆 [[wb-src-home-c3-split-playbook]](留空壳 + `*-html.js`/`modal-*.js` eval 期注入 inner + 子树初译 + import 置消费模块前;消费模块 load()/_bindOnce/DOMContentLoaded 守卫绑定→eval 注入恒早于首次绑定·非竞态)。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。范式见记忆 [[wb-src-home-c3-split-playbook]](留空壳 + `*-html.js`/`modal-*.js` eval 期注入 inner + 子树初译 + import 置消费模块前;消费模块 load()/_bindOnce/DOMContentLoaded 守卫绑定→eval 注入恒早于首次绑定·非竞态)。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。
...
- **最后 commit**:`83099ee`(C3 R19 删死代码 `#billing-modal-mask` · REFACTOR-WB-C3 · 纯删 home.html·版本仍 11850057)· `git log --oneline -8` 查最新。**均已 push 上线·prod 11850057 健康**。**整顿轮 cache-bust:仍 bump `?v=` 保缓存正确,但不重写 release_notes(Zihao 2026-06-01:纯重构忽视新版本横幅·见记忆 [[refactor-skip-version-banner]])**。home.js 拆迁全收官 33768→0(范式 [[home-js-batch-split-bridge]])。
