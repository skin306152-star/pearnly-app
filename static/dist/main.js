...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function n($,x){let b=t($)||$;if(x)for(const q in x)b=b.replace(new RegExp("\\{"+q+"\\}","g"),String(x[q]));return b}async function a(){try{const $=e.currentClient||"",x="/api/exceptions/stats?status=pending"+($?"&client_id="+encodeURIComponent($):""),b=await fetch(x,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!b.ok)return;const q=await b.json(),z=document.getElementById("nav-exc-badge");if(!z)return;const Y=parseInt(q.pending||0,10);Y>0?(z.textContent=Y>99?"99+":String(Y),z.style.display=""):z.style.display="none"}catch{}}function o($){return $==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        ...
        `;const O=document.getElementById("exc-fld-edit");O&&O.addEventListener("click",()=>{v.editing=!0,v.editFields={..._(v.history)},H()});const y=document.getElementById("exc-fld-cancel");y&&y.addEventListener("click",()=>{v.editing=!1,v.editFields=null,H()});const F=document.getElementById("exc-fld-save");F&&F.addEventListener("click",()=>D()),document.querySelectorAll(".exc-field-input").forEach(ae=>{ae.addEventListener("input",()=>{v.editFields||(v.editFields={}),v.editFields[ae.dataset.editKey]=ae.value})});const W=document.getElementById("exc-pdf-retry");W&&v.openExcId&&W.addEventListener("click",()=>{v.excRow&&S(v.excRow.history_id,v.openExcId)});const X=$.status==="pending",de=!!($.seller_name&&$.seller_name.trim()),ie=document.getElementById("exc-btn-resolve"),ce=document.getElementById("exc-btn-ignore");ie.disabled=!X,ce.disabled=!X||!de,ce.title=de?t("exc-ignore-hint"):t("exc-ignore-no-seller")}async function D(){if(!v.openExcId||!v.history||!v.history.pages||v.loading)return;v.loading=!0;const $=showToast(t("exc-fld-saving"),"loading",0);try{const x=JSON.parse(JSON.stringify(v.history.pages||[]));let b=x.findIndex(re=>!re.is_duplicate&&!re.is_copy);b<0&&(b=0),x[b]||(x[b]={fields:{}});const q=x[b].fields||{},z=v.editFields||{},Y=new Set(["subtotal","vat","total_amount"]),Z={...q};for(const re in z){let K=z[re];if((K===""||K===void 0)&&(K=null),Y.has(re)&&K!==null){const ee=parseFloat(K);K=isNaN(ee)?null:ee}Z[re]=K}x[b].fields=Z;const le=await fetch("/api/history/"+encodeURIComponent(v.history.id),{method:"PUT",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({pages:x})});if(!le.ok)throw new Error("http "+le.status);$(),showToast(t("exc-fld-save-ok"),"success"),A(),await u(),await w(),a()}catch(x){$(),console.warn("save fields fail",x),showToast(t("exc-fld-save-fail"),"error")}finally{v.loading=!1}}async function R(){if(!(!v.openExcId||v.loading)){v.loading=!0;try{const $=await fetch("/api/exceptions/"+v.openExcId+"/resolve",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!$.ok)throw new Error("http "+$.status);showToast(t("exc-toast-resolved"),"success"),A(),await u(),await w(),a()}catch($){console.warn("resolve fail",$),showToast(t("exc-toast-action-fail"),"error")}finally{v.loading=!1}}}async function I(){if(!(!v.openExcId||v.loading)){v.loading=!0;try{const $=await fetch("/api/exceptions/"+v.openExcId+"/ignore",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!$.ok)throw new Error("http "+$.status);showToast(t("exc-toast-ignored"),"success"),A(),await u(),await w(),a()}catch($){console.warn("ignore fail",$),showToast(t("exc-toast-action-fail"),"error")}finally{v.loading=!1}}}let P=!1;async function U(){if(P)return;const $=Array.from(e.selectedIds);if($.length===0||!await showConfirm(n("exc-batch-confirm-resolve",{n:$.length})))return;P=!0;const b=showToast(n("exc-batch-count",{n:$.length})+" …","loading",0);try{const q=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:$,action:"resolve"})});if(!q.ok)throw new Error("http "+q.status);const z=await q.json();b(),showToast(n("exc-toast-batch-resolved",{n:z.processed||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(q){b(),console.warn("batch resolve fail",q),showToast(t("exc-toast-batch-fail"),"error")}finally{P=!1}}async function G(){if(P)return;const $=Array.from(e.selectedIds);if($.length===0||!await showConfirm(n("exc-batch-confirm-ignore",{n:$.length}),{danger:!1}))return;P=!0;const b=showToast(n("exc-batch-count",{n:$.length})+" …","loading",0);try{const q=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:$,action:"ignore"})});if(!q.ok)throw new Error("http "+q.status);const z=await q.json();b(),showToast(n("exc-toast-batch-ignored",{n:z.processed||0,wl:z.whitelist_added||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(q){b(),console.warn("batch ignore fail",q),showToast(t("exc-toast-batch-fail"),"error")}finally{P=!1}}function N(){e.selectedIds.clear(),d(e.listCache)}document.addEventListener("click",$=>{$.target.closest("#exc-drawer-close")&&A(),$.target.closest("#exc-drawer-mask")&&A(),$.target.closest("#exc-btn-resolve")&&R(),$.target.closest("#exc-btn-ignore")&&I(),$.target.closest("#exc-batch-resolve")&&U(),$.target.closest("#exc-batch-ignore")&&G(),$.target.closest("#exc-batch-clear")&&N(),$.target.closest("#exc-loadmore")&&C()}),document.addEventListener("keydown",$=>{$.key==="Escape"&&v.openExcId&&A()}),document.addEventListener("click",$=>{$.target.closest("#btn-exc-refresh")&&(typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a())}),document.addEventListener("change",$=>{if(!$.target.closest("#exc-client-filter"))return;const x=$.target;e.currentClient=x.value||"",e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a()}),document.addEventListener("click",$=>{const x=$.target.closest("#exc-status-tabs .exc-status-tab");if(!x)return;const b=x.dataset.status||"pending";b!==e.currentStatus&&(e.currentStatus=b,e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage())}),window.addEventListener("online",()=>{e.loadFailed&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage&&window.loadExceptionsPage()}),setTimeout(a,1500),setInterval(a,6e4);function Q($){if(!$)return"—";try{const x=new Date($),b=q=>String(q).padStart(2,"0");return`${x.getFullYear()}-${b(x.getMonth()+1)}-${b(x.getDate())} ${b(x.getHours())}:${b(x.getMinutes())}`}catch{return $.slice(0,16).replace("T"," ")}}async function oe(){const $=document.getElementById("learned-list");if($){$.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-loading"))}</div>`;try{const x=await fetch("/api/exception-whitelist",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);const q=(await x.json()).items||[];if(q.length===0){$.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-empty"))}</div>`;return}const z=`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                ...
                <span class="ex-reason" title="${escapeHtml($)}">${escapeHtml($)}${N.error_code?` <span class="erp-exc-code">${escapeHtml(N.error_code)}</span>`:""}</span>
