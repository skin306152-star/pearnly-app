...
## 🎯 状态卡（2026-06-27 · **B8 RLS · 孤儿 re-enroll 连推 3 域 enroll 上线:sales(`8a7d8485`)+ suppliers(`4cc03b42`)+ line 绑定(`cd3cc12d`)**· CI 6 闸全绿 + prod 金丝雀 PASS · companion 未动 · 交接 `docs/refactor/b8-rls-HANDOFF-2026-06-25.md` §7.11/§7.12/§7.13）

- **★ 孤儿 re-enroll · line 绑定域 enroll 上线 prod 验过**（`cd3cc12d`）：`line_bindings`/`line_binding_codes` 补回 policy。**★模板纠偏**:INCIDENT §2 误归 tenant_or_user,实测两表只有 user_id 无 tenant_id → 改纯 `apply_user_rls`(同 email_ingest)。新 `ensure_line_binding_rls`(逐表验存在·force=False)进 boot_ensures、dal_reexports 暴露。**零业务代码改动**:store 全 owner(LINE webhook 无登录态·靠 line_user_id 反查·穿不进 user 上下文)→ 必须保留 owner bypass·policy 仅二道防线。集成 3 例 + 4973 单测 + CI 6 闸绿（run `28281294683`）；金丝雀:两表 rls=on/npol=1·`line_binding_codes` owner=113/真用户=53/假 0。**教训:模板永远按真实列验,别信 INCIDENT §2 分类。**
- **★ 孤儿 re-enroll · suppliers 域 enroll 上线 prod 验过**（`4cc03b42`）：`supplier_categories`/`buyer_to_client_memory`（均 tenant_or_user·tenant_id 可空+user_id 兜底·同 clients）补回 policy。enroll 落进**现有** `ensure_supplier_categories_table`/`ensure_buyer_to_client_table`（已在 boot_ensures）→ 无新 ensure_。同步迁裸 get_cursor:supplier_categories 4 DAL + buyer_to_client_memory 唯一裸写 `learn_buyer_to_client`（`try_resolve` 早是 RLS）穿 tenant+user。`supplier_client_mapping` 代码无 CREATE/访问点(仅 BACKLOG)→ 跳过。集成 4 例 + 4973 单测 + CI 6 闸绿（run `28280969279`）；金丝雀：两表 rls=on/npol=1·`buyer_to_client_memory` owner=77/真租户=19/假 0（per-tenant 切片）。
- **★ 孤儿 re-enroll · sales 域 enroll 上线 prod 验过**（`8a7d8485`）：`sales_documents`/`_lines`/`_sends`/`sales_settings`/`document_number_sequences` 补回 `tenant` policy。新 `services/sales/schema.py` `ensure_sales_rls`（逐表验存在·force=False）进 `boot_ensures`、`dal_reexports` 暴露。坑：sales_documents 账套列名 `seller_workspace_client_id`、连号计数器 workspace_client_id 运行时加且可空 → 不用 tenant_ws（账套强隔离靠 `uq_dns_ws`）。DAL 早全走 get_cursor_rls → 零访问点改动。金丝雀：5 表 rls=on/npol=1·真租户 sales_documents 34/假 0。**剩余孤儿按域续做**（line/knowledge/products/etax/automation/settings 杂项·见交接 §7.4）。
