...
> **当前态(2026-05-27 · 持续更新 · 含主控换窗口)**:① E2E 安全网 10 个(env-gated skip 保 CI 绿)。② **home.js batch**:R1-R7 抽 **28 块** → home.js **22970→10279**(`src/home/*` · 全接线 · 每轮生产 E2E 10/10 + 守门 6 道全绿)。③ **home.css batch(并行)· ✅ 收官**:R1-R8 抽 **36 个组件 css** → home.css **16673→0**(全抽尽 · 文件留空壳 · `static/home-*.css` · 每轮字节铁证「拼接==原始 home.css」482214 bytes + 生产 sha256 校验 · R8 收官批由主控验毕提交 `0818691` · CI success)。④ db.py membership+tenant copy-out 未接线。⑤ **home.js 窗口 R7 收尾(干净轮)· master 全绿**。**⚠️ 干净单 IIFE 基本抽尽**:剩多为顶层函数群(erp-endpoints-logs/automation/early 区 routeTo 中枢/export/settings)+ 🔴高敏 4 块 + LINE Bot 绑定面板(敏感)+ team(borderline)+ _shared 收官 → **下一步多需 Zihao 在场/主控判性质**(见 §13 头部「R7 后地图」· 拿不准就报)。
...
> ════════════════════════════════════════════════════════════
> **【第四十会话 · 交接 · 2026-05-27】home.js R6+R7:并行抽 7 块 safe IIFE(12929→10279 · -2650)· 每轮 6 道守门 + 生产 E2E 10/10 · 干净 IIFE 近抽尽**
> ════════════════════════════════════════════════════════════
> **本会话(整顿 REFACTOR-C1 · home.js batch 续 · R6+R7 两轮完整 · 含流程补丁 §9.5#8 落实)**:
...
>   R6 字节级:纯删 1031 行 0 插入(CRLF 完好)· 守门全绿 pytest 1131 · 生产 E2E 10/10 · 部署 11850016。
>
> **下窗口接续(R8 · ⚠️ 干净 IIFE 近抽尽 · 进入"判性质"阶段)**:§13 头部「**R7 后地图**」已写明 —— 剩下基本没有"干净单 IIFE"了,多是:① **顶层函数群**(erp-endpoints-logs/automation `loadErpEndpoints`/`loadErpLogs`/`loadAutomationPage` · early 区 chrome-quota/upload-camera/ocr-run/results-drawer/rd-sync/history/settings · export L4392)—— 搬顶层函数前**必须 grep 确认它如何被 routeTo/其它模块引用**(bare 名 vs `window.`),bare 引用的搬走会断;② **sidebar-routing/routeTo 中枢** = 地板,Zihao 在场;③ 🔴高敏 4 块 + LINE Bot 绑定面板(敏感)+ team(员工改密 borderline)= 别碰;④ `_humanizeBackendError`(L6162 顶层全局 · bank-recon-v2 module 依赖)别动。**建议下窗口先用 Explore/Agent 把 early 区 + erp-endpoints 区重新测绘"哪些还是可安全 copy-out 的独立 IIFE",拿不准就报 Zihao;真到顶层函数群/routeTo 就交回 Zihao 在场做。** 抽前必 re-grep(R7 后行号 -1619 漂移)。
>
> ════════════════════════════════════════════════════════════
