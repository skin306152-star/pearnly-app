...
锁定(防搬迁回归):
  1. router 注册的 15 条路由 path+method 契约不变(防丢路由 / 改 URL)
     · REFACTOR-WA-B4 新增 /api/ready 真探活端点(铁律 #23.7)
     · PO-B3 新增 /pos · /pos/{rest:path} 收银前台 SPA 页面路由
  2. app.py 通过 include_router 真挂上了全部 15 条(防漏挂)
...
class PagesRoutesContractTests(unittest.TestCase):
    def test_router_registers_expected_routes(self):
        """15 条路由 path+method 契约 · 防搬迁丢路由 / 改 URL"""
        got = set()
    ...
    def test_app_includes_pages_router(self):
        """防 include_router 漏挂 · app 必须能路由到全部 15 条"""
        import app
