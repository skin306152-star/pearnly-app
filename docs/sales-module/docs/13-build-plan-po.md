...
- [x] PO-1(2026-06-06 · schema 上线 + prod 库 apply)  - [x] PO-2(商品 CRUD)  - [x] PO-3(Excel 导入)  - [x] PO-4(开票核心 · 连号/不可改/算税 · 真账号验)  - [x] PO-5(红冲/补开 · 真账号验)  - [x] PO-6(合规 PDF + 账套主体开票资料 · 真账号验 + 实物 PDF)  - [ ] PO-7
- [ ] PO-8  - [ ] PO-9  - [ ] PO-10  - [ ] PO-11  - [ ] PO-12  - [ ] PO-13  - [ ] PO-14  - [ ] PO-15

> **2026-06-06 大进展**:Phase 1 后端基本通(PO-1~6 + 账套主体开票字段 UI)· 全部 master 上线 + 真账号 E2E。
> **2026-06-06(二)买方模块 + 合规四块后端**(commit `426caee` · **未 push · 待 prod apply + E2E**):
> 买方动态模块(`docs/15`)+ §A 双方冻结 + §B 完整性闸 + §J 合并单/收款 + D 折扣 + E1/E2 纸张正副本
> + G 日期历法。迁移 0009~0011。本地全量 2399 单测 + 守门绿;**prod 应用与真账号 E2E 待 Zihao**
> (prod 不自动迁移·须先 `alembic upgrade head` 再 push·否则开票端 500)。详见 `STATE.md` 顶部 2026-06-06 节。
> 买方块前端只出了**草稿**(`桌面/sales-buyer-block-draft.html`·逻辑已认可·**样式等最终设计稿**)。
> **下个窗口**:待 Zihao apply+push+E2E 回填结果 → 再 PO-10 前端(等设计稿)/ PO-7 发送 / C 价内外(默认价外·单据级)。
