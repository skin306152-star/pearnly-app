...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "系统已升级顶部右上角的切换为「工作模式」:可在个人事务与具体客户之间切换,选定客户后本次上传与处理会自动归属该客户。同时,推送到外部 ERP 成功后,可直接查看并复制 ERP 生成的单号。即日生效。",
            "th": "ระบบได้ปรับปรุงปุ่มสลับมุมขวาบนเป็น «โหมดการทำงาน»: สลับระหว่างงานส่วนตัวกับลูกค้าแต่ละรายได้ เมื่อเลือกลูกค้าแล้ว การอัปโหลดและประมวลผลครั้งนี้จะถูกจัดให้กับลูกค้านั้นโดยอัตโนมัติ นอกจากนี้ หลังส่งไปยัง ERP ภายนอกสำเร็จ สามารถดูและคัดลอกเลขที่เอกสารที่ ERP สร้างได้ทันที มีผลทันที",
            "en": "The top-right switcher has been upgraded to a Work Mode control: switch between personal work and a specific client, and once a client is selected this session's uploads and processing are attributed to that client automatically. In addition, after a successful push to an external ERP you can now view and copy the document number generated by the ERP. Effective immediately.",
            "ja": "右上の切り替えが「作業モード」に進化しました:個人業務と特定クライアントを切り替えられ、クライアントを選択すると今回のアップロードと処理が自動的にそのクライアントに帰属します。また、外部 ERP への送信成功後、ERP が生成した伝票番号を表示・コピーできるようになりました。即日有効。",
        },
    }
