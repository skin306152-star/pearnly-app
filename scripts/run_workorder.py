...
import psycopg2  # noqa: E402 - --watch 观测线程用独立直连(不借非线程安全的连接池)
...
def _watch_progress(tenant_id: str, work_order_id: str, stop: threading.Event) -> None:
    """独立直连轮询 current_step + 已识别张数,滚动打印真实进度。

    ⚠️ 用独立直连(不借跑批线程共用的 SimpleConnectionPool——它非线程安全,并发借还会
    race 到池内部状态,拖累真正的跑批)。连接建不上就跳过观测,绝不拖累跑批。
    引擎每进一步先独立提交 current_step,故这里能立刻看到。
    """
    from services.workorder import evidence

    try:
        conn = psycopg2.connect(
            db._get_database_url(), sslmode=os.environ.get("PGSSLMODE", "require")
        )
        conn.autocommit = True
    except Exception as e:  # 观测层建连失败不影响跑批
        print(f"  (进度观测连接失败,跳过 --watch:{e})", flush=True)
        return
    ...
    try:
        while not stop.wait(3.0):
            ...
            if not wo:
                continue
            # replay 按 item 去重(续跑同件二次 classify 不重复计),与 reconcile 回放同口径。
            classified = len(evidence.replay_items_by_type(events, "item_classified"))
            line = f"  … {wo['status']}/{wo['current_step']} · 已识别 {classified} 张 · 事件 {len(events)}"
