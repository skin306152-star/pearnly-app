...
(async () => {
    ...
    for (const job of jobs) {
        const src = fs.readFileSync(job.path, 'utf8');
        const ok = await run(src, [], []);
        R[job.name] = {
            ok: ok,
            // 解码器 404(部署漏传 / CDN 还没热)时的表现
            flaky: await run(src, ok.cached.filter((u) => u.includes('scan.js')), []),
            // 换了版本号:旧缓存必须被清掉,当前这个不许动
            evict: await run(src, [], ['stale-cache-v0', ok.opened[0]]),
            // 部署重启那十几秒 nginx 对外壳回 502,而这台机器里已经躺着一份上一版的缓存。
            // 上一轮只跑过「解码器 404 且 existingKeys 为空」——那两个条件恰好都躲开了这条路。
            shellDown: await run(src, [job.shell], ['stale-cache-v0', ok.opened[0]]),
        };
    }
...
class PosSwPrecacheTest(unittest.TestCase):
    ...
    def test_activate_evicts_stale_versions_only(self):
        ...
        for name in WORKERS:
            self.assertEqual(self.r[name]["evict"]["deleted"], ["stale-cache-v0"])
