...
## 🎯 状态卡(2026-06-01 · ✅ **MR.ERP DMS 全链路闭环上线**(prod 11850053)· **下个窗口恢复整顿主线 C3 拆 home.html**)

- **模式**:整顿封锁期 · **整顿主线恢复**(Zihao 2026-06-01 拍板:DMS 全链路收尾闭环 → 回整顿)。**下个窗口 task = C3 拆 home.html 1495→<1000(还差 ~495)**。
...
- **C3 进度封存**:home.html 1730→**1495**(本窗口 R13 抽 `#page-history`→`src/home/page-history.js` `4d3b9e0` + R14 抽 `#onboarding-modal`→`src/home/welcome-wizard-html.js` 懒注入 `99c5da4`·均字节等价+prod E2E 验)。**下个 C3 候选**(由易到难):bank-cand-drawer(~24)/ 客户编辑等 on-demand modal / page-ocr(~150·识别主页高耦合·留最后)。**app shell(顶栏~128/侧栏~143)boot 关键勿轻动**。范式见记忆 [[wb-src-home-c3-split-playbook]]。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。
...
- **最后 commit**:`63af109`(/simplify 后端清理:push_type 单一判定 + 友好码按长排序 + id_card_tail SQL 提取)· `git log --oneline -8` 查最新。**均已 push 上线·prod 11850053 健康**。home.js 拆迁全收官 33768→0(范式 [[home-js-batch-split-bridge]])。
