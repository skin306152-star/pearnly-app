...
> **最近更新**:2026-05-25(**第二十一会话 · D1 补 router 契约测试 + C1 抽测试中心**)· **🟢 已 push + CI 双 job 绿 + 生产 playwright 验证。**
...
> **全程提交只 `git add` 我自己的文件 · 没碰那批 app.py/email_ingest.py/line_client.py 改动。**
> **⚠️ 更正(Zihao 确认只有一个窗口)**:工作区里 app.py/email_ingest.py/line_client.py 的未提交改动 + 未跟踪 `services/ocr/entrypoints.py`/`test_ocr_entrypoints_contract.py` **不是"并发窗口"**,是**上一会话遗留、没提交的 OCR 重构半成品**(把 OCR helper 搬进 services/ocr/entrypoints.py · 通过全部 5 道守门 + 自带契约测试)。它改 OCR 核心路径 · 本窗口未亲自端到端验证(验它要烧真 OCR 额度)· 故**不擅自提交+部署**给付费用户(铁律 #16/#25)· 留 Zihao 拍板是否做完上线。**它一直 parked · 是 C1 续拆全体用户块的卡点**(改全体用户 home.js 块要 bump home.js?v= → 改 app.py release_notes · 而 app.py 被这批 WIP 占着)。
> **⚠️ admin-cost 抽迁已尝试并回退(未提交)**:管理员成本面板(L10940-11167)发现 **load-order 纠缠**——另一个 billing IIFE(L11490)`_orig = window.loadAdminCostPage` 装饰它 + DOMContentLoaded 直绑刷新/导出按钮 · "只 bump main.js?v="模式下老缓存超管会双绑 + 丢 billing 包裹。判定"不够干净·不上 warts"· `git checkout home.js` 回退。**教训:test-center 是少有的干净自包含块(on-demand 绑定 + 无装饰器 + ?test_center=1 可验)· 多数 home.js 块有 load-order 装饰器/裸名调/inline onclick 纠缠 · 不能简单 verbatim 抽。**
> ⚠️⚠️ **下个窗口**:① 想快推 C1 续拆 home.js → 先处理 parked OCR WIP(Zihao 决定:做完验证上线 / stash 搁置)让 app.py 空出 → 才能 bump home.js?v= + 写 release_notes 正经抽**全体用户块**(且这些块多纠缠 + 碰付费用户 UI · 建议 Zihao 在场眼验)。② 不想碰 app.py → 走**无需 app.py 的目标**:**C2 home.css(16131 行 · bump home.css?v= 不影响 /api/version → 不弹横幅 · 完全不碰 app.py)** 或 B2 db.py→services 或继续补 D 测试。③ B1 已到顶。
>
> **(第二十会话 · B1 收尾 + C1 启动)**:
