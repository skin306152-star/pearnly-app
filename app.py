...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.14 · 修 erp_endpoints CHECK 约束少了 mrerp + 加 schema migration:\n• v118.34.13 的 last_500 诊断起作用 · 用户看到 CheckViolation: adapter='mrerp' 不在 erp_endpoints_adapter_chk 白名单\n• 根因:DB 表 CHECK constraint 还是老白名单(webhook/xero/flowaccount)· 加 mrerp adapter 代码时漏了 schema migration\n• 修:\n  - 加 db.ensure_erp_endpoints_adapter_constraint() · pg_catalog 查约束 · drop+rebuild 包含 mrerp · 幂等\n  - 加 db.ensure_erp_push_logs_adapter_constraint() · 同步处理 push log 表(若也有此约束)\n  - lifespan 加调用 · 部署后第一次启动自动 migrate\n• 加守门测试 test_create_endpoint_with_adapter_mrerp_does_not_violate_constraint · 真 DB · SAVEPOINT + ROLLBACK 不污染数据 · 本地缺 DATABASE_URL 自动跳\n\n部署后再点完成 · 应该 200 + 返 endpoint id",
            "en": "v118.34.14 · Schema migration for erp_endpoints CHECK constraint to include 'mrerp':\n• v118.34.13's last_500 diagnostic paid off. Operator pulled /api/version and saw the actual traceback: psycopg2.errors.CheckViolation — new row for relation 'erp_endpoints' violates check constraint 'erp_endpoints_adapter_chk', adapter = 'mrerp'.\n• Root cause: the mrerp adapter code shipped without an accompanying schema migration. The DB's CHECK constraint only whitelisted webhook/xero/flowaccount.\n• Fix:\n  - New db.ensure_erp_endpoints_adapter_constraint() queries pg_catalog for the existing adapter CHECK, drops it, recreates with the canonical whitelist that includes 'mrerp'. Idempotent (skips if 'mrerp' is already in the constraint definition).\n  - Same for erp_push_logs (in case future push logging adds a column constraint there).\n  - Both wired into lifespan startup so the migration runs on every restart without operator intervention.\n• Added test_create_endpoint_with_adapter_mrerp_does_not_violate_constraint — real DB, SAVEPOINT/ROLLBACK so it doesn't pollute. Auto-skips when PEARNLY_DATABASE_URL isn't set (local dev).\n\nAfter deploy, retry the wizard 'Finish' button — should return 200 with the endpoint id.",
