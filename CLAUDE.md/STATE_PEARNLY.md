...
## 当前状态卡(2026-07-25 · Express 库存闭环上线 + 软删行 P0 根治)

- **主站 `3ae169fe`(CI 绿)· 小助手 `31f56be` = v1.1.53 已发版**。
- **交接全文:`docs/integrations/express-push/36-2026-07-25-库存闭环上线与软删行P0.md`** —— 接手先读它。
...
- **★ 软删行 P0**:DBF 删除是打标记,会计在 Express 删过的单**永远重推不进去还报成功**。
  三堵墙(幂等闸 / 读回校验 / 明细校验)全修,判据收口 `dbf_schema.live_rows()`。
- **六条真账套取证**(别靠推演改回去):负库存是常态 91% 带成本 · 成本来源是 `STNPR` 标准成本非均价 ·
  994 品无一先卖后买 · 科目组不能按众数选 · `STKDES` 是 C(50) 非 40 · 单位码要查客户自己的 ISTAB。
- **在做**:三档科目组(1 个自动用 / 0 个让会计建 / ≥2 个选一次)+ 导出表标出自动新建的行。
- **待办**:接 `duplicate_skipped`(区分真写了 vs 幂等跳过)· 真机 Express GUI 验一眼 ·
  重分类 SEQIT 7/8 未经真机确认。
- **血泪**:合成表 47/47 全绿而真表立刻爆 P0 → 合成表只当回归网;跨边界接缝一天断三次 →
  跨仓库字段必须有端到端测试压缝;「报成功但没东西」第 0 步是**复跑真代码**不是查环境。
