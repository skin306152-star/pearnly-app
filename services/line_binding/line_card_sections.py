...
def _display_item_name(raw, i: int, t: dict) -> str:
    """明细名展示兜底:OCR 读不出的泰文细品名常返「?????」/替换符,展示成「รายการที่ N」而非乱码。

    去掉 ?/replacement char 后仍有 ≥2 个真实字符(任意文种字母或数字)→ 用清洗后的名
    (「TW ?????」→「TW」保留已读部分);否则整名不可读 → 编号占位(t['item_n'])。
    """
    cleaned = re.sub(r"[?�]", "", str(raw or "")).strip()
    meaningful = re.sub(r"[\W_]", "", cleaned, flags=re.UNICODE)
    return cleaned if len(meaningful) >= 2 else t["item_n"].format(n=i)
