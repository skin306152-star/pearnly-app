...
(function (root) {
  ...
  function gateHtml() {
    var langbar = '<div class="langbar">' + LANGS.map(function (l) {
      return '<button type="button" class="lang-btn' + (l === state.lang ? ' on' : '') + '" data-lang="' + l + '">' + LANG_LABEL[l] + '</button>';
    }).join('') + '</div>';
    if (state.gate === 'login') {
      return langbar + '<div class="gate-card">' +
        '<h1>' + escapeHtml(t('daily.title')) + '</h1>' +
    ...
    return langbar + '<div class="gate-card">' +
      '<h1>' + escapeHtml(t(cfg[0])) + '</h1>' +
      '<p class="gate-body">' + escapeHtml(t(cfg[1])) + '</p>' +
      (cfg[2] ? '<button type="button" class="gate-retry" data-retry data-i18n="daily.gate.retry"></button>' : '') +
      (state.token ? '<button type="button" class="gate-logout" data-logout data-i18n="daily.gate.logout"></button>' : '') +
      '</div>';
  }
  ...
  function applyStaticI18n() {
    ...
    for (var k = 0; k < langBtns.length; k++) {
      langBtns[k].className = langBtns[k].getAttribute('data-lang') === state.lang ? 'lang-btn on' : 'lang-btn';
    }
  }
