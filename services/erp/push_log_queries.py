...
# 今日统计拆到 push_stats_today(500 行闸)· 此处 re-import 保 facade
from services.erp.push_stats_today import get_push_stats_today  # noqa: F401
...
def list_push_logs(
    ...
    try:
        ...
        with db.get_cursor_rls(tenant_id=tenant_id, user_id=user_id) as cur:
            ...
            # PO-4 同源 · 套账硬边界:列在已 JOIN 的 ocr_history h 上(孤儿行得 NULL → 回落仍显示)。
            count_join_h = ""
            if workspace_client_id is not None:
                outer.append("(h.workspace_client_id = %s OR h.workspace_client_id IS NULL)")
                params.append(int(workspace_client_id))
                count_join_h = " LEFT JOIN ocr_history h ON h.id = l.history_id"
            ...
            cur.execute(
                ranked_cte + f"""
                    ...
                    LEFT JOIN erp_endpoints e ON e.id = l.endpoint_id{count_join_h}
                    WHERE {outer_sql}
                """,
                tuple(params),
            )
