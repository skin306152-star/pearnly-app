...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.35.0.17 · 后台精简 + 弹窗体验升级:\n• 管理后台语言选项精简到只剩中文/泰文(其它语种没翻完就先收起,免得点了一片空白)\n• 后台调整余额时弹的输入框换成漂亮的卡片样式(原来是浏览器自带丑框)\n• 沉淀本次修复经验到团队规则:删后端字段必须同步删响应模型 · 避免再次出现『设置页一片空白』那种连环 bug",
            "th": "v118.35.0.17 · ปรับหลังบ้านให้คลีน + กล่องป๊อปอัปสวยขึ้น:\n• เมนูภาษาในหลังบ้านเหลือ ไทย/中文 (ภาษาที่ยังแปลไม่ครบเก็บไว้ก่อน เพื่อไม่ให้คลิกแล้วเจอช่องว่าง)\n• กล่องใส่ตัวเลขเวลาปรับยอดคงเหลือเปลี่ยนเป็นการ์ดในสไตล์เดียวกับระบบ ไม่ใช้กล่องเบราว์เซอร์อีกแล้ว\n• เพิ่มกฎภายในทีม: ถ้าลบ field จาก backend ต้องลบจาก response model พร้อมกัน เพื่อกัน bug 'หน้าตั้งค่าว่างเปล่า' กลับมาอีก",
            "en": "v118.35.0.17 · Admin polish + dialog upgrade:\n• Admin language picker trimmed to ZH/TH only (other locales hidden until translations are complete, so clicks no longer land on a blank screen)\n• Balance-update dialog in admin replaced the browser's plain prompt with our card-style input modal\n• Codified this week's lesson into a team rule: removing a backend field must also update the response model, to prevent the 'settings page blank' chain bug we just hit",
            "ja": "v118.35.0.17 · 管理画面の整理 + ダイアログ刷新:\n• 管理画面の言語切替は中国語/タイ語のみに整理(未翻訳の言語は一旦非表示 · クリック後の白紙画面を回避)\n• 残高更新時の入力ダイアログをブラウザ素の prompt から自社カード UI に置き換え\n• 今回の教訓をチームルール化: バックエンドのフィールド削除時はレスポンスモデルも同時に更新する(『設定画面が真っ白』の連鎖バグ防止)",
