...
## 🎯 状态卡（2026-06-27 · **B8 RLS · 孤儿 re-enroll sales 域 enroll 上线**（`8a7d8485`）· CI 6 闸全绿 + prod 金丝雀 PASS · companion 未动 · 交接 `docs/refactor/b8-rls-HANDOFF-2026-06-25.md` §7.11）

- **★ 孤儿 re-enroll · sales 域 enroll 上线 prod 验过**（`8a7d8485`）：`sales_documents`/`sales_document_lines`/`sales_document_sends`/`sales_settings`/`document_number_sequences` 全补回 `tenant` policy。新 `services/sales/schema.py` `ensure_sales_rls`（逐表先验存在再 enroll·force=False）进 `boot_ensures`、`dal_reexports` 暴露成 `db.*`。**5 表均 tenant_id NOT NULL 无 user_id → 纯 tenant**；坑：sales_documents 账套列名 `seller_workspace_client_id`、连号计数器 workspace_client_id 运行时加且可空 → 都不用 tenant_ws（账套强隔离靠应用层 `uq_dns_ws` 唯一索引）。**DAL 早全走 get_cursor_rls（wave3 期就位）→ 零访问点改动**，唯一保留 owner 旁路=share-token 公开取件 + 迁移回填。集成 3 例（docker pg·FORCE RLS）+ 4973 单测 + CI 6 闸绿（run `28280460586`）；金丝雀：5 表 rls=on/npol=1·孤儿 0·真租户(role) sales_documents 见 34/连号 5、假租户全 0。**剩余孤儿按域续做**（line/suppliers/knowledge/products/etax/automation/settings 杂项·见交接 §7.4）。
