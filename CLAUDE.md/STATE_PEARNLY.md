...
## 🎯 状态卡(2026-05-31 批2 收尾重写)

- **模式**:整顿封锁期 · 0 新功能 · 当前主线 = **home.js 拆迁 9 批**(批1+2✅,批3-9 待续·下一批3=导出 _runExport 等→src/home/export.js)。
- **批1+2 闭环**(commit 57def27/f7244e7 已上线·prod 验过):批1 OCR结果表+抽屉→`ocr-results.js`(474行);批2 OCR推ERP三件套→`ocr-push.js`(191行)· home.js **5344→4777**(净减567)· 每批 6 道守门全绿 + 真浏览器验过。
...
- **剩余瓶颈**:home.js 4777→目标200(批3-9)· app.py 1727→500(ocr_recognize/LINE 高敏)· 模块化缺 CSS。
- **待 Zihao 拍板**:① home.js 死拗到200还是放宽(Zihao 已答:目标不变) ② 窗口数(3→1专攻?)。
- **最后 commit**:f7244e7(批2)· `git log --oneline -5` 查最新。
