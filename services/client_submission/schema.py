"""ERP → Cowork 提交 outbox 的启动期 schema。"""
...
def ensure_client_submission_schema() -> None:
    from core import db
    ...
    with db.get_cursor(commit=True) as cur:
        ...
        for ddl in _INDEXES:
            cur.execute(ddl)
        apply_client_submission_rls(cur)
    logger.info("client_submissions schema ready")


def apply_client_submission_rls(cur) -> None:
    """双方只读；商户按有效关系插入；状态更新和删除只允许系统 bypass。"""
    ...
    cur.execute("ALTER TABLE client_submissions ENABLE ROW LEVEL SECURITY")
    for name in (
        ...
        cur.execute(f"DROP POLICY IF EXISTS {name} ON client_submissions")
    cur.execute(
