...
百分比换算/色族判定、搜索匹配(名字+税号)、按客户过滤矩阵格子。HTML 拼装/事件委托依赖
at()/AI.state/AI.matrixRender,不在 node 里独立可测,由 tests/e2e/_entryclose_verify.spec.js
真浏览器覆盖(同 ai-vatcheck-render.js 先例)。
"""
...
class CompletenessTests(unittest.TestCase):
    ...
    def test_chip_class_thresholds(self):
        out = _run_node(f"""
            const r = require({_RENDER});
            process.stdout.write(JSON.stringify([
                r.completenessChipClass(100), r.completenessChipClass(50), r.completenessChipClass(0),
            ]));
            """)
        self.assertEqual(out, ["g", "w", "n"])
