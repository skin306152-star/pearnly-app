...
def main():
    ...
    _row(
        "total_cost_thb",
        before_agg["total_cost_thb"],
        after_agg["total_cost_thb"],
        fmt_str="฿{:.4f}",
    )
    _row(
        "avg_cost_per_page",
        before_agg["avg_cost_per_page_thb"],
        after_agg["avg_cost_per_page_thb"],
        fmt_str="฿{:.4f}",
    )
    _row("L3 triggered pages", before_agg["l3_triggered_pages"], after_agg["l3_triggered_pages"])
    _row(
        "L3 trigger rate",
        before_agg["l3_trigger_rate"],
        after_agg["l3_trigger_rate"],
        fmt_str="{:.3%}",
    )
    _row("L3 failed pages", before_agg["l3_failed_pages"], after_agg["l3_failed_pages"])
    _row(
        "L3 failure rate",
        before_agg["l3_failure_rate"],
        after_agg["l3_failure_rate"],
        fmt_str="{:.3%}",
    )
    _row(
        "needs_manual_review pages",
        before_agg["needs_manual_review_pages"],
        after_agg["needs_manual_review_pages"],
    )

    print("\n--- IV69 invoice_number accuracy (slash separator preserved) ---")
    _row("IV69 / similar pages total", before_agg["iv69_total"], after_agg["iv69_total"])
    _row("with `/` (correct format)", before_agg["iv69_with_slash"], after_agg["iv69_with_slash"])
    _row(
        "correct %",
        before_agg["iv69_correct_pct"],
        after_agg["iv69_correct_pct"],
        fmt_str="{:.1f}%",
    )
    ...
    for key in sorted(set(bef_iv.keys()) | set(aft_iv.keys())):
        ...
        delta = (
            "✓ FIXED"
            if (b and a and not b["has_slash"] and a["has_slash"])
            else (
                "✗ BROKEN"
                if (b and a and b["has_slash"] and not a["has_slash"])
                else (
                    "✓ STILL OK"
                    if (b and a and b["has_slash"] and a["has_slash"])
                    else (
                        "✗ STILL WRONG"
                        if (b and a and not b["has_slash"] and not a["has_slash"])
                        else ""
                    )
                )
            )
