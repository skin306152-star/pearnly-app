...
tax:        tax.filing.view  tax.filing.create   # 生成税表草稿
            ...
            tax.filing.file     # 申报回执补挂(冻结后唯一可写路径)
            tax.settings.manage
...
- **会计**:全业务读写 + 审批/开票/过账/报税 + 业务主数据(科目表/商品/供应商)+ 工单复核签批/申报回执(tax.filing.review/file)。不碰团队、计费、模块开关。给专职会计/事务所做账员工。
...
- **工单四权分立**(C3 · `pearnly_ai_sod` 闸,默认关):`tax.filing.create/review/approve/file` 分别对应制单/复核/授权/申报。闸关 = 单人所全兼现状不变(码不互斥,owner/accountant 天然全含);闸开(事务所)= 强制复核人∉制单集、冻结授权人∉制单集且须有效复核在场,判定见 `services/workorder/sod.py`。
