...
(auth_password_routes 生成 {host}/reset?token=)点了即报错。
演进(2026-07-10):原内联 routes/reset_page.py 常量(规避 webhook 不拾取新增 static 根文件);
dist/ 子目录部署可靠,现改可读源 reset.html + build 压成 dist/reset.html(FileResponse 服务)。
无 jsdom · 静态锁:页面/脚本齐 + 4 语齐 + 接对后端端点 + 读 token + 路由指 dist 产物。
"""
...
PAGE = os.path.join(ROOT, "reset.html")
...
class ResetPageStatic(unittest.TestCase):
    ...
    def test_route_serves_dist_product(self):
        # /reset 出 dist minified 产物(dist/ 部署可靠);禁回退到不部署的 static 根文件。
        self.assertIn('FileResponse("static/dist/reset.html"', self.routes, "/reset 须出 dist 产物")
        self.assertNotIn(
            'FileResponse("static/reset.html"', self.routes, "禁回退到不部署的 static 根文件"
        )
