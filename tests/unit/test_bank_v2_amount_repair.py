...
from bank_recon_v2 import (
    StatementRow, _verify_row_balances, _repair_amount_from_balance,
)
...
class AmountRepairTests(unittest.TestCase):
    ...
    def test_small_misread_amount_autofixed(self):
        ...
        rows = [_row(dep=500, bal=1500),     # 1000+500=1500 ✓
                _row(dep=250, bal=1800),     # 印错;真值 300
                _row(dep=200, bal=2000)]     # 1800+200=2000 ✓ 佐证上一行余额
        _run(rows, 1000.0)
    ...
    def test_unreliable_prev_not_fixed(self):
        ...
        rows = [_row(dep=500, bal=9999),     # 1000+500≠9999 → balance_ok False
    ...
    def test_next_not_corroborating_not_fixed(self):
        ...
        rows = [_row(dep=500, bal=1500),
                ...
                _row(dep=200, bal=5000)]     # 1800+200≠5000 · 不佐证
        _run(rows, 1000.0)
        self.assertFalse(rows[1].amount_autocorrected)

    def test_correct_rows_untouched(self):
        rows = [_row(dep=500, bal=1500),
                _row(dep=300, bal=1800),
