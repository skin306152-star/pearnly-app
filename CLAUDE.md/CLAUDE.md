...
FastAPI + 原生 JS/Vite + Supabase Postgres,一个仓库装多个入口壳(主站 `/home`、POS `/pos`、超管 `/earn`、AI `/ai`、DMS `/dms`)。**入口 → 可读源文件的映射唯一权威 = `.claude/skills/frontend-change/entry-map.md`**(改对外页面前必看:改错位置 = 新域名永远跑老版本)。
