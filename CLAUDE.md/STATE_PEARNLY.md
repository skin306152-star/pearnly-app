...
## 当前状态卡(2026-08-01 · POS 条码扫码上线 · master `b4515b22` · CI 绿)

- **三处扫码上线(建品 / 入库 / 收银台),条码枪与手机摄像头共用一套地基 `static/scan/`。**
  线上 `/api/version=12044201`。六轮施工 + 一轮 /simplify,主线只有一件事:**让「引擎判对了
  但店员看不见」的静默失败全部出声**。
- **🔴 一条没有结论的风险悬着,真机验第一顺位**:便宜安卓机上举着货不动可能被记两次
  (顾客一件付两件的钱)。终审原话「既不能判真 bug 也不能判 harness 噪音,**现状是没结论**」。
  触发条件是「条码只有一半的帧读得出」= 反光/斜拿常态,而目标机就是慢解码的便宜安卓机。
  **在这条有结论前,不要对外说「去重验过了」。**
- **做不到的(物理限制,产品内已四语说明)**:新增商品**只能自动填条码,名字和价格仍要手打**
  —— EAN-13 本身不含这些,Odoo 社区版写了同样承诺也没实现。**箱码/单位维护界面**与
  **开票向导扫码**是 Zihao 拍板不做(后端能力和唯一索引都留着)。
- **🛑 六轮的教训链(每轮诊断比上一轮深一层)**:①P0 全在机械闸照不到的地方 → ②用不会出事的
  输入验会出事的判据 → ③**判据本身量错了东西**(墙钟被解码器自己在失败帧上的开销吃掉)→
  ④只量了一侧的代价 → ⑤同一个洞两个消费方修了一个漏了一个 → ⑥打通。
- **⚠️ `static/scan/scan-camera.js` 现在 499/500 行**,下一个往里加东西的人必须先拆。
- **交接单 `docs/pos/HANDOVER-2026-08-01-barcode-scan.md`** —— 待办按优先级排好(最该先做:
  后端条码匹配规则抽一份 / 启动路径加探针),真机清单在 §4,给下个窗口的五条警告在 §5。

## 上一轮状态卡(2026-07-31 · 清欠账第三轮 · §三 本机能做的全清光 · CI 绿)

- **主站 `62c5c8d5`(第三轮 40 笔已 push · 本窗口累计 85 笔)· CI run `30627117412` conclusion=success 10/10 全绿 ·
  单测 11649 例 0 FAIL · 闸 25 → 27 道 · 小助手线上 1.1.58,现网 **0 台**装(最新心跳 07-30 13:00Z 仍报 1.1.55)。**
- **🛑 本轮最值钱的一条:单元测试把整个仓库删空了,差一步推上生产。** git 调钩子时注入的 `GIT_DIR`/`GIT_INDEX_FILE`
  **优先级高于 `subprocess` 的 `cwd=`**;当天装的 pre-push 钩子里有一步跑全量单测,于是 `test_check_cachebust` 那几条
  只靠 `cwd=tmp` 防护的 `git init/add/commit` 全落进宿主仓 —— **4838 files / -794172 行**。已修(临时仓走
  `_git_sandbox.py` 摘掉宿主 `GIT_*`)+ 建第 26 道闸。**连带残留:`git config user.email t@t` 被劫持写进真仓库,
  11 笔提交作者成了 `t <t@t>`,禁 amend 改不回来。** 同天同形态第二例:`tests/integration` 把开发库拆掉 62 张表
  → 判据从「env 有没有值」换成「**目标库里有没有哨兵表**」+ 第 27 道闸。
- **🛑 第二条:并行代理各自抓的「现状快照」会互相矛盾,不能当判据。** 三路各自报「`check_file_size` 红:store.py 507 /
  mutations.py 514」,实测是 **462 / 490,闸 EXIT=0** —— 那是它们飞行途中的快照,作者同一轮里就自己拆好了。
  **真正红着的是另一件事:`check_tracked_imports`**(新模块 import 得到但没 `git add`,clean clone 会 ModuleNotFoundError,
  正是 2026-06-11 全 worker 起不来那个形态)。**收口时闸必须自己当场跑一遍。**
- **§三 关掉五条**:3.13 接地闸不许切在一个数中间(4567 条真语料验,误伤 0)· 3.15 STCRD REFNUM(真缺口在**在产的
  小助手**那份,Express 标配库存卡 12 种模板按它取数)· 3.17 两处棘轮收紧(第三处实测无格可收)· 3.18 那个 APBAL
  组合追到底(**客户账套里一行没有主档的残骸,不是我们算错**,发号够不到它 → 不改代码)· 3.21 过账去向就地补选重推。
- **判为不做(是判断不是没排期)**:**3.14 OCR Time in/out** —— 提示词那步真读得出跨日时刻,但两种放法各自把一张 A4
  采购税票的税号稳定读错,**拿抵扣凭据换「104 张里 1 张入账日差一天、申报期还不变」不划算**;`tests/integration`
  不进 CI(会拆库 / 彼此不隔离 / 隔离后仍红 7 个);触控目标**只抬四类**不全站一刀切。
- **★ 新发现没修**:账套硬门的错误提示**物理不可见**(422 有话、toast 也调了,但 `visibility:hidden` —— 用户看到的是
  「点了确定什么也没发生」)· 「后端 500 显示成空态」是**系统性**的,`/home` 三个面同病(本轮只修掉识别记录)·
  OCR 失败把 **Python 异常类名**印给会计(四语都印 `Layer1AuthError`)· 390 下 `/ai` 工作台 KPI 徽章被搜索框压住
  (**只测 zh 会漏**,zh 文案短侥幸不撞)· `direct_read` 把 VAT 内含的 ABB 小票判成结构不可能,**16 张里 9 张白回落 Vision**·
  **四张 legacy 表的建表 DDL 从没进过版本控制**(96 条迁移全是 ALTER)→ 任何空库都建不出来。
- **没做的**:Accserver 六条**物理不通**(本机 172.20.10.3 热点网段)· companion 1.1.59 **未发版**,两笔改动按铁律
  等于没生效 · 走查三轮 383 张,`/pos` `/admin` 仍只到公开页(**产品里现在一个超管都不存在**,`is_super_admin`=0 行)。
- 交接单 `docs/ai/HANDOVER-2026-07-27.md` 已按第三轮实测复判(§0/§二/§三/§四/§六/§七 + 新增 §十)。

## 状态卡(2026-07-31 · 清欠账第二轮 · 三道新闸进 CI · CI 绿)

- **主站 `5a1097f1`(第二轮 13 笔已 push · 本窗口累计 38 笔)· CI run `30579711972` conclusion=success
  10/10 job 全绿 · 单测 11451 例 0 FAIL · 小助手源码 `7e43877`(1.1.58 已 bump **未发版**,线上仍 1.1.57)。**
- **闸 22 → 25 道**(数表格数据行数出来的,不是算的):`check_e2e_stub_contracts`(桩的顶层键不得少于真后端)·
  `check_home_i18n_refs`(3354 处取词必须查得到定义)· `pg-smoke`(**本仓第一个带 Postgres service 的 CI job**,
  首跑 `Ran 13 tests / OK` 无 skip;**skip 与零发现都判红**,否则连不上库退 0 = 闸没看过)。
- **★ 本轮最值钱的一条:「桩比真后端少给键」两轮共抓到四处** —— sessions 回空 / status 漏 attachments /
  attachments 写成 null(键在值不在,与漏键同罪)/ sessions 只回最后一轮。**其中一处让已入库当验收证据的
  截图失真。** 根治是登记表式的桩契约闸 + 覆盖率测试(后端加键先红),**不是逐个修**。
- **关掉的欠账**:§3.8 台账 numeric 精度 / advisory lock 真串行 / RLS 真隔离改在真 Postgres 上验且已进 CI
  (`f41db5be` `b4965755`,三条反证自带)· §3.16 交付物下载验到「真下到文件」(`84232e68`:真跑 package →
  真下载 → 逐字节 sha 比对 → 真浏览器点击)· §二.7「CI 要不要加 Postgres」作废(做了)。
- **★ §3.1「我知道,还是推」判为暂不做 —— 判断不是没排期。** 三条:① 发布版小助手 1.1.57 里根本没那道闸,
  按钮今天上线必然是死按钮 ② 唯一在产的 `PRIOR_DOC_STILL_IN_ERP` 上「还是推」是错答案(挡住的必然是同一张票)
  ③ 队列是 at-least-once,`duplicate_confirmed` 恰好关掉唯一兜住它的幂等,且 `_find_existing` 取物理序第一条
  → **丢一次 ack 多一张,无上界**。前半(诚实展示差异、会计看得懂该动哪张)已闭环(`81282c41`)。
- **CI 红一次自己修**(run `30576549338`):覆盖率断言写死「≥8」,那个 8 是开发机上现数的(本机未跟踪脚本
  贡献 4 处,CI 只有跟踪文件)。没改成 7 交差 —— 改成按 `git ls-files` 过滤 + 每个登记端点至少一处。
  **写死的计数门槛先问一句「这个数在别人机器上还成立吗」。**
- **★ 视觉走查第二批 171 张(两轮共 278 张)揪出 8 条,一条没修**:/home 客户管理手机端公司名列宽 0px ·
  识别记录 500 显示空态且无载入中态 · **/ai 与 /dms 启动探针把 5xx 当「未受邀」**(后端抖一下,已受邀用户
  会来要权限)· ฿ 写法四个壳四套 · /ai 侧栏窄轨泰日标签被截。**第一轮那 7 条也原样欠着。**
- **没做的**:Accserver 一族全部(本机 172.20.10.3 热点网段,`192.168.0.11:445` 物理不通)· 小助手 1.1.58
  **只到源码没发版** · 四态里「载入中/失败」**只在 4 个面上主动构造过**,不是全覆盖 · /pos 与 /admin **只走到
  公开页与未授权落地态**(账号权限进不去,按「别硬造账号绕过入口授权」没改库提权)· /home 报税中心验不到。
- **血泪三条**:`test_file_crypto` 被记在「已知本机假红」名单里,查下来是那条断言**从来没真验过 stderr 内容**
  —— **名单本身有代价** · `_b2m1_rework_verify` 的必验1 叫「两个不同工具」其实一直是同一个工具换期间,
  **入库标准要加「它验的东西和名字对得上吗」** · 跑全量单测前先 `rm -rf test-results`(Playwright trace 残留会被破缓存闸扫成假红)。
- 交接单 `docs/ai/HANDOVER-2026-07-27.md` 已按第二轮实测复判(§0/§二/§三/§六/§七 + 新增 §九)。

## 状态卡(2026-07-30 · 清欠账第一轮 · 两仓已推平 · CI 绿)
