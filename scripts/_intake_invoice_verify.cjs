...
const RECOG = {
    ...
    confidence: 'low',
    needs_review: true,
    missed_invoice_warnings: ['maybe missed one'],
    duplicate_warnings: [],
    pages: [{ fields: {} }],
    invoices: [
        {
            ...
            fields: {
                ...
                buyer_name: 'Skin',
                items: [
                    { name: 'Coffee', qty: '2', price: '60' },
                    { name: 'Milk', qty: '1', price: '40' },
                ],
            },
        },
        {
    ...
    await page.route('**/api/**', async (route) => {
        ...
        if (u.includes('/api/ocr/recognize')) {
            if (recMode === 'fail')
                return route.fulfill({
                    ...
                    body: JSON.stringify({ detail: 'server' }),
                });
        ...
        if (u.includes('/api/erp/endpoints'))
            return route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify(epMode === 'empty' ? EPS_EMPTY : EPS_OK),
            });
        if (u.includes('/api/erp/push'))
            return route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ ok: true, endpoint_name: 'MR.ERP 主账套' }),
            });
        if (
            u.includes('/api/ocr/export') ||
            u.includes('/api/reports/history/batch_export') ||
            u.includes('/api/erp/mrerp-xlsx-batch')
        )
            return route.fulfill({
                status: 200,
                contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                headers: { 'X-Filename': 'out.xlsx' },
                body: Buffer.from('PK'),
            });
        return route.fulfill({
            status: 200,
...
async function run() {
    ...
    await chk('★原图查看器可见(共用 iv-*)', vis(page, '.dx-acc-item.open .iv-viewport'));
    await page
        .waitForFunction(
            () => {
                const i = document.querySelector('.dx-acc-item.open .iv-img');
    ...
    await chk(
        ...
        page.evaluate(() => {
            const c = document.querySelector('.dx-acc-item.open .iv-card');
            const i = document.querySelector('.dx-acc-item.open .iv-img');
            return !!i && i.src.startsWith('blob:') && !c.classList.contains('noimg');
        })
    );
    await page.click('.dx-acc-item.open [data-iv="in"]');
    await chk(
        '★放大按钮改变缩放标签',
        page
            .locator('.dx-acc-item.open .iv-zoom')
            .innerText()
            .then((s) => s !== '100%')
    );
    await page.click('.dx-acc-item.open [data-iv="reset"]');
    await chk(
        '★重置回 100%',
        page
            .locator('.dx-acc-item.open .iv-zoom')
    ...
    await chk(
        '结果·查看记录跳 history',
        page
            .locator('#dx-inv-view-rec')
            .click()
            .then(() => page.evaluate(() => window.__routes.includes('history')))
    );
    await chk(
        '结果·查看推送跳 integrations',
        page
            .locator('#dx-inv-view-push')
            .click()
            .then(() => page.evaluate(() => window.__routes.includes('integrations')))
    );
    ...
    await p2.evaluate(() => {
        ...
        window.__logsTab = false;
        window.routeTo = (r) => window.__routes.push(r);
        window.activateIntegrationsLogsTab = () => {
            window.__logsTab = true;
        };
        window.setErpLogAdapter = (a) => {
            window.__adapter = a;
        };
    });
    ...
    await chk(
        '发票·查看记录→history',
        p2.evaluate(() => window.__routes.includes('history'))
    );
    ...
    await chk(
        '身份证·查看记录→集成推送日志tab',
        p2.evaluate(() => window.__logsTab === true)
    );
    await chk(
        '身份证·推送日志筛 mrerp_dms',
        p2.evaluate(() => window.__adapter === 'mrerp_dms')
    );
    await p2.close();

    // ── ERP 未配置 → 黑底指引 ──
    const { page: p3 } = await boot(ctx, 'empty');
    await p3.setInputFiles('#dx-inv-file', {
        name: 'a.pdf',
        mimeType: 'application/pdf',
        buffer: Buffer.from('x'),
    });
    ...
    await chk(
        '点指引→integrationss',
        p3.evaluate(() => window.__routes.includes('integrations'))
    );
    ...
    await pStop.setInputFiles('#dx-inv-file', {
        name: 's.pdf',
        mimeType: 'application/pdf',
        buffer: Buffer.from('x'),
    });
    ...
    await pFail.setInputFiles('#dx-inv-file', {
        name: 'f.pdf',
        mimeType: 'application/pdf',
        buffer: Buffer.from('x'),
    });
    ...
    await pImg.setInputFiles('#dx-inv-file', {
        name: 'photo.png',
        mimeType: 'image/png',
    ...
    const { page: mp } = await (async () => {
        const r = await boot(m, 'ok');
        return r;
    })();
    await mp.setInputFiles('#dx-inv-file', {
        name: 'a.pdf',
        mimeType: 'application/pdf',
        buffer: Buffer.from('x'),
    });
