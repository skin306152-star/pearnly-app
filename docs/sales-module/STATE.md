...
## 2026-06-06(续3)· PO-7 发送上线 prod + 真投递 E2E 过(HEAD 427d30e · 迁移 0018)

**做了什么**(§L5):`POST /api/sales/documents/{id}/send`(channel+identity+to+message)。
- **邮件·官方代发**(`services/sales/send.py`):hello@pearnly.com 发 PDF 附件,From 显示卖方名、
  Reply-To 卖方邮箱(复用现成 SMTP)。
- **LINE·我自己转发**(`services/sales/share.py`):能力 token(`sales_documents.share_token`)+
  公开 `GET /shared/{token}/pdf`(不鉴权·token 即凭证·bypass-RLS 反查渲染),卖方自己转。
- 投递日志 `sales_document_sends`;PDF 组装抽 `services/sales/render.py`(下载/发送/分享共用);
  卖方加 email 列(Reply-To·随快照冻结)。迁移 0018。
- **deferred**:email/gmail(私人 Gmail·需 Google 审核·**Console 审核 Zihao 启动**·窗口期 App Password
  兜底)+ line/official 官号推送(高敏)→ 路由返 501。

**验证**:本地 tests/unit **2516 OK** · 守门全绿 · **prod 真库 E2E 全过**:share token 生成/复用/反查 +
公开 token 渲染 PDF;**邮件官方代发真自发到 hello@ 收件箱成功**(From=卖方名·Reply-To=快照卖方邮箱·
PDF 附件);投递日志写入;全程 ROLLBACK 零残留。详见 [[sales-mhz-blocks-and-prod-ops]]。
