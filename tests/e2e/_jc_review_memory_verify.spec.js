...
// 盖六条验收断言:①未判优先+已判折叠+计数器 ②收件箱最新裁决值+已人工修正徽标
// ③批量确认一步弹窗+落库数量+组消失 ④J-A 建议值三字段预填/无建议维持现状
// ⑤390×844 无横溢 ⑥(lint/build/unit 在自测脚本里,不在本 spec)。
...
test('客户页队列:未判优先聚焦 + 已判折叠 + 计数器「未判 3/共 5」+ 展开改判', async ({
    page,
}) => {
    const decideCalls = [];
    const flagged = [
        {
            ...
            file_ref: '/x/u1.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '100', vat: '7', total_amount: '107', invoice_number: 'U1' },
        },
        {
            item_id: 'u2',
            kind: 'purchase_invoice',
            flag_reason: 'ocr_low_confidence',
            file_ref: '/x/u2.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '200', vat: '14', total_amount: '214', invoice_number: 'U2' },
        },
        {
            item_id: 'u3',
            kind: 'purchase_invoice',
            flag_reason: 'ocr_low_confidence',
            file_ref: '/x/u3.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '300', vat: '21', total_amount: '321', invoice_number: 'U3' },
        },
        {
            item_id: 'd1',
            kind: 'purchase_invoice',
            flag_reason: 'ocr_low_confidence',
            file_ref: '/x/d1.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '400', vat: '28', total_amount: '428', invoice_number: 'D1' },
            decision: {
                decision: 'face_value',
                values: {},
                actor: 'user:aaaaaaaa',
        ...
        },
        {
            ...
            flag_reason: 'ocr_low_confidence',
            file_ref: '/x/d2.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '500', vat: '35', total_amount: '535', invoice_number: 'D2' },
            decision: {
                decision: 'exclude',
                values: {},
                actor: 'user:bbbbbbbb',
    ...
    await expect(page.locator('#rvCounter')).toHaveText('未判 3 / 共 5', { timeout: 15000 });
    ...
    await page.screenshot({ path: path.join(ART, '01-undecided-first-folded.png'), fullPage: true });
    ...
    const flagged = [
        {
            ...
            file_ref: '/x/m1.jpg',
            ocr_read: { seller_tax: '0105', subtotal: '100', vat: '7', total_amount: '107', invoice_number: 'M1' },
        },
    ];
...
// ============ 断言 4:J-A 建议值三字段预填 / 无建议维持现状 ============

test('客户页队列:命中建议值三字段预填+琥珀提示,无建议票维持单字段现状', async ({ page }) => {
    const flagged = [
        {
            item_id: 'sugg1',
            kind: 'purchase_invoice',
    ...
    // 首张=sugg1(队列原序未判票,filterPurchaseQueue 不重排)。按 E 进三字段建议表单。
    ...
    // 下一张(plain1)无建议 → 现状单字段表单,无三字段/无琥珀提示。
    await page.keyboard.press('ArrowRight');
    await page.keyboard.press('e');
    await expect(page.locator('#rvVatInput')).toBeVisible();
    await expect(page.locator('#rvNetInput')).toHaveCount(0);
    await expect(page.locator('.rv-suggest-note')).toHaveCount(0);
    await page.screenshot({ path: path.join(ART, '05-no-suggest-plain-form.png'), fullPage: true });
});

// ============ 断言 3:批量确认一步弹窗 → 落库数量断言 → 组消失;取消=零请求 ============

test('客户页队列:全部按建议处理 · 一步确认后逐张落库,组消失;取消=零请求', async ({
    page,
}) => {
    const batchCalls = [];
    const mkItem = (id) => ({
        ...
        flag_reason: 'ocr_low_confidence',
        file_ref: `/x/${id}.jpg`,
        ocr_read: { seller_tax: '0105', subtotal: '100', vat: '7', total_amount: '107', invoice_number: id },
        verdict_hint: { severity: 'warn', confidence: 'high', suggested_decision: { decision: 'face_value' } },
    });
    ...
    // 确认 → 逐张落库(数量断言)→ 组消失(未判票清空,按钮不再渲染)。
    page.once('dialog', (d) => d.accept());
    await bulkBtn.click();
    await expect
        .poll(() => batchCalls.length, { timeout: 8000 })
        .toBe(1);
    ...
    await page.screenshot({ path: path.join(ART, '07-bulk-confirmed-group-gone.png'), fullPage: true });
});
