...
## 🎯 状态卡（2026-06-27 · **B8 RLS · 孤儿 re-enroll automation + etax 两域连收（第 5/6 棒）**:automation_rules(tenant_or_user)+ error_events(纯日志不开)`aab5ff97`;etax_submissions/channel_settings + invoice_risk_checks(均纯 tenant)`51318414`。两域 CI 6 闸全绿 + prod 金丝雀全 PASS(rls=on/npol=1·假租户见 0)。**模板纠偏续(再证 §7.15.2)**:repo 无 DDL 的 legacy 孤儿必 SSH prod `\d` 查真实列——automation=user_id NOT NULL+tenant_id 可空=tenant_or_user;etax 三表=tenant_id NOT NULL+无 user_id+workspace_client_id 可空=纯 tenant(tenant_ws 会隐藏 firm-wide NULL 行)。交接 §7.16/§7.17。**剩余孤儿域:settings 杂项 → knowledge(最复杂留最后)+ 零暴露孤儿(最低优先)。** 下接前 4 棒 ↓）
