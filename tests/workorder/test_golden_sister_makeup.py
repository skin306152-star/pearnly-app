...
语料形态决定的必经人工阶段(与 G1R2 产品路径同口径,均走真产品口 api.py):
  1. 批量方向裁决:本语料销项证据全是实物翻拍(Ocha 小票/EDC 结算/银行流水打印件),
     锚对不上的落 kind=unknown 方向票,守恒闸不裁决不放行 → 按答案键逐张
     assign_kind(见 _direction_kind)。MC1-c.1 起锚命中的 Ocha 小票由 sort 自动归
     sales_doc 堆 flagged 留人工过目,另走批量确认阶段(_confirm_sales_docs);
     SA-2a 起 EDC 日终结算票自动归 edc_settlement 佐证堆,零人工触碰。
...
# SA-2a 金标锚:语料里的 EDC 日终结算票恰是这 9 张照片(2026-07-14 逐张人眼盘点;其中
# 2564/2606/2620/2631 一纸两联,OCR 单页只出顶联)。归堆命中数不写死(OCR 非确定),但
# 误归零容忍:任何 edc_settlement 件落在集外(单笔 SALE 支付条/银行打印页/税票)即红。
EDC_SETTLEMENT_MARKS = tuple(
    f"IMG_{n}" for n in (2564, 2582, 2585, 2588, 2592, 2595, 2606, 2620, 2631)
)
...
class GoldenSisterMakeupTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 本 harness 是「单大事务(无 cursor_factory)+真并发 OCR+真库」形态:此形态下成本封顶
        # 自动不启用(from_ctx 无 factory 返 None),不会把 ai_usage 锁攥进长事务与 worker 首写的
        # RLS ALTER 死锁——无需再手动关封顶,行为已内生(封顶验证另有 test_workorder_ocr_cost_cap)。
    ...
    def _confirm_sales_docs(self, cur, work_order_id) -> int:
        """批量确认本方销项票(MC1-c.1 后的新人工阶段):sort 自动归 sales_doc 堆的票默认
        flagged(sales_doc_review)留一次人工过目,守恒闸不确认不出包——镜像 MC1-b 审核队列
        的一键批量确认(assign_kind=sales_doc),走同一真产品口。返回确认张数。"""
        from services.workorder import api

        count = 0
        items = self.store.list_items(cur, tenant_id=TENANT_ID, work_order_id=work_order_id)
        for it in items:
            if it["kind"] != decisions.SALES_DOC or it["status"] != "flagged":
                continue
            api.record_decision(
                cur,
                tenant_id=TENANT_ID,
    ...
    def test_04_face_value_then_recalc_decision_paths(self):
        ...
        with self.db.get_cursor(commit=True) as cur:
            ...
            # 阶段 1b:批量确认本方销项票(MC1-c.1 起 60 张 Ocha 小票由 sort 自动归 sales_doc
            # 堆并 flagged 留人工过目,守恒闸不确认不出包——镜像 MC1-b 一键批量确认)。
            self.assertGreaterEqual(self._confirm_sales_docs(cur, wo_id), 1)
    ...
    def test_04b_edc_settlement_bins_and_matches_cli_byte_identical(self):
        """SA-2 金标三断言:① EDC 归堆命中/漏归如实报 + 集外零误归;② 工单佐证聚合与
        sales_agg CLI 直跑同料逐字节一致(SA-1 是唯一算法);③ 佐证只挂 gates 不改税额
        (官方数零漂移由 test_04 的 pp30 断言锁死,本方法只验佐证链)。"""
        import io
        import json
        import tempfile
        ...
        edc_items = [it for it in items if it["kind"] == "edc_settlement"]
        misbinned = [
            Path(it["file_ref"]).name
            for it in edc_items
            if not any(m in (it["file_ref"] or "") for m in EDC_SETTLEMENT_MARKS)
        ]
        self.assertEqual(misbinned, [], f"EDC 集外误归(钱路污染):{misbinned}")
        hit = len(edc_items)
        missed = [
            m
            for m in EDC_SETTLEMENT_MARKS
            if not any(m in (it["file_ref"] or "") for it in edc_items)
        ]
        # 命中数如实打印(验收报告抄这里);至少命中 1 张才证明链路通,漏归不掩(打印点名)。
        print(f"\n[SA-2 金标] EDC 结算票归堆:命中 {hit}/9 · 漏归 {len(missed)}:{missed}")
        self.assertGreaterEqual(hit, 1)
        for it in edc_items:
            self.assertEqual(it["status"], "ok")

        # ② 同料喂 CLI,聚合报告与适配器路逐字节一致。
        payloads = edc_corroboration.payloads_from_events(events)
        self.assertEqual(len(payloads), hit)
        adapter_report = edc_corroboration.aggregate_report(payloads)
        with tempfile.TemporaryDirectory() as td:
            in_path = Path(td) / "edc_in.json"
            out_path = Path(td) / "edc_out.json"
            in_path.write_text(
        ...
        print(
            f"[SA-2 金标] 聚合毛额 {cli_report['month_total']['gross']} · "
            f"销售额 {cli_report['month_total']['sales_amount']} · "
            f"销项税 {cli_report['month_total']['output_vat']}(CLI 直跑=适配器,逐字节一致)"
        )

        # ③ 佐证挂进 reconcile 冻结 gates,聚合三数与 CLI 同值;不碰 R2 权威(858,780.16)。
        from services.workorder import evidence

        reconcile_done = evidence.replay_step_done(events, "reconcile") or {}
        gate = (reconcile_done.get("gates") or {}).get("r2_edc_corroboration")
        self.assertIsNotNone(gate, "reconcile 完整跑后必须挂 EDC 聚合佐证")
        self.assertEqual(gate["gross_total"], str(cli_report["month_total"]["gross"]))
        self.assertEqual(gate["net_total"], str(cli_report["month_total"]["sales_amount"]))
        self.assertEqual(gate["vat_total"], str(cli_report["month_total"]["output_vat"]))
        self.assertEqual(Decimal(gate["authoritative_net"]), GOLDEN_SALES_AMOUNT)
        self.assertEqual(gate["covered_state"], "amber")
    ...
    def test_07_resume_after_kill_mid_classify_matches_uninterrupted_run(self):
        ...
        with self.db.get_cursor(commit=True) as cur:
            ...
            # 续跑前的人工四阶段(与 test_04 同口径:方向裁决 → 确认销项票 → 采信标黄票 →
            # 填销项 → 补正)。
            self.assertGreaterEqual(self._assign_direction_tickets(cur, wo["id"]), 1)
            self.assertGreaterEqual(self._confirm_sales_docs(cur, wo["id"]), 1)
