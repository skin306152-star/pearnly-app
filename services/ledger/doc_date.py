...
⚠️ 2026-07-31 实测:改成解析文本之后,这条规则**照样一次都没跑到**。拿 SM 5 月真语料
(桌面 Pearnly-产品语料测试数据/SM-照片语料)跑真 pipeline,6 张印了 Time in/out 的 Ocha
场次票、12 次运行(修改前后各一轮)无一例外:`date_raw` 只拿到 "27-05-2569"(模型只抄了
Time in 那一行),`notes` 拿到的是 "Thank You Ka~"。带 in/out 标签的文本一次都没进来过,
`_SESSION_LABEL` 于是从未命中 —— 换掉键、改成文本,只是把一条死分支换成了另一条。

后果有界但真实:唯一跨日的那张(02000139,in 27-05 16:19 / out 28-05 08:29)现在按 27 入账,
而 MR.ERP 销项税登记簿记的是 28/05;同属 5 月申报期,故只差一天、不错期。

已试过的修法与为什么没上(证据见交接单 §3.14):给 ThaiInvoice 补 time_in/time_out 两格本身
无回归(只补字段不动提示词:IMG_2643 3/3、IMG_2640 2/2 税号照旧全对),但**模型不会填**;
要它填就得往共用的发票提示词里加规则,而一加,A4 采购税票的税号就被稳定读错:

  · 规则放进 CRITICAL RULES 编号表 → IMG_2640 的 buyer_tax 由改前 4/4 读对变成 7/7 读空
    (长写法 4 次 + 精简写法 3 次,全空);
  · 同一句挪到提示词末尾 → IMG_2640 回正 3/3,换成 IMG_2643 的 seller_tax 被读成买方自己的
    税号,4 次错 3 次(改前 4/4 全对)。

税号是进项能不能抵的凭据,拿它换「104 张里 1 张的入账日差一天、且申报期不变」不划算。要闭合
得先解决「同一张票上两行同名 เลขประจำตัวผู้เสียภาษี 怎么分买卖方」,再连提示词一起评测。
