...
# 泰数字 → 阿拉伯(会计/正式文档常用·数字符号非词子串·安全·thai-language.com/ref/numbers)。
...
def normalize_words(text: str) -> str:
    """数字简写归一(确定性):泰数字๕๐→50 · 1k→1000 · 2หมื่น→20000 · 欧式 1.250,50→1250.50。"""
    s = (text or "").translate(_THAI_DIGITS)
...
def strip_nonmoney(text: str) -> str:
    ...
    # (「ป้ายทะเบียน」车牌)照剥。标签后可夹 1-2 泰文辅音(车牌「ทะเบียน กข 1234」),不误伤「เลขที่บิล 500」。
    s = re.sub(
        rf"(?<!ค่า)(?<!ต่อ)(?:{before})\s*(?:[ก-ฮ]{{1,2}}\s*)?\.?\s*({_NUM})",
        " ",
        s,
        flags=re.IGNORECASE,
    )
