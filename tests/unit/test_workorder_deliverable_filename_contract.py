...
def _common_patches(*, db, artifact_path):
    return [
        ...
        mock.patch.object(wr.api, "deliverable_artifact_path", return_value=artifact_path),
    ]
    ...
    async def test_filename_has_client_name_period_and_report_label(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with contextlib.ExitStack() as stack:
                ...
                stack.enter_context(
                    mock.patch.object(wr.storage, "resolve_within_order", return_value=path)
                )
                resp = await wr.download_deliverable("wo-1", "pp30_draft", mock.Mock())
