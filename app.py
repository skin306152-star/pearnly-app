...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "系统已优化『银行对账』模块的余额录入功能。\n\n根据用户反馈,在原有 3 个余额录入框基础上新增『Statement 期末余额』录入框,共 4 个锚点字段可手动覆盖系统识别值。\n\n如系统检测到您上次录入或识别过的余额数据,会自动预填到录入框中,并以橙色背景标识来源,提示文字明确告知数据来自上次 OCR 识别。点击任意录入框修改后,标识自动消失。\n\n导出的 Excel 文件标题、表头、对照表语言始终跟随当前界面语言(中文 / 英文 / 泰文 / 日文)。\n\n即日生效。",
            "th": "ระบบได้ปรับปรุงฟังก์ชันกรอกยอดในโมดูล『กระทบยอดบัญชีธนาคาร』\n\nตามที่ผู้ใช้แจ้ง ได้เพิ่มช่องกรอก『ยอดยกไป STATEMENT』 ทำให้ฟิลด์แอนเคอร์ที่กรอกได้รวมเป็น 4 ช่อง สำหรับใช้แทนค่าที่ระบบตรวจพบในกรณีที่ OCR อ่านไม่แม่นยำ\n\nหากระบบตรวจพบยอดที่คุณเคยกรอกหรือเคยอ่านได้ จะเติมค่าให้อัตโนมัติพร้อมพื้นหลังสีส้ม และข้อความระบุชัดเจนว่าค่ามาจาก OCR ครั้งก่อน คลิกที่ช่องใดเพื่อแก้ไข แล้วเครื่องหมายจะหายไป\n\nหัวข้อ ส่วนหัวตาราง และตารางเปรียบเทียบในไฟล์ Excel ที่ส่งออก จะตรงกับภาษาที่เลือกในหน้าจอเสมอ\n\nมีผลทันที",
            "en": "The『Bank Reconciliation』balance input feature has been improved.\n\nBased on user feedback, a『Statement Closing Balance』input field has been added. Together with the existing fields, all 4 anchor values can now be manually overridden when system detection is unstable.\n\nIf the system finds balance values from your previous entry or detection, they will be auto-prefilled into the input fields with an orange background indicator, accompanied by a notice clearly stating the data source is from the last OCR scan. Click any input to edit; the indicator will disappear automatically.\n\nThe exported Excel file's title, headers, and comparison tables consistently follow the current interface language (Chinese / English / Thai / Japanese).\n\nEffective immediately.",
            "ja": "『銀行照合』モジュールの残高入力機能を改善しました。\n\nユーザーからのフィードバックに基づき、『Statement 期末残高』入力欄を追加しました。既存の入力欄と合わせ、合計 4 つのアンカー値を手動で上書きでき、システム検出が不安定な場合のバックアップとして利用できます。\n\n以前に入力した残高や検出結果が見つかった場合、入力欄にオレンジ色の背景で自動入力され、データ元が前回 OCR 検出であることを明示します。いずれかの欄をクリックして修正すると、マークは自動的に消えます。\n\n出力される Excel ファイルのタイトル、ヘッダー、比較表は、常に現在のインターフェース言語に従います。\n\n即日有効。"
