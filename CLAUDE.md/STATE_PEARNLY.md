...
## 🎯 状态卡(2026-06-01 · **整顿主线 C3 进行中** · R15~R17 抽 5 个 modal/drawer · home.html 1495→1324 上线 prod 11850056)

- **模式**:整顿封锁期 · **整顿主线 C3 进行中**(Zihao 2026-06-01 拍板:DMS 全链路收尾闭环 → 回整顿)。**当前 task = C3 拆 home.html → <1000**:本窗口 R15~R17 后 **1324**(还差 ~324)。下个候选(由易到难):`endpoint-modal`~80〔⚠️erp-integration.js eval IIFE **无守卫**绑 btn-ep-*/ep-url·注入须严格 import 在前·一旦元素缺失全员 boot 崩〕/ `billing-modal-mask`~41〔**待查消费者:grep src/home/*.js 无 billing-modal 引用·先确认是否 dead/别处绑再动**〕/ `endpoint-modal` 等·逐个查绑定时机。page-ocr(~150·识别主页高耦合·留最后)。**app shell(顶栏~128/侧栏~143)boot 关键勿轻动**。`email-modal` 牵涉 `line-email-modal.js`(🔴高敏)·跳过。
...
- **C3 进度封存**:home.html 1730→**1324**(R13 `#page-history` `4d3b9e0` + R14 `#onboarding-modal` `99c5da4` + **R15 `#bank-cand-drawer`→`bank-cand-drawer-html.js` `277b377`(-22)** + **R16 `#client-modal-mask`+`#wsclient-modal-mask`→`modal-client-edit.js` `bf42884`(-98)** + **R17 `#camera-tips-modal`→`camera-tips-html.js` + `#bank-client-picker-modal`→`bank-client-picker-html.js` `d773424`(-51)**·均字节等价〔脚本提取+校验模板===git HEAD inner〕+prod E2E 验〔R16 真浏览器开弹窗验·R17 查空壳 wbInjected+子元素在场〕)。范式见记忆 [[wb-src-home-c3-split-playbook]](留空壳 + `*-html.js`/`modal-*.js` eval 期注入 inner + 子树初译 + import 置消费模块前;消费模块 load()/_bindOnce/DOMContentLoaded 守卫绑定→eval 注入恒早于首次绑定·非竞态)。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。
...
- **最后 commit**:`d773424`(C3 R17 抽拍照贴士+银行客户绑定弹窗 inner→运行期注入 · REFACTOR-WB-C3 · bump `?v=` 11850055→11850056)· `git log --oneline -8` 查最新。**均已 push 上线·prod 11850056 健康**。**整顿轮 cache-bust:仍 bump `?v=` 保缓存正确,但不重写 release_notes(Zihao 2026-06-01:纯重构忽视新版本横幅·见记忆 [[refactor-skip-version-banner]])**。home.js 拆迁全收官 33768→0(范式 [[home-js-batch-split-bridge]])。
