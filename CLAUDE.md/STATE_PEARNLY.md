...
> **当前数字(master 全绿·工作树干净)**:home.js **8941** · app.py 4839 · db.py 3371 · home.css 0 · home.html 6658 · 单测 **1516** · E2E **10**。`gh.exe` 路径 `C:\Program Files\GitHub CLI\gh.exe` · repo `skin306152-star/pearnly-app`。
> **🅱 计费模型已变(重要)**:产品现在**只有「充值+按量扣费」** · 老月订/年订/体验(trial)/免费(free)套餐**全下线** → home.js+后端大量老套餐 UI/PLG 试用闸/升级弹窗 = **死码可清**(I2)。
> **下一个任务**:见下方【第四十一会话】块。大块 `home.js L5687-8376 IIFE`(2690 行·套餐/PLG/LINE + admin 用户管理 混装·大半死码)**不可直接 copy-out**(写 `_userInfo`/`_quota` + 裸调 5 个未挂 window 的闭包函数 + 劫持 fetch)· 正确做法=先在 home.js 把那 5 个 render 函数 + `_userInfo`/`_quota` 改 window 桥接 → 再结合「只剩充值」删套餐死码 + 核实 admin 半是否独立 `/admin` 死码后删/搬 → 分两模块,**一步一 E2E·牵连核心状态别 rush**。也可先挑更干净的小目标(automation L5260 / settings 群)热身。
