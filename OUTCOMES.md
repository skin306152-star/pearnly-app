...
| 1b | **按钮真统一(像素级·非 grep)** | Zihao 截图 + 主控见:**至少 5 页仍黑底动作按钮**——客户管理「设为当前」/异常栏「批量重试·批量删除」/集成推送日志「批量重推·批量删除·取消选择」+ 行内「重试推送·重试」。**⚠️主控2轮复验受阻(05-31)**:三个 E2E 测试账号【数据为空】——「设为当前」需账套主体数据/「批量重试」需推送异常数据/批量栏需勾选行才显示,空号全抓不到。已抓到的:`btn-primary`=rgb(37,99,235)蓝✅、`btn-ghost`合规、唯一黑底=异常栏 `exc-chip active` rgb(17,17,17)=**chip选中态非动作按钮**(§3②已拍"tab/chip不强制蓝"→不算违规)。**真正被质疑的「设为当前」「批量重试」黑按钮 = 数据缺失未验到,不下结论**。 | 同前:DOM 有 class 但渲染色待验;但验证被空测试账号阻塞。 | **阻塞**:需有数据测试账号(灌数据有风险待Zihao点头)或用 Zihao 带数据页截图抓该按钮 CSS。主控不空号假验。 |
