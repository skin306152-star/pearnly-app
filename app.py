...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.38 · 设置弹窗滚动条永远可见 + 显式视觉提示:\n• 用户反馈 v34.37 部署后 ERP 集成页 OK 但设置弹窗仍看不到 '联系我们' · 调查后推断: overflow-y: auto 在某些 OS/浏览器下默认隐藏滚动条 · 用户以为不能滚 (其实能 · 只是没视觉提示)\n• 修: 设置弹窗 side-nav overflow-y 从 auto 改成 scroll !important · 永远显示滚动条 · 加 webkit/firefox 滚动条样式 (8px 宽 · 半透明灰)\n• 全部静态资源 v=11834937 → 11834938 + 启动 _purge_stale_static_gz() 继续 trigger\n\n累积 fix: 对账中心 78 条 4 语术语统一 · Excel sheet 名 รายละเอียดบัญชี → รายละเอียดSTATEMENT · ERP 集成页布局 · 设置弹窗响应式 · Xero/MR.ERP 卡同高",
