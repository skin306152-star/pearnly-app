# 图文一体入口 · 设计 + 施工实录(方案 M · 2026-07-03 已施工)

> 2026-07-03 · P2 backlog 第二件。目标:用户「发图 + 一句话说这张怎么处理」一步到位。
> **Zihao 真机截图验收坐实缺口**(发图 IV69/00179 ฿2889 后说「นำข้อมูล...เข้า ERP」,
> 机器人却定位到旧载体 PS2-0702 ฿456——confirm 卡兜住没错推,但目标定位错)→ 当天按 M 施工。
>
> ## 施工实录(照 M·一处务实调整)
> - M-1 图片路写锚点:`line_image_ocr` 出卡后 `anchors.record_image_docs`(唯一一张才锚·
>   多张不猜·闸关/故障 no-op);`_locate_doc` 锚点优先,图片单锚经 `doc_fallback` 载体机制
>   拼 hist,**读时复核状态**(posted/draft 才认·ทิ้ง/撤销拒认回落);纯读(查推送状态)不插载体行。
> - M-2 回溯问询 → **落成回溯提示**:模型路由本就区分「刚才那张」(push_to_erp·现走锚点
>   确定命中)与「下一张」(plan 意图);plan 回执在刚处理过图时明说「这只管下一张;说的是刚才
>   那张就说『把刚才那张推进ERP』」——比再造一套问询状态机少一层确认(push 本就有 confirm 卡,
>   问询+确认=问两遍),防错配效果同。
> - 验收:单测 5743 绿 + 截图场景模拟复现(修前 PS2-0702/修后 IV69/00179)+ 死单回落。
>   剩真机人类门(163):发图→「把刚才那张推进ERP」→确认卡=同一张。
>
> 以下为施工前的侦察原文(留档)。
