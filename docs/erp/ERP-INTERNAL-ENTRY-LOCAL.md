...
## 2026-09-17 POS组件复用、商品库存整合与冗余清理

范围：本对话的ERP采购、销售、入库、出库，复用原POS录入和商品库存组件。最终候选4f1cbab9f740ebf381f31d1c3eabdd5864e88916。

清理：删除无调用的旧表格CSS、空更新函数、网页保存的隐藏按钮转发层、重复库存锁；移除旧构建分块。LINE模拟器与本地验收目录从生产镜像排除，仍保留本地验收能力。

保留：manual_layout=1历史单据计算分支、原POS仍使用的库存路径、真实账套与历史单据。未迁移旧POS库存/批次，未纠正线上旧错单，未编造缺失的商品条码或成本。

验证：本地PostgreSQL与表单定向49项测试/6subtests；浏览器实际保存入库IR-25690917-0001（1件×20），销售草稿保存验证；四组浏览器验收全部通过（含入口隔离67项）。扫码测试的后端使用桩，不等于真实业务数据库验收。测试修正仅更新已更名的商品与库存菜单，以及Chromium日期分段自动跳转的键盘操作；未放松断言。

限制：LINE真机视觉、授权与操作未由本次浏览器模拟替代。生产仅检查版本/资源/健康，不向真实账套写测试单。

补充检查：最终pre-push全绿，1198模块/6分片；手机盘点EAN/QR、逐笔累计、响应丢失重试、LINE壳与凭证、撤销/锁定和相机释放测试通过。

发布前追加结清修复：POS新表单的已收/已付金额按net_payable判断；销售原生grand_total沿用既有销售计算器的扣WHT后应收语义。预览/历史total_amount继续为含税总额。27项定向测试通过，包含网页/LINE两来源×采购/销售×已收付/未收付8个真库子场景：含税107、VAT7、WHT5，应收付102；选已收付时paid_amount102且paid，未收付时0且unpaid。

第一次Manual CD 35175979848在追加结清修复时取消；schema-fdfcm在取消后独立正常完成，Web/Worker均保持c5fcee41785d各100%，没有切换到52fd7586。确认该schema执行结束后，才启动最终候选的Manual CD 35176391558。

最终已发布：`4f1cbab9f740ebf381f31d1c3eabdd5864e88916`，Manual CD 35176391558 success；Web/Worker Ready、各100%，正式28个资源逐字节一致。digest、schema及取消候选的完整状态见部署账本。本地7872已重启到同版代码；本轮生成的三份blink视频及临时组件验证脚本已删除，其他工作树、依赖、测试数据保留。
