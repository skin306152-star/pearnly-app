...
## 🟢 2026-05-23 改善计划收尾(Zihao 拍板)

整改告一段落 · **下窗口回归 `REFACTOR_MASTER_PLAN.md` 工程整顿(从 A5 续)**。

| 模块 | 收尾状态 |
|---|---|
| **M4 银行对账** | ✅ **闭环**:坐标解析根治密集件(BAY 314/31、KBank 266/33 免费)+ KTB.xls 静默错误修复 + 期初期末回填 + 缺页提示 + 完整性告警;14 份全量实测行级准确率 ≈99.9%(真错 2 处全告警)。 |
| **M1 / M2 / M3** | ⏸️ **暂缓** · 等用户反馈具体问题再逐一改(不再主动推进)。 |
| **Phase 1**(OCR 痕迹 + 字段置信度) | 部分(M4 完成 · M2 发票侧 field_overrides 已写)· 其余暂缓。 |
| **Phase 2 / 3**(用户改→OCR 学 · 模板库) | 🟡 **完整设计已出 · 但拍板"全部不做"**(不要空壳/半成品)。设计要点见下方 Phase 2/3 + 第八会话讨论:值记忆(`field_correction_memory` 库 · 跨模块共享 · 白名单稳定字段)、Excel 回传 diff(row_uid 对齐)、自我纠偏(命中升/推翻降)、5 条可靠性红线。真要做从切片 S1(建库 + 接 M1 现有纠正入口)起。 |
| **外部 OCR 模型** | 评估暂不值得(HANDOVER §9)。 |
...
> 详见 `AGENTS.md` §铁律 #21 · 此处摘录给整改工作流用
