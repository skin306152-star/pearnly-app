...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "v118.35.0.42 · 🚨 紧急修复:GL Excel 上传 0 行(泰国佛历年识别不出)\n• 客户反馈:收入对账 / 银行对账上传 GL Excel 显示 0 แถว(0 行)· 整张对账报告废\n• 根因:客户 Excel 里日期列存的是『佛历年』datetime(2568 年 = 公历 2025 年)· 我们的日期解析函数把它误判为字符串 '2568-12-31 00:00:00' · 分割成 4 段不识别 → 0 行\n• 修法:日期解析函数加 4 层兜底:1) datetime 类型直通 2) 自动佛历 → 公历转换(-543)3) ISO 字符串带时分秒自动去时分秒 4) yyyy-mm-dd 路径也加佛历转换\n• 效果:同款 GL 文件现在能读 1+ 行 · 日期正确显示为公历\n• 8 道守门测试锁住 8 种日期格式:datetime BE / date BE / ISO str BE / ISO T str / yyyy-mm-dd BE / dd/mm/yy 短年 / CE 字符串 / 垃圾→None\n\n— 以下面向用户(不变) —\n\nv118.35.0.41 · 内部:4 模块表加『手改字段追踪』JSONB 列(Phase 1 起步)\n• 内部基础设施:4 个核心表(发票上传 / 销售税对账行 / 收入对账 / 银行对账)各加 field_overrides JSONB 列\n• 这是 Pearnly 项目第一次跑正式 Alembic 数据库迁移(REFACTOR-A2.2 + B3 激活 · 借整改 Phase 1 P1.1)· 借此把数据库 schema 改动从『启动时硬建』升级到『版本化迁移』\n• 0 用户感知:列默认 NULL · 老对账 / 老发票数据不动\n• 后续 Phase 1(P1.2-P1.7)所有『OCR 抽到 vs 用户改了什么』全部统一存这列 · 跨 4 模块共享同一套追踪结构\n• 双跑兼容:Alembic 002 文件 + ensure_field_overrides_columns() 启动钩子 · 确保 prod 不漏\n\n— 以下面向用户(不变) —\n\nv118.35.0.40 · 通用弹窗 / 抽屉滚动防御性兜底(Phase 0 收尾)\n• 内部审计:扫了项目里 6 类弹窗 / 抽屉 · 找跟『设置』弹窗当年同款的滚不到底病(v118.34.39 / v118.35.0.35 修过 2 次)\n• 给 .modal(通用弹窗)和 .drawer(抽屉)加了一行『保险栓』属性(min-height:0)· 防 zoom 偏移时滚动条不出现\n• 0 用户感知变化:平时跟以前一模一样 · 只在浏览器 zoom 触发 px rounding 时有差异\n• 完整审计文档 docs/audits/2026-05-23-modal-flex-chain-audit.md · 列 6 类弹窗 / 风险等级 / 修法预案(撞 bug 时直接抄)\n• 没用更激进的 max-height calc 修法 · 防『预修引入新 bug』· 等真 bug 报告再升级到激进修法\n\n— 以下面向用户(不变) —\n\nv118.35.0.39 · 收入对账历史详情 · 手动录入对照表(BUG-B 收尾)\n• 跑完对账 + 看历史 task 详情时:KPI 卡片下面多一个浅橘色面板『⚠ 本次对账含手动录入』\n• 表格 4 列:Anchor 字段 / OCR 抽到的(灰字)/ 你填的(黄底粗体)/ 差额(绿涨红跌)\n• 只在你真的改过 anchor 时显示(老 task / 没改过 anchor 的 task 看不到)· 不打扰\n• 切语言时自动重渲染 · 4 语完整\n• 跟 v0.38 Excel 导出的『手动录入痕迹』section 是同源数据 · 屏幕看完跟 Excel 拿到的对得上\n\n— 以下面向用户(不变) —\n\nv118.35.0.38 · 收入对账 Excel 导出 · 手动录入痕迹一目了然\n• 导出的 Excel 现在末尾多一段『手动录入痕迹』· 列出你改过的 anchor:OCR 抽到的多少 · 你改成多少 · 差多少\n• 被你改的 cell 标黄(暖色 FFE082)· hover Excel cell 能看 OCR / User / 差 完整对照(comment)\n• 报告顶部多一行警示『⚠ 本报告含手动录入 · 看末尾对照表』· 老板审计一眼看到\n• 只在你真的改过 anchor 时出现这段(没改时 layout 不变 · 老用户感知不到)\n• 4 语完整(zh / en / th / ja)\n\n— 以下面向用户(不变) —\n\nv118.35.0.37 · 收入对账『3 个余额录入框』自动预填上次 OCR 抽到的值\n• 改善:之前每次进对账要从零填 3 个『GL 期末 / Statement 期初 / GL 期初』· 现在浏览器记着上次 OCR 抽到的数 · 自动预填 · 不准点一下就能改\n• 灰字斜体显示『· OCR』· 一眼能区分哪些是上次 OCR 抽的(参考)· 哪些是你自己改的(权威)\n• 后端总是落库 OCR 抽到的 3 个值(不管你改没改)· 给即将上线的『手动录入痕迹』功能用\n• 4 语完整(zh / en / th / ja)· 不破坏 BUG-B 既有兜底逻辑\n\n— 以下面向用户(不变) —\n\nv118.35.0.36 · 收入对账『GL / Statement 3 个余额』可手动录入\n• 客户反馈:OCR 抽 GL 期末余额 / 期初 Statement 余额 / GL 期初余额 这 3 个『锚点』数字不准时 · 整张对账报告就废 · 现在加 3 个录入框 · 填了就用你填的 · 留空仍走 OCR\n• 位置:上传银行账单 + GL 文件下方 · 现有 3 个保存按钮上方 · 跟现有设计语言一致(暖灰底 / 黑色 focus 边框 / 数字右对齐)\n• 业务等式提示:填了 Statement 期初 + GL 期初 → 实时显示『期初差额 = Statement 期初 − GL 期初』· 让会计师肉眼对一下账\n• 后端记录:用户填的 / OCR 抽的 都落库 · 回查任务时能看出哪几个数是手填的\n• 4 语完整(zh / en / th / ja)· 手机端 3 个录入框堆成 1 列适配\n\n— 以下面向用户(不变) —\n\nv118.35.0.35 · 设置弹窗右栏滚动彻底修好\n• 浏览器放大缩小不同比例时(67% / 75% / 80% / 90% / 100% 等)·『设置』弹窗右侧表单不能滚动看到底部字段(国家 / LINE ID / 公司信息 tab 等)· 现已修好 · 任何缩放比例都能正常滚动录入\n• 跟之前修左侧导航栏滚动同款病(v118.34.39)· 这次轮到右栏 · 直接 max-height 卡死高度 · 不再靠 flex chain 推 overflow\n\n— 以下面向用户(不变) —\n\nv118.35.0.33 · 清理老订阅残留(用户端 + Earn 后台)\n• Earn 后台管理(超管可见):删除老订阅模式残留 UI · 套餐分布 KPI 4 张卡 / Trial 即将到期段 / 修改套餐弹窗(Trial · 月度 ฿299 · 年度 ฿2990 · 终身)/ 升级套餐按钮 / 套餐列 / 套餐筛选 / 待审核付款 list · 完全统一到 credits 按用量计费模式\n• 用户端:头像菜单『订阅 & 套餐』改『账户 & 余额』 · 设置页『套餐 & 用量』标签改『用量』 · 4 语同步 · 0 功能影响\n• 后端:5 个老订阅 admin 路由整段删(POST /api/admin/users/upgrade · GET /api/admin/payments/pending · GET /api/admin/payments/{id}/screenshot · POST /api/admin/payments/{id}/review · GET /api/admin/users/funnel 改瘦)· 防『还能点升级』误操作\n\n— 以下面向用户(不变) —\n\nv118.35.0.32 · 前端打包工具升级(整顿期阶段 A 第一步)\n• 内部:首页前端 JS 改用 Vite 工具打包 · 文件压缩了 37% · 加载稍快一点 · 用户用起来跟以前一模一样\n• 后续所有新拆出的小模块都走这套流水线 · 是 5-8 个月整顿计划的工程化起点\n\n— 以下面向用户(不变) —\n\nv118.35.0.31 · 代码可读性提升(续)\n• 内部:再清 12 处静默吞错点 + 3 处『应该日志但被吞了』的改成 console.warn 留 trace(网络错误细节解析 / i18n 翻译失败)· 0 用户感知 · 让 DevTools 能看到隐藏的问题\n\n— 以下面向用户(不变) —\n\nv118.35.0.30 · 代码可读性提升\n• 内部:给 15 处『静默忽略错误』加注释说明为什么是有意忽略的(私模 localStorage / 已 abort / 已 revoke 等)· 0 用户感知 · 让排查问题时知道哪些 catch 是故意吞的\n\n— 以下面向用户(不变) —\n\nv118.35.0.29 · 代码整理(续)\n• 内部:充值弹窗 + 余额自动刷新逻辑独立成模块 · 0 用户感知变化 · 跟上版本对齐继续清理巨石文件\n\n— 以下面向用户(不变) —\n\nv118.35.0.28 · 代码整理\n• 内部:首页『仪表盘』加载代码独立成模块 · 0 用户感知变化 · 为后续模块化清理打底\n\n— 以下面向用户(不变) —\n\nv118.35.0.27 · 多页报告溯源 + 任务队列基础设施\n• 销项税对账多份 PDF 合并后 · 每行附带『来源文件 + 页码』· 对账有差异时能定位原 PDF 第几页(以前只能凭记忆翻找)\n• 内部:Earn 后台系统监控增加 4 张任务队列卡片(待处理 / 处理中 / 最近成功 / 最近失败)· 为日后高并发铺路\n\n— 以下面向用户(不变) —\n\nv118.35.0.26 · 收入对账容差优化 + 系统监控扩展\n• 收入对账金额对比加入 ฿0.01 容差:小到分位的浮点差异不再误标『有差异』· 真差异仍清晰显示\n• 内部:Earn 后台『系统监控』新增内存使用 / CPU 负载 / 核数 / Worker 进程 4 张卡 · 全面运维视图\n\n— 以下面向用户(不变) — \n\nv118.35.0.24 · 按用量付费正式启用\n• 计费方式:发票识别按张计费,前 200 张每张 ฿1.50,第 201 张起每张 ฿0.75;Excel / CSV / Word 文件按字符计费,每 50 字符 ฿0.01\n• 余额查看:首页『账户余额』卡片可随时查看当前余额与本月用量\n• 余额不足时上传会清晰提示当前余额,可直接进入充值\n• 单次充值最低 ฿10、上限 ฿500,000\n• 银行流水 Excel 上传支持自动直读,识别更快更稳\n• 系统性能与稳定性持续优化",
            "th": "v118.35.0.42 · 🚨 แก้ด่วน:อัปโหลด GL Excel ขึ้น 0 แถว (อ่านปี พ.ศ. ไทยไม่ออก)\n• ลูกค้าแจ้ง:กระทบยอดรายได้ / กระทบยอดบัญชีธนาคาร อัปโหลด GL Excel แสดง 0 แถว · รายงานเสียทั้งฉบับ\n• ต้นเหตุ:คอลัมน์วันที่ใน Excel ของลูกค้าเก็บเป็น datetime ปี พ.ศ. (2568 = ค.ศ. 2025) · ฟังก์ชันอ่านวันที่ของเราเข้าใจผิดเป็นสตริง '2568-12-31 00:00:00' · แยกออกเป็น 4 ส่วนแล้วไม่รู้จัก → 0 แถว\n• วิธีแก้:ฟังก์ชันอ่านวันที่เพิ่มทางสำรอง 4 ชั้น:1) รับ datetime โดยตรง 2) แปลง พ.ศ. → ค.ศ. อัตโนมัติ (-543) 3) สตริง ISO มีเวลา ตัดเวลาออกอัตโนมัติ 4) yyyy-mm-dd ก็เพิ่ม พ.ศ. → ค.ศ.\n• ผล:ไฟล์ GL แบบเดียวกันอ่านได้ 1+ แถว · วันที่แสดงเป็น ค.ศ. ถูกต้อง\n• 8 การทดสอบ guard ล็อกรูปแบบวันที่ 8 แบบ:datetime พ.ศ. / date พ.ศ. / ISO str พ.ศ. / ISO T str / yyyy-mm-dd พ.ศ. / dd/mm/yy ปีสั้น / ค.ศ. สตริง / ขยะ → None\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.41 · ภายใน:เพิ่มคอลัมน์ JSONB『ติดตามฟิลด์ที่แก้เอง』ใน 4 ตาราง (เริ่ม Phase 1)\n• โครงสร้างพื้นฐาน:เพิ่มคอลัมน์ field_overrides JSONB ให้ 4 ตารางหลัก (อัปโหลดใบกำกับ / แถวกระทบยอดภาษีขาย / กระทบยอดรายได้ / กระทบยอดบัญชีธนาคาร)\n• ครั้งแรกที่ Pearnly รัน Alembic migration อย่างเป็นทางการ (เปิด REFACTOR-A2.2 + B3 · ผ่าน Phase 1 P1.1) · ยกระดับการแก้ schema จาก『สร้างตอนสตาร์ท』เป็น『migration แบบเวอร์ชัน』\n• ผู้ใช้ไม่รู้สึก:คอลัมน์เป็น NULL โดยปริยาย · ข้อมูลเก่าไม่แตะ\n• Phase 1 ต่อไป (P1.2-P1.7) ทุกคำว่า『OCR อ่านได้อะไร vs ผู้ใช้แก้อะไร』จะถูกเก็บในคอลัมน์เดียวกันนี้ · ใช้โครงสร้างเดียวร่วม 4 โมดูล\n• รันคู่:ไฟล์ Alembic 002 + ฟังก์ชัน ensure_field_overrides_columns() ตอนสตาร์ท · ป้องกัน prod ตก\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.40 · ป๊อปอัพทั่วไป / ลิ้นชัก · ระบบเลื่อนป้องกันสำรอง (ปิด Phase 0)\n• ตรวจภายใน:สแกนป๊อปอัพ / ลิ้นชัก 6 ประเภทในโปรเจกต์ · หาโรคเดียวกับ『ตั้งค่า』ที่เลื่อนถึงล่างไม่ได้ (เคยแก้ 2 รอบใน v118.34.39 / v118.35.0.35)\n• เพิ่ม『สลักเข็มขัด』(min-height:0) ให้ .modal (ป๊อปอัพทั่วไป) และ .drawer (ลิ้นชัก) · ป้องกันสกอร์ลบาร์ไม่ปรากฏเมื่อเบราว์เซอร์ซูม\n• ผู้ใช้ไม่รู้สึก:ปกติเหมือนเดิม · ต่างเฉพาะเมื่อเบราว์เซอร์ซูมทำ px rounding\n• เอกสารตรวจสอบครบ docs/audits/2026-05-23-modal-flex-chain-audit.md · ระบุ 6 ประเภท / ระดับความเสี่ยง / แผนแก้ (ลอกได้ตอนเจอบักจริง)\n• ไม่ใช้วิธีแก้เชิงรุก max-height calc · ป้องกัน『แก้ก่อนเกิดบักใหม่』· รอรายงานจริงค่อยอัปเป็นวิธีเชิงรุก\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.39 · รายละเอียดประวัติกระทบยอด · ตารางเปรียบเทียบการกรอกเอง (ปิดงาน BUG-B)\n• เมื่อกระทบยอดเสร็จ + ดูรายละเอียดงานในประวัติ:ใต้การ์ด KPI จะมีแผงสีส้มอ่อน『⚠ การกระทบยอดครั้งนี้มีการกรอกเอง』\n• ตาราง 4 คอลัมน์:ฟิลด์ Anchor / ค่า OCR (ตัวเทา) / ค่าที่คุณกรอก (พื้นเหลืองตัวหนา) / ผลต่าง (เขียวขึ้น แดงลง)\n• แสดงเฉพาะเมื่อคุณแก้ anchor จริง (งานเก่า / งานที่ไม่ได้แก้ จะไม่เห็น) · ไม่รบกวน\n• สลับภาษาจะรีเฟรชอัตโนมัติ · ครบ 4 ภาษา\n• ใช้ข้อมูลเดียวกับ section『ร่องรอยการกรอกเอง』ใน Excel v0.38 · ดูบนหน้าจอกับใน Excel ตรงกัน\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.38 · ไฟล์ Excel ส่งออก · เห็นร่องรอยการกรอกเองชัดเจน\n• ไฟล์ Excel ที่ส่งออกตอนนี้มีส่วน『ร่องรอยการกรอกยอดเอง』ท้ายไฟล์ · แสดง anchor ที่คุณแก้:OCR อ่านได้เท่าไหร่ · คุณแก้เป็นเท่าไหร่ · ต่างเท่าไหร่\n• เซลล์ที่คุณแก้ทำสีเหลือง(โทนอบอุ่น FFE082)· hover Excel cell เห็น OCR / User / ผลต่าง ครบ(comment)\n• หัวรายงานเพิ่มแถวเตือน『⚠ รายงานนี้มีการกรอกเอง · ดูตารางท้ายไฟล์』· ผู้ตรวจสอบเห็นทันที\n• ส่วนนี้แสดงเฉพาะเมื่อคุณแก้ anchor จริง(ไม่แก้ก็ไม่แสดง · ผู้ใช้เก่าไม่รู้สึก)\n• ครบ 4 ภาษา(zh / en / th / ja)\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.37 · กระทบยอดรายได้ · 3 ช่องกรอกยอด เติมค่า OCR ครั้งก่อนอัตโนมัติ\n• ปรับปรุง:เดิมทุกครั้งที่เข้าหน้ากระทบยอดต้องกรอก『ยอดยกไป GL / ยอดยกมา Statement / ยอดยกมา GL』ใหม่ตั้งแต่ศูนย์ · ตอนนี้เบราว์เซอร์จำค่าที่ OCR เคยอ่านได้ · เติมให้อัตโนมัติ · ถ้าไม่ตรงคลิกแก้ได้ทันที\n• แสดงเป็นตัวเอียงสีเทาพร้อมป้าย『· OCR』· มองปราดเดียวก็แยกออกว่าค่าใดมาจาก OCR ครั้งก่อน(อ้างอิง)· ค่าใดเป็นที่คุณกรอกเอง(ค่าจริง)\n• หลังบ้านบันทึก 3 ค่าที่ OCR อ่านได้ทุกครั้ง(ไม่ว่าคุณแก้หรือไม่)· เตรียมไว้สำหรับฟีเจอร์『ร่องรอยการกรอกเอง』ที่กำลังจะมา\n• ครบ 4 ภาษา(zh / en / th / ja)· ไม่กระทบโหมดสำรอง BUG-B เดิม\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.36 · กระทบยอดรายได้ กรอกยอด GL / Statement 3 ช่องเองได้\n• ลูกค้าแจ้งว่า:เมื่อ OCR อ่านยอดยกไป GL / ยอดยกมา STATEMENT / ยอดยกมา GL ไม่แม่น · รายงานกระทบยอดเสียทั้งฉบับ · ตอนนี้เพิ่ม 3 ช่องกรอกเอง · ถ้ากรอกจะใช้ค่าที่กรอก · ถ้าปล่อยว่างยังใช้ OCR\n• ตำแหน่ง:ใต้พื้นที่อัปโหลด Statement + GL · เหนือปุ่มบันทึก 3 ปุ่มที่มีอยู่ · เข้ากันกับดีไซน์ปัจจุบัน (พื้นสีเทาอุ่น / ขอบดำเมื่อ focus / ตัวเลขชิดขวา)\n• แสดงผลต่างทันที:เมื่อกรอก ยอดยกมา Statement + ยอดยกมา GL ครบ → แสดง『ผลต่างยอดยกมา = Statement ยกมา − GL ยกมา』ทันที · ให้บัญชีตาเปล่าตรวจสอบได้\n• หลังบ้านบันทึก:ค่าที่กรอก / ค่าที่ OCR อ่านได้ ถูกเก็บไว้ · เปิดงานเก่าก็เห็นว่าตัวไหนกรอกเอง\n• ครบ 4 ภาษา (zh / en / th / ja) · บนมือถือ 3 ช่องเรียงเป็นคอลัมน์เดียว\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.35 · ป๊อปอัพ『ตั้งค่า』ฟอร์มขวาเลื่อนได้แล้ว\n• ตอนซูมเบราว์เซอร์ในระดับต่าง ๆ (67% / 75% / 80% / 90% / 100% เป็นต้น) · ฟอร์มฝั่งขวาในป๊อปอัพ『ตั้งค่า』เลื่อนดูช่องด้านล่างไม่ได้ (ประเทศ / LINE ID / แท็บข้อมูลบริษัท ฯลฯ) · แก้แล้ว · ทุกระดับการซูมเลื่อนกรอกได้ปกติ\n• เป็นโรคเดียวกับที่เคยแก้แถบเมนูซ้ายในป๊อปอัพไปแล้ว (v118.34.39) · รอบนี้แก้ฝั่งขวา · ล็อกความสูง max-height โดยตรง · ไม่พึ่งกลไก flex chain อีก\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.33 · เคลียร์ส่วนที่เหลือของระบบสมัครสมาชิกเก่า\n• แอดมิน Earn (เฉพาะผู้ดูแลระบบ): ลบ UI ของระบบสมัครสมาชิกเก่าออก · การ์ด KPI กลุ่มแพ็กเกจ 4 ใบ / ส่วน Trial ใกล้หมดอายุ / หน้าต่างเปลี่ยนแพ็กเกจ (Trial · รายเดือน ฿299 · รายปี ฿2990 · ตลอดชีพ) / ปุ่มอัปเกรด / คอลัมน์แพ็กเกจ / ตัวกรองแพ็กเกจ / รายการรอตรวจสอบการชำระ · รวมเป็นโหมด credits คิดตามการใช้งานเท่านั้น\n• ฝั่งผู้ใช้: เมนูอวตาร『สมัครสมาชิก & แพ็คเกจ』เปลี่ยนเป็น『บัญชี & ยอดคงเหลือ』 · แท็บตั้งค่า『แพ็กเกจ & การใช้งาน』เปลี่ยนเป็น『การใช้งาน』 · ครบ 4 ภาษา · ไม่กระทบฟังก์ชัน\n• ฝั่งหลังบ้าน: ลบ 5 เส้นทาง admin ของระบบสมัครสมาชิกเก่าออกทั้งก้อน · ป้องกัน『กดอัปเกรดได้ผล』ผิดพลาด\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.32 · อัปเกรดเครื่องมือ build หน้าเว็บ (ขั้นแรกของแผนจัดระเบียบ A)\n• ภายใน: หน้าแรกใช้เครื่องมือ Vite ในการรวมไฟล์ JS · ขนาดไฟล์ลด 37% · โหลดเร็วขึ้นเล็กน้อย · ผู้ใช้งานไม่รู้สึกถึงการเปลี่ยนแปลง\n• โมดูลใหม่ทั้งหมดในอนาคตจะใช้สายงานนี้ · เป็นจุดเริ่มต้นของแผนจัดระเบียบโปรเจกต์ 5-8 เดือน\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.31 · ปรับปรุงความสามารถในการอ่านโค้ด (ต่อ)\n• ภายใน: เคลียร์อีก 12 จุดที่เพิกเฉยข้อผิดพลาดอย่างเงียบ + 3 จุดที่『ควรล็อกแต่ถูกกลืน』 เปลี่ยนเป็น console.warn เพื่อทิ้ง trace (การแยก error ของ network / การแปล i18n ที่ล้มเหลว) · ผู้ใช้ไม่รู้สึกถึงการเปลี่ยนแปลง · ทำให้ DevTools เห็นปัญหาที่ซ่อนอยู่\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.30 · ปรับปรุงความสามารถในการอ่านโค้ด\n• ภายใน: ใส่คอมเมนต์อธิบาย 15 จุดที่ 『เพิกเฉยข้อผิดพลาดอย่างเงียบ』 ว่าเป็นความตั้งใจ (โหมดส่วนตัว localStorage / ถูก abort ไปแล้ว / ถูก revoke ไปแล้ว ฯลฯ) · ผู้ใช้ไม่รู้สึกถึงการเปลี่ยนแปลง · ทำให้การดีบักรู้ว่า catch ตัวไหนตั้งใจเงียบ\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.29 · จัดระเบียบโค้ด (ต่อ)\n• ภายใน: หน้าต่างเติมเงิน + ตัวรีเฟรชยอดคงเหลือถูกแยกเป็นโมดูลของตัวเอง · ผู้ใช้ไม่รู้สึกถึงความเปลี่ยนแปลง · ต่อเนื่องจากเวอร์ชันก่อนหน้าเพื่อจัดระเบียบไฟล์ขนาดยักษ์\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.28 · จัดระเบียบโค้ด\n• ภายใน: ตัวโหลดแดชบอร์ดหน้าแรกถูกแยกเป็นโมดูลของตัวเอง · ผู้ใช้ไม่รู้สึกถึงความเปลี่ยนแปลง · เตรียมพื้นฐานสำหรับการจัดระเบียบโมดูลในอนาคต\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.27 · ตรวจสอบที่มาของรายงานหลายหน้า + โครงสร้างคิวงาน\n• การกระทบยอดภาษีขายเมื่อรวมหลายไฟล์ PDF · แต่ละรายการแนบ『ชื่อไฟล์ต้นทาง + เลขหน้า』· เมื่อพบความแตกต่างสามารถระบุได้ทันทีว่ามาจากไฟล์ใด หน้าใด (เดิมต้องเปิดทีละไฟล์ค้นเอง)\n• ภายใน:หน้าจอ Earn เพิ่ม 4 การ์ดติดตามคิวงาน (รอดำเนินการ / กำลังทำ / สำเร็จล่าสุด / ล้มเหลวล่าสุด)· เตรียมพร้อมสำหรับปริมาณงานสูง\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.26 · ปรับค่าเผื่อการกระทบยอดรายได้ + ขยายการติดตามระบบ\n• การเปรียบเทียบยอดเพิ่มค่าเผื่อ ฿0.01:ส่วนต่างเล็กระดับสตางค์จะไม่ถูกเตือนว่า『ไม่ตรง』อีก · ส่วนต่างจริงยังคงแสดงชัดเจน\n• ภายใน:Earn『การติดตามระบบ』เพิ่มการ์ดหน่วยความจำ / โหลด CPU / จำนวนคอร์ / โปรเซส Worker · ภาพรวมการทำงานครบถ้วน\n\n— ด้านล่างคือฟีเจอร์สำหรับผู้ใช้ (ไม่เปลี่ยนแปลง) —\n\nv118.35.0.24 · ระบบคิดเงินตามการใช้งานเปิดให้บริการแล้ว\n• อัตราค่าบริการ:ใบกำกับภาษี PDF 200 ใบแรก ฿1.50/ใบ · ใบที่ 201 ขึ้นไป ฿0.75/ใบ;ไฟล์ Excel / CSV / Word คิดตามตัวอักษร 50 ตัวอักษร = ฿0.01\n• ตรวจสอบยอด:การ์ด『ยอดคงเหลือ』บนหน้าแรก แสดงยอดและการใช้งานของเดือนนี้\n• เมื่อยอดเงินไม่พอ ระบบจะแสดงยอดปัจจุบันและค่าบริการที่ต้องชำระ พร้อมเข้าหน้าเติมเงินทันที\n• เติมเงินขั้นต่ำ ฿10 · สูงสุดต่อครั้ง ฿500,000\n• อัปโหลดไฟล์ Statement Excel รองรับการอ่านอัตโนมัติ · เร็วและเสถียรขึ้น\n• ปรับปรุงประสิทธิภาพและเสถียรภาพของระบบอย่างต่อเนื่อง",
            "en": "v118.35.0.42 · 🚨 Hotfix: GL Excel upload showed 0 rows (Thai Buddhist-era year not recognized)\n• Customer report: Income reconciliation / Bank reconciliation GL Excel upload showed 0 แถว (0 rows) · entire report broken\n• Root cause: The customer's Excel stores dates as Buddhist-era datetime (BE 2568 = CE 2025) · our date parser misread it as the string '2568-12-31 00:00:00' · split into 4 parts and didn't recognize → 0 rows\n• Fix: date parser now has 4 fallback layers: 1) datetime type direct passthrough 2) auto BE → CE conversion (-543) 3) ISO strings with time auto-strip time 4) yyyy-mm-dd path also adds BE → CE\n• Effect: same GL file now reads 1+ rows · date correctly displayed in CE\n• 8 guard tests lock down 8 date formats: datetime BE / date BE / ISO str BE / ISO T str / yyyy-mm-dd BE / dd/mm/yy short year / CE string / garbage → None\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.41 · Internal: 4-table『manual-edit field trail』JSONB column added (Phase 1 kickoff)\n• Infrastructure: 4 core tables (invoice upload / VAT-output rec rows / income rec / bank rec) each gain a field_overrides JSONB column\n• This is Pearnly's first real Alembic database migration (REFACTOR-A2.2 + B3 activated · via Phase 1 P1.1) · upgrading schema changes from『hard-build at startup』to『versioned migrations』\n• Zero user-visible impact: column defaults NULL · existing reconciliations / invoices untouched\n• Subsequent Phase 1 work (P1.2-P1.7) will store all『what OCR read vs what user edited』in this single column · shared structure across all 4 modules\n• Dual-run safety: Alembic 002 file + ensure_field_overrides_columns() startup hook · ensures prod doesn't miss\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.40 · Generic modal / drawer scroll defensive safeguard (Phase 0 wrap-up)\n• Internal audit: scanned 6 modal / drawer types in the project · looking for the same『can't scroll to bottom』bug as the Settings modal (fixed twice in v118.34.39 / v118.35.0.35)\n• Added a『safety pin』property (min-height:0) to .modal (generic modal) and .drawer · prevents the scrollbar from failing to appear when the browser zoom shifts\n• Zero user-visible change: everything works as before · only differs in edge cases caused by browser zoom px rounding\n• Full audit doc: docs/audits/2026-05-23-modal-flex-chain-audit.md · lists 6 modal types / risk tier / fix preset (copy-paste when a real bug hits)\n• Did NOT pre-apply the more aggressive max-height calc fix · avoids『pre-fixing introduces new bugs』· wait for a real bug report before upgrading to the aggressive fix\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.39 · Income reconciliation history detail · manual entry comparison table (BUG-B wrap-up)\n• When finishing a reconciliation + opening a history task detail: below the KPI cards a light-orange panel appears『⚠ This run contains manually entered anchors』\n• 4-column table: Anchor Field / OCR Value (grey) / Your Value (yellow background, bold) / Diff (green up, red down)\n• Only shown when you actually edited an anchor (old tasks / unedited tasks won't see it) · non-intrusive\n• Auto-refreshes on language switch · full 4-language coverage\n• Uses the same data source as the『Manual Entry Audit Trail』section in v0.38 Excel exports · on-screen matches what you get in Excel\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.38 · Income reconciliation Excel export · manual entry trail at a glance\n• The exported Excel now has a『Manual Entry Audit Trail』section at the end · lists every anchor you edited: OCR value · your value · diff\n• Cells you edited are highlighted yellow (warm tone FFE082) · hover the Excel cell to see OCR / User / Diff in full (Excel comment)\n• A new warning row at the top: '⚠ This report contains manually entered anchor values · see audit trail at bottom' · auditors can spot it instantly\n• Only appears when you actually edited an anchor (no edit · layout unchanged · existing users won't notice)\n• Full 4-language coverage (zh / en / th / ja)\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.37 · Income reconciliation · 3 anchor boxes now auto-prefill from your last OCR scan\n• Improvement: previously you had to refill the 3 boxes (GL closing / Statement opening / GL opening) from scratch every time · the browser now remembers what OCR read last time and auto-fills them · one click to edit if wrong\n• Grey italic text with a『· OCR』tag · easy to tell at a glance which values came from the last OCR scan (reference) vs. which you typed yourself (authoritative)\n• Backend always records the 3 OCR-scanned values (regardless of whether you edit them) · ready for the upcoming『manual entry audit trail』feature\n• Full 4-language coverage (zh / en / th / ja) · existing BUG-B fallback logic stays intact\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.36 · Income reconciliation · manual entry for the 3 anchor balances (GL / Statement)\n• Customer feedback: when OCR misreads the 3 anchor numbers (GL closing · Statement opening · GL opening), the entire reconciliation report breaks · 3 manual entry boxes added · if you fill them they take precedence, leave them blank and OCR is used\n• Position: below the bank statement + GL upload zone, above the existing 3 save buttons · matches the current design language (warm-grey background / black focus border / right-aligned numbers)\n• Live equation hint: when both Statement opening + GL opening are filled, the『opening difference = Statement opening − GL opening』is shown immediately so accountants can sanity-check by eye\n• Backend trace: both the user-entered value and the OCR value are stored · when reopening a task you can see which anchors were manually overridden\n• Full 4-language coverage (zh / en / th / ja) · on mobile the 3 boxes stack into a single column\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.35 · Settings modal right-pane scroll fully fixed\n• At various browser zoom levels (67% / 75% / 80% / 90% / 100% etc.) the right-side form in the Settings modal could not scroll to bottom fields (Country / LINE ID / Company Info tab etc.) · fixed · scrolls normally at any zoom level\n• Same family of bug as the left side-nav scroll fix (v118.34.39) · this time on the right pane · directly pins max-height · no longer depends on the flex chain to drive overflow\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.33 · Cleanup of legacy subscription residue (user-side + Earn back-office)\n• Earn back-office (super-admin only): legacy subscription UI removed — 4 plan-distribution KPI cards / Trial-expiring section / upgrade-plan modal (Trial · Monthly ฿299 · Yearly ฿2990 · Lifetime) / upgrade button / plan column / plan filter / pending-payment list · fully unified to the credits pay-as-you-go model\n• User side: avatar menu 'Billing & Plan' renamed 'Account & Balance' · settings tab 'Plan & Usage' renamed 'Usage' · all 4 languages · zero functional impact\n• Backend: 5 legacy subscription admin routes deleted in full (POST /api/admin/users/upgrade · GET /api/admin/payments/pending · GET /api/admin/payments/{id}/screenshot · POST /api/admin/payments/{id}/review · GET /api/admin/users/funnel slimmed) · prevents 'upgrade still goes through' misoperations\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.32 · Frontend build-tool upgrade (cleanup phase A · step one)\n• Internal: the homepage frontend JS is now bundled with Vite · 37% smaller payload · slightly faster load · zero user-visible change\n• All future modules will go through this pipeline · the engineering starting point of the 5-8 month cleanup\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.31 · Code readability improvement (cont'd)\n• Internal: 12 more silently-swallowed exceptions annotated + 3 'should-have-logged-but-swallowed' spots converted to console.warn for traceability (network error detail parsing / i18n translation failures) · zero user-visible change · DevTools now surfaces previously-hidden problems\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.30 · Code readability improvement\n• Internal: 15 'silently-swallowed exception' spots now carry inline comments explaining the intent (private-mode localStorage / already aborted / already revoked etc.) · zero user-visible change · easier to tell apart intentional silence from real bugs when debugging\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.29 · Code reorganization (cont'd)\n• Internal: the top-up modal + balance auto-refresh logic split into its own module · zero user-visible change · continues the previous release's cleanup of the mega-file\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.28 · Code reorganization\n• Internal: the homepage dashboard loader has been split into its own module · zero user-visible change · groundwork for future modularization\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.27 · Multi-page report traceability + task queue foundation\n• When several VAT-output PDFs are merged for reconciliation, each row now carries 'source filename + page number' · when a mismatch is found you can jump straight to the exact source file and page (previously you had to search by memory)\n• Internal: the Earn back-office system monitor adds 4 task-queue cards (pending / running / recent success / recent failure) · laying the groundwork for high concurrency\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.26 · Income-reconciliation tolerance + monitoring expansion\n• Amount comparison now allows a ฿0.01 tolerance: sub-cent floating-point drift is no longer flagged as a mismatch; real differences are still shown clearly\n• Internal: the Earn 'System monitor' panel adds memory use / CPU load / core count / worker processes cards · complete ops view\n\n— Below: features for end users (unchanged) —\n\nv118.35.0.24 · Pay-as-you-go billing now live\n• Pricing: PDF invoice recognition at ฿1.50 per page for the first 200 pages, ฿0.75 per page from page 201; Excel / CSV / Word imports at ฿0.01 per 50 characters\n• Balance at a glance: the homepage 'Account Balance' card shows your current balance and this month's usage\n• When balance is low, upload prompts clearly show your current balance, with one-tap access to top up\n• Top-up minimum ฿10, single-transaction cap ฿500,000\n• Bank-statement Excel uploads are now read directly for faster, more reliable results\n• Ongoing performance and stability improvements",
            "ja": "v118.35.0.42 · 🚨 緊急修正:GL Excel アップロード 0 行(タイ仏暦年認識不可)\n• ユーザー報告:収益照合 / 銀行照合の GL Excel アップロードが 0 แถว(0 行)を表示 · レポート全体無効\n• 根本原因:お客様の Excel は日付列を仏暦 datetime(BE 2568 = CE 2025)で保存 · 日付パーサーが文字列 '2568-12-31 00:00:00' と誤認識 · 4 つに分割して認識不可 → 0 行\n• 修正:日付パーサーに 4 層フォールバック追加:1) datetime 型直接通過 2) 仏暦 → 西暦自動変換 (-543) 3) ISO 文字列の時刻部分自動削除 4) yyyy-mm-dd ルートにも仏暦変換追加\n• 効果:同じ GL ファイルが 1+ 行読み込み可能に · 日付は西暦で正しく表示\n• 8 つの guard テストで 8 種類の日付形式をロック:datetime BE / date BE / ISO str BE / ISO T str / yyyy-mm-dd BE / dd/mm/yy 短年 / CE 文字列 / ゴミ → None\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.41 · 内部:4 テーブルに『手動編集フィールド追跡』JSONB カラム追加(Phase 1 開始)\n• 基盤整備:4 つのコアテーブル(請求書アップロード / 売上税照合行 / 収益照合 / 銀行照合)に field_overrides JSONB カラムを追加\n• Pearnly 初の正式 Alembic データベースマイグレーション実行(REFACTOR-A2.2 + B3 起動 · Phase 1 P1.1 経由)· schema 変更を『起動時ハードビルド』から『バージョン管理マイグレーション』に昇格\n• ユーザーへの影響なし:カラムはデフォルト NULL · 既存の照合 / 請求書データは不変\n• Phase 1 続編(P1.2-P1.7)で『OCR が読んだ値 vs ユーザーが編集した値』を全てこのカラムに統一保存 · 4 モジュール共通構造\n• デュアルラン安全策:Alembic 002 ファイル + ensure_field_overrides_columns() 起動フック · prod の取りこぼし防止\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.40 · 汎用モーダル / ドロワーのスクロール予防的安全網(Phase 0 仕上げ)\n• 内部監査:プロジェクト内の 6 種類のモーダル / ドロワーをスキャン · 『設定』モーダルと同じ底までスクロールできないバグを探した(v118.34.39 / v118.35.0.35 で 2 回修正済)\n• .modal(汎用モーダル)と .drawer(ドロワー)に『安全ピン』属性(min-height:0)を追加 · ブラウザのズーム ずれ時にスクロールバーが出ないのを防ぐ\n• ユーザーへの影響なし:通常時は従来通り · ブラウザズームの px 丸めが発生したエッジケースのみ違いが出る\n• 完全監査ドキュメント:docs/audits/2026-05-23-modal-flex-chain-audit.md · 6 種類 / リスク階層 / 修正プリセット(実際にバグが出たらコピペ)を一覧\n• より積極的な max-height calc 修正は事前適用していない · 『先回り修正で新バグ』を防ぐため · 実際のバグ報告を待ってから積極的修正へ\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.39 · 収益照合履歴詳細 · 手入力比較表(BUG-B 仕上げ)\n• 照合完了後 + 履歴タスクの詳細を開いた時:KPI カードの下に薄オレンジパネル『⚠ 今回の照合は手入力アンカーを含む』を表示\n• 4 列のテーブル:アンカー項目 / OCR 値(グレー)/ 入力値(黄色背景・太字)/ 差額(プラスは緑・マイナスは赤)\n• 実際にアンカーを編集した時のみ表示(旧タスク / 未編集タスクには表示されない)· 邪魔にならない\n• 言語切替時に自動再描画 · 全 4 言語対応\n• v0.38 の Excel 出力『手入力履歴』セクションと同一データソース · 画面と Excel の内容が一致\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.38 · 収益照合 Excel 出力 · 手入力履歴が一目瞭然\n• 出力した Excel ファイルの末尾に『手入力履歴』セクションを追加 · 編集したアンカーごとに OCR 値 · 入力値 · 差額を表示\n• 編集したセルは黄色(暖色 FFE082)でハイライト · Excel セルを hover すると OCR / User / 差額の完全対照(コメント)を表示\n• レポート上部に警告行『⚠ 本レポートは手入力アンカーを含む · ファイル末尾の照合表参照』· 監査担当者が即時認識可能\n• 実際にアンカーを編集した時のみ表示(編集なし · レイアウト不変 · 既存ユーザーに影響なし)\n• 全 4 言語対応(zh / en / th / ja)\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.37 · 収益照合『3 つの残高ボックス』が前回 OCR 値を自動入力\n• 改善:従来は照合ページを開くたびに『GL 期末 / Statement 期首 / GL 期首』の 3 ボックスをゼロから入力する必要があった · ブラウザが前回 OCR で読み取った値を記憶し自動入力 · 違えばクリック一つで修正可能\n• グレーのイタリック体に『· OCR』タグ付き · どの値が前回 OCR の参照値で、どの値が自分で入力した正式な値かを一目で識別可能\n• バックエンドは 3 つの OCR 読取値を常に記録(編集の有無に関わらず)· 近日公開の『手入力履歴』機能で活用予定\n• 全 4 言語対応(zh / en / th / ja)· 既存 BUG-B フォールバック機能はそのまま維持\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.36 · 収益照合『GL / Statement 3 つの残高』手入力対応\n• お客様からの要望:OCR が GL 期末残高 / Statement 期首残高 / GL 期首残高 の 3 つの『アンカー』を誤読すると、照合レポート全体が無効になる · 手入力ボックス 3 つを追加 · 入力された値が優先 · 空欄なら OCR を使用\n• 位置:Statement + GL アップロード領域の下、既存の保存ボタン 3 つの上 · 現行のデザイン言語に統一(暖色グレー背景 / 黒フォーカス枠 / 右寄せ数値)\n• ライブ差額表示:Statement 期首 + GL 期首 を入力すると『期首差額 = Statement 期首 − GL 期首』を即時表示 · 経理担当者が目視で確認可能\n• バックエンド記録:入力値 / OCR 値 の両方を保存 · 過去タスクを開いた際にどのアンカーを手動上書きしたか確認可能\n• 全 4 言語対応 (zh / en / th / ja) · モバイルでは 3 つのボックスが 1 列に積み重なる\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.35 · 設定モーダル右ペインのスクロールを完全修正\n• ブラウザを各種ズームレベル(67% / 75% / 80% / 90% / 100% 等)に変更した際、『設定』モーダルの右側フォームが下のフィールド(国 / LINE ID / 会社情報タブ等)までスクロールできなかった · 修正済 · どのズーム倍率でも正常にスクロール入力可能\n• 以前修正した左側ナビゲーションのスクロール問題(v118.34.39)と同系統のバグ · 今回は右ペイン · max-height を直接固定 · flex chain に依存しない方式へ\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.33 · 旧サブスク残骸の整理(ユーザー側 + Earn 管理画面)\n• Earn 管理画面(スーパー管理者のみ):旧サブスク UI を削除 · プラン分布 KPI 4 枚 / Trial 期限間近セクション / プラン変更モーダル(Trial · 月額 ฿299 · 年額 ฿2990 · ライフタイム)/ アップグレードボタン / プラン列 / プランフィルタ / 入金承認リスト · 完全に credits 従量課金モデルに統一\n• ユーザー側:アバターメニュー『サブスクリプション・プラン』を『アカウント・残高』に変更 · 設定タブ『プラン & 使用量』を『使用量』に変更 · 全 4 言語 · 機能への影響なし\n• バックエンド:旧サブスクの管理者ルート 5 本を全て削除(POST /api/admin/users/upgrade · GET /api/admin/payments/pending · GET /api/admin/payments/{id}/screenshot · POST /api/admin/payments/{id}/review · GET /api/admin/users/funnel スリム化)· 『アップグレードが通ってしまう』誤操作を防止\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.32 · フロントエンドビルドツールのアップグレード(整備フェーズ A 第一歩)\n• 内部:ホーム画面のフロントエンド JS を Vite で一括ビルドするように変更 · ファイルサイズが 37% 縮小 · 読み込みがやや高速化 · ユーザーへの影響なし\n• 今後の新モジュールはすべてこのパイプラインを通る · 5-8 か月の整備計画におけるエンジニアリング起点\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.31 · コード可読性の向上(続き)\n• 内部:さらに 12 箇所の沈黙して握りつぶした例外にコメント追加 + 3 箇所の『本来ログすべきだったが握りつぶしていた』箇所を console.warn に変換しトレース可能に(ネットワークエラー詳細解析 / i18n 翻訳失敗)· ユーザーへの影響なし · DevTools で隠れていた問題が見えるように\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.30 · コード可読性の向上\n• 内部:15 箇所の『例外を黙って無視している』コードに、なぜ意図的に無視しているか(プライベートモードの localStorage / 既に abort 済み / 既に revoke 済み 等)を説明するコメントを追加 · ユーザーへの影響なし · デバッグ時に意図的な silent と本物のバグを見分けやすくする\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.29 · コード整理(続き)\n• 内部:チャージ画面 + 残高自動更新のロジックを独立モジュールに分離 · ユーザーには変化なし · 前バージョンに続く巨大ファイル整理\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.28 · コード整理\n• 内部:ホーム画面ダッシュボードのローダーを独立モジュールに分離 · ユーザーには変化なし · 今後のモジュール化整備の地ならし\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.27 · 複数ページレポートの出典追跡 + タスクキュー基盤\n• 売上税レポートを複数 PDF 統合した際、各行に『出典ファイル名 + ページ番号』を付与 · 差異発生時は元 PDF の該当ページへ即座に辿れます(従来は記憶頼りで探していました)\n• 内部:Earn 管理画面のシステム監視にタスクキュー 4 種(待機中 / 実行中 / 直近成功 / 直近失敗)を追加 · 大規模同時処理に備えた基盤\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.26 · 収益照合の許容差 + 監視拡張\n• 金額比較に ฿0.01 の許容差を導入:銭以下の浮動小数誤差を『不一致』と誤判定しなくなりました · 本当の差額は引き続き明確に表示\n• 内部:Earn『システム監視』にメモリ使用量 / CPU 負荷 / コア数 / Worker プロセスのカードを追加 · 運用全体像を可視化\n\n— 以下はエンドユーザー向け機能(変更なし) —\n\nv118.35.0.24 · 従量課金プランの提供を開始\n• 料金体系:PDF 請求書の認識は最初の 200 枚まで 1 枚 ฿1.50、201 枚目から 1 枚 ฿0.75;Excel / CSV / Word の取り込みは 50 文字あたり ฿0.01\n• 残高表示:ホーム画面の『アカウント残高』カードで現在の残高と当月利用量を常時確認可能\n• 残高不足時にはアップロード画面で現在の残高を提示し、そのままチャージ画面へ遷移可能\n• チャージは最低 ฿10、1 回あたりの上限 ฿500,000\n• 銀行明細 Excel のアップロードを自動直接読取に対応・より高速で安定\n• システムのパフォーマンスと安定性を継続的に改善",
            "zh_archived_v34_19_old": "(已归档 · 上一版本说明已被本次更新替代)",
