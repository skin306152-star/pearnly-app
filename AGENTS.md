...
> **下个 task = 模块化深化继续拆 >500**(✅`bank_recon_v2` 6745→422 + ✅`gl_vat_reconciler` 1423→72 收官(7 刀 verbatim·facade 纯壳·recon 引擎全 <500)/ 接 `recon_routes`2000(路由组拆·含计费高敏)→ ERP 周边 → 报表 → 前端 `src/home/*`)。**完整清单见 STATE 状态卡**。**目录重组(86 root .py→包)= 最后做**(文件全 <500 后)。
