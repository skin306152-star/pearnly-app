...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.19 · 推送日志移到集成主页面顶部 tab · 抽屉清空日志 DOM:\n• 上窗口你截图说的:ERP 抽屉里的推送日志被窄抽屉压缩看不清\n• 集成主页面 (home#/integrations) 顶部加 2 个 tab 切换:\n  - tab 1「集成卡片」:现有 integration-row 列表(Google Drive / Sheets / Gmail / LINE / 文件夹 / ERP / 智能提醒)\n  - tab 2「推送日志」:全宽全屏视图 · filter chips + 批量重推 + 友好文案 + 查看原文折叠 · 不再被抽屉宽度压扁\n• ERP 抽屉里(int-drawer)里的 erp-subpanel='logs' 整块删 + erp-subtab logs 按钮删 · 只留 连接 / 字段映射 2 tab\n• 集成页 ERP 卡片 + ERP 抽屉里 ERP 连接卡片 都加「看推送日志 →」链接 · 点了关抽屉 + 跳主页 tab 2 · 自动 scroll 顶\n• int-tab-cards / int-tab-logs / int-btn-view-logs 4 语全补 · subscribeI18n 切语言时 integrations 路由也刷新 erp logs\n\n刷浏览器 → 进集成页 → 顶部应该有 2 个 tab · 点「推送日志」看是不是全宽 · ERP 抽屉里应该没有「推送日志」tab 了",
            "en": "v118.34.19 · Push log promoted to integrations page top-tab · drawer cleared of logs DOM:\n• Last screenshot you sent: push log inside the ERP drawer was squashed by the narrow drawer width.\n• Integrations page (home#/integrations) now has 2 top tabs:\n  - tab 1 'Integrations': existing integration-row list (Google Drive / Sheets / Gmail / LINE / Folder / ERP / Alert)\n  - tab 2 'Push Log': full-width view · filter chips + batch retry + friendly messages + view-raw fold · no longer squashed by drawer width\n• The ERP drawer (int-drawer)'s erp-subpanel='logs' block + the 'logs' erp-subtab button deleted · only 连接 / 字段映射 (Connect / Mappings) remain inside the drawer\n• Both the ERP integration-row on the integrations page AND the connect subpanel inside the ERP drawer got a 'View push log →' link · click closes drawer + jumps to integrations#/tab=logs + auto-scrolls to top\n• int-tab-cards / int-tab-logs / int-btn-view-logs in all 4 langs · subscribeI18n rerender hook now refreshes erp logs on integrations route too\n\nRefresh → open Integrations page → top should show 2 tabs · click 'Push Log' to confirm it's full-width · the ERP drawer should NO LONGER have a 'Push Log' tab inside.",
            "th": "v118.34.19 · ย้ายบันทึกการส่งไปแท็บบนสุดของหน้า Integrations · ล้าง DOM ในลิ้นชัก:\n• สกรีนช็อตล่าสุดที่ส่งมา: บันทึกการส่งใน ERP drawer ถูกบีบจนแคบเกินไปอ่านไม่ออก\n• หน้า Integrations (home#/integrations) ตอนนี้มีแท็บบนสุด 2 อัน:\n  - แท็บ 1 'การ์ดเชื่อมต่อ': รายการ integration-row เดิม (Google Drive / Sheets / Gmail / LINE / โฟลเดอร์ / ERP / แจ้งเตือน)\n  - แท็บ 2 'บันทึกการส่ง': มุมมองเต็มความกว้าง · filter chips + ส่งซ้ำหลายรายการ + ข้อความที่อ่านง่าย + กางดูข้อความดิบ · ไม่ถูกบีบจากความกว้าง drawer อีกแล้ว\n• ลบ erp-subpanel='logs' + ปุ่ม erp-subtab='logs' ออกจาก ERP drawer (int-drawer) · เหลือแค่ 連接 / 字段映射\n• ทั้งการ์ด ERP บนหน้า Integrations และการ์ดเชื่อมต่อใน ERP drawer มี link 'ดูบันทึก →' · คลิกแล้วปิด drawer + กระโดดไปแท็บ 2 + scroll ขึ้นบน\n• int-tab-cards / int-tab-logs / int-btn-view-logs ครบ 4 ภาษา · subscribeI18n hook รีเฟรช erp logs บน route integrations ด้วย\n\nรีเฟรช → เปิดหน้า Integrations → ด้านบนควรมีแท็บ 2 อัน · กด 'บันทึกการส่ง' ดูว่าเต็มจอไหม · ใน ERP drawer ไม่ควรมีแท็บ 'บันทึกการส่ง' แล้ว",
            "ja": "v118.34.19 · プッシュログを Integrations ページ最上部タブに昇格 · ドロワーからログ DOM 削除:\n• 直近のスクショ: ERP ドロワー内のプッシュログが狭い幅で見づらかった\n• Integrations ページ (home#/integrations) 最上部に 2 タブ追加:\n  - タブ 1「連携カード」: 既存の integration-row リスト (Google Drive / Sheets / Gmail / LINE / フォルダ / ERP / 通知)\n  - タブ 2「送信ログ」: フル幅表示 · フィルターチップ + 一括再送 + わかりやすいメッセージ + 原文展開 · ドロワー幅に圧縮されない\n• ERP ドロワー (int-drawer) 内の erp-subpanel='logs' ブロック + 'logs' erp-subtab ボタン削除 · ドロワー内は 連接 / 字段映射 のみ\n• Integrations ページの ERP カード + ERP ドロワー内 connect カードの両方に「送信ログを見る →」リンク追加 · クリックでドロワーを閉じてタブ 2 へジャンプ + 自動スクロール\n• int-tab-cards / int-tab-logs / int-btn-view-logs を 4 言語追加 · subscribeI18n が integrations ルートでも erp logs を再読み込み\n\nブラウザ更新 → Integrations を開く → 上部に 2 タブが表示されるはず · 「送信ログ」をクリックして全幅か確認 · ERP ドロワー内に「送信ログ」タブが残っていないことも確認"
        }
    }
