// POS 项目 · PO-A1 前端配套 · 主程序导航按租户模块开关显隐
// 接 GET /api/me/modules(信封 {ok,data:{modules}})· 04 §2 · core/pos_api。
// 收银业务组默认隐藏(app-shell style=display:none);此模块据开关揭示:
//   inventory 开 → 显「库存」· pos 开 → 显「销售报表 / 切到收银台」· 任一开 → 显整组。
// 既有租户 inventory/pos 默认关 → 整组对所有人不可见,直到 onboarding 开通。
/* global apiGet */
...
function apply(modules: Record<string, ModuleFlag>) {
    ...
    show(document.getElementById('nav-group-pos'), inv || pos);
