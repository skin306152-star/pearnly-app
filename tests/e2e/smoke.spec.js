...
test('着陆页 smoke · 加载 + 顶栏 + 4 语切换 + 无 console error', async ({ page }) => {
    ...
    await expect(page.locator('[data-open-auth="login"]').first(), '顶栏登录按钮').toBeVisible();
    await expect(
    ...
    for (const lang of ['zh', 'en', 'ja', 'th']) {
        ...
        await expect(page.locator('#lang-dd-menu'), `${lang} · 菜单展开`).toHaveClass(/show/);
        ...
        expect(LANG_TEXT_OK[lang](txt), `${lang} · 顶栏登录文字字符集 (got="${txt}")`).toBe(true);
    }
