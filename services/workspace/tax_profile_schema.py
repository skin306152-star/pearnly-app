...
def ensure_tax_profile_schema() -> None:
    ...
    # 工资进料 2 表(批次 H · alembic 0072 双跑)挂本 ensure 链尾:startup.py 已贴 500 行
    # 上限,照 seller_routing 挂本函数的同款接法(startup → ensure_seller_route_table →
    # 本函数 → ensure_payroll_schema)。独立 try:失败只告警,不连坐画像表建表。
    try:
