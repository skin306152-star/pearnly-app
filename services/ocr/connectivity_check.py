...
def main():
    ...
    if not creds_path:
        _fail(1, "GOOGLE_APPLICATION_CREDENTIALS set",
    ...
    try:
        ...
        _fail(2, "Credentials file can be opened",
              f"{type(e).__name__}: {e}")

    required_keys = ["type", "project_id", "private_key", "client_email"]
    missing = [k for k in required_keys if k not in data]
    if missing:
        _fail(2, "JSON has service_account fields",
              f"missing keys: {missing}")
    if data.get("type") != "service_account":
        _fail(2, "JSON type is service_account",
              f"got type={data.get('type')!r}")
    project_id = data.get("project_id")
    client_email = data.get("client_email")
    _print(f"JSON parsed: project_id={project_id} client_email={client_email}",
           "PASS")
    ...
    try:
        ...
        _fail(3, "import google.cloud.vision",
              f"{e}. Install with: pip install google-cloud-vision")
    ...
    try:
        ...
        _fail(4, "vision.ImageAnnotatorClient()",
              f"{type(e).__name__}: {e}")
    ...
    try:
        ...
        _fail(5, "Vision API document_text_detection call",
              f"{type(e).__name__}: {e}")

    error = getattr(resp, "error", None)
    if error is not None and getattr(error, "code", 0) != 0:
        _fail(5, "API response no error",
              f"code {error.code}: {error.message}")
    full_text_len = len(resp.full_text_annotation.text or "")
    _print(f"API call OK (latency: {elapsed_ms}ms, "
           f"full_text_len: {full_text_len})", "PASS")
    ...
    print(f"  Endpoint:      vision.googleapis.com (direct, no proxy)")
