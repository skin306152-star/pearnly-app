...
from routes.ocr_recognize_routes import router as ocr_recognize_router  # OCR
...
from routes.vat_report_checks_routes import (
    router as vat_report_checks_router,
)  # N1-a · 销项税报告三查(连号/买家分组/期间) · 2026-07-11
from routes.fileconv_routes import (
    router as fileconv_router,
)  # K1b · 财务文件转换(PDF→结构化结果/xlsx·pearnly_ai_m1 闸) · 2026-07-12
from routes.payroll_routes import (
    router as payroll_router,
)  # H1b · ภ.ง.ด.1 工资预扣进料工具卡(pearnly_ai_m1 闸) · 2026-07-13
...
from routes.inventory_report_routes import (
    router as inventory_report_router,
)  # POS 库存报表 C1 · 2026-06-07
from routes.pos_auth_routes import (
    router as pos_auth_router,
...
from routes.pos_modules_routes import (
    router as pos_modules_router,
)  # POS 设置页模块管理 C3 · 2026-06-07
from routes.pos_restaurant_routes import (
    router as pos_restaurant_router,
)  # 餐厅 POS 前台/后厨/埋单 PO-R · 2026-06-07
...
from routes.pos_sales_log_routes import (
    router as pos_sales_log_router,
)  # POS 交易明细日志 + CSV 导出(老板后台)· 2026-07-08
...
)  # 集成中心 Google Drive/Sheets 授权 · 2026-06-14
from routes.line_liff_routes import (
    router as line_liff_router,
)  # LINE LIFF 鉴权 + 复核屏入口 · 2026-06-14
...
from routes.sales_settings_routes import (
    router as sales_settings_router,
)  # 销项开票设置 §M7 · 2026-06-06
...
from routes.console_roles_routes import router as console_roles_router  # 权限 G3 · 自定义角色
...
from routes.dms_roster_routes import router as dms_roster_router  # DL-8
...
from routes.admin_pearnly_ai_routes import router as admin_pearnly_ai_router  # AI 邀请
from routes.admin_dms_routes import router as admin_dms_router  # DMS 邀请
...
app.include_router(vat_report_checks_router)  # N1-a · 销项税报告三查 · 2026-07-11
app.include_router(fileconv_router)  # K1b · 财务文件转换(pearnly_ai_m1 闸) · 2026-07-12
app.include_router(payroll_router)  # H1b · ภ.ง.ด.1 工资预扣工具卡(pearnly_ai_m1 闸) · 2026-07-13
...
app.include_router(console_roles_router)  # 权限 G3 · 自定义角色 5 路由(2026-06-11)
...
app.include_router(client_import_router)  # IN-0d · 客户名录批量导入(parse/commit) · 2026-07-15
...
app.include_router(admin_dms_router)  # DMS 订车入口邀请管理 4 路由
