...
> **第四十一/二会话续(2026-05-27 · home.js 7006 行)**:`7195d63` 抽 ERP 集成页(10071→8941);`54bd1f1` 抽 v109.3 大 IIFE **后半 admin 用户管理** → `src/home/admin-users.js`(8941→7006)。**v109.3 大 IIFE 现只剩前半**(套餐/PLG/试用横幅/LINE 绑定 + `window.fetch` 429 拦截 + `loadPlan` 60s 轮询 · ~L5687-6441 · 写 `_userInfo`/`_quota` + 裸调 5 render · 计费改只剩充值后**大半死码** → 清不是搬):下一步结合「只剩充值」核实可达性后删套餐死码,留真活的 429/LINE。**admin-users 模块实测对超管不可达(Earn 落独立 /admin SPA)= 基本死码,已隔离 → 确认全角色不可达可整模块物理删。**
