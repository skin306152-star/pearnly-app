...
_COORD_TIME = re.compile(r"^\d{1,2}:\d{2}")  # 09:06:55 交易时间 · 摘要里剔掉
...
def _coords_by_xcluster(doc):
    ...
    for pno in range(doc.page_count):
        ...
        # 摘要候选文字(非金额/日期/时间)· 留 x0 用于按列定位
        texts = [
            (w[0], w[1], w[4])
            for w in words
            if not _COORD_AMT.match(w[4])
            and not _COORD_DATE.match(w[4])
            and not _COORD_TIME.match(w[4])
        ]
        per_page.append((amts, dates, texts))
    ...
    bal_c = sig[-1]
    amt_cs = sig[:-1]
    # 摘要列右界:最左金额列左缘 − 8(摘要文字都在金额列左侧)
    desc_x_max = min(c[2] for c in amt_cs) - 8
    ...
    def desc_for(texts, by):
        """本笔摘要:金额列左侧、本行 y 区间(+16px 跨明细行)的文字拼接(渠道/详情/对方)。
        按(行 y · 再列 x)排序保阅读顺序 — 否则上下两行的词会被 x 交错打乱。"""
        toks = [
            (y0, x0, t) for x0, y0, t in texts if 95 <= x0 < desc_x_max and by - 4 <= y0 < by + 16
        ]
        toks.sort(key=lambda p: (round(p[0] / 4), p[1]))  # y 归行(4px 容差)→ 行内按 x
        return " ".join(t for _y, _x, t in toks)[:120]

    rows: List[StatementRow] = []
    prev = None
    for amts, dates, texts in per_page:
        ...
        for _bx, by, bv in bals:
            ...
            desc = desc_for(texts, by)
            # 方向按余额涨跌(prev 未知时暂记存款 · 交 _correct_direction 兜底)
            if prev is not None and b < prev:
                rows.append(
                    StatementRow(date=d, description=desc, withdrawal=amt, deposit=0.0, balance=b)
                )
            else:
                rows.append(
                    StatementRow(date=d, description=desc, withdrawal=0.0, deposit=amt, balance=b)
                )
            prev = b
