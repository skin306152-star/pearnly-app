...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.35.0.18 · 注册失败提示更友好:\n• 注册时如果系统出意外故障(数据库连接超时 / 邮件服务异常等),用户原本会看到一串吓人的英文报错。现在改成一句人话:『注册失败 · 请稍后重试 · 如持续异常请联系 hello@pearnly.com』· 4 语都翻好\n• 同时管理后台语言菜单精简到中文/泰文(其它语种没翻完先收起,免得点了空白)· 管理员『更新余额』输入框换成漂亮的卡片样式",
            "th": "v118.35.0.18 · ข้อความตอนสมัครล้มเหลวอ่อนโยนขึ้น:\n• ถ้าระบบล่มกลางคันตอนสมัคร (DB ล่ม / อีเมลเซิร์ฟไม่ตอบ ฯลฯ) ผู้ใช้เคยเห็นข้อความภาษาอังกฤษน่ากลัว · ตอนนี้เปลี่ยนเป็น 'สมัครไม่สำเร็จ · ลองอีกครั้งภายหลัง · ติดต่อ hello@pearnly.com' ครบ 4 ภาษา\n• เมนูภาษาในหลังบ้านเหลือ ไทย/中文 · กล่องอัปเดตยอดคงเหลือใช้การ์ดสไตล์เดียวกับระบบ",
            "en": "v118.35.0.18 · Friendlier signup-failure message:\n• If the system hits an unexpected failure during signup (DB timeout, mail service down, etc.), users previously saw a raw English exception. Now they get a plain-language line: 'Signup failed · please retry · if it persists contact hello@pearnly.com' (4 langs)\n• Also: admin language picker trimmed to ZH/TH; admin balance-update dialog switched to our card-style modal",
            "ja": "v118.35.0.18 · 登録失敗メッセージをより親切に:\n• 登録中にシステム障害(DB 接続切れ・メール送信エラー等)が発生した場合、従来は英語の例外文字列がそのまま表示されていた · 4 言語で『登録に失敗しました · 後ほど再試行してください · 続く場合は hello@pearnly.com まで』に変更\n• 同時に管理画面の言語切替を中/タイのみに整理 · 残高更新ダイアログを自社カード UI に置き換え",
