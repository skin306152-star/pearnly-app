 ...
 * 向导只露 账号/密码/订车单号前缀;保存 adapter=mrerp_dms · config.id_card_auto_push=true ·
 * auto_push=false(后端再兜底 false,防发票自动推送误投)。凭据明文塞 username_enc/password_enc,
 * 路由 kms 加密后落地(前端不加密不留明文)。挂 window.DXCONNECT + window._mrerpDmsOpenWizard。 */
(function (root) {
    'use strict';
    ...
    function renderCard() {
        ...
        host.innerHTML =
            ...
            esc(status) +
            '</span></div><div class="dx-erp-acts">' +
            acts +
            '</div></div></div>';
    }
    ...
    // 向导上下文卡入口(dms-intake-erp-cards.js 的「配置」按钮)。
