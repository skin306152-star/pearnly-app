...
docs commit:`4bdd0465`/`0fa96777`/`09c4c7b3`/`0f915e5c`。**8 个 commit CI 全 success**(末次 `0f915e5c` 绿=master 绿)。
...
3. **enroll 落点**(优先级):有现成 startup ensure 钩子 → **内联** `apply_*(cur, "表")`(无新 ensure·无 NEW-DEBT-EXEMPT);同源已有 ensure_*_rls(如 products 之于 sales)→ 加进其表元组;legacy 无钩子 → **新建独立 `ensure_<域>_rls()`**(独立事务·逐表 `information_schema` 验存在·进 `boot_ensures`·`dal_reexports` 暴露成 `db.*`·commit 加 `NEW-DEBT-EXEMPT`)。**务必在 startup 末步 `ensure_no_orphan_rls` 之前**(boot_ensures 内即满足)。
...
| 域 | 表 | §2 建议(需自验) | 打法要点 |
|---|---|---|---|
| **automation** | `automation_rules`、`error_events` | tenant_or_user | 小·先做。验 error_events 是否纯日志(若纯系统日志无租户查询→可当日志不 enroll/或 tenant)。找 ensure 钩子,无则独立 ensure_*_rls。 |
