...
POST /api/uploads/image:登录 + 租户隔离,收文件 → 校验归一化(Pillow 解得开就收,含 HEIC;
EXIF 转正 + 长边 1600 缩放重编码,25MB 之上才拒 / 磁盘守卫)→ uuid 命名落租户目录 → 返 URL。
GET /api/uploads/image/{tenant_id}/{name}:登录 + 租户匹配,沙盒内本地文件回流。
薄层:鉴权 + 整形 + 错误映射;存储/校验在 services/imaging/image_store.py。
"""
...
async def api_upload_image(request: Request, file: UploadFile = File(...)):
    """上传一张图(Pillow 解得开就收 · ≤25MB · 落盘前归一化)。返回可存进 image_url/logo_url 的 URL。"""
