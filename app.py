...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.35.0.3 · 上传体验三连修:\n• 上传发票卡片之前一边说支持 PDF/图片/Excel/CSV/Word 一边弹「只支持 PDF」黄色告警 · 现在跟下方 drop 区一致 · 真的能直接传 Excel、CSV、Word、图片\n• 对账中心上传时遇到识别不出来的文件 · 之前会甩一长串技术报错(全是英文字段名 + 网址)吓人 · 现在变成一句中文说人话:「字段识别失败 · 请确认文件表头是否完整」\n• 销项税报告核查的「销项税报告」+「总账」两个上传口现在都支持多文件了 · 同 tab 内可以一次拖多份 + 自动并行解析 + 合并对账",
            "th": "v118.35.0.3 · แก้ UX อัปโหลด 3 จุด:\n• การ์ดอัปโหลดใบกำกับเดิมเขียนว่ารองรับ PDF/รูปภาพ/Excel/CSV/Word แต่ขึ้น banner เตือน 'รองรับเฉพาะ PDF' · ตอนนี้ทำได้จริงตามคำโฆษณา · Excel/CSV/Word/รูปภาพอัปได้ตรง ๆ\n• ศูนย์กระทบยอดเมื่อเจอไฟล์ที่อ่านไม่ออก · เดิมโชว์ error ยาวเป็นภาษาอังกฤษ + URL ของไลบรารี · ตอนนี้สั้น ๆ 'อ่านฟิลด์ไม่สำเร็จ · กรุณาตรวจสอบหัวคอลัมน์'\n• แท็บตรวจสอบรายงานภาษีขาย ทั้งช่อง 'รายงานภาษีขาย' และ 'บัญชีแยกประเภท' รับหลายไฟล์แล้ว · ลากหลายไฟล์เข้าได้ในครั้งเดียว · แยกอ่านพร้อมกัน + รวมผลก่อนกระทบยอด",
            "en": "v118.35.0.3 · Upload UX triple fix:\n• Invoice upload card no longer contradicts itself — the 'PDF only' yellow warning is gone · Excel, CSV, Word, and images can be uploaded directly like the drop hint already promised\n• Reconciliation center upload failures used to dump a wall of pydantic technical errors with library URLs into a red toast · now it's a single human sentence: 'Field detection failed · please check the file header'\n• VAT report check tab — both upload slots (VAT report + GL) now accept multiple files · drag and drop several at once · parsed in parallel and merged before reconciliation",
            "ja": "v118.35.0.3 · アップロード UX 3 点修正:\n• 請求書アップロードカードが「PDF/画像/Excel/CSV/Word 対応」と書きながら別の警告で「PDF のみ」と矛盾していた問題を修正 · 約束通り Excel・CSV・Word・画像を直接アップロード可能に\n• 照合センターでアップロードに失敗した際、ライブラリの URL を含む英語の技術エラーが赤いトーストに流れ込んでいたのを 1 行の日本語に置き換え: 「フィールド検出に失敗しました · ヘッダーをご確認ください」\n• 売上税報告チェックタブの 2 つのアップロード枠(売上税報告 + 総勘定元帳)が複数ファイルに対応 · 複数のファイルを一度にドロップ可能 · 並列解析 + マージしてから照合",
