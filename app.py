...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.35.0.7 · 普通用户不再看到管理员/测试中心入口 + credits 钱包卡接通:\n• 头像菜单的「管理员后台」「测试中心」两个特权入口现在默认隐藏 · 只有真正的超管 / 测试白名单账号才显示\n• 设置 → 套餐与用量页接通 credits 钱包卡 · 老板看到余额 + 本月页数 + 充值按钮 · 员工只看自己识别量\n• 充值按钮直接提交申请 · 等管理员审核",
            "th": "v118.35.0.7 · ผู้ใช้ทั่วไปไม่เห็นเมนูผู้ดูแล/ศูนย์ทดสอบ + เชื่อมการ์ดยอดเงิน credits:\n• เมนู 'คอนโซลผู้ดูแล' และ 'ศูนย์ทดสอบ' ในเมนูอวาตาร์ถูกซ่อนเป็นค่าเริ่มต้น · แสดงเฉพาะผู้ดูแลตัวจริง / บัญชีทดสอบ\n• ตั้งค่า → แพ็กเกจ&การใช้งาน เชื่อมการ์ดกระเป๋าเงิน · เจ้าของเห็นยอด + หน้าที่ใช้เดือนนี้ + ปุ่มเติมเงิน\n• ปุ่มเติมเงินส่งคำขอตรง · รอแอดมินอนุมัติ",
            "en": "v118.35.0.7 · Regular users no longer see Admin Console / Test Center entries + credits wallet card wired up:\n• 'Admin Console' and 'Test Center' in the avatar menu now hidden by default · visible only to actual super-admins / test whitelist accounts\n• Settings → Plan & Usage now shows credits wallet card · owners see balance + pages this month + top-up button · employees see only their own usage\n• Top-up button submits request directly · admin approves",
            "ja": "v118.35.0.7 · 通常ユーザーに管理者/テストセンター入口を非表示 + credits ウォレットカード接続:\n• アバターメニューの「管理者コンソール」「テストセンター」をデフォルト非表示 · スーパー管理者 / テスト許可リストのみ表示\n• 設定 → プラン&利用状況 に credits ウォレットカードを接続 · オーナーは残高 + 今月のページ + チャージボタンを表示 · 従業員は自分の利用量のみ\n• チャージボタンで申請を送信 · 管理者承認",
