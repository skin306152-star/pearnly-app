...
def find_contact_by_name(
    access_token: str, xero_tenant_id: str, name: str
...
def _lookup_mapping(
    mappings: Dict[str, Any], kind: str, key_field: str, key_value: Any
) -> Optional[str]:
...
def build_invoice_payload(
    history: Dict[str, Any], mappings: Dict[str, Any], contact: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    tax_type = (
        _lookup_mapping(mappings, "taxes", "pearnly_tax_kind", tax_kind) or "OUTPUT"
    )  # OUTPUT = Xero 默认 7% sales tax

    # 默认科目代码 · 取 revenue_sales 映射(若有)
    account_code = (
        _lookup_mapping(mappings, "accounts", "pearnly_category", "revenue_sales") or "200"
    )
...
def push_invoice(
    access_token: str, xero_tenant_id: str, payload: Dict[str, Any]
