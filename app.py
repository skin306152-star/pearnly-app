...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "v118.33.13.5 更新:\n• 修复 merge_gl_files 仍用旧公式（期末 = 期初 + 贷方 − 借方，费用账视角）覆盖解析器返回的正确值。现在统一为现金账户公式：期末 = 期初 + 借方 − 贷方。GL 期末余额从错的 305,356.06 → 正确的 125,100.06\n\nv118.33.13.4 更新:\n• 修复 GL 解析器（核心修复，解决 0% 匹配率）：\n  - 新增 Mr.erp 泰文 GL 单格行解析器（处理 pdfplumber 把整行交易塞进一个单元格的格式）\n  - 用 สมุด 字段（รับ/จ่าย/ทั่วไป）+ 余额增减综合判定借贷方向\n  - 处理泰文 PUA 字符（U+F70A..F712）→ 标准 Unicode 映射，避免 prompt 关键字匹配失败\n  - 修正现金科目期末余额公式（opening + 借方 − 贷方）\n• 新增 GL 方向健全性检查：若 GL 全是借方/全是贷方，提示用户「这可能不是银行账户分类账」\n• 测试结果：12/12 行正确解析、Opening 215,228.06、Closing 125,100.06 与 PDF 实际一致",
            "en": "v118.33.13.4 updates:\n• Fix GL parser (the root cause of 0% match rate):\n  - New Mr.erp Thai GL single-cell-row parser (handles pdfplumber-merged rows)\n  - Direction inferred from book type (รับ→debit, จ่าย→credit, ทั่วไป→balance delta)\n  - Thai PUA char normalization (U+F70A..F712 → standard) so book-type keywords match\n  - Fixed cash-ledger closing balance formula (opening + debit − credit)\n• New GL direction sanity check: warns user if GL is all-debit or all-credit (likely not a cash-account ledger)\n• Verified: 12/12 rows extracted, opening 215,228.06, closing 125,100.06 — matches real PDF",
            "th": "v118.33.13.4 อัปเดต:\n• แก้ตัวอ่าน GL (สาเหตุหลักที่จับคู่ได้ 0%):\n  - เพิ่มตัวอ่านแถวเซลล์เดียวสำหรับ GL ภาษาไทยแบบ Mr.erp\n  - กำหนดทิศทางจากสมุด (รับ→เดบิต, จ่าย→เครดิต, ทั่วไป→ดูส่วนต่างยอด)\n  - แปลงอักขระ Thai PUA (U+F70A..F712) → Unicode มาตรฐาน เพื่อจับคู่คีย์เวิร์ดได้\n  - แก้สูตรยอดปิดบัญชีเงินสด (ยอดเปิด + เดบิต − เครดิต)\n• เพิ่มการตรวจสอบทิศทาง GL: เตือนผู้ใช้หาก GL มีแต่เดบิต/แต่เครดิต (อาจไม่ใช่บัญชีธนาคาร)\n• ทดสอบ: อ่านได้ 12/12 แถว, ยอดเปิด 215,228.06, ยอดปิด 125,100.06 ตรง PDF จริง",
            "ja": "v118.33.13.4 更新:\n• GL解析器の修正（一致率0%の根本原因）:\n  - 新規Mr.erp式タイGL単一セル行パーサー（pdfplumberが1セルに丸めた取引を処理）\n  - 書類種別（รับ→借方/จ่าย→貸方/ทั่วไป→残高差）から方向判定\n  - タイPUA文字（U+F70A..F712）→標準Unicode正規化、キーワード一致を確保\n  - 現金元帳期末残高の公式修正（期首+借方−貸方）\n• GL方向の妥当性チェック追加: GLが全て借方/全て貸方の場合は警告（銀行口座元帳でない可能性）\n• 検証済: 12/12行抽出、期首215,228.06、期末125,100.06、実PDFと一致"
        }
    }
