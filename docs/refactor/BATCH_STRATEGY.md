...
>
> **⚠️ 进度更新(第三十八/九/四十会话 · 2026-05-27)**:已抽 **28 块** · home.js **22970 → 10279 行**(累计 **-12691**)· R1-R5 抽 21 块,**R6 并行 4 块**(ERP 尾区):erp-mappings/unified-push/erp-map-advanced/erp-onboard(`8358366` · -1031),**R7 并行 3 块**:recon-job-poll + bank-recon-v2(对账对)+ admin-misc(两 IIFE)(`46567d5` · -1619)· 均守门 6 道全绿(prettier/全量 unittest 1132/imports/i18n/node/build)· 生产 E2E 10/10。**②表行号已严重漂移 · 抽前必 re-grep**。
> **R7 后当前 home.js IIFE 地图(10279 行 · 行号已 -1619 漂移 · 必 re-grep)**:🔴plans-plg-line ~7026-9715 / 🔴password ~9723-9979 / 🔴line-email-modal ~10010-10134 / **🔴session-heartbeat ~10219(`_sessionCheck` · 末尾)**;`window.pearnlyConfirm`(~L10142)= 共享工具留 home.js 别动。
> **⚠️ 干净单 IIFE 基本抽尽 · 剩下多为"地板/较难/敏感"(下窗口务必逐块判性质,拿不准就报 Zihao)**:① **⚠️ LINE Bot 绑定面板(R7 后 ~6501-6743 · `_codeTimer`/`_pollTimer` 绑定码轮询 · 账号绑定)→ 敏感,跳过**。② **erp-endpoints-logs / automation 区(~3845-5141 一带)= 顶层函数群(`loadErpEndpoints`/`loadErpLogs`/`loadAutomationPage` 等被 routeTo bare 调)+ 夹缝 IIFE(`_bindErpBatchButtonsDirect`/`initAutomationPage`)** · 顶层函数搬走要确认无 bare 外部引用 · 归"较难/地板"。③ **early 区(L800-5400)banner 分隔函数群**(sidebar-routing/routeTo 中枢=地板 · chrome-quota/upload-camera/ocr-run/results-drawer/rd-sync/history/settings · 多为顶层函数非干净 IIFE · 较难)· **team**(含员工改密 · borderline)· **export L4392 一带**(顶层函数+setInterval · 地板)。④ 🔴高敏 4 块 + `_humanizeBackendError`(L6162 顶层全局 · 被 bank-recon-v2 module 经 bare 调 · 别动别搬)。**地板临界:再往下抽需 Zihao 在场判断顶层函数群/routeTo 中枢/_shared 收官**(见 §13 末「地板」+ 第四十会话 STATE)。
