...
Cloud Run 回退是把流量切回**已验证、与当前数据库及文件兼容**的 revision。数据库或持久文件变更不能仅靠旧镜像恢复。需要回到 Vultr 时，先确认其保留状态和数据兼容性，再协调流量与单一调度器；不得直接启动旧服务造成双写。

Vultr 销毁是独立且不可逆的步骤：备份及可恢复性、新站关键路径验证、回退决策都须有证据。退款是否批准是单独事项，不把迁移完成等同于余额到账。
