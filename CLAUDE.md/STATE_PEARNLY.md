...
> **下一个任务 = 计费迁移收尾 step2(死码清理)· step1 已上线(`101824d`)**:Zihao 已拍板「全迁充值版」· step1(`_get_plan` 非超管恒返 credits)已上线验毕 → 套餐横幅对所有用户隐藏、人人 credits、银行/扣费零影响。**剩 step2 = 删死码,但缠着活码,需专注一刀别 rush**(精确「死 vs 活」边界见下方【第四十二会话】Part B step2 段):明确死=429 hijack(402≠429)/ LINE modal / renderTrialBanner / loadPlan 轮询 / PLAN_CONFIG 老档 / LEGACY_PLAN_MAP;**勿伤的活码**=`V109_3_I18N` 里混的 `scanner-*` 相机翻译 key、`/api/me/plan` 端点(loadAll+上传上限仍调)、5 个 render 定义。做法:Zihao 在场,后端先删 PLAN_CONFIG 老档(改 fallback→credits·pytest 守门)→ 前端外科式删 hijack/LINE/banner/轮询(保 scanner i18n)→ 每步真账号 E2E(登录+上传看相机+充值)。
...
> ════════════════════════════════════════════════════════════
> **【第四十二会话 · 交接 · 2026-05-27】抽+整删老 home.html admin 布局死码(-4566 行)· home.js 8941→6955 · home.html 6658→6428 · 计费迁移收尾 step1(`_get_plan`→credits · 套餐横幅全隐)· 全程双账号实测 + CI 绿 · 计费 step2 死码清理已精确 scope 待专注一刀**
> ════════════════════════════════════════════════════════════
...
>   - **⛔ step2(死码清理 · 未做 · 需专注一刀 · 别 rush)· 已查实「死 vs 活」精确边界**:
