...
- **代码规模快照**(实时跑 `scripts/refactor_progress.py`):home.js **5423**→<200 · app.py **1701**→<500 · db.py **332**→<500 **✅达标** · home.css **0** ✅ · home.html **4410**→<1000 · 综合 **~80%**。
...
- **模块/测试**:src/home **38**(+line-email-modal/change-password/session-heartbeat WB) · *_routes **36** · services **121**(WA 抽出:playwright_bootstrap/background_loops/static_assets/erp.auto_push/startup/error_handlers/importer.{coerce,keys,ai_mapping,synonyms,gl_inference}/recon.{vat_recon_schema,recon_resolve,bank_recon_match}/clients.buyer_resolve/exceptions.{exceptions_schema,exceptions_whitelist}/workspace.seller_routing/erp.{product_mappings_store,push_log_queries})· unit **1712** · integration **47**(WC-D3 +signup +VAT excel +bank-recon +导入器映射 +OCR recognize 守门 契约)· E2E **17** · home.js silent **0**。
