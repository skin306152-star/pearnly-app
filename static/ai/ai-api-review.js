 ...
 *
 * 单文件<500 行铁律:同 ai-api-payroll.js 先例拆出,体量独立、依赖面窄(只用 root/
 * authHeaders/handleResponse 三个 apiFactory 内部闭包量)。MC1-b1 四端点(review-queue/
 * decisions:batch/review-reject/self-review-declare)+ 既有三端点(review 签批/archive
 * 冻结/receipt 回执补挂,C3 批次已落地但此前无前端调用方)。
 *
...
(function (root) {
    ...
    function create(root, authHeaders, handleResponse) {
        ...
        return {
            ...
            // 客户池 pending 数 + 义务到期日 + 返工标记。query 缺省字段不上串(undefined/null
            // 一律略过,同 listOrders 先例)。
            getReviewQueue: function (query) {
                var q = query || {};
                var parts = [];
                ['period', 'severity', 'client_id'].forEach(function (k) {
                    if (q[k] !== undefined && q[k] !== null && q[k] !== '') {
                        parts.push(encodeURIComponent(k) + '=' + encodeURIComponent(q[k]));
                    }
                });
                var qs = parts.length ? '?' + parts.join('&') : '';
                return call('GET', '/api/workorder/review-queue' + qs);
            },
