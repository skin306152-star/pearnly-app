...
1. **★ 71 张孤儿表按域 re-enroll**(事故止血时全 DISABLE,proper 隔离待补)——优先级最高。模板分类见 `INCIDENT.md` §2。**已收:sales 域(§7.11)+ suppliers 域(§7.12:`supplier_categories`/`buyer_to_client_memory`·`supplier_client_mapping` 代码无 CREATE/访问点·仅 BACKLOG 计划·跳过)。** 剩余孤儿按域:line(`line_bindings`/`line_binding_codes`)、knowledge(`knowledge_*` 8 张)、products(`products`/`client_rules`/`member_scopes`)、etax(`etax_*`/`invoice_risk_checks`)、automation(`automation_rules`/`error_events`)、settings 杂项(`user_settings`/`api_keys`/`invitations`/`ownership_transfers`/`client_assignments`/`payment_pending`/`operation_logs`)。超管/钱/根表类(`memberships`/`user_company_roles`/`roles`/`users`/审计日志)按既定口径不 enroll 或 bypass。
