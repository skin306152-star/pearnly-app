...
class GeminiModelsTests(unittest.TestCase):
    ...
    def test_defaults(self):
        # 2026-07-22 拍板:OCR 主力 3.6-flash(3.5 全线退役);大脑独立档钉 2.5。
        self.assertEqual(gm.flash(), "gemini-3.6-flash")
        self.assertEqual(gm.flash_lite(), "gemini-3.6-flash")
        self.assertEqual(gm.fallback(), "gemini-3.6-flash")
    ...
    def test_models_with_fallback_order_and_dedup(self):
        ...
        self.assertEqual(gm.models_with_fallback(), ["gemini-3.6-flash"])
        # primary != fallback → 按 [主, 兜底] 顺序
        os.environ["OCR_FLASH_MODEL"] = "gemini-2.5-flash"
        self.assertEqual(gm.models_with_fallback(), ["gemini-2.5-flash", "gemini-3.6-flash"])
    ...
    def test_escalate_defaults_to_fallback(self):
        # image-first 升级臂默认 = fallback(3.6-flash),不另起档位。
        self.assertEqual(gm.escalate(), "gemini-3.6-flash")
    ...
    def test_try_escalates_on_primary_failure(self):
        ...
        self.assertEqual(r, {"ok": True, "model": "gemini-3.6-flash"})
        self.assertEqual(tried, ["gemini-2.5-flash", "gemini-3.6-flash"])
    ...
    def test_primary_success_no_escalation(self):
        ...
        self.assertEqual(tried, ["gemini-3.6-flash"])  # 兜底未被调用(默认主=兜底·单档)
