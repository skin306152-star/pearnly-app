...
- **当前 task**:3 窗口 loop(Loop 1 拆巨石 + 文档/测试)· **WA 后端(3 commit:playwright_bootstrap 抽启动自举 + background_loops 抽 ERP重试/邮箱抓取 + static_assets 抽前端版本/gz清理 + ensure_user_profile_columns 列归位 + 删死码 _calc_trial_days_left · app.py 3286→2675)** / **WB-C1 前端(3 commit:抽 line-email-modal + change-password + session-heartbeat 3 个自包含 IIFE→src/home · home.js 6190→5737 · 高敏 4 块抽 3 · plans-plg-line 实已删)· ⛔ 自包含 IIFE 安全 surface 已 100% 耗尽:剩余全部(history/识别/抽屉/RD/export/settings/team/camera)耦合共享状态簇 `{_results,_drawerIdx}`/`{_selectedFiles}`(home.js 顶层 let · 会重赋值 · window 镜像必 desync)或被 `loadAll()` init 期调用(module defer 后加载→ReferenceError)→ 必须先做 C9 状态管理(已 gated 在 Loop 1 完成后)才能安全抽。盲桥接 desync 会静默损坏付费用户发票编辑(抽屉=VAT 钱)。真账号 E2E 因 3 窗口共用 1 测试账号 session 互踢不可靠 · 无交互级安全闸。WB 安全活到此为止 · 等 C9)** / **WC 文档扫荡✅→已转 D3 测试**(WC-DOC 二级文档守门5→6道+署名4.7→4.8+铁律17→28 · WC-D3 注册 signup + 销项税 VAT excel + 收入对账 bank-recon + 通用导入器映射 4xx 校验契约集成测试 ×4)· 零串台 · app.py→2675 home.js→5737。
...
- **代码规模快照**(实时跑 `scripts/refactor_progress.py`):home.js **5737**→<200 · app.py **2675**→<500 · db.py **819**→<500 · home.css **0** ✅ · home.html **4410**→<1000 · 综合 **~78%**。
- **模块/测试**:src/home **38**(+line-email-modal/change-password/session-heartbeat WB) · *_routes **36** · services **104**(+playwright_bootstrap +background_loops +static_assets WA)· unit **1656** · integration **46**(WC-D3 +signup +VAT excel +bank-recon +通用导入器映射 契约)· E2E **17** · home.js silent **0**。
