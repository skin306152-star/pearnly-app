...
## 当前状态卡(2026-08-05 · 真机反馈日收官 + 日切曼谷落地 + SM 批次开工(G1 原型已交))

- **一天八件全上线且 CI 绿**:①手机端库存页竖排修复 ②商品图上传去白名单(HEIC/大图任意传,
  服务端 Pillow 归一化:EXIF 转正/长边1600/剥 GPS;已入线程池防冻事件循环)③数字表头对齐
  (交易明细/交接班)④登录默认恢复上次套账(门里静默恢复,失效照旧弹)⑤cryptography CVE
  三连清(48→50,prod venv 已确认补丁版)⑥**销售仪表盘按 Zihao 定稿重建**(Hero 全局日期器
  按日/按月·走势按月一页点天联动·商品构成环图·分时热力14天;四模块 sales-report-*)
  ⑦**银行转账不再被支付构成吞掉**(by_method 全量画,935−835=100 那笔的账务修复)
  ⑧**扫码误读根治**(scan-track.js 多帧确认·known 免投票保地板时序·ITF-14 长度闸)。
- **施工模式换代(Zihao 拍板)**:派子代理施工保主上下文干净;设计先出**交互原型**到桌面
  拍板再动工(桌面「销售仪表盘设计稿」三个 html 是本次的定稿证据)。两个施工代理交付
  质量高;主窗兜住的缝:新 E2E 未注册台账/合并 mtime 刷新致证据过期(全部真重跑,不 touch)。
- **SM 语料评审已立案**:SM=Sister Makeup 曼谷美妆店=ICP;四缺口批次(ABB 小票合规 P0/
  全式税票通路/销项直通代账/小票二维码)工单在 `docs/pos/WORKORDER-SM-gaps-2026-08-04.md`,
  记忆 sm-icp-and-pos-compliance-gaps。**下窗口从工单开工,做通用不做 SM 特例。**
- **simplify 缓做单**(四路审查已修 9 组,余五组):报表 sections 轻参数(走势翻月白拉 11 句
  SQL)/heat_range 回显/bump_v 写侧工具/smoke server 抽取(4 份拷贝)/HEIC 注册收敛。
- **日切曼谷已落地(Zihao 拍板)**:report/sales_log(CSV 同源)/audit_report 三报表面 +
  Google 留档列同轴曼谷日(窗口/分组/显示三层同改·真库边界 E2E 5/5);口径收口单一事实源
  `services/pos/report_window` + `dates.iso_bangkok`。**遗留另派**:upgrade.py 开票日 UTC
  (全式票号段+VAT 归属期错位,另派里最高优先)、sale.py 连号年桶(仅跨年 7h 窗,12 月前
  切换零风险)、KDS 叫号不做。**抠图收案**:不做 AI,Odoo 也只缩放,现管线已对齐。
- **画像方案待 Zihao 拍板**(勘察实证 2026-08-05):/ai 税务画像=手填 13 字段但完整度只数
  6 个;记账画像退役未退净(dms-intake 步④仍在问管不管库存);VAT 登记态两份事实源
  (workspace_clients vs tax_settings)零同步——**G1 按 VAT 切版式前必收口**。方案=画像卡
  三层信号(RD 官方接口/票流推断/管家补料)+机器提议人一键确认,每字段亮出处。
- **G1 小票原型 v1 已交桌面**(`POS小票设计稿\ABB小票交互原型v1.html`·5 拍板点在页内
  折叠框;SM 真票实证:Ocha 票无泰文法定抬头+手写 SW 号=G4 二维码要消灭的纸质流)。
- **慢机去重地板实证**:睿频耗尽机上地板漂到 ≈2.6s,2s 空档走「提示+加1」人工路
  (件数不错)——真机验收按此预期,交接单 §4 那条风险的桌面版答案。
