...
ITEMS = [
    ...
    ("ค่าขนส่ง", "1", "80.00"),
    ("ข้าวกล่อง", "4", "65.00"),
    ("น้ำมันดีเซล ลิตร", "12.35", "32.12"),
    ("บริการซ่อมแซม", "1", "1500.00"),
    ("สายชาร์จ USB-C", "2", "199.00"),
    ("นมสด", "3", "28.00"),
]
...
def a4_invoice(case):
    ...
    draw.text(
        (70, y + 42),
        "เลขประจำตัวผู้เสียภาษี " + case["seller_tax"],
        font=fnt(24),
        fill=(20, 20, 20),
    )
    ...
    draw.text(
        (70, y + 32), "Buyer Tax: " + case.get("buyer_tax", ""), font=fnt(23), fill=(20, 20, 20)
    )
    y += 86
    ...
    for i, it in enumerate(rows, 1):
        ...
        for x, t in zip(xs[:-1], vals):
            draw.text(
                (x + 10, y + 10), t, font=fnt(19 if len(rows) > 15 else 23), fill=(20, 20, 20)
            )
    ...
    if case.get("handwritten"):
        draw.text(
            (210, 1450),
            "ยอดรวมเขียนมือ " + case["total_amount"],
            font=fnt(50, True),
            fill=(32, 80, 190),
        )
    ...
    for tag in tags:
        if tag.startswith("motion_blur"):
            ...
            a = cv2.warpPerspective(
                a, cv2.getPerspectiveTransform(src, dst), (w, h), borderValue=(236, 229, 215)
            )
        elif tag in ("low_res",):
            h, w = a.shape[:2]
            a = cv2.resize(
                cv2.resize(a, (max(1, w // 3), max(1, h // 3))),
                (w, h),
...
def tune(case, trap_no, v):
    ...
    if trap_no == 1:
        case["items"] = [
            {"name": "ข้าวกล่อง", "qty": "1", "price": "109.00", "subtotal": "109.00"},
            {"name": "น้ำดื่ม", "qty": "1", "price": "10.00", "subtotal": "10.00"},
            {"name": "กาแฟเย็น", "qty": "1", "price": "48.00", "subtotal": "48.00"},
        ]
        case.update(
        ...
        case["items"] = [
            {"name": "ชาไทย 3 แก้ว แก้วละ 50", "qty": "3", "price": "50.00", "subtotal": "150.00"}
        ]
        case.update(subtotal="150.00", vat="0.00", total_amount="150.00", items_count=1)
    elif trap_no == 3:
        case["extras"] += [
            ("โทร 089-123-4567", ""),
            ("สมาชิก 7000123456789", ""),
            ("POS 202607050001", ""),
        ]
    elif trap_no == 4:
        ...
        if v % 3 == 0:
            ...
            case.update(vat="0.00", total_amount=case["subtotal"])
            case["extras"].append(("NON VAT", "0.00"))
    elif trap_no == 7:
        case["items"] = [
            {"name": "ค่าบริการที่ปรึกษา", "qty": "1", "price": "1000.00", "subtotal": "1000.00"}
        ]
        case.update(
            subtotal="1000.00",
            vat="70.00",
        ...
        case["items"] = [
            {"name": "Airport shuttle service", "qty": "1", "price": "110.75", "subtotal": "110.75"}
        ]
        case.update(
            currency="USD" if v % 2 else "EUR", total_amount="118.50", subtotal="110.75", vat="7.75"
        )
        case["extras"].append(("Foreign currency", case["currency"]))
    elif trap_no == 11:
        case["layout"] = "multi"
        case["additional_invoices"] = [
            base_case(
                f"{case['id']}_B",
                "multi_invoice_same_image",
                ["same_image_extra_invoice"],
                [],
                v + 40,
            ),
            base_case(
        ...
        docs = [
            ("ใบสั่งซื้อ / PURCHASE ORDER", "order_evidence"),
            ("ใบเสนอราคา / QUOTATION", "other"),
            ("ใบส่งของ / DELIVERY NOTE", "other"),
            ("COPY สำเนา", "other"),
            ("ใบจองสินค้า", "order_evidence"),
        ]
        ...
        case.update(
            ...
            document_type=dtype,
            is_not_invoice=True,
            is_copy_or_duplicate="COPY" in title,
        )
    elif trap_no == 13:
        case["layout"] = "dense"
        case["items"] = [
            {
                "name": ITEMS[i % len(ITEMS)][0],
                "qty": "1",
                "price": m(18 + i * 7.25),
                "subtotal": m(18 + i * 7.25),
            }
            for i in range(18)
        ]
        sub = sum(d(x["subtotal"]) for x in case["items"])
        vat = d(sub * Decimal("0.07"))
        ...
        case["printed_only_items"] = [
            {"name": "ของแถมโปรโมชั่น 0 บาท", "qty": "1", "price": "0.00", "subtotal": "0.00"}
        ]
...
def generate_invoices(manifest):
    for i in range(10):
        c = base_case(
            f"base_sharp_{i + 1:03d}", "sharp_baseline", [], [], i, "pos" if i % 2 else "a4"
        )
...
def table_image(title, meta, headers, rows, totals=None):
    ...
    y = 100
    for s in meta:
        draw.text((70, y), s, font=fnt(25), fill=(20, 20, 20))
        y += 32
    y += 14
    ...
    for k, v in totals or []:
        draw.text((1040, y), k, font=fnt(24, True), fill=(20, 20, 20))
        right(draw, (1560, y), v, fnt(24, True))
        y += 32
    return img
...
def bank_case(i):
    ...
    for r in range(10 + i % 9):
        ...
        wd = d(0)
        if (r + i) % 3 == 0:
            dep = d(2500 + r * 211 + i * 17)
            bal += dep
            direction = "deposit"
        else:
            wd = d(1430 + r * 137 + i * 19)
            bal -= wd
            direction = "withdrawal"
        entries.append(
            {
                "transaction_date": f"2025-12-{r + 1:02d}",
                "description": f"โอนเงิน REF{i:02d}{r:04d}",
                "reference": "KPLUS",
                "deposit": m(dep) if dep else "",
                "withdrawal": m(wd) if wd else "",
                "amount": m(dep or wd),
                "balance": m(bal),
                "direction": direction,
            }
    ...
    traps = []
    if i < 6:
        traps.append("overdraft_negative")
    if i % 6 in (0, 1):
        traps.append("stacked_columns")
    if i % 8 == 0:
        traps.append("partial_page_only")
    deg = [
        ["phone_photo_skew", "shadow"],
        ["flatbed_scan"],
        ["glare"],
        ["fold_crease"],
        ["low_res", "jpeg_artifact"],
        ["moire"],
        ["dark"],
        ["overexposed"],
    ][i % 8]
    cid = f"bank_{bank[1]}_{i + 1:02d}"
    total_dep = sum(d(e["deposit"] or 0) for e in entries)
    total_wd = sum(d(e["withdrawal"] or 0) for e in entries)
    data = {
        "id": cid,
        "file": f"bank/images/{cid}.jpg",
        "doc_type": "bank_statement",
        "document_type": "bank_statement",
        "bank_name": bank[0],
        "bank_code": bank[1],
        "account_name": "บริษัท ดาต้าทูลส์ จำกัด",
        "account_number": f"102-3-{82640+i:05d}-{i%10}",
        "account_last4": f"{82640+i:04d}"[-4:],
        "period_start": "2025-12-01",
        "period_end": "2025-12-31",
        "opening_balance": m(opening),
        "closing_balance": m(bal),
        "entry_count": len(entries),
        "total_deposit": m(total_dep),
        "total_withdrawal": m(total_wd),
        "entries": entries,
        "scenario": "stacked_columns" if "stacked_columns" in traps else "bank_statement_photo",
        "traps": traps,
        "degradation": deg,
        "_note": "synthetic bank statement with reconciled running balance",
    }
    rows = [
        [
            e["transaction_date"][5:],
            e["description"],
            e["reference"],
            e["withdrawal"],
            e["deposit"],
    ...
    img = table_image(
        bank[0],
        [data["account_name"], data["account_number"], "01/12/2568 - 31/12/2568"],
...
def gl_case(i):
    ...
    for r in range(12 + i % 6):
        ...
        entries.append(
            {
                "transaction_date": f"2026-01-{r + 1:02d}",
                "voucher_no": f"JV681{r:03d}.{i}",
                "account_code": "1112-07",
                "description": f"รับ/จ่าย เลขงาน 6091-{r}",
                "debit": m(debit) if debit else "",
                "credit": m(credit) if credit else "",
                "amount": m(debit or credit),
    ...
    if i % 4 == 0:
        traps.append("voucher_number_like_amount")
    if 4 <= i < 7:
        traps.append("rounding_tail_difference")
    deg = [
        ["phone_photo_skew"],
        ["fold_crease", "shadow"],
        ["low_res"],
    ...
    total_debit = sum(d(e["debit"] or 0) for e in entries)
    total_credit = sum(d(e["credit"] or 0) for e in entries)
    data = {
        "id": cid,
        "file": f"gl/images/{cid}.jpg",
        ...
        "entries": entries,
        "scenario": "gl_stacked" if i < 4 else "gl_photo",
        "traps": traps,
        "degradation": deg,
        "_note": "GL balance chain is continuous",
    }
    rows = [
        [
            e["transaction_date"][5:],
    ...
    img = table_image(
        ...
        [data["account_name"], "Period 01/01/2569 - 31/01/2569"],
        ["Date", "Voucher", "Acct", "Description", "Debit/Credit", "Balance"],
        rows,
        [
...
def vat_case(i):
    ...
    report_type = "sales" if i % 2 == 0 else "purchase"
    rows = []
    total_sub = total_vat = Decimal("0")
    for r in range(9 + i % 8):
        sub = d(500 + r * 137 + i * 21)
        vat = d(0 if (i < 3 and r % 4 == 0) else sub * Decimal("0.07"))
        total = sub + vat
        total_sub += sub
        total_vat += vat
        rows.append(
            {
                "seq_no": str(r + 1),
                "transaction_date": f"2026-02-{r + 1:02d}",
    ...
    traps = []
    if i < 4:
        traps.append("multi_page_total")
    if i < 3:
        traps.append("zero_vat_rows")
    total_total = total_sub + total_vat
    data = {
        "id": cid,
        "file": f"vat/images/{cid}.jpg",
        "doc_type": "vat_report",
        ...
        "total_vat": m(total_vat),
        "total_amount": m(total_total),
        "total_total": m(total_total),
        "rows": rows,
        "entries": rows,
        "scenario": "vat_report_photo",
        "traps": traps,
        "degradation": deg,
        "_note": "row VAT totals reconcile with report summary",
    }
    table_rows = [
        [
            x["seq_no"],
            x["transaction_date_raw"],
            x["invoice_no"],
            x["customer_name"],
            x["subtotal"],
            x["vat"],
    ...
    img = table_image(
        "รายงานภาษี" + ("ขาย" if report_type == "sales" else "ซื้อ"),
        ["บริษัท ดาต้าทูลส์ จำกัด", "เดือน 02/2569"],
        ["No", "Date", "Invoice", "Customer", "Net", "VAT"],
        table_rows,
        [
            ("Total Net", data["total_subtotal"]),
            ("Total VAT", data["total_vat"]),
            ("Grand", data["total_total"]),
        ],
    )
            ...
            manifest.append(
                {
                    "id": data["id"],
                    "file": data["file"],
                    "gt": f"{kind}/ground_truth/{data['id']}.json",
...
def main():
    ...
    (OUT / "manifest.jsonl").write_text(
        "".join(json.dumps(x, ensure_ascii=False) + "\n" for x in invoice_manifest),
        encoding="utf-8",
    )
    (OUT / "manifest_recon.jsonl").write_text(
        "".join(json.dumps(x, ensure_ascii=False) + "\n" for x in recon_manifest), encoding="utf-8"
    )
    summary = {
        "invoice_count": len(invoice_manifest),
        "bank_count": 24,
        "gl_count": 12,
        "vat_count": 12,
    }
