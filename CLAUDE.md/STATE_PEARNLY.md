...
## 🎯 状态卡（2026-06-27 · **B8 RLS · 孤儿 re-enroll automation+etax+settings+knowledge 四域连收（第 5~8 棒·本窗）**:automation(`aab5ff97`)+ etax(`51318414`)+ settings 杂项(`487a2d28`)+ knowledge RAG(`661a5f1c`)。四域 CI 6 闸全绿 + prod 金丝雀全 PASS·真数据验真隔离(invitations 44 行 / knowledge_documents 37 行 / answers 29 行 假租户均见 0)。knowledge 6 表均纯 tenant(子表自带 tenant_id 不需 via-parent);附带抽 `services/rls_boot.py` 收口 startup(495 行·**接手新增 RLS enroll 改 rls_boot 的 enrolls 元组**)。不 enroll 裁决:error_events/operation_logs(审计日志)·rd_daily_usage(限流计数器)。**贯穿教训(强·再证 §7.15.2)**:repo 无 DDL 的 legacy 孤儿必 SSH prod `\d` 查真实列定模板,别照搬 INCIDENT §2(本窗 automation/etax/settings/knowledge 模板均纠偏)。交接 §7.16~§7.19。**剩余仅:零暴露孤儿(erp_oauth_*/mrerp_credentials/erp_connectors/excel_templates·代码树无访问点·守卫已 DISABLE·最低优先)+ P4 收口(§7.15.5)。** 下接前 4 棒 ↓）
