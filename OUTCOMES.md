...
| 2 | **删桌面「上传图片」按钮**(手机端保留拍照入口)(审计 B 段·home.html L466-481) | ✅ | ⬜ | 截图核:桌面视口无该按钮、手机视口(@media)仍有。 | B | ⚠️待主控确认目标·窗口B不盲删:**`btn-upload-pic`/`上传图片`/`拍照` 标记在当前 home.html grep=0**(也不在 home.js/src/*/i18n-data.js)→ 该按钮可能已删 或 plan 标记(L466-481/btn-upload-pic)已过时。窗口B 不猜测删别的按钮(避免误删手机版/错按钮)。请主控指认现桌面上传按钮的真实 selector/位置再删。 |
