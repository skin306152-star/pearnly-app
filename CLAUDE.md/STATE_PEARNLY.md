...
## 🎯 状态卡（2026-06-27 · **B8 RLS · 孤儿 re-enroll automation+etax+settings 三域连收（第 5/6/7 棒·本窗）**:automation(`aab5ff97`)+ etax(`51318414`)+ settings 杂项(`487a2d28`)。三域 CI 6 闸全绿 + prod 金丝雀全 PASS。settings:user_settings/api_keys(tou)·invitations/ownership_transfers(tenant)·client_assignments/payment_pending(user)6 表 enroll·operation_logs/rd_daily_usage 设计裁决不开;**invitations 真 44 行假租户见 0(真数据验真隔离)**。**贯穿教训(再证 §7.15.2)**:repo 无 DDL 的 legacy 孤儿必 SSH prod `\d` 查真实列定模板,别照搬 INCIDENT §2。交接 §7.16/§7.17/§7.18。**剩余孤儿域:knowledge(最复杂·留最后)+ 零暴露孤儿(最低优先)。** 下接前 4 棒 ↓）
