# -*- coding: utf-8 -*-
"""银行对账 v2 · 业务警告(4 语)与锚点覆盖 · 路由与后台 worker 共用。

从 routes/recon_routes_bankv2_helpers 下沉:worker(services/recon_jobs)要出同一批
警告/锚点快照,业务判据放 services 层才不用服务层反向 import 路由层。警告文案落库进
summary_json(_brv2_warnings),不是 HTTP 层的展示细节;HTTP 错误文案(_brv2_err)仍留路由。
"""
...
def brv2_warn(key: str, lang: str = "th", **fmt) -> str:
...
def completeness_details(issues, lang) -> list:
...
def detect_recon_mismatch(stmt_rows, gl_rows, matched_count, lang) -> list:
    ...
    try:
        ...
        if s_dates and g_dates:
            ...
            if smax < gmin or gmax < smin:  # 完全不重叠
                warnings.append(
                    brv2_warn("period_mismatch", lang, g=f"{gmin}~{gmax}", s=f"{smin}~{smax}")
                )
        ...
        if len(stmt_rows) >= 20 and len(gl_rows) * 5 < len(stmt_rows):
            warnings.append(brv2_warn("gl_too_few", lang, n=len(gl_rows), m=len(stmt_rows)))
        if matched_count == 0 and (len(stmt_rows) + len(gl_rows)) >= 10:
            warnings.append(brv2_warn("no_match", lang))
    except Exception:
        pass
    return warnings
