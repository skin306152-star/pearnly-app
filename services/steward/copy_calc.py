...
ERRORS = {
    tools_calc.ERR_AMOUNT_UNREADABLE: {
        "zh": "「{raw}」这个数我没算明白。直接说数字就行,比如「10700 含税的」。",
        "th": 'ตัวเลข "{raw}" อ่านไม่ออกค่ะ พิมพ์เป็นตัวเลขมาได้เลย เช่น「10700 รวม VAT แล้ว」',
    },
    tools_calc.ERR_BASIS_UNCLEAR: {
        "zh": "฿{amount} 这个数是含税的还是没含税的?说一声我就算。",
        "th": "฿{amount} นี้รวม VAT แล้วหรือยังคะ บอกมาแล้วจะคำนวณให้ทันที",
    },
    tools_calc.ERR_RATE_UNREADABLE: {
        "zh": "预扣税率「{raw}」没听清。说「3%」这样的数字给我 —— 该扣几个点得你定,我不替你挑。",
        "th": 'อัตราหัก ณ ที่จ่าย "{raw}" ไม่ชัดค่ะ บอกเป็นตัวเลข เช่น「3%」 — จะหักกี่เปอร์เซ็นต์คุณเป็นคนกำหนด ไม่ใช่ระบบ',
    },
}
