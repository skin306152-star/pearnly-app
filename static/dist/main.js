...
`;(function(){const e=document.getElementById("page-history");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=ba,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();function kt(){const e=document.getElementById("history-batch-bar"),n=document.getElementById("history-batch-count"),a=document.getElementById("history-check-all");if(!e||!n)return;const o=_historySelected.size;if(o>0?(e.style.display="",n.textContent=t("history-batch-count",{n:o})):e.style.display="none",a){const s=_historyState.items||[];if(s.length===0)a.checked=!1,a.indeterminate=!1;else{const i=s.filter(r=>_historySelected.has(r.id)).length;a.checked=i===s.length,a.indeterminate=i>0&&i<s.length}}}function wa(){_historySelected.clear(),kt()}async function xt(){if(!_userInfo){setTimeout(()=>xt(),300);return}const e=document.getElementById("history-free-block"),n=document.getElementById("history-main"),a=document.getElementById("history-empty");if(!e||!n||!a){console.warn("[History] container missing");return}if(!_userInfo.can_view_history){e.style.display="",n.style.display="none",a.style.display="none";return}e.style.display="none",_historyState.loading=!0;try{const o=_historyState.page*_historyState.pageSize,s=new URLSearchParams({limit:_historyState.pageSize,offset:o});_historyState.keyword&&s.set("keyword",_historyState.keyword);const i=typeof window.getCurrentClientId=="function"?window.getCurrentClientId():null;i&&s.set("client_id",String(i));const r=await fetch(`/api/history?${s}`,{headers:{Authorization:"Bearer "+token}});if(r.status===401){localStorage.removeItem("mrpilot_token");const p=await r.json().catch(()=>({}));if((typeof p.detail=="string"?p.detail:p.detail&&p.detail.code||"")==="auth.session_revoked"){_showSessionRevokedModal();return}window.location.href="/";return}const u=await r.json();_historyState.items=u.items||[],_historyState.total=u.total||0;const l=new Set(_historyState.items.map(p=>p.id));for(const p of Array.from(_historySelected))l.has(p)||_historySelected.delete(p);en()}catch(o){console.error("load history failed",o)}finally{_historyState.loading=!1}}function en(){const e=document.getElementById("history-main"),n=document.getElementById("history-empty"),a=_historyState.items,o=document.getElementById("history-search-matches");if(o&&(o.textContent=_historyState.keyword?t("search-matches",{n:_historyState.total}):""),a.length===0&&_historyState.total===0&&!_historyState.keyword){e.style.display="none",n.style.display="";return}e.style.display="",n.style.display="none";let s=0;a.forEach(p=>{p.confidence==="high"&&s++});const i=a.length>0?Math.round(s/a.length*100):0;document.getElementById("history-stats").innerHTML=`
    ...
    `;const r=document.getElementById("history-tbody");a.length===0?r.innerHTML=`<div class="history-row-empty">${escapeHtml(t("history-empty-title"))}</div>`:r.innerHTML=a.map(p=>{const f=new Date(p.created_at),c=String(f.getMonth()+1).padStart(2,"0"),d=String(f.getDate()).padStart(2,"0"),v=String(f.getHours()).padStart(2,"0"),w=String(f.getMinutes()).padStart(2,"0"),C=`${c}-${d} ${v}:${w}`,E=escapeHtml(p.filename||""),m=E.length>50?E.substring(0,50)+"…":E,h=p.invoice_no?escapeHtml(p.invoice_no):m,S=[];p.seller_name&&S.push(escapeHtml(p.seller_name)),p.invoice_no&&p.filename&&S.push(m);const L=S.join(" · ")||"-",D=p.category_tag?`<span class="history-badge category">${escapeHtml(p.category_tag)}</span>`:"",H=p.source_total&&p.source_total>1?`<span class="history-badge multi">${escapeHtml(t("invoice-part-of",{i:p.source_index||1,n:p.source_total}))}</span>`:"",_=p.total_amount!==null&&p.total_amount!==void 0?Number(p.total_amount).toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}):'<span class="history-cell-amount-empty">—</span>',g=[];(p.total_amount===null||p.total_amount===void 0)&&g.push(t("field-amount")),p.invoice_no||g.push(t("field-invoice-no")),p.invoice_date||g.push(t("field-invoice-date")),p.seller_name||g.push(t("field-seller-name")),g.length>0&&`${escapeHtml(p.id)}${escapeHtml(t("history-needs-review-tip")+" · "+g.join(" · "))}${escapeHtml(t("history-needs-review-tip"))}${svgIcon("alert",14)}`,p.edited&&`${escapeHtml(t("history-edited",{n:p.edit_count||1}))}`;const I=p.smart_assigned_flag?`<span class="history-badge smart-assigned" title="${escapeHtml(t("history-smart-assigned"))}">${svgIcon("sparkle",11)}</span>`:"",M=p.confidence==="high"?"high":p.confidence==="medium"?"mid":"low",P=p.confidence==="high"?t("conf-high"):p.confidence==="medium"?t("conf-medium"):t("conf-low"),R=`<span class="history-badge conf-${M}">${escapeHtml(P)}</span>`;let B="";const j=p.source||"manual";return j==="email"?B=`<span class="history-badge source source-email" title="${escapeHtml(t("history-source-email"))}">${svgIcon("mail",11)}<span>${escapeHtml(t("history-source-email"))}</span></span>`:j==="folder"?B=`<span class="history-badge source source-folder" title="${escapeHtml(t("history-source-folder"))}">${svgIcon("folder",11)}<span>${escapeHtml(t("history-source-folder"))}</span></span>`:j==="api"&&(B=`<span class="history-badge source source-api" title="${escapeHtml(t("history-source-api"))}">${svgIcon("api",11)}<span>${escapeHtml(t("history-source-api"))}</span></span>`),`
                <div class="history-row" data-hid="${escapeHtml(p.id)}">
                    ...
                    <div class="history-cell-file">
                        <div class="history-cell-filename">${h} ${D} ${H} ${B} ${I}</div>
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){const e=H=>document.getElementById(H);async function n(H,_){return await fetch(H,{method:"PATCH",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify(_||{})})}async function a(H){return await fetch(H,{method:"DELETE",headers:{Authorization:"Bearer "+token}})}let o=null;async function s(){try{o=await apiGet("/api/line/binding")}catch{o={bound:!1}}return o}function i(H,_){if(!H)return;H.style.display="",H.className="notif-line-check "+(_?"bound":"unbound");const g=_?'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>':'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>';H.innerHTML=g+"<span>"+escapeHtml(t(_?"notif-line-bound":"notif-line-not-bound"))+"</span>"}function r(H){if(H==null)return"-";const _=Number(H);return isNaN(_)?String(H):"฿ "+_.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function u(H){if(!H)return"-";try{const _=new Date(H),g=(_.getMonth()+1).toString().padStart(2,"0"),I=_.getDate().toString().padStart(2,"0"),M=_.getHours().toString().padStart(2,"0"),P=_.getMinutes().toString().padStart(2,"0");return`${g}-${I} ${M}:${P}`}catch{return H}}function l(H){const _=e("notif-rules-list"),g=e("notif-rules-empty"),I=e("notif-rules-count");if(!(!_||!g)){if(I.textContent=String(H.length),I.className="auto-status-pill "+(H.length>0?"active":"none"),!H.length){g.style.display="",_.style.display="none",_.innerHTML="";return}g.style.display="none",_.style.display="",_.innerHTML=H.map(M=>{const P=M.template_code==="large_invoice",R=P?"notif-rule-large-tag":"notif-rule-exception-tag",B=P?"large":"";let j=[];if(P){const A=M.params&&M.params.threshold?r(M.params.threshold):"-";j.push(escapeHtml(t("notif-rule-threshold-prefix"))+" "+A)}M.enabled||j.push('<span style="color:#9ca3af;">'+escapeHtml(t("notif-rule-disabled"))+"</span>");const Y=j.length?j.join(' <span class="dot"></span> '):"";return`
                ...
                </div>`}).join("")}}function p(H){const _=e("notif-logs-list");if(_){if(!H.length){_.innerHTML='<div class="notif-logs-empty">'+escapeHtml(t("notif-logs-empty"))+"</div>";return}_.innerHTML=H.map(g=>{const I=g.status==="sent",M=g.event_type==="exception_high"?"notif-event-exception-high":g.event_type==="large_invoice"?"notif-event-large-invoice":"notif-event-test-send",P=I?"":" · "+escapeHtml(g.error||"failed");return`
                <div class="notif-log-row">
                    ...
                    <div class="notif-log-main">
                        ...
                        <div class="notif-log-meta">${escapeHtml(g.template_code||"-")}${P}</div>
                    </div>
                    <div class="notif-log-time">${u(g.sent_at)}</div>
                </div>`}).join("")}}async function f(){try{const H=await apiGet("/api/notifications/rules");c=H&&H.items||[],l(c)}catch(H){console.warn("load rules fail",H)}try{const H=await apiGet("/api/notifications/logs?limit=20");d=H&&H.items||[],p(d)}catch(H){console.warn("load logs fail",H)}}let c=null,d=null;function v(){c&&l(c),d&&p(d);const H=e("notif-new-modal");H&&H.style.display!=="none"&&o&&i(e("notif-line-check"),!!(o&&o.bound))}function w(){const H=e("notif-new-modal");H&&(H.style.display="",e("notif-new-name").value="",e("notif-new-threshold").value="",e("notif-new-threshold-row").style.display="none",document.querySelectorAll('input[name="notif-template"]').forEach(_=>_.checked=!1),s().then(_=>i(e("notif-line-check"),!!(_&&_.bound))))}function C(){const H=e("notif-new-modal");H&&(H.style.display="none")}function E(){const H=document.querySelector('input[name="notif-template"]:checked'),_=e("notif-new-threshold-row");if(!H){_.style.display="none";return}_.style.display=H.value==="large_invoice"?"":"none";const g=e("notif-new-name");g&&!g.value.trim()&&(g.value=H.value==="large_invoice"?t("notif-tmpl-large-name"):t("notif-tmpl-exception-name"))}async function m(){const H=document.querySelector('input[name="notif-template"]:checked');if(!H){showToast(t("notif-new-template"),"error");return}const _=(e("notif-new-name").value||"").trim();if(!_){showToast(t("notif-name-required"),"error");return}const g={name:_,template_code:H.value,params:{},enabled:!0};if(H.value==="large_invoice"){const I=parseFloat(e("notif-new-threshold").value||"0");if(!I||I<=0){showToast(t("notif-threshold-required"),"error");return}g.params.threshold=I}try{const I=await apiPost("/api/notifications/rules",g);if(I&&I.ok)showToast(t("notif-toast-created"),"success"),C(),f();else{const M=await(I&&I.json&&I.json().catch(()=>({})))||{};showToast(M&&M.detail||"save failed","error")}}catch{showToast("save failed","error")}}async function h(H,_,g){if(H==="toggle"){const I=g.classList.contains("on"),M=await n("/api/notifications/rules/"+_,{enabled:!I});M&&M.ok?(showToast(t(I?"notif-toast-toggled-off":"notif-toast-toggled-on"),"success"),f()):showToast("toggle failed","error");return}if(H==="test"){const I=await s();if(!I||!I.bound){showToast(t("notif-line-error-bind-first"),"error");return}const M=await apiPost("/api/notifications/rules/"+_+"/test",{});if(M&&M.ok)showToast(t("notif-toast-test-sent"),"success"),f();else{const P=await(M&&M.json&&M.json().catch(()=>({})))||{},R=P&&P.detail||"";showToast(R||t("notif-toast-test-failed"),"error"),f()}return}if(H==="delete"){if(!await showConfirm(t("notif-confirm-delete"),{danger:!0}))return;const M=await a("/api/notifications/rules/"+_);M&&M.ok?(showToast(t("notif-toast-deleted"),"success"),f()):showToast("delete failed","error");return}}let S=!1;function L(){if(S)return;S=!0;const H=e("notif-btn-new");H&&H.addEventListener("click",w);const _=e("notif-btn-refresh-logs");_&&_.addEventListener("click",f);const g=e("notif-new-close");g&&g.addEventListener("click",C);const I=e("notif-new-cancel");I&&I.addEventListener("click",C);const M=e("notif-new-save");M&&M.addEventListener("click",m),document.querySelectorAll('input[name="notif-template"]').forEach(B=>{B.addEventListener("change",E)});const P=e("notif-rules-list");P&&P.addEventListener("click",B=>{const j=B.target.closest("button[data-action]");if(!j)return;const Y=j.closest("[data-rule-id]");Y&&h(j.getAttribute("data-action"),Y.getAttribute("data-rule-id"),j)});const R=e("notif-new-modal");R&&R.addEventListener("click",B=>{B.target===R&&C()})}async function D(){L(),await f()}window._loadNotificationsPanel=D,window._rerenderNotifications=v})();(function(){function n(m,h){try{return window.t&&window.t(m)||h}catch{return h}}function a(){var m="";try{m=localStorage.getItem("mrpilot_token")||""}catch{}return m?{Authorization:"Bearer "+m}:{}}var o=[{tbody:"vex-task-tbody",api:"/api/recon/tasks/batch_delete",reload:function(){try{window.loadRecentTasks&&window.loadRecentTasks()}catch{}},kind:"vex"},{tbody:"glv-history-tbody",api:"/api/recon/gl-vat/tasks/batch_delete",reload:function(){try{window._loadGlvHistory&&window._loadGlvHistory()}catch{}},kind:"glv"},{tbody:"brv2-history-tbody",api:"/api/recon/bank-v2/tasks/batch_delete",reload:function(){try{window._brv2LoadHistory&&window._brv2LoadHistory()}catch{}},kind:"brv2"}];function s(){if(!document.getElementById("recon-batch-style")){var m=document.createElement("style");m.id="recon-batch-style",m.textContent=".recon-sel-cell{width:36px;text-align:center;padding-left:10px!important;padding-right:6px!important}.recon-sel-cb,.recon-master-cb{cursor:pointer;width:14px;height:14px;accent-color:#111;margin:0;vertical-align:middle}th.recon-time-col,td.recon-time-col{white-space:nowrap}tr.recon-thead-batch{display:none}thead.recon-batch-mode tr.recon-thead-default{display:none}thead.recon-batch-mode tr.recon-thead-batch{display:table-row}tr.recon-thead-batch th{background:#fafaf8;border-bottom:1px solid #e8e8e3;padding:8px 12px}tr.recon-thead-batch .recon-batch-inline{display:flex;align-items:center;gap:10px;font-size:12px;color:#111;font-weight:normal}tr.recon-thead-batch .recon-batch-count-inline{font-weight:600;color:#111;margin-right:4px}tr.recon-thead-batch .recon-batch-del-inline{background:#dc2626;color:#fff;border:none;border-radius:6px;padding:4px 10px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:4px}tr.recon-thead-batch .recon-batch-del-inline:hover{background:#b91c1c}tr.recon-thead-batch .recon-batch-clear-inline{background:transparent;border:none;color:#555;cursor:pointer;font-size:12px;font-family:inherit;text-decoration:underline;padding:4px 2px}tr.recon-thead-batch .recon-batch-clear-inline:hover{color:#111}.recon-batch-bar{display:none!important}",document.head.appendChild(m)}}function i(m){return m?m.dataset&&m.dataset.taskId?m.dataset.taskId:m.dataset&&m.dataset.taskid?m.dataset.taskid:"":""}function r(m){var h=document.getElementById(m.tbody);if(!h)return null;var S=h.closest("table");if(!S)return null;var L=S.querySelector("thead");if(!L)return null;if(L._reconReady)return L;var D=L.querySelector("tr");if(!D)return null;if(D.classList.add("recon-thead-default"),!D.querySelector(".recon-master-cb")){var H=document.createElement("th");H.className="recon-sel-cell";var _=document.createElement("input");_.type="checkbox",_.className="recon-master-cb",_.setAttribute("aria-label","select all"),_.addEventListener("change",function(){f(m,_.checked)}),H.appendChild(_),D.insertBefore(H,D.firstElementChild)}var g=D.children[1];g&&!g.classList.contains("recon-time-col")&&g.classList.add("recon-time-col");var I=D.children.length,M=document.createElement("tr");M.className="recon-thead-batch";var P=document.createElement("th");P.className="recon-sel-cell";var R=document.createElement("input");R.type="checkbox",R.className="recon-master-cb",R.checked=!0,R.setAttribute("aria-label","select all"),R.addEventListener("change",function(){f(m,R.checked)}),P.appendChild(R);var B=document.createElement("th");return B.setAttribute("colspan",String(I-1)),B.innerHTML='<div class="recon-batch-inline"><span class="recon-batch-count-inline" data-recon-count></span><button type="button" class="recon-batch-del-inline" data-recon-del><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="13" height="13"><polyline points="3 4 13 4"/><path d="M6 4V2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V4"/><path d="M5 4l1 9a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1l1-9"/></svg><span data-recon-del-label></span></button><button type="button" class="recon-batch-clear-inline" data-recon-clear></button></div>',M.appendChild(P),M.appendChild(B),L.appendChild(M),B.querySelector("[data-recon-del]").addEventListener("click",function(){w(m)}),B.querySelector("[data-recon-clear]").addEventListener("click",function(){v(m)}),L._reconReady=!0,d(m),L}function u(m){var h=document.getElementById(m.tbody);if(h){var S=h.querySelectorAll("tr");S.forEach(function(L){var D=i(L);if(D&&!L.querySelector(".recon-sel-cb")){var H=L.querySelector("td");if(H){var _=document.createElement("td");_.className="recon-sel-cell";var g=document.createElement("input");g.type="checkbox",g.className="recon-sel-cb",g.dataset.taskId=D,g.dataset.kind=m.kind,g.addEventListener("click",function(M){M.stopPropagation()}),g.addEventListener("change",function(){c(m)}),_.appendChild(g),L.insertBefore(_,H);var I=L.children[1];I&&!I.classList.contains("recon-time-col")&&I.classList.add("recon-time-col")}}}),c(m)}}function l(m){var h=document.getElementById(m.tbody);return h?Array.prototype.slice.call(h.querySelectorAll(".recon-sel-cb")):[]}function p(m){return l(m).filter(function(h){return h.checked}).map(function(h){return h.dataset.taskId})}function f(m,h){l(m).forEach(function(S){S.checked=!!h}),c(m)}function c(m){var h=p(m),S=l(m),L=document.getElementById(m.tbody);if(L){var D=L.closest("table"),H=D&&D.querySelector("thead");if(H){h.length>0?H.classList.add("recon-batch-mode"):H.classList.remove("recon-batch-mode"),H.querySelectorAll(".recon-master-cb").forEach(function(g){if(S.length===0){g.checked=!1,g.indeterminate=!1;return}h.length===S.length?(g.checked=!0,g.indeterminate=!1):h.length===0?(g.checked=!1,g.indeterminate=!1):(g.checked=!1,g.indeterminate=!0)});var _=H.querySelector("[data-recon-count]");_&&(_.textContent=n("recon-batch-selected-n","已选 {n} 条").replace("{n}",h.length))}}}function d(m){var h=document.getElementById(m.tbody);if(h){var S=h.closest("table"),L=S&&S.querySelector("thead");if(L){var D=L.querySelector("[data-recon-del-label]"),H=L.querySelector("[data-recon-clear]");D&&(D.textContent=n("recon-batch-delete","批量删除")),H&&(H.textContent=n("recon-batch-clear","取消")),c(m)}}}function v(m){l(m).forEach(function(h){h.checked=!1}),c(m)}async function w(m){var h=p(m);if(h.length){var S=n("recon-batch-delete-confirm","确定删除选中的 {n} 条对账任务?此操作不可恢复").replace("{n}",h.length),L=!1;try{typeof window.pearnlyConfirm=="function"?L=await window.pearnlyConfirm(S,n("recon-batch-delete-title","批量删除")):L=window.confirm(S)}catch{L=!1}if(L)try{var D=Object.assign({"Content-Type":"application/json"},a()),H=m.kind==="glv"?h.map(function(M){return parseInt(M,10)}):h,_=await fetch(m.api,{method:"POST",headers:D,body:JSON.stringify({ids:H})});if(!_.ok){typeof window.showToast=="function"&&window.showToast(n("recon-batch-delete-fail","批量删除失败"),"error");return}var g=await _.json(),I=g&&(g.deleted!=null?g.deleted:g.count)||h.length;typeof window.showToast=="function"&&window.showToast(n("recon-batch-delete-ok","已删除 {n} 条").replace("{n}",I),"success"),m.reload()}catch{typeof window.showToast=="function"&&window.showToast(n("recon-batch-delete-fail","批量删除失败"),"error")}}}function C(m){r(m),u(m);var h=document.getElementById(m.tbody);if(!(!h||h._reconBatchWatched)){h._reconBatchWatched=!0;var S=new MutationObserver(function(){u(m)});S.observe(h,{childList:!0,subtree:!1})}}function E(){s(),o.forEach(C),document.querySelectorAll(".recon-batch-bar").forEach(function(m){try{m.remove()}catch{}})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",E):E(),setTimeout(E,1500),setTimeout(E,4e3),document.addEventListener("keydown",function(m){m.key==="Escape"&&o.forEach(function(h){p(h).length>0&&v(h)})}),typeof window.subscribeI18n=="function"&&window.subscribeI18n("recon-batch-thead",function(){o.forEach(d)})})();(function(){let e={role:"",monthly_volume:"",country:"",line_id:""},n=1;const a=4,o="pilot_ob_dismiss",s="pilot_ob_done";window.maybeShowOnboarding=function(p){};function i(p){n=p;for(let d=1;d<=a;d++){const v=document.getElementById("ob-step-"+d);v&&(v.style.display=d===p?"block":"none")}document.querySelectorAll(".ob-dot").forEach(d=>{const v=parseInt(d.dataset.step,10);d.classList.toggle("active",v===p),d.classList.toggle("done",v<p)});const f=document.getElementById("ob-step-label");f&&(f.textContent=p+" / "+a);const c=document.getElementById("ob-next");if(c&&(c.textContent=p===a?t("ob-finish"):t("ob-next")),p===4){const d=document.getElementById("ob-line-input");d&&(d.value=e.line_id||"")}}function r(p){const f=document.querySelector(".onboarding-modal");if(!f)return;let c=f.querySelector(".ob-feedback");c||(c=document.createElement("div"),c.className="ob-feedback",f.appendChild(c)),c.textContent=p,c.classList.add("show"),setTimeout(()=>c.classList.remove("show"),1800)}document.addEventListener("click",p=>{const f=p.target.closest(".ob-option");if(!f)return;const c=f.parentElement;if(!c||!c.classList.contains("ob-options"))return;c.querySelectorAll(".ob-option").forEach(v=>v.classList.remove("selected")),f.classList.add("selected");const d=f.dataset.value;c.id==="ob-role-options"?e.role=d:c.id==="ob-volume-options"?e.monthly_volume=d:c.id==="ob-country-options"&&(e.country=d)}),document.addEventListener("click",p=>{p.target.id==="ob-skip"&&u()}),document.addEventListener("click",p=>{if(p.target.id==="ob-next"){if(n===4){const f=document.getElementById("ob-line-input");e.line_id=(f&&f.value||"").trim().replace(/^@+/,"")}u()}}),document.addEventListener("click",p=>{if(p.target.closest("#ob-close")){localStorage.setItem(o,String(Date.now()));const f=document.getElementById("onboarding-modal");f&&(f.style.display="none")}});function u(){n===1&&e.role?r(t("ob-fb-role")):n===2&&e.monthly_volume?r(t("ob-fb-volume")):n===3&&e.country?r(t("ob-fb-country")):n===4&&e.line_id&&r(t("ob-fb-line")),n<a?setTimeout(()=>i(n+1),e[Object.keys(e)[n-1]]?350:0):l()}async function l(){const p=document.getElementById("onboarding-modal");localStorage.setItem(s,"1"),localStorage.removeItem(o);const f={};if(e.role&&(f.role=e.role),e.monthly_volume&&(f.monthly_volume=e.monthly_volume),e.country&&(f.country=e.country),e.line_id&&(f.line_id=e.line_id),Object.keys(f).length===0){p&&(p.style.display="none");return}try{const c=await fetch("/api/me/profile",{method:"PUT",headers:{Authorization:"Bearer "+(window.token||localStorage.getItem("mrpilot_token")),"Content-Type":"application/json"},body:JSON.stringify(f)});c.ok?(r(t("ob-fb-done")),window._userInfo&&Object.assign(window._userInfo,f),setTimeout(()=>{p&&(p.style.display="none")},1200)):(localStorage.setItem("pilot_ob_pending",JSON.stringify(f)),console.warn("onboarding profile save failed",c.status),r(t("ob-fb-saved-local")),setTimeout(()=>{p&&(p.style.display="none")},1500))}catch(c){console.error("onboarding submit",c),localStorage.setItem("pilot_ob_pending",JSON.stringify(f)),p&&(p.style.display="none")}}})();(function(){const e=document.getElementById("archive-rule-modal");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){const e=document.getElementById("archive-token-modal");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        <div class="modal" style="max-width:420px;">
            ...
            <a class="contact-item" href="mailto:${escapeHtml(P)}">
        ...
        `}async function f(){try{const _=await fetch("/api/settings/dup-check",{headers:{Authorization:"Bearer "+token}});if(!_.ok)return;const g=await _.json(),I=document.getElementById("pref-dup-check");I&&(I.checked=!!g.enabled)}catch(_){console.warn("load prefs failed",_)}}const c=document.getElementById("pref-dup-check");c&&!c.dataset.bound&&(c.dataset.bound="1",c.addEventListener("change",async _=>{const g=_.target.checked;try{(await fetch("/api/settings/dup-check",{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({enabled:g})})).ok?showToast(g?t("pref-dup-check-on-toast"):t("pref-dup-check-off-toast"),"success"):(_.target.checked=!g,showToast(t("pref-save-failed"),"error"))}catch{_.target.checked=!g,showToast(t("pref-save-failed"),"error")}}));async function d(){const _=!!(_userInfo&&_userInfo.can_customize_archive);o=!_;const g=document.getElementById("archive-upgrade-banner");g&&(g.style.display=_?"none":"");const I=document.getElementById("archive-plus-badge");I&&(I.style.display=_?"none":"");try{const M=await fetch("/api/archive/settings",{headers:{Authorization:"Bearer "+token}});if(!M.ok)throw new Error("load failed");const P=await M.json();e=Array.isArray(P.name_template)?P.name_template:[],n=P.folder_strategy||"by_month_seller"}catch(M){console.error("load archive settings failed",M),showToast(t("archive-load-failed"),"error");return}v(),w(),C(),E()}function v(){const _=document.getElementById("archive-rule-canvas");if(_){if(e.length===0){_.innerHTML=`<div class="archive-empty">${escapeHtml(t("archive-rule-empty"))}</div>`;return}_.innerHTML=e.map((g,I)=>{const M=i[g.type]||{label:g.type},P=s[g.type]||"",R=g.type==="sep"?`"${escapeHtml(g.val||"_")}"`:escapeHtml(t(M.label));return`
                <span class="archive-token ${g.type}"
                      data-token-idx="${I}"
                      draggable="${o?"false":"true"}">
                    <span class="token-icon">${P}</span>
                    <span class="token-label">${R}</span>
                </span>
            `}).join("")}}function w(){const _=document.getElementById("archive-field-palette");if(!_)return;const g=["date","seller","category","amount","invoice","buyer","sep"];_.innerHTML=g.map(I=>{const M=i[I],P=s[I]||"";return`
                <button class="archive-palette-btn ${I}" data-add-field="${I}" ${o?"disabled":""}>
                    <span class="token-icon">${P}</span>
                    <span>${escapeHtml(t(M.label))}</span>
                </button>
            `}).join("")}function C(){document.querySelectorAll('input[name="folder-strategy"]').forEach(_=>{_.checked=_.value===n,_.disabled=o})}async function E(){const _=document.getElementById("archive-preview-name"),g=document.getElementById("archive-preview-hint");if(g&&(g.textContent=t("archive-preview-hint",{category:r()})),!!_){if(e.length===0){_.textContent="-";return}try{const M=await(await fetch("/api/archive/rename-preview",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({merged_fields:l().merged_fields,name_template:e})})).json();_.textContent=(M.name||"-")+".pdf"}catch{_.textContent="("+t("archive-preview-error")+")"}}}window._rerenderArchiveAll=function(){const _=document.getElementById("archive-rule-modal");!_||_.style.display==="none"||(v(),w(),E())};let m=-1;document.addEventListener("dragstart",_=>{const g=_.target.closest(".archive-token");!g||o||(m=parseInt(g.dataset.tokenIdx,10),g.classList.add("dragging"),_.dataTransfer.effectAllowed="move")}),document.addEventListener("dragend",_=>{document.querySelectorAll(".archive-token").forEach(g=>g.classList.remove("dragging","drop-target")),m=-1}),document.addEventListener("dragover",_=>{const g=_.target.closest(".archive-token");g&&(_.preventDefault(),_.dataTransfer.dropEffect="move",document.querySelectorAll(".archive-token").forEach(I=>I.classList.remove("drop-target")),g.classList.add("drop-target"))}),document.addEventListener("drop",_=>{const g=_.target.closest(".archive-token");if(!g||m<0||o)return;_.preventDefault();const I=parseInt(g.dataset.tokenIdx,10);if(I===m)return;const M=e.splice(m,1)[0];e.splice(I,0,M),m=-1,v(),E()}),document.addEventListener("click",_=>{if(_.target.closest("#btn-open-archive-rule")||_.target.closest("#btn-open-archive-rule-from-settings")){const P=document.getElementById("archive-rule-modal");P&&(P.style.display="",d());return}if(_.target.closest("#archive-rule-modal-close")||_.target.id==="archive-rule-modal"){const P=document.getElementById("archive-rule-modal");P&&(P.style.display="none");return}const g=_.target.closest(".settings-nav-item");if(g){switchSettingsTab(g.dataset.settingsTab);return}if(o&&_.target.closest(".archive-token, [data-add-field], #btn-archive-save, #btn-archive-reset")){showToast(t("feature-contact-us"),"info");return}const I=_.target.closest("[data-add-field]");if(I){const P=I.dataset.addField,R=i[P],B={type:P,...R.defaultCfg};e.push(B),v(),E();return}const M=_.target.closest(".archive-token");if(M&&!o){h(parseInt(M.dataset.tokenIdx,10));return}if(_.target.closest("#btn-archive-save"))return D();if(_.target.closest("#btn-archive-reset"))return H();(_.target.closest("#archive-token-close")||_.target.id==="archive-token-modal")&&(document.getElementById("archive-token-modal").style.display="none"),_.target.closest("#btn-archive-token-ok")&&S(),_.target.closest("#btn-archive-token-delete")&&L()}),document.addEventListener("change",_=>{_.target.name==="folder-strategy"&&(n=_.target.value)});function h(_){a=_;const g=e[_];if(!g)return;const I=document.getElementById("archive-token-body");let M="";g.type==="date"?M=`
                <div class="form-group">
                ...
                </div>`:M=`<div class="form-hint">${escapeHtml(t("archive-token-no-option"))}</div>`,I.innerHTML=M,document.getElementById("archive-token-modal").style.display="",I.querySelectorAll(".sep-chip").forEach(P=>{P.addEventListener("click",()=>{I.querySelectorAll(".sep-chip").forEach(B=>B.classList.remove("active")),P.classList.add("active");const R=document.getElementById("token-sep-custom");R&&(R.value="")})})}function S(){const _=e[a];if(_){if(_.type==="date")_.format=document.getElementById("token-date-format").value;else if(_.type==="seller")_.short=document.getElementById("token-seller-short").checked;else if(_.type==="amount")_.with_currency=document.getElementById("token-amount-currency").checked;else if(_.type==="sep"){const g=document.querySelector("#archive-token-body .sep-chip.active"),I=document.getElementById("token-sep-custom").value;_.val=I||(g?g.dataset.sep:"_")}document.getElementById("archive-token-modal").style.display="none",v(),E()}}function L(){a<0||(e.splice(a,1),document.getElementById("archive-token-modal").style.display="none",v(),E())}async function D(){if(e.length===0){showToast(t("archive-rule-cannot-empty"),"error");return}try{if(!(await fetch("/api/archive/settings",{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({name_template:e,folder_strategy:n})})).ok)throw new Error("save failed");showToast(t("archive-save-ok"),"success");const g=document.getElementById("archive-rule-modal");g&&(g.style.display="none")}catch{showToast(t("archive-save-fail"),"error")}}async function H(){await showConfirm(t("archive-reset-confirm"),{danger:!0})&&(e=[{type:"date",format:"YYYY-MM-DD"},{type:"sep",val:"_"},{type:"seller",short:!0},{type:"sep",val:"_"},{type:"category"},{type:"sep",val:"_"},{type:"amount",with_currency:!0}],n="by_month_seller",v(),C(),E())}})();(function(){const o="pearnly_big_batch_tip_shown";let s=null,i=null,r=0,u=0,l=!1;function p(h){const S=typeof t=="function"?t("big-batch-unload-warn"):"Batch OCR running · close anyway?";return h.preventDefault(),h.returnValue=S,S}function f(){l||(l=!0,window.addEventListener("beforeunload",p))}function c(){l&&(l=!1,window.removeEventListener("beforeunload",p))}function d(){if(document.getElementById("big-batch-progress"))return;const h=document.getElementById("file-list");if(!h||!h.parentNode)return;const S=document.createElement("div");S.id="big-batch-progress",S.className="big-batch-progress",S.innerHTML='<div class="bbp-row"><svg class="bbp-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.5"/><path d="M8 4.5v3.5l2.5 1.5"/></svg><span class="bbp-text" id="bbp-text"></span></div><div class="bbp-track"><div class="bbp-fill" id="bbp-fill" style="width:0%"></div></div>',h.parentNode.insertBefore(S,h);const L=document.getElementById("bbp-text");L&&(L.textContent=typeof t=="function"?t("big-batch-progress-init"):"Starting...")}function v(){const h=document.getElementById("big-batch-progress");h&&h.remove()}function w(){if(!i)return;let h=0;for(let M=0;M<i.length;M++){const P=i[M].status;(P==="success"||P==="error"||P==="cancelled")&&h++}const S=r,L=S>0?Math.min(100,Math.floor(100*h/S)):0,D=(Date.now()-u)/1e3;let H;if(h>=3&&D>1){const M=D/h;H=(S-h)*M}else H=(S-h)*6/6;const _=Math.max(1,Math.ceil(H/60)),g=document.getElementById("bbp-fill"),I=document.getElementById("bbp-text");g&&(g.style.width=L+"%"),I&&(h>=S?I.textContent=t("big-batch-progress-done").replace("{total}",S):I.textContent=t("big-batch-progress-running").replace("{done}",h).replace("{total}",S).replace("{min}",_))}function C(h){try{if(localStorage.getItem(o)==="1")return}catch{}const S=Math.max(1,Math.ceil(h*6/6/60)),L=t("big-batch-first-tip").replace("{n}",h).replace("{min}",S);typeof showToast=="function"&&showToast(L,"info",8e3);try{localStorage.setItem(o,"1")}catch{}}function E(h){!h||h.length<100||(i=h,r=h.length,u=Date.now(),d(),f(),C(r),s&&clearInterval(s),s=setInterval(w,250),w())}function m(){s&&(clearInterval(s),s=null),c(),i&&r>=100?(w(),setTimeout(v,1200)):v(),i=null,r=0}window._bigBatchStart=E,window._bigBatchStop=m,typeof window.subscribeI18n=="function"&&window.subscribeI18n("big-batch-progress",function(){i&&w()})})();(function(){let e=null,n=!1,a=!1;function o(g){return typeof escapeHtml=="function"?escapeHtml(g==null?"":String(g)):String(g??"")}function s(g,I){try{typeof showToast=="function"&&showToast(g,I||"info")}catch{}}function i(){const g=typeof _userInfo<"u"?_userInfo:null;return!!(g&&(g.role==="owner"||g.is_super_admin))}function r(){try{return(typeof _results<"u"?_results:[])[typeof _drawerIdx<"u"?_drawerIdx:-1]||null}catch{return null}}function u(g){if(!g)return!1;const I=String(g.status||"").toLowerCase();return I==="exception"||I==="exception_pending"||I==="rejected"}async function l(g){if(n&&!g)return e;const I=localStorage.getItem("mrpilot_token");if(!I)return null;try{const M=await fetch("/api/erp/xero/status",{headers:{Authorization:"Bearer "+I}});if(!M.ok)throw new Error("http_"+M.status);e=await M.json(),n=!0}catch{e={configured:!1,connected:!1,organisations:[]},n=!1}return e}function p(){const g=document.getElementById("erp-connect-cards");if(!g)return;const I=e;let M,P=!1;I?I.configured?I.connected?(P=!0,M='<span class="mrerp-card-pill mrerp-pill-ok">'+o(t("xero-card-connected"))+"</span>"):M='<span class="mrerp-card-pill mrerp-pill-neutral">'+o(t("xero-card-not-connected"))+"</span>":M='<span class="mrerp-card-pill mrerp-pill-neutral">'+o(t("xero-card-not-configured"))+"</span>":M='<span class="mrerp-card-pill mrerp-pill-neutral">'+o(t("xero-card-not-connected"))+"</span>";let R="";if(!I||!I.configured)R='<button type="button" class="int-btn-configure" id="btn-xero-connect">'+o(t("xero-connect-btn"))+"</button>";else if(!I.connected)i()&&(R='<button type="button" class="int-btn-configure" id="btn-xero-connect">'+o(t("xero-connect-btn"))+"</button>");else{const x=!!I.auto_push,b=x?t("card-btn-disable"):t("card-btn-enable");R='<button type="button" class="'+(x?"mrerp-card-toggle mrerp-card-toggle-disable":"mrerp-card-toggle mrerp-card-toggle-enable")+'" id="btn-xero-toggle-enabled" data-xero-enabled="'+(x?"1":"0")+'" title="'+o(x?t("erp-auto-push-on-tip"):t("erp-auto-push-off-tip"))+'">'+o(b)+'</button><button type="button" class="int-btn-configure" id="btn-xero-edit-toggle">'+o(t("card-btn-edit"))+"</button>"}const B=I&&I.connected?"xero-card-desc-connected":"xero-card-desc-default",j=I&&I.connected?t("xero-card-connected")||"Connected · default org will receive pushes":"Cloud accounting · push invoices to your default Xero org",Y=(function(){const x=t(B);return x===B?j:x})();let A='<div class="integration-row erp-connect-xero'+(P?" connected":"")+'"><div class="int-icon ic-xero"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="9" cy="10" r="1.3" fill="currentColor"/><circle cx="15" cy="14" r="1.3" fill="currentColor"/><path d="M9 14l6-4"/></svg></div><div class="int-info"><div class="int-name"><span>'+o(t("xero-card-title")||"Xero")+"</span>"+M+'</div><div class="int-desc">'+o(Y)+'</div></div><div class="int-actions">'+R+"</div></div>";if(I&&I.configured&&I.connected&&i()){const x=I.organisations||[];let b="";if(x.length>0){b+='<div class="erp-cc-meta">'+o((t("xero-org-count")||"").replace("{n}",String(x.length)))+"</div>",b+='<div class="erp-cc-org-label">'+o(t("xero-default-org"))+":</div>",b+='<div class="erp-cc-orgs">',x.forEach(function(K){b+='<label class="erp-cc-org-row"><input type="radio" name="xero-default-org" value="'+o(K.id)+'"'+(K.is_default?" checked":"")+'><span class="erp-cc-org-name">'+o(K.organisation_name||K.organisation_id)+"</span></label>"}),b+="</div>";const z=!!I.auto_push,F=z?t("erp-auto-push-on-tip"):t("erp-auto-push-off-tip");b+='<div class="erp-cc-auto-push" title="'+o(F)+'"><label class="erp-cc-toggle"><input type="checkbox" id="xero-auto-push-toggle"'+(z?" checked":"")+'><span class="erp-cc-toggle-slider"></span><span class="erp-cc-toggle-label">'+o(t("erp-auto-push-label"))+"</span></label></div>",b+='<div class="erp-cc-actions"><button class="btn btn-ghost btn-tiny" type="button" id="btn-xero-disconnect">'+o(t("xero-disconnect-btn"))+"</button></div>"}A+='<div class="erp-xero-details" id="erp-xero-details" style="display:none; margin:8px 0 16px; padding:12px 14px; border:1px solid var(--line); border-radius:8px; background:var(--bg);">'+b+"</div>"}const W=g.querySelector(".erp-connect-xero"),ee=g.querySelector("#erp-xero-details");ee&&ee.remove(),W?W.outerHTML=A:g.insertAdjacentHTML("afterbegin",A);const oe=document.getElementById("btn-xero-edit-toggle");oe&&oe.addEventListener("click",function(x){x.preventDefault();const b=document.getElementById("erp-xero-details");b&&(b.style.display=b.style.display==="none"?"":"none")});const $=document.getElementById("btn-xero-toggle-enabled");$&&$.addEventListener("click",async function(x){if(x.preventDefault(),$.disabled)return;const z=!($.getAttribute("data-xero-enabled")==="1");if(!z)try{if(!await window.pearnlyConfirm(t("card-toggle-disable-confirm")))return}catch{}$.disabled=!0,await v(z,null)})}async function f(){const g=localStorage.getItem("mrpilot_token");if(g)try{const I=await fetch("/api/erp/xero/auth/start",{method:"GET",headers:{Authorization:"Bearer "+g}});if(!I.ok){let P="unknown";try{P=(await I.json()).detail||"unknown"}catch{}const R=String(P).replace(/^xero\./,"").toLowerCase();s(t("xero-push-fail").replace("{err}",t("xero-err-"+R)||P),"error");return}const M=await I.json();M.redirect_url&&(window.location.href=M.redirect_url)}catch(I){s(t("xero-push-fail").replace("{err}",I.message||"network"),"error")}}async function c(){if(!await window.pearnlyConfirm(t("xero-disconnect-confirm")))return;const I=localStorage.getItem("mrpilot_token");try{const M=await fetch("/api/erp/xero/disconnect",{method:"POST",headers:{Authorization:"Bearer "+I}});if(!M.ok)throw new Error("http_"+M.status);await l(!0),p()}catch(M){s(t("xero-push-fail").replace("{err}",M.message),"error")}}async function d(g){const I=localStorage.getItem("mrpilot_token");try{const M=await fetch("/api/erp/xero/select_org",{method:"POST",headers:{Authorization:"Bearer "+I,"Content-Type":"application/json"},body:JSON.stringify({token_id:g})});if(!M.ok)throw new Error("http_"+M.status);await l(!0),p()}catch(M){s(t("xero-push-fail").replace("{err}",M.message),"error")}}async function v(g,I){const M=localStorage.getItem("mrpilot_token");I&&(I.disabled=!0);try{const P=await fetch("/api/erp/xero/auto-push",{method:"POST",headers:{Authorization:"Bearer "+M,"Content-Type":"application/json"},body:JSON.stringify({on:!!g})});if(!P.ok){let R="unknown";try{R=(await P.json()).detail||"unknown"}catch{}throw new Error(R)}s(g?t("erp-auto-push-toggled-on"):t("erp-auto-push-toggled-off"),"success"),n=!1,await l(!0),p()}catch(P){I&&(I.checked=!g),s(t("erp-auto-push-toggle-fail").replace("{err}",P.message||"network"),"error")}finally{I&&(I.disabled=!1)}}async function w(){const g=document.getElementById("drawer-history-save");if(!g||g.querySelector("#btn-xero-push")||g.querySelector("#pn-push-wrap")||(await l(!1),g.querySelector("#pn-push-wrap"))||g.querySelector("#btn-xero-push"))return;const I=r();if(!(I&&(I._historyId||I.history_id)))return;let P=!1,R="xero-push-tip";!e||!e.configured?(P=!0,R="xero-err-not_configured"):e.connected?u(I)&&(P=!0,R="xero-push-disabled-exc"):(P=!0,R="xero-push-disabled-no-conn");const B=document.createElement("button");B.type="button",B.id="btn-xero-push",B.className="btn btn-ghost"+(P?" disabled":""),B.disabled=P,B.title=t(R)||"",B.innerHTML='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M5 8l2 2 4-4"/></svg><span style="margin-left:4px;">'+o(t("xero-push-btn"))+"</span>",B.addEventListener("click",C);const j=document.getElementById("btn-push-erp");j&&j.parentNode?j.parentNode.insertBefore(B,j.nextSibling):g.insertBefore(B,g.firstChild)}async function C(){const g=r(),I=g&&(g._historyId||g.history_id);if(!I)return;const M=document.getElementById("btn-xero-push");M&&(M.disabled=!0,M.classList.add("loading"));const P=localStorage.getItem("mrpilot_token");try{const R=await fetch("/api/erp/xero/push/"+encodeURIComponent(I),{method:"POST",headers:{Authorization:"Bearer "+P}});if(!R.ok){let B="unknown";try{B=(await R.json()).detail||"unknown"}catch{}const j=String(B).replace(/^xero\./,"").toLowerCase(),Y=t("xero-"+j),A=Y&&Y!=="xero-"+j?Y:B;s(t("xero-push-fail").replace("{err}",A),"error");return}s(t("xero-push-ok"),"success")}catch(R){s(t("xero-push-fail").replace("{err}",R.message||"network"),"error")}finally{M&&(M.disabled=!1,M.classList.remove("loading"))}}async function E(){await l(!0),p(),m()}async function m(){const g=document.getElementById("erp-global-push-mode");if(!g)return;const I=localStorage.getItem("mrpilot_token");if(I)try{const M=await fetch("/api/settings/erp-push-mode",{headers:{Authorization:"Bearer "+I}});if(M.ok){const P=await M.json();P.mode&&(g.value=P.mode,g.dataset.prev=P.mode)}}catch{}}async function h(g){const I=g.value,M=localStorage.getItem("mrpilot_token");try{(await fetch("/api/settings/erp-push-mode",{method:"PUT",headers:{Authorization:"Bearer "+M,"Content-Type":"application/json"},body:JSON.stringify({mode:I})})).ok?(g.dataset.prev=I,s(t("pref-erp-mode-saved"),"success")):(g.value=g.dataset.prev||"smart",s(t("pref-save-failed"),"error"))}catch{g.value=g.dataset.prev||"smart",s(t("pref-save-failed"),"error")}}function S(){try{const g=String(window.location.hash||"");if(g.indexOf("xero=ok")>=0){const I=g.match(/n=(\d+)/),M=I?I[1]:"1";s((t("xero-toast-redirected-ok")||"").replace("{n}",M),"success"),history.replaceState(null,"",window.location.pathname+"#automation"),l(!0).then(p)}else g.indexOf("xero=err")>=0&&(s(t("xero-toast-redirected-err"),"error"),history.replaceState(null,"",window.location.pathname+"#automation"))}catch{}}function L(){if(a)return;a=!0,document.addEventListener("click",function(I){if(I.target.closest('.erp-subtab[data-erp-subtab="connect"]')){setTimeout(E,50);return}if(I.target.closest('.auto-nav-item[data-auto-tab="erp"]')){setTimeout(E,80);return}if(I.target.closest("#btn-xero-connect")){I.preventDefault(),f();return}if(I.target.closest("#btn-xero-disconnect")){I.preventDefault(),c();return}}),document.addEventListener("change",function(I){I.target&&I.target.matches('input[name="xero-default-org"]')&&d(I.target.value),I.target&&I.target.id==="xero-auto-push-toggle"&&v(I.target.checked,I.target),I.target&&I.target.id==="erp-global-push-mode"&&h(I.target)});const g=function(){return document.getElementById("drawer-body")};try{const I=new MutationObserver(function(){document.getElementById("drawer-history-save")&&!document.getElementById("btn-xero-push")&&w()}),M=g();if(M)I.observe(M,{childList:!0,subtree:!0});else{const P=new MutationObserver(function(){const R=g();R&&(I.observe(R,{childList:!0,subtree:!0}),P.disconnect())});P.observe(document.body,{childList:!0,subtree:!0})}}catch{}setTimeout(S,500)}function D(){e&&p();const g=document.getElementById("btn-xero-push");if(g){const I=g.querySelector("span");I&&(I.textContent=t("xero-push-btn"))}}L(),typeof window.subscribeI18n=="function"&&window.subscribeI18n("xero-adapter",D);async function H(g){const I=Date.now();for(;Date.now()-I<g;){if(typeof _userInfo<"u"&&_userInfo&&_userInfo.id)return _userInfo;await new Promise(M=>setTimeout(M,80))}return null}async function _(){await H(5e3);const g=document.querySelector('.auto-nav-item.active[data-auto-tab="erp"]'),I=document.querySelector('.erp-subtab.active[data-erp-subtab="connect"]');g&&I&&await E()}setTimeout(_,200)})();(function(){var e="https://www.mrerp4sme.com/dms/index.php",n=null,a=!1;function o(E){return typeof escapeHtml=="function"?escapeHtml(E==null?"":String(E)):String(E??"")}function s(E,m){try{typeof showToast=="function"&&showToast(E,m||"info")}catch{}}function i(){return localStorage.getItem("mrpilot_token")}async function r(E){var m=i();if(!m)return null;try{var h=await fetch("/api/erp/endpoints",{headers:{Authorization:"Bearer "+m}});if(!h.ok)throw new Error("http_"+h.status);var S=await h.json(),L=S&&S.items||[];n=L.find(function(D){return D&&(D.adapter||"").toLowerCase()==="mrerp_dms"})||null,a=!0}catch{n=null,a=!1}return n}function u(){var E=document.getElementById("erp-connect-cards");if(E){var m=E.querySelector("[data-mrerp-dms-zone]");m||(m=document.createElement("div"),m.setAttribute("data-mrerp-dms-zone","1"),E.appendChild(m));var h=n,S=!!(h&&h.enabled!==!1),L;h?S?L='<span class="mrerp-card-pill mrerp-pill-ok">'+o(t("dms-card-connected"))+"</span>":L='<span class="mrerp-card-pill mrerp-pill-neutral">'+o(t("dms-card-disabled-pill"))+"</span>":L='<span class="mrerp-card-pill mrerp-pill-neutral">'+o(t("dms-card-not-connected"))+"</span>";var D;if(!h)D='<button type="button" class="int-btn-configure" id="btn-dms-connect">'+o(t("dms-card-connect"))+"</button>";else{var H=S?t("dms-card-disable"):t("dms-card-enable");D='<button type="button" class="int-btn-configure" id="btn-dms-edit">'+o(t("dms-card-edit"))+'</button><button type="button" class="int-btn-configure" id="btn-dms-test">'+o(t("dms-card-test"))+'</button><button type="button" class="int-btn-configure" id="btn-dms-toggle">'+o(H)+"</button>"}m.innerHTML='<div class="integration-row erp-connect-mrerp-dms'+(S?" connected":"")+'"><div class="int-icon ic-mrerp-dms" style="background:#0a5c8a;color:#fff;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 13l2-5a2 2 0 011.9-1.4h10.2A2 2 0 0119 8l2 5"/><path d="M3 13h18v4a1 1 0 01-1 1h-1a1 1 0 01-1-1v-1H6v1a1 1 0 01-1 1H4a1 1 0 01-1-1z"/><circle cx="7" cy="15.5" r="1"/><circle cx="17" cy="15.5" r="1"/></svg></div><div class="int-info"><div class="int-name"><span>'+o(t("dms-card-title"))+"</span>"+L+'</div><div class="int-desc">'+o(t("dms-card-desc"))+'</div></div><div class="int-actions">'+D+"</div></div>"}}function l(){var E=document.getElementById("dms-wizard-overlay");E&&E.remove(),document.removeEventListener("keydown",p)}function p(E){E.key==="Escape"&&l()}function f(){l();var E=n,m=E&&E.config&&E.config.booking_defaults&&E.config.booking_defaults.booking_prefix||"PN",h=function(D,H,_,g,I){return'<label style="display:block;margin-bottom:12px;"><span style="display:block;font-size:13px;color:var(--muted,#6b6b66);margin-bottom:5px;">'+o(t(D))+'</span><input id="'+H+'" type="'+_+'" value="'+o(g||"")+'" placeholder="'+o(I||"")+'" autocomplete="new-password" style="width:100%;box-sizing:border-box;padding:9px 11px;border:1px solid var(--line,#ddd);border-radius:8px;font-size:14px;"></label>'},S=document.createElement("div");S.id="dms-wizard-overlay",S.style.cssText="position:fixed;inset:0;z-index:13000;background:rgba(0,0,0,.42);display:flex;align-items:center;justify-content:center;padding:16px;",S.innerHTML='<div class="dms-wizard mrerp-wizard" role="dialog" aria-modal="true" style="background:var(--card,#fff);border-radius:14px;max-width:440px;width:100%;padding:24px;box-shadow:0 12px 40px rgba(0,0,0,.18);max-height:90vh;overflow:auto;"><div style="font-size:17px;font-weight:700;margin-bottom:4px;">'+o(t("dms-wizard-title"))+'</div><div style="font-size:13px;color:var(--muted,#6b6b66);margin-bottom:18px;">'+o(t("dms-card-desc"))+"</div>"+h("dms-wizard-username","dms-w-user","text","","")+h("dms-wizard-password","dms-w-pass","password","","")+h("dms-wizard-prefix","dms-w-prefix","text",m,"PN")+'<div id="dms-w-err" style="display:none;color:#b3261e;font-size:13px;margin:4px 0 12px;"></div><div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;"><button type="button" class="btn btn-ghost" id="dms-w-cancel">'+o(t("dms-wizard-cancel"))+'</button><button type="button" class="btn btn-primary" id="dms-w-save">'+o(t("dms-wizard-save"))+"</button></div></div>",document.body.appendChild(S),document.addEventListener("keydown",p),S.addEventListener("click",function(D){D.target===S&&l()});var L=document.getElementById("dms-w-user");L&&L.focus()}function c(E){var m=document.getElementById("dms-w-err");m&&(m.textContent=E,m.style.display=E?"":"none")}async function d(){var E=n&&n.config&&n.config.system_url||e,m=(document.getElementById("dms-w-user")||{}).value||"",h=(document.getElementById("dms-w-pass")||{}).value||"",S=(document.getElementById("dms-w-prefix")||{}).value||"PN";if(E=E.trim(),m=m.trim(),!E||!m||!h){c(t("dms-wizard-required"));return}var L=document.getElementById("dms-w-save");L&&(L.disabled=!0,L.textContent=t("dms-wizard-saving")),c("");var D=i();try{var H=await fetch("/api/erp/test-connection",{method:"POST",headers:{Authorization:"Bearer "+D,"Content-Type":"application/json"},body:JSON.stringify({adapter:"mrerp_dms",config:{system_url:E,username:m,password:h}})}),_=await H.json().catch(function(){return{}});if(!H.ok||!_.ok){var g=_.error_friendly&&(_.error_friendly[window.currentLang]||_.error_friendly.en)||t("dms-connect-fail-generic");c(g),L&&(L.disabled=!1,L.textContent=t("dms-wizard-save"));return}var I={system_url:E,username_enc:m,password_enc:h,id_card_auto_push:!0,booking_defaults:{booking_prefix:S.trim()||"PN"}},M,P;n&&n.id?(M="PATCH",P="/api/erp/endpoints/"+encodeURIComponent(n.id)):(M="POST",P="/api/erp/endpoints");var R=M==="POST"?{name:"MR.ERP DMS",adapter:"mrerp_dms",config:I,is_default:!1,auto_push:!1}:{config:I,auto_push:!1},B=await fetch(P,{method:M,headers:{Authorization:"Bearer "+D,"Content-Type":"application/json"},body:JSON.stringify(R)});if(!B.ok){var j=await B.json().catch(function(){return{}}),Y=j&&j.detail&&(j.detail.code||j.detail)||"save_failed";c(t("dms-save-fail")+" ("+o(String(Y))+")"),L&&(L.disabled=!1,L.textContent=t("dms-wizard-save"));return}l(),s(t("dms-connect-ok"),"success"),await r(!0),u(),typeof window._refreshOcrDocMode=="function"&&window._refreshOcrDocMode()}catch{c(t("dms-connect-fail-generic")),L&&(L.disabled=!1,L.textContent=t("dms-wizard-save"))}}async function v(){if(!(!n||!n.id)){s(t("dms-test-running"),"info");var E=i();try{var m=await fetch("/api/erp/endpoints/"+encodeURIComponent(n.id)+"/test-connection?refresh=1",{method:"POST",headers:{Authorization:"Bearer "+E}}),h=await m.json().catch(function(){return{}});s(h&&h.ok?t("dms-test-ok"):t("dms-test-fail"),h&&h.ok?"success":"error")}catch{s(t("dms-test-fail"),"error")}}}async function w(){if(!(!n||!n.id)){var E=n.enabled===!1;if(!E)try{var m=await window.pearnlyConfirm(t("dms-confirm-disable"));if(!m)return}catch{}var h=i();try{var S=await fetch("/api/erp/endpoints/"+encodeURIComponent(n.id),{method:"PATCH",headers:{Authorization:"Bearer "+h,"Content-Type":"application/json"},body:JSON.stringify({enabled:E})});if(!S.ok)throw new Error("http_"+S.status);s(E?t("dms-enabled-toast"):t("dms-disabled-toast"),"success"),await r(!0),u(),typeof window._refreshOcrDocMode=="function"&&window._refreshOcrDocMode()}catch{s(t("dms-save-fail"),"error")}}}function C(){r().then(u)}document.addEventListener("click",function(E){if(E.target.closest('.erp-subtab[data-erp-subtab="connect"]')){setTimeout(C,60);return}if(E.target.closest('.auto-nav-item[data-auto-tab="erp"]')){setTimeout(C,90);return}if(E.target.closest("#btn-dms-connect")||E.target.closest("#btn-dms-edit")){E.preventDefault(),f();return}if(E.target.closest("#dms-w-cancel")){E.preventDefault(),l();return}if(E.target.closest("#dms-w-save")){E.preventDefault(),d();return}if(E.target.closest("#btn-dms-test")){E.preventDefault(),v();return}if(E.target.closest("#btn-dms-toggle")){E.preventDefault(),w();return}}),typeof window.subscribeI18n=="function"&&window.subscribeI18n("mrerp-dms-adapter",u),setTimeout(function(){var E=document.querySelector('.auto-nav-item.active[data-auto-tab="erp"]'),m=document.querySelector('.erp-subtab.active[data-erp-subtab="connect"]');E&&m&&C()},250)})();(function(){const e={};function n(){if(document.getElementById("report-modal"))return;const p=`
        ...
        `;f.innerHTML=c+d}function r(p){return p==null?"":String(p).replace(/[&<>"']/g,f=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[f])}function u(p){const f=new Date,c=f.getFullYear(),d=f.getMonth()+1;if(p==="all")return"all";if(p==="this-month")return`${c}-${String(d).padStart(2,"0")}`;if(p==="last-month"){const v=new Date(c,d-2,1);return`${v.getFullYear()}-${String(v.getMonth()+1).padStart(2,"0")}`}return p==="this-year"?`${c}`:p==="this-quarter"?`${c}-Q${Math.floor((d-1)/3)+1}`:"all"}window.openReportModal=async function(p){p=p||{},n(),typeof applyLang=="function"?applyLang(currentLang):document.querySelectorAll("#report-modal [data-i18n]").forEach(C=>{const E=C.getAttribute("data-i18n");I18N[currentLang]&&I18N[currentLang][E]&&(C.textContent=I18N[currentLang][E])});const f=document.getElementById("report-period-section");f&&(f.style.display=p.mode==="client"?"":"none");const c=document.getElementById("report-tpl-list");c.innerHTML=`<div class="report-tpl-loading">${r(t("report-modal-loading"))}</div>`,document.getElementById("report-modal").style.display="";const d=await s(currentLang,p&&p.mode);i(d),o=p;const v=document.getElementById("report-modal-download"),w=v.cloneNode(!0);v.parentNode.replaceChild(w,v),w.addEventListener("click",()=>l(o))};async function l(p){if(!p)return;const f=document.querySelector('input[name="report-tpl"]:checked');if(!f){showToast(t("report-toast-no-selection"),"info");return}const c=f.value,d=document.querySelector('input[name="report-period"]:checked'),v=d?d.value:"all",w=u(v),C=document.getElementById("report-modal-download"),E=C.innerHTML;C.disabled=!0,C.innerHTML=`<span>${r(t("report-modal-loading"))}</span>`;try{const m=localStorage.getItem("mrpilot_token");let h,S;if(p.mode==="records")h=await fetch("/api/reports/export",{method:"POST",headers:{Authorization:"Bearer "+m,"Content-Type":"application/json"},body:JSON.stringify({template:c,lang:currentLang,records:p.records||[],meta:p.meta||{}})}),S=`mrpilot-${c}-${Date.now()}.xlsx`;else if(p.mode==="client"){const M=`/api/reports/clients/${p.clientId}/export?template=${encodeURIComponent(c)}&lang=${encodeURIComponent(currentLang)}&month=${encodeURIComponent(w)}`;h=await fetch(M,{headers:{Authorization:"Bearer "+m}}),S=`${(p.clientName||"client").replace(/[^a-zA-Z0-9\u0e00-\u0e7f\u4e00-\u9fff]/g,"_").slice(0,40)}-${c}.xlsx`}else if(p.mode==="history-batch")c==="sales_detail_th"?(h=await fetch("/api/ocr/export-by-history-ids",{method:"POST",headers:{Authorization:"Bearer "+m,"Content-Type":"application/json"},body:JSON.stringify({template:"sales_detail_th",lang:currentLang,history_ids:p.historyIds||[],client_id:p.clientId||null})}),S=`Pearnly_SalesDetail_${Date.now()}.xlsx`):(h=await fetch("/api/reports/history/batch_export",{method:"POST",headers:{Authorization:"Bearer "+m,"Content-Type":"application/json"},body:JSON.stringify({template:c,lang:currentLang,history_ids:p.historyIds||[],client_id:p.clientId||null})}),S=`mrpilot-batch-${c}-${Date.now()}.xlsx`);else throw new Error("unknown mode: "+p.mode);if(!h.ok){let M="HTTP "+h.status;try{const P=await h.json();P&&P.detail&&(M=P.detail)}catch(P){console.warn("[batch-export] resp.json err.detail parse failed:",P)}h.status===404?showToast(t("report-toast-empty"),"info"):showToast(t("report-toast-fail")+" · "+M,"error");return}const L=await h.blob();let D=S;const _=(h.headers.get("Content-Disposition")||"").match(/filename\*=UTF-8''([^;]+)/i);if(_)try{D=decodeURIComponent(_[1])}catch{}const g=URL.createObjectURL(L),I=document.createElement("a");I.href=g,I.download=D,document.body.appendChild(I),I.click(),document.body.removeChild(I),URL.revokeObjectURL(g),showToast(t("report-toast-success"),"success"),a()}catch(m){console.error("doDownload fail",m),showToast(t("report-toast-fail")+" · "+(m.message||""),"error")}finally{C.disabled=!1,C.innerHTML=E}}document.addEventListener("DOMContentLoaded",()=>{const p=document.getElementById("btn-export");if(p){const c=p.cloneNode(!0);p.parentNode.replaceChild(c,p),c.addEventListener("click",()=>{if(typeof _results>"u"||!_results||_results.length===0){showToast(t("report-toast-no-selection"),"info");return}openReportModal({mode:"records",records:_results.map(d=>({filename:d.filename,merged_fields:d.merged_fields||{}}))})})}const f=document.getElementById("history-batch-export");f&&f.addEventListener("click",()=>{if(typeof _historySelected>"u"||_historySelected.size===0){showToast(t("report-toast-no-selection"),"info");return}openReportModal({mode:"history-batch",historyIds:Array.from(_historySelected)})})}),window.openClientExportModal=function(p,f){openReportModal({mode:"client",clientId:p,clientName:f||""})}})();(function(){const n=typeof window<"u"&&"showDirectoryPicker"in window,a=/\.(pdf|jpe?g|png|webp)$/i,o="mrpilot_folder_watcher",s=1;let i=null,r=null,u=null,l=60,p=!1,f=!1,c=0,d=0,v=0,w=[],C=null,E=!1;function m(){return i||(i=new Promise((k,T)=>{const q=indexedDB.open(o,s);q.onupgradeneeded=G=>{const O=G.target.result;O.objectStoreNames.contains("handles")||O.createObjectStore("handles"),O.objectStoreNames.contains("seen")||O.createObjectStore("seen"),O.objectStoreNames.contains("config")||O.createObjectStore("config")},q.onsuccess=G=>k(G.target.result),q.onerror=G=>T(G.target.error)}),i)}function h(k,T){return m().then(q=>new Promise((G,O)=>{const N=q.transaction(k,"readonly").objectStore(k).get(T);N.onsuccess=()=>G(N.result),N.onerror=()=>O(N.error)}))}function S(k,T,q){return m().then(G=>new Promise((O,y)=>{const N=G.transaction(k,"readwrite");N.objectStore(k).put(q,T),N.oncomplete=()=>O(),N.onerror=()=>y(N.error)}))}function L(k,T){return m().then(q=>new Promise((G,O)=>{const y=q.transaction(k,"readwrite");y.objectStore(k).delete(T),y.oncomplete=()=>G(),y.onerror=()=>O(y.error)}))}function D(k){return m().then(T=>new Promise((q,G)=>{const O=T.transaction(k,"readwrite");O.objectStore(k).clear(),O.oncomplete=()=>q(),O.onerror=()=>G(O.error)}))}async function H(k){if(!k)return!1;try{const T={mode:"read"};let q=await k.queryPermission(T);return q==="granted"?!0:(q=await k.requestPermission(T),q==="granted")}catch(T){return console.warn("[folder] permission check failed:",T),!1}}function _(k,T){const q=document.getElementById("folder-status-summary");q&&(q.setAttribute("data-i18n",k),q.textContent=t(k),q.className="auto-status-pill"+(T?" "+T:""))}function g(k){["folder-unsupported","folder-empty","folder-active"].forEach(T=>{const q=document.getElementById(T);q&&(q.style.display=T===k?"":"none")})}function I(k){if(!k)return"-";const T=k instanceof Date?k:new Date(k),q=String(T.getHours()).padStart(2,"0"),G=String(T.getMinutes()).padStart(2,"0"),O=String(T.getSeconds()).padStart(2,"0");return`${q}:${G}:${O}`}function M(){g("folder-active");const k=document.getElementById("folder-config-path");k&&r&&(k.textContent=r.name||"-");const T=document.getElementById("folder-interval-select");T&&(T.value=String(l)),document.getElementById("folder-stat-last").textContent=C?I(C):"-",document.getElementById("folder-stat-processed").textContent=String(c),document.getElementById("folder-stat-failed").textContent=String(d),document.getElementById("folder-stat-queue").textContent=String(v);const q=document.getElementById("btn-folder-pause"),G=document.getElementById("btn-folder-resume");q&&(q.style.display=p?"none":""),G&&(G.style.display=p?"":"none"),p?_("folder-status-paused","paused"):_("folder-status-running","running");const O=document.getElementById("folder-status-text");O&&(O.setAttribute("data-i18n",p?"folder-status-paused":"folder-status-running"),O.textContent=t(p?"folder-status-paused":"folder-status-running"));const y=document.getElementById("folder-status-pulse");y&&(y.className="folder-status-pulse"+(p?" paused":"")),P()}function P(){const k=document.getElementById("folder-recent-list");if(k){if(w.length===0){k.innerHTML=`<div class="folder-recent-empty">${escapeHtml(t("folder-recent-empty"))}</div>`;return}k.innerHTML=w.slice(0,20).map(T=>{let q;T.status==="ok"?q=`<span class="folder-recent-icon ok">${svgIcon("check",12)}</span>`:T.status==="dup"?q=`<span class="folder-recent-icon dup" title="${escapeHtml(t("folder-recent-dup"))}">${svgIcon("copy",12)}</span>`:T.status==="skip"?q=`<span class="folder-recent-icon skip" title="${escapeHtml(t("folder-recent-skip"))}">${svgIcon("minus",12)}</span>`:q=`<span class="folder-recent-icon fail">${svgIcon("alert",12)}</span>`;const G=T.status==="fail"&&T.error?T.error:T.status==="dup"&&T.reason||T.status==="skip"&&T.reason?T.reason:"",O=G?`<div class="folder-recent-err">${escapeHtml(G)}</div>`:"";return`
                <div class="folder-recent-item">
                    ...
                    <div class="folder-recent-time">${I(T.time)}</div>
                </div>
            `}).join("")}}function R(k){w.unshift(k),w.length>50&&(w.length=50),S("config","recent_list",w).catch(()=>{})}async function B(k){const T=new FormData;T.append("file",k,k.name),T.append("source","folder");const q=await fetch("/api/ocr/recognize?source=folder",{method:"POST",headers:{Authorization:"Bearer "+token,"X-Source":"folder"},body:T});if(!q.ok){let G="http_"+q.status;try{const O=await q.json();G=O&&O.detail?typeof O.detail=="string"?O.detail:O.detail.code||JSON.stringify(O.detail):G}catch{}throw new Error(G)}return await q.json()}async function j(k){try{const q=(await k.getFile()).size;return await new Promise(O=>setTimeout(O,3e3)),(await k.getFile()).size===q&&q>0}catch{return!1}}async function Y(k,T,q,G){if(G>10)return;let O;try{O=await k.queryPermission({mode:"read"})}catch{O="denied"}if(O==="granted")for await(const y of k.values()){const N=T?`${T}/${y.name}`:y.name;if(y.kind==="file"){if(!a.test(y.name))continue;let V;try{V=await y.getFile()}catch{continue}const J=`${N}::${V.size}::${V.lastModified}`;if(await h("seen",J))continue;q.push({entry:y,file:V,seenKey:J,relPath:N})}else if(y.kind==="directory")try{await Y(y,N,q,G+1)}catch{}}}async function A(){if(!(f||p||!r)){f=!0;try{if(await r.queryPermission({mode:"read"})!=="granted"){console.warn("[folder] permission lost · stop"),K(),showToast("warn",t("folder-permission-lost"));return}C=new Date;const T=[];await Y(r,"",T,0),v=T.length,M();for(const q of T){if(p)break;if(!await j(q.entry)){v=Math.max(0,v-1),M();continue}try{let O;try{O=await q.entry.getFile()}catch{O=q.file}const y=await B(O);await S("seen",q.seenKey,{name:O.name,relPath:q.relPath,size:O.size,lastModified:O.lastModified,processed_at:Date.now()});const N=y.history_ids||(y.history_id?[y.history_id]:[]),V=y.duplicate_warnings||[],J=q.relPath||O.name;N.length>0?(c+=N.length,R({name:J,status:"ok",time:new Date,history_id:N[0],count:N.length}),await S("config","processed_count",c)):V.length>0?R({name:J,status:"dup",time:new Date,reason:t("folder-recent-dup-reason")}):R({name:J,status:"skip",time:new Date,reason:t("folder-recent-skip-reason")})}catch(O){d++,R({name:q.relPath||q.file.name,status:"fail",time:new Date,error:String(O.message||O)}),await S("config","failed_count",d)}v=Math.max(0,v-1),M()}}catch(k){console.warn("[folder] scan error:",k)}finally{f=!1,M()}}}function W(){u&&clearInterval(u),u=setInterval(A,l*1e3)}function ee(){u&&(clearInterval(u),u=null)}function oe(k){if(!r||p)return;const T=typeof t=="function"?t("folder-unload-warn"):"Folder watcher running · close anyway?";return k.preventDefault(),k.returnValue=T,T}function $(){window._pearnlyFolderUnloadAttached||(window._pearnlyFolderUnloadAttached=!0,window.addEventListener("beforeunload",oe))}function x(){window._pearnlyFolderUnloadAttached&&(window._pearnlyFolderUnloadAttached=!1,window.removeEventListener("beforeunload",oe))}function b(){p=!1,W(),$(),M(),A()}function z(){p=!0,ee(),x(),M()}function F(){p=!1,W(),$(),M(),A()}function K(){p=!0,ee(),x()}async function Z(){try{const k=await window.showDirectoryPicker({mode:"read",startIn:"documents"});if(!await H(k)){showToast("warn",t("folder-permission-denied"));return}r=k,await S("handles","main",k),c=0,d=0,v=0,w=[],await S("config","processed_count",0),await S("config","failed_count",0),await D("seen"),b()}catch(k){k&&k.name!=="AbortError"&&console.warn("[folder] pick failed:",k)}}async function le(){await showConfirm(t("folder-confirm-remove"),{danger:!0})&&(K(),r=null,c=0,d=0,v=0,w=[],await L("handles","main"),await L("config","processed_count"),await L("config","failed_count"),await D("seen"),g("folder-empty"),_("folder-status-empty",""))}async function re(){w=[];try{await L("config","recent_list")}catch{}P()}async function U(){if(E)return;if(E=!0,!n){g("folder-unsupported"),_("folder-status-unsupported",""),ne();return}Q();let k=null;try{k=await h("handles","main")}catch{}if(!k){g("folder-empty"),_("folder-status-empty","");return}if(!await H(k)){g("folder-empty"),_("folder-status-empty",""),await L("handles","main"),showToast("warn",t("folder-permission-lost-restart"));return}r=k;try{c=await h("config","processed_count")||0}catch{}try{d=await h("config","failed_count")||0}catch{}try{const q=await h("config","recent_list");Array.isArray(q)&&(w=q.map(G=>({...G,time:G.time?new Date(G.time):new Date})))}catch{}b()}function Q(){const k=document.getElementById("btn-folder-pick"),T=document.getElementById("btn-folder-pause"),q=document.getElementById("btn-folder-resume"),G=document.getElementById("btn-folder-scan-now"),O=document.getElementById("btn-folder-remove"),y=document.getElementById("btn-folder-clear-recent"),N=document.getElementById("folder-interval-select");k&&k.addEventListener("click",Z),T&&T.addEventListener("click",z),q&&q.addEventListener("click",F),G&&G.addEventListener("click",()=>{A()}),O&&O.addEventListener("click",le),y&&y.addEventListener("click",re),N&&N.addEventListener("change",V=>{l=parseInt(V.target.value,10)||60,p||W()}),te()}function te(){document.querySelectorAll('[data-auto-panel="folder"] [data-tab-jump]').forEach(k=>{k.dataset.tabJumpBound||(k.dataset.tabJumpBound="1",k.addEventListener("click",T=>{const q=T.currentTarget.dataset.tabJump;if(q==="email")typeof switchAutomationTab=="function"&&switchAutomationTab("email");else if(q==="upload"){const G=document.querySelector('[data-page="recognize"]')||document.querySelector('[data-page="upload"]');G&&G.click()}}))})}function ne(){te()}window._loadFolderWatcherPanel=U;function se(){try{if(navigator.userAgentData&&Array.isArray(navigator.userAgentData.brands))return navigator.userAgentData.brands.some(T=>/chromium|google chrome|microsoft edge/i.test(T.brand||""))}catch{}const k=navigator.userAgent||"";return!!(/Edg\//.test(k)||/Chrome\//.test(k)&&!/OPR\/|YaBrowser|Opera/.test(k))}function ue(){try{if(se()||localStorage.getItem("pearnly_chrome_banner_dismissed")==="1")return;const k=document.getElementById("chrome-only-banner");if(!k)return;const T=k.querySelector('[data-i18n="chrome-banner-msg"]'),q=k.querySelector('[data-i18n="chrome-banner-dismiss"]');T&&typeof t=="function"&&(T.textContent=t("chrome-banner-msg")),q&&typeof t=="function"&&(q.textContent=t("chrome-banner-dismiss")),k.style.display="";const G=document.getElementById("chrome-only-banner-close");G&&!G.dataset.bound&&(G.dataset.bound="1",G.addEventListener("click",()=>{k.style.display="none";try{localStorage.setItem("pearnly_chrome_banner_dismissed","1")}catch{}}))}catch{}}typeof document<"u"&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ue):setTimeout(ue,0)),window._refreshChromeBanner=ue})();(function(){let e=null,n=null,a="new",o=!1,s=!1;async function i(){const B=document.getElementById("email-empty"),j=document.getElementById("email-account-card");if(document.getElementById("email-logs-section"),!(!B||!j))try{const Y=await fetch("/api/email-ingest/account",{headers:{Authorization:"Bearer "+token}});if(Y.status===401){localStorage.removeItem("mrpilot_token");const W=await Y.json().catch(()=>({}));if((typeof W.detail=="string"?W.detail:W.detail&&W.detail.code||"")==="auth.session_revoked"){_showSessionRevokedModal();return}window.location.href="/";return}if(!Y.ok){u("none");return}const A=await Y.json();e=A.account||null,n=A.presets||{},o=!0,r(),e&&I()}catch(Y){console.error("[email-ingest] load failed",Y),u("none")}}function r(){const B=document.getElementById("email-empty"),j=document.getElementById("email-account-card"),Y=document.getElementById("email-logs-section");if(!e){B.style.display="",j.style.display="none",Y&&(Y.style.display="none"),u("none");return}B.style.display="none",j.style.display="",Y&&(Y.style.display="");const A=document.getElementById("email-account-addr"),W=document.getElementById("email-account-host"),ee=document.getElementById("email-account-last"),oe=document.getElementById("email-last-error"),$=document.getElementById("email-enabled-toggle");if(A&&(A.textContent=e.email_address||"-"),W&&(W.textContent=`${e.imap_host||"-"}:${e.imap_port||993}`),ee){const x=e.last_fetched_at;if(!x)ee.textContent=t("email-last-never");else{const b=l(x),z=!e.last_error;ee.textContent=z?t("email-last-ok",{time:b}):t("email-last-fail",{time:b})}}oe&&(e.last_error?(oe.style.display="",oe.textContent=p(e.last_error)):oe.style.display="none"),$&&($.checked=!!e.enabled),e.enabled?e.last_error?u("error"):u("on"):u("off")}function u(B){const j=document.getElementById("email-status-summary");if(!j)return;j.classList.remove("none","ready","active","coming");let Y="auto-status-loading";B==="none"?(Y="email-status-none",j.classList.add("none")):B==="on"?(Y="email-status-on",j.classList.add("active")):B==="off"?(Y="email-status-off",j.classList.add("coming")):B==="error"&&(Y="email-status-error",j.classList.add("none")),j.setAttribute("data-i18n",Y),j.textContent=t(Y)}function l(B){if(!B)return"";const j=new Date(B);if(isNaN(j.getTime()))return"";const Y=A=>String(A).padStart(2,"0");return`${Y(j.getMonth()+1)}-${Y(j.getDate())} ${Y(j.getHours())}:${Y(j.getMinutes())}`}function p(B){if(!B)return"";const j=String(B);return/auth|AUTHENTICATIONFAILED|invalid credentials/i.test(j)?t("email-test-auth-fail"):/timeout|timed out/i.test(j)?t("email.timeout"):(/ENOTFOUND|getaddrinfo/i.test(j),j)}function f(B){a=B;const j=document.getElementById("email-modal");if(!j)return;const Y=document.getElementById("email-preset");Y.innerHTML="";const A=n||{},W=["gmail","outlook","yahoo","icloud","qq","163","custom"],ee={gmail:"Gmail",outlook:"Outlook / Office365",yahoo:"Yahoo Mail",icloud:"iCloud",qq:"QQ 邮箱",163:"网易 163"};W.forEach(Q=>{if(!A[Q])return;const te=document.createElement("option");te.value=Q,te.textContent=Q==="custom"?t("email-preset-custom"):ee[Q]||Q,Y.appendChild(te)});const oe=document.getElementById("email-modal-title"),$=document.getElementById("email-address"),x=document.getElementById("email-password"),b=document.getElementById("email-imap-host"),z=document.getElementById("email-imap-port"),F=document.getElementById("email-imap-ssl"),K=document.getElementById("email-folder"),Z=document.getElementById("email-mark-read"),le=document.getElementById("email-bind-enabled"),re=document.getElementById("email-test-result"),U=document.getElementById("email-adv-details");if(re&&(re.style.display="none",re.textContent=""),B==="edit"&&e){oe.setAttribute("data-i18n","email-modal-title-edit"),oe.textContent=t("email-modal-title-edit"),$.value=e.email_address||"",x.value="",x.setAttribute("data-i18n-placeholder","email-field-password-edit-ph"),x.placeholder=t("email-field-password-edit-ph"),b.value=e.imap_host||"",z.value=e.imap_port||993,F.checked=e.imap_use_ssl!==!1,K.value=e.folder||"INBOX",Z.checked=e.mark_as_read!==!1,le.checked=e.enabled!==!1;const Q=document.getElementById("email-filter-sender"),te=document.getElementById("email-filter-subject");Q&&(Q.value=e.filter_sender||""),te&&(te.value=e.filter_subject||""),h(e.interval_min||15),Y.value=C(e.imap_host)||"custom",U&&(U.open=!0)}else{oe.setAttribute("data-i18n","email-modal-title-new"),oe.textContent=t("email-modal-title-new"),$.value="",x.value="",x.setAttribute("data-i18n-placeholder","email-field-password-ph"),x.placeholder=t("email-field-password-ph"),Y.value="gmail",d("gmail"),K.value="INBOX",Z.checked=!0,le.checked=!0;const Q=document.getElementById("email-filter-sender"),te=document.getElementById("email-filter-subject");Q&&(Q.value=""),te&&(te.value=""),h(15),U&&(U.open=!1)}m(),j.style.display="flex",setTimeout(()=>$.focus(),60)}function c(){const B=document.getElementById("email-modal");B&&(B.style.display="none")}function d(B){const j=(n||{})[B];if(!j||B==="custom")return;const Y=document.getElementById("email-imap-host"),A=document.getElementById("email-imap-port"),W=document.getElementById("email-imap-ssl");Y&&(Y.value=j.host||""),A&&(A.value=j.port||993),W&&(W.checked=j.ssl!==!1)}const v={"gmail.com":"gmail","googlemail.com":"gmail","outlook.com":"outlook","hotmail.com":"outlook","live.com":"outlook","office365.com":"outlook","msn.com":"outlook","yahoo.com":"yahoo","yahoo.co.jp":"yahoo","icloud.com":"icloud","me.com":"icloud","mac.com":"icloud","qq.com":"qq","foxmail.com":"qq","163.com":"163","126.com":"163","yeah.net":"163"};function w(B){if(!B||!B.includes("@"))return;const j=B.split("@")[1].toLowerCase().trim(),Y=v[j];if(!Y)return;const A=document.getElementById("email-preset");if(!A)return;const W=A.value;W&&W!=="custom"&&W!==""&&W===Y||(A.value=Y,d(Y))}function C(B){if(!B)return null;const j=n||{};for(const Y in j)if(Y!=="custom"&&j[Y]&&j[Y].host===B)return Y;return null}function E(){const B=document.querySelector("#email-interval-options .email-interval-btn.active"),j=B?parseInt(B.dataset.interval,10):15;return{email_address:(document.getElementById("email-address").value||"").trim(),password:document.getElementById("email-password").value||"",imap_host:(document.getElementById("email-imap-host").value||"").trim(),imap_port:parseInt(document.getElementById("email-imap-port").value||"993",10)||993,imap_use_ssl:document.getElementById("email-imap-ssl").checked,folder:(document.getElementById("email-folder").value||"INBOX").trim()||"INBOX",mark_as_read:document.getElementById("email-mark-read").checked,enabled:document.getElementById("email-bind-enabled").checked,interval_min:[5,15,60].includes(j)?j:15,filter_sender:(document.getElementById("email-filter-sender").value||"").trim()||null,filter_subject:(document.getElementById("email-filter-subject").value||"").trim()||null}}function m(){const B=document.getElementById("email-interval-options");!B||B._bound||(B._bound=!0,B.addEventListener("click",j=>{const Y=j.target.closest(".email-interval-btn");Y&&(B.querySelectorAll(".email-interval-btn").forEach(A=>A.classList.remove("active")),Y.classList.add("active"))}))}function h(B){const j=[5,15,60].includes(B)?B:15,Y=document.getElementById("email-interval-options");Y&&Y.querySelectorAll(".email-interval-btn").forEach(A=>{A.classList.toggle("active",parseInt(A.dataset.interval,10)===j)})}function S(B,j){const Y=document.getElementById("email-test-result");Y&&(Y.style.display="",Y.textContent=j,Y.className="form-test-result "+(B==="ok"?"ok":B==="running"?"running":"fail"))}async function L(){const B=E();if(!B.email_address){S("fail",t("email-addr-required"));return}if(!B.password){S("fail",t("email-password-required"));return}if(!B.imap_host){S("fail",t("email-host-required"));return}const j=document.getElementById("btn-email-modal-test");j&&(j.disabled=!0),S("running",t("email-test-running"));try{const Y=await fetch("/api/email-ingest/test",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({email_address:B.email_address,password:B.password,imap_host:B.imap_host,imap_port:B.imap_port,imap_use_ssl:B.imap_use_ssl,folder:B.folder})}),A=await Y.json().catch(()=>({}));if(Y.ok&&A.success)S("ok",t("email-test-ok",{folder:B.folder,n:A.folder_count??"?"}));else{const W=A.error_msg||"";W==="auth_failed"||/auth/i.test(W)?S("fail",t("email-test-auth-fail")):S("fail",t("email-test-fail",{msg:W||Y.status}))}}catch(Y){S("fail",t("email-test-fail",{msg:String(Y).slice(0,120)}))}finally{j&&(j.disabled=!1)}}async function D(){const B=E();if(!B.email_address){S("fail",t("email-addr-required"));return}if(a==="new"&&!B.password){S("fail",t("email-password-required"));return}if(!B.imap_host){S("fail",t("email-host-required"));return}const j=document.getElementById("btn-email-modal-save");j&&(j.disabled=!0);const Y={...B};a==="edit"&&!Y.password&&delete Y.password;try{const A=await fetch("/api/email-ingest/account",{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify(Y)}),W=await A.json().catch(()=>({}));if(A.ok&&W.ok)e=W.account,showToast(t("email-save-ok"),"success"),c(),r(),I();else{const oe="email."+(W.detail||"").split(".").slice(-1)[0];S("fail",t(oe)!==oe?t(oe):t("email-save-fail"))}}catch{S("fail",t("email-save-fail"))}finally{j&&(j.disabled=!1)}}async function H(){if(!(!e||!await showConfirm(t("email-unbind-confirm",{email:e.email_address}),{danger:!0,okText:t("email-btn-unbind")})))try{if((await fetch("/api/email-ingest/account",{method:"DELETE",headers:{Authorization:"Bearer "+token}})).ok){e=null,showToast(t("email-unbind-ok"),"success"),r();const Y=document.getElementById("email-logs-list");Y&&(Y.innerHTML="")}else showToast(t("email-unbind-fail"),"error")}catch{showToast(t("email-unbind-fail"),"error")}}async function _(){if(!e||s)return;if(!e.enabled){showToast(t("email.disabled"),"error");return}s=!0;const B=document.getElementById("btn-email-trigger"),j=B?B.innerHTML:"";B&&(B.disabled=!0,B.innerHTML=`<span>${escapeHtml(t("email-trigger-running"))}</span>`);try{const Y=await fetch("/api/email-ingest/trigger",{method:"POST",headers:{Authorization:"Bearer "+token}}),A=await Y.json().catch(()=>({}));if(Y.ok){const W=A.emails_scanned||0,ee=A.ocr_succeeded||0,oe=A.ocr_failed||0;W===0&&ee===0&&oe===0?showToast(t("email-trigger-empty"),"success"):showToast(t("email-trigger-result",{scanned:W,ok:ee,fail:oe}),oe>0?"warn":"success")}else{const ee="email."+(A.detail||"").split(".").slice(-1)[0];showToast(t(ee)!==ee?t(ee):t("email-trigger-fail"),"error")}await i()}catch{showToast(t("email-trigger-fail"),"error")}finally{s=!1,B&&(B.disabled=!1,B.innerHTML=j)}}async function g(){if(!e)return;const B=document.getElementById("email-enabled-toggle"),j=!!(B&&B.checked),Y=e.enabled;try{const A=await fetch("/api/email-ingest/account",{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({email_address:e.email_address,imap_host:e.imap_host,imap_port:e.imap_port,imap_use_ssl:e.imap_use_ssl,folder:e.folder||"INBOX",filter_subject:e.filter_subject||null,filter_sender:e.filter_sender||null,mark_as_read:e.mark_as_read!==!1,enabled:j})}),W=await A.json().catch(()=>({}));A.ok&&W.ok?(e=W.account,r()):(B&&(B.checked=Y),showToast(t("email-toggle-fail"),"error"))}catch{B&&(B.checked=Y),showToast(t("email-toggle-fail"),"error")}}async function I(){const B=document.getElementById("email-logs-list");if(B){B.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-loading"))}</div>`;try{const j=await fetch("/api/email-ingest/logs?limit=20",{headers:{Authorization:"Bearer "+token}});if(!j.ok){B.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-error"))}</div>`;return}const Y=await j.json();if(!Array.isArray(Y)||Y.length===0){B.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("email-logs-empty"))}</div>`;return}B.innerHTML=Y.map(M).join("")}catch{B.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-error"))}</div>`}}}function M(B){const j=l(B.created_at),Y=B.status||"failed",A=Y==="success"?"ok":Y==="partial"?"partial":"fail",W=Y==="success"?"✓":Y==="partial"?"◐":"✗",ee=B.trigger==="manual"?`<span class="log-tag manual">${escapeHtml(t("email-log-tag-manual"))}</span>`:`<span class="log-tag auto">${escapeHtml(t("email-log-tag-auto"))}</span>`,oe=t("email-log-counts",{scanned:B.emails_scanned||0,att:B.attachments_found||0,ok:B.ocr_succeeded||0,fail:B.ocr_failed||0}),$=(B.elapsed_ms||0)+"ms";return`
        ...
        `}function P(){const B=document.getElementById("btn-email-bind");B&&B.addEventListener("click",()=>f("new"));const j=document.getElementById("btn-email-edit");j&&j.addEventListener("click",()=>f("edit"));const Y=document.getElementById("btn-email-unbind");Y&&Y.addEventListener("click",H);const A=document.getElementById("btn-email-trigger");A&&A.addEventListener("click",_);const W=document.getElementById("email-enabled-toggle");W&&W.addEventListener("change",g);const ee=document.getElementById("email-modal-close");ee&&ee.addEventListener("click",c);const oe=document.getElementById("btn-email-modal-cancel");oe&&oe.addEventListener("click",c);const $=document.getElementById("btn-email-modal-test");$&&$.addEventListener("click",L);const x=document.getElementById("btn-email-modal-save");x&&x.addEventListener("click",D);const b=document.getElementById("email-preset");b&&b.addEventListener("change",K=>d(K.target.value));const z=document.getElementById("email-address");z&&!z.dataset.autoBound&&(z.dataset.autoBound="1",z.addEventListener("blur",K=>w((K.target.value||"").trim())),z.addEventListener("input",K=>{const Z=(K.target.value||"").trim();Z.includes("@")&&Z.split("@")[1].includes(".")&&w(Z)}));const F=document.getElementById("btn-email-refresh-logs");F&&F.addEventListener("click",()=>{F.classList.add("spinning"),setTimeout(()=>F.classList.remove("spinning"),600),I()})}P(),window._loadEmailIngestPanel=i,window._rerenderEmailIngest=function(){if(!o)return;r();const B=document.getElementById("email-logs-section");e&&B&&B.open&&I()};let R=null;window._startEmailLogAutoRefresh=function(){R||(R=setInterval(()=>{e&&o&&I()},3e4))},window._stopEmailLogAutoRefresh=function(){R&&(clearInterval(R),R=null)}})();(function(){let e=[],n=null,a=[],o="all",s=null,i=!1;async function r(){if(i){P();return}i=!0,u(),await l(),P()}function u(){const y=document.getElementById("bank-file-input");y&&!y._bound&&(y._bound=!0,y.addEventListener("change",w));const N=document.getElementById("btn-bank-queue-clear-done");N&&!N._bound&&(N._bound=!0,N.addEventListener("click",L));const V=document.getElementById("btn-bank-back");V&&!V._bound&&(V._bound=!0,V.addEventListener("click",()=>{n=null,a=[],ee()}));const J=document.getElementById("btn-bank-delete");J&&!J._bound&&(J._bound=!0,J.addEventListener("click",g));const X=document.getElementById("btn-bank-run-match");X&&!X._bound&&(X._bound=!0,X.addEventListener("click",_)),document.querySelectorAll(".bank-filter-btn").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",()=>{o=fe.dataset.bankFilter||"all",document.querySelectorAll(".bank-filter-btn").forEach(me=>{me.classList.toggle("active",me===fe)}),Z()}))}),document.querySelectorAll("[data-bank-cand-close]").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",ne))});const de=document.getElementById("btn-bank-cand-pane-close");de&&!de._bound&&(de._bound=!0,de.addEventListener("click",ne));const ie=document.getElementById("btn-bank-cand-ignore");ie&&!ie._bound&&(ie._bound=!0,ie.addEventListener("click",I));const ce=document.getElementById("btn-bank-cand-ignore-pane");ce&&!ce._bound&&(ce._bound=!0,ce.addEventListener("click",I));const ae=document.getElementById("bank-client-badge");ae&&!ae._bound&&(ae._bound=!0,ae.addEventListener("click",x)),document.querySelectorAll("[data-bank-client-picker-close]").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",b))});const pe=document.getElementById("btn-bank-client-picker-save");pe&&!pe._bound&&(pe._bound=!0,pe.addEventListener("click",F)),document.querySelectorAll(".bank-sessions-chip").forEach(fe=>{fe._bound||(fe._bound=!0,fe.addEventListener("click",()=>{R=fe.dataset.sessFilter||"all",document.querySelectorAll(".bank-sessions-chip").forEach(me=>{me.classList.toggle("active",me===fe)}),B()}))})}async function l(){try{const y=await fetch("/api/bank-recon/sessions?limit=30",{headers:{Authorization:"Bearer "+token}});if(!y.ok)throw new Error("sessions:"+y.status);e=await y.json(),B()}catch(y){console.warn("[bank-recon] loadSessions failed",y),e=[],B()}}async function p(y){try{const N="/api/bank-recon/sessions/"+encodeURIComponent(y)+(o!=="all"?"?filter="+o:""),V=await fetch(N,{headers:{Authorization:"Bearer "+token}});if(!V.ok)throw new Error("detail:"+V.status);const J=await V.json();n=J.session,a=J.transactions||[],W()}catch(N){console.warn("[bank-recon] loadSessionDetail failed",N),showToast(t("bank-load-failed"),"error")}}let f=[],c=0;const d=3;function v(){return c+=1,"q"+c+"_"+Date.now()}async function w(y){const N=Array.from(y.target.files||[]);if(y.target.value="",N.length!==0){for(const V of N){const J={id:v(),file:V,name:V.name,size:V.size,status:"pending",progress:0,error_code:null,tx_count:0,session_id:null};V.name.toLowerCase().endsWith(".pdf")?V.size>20*1024*1024&&(J.status="failed",J.error_code="bank_recon.file_too_large"):(J.status="failed",J.error_code="bank_recon.only_pdf"),f.push(J)}C(),E(),D()}}function C(){const y=document.getElementById("bank-upload-queue");y&&(y.style.display=""),se(),ue()}function E(){const y=document.getElementById("bank-upload-queue-list"),N=document.getElementById("bank-upload-queue-summary");if(!y)return;if(f.length===0){y.innerHTML="",N&&(N.textContent="");const ie=document.getElementById("bank-upload-queue");ie&&(ie.style.display="none");return}let V=0,J=0,X=0,de=0;for(const ie of f)ie.status==="ok"?V++:ie.status==="failed"?J++:ie.status==="uploading"||ie.status==="parsing"?X++:de++;N&&(N.textContent=t("bank-queue-summary").replace("{ok}",V).replace("{run}",X).replace("{wait}",de).replace("{fail}",J)),y.innerHTML=f.map(m).join(""),y.querySelectorAll("[data-q-act]").forEach(ie=>{const ce=ie.dataset.qAct,ae=ie.dataset.qId;ie.addEventListener("click",()=>{ce==="retry"&&h(ae),ce==="remove"&&S(ae)})})}function m(y){const N=(y.size/1024).toFixed(0)+" KB";let V="",J="";if(y.status==="pending")V='<span class="bq-stat bq-wait">'+t("bank-queue-status-wait")+"</span>",J='<button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>';else if(y.status==="uploading")V='<span class="bq-stat bq-run">'+t("bank-queue-status-uploading")+'</span><div class="bq-bar"><div class="bq-bar-fill" style="width:'+(y.progress||0)+'%"></div></div>';else if(y.status==="parsing")V='<span class="bq-stat bq-run">'+t("bank-queue-status-parsing")+'</span><div class="bq-bar"><div class="bq-bar-fill bq-bar-indet"></div></div>';else if(y.status==="ok")V='<span class="bq-stat bq-ok">'+t("bank-queue-status-ok").replace("{n}",y.tx_count||0)+"</span>",J='<button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>';else if(y.status==="failed"){const X=k(y.error_code||"unknown");V='<span class="bq-stat bq-fail" title="'+O(X)+'">'+O(X)+"</span>",J='<button data-q-act="retry" data-q-id="'+O(y.id)+'" class="bq-act bq-act-retry">'+t("bank-queue-retry")+'</button><button data-q-act="remove" data-q-id="'+O(y.id)+'" class="bq-act">×</button>'}return'<div class="bq-row" data-q-row="'+O(y.id)+'"><div class="bq-name" title="'+O(y.name)+'">'+O(y.name)+'</div><div class="bq-size">'+N+'</div><div class="bq-status">'+V+'</div><div class="bq-actions">'+J+"</div></div>"}function h(y){const N=f.find(V=>V.id===y);N&&(N.status="pending",N.error_code=null,N.progress=0,E(),D())}function S(y){const N=f.findIndex(J=>J.id===y);if(N<0)return;const V=f[N];V.status==="uploading"||V.status==="parsing"||(f.splice(N,1),E())}function L(){f=f.filter(y=>y.status!=="ok"),E()}async function D(){for(;;){if(f.filter(V=>V.status==="uploading"||V.status==="parsing").length>=d)return;const N=f.find(V=>V.status==="pending");if(!N){f.every(V=>V.status==="ok"||V.status==="failed")&&(await l(),typeof window.loadReconcilePage=="function"&&window.loadReconcilePage());return}H(N).then(()=>D())}}async function H(y){y.status="uploading",y.progress=0,E();try{const N=new FormData;N.append("file",y.file,y.name);const V=await new Promise((X,de)=>{const ie=new XMLHttpRequest;ie.open("POST","/api/bank-recon/upload"),ie.setRequestHeader("Authorization","Bearer "+token),ie.upload.onprogress=ce=>{ce.lengthComputable&&(y.progress=Math.min(99,Math.round(ce.loaded/ce.total*100)),E())},ie.upload.onload=()=>{y.status="parsing",E()},ie.onload=()=>{ie.status>=200&&ie.status<300?X({status:ie.status,text:ie.responseText}):X({status:ie.status,text:ie.responseText})},ie.onerror=()=>de(new Error("network")),ie.send(N)});let J={};try{J=JSON.parse(V.text||"{}")}catch{J={}}if(V.status>=400){y.status="failed",y.error_code=J&&J.detail||"unknown",E();return}if(J.parse_status==="parse_failed"){y.status="failed",y.error_code=J.error==="scanned_pdf_not_yet"?"bank_recon.scanned":"bank_recon.no_tx",E();return}y.status="ok",y.tx_count=J.tx_count||0,y.session_id=J.session_id||null,E()}catch(N){console.warn("[bank-recon] upload failed",N),y.status="failed",y.error_code="network",E()}}async function _(){if(!n)return;const y=document.getElementById("btn-bank-run-match"),N=y.innerHTML;y.disabled=!0,y.innerHTML="<span>"+t("bank-matching")+"</span>";try{const V=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id)+"/match",{method:"POST",headers:{Authorization:"Bearer "+token}});if(!V.ok)throw new Error("match:"+V.status);const J=await V.json();showToast(t("bank-match-done").replace("{matched}",J.matched).replace("{suggested}",J.suggested).replace("{unmatched}",J.unmatched),"success"),await p(n.id),await l()}catch(V){console.warn("[bank-recon] match failed",V),showToast(t("bank-match-failed"),"error")}finally{y.disabled=!1,y.innerHTML=N}}async function g(){if(!(!n||!await showConfirm(t("bank-delete-confirm"),{danger:!0})))try{const N=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id),{method:"DELETE",headers:{Authorization:"Bearer "+token}});if(!N.ok)throw new Error("delete:"+N.status);showToast(t("bank-deleted"),"success"),n=null,a=[],ee(),await l()}catch(N){console.warn("[bank-recon] delete failed",N),showToast(t("bank-delete-failed"),"error")}}async function I(){if(s)try{const y=await fetch("/api/bank-recon/tx/"+encodeURIComponent(s.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"ignored"})});if(!y.ok)throw new Error("ignore:"+y.status);ne(),await p(n.id)}catch{showToast(t("bank-action-failed"),"error")}}async function M(y){if(s)try{const N=await fetch("/api/bank-recon/tx/"+encodeURIComponent(s.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"matched",history_id:y})});if(!N.ok)throw new Error("pick:"+N.status);showToast(t("bank-matched-ok"),"success"),ne(),await p(n.id)}catch{showToast(t("bank-action-failed"),"error")}}function P(){const y=document.getElementById("bank-status-summary");if(!y)return;if(e.length===0){y.textContent=t("bank-pill-none");return}let V=0;for(const J of e)J.parse_status==="parsed"&&(J.unmatched_count||0)>0&&V++;y.textContent=V>0?t("bank-pill-pending").replace("{n}",V):t("bank-pill-ok")}let R="all";function B(){const y=document.getElementById("bank-sessions-list");if(!y)return;let N=e||[];if(R==="parsed"?N=N.filter(V=>V.parse_status==="parsed"):R==="failed"&&(N=N.filter(V=>V.parse_status==="parse_failed")),!e||e.length===0){y.innerHTML='<div class="bank-empty" data-i18n="bank-sessions-empty">'+t("bank-sessions-empty")+"</div>";return}if(N.length===0){y.innerHTML='<div class="bank-empty">'+t("bank-sess-filter-empty")+"</div>";return}y.innerHTML=N.map(V=>Y(V)).join(""),y.querySelectorAll(".bank-session-row").forEach(V=>{V.addEventListener("click",J=>{J.target.closest(".bank-session-trash")||p(V.dataset.sessionId)})}),y.querySelectorAll(".bank-session-trash").forEach(V=>{V.addEventListener("click",J=>{J.stopPropagation();const X=V.dataset.sessionId,de=V.dataset.sessionName||"";j(X,de)})})}async function j(y,N){if(!y)return;const V=(t("bank-session-delete-confirm")||"确定删除这条对账记录吗?").replace("{name}",N||"");if(await showConfirm(V,{danger:!0}))try{const X=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(y),{method:"DELETE",headers:{Authorization:"Bearer "+token}});if(!X.ok)throw new Error("delete:"+X.status);showToast(t("bank-deleted"),"success"),n&&n.id===y&&(n=null,a=[],ee()),await l(),typeof window.loadReconcilePage=="function"&&window.loadReconcilePage()}catch(X){console.warn("[bank-recon] delete failed",X),showToast(t("bank-delete-failed"),"error")}}window._deleteBankSession=j;function Y(y){const N=(y.bank_code||"OTHER").toUpperCase(),V=G(y.period_start,y.period_end),J=y.account_last4?"···"+y.account_last4:"",X=A(y),de=q(y.created_at);return`
            <div class="bank-session-row" data-session-id="${O(y.id)}">
                ...
                <div class="bank-session-info">
                    ...
                    <div class="bank-session-meta">${O(V)} · ${O(J)} · ${O(de)}</div>
                </div>
        ...
        `}function A(y){if(y.parse_status==="parse_failed")return`<span class="bank-session-count cnt-failed">${t("bank-count-parse-failed")}</span>`;if(y.parse_status!=="parsed")return`<span class="bank-session-count">${t("bank-count-parsing")}</span>`;const N=y.tx_count||0,V=y.matched_count||0,J=y.unmatched_count||0,X=[`<span class="bank-session-count">${N} ${t("bank-count-tx")}</span>`];return V>0&&X.push(`<span class="bank-session-count cnt-matched">${V} ${t("bank-count-matched")}</span>`),J>0&&X.push(`<span class="bank-session-count cnt-unmatched">${J} ${t("bank-count-unmatched")}</span>`),X.join("")}function W(){document.getElementById("bank-detail").style.display="",document.querySelector(".bank-sessions-section").style.display="none",K(),Z(),oe()}function ee(){document.getElementById("bank-detail").style.display="none",document.querySelector(".bank-sessions-section").style.display="";const y=document.getElementById("bank-detail-body");y&&y.classList.remove("has-pane"),s=null}function oe(){const y=document.getElementById("bank-client-badge");if(!y||!n)return;const N=n.client_id,V=document.getElementById("bank-client-badge-dot"),J=document.getElementById("bank-client-badge-name"),X=document.getElementById("bank-client-badge-caret"),de=typeof _userInfo<"u"?_userInfo:null,ie=!(de&&de.role==="member");if(N!=null){const ce=(window._clientsCache||[]).find(ae=>Number(ae.id)===Number(N));y.classList.remove("is-empty"),V&&(V.style.background=ce&&ce.color||"#111111"),J&&(J.textContent=ce&&(ce.short_name||ce.name)||"#"+N)}else y.classList.add("is-empty"),V&&(V.style.background=""),J&&(J.textContent=t("bank-client-none"));ie?(y.classList.remove("is-readonly"),y.disabled=!1,X&&(X.style.display="")):(y.classList.add("is-readonly"),y.disabled=!0,X&&(X.style.display="none")),y.style.display=""}let $=null;function x(){if(!n)return;const y=typeof _userInfo<"u"?_userInfo:null;if(!!(y&&y.role==="member"))return;$=n.client_id!=null?Number(n.client_id):null,z();const V=document.getElementById("bank-client-picker-modal");V&&(V.style.display="")}function b(){const y=document.getElementById("bank-client-picker-modal");y&&(y.style.display="none"),$=null}function z(){const y=document.getElementById("bank-client-picker-list");if(!y)return;const N=(window._clientsCache||[]).filter(J=>J&&(J.is_active===!0||J.is_active===void 0)),V=[];V.push('<div class="bank-client-picker-row is-none'+($==null?" is-selected":"")+'" data-cid=""><span class="bank-cp-dot"></span><span>'+O(t("bank-client-picker-none"))+"</span></div>"),N.forEach(J=>{const X=Number(J.id)===Number($)?" is-selected":"";V.push('<div class="bank-client-picker-row'+X+'" data-cid="'+O(J.id)+'"><span class="bank-cp-dot" style="background:'+O(J.color||"#111111")+'"></span><span>'+O(J.short_name||J.name||"#"+J.id)+"</span></div>")}),y.innerHTML=V.join(""),y.querySelectorAll(".bank-client-picker-row").forEach(J=>{J.addEventListener("click",()=>{const X=J.dataset.cid;$=X?Number(X):null,z()})})}async function F(){if(n)try{const y=await fetch("/api/bank-recon/sessions/"+encodeURIComponent(n.id)+"/client",{method:"PATCH",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({client_id:$})});if(!y.ok)throw new Error("client:"+y.status);n.client_id=$,oe(),showToast(t("bank-client-changed"),"success"),b();try{await l()}catch{}}catch(y){console.warn("[bank-recon] save client failed",y),showToast(t("bank-client-change-failed"),"error")}}function K(){if(!n)return;const y=n;document.getElementById("bank-detail-title").textContent=(y.bank_code||"-")+(y.account_last4?" ···"+y.account_last4:"")+" · "+(y.source_filename||""),document.getElementById("bank-meta-period").textContent=G(y.period_start,y.period_end)||"-",document.getElementById("bank-meta-opening").textContent=T(y.opening_balance),document.getElementById("bank-meta-inflow").textContent="+"+T(y.total_inflow),document.getElementById("bank-meta-outflow").textContent="-"+T(y.total_outflow),document.getElementById("bank-meta-closing").textContent=T(y.closing_balance);const N=a||[],V=N.length;let J=0,X=0,de=0;for(const ie of N){const ce=ie.match_status||"unmatched";ce==="matched"?J++:ce==="suggested"?X++:de++}document.getElementById("bank-stat-total").textContent=V,document.getElementById("bank-stat-matched").textContent=J,document.getElementById("bank-stat-suggested").textContent=X,document.getElementById("bank-stat-unmatched").textContent=de}function Z(){const y=document.getElementById("bank-tx-tbody");if(!y)return;let N=a||[];if(o!=="all"&&(N=N.filter(V=>o==="matched"?V.match_status==="matched":o==="suggested"?V.match_status==="suggested":o==="unmatched"?V.match_status==="unmatched"||V.match_status==="ignored":!0)),N.length===0){y.innerHTML=`<tr><td colspan="4" class="bank-empty">${t("bank-tx-empty")}</td></tr>`;return}if(y.innerHTML=N.map(V=>le(V)).join(""),y.querySelectorAll("tr[data-tx-id]").forEach(V=>{V.addEventListener("click",()=>{const J=V.dataset.txId,X=a.find(de=>de.id===J);X&&(y.querySelectorAll("tr.is-selected").forEach(de=>de.classList.remove("is-selected")),V.classList.add("is-selected"),re(X))})}),s){const V=y.querySelector('tr[data-tx-id="'+s.id+'"]');V&&V.classList.add("is-selected")}}function le(y){const N=y.direction==="OUT",V=N?"-":"+",J=N?"bank-out":"bank-in",X=y.match_status||"unmatched",de=t("bank-match-"+X)||X,ie=q(y.tx_date),ce=y.channel?`<span class="bank-tx-channel">${O(y.channel)}</span>`:"";return`
            <tr data-tx-id="${O(y.id)}">
                <td class="bank-tx-date">${O(ie)}</td>
                <td class="bank-tx-desc">${ce}${O(y.description||"-")}</td>
                <td class="bank-td-amount ${J}">${V}${T(y.amount)}</td>
                <td><span class="bank-tx-match mt-${X}">${O(de)}</span></td>
            </tr>
        `}async function re(y){s=y;const N=document.getElementById("bank-detail-body");N&&N.classList.add("has-pane");const V=document.getElementById("bank-cand-pane-title"),J=document.getElementById("bank-cand-pane-sub"),X=document.getElementById("bank-cand-pane-foot");if(V&&(V.textContent=t("bank-cand-pane-current")),J){const ie=y.direction==="OUT"?"-":"+",ce=y.direction==="OUT"?"bank-out":"bank-in";J.innerHTML=`${O(q(y.tx_date))}
                <span style="margin:0 6px;color:#D1D5DB">·</span>
                <span>${O(y.description||"-")}</span>
                <span style="margin:0 6px;color:#D1D5DB">·</span>
                <strong class="${ce}">${ie}${T(y.amount)}</strong>`}X&&(X.style.display="");const de=document.getElementById("bank-cand-pane-body");if(de){de.innerHTML=`<div class="bank-empty">${t("bank-cand-loading")}</div>`;try{const ie=await fetch("/api/bank-recon/tx/"+encodeURIComponent(y.id)+"/candidates",{headers:{Authorization:"Bearer "+token}});if(!ie.ok)throw new Error("cands:"+ie.status);const ce=await ie.json();te(y,ce.candidates||[])}catch{de.innerHTML=`<div class="bank-empty">${t("bank-cand-load-failed")}</div>`}}}function U(y){const N=Number(y||0);let V="score-low";return N>=85?V="score-high":N>=60&&(V="score-mid"),'<span class="bank-cand-score '+V+'">'+N.toFixed(0)+" "+t("bank-cand-score-unit")+"</span>"}function Q(y,N,V){const J=N.history_id,X=N.invoice_no||"-",de=N.vendor||"-",ie=N.amount_total!==null&&N.amount_total!==void 0?T(N.amount_total):"-",ce=N.invoice_date?q(N.invoice_date):"-",ae=N.filename||"",pe=!!V&&y.matched_history_id===J,fe="bank-cand-card"+(N.is_auto_picked?" is-auto":"")+(pe?" is-picked":"");let me="";return pe?me='<button class="btn btn-ghost btn-small" data-act="unmatch"><span>'+t("bank-cand-unmatch")+"</span></button>":me='<button class="btn btn-primary btn-small" data-act="pick" data-hid="'+O(J)+'"><span>'+t(N.is_auto_picked?"bank-cand-confirm":"bank-cand-pick-this")+"</span></button>",'<div class="'+fe+'" data-hid="'+O(J)+'"><div class="bank-cand-card-head"><div class="bank-cand-card-vendor">'+O(de)+"</div>"+U(N.score)+'</div><div class="bank-cand-card-fields"><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-invoice-no")+"</span> "+O(X)+'</span><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-amount")+"</span> "+ie+'</span><span class="bank-cand-field"><span class="bank-cand-flabel">'+t("bank-cand-fld-date")+"</span> "+O(ce)+"</span></div>"+(ae?'<div class="bank-cand-card-file" title="'+O(ae)+'">'+O(ae)+"</div>":"")+(N.reason?'<div class="bank-cand-card-reason">'+O(N.reason)+"</div>":"")+'<div class="bank-cand-card-actions">'+me+"</div></div>"}function te(y,N){const V=document.getElementById("bank-cand-pane-body");if(!V)return;const J=N||[];let X="";if(y.match_status==="matched")X='<div class="bank-cand-hint hint-matched">'+t("bank-cand-hint-matched").replace("{n}",J.length)+"</div>";else if(y.match_status==="suggested")X='<div class="bank-cand-hint hint-suggested">'+t("bank-cand-hint-suggested").replace("{n}",J.length)+"</div>";else if(J.length>0)X='<div class="bank-cand-hint hint-low">'+t("bank-cand-hint-low").replace("{n}",J.length)+"</div>";else{V.innerHTML='<div class="bank-empty">'+t("bank-cand-no-match-detail")+"</div>";return}const de=y.match_status==="matched",ie=J.map(ce=>Q(y,ce,de)).join("");V.innerHTML=X+'<div class="bank-cand-list">'+ie+"</div>",V.querySelectorAll('[data-act="pick"]').forEach(ce=>{ce.addEventListener("click",()=>{M(ce.dataset.hid)})}),V.querySelectorAll('[data-act="unmatch"]').forEach(ce=>{ce.addEventListener("click",async()=>{try{await fetch("/api/bank-recon/tx/"+encodeURIComponent(y.id)+"/override",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({status:"unmatched"})}),ne(),await p(n.id)}catch{showToast(t("bank-action-failed"),"error")}})})}function ne(){const y=document.getElementById("bank-detail-body");y&&y.classList.remove("has-pane");const N=document.getElementById("bank-cand-pane-title"),V=document.getElementById("bank-cand-pane-sub"),J=document.getElementById("bank-cand-pane-body"),X=document.getElementById("bank-cand-pane-foot");N&&(N.textContent=t("bank-cand-pane-empty-title")),V&&(V.textContent=t("bank-cand-pane-empty-sub")),X&&(X.style.display="none"),J&&(J.innerHTML='<div class="bank-cand-pane-empty"><svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="14" y="10" width="36" height="44" rx="3"/><path d="M22 22h20M22 30h20M22 38h12"/></svg><div>'+t("bank-cand-pane-empty-hint")+"</div></div>");const de=document.getElementById("bank-tx-tbody");de&&de.querySelectorAll("tr.is-selected").forEach(ie=>ie.classList.remove("is-selected")),s=null}function se(y){const N=document.getElementById("bank-upload-progress");N&&(N.style.display="none")}function ue(){const y=document.getElementById("bank-upload-error");y&&(y.style.display="none")}function k(y){return{"bank_recon.only_pdf":t("bank-err-only-pdf"),"bank_recon.empty_file":t("bank-err-empty"),"bank_recon.file_too_large":t("bank-err-too-large"),"bank_recon.save_failed":t("bank-err-server"),"bank_recon.scanned":t("bank-err-scanned"),"bank_recon.no_tx":t("bank-err-no-tx"),network:t("bank-err-network")}[y]||t("bank-err-unknown")+" ("+y+")"}function T(y){if(y==null)return"-";const N=Number(y);return isNaN(N)?"-":N.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function q(y){if(!y)return"-";const N=String(y);return N.length>=10?N.slice(0,10):N}function G(y,N){return!y&&!N?"":(q(y)||"?")+" ~ "+(q(N)||"?")}function O(y){return y==null?"":String(y).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}window._loadBankReconPanel=r,window._rerenderBankRecon=function(){if(currentRoute==="automation"){if(B(),n&&(K(),Z(),oe(),!s)){const y=document.getElementById("bank-cand-pane-title"),N=document.getElementById("bank-cand-pane-sub");y&&(y.textContent=t("bank-cand-pane-empty-title")),N&&(N.textContent=t("bank-cand-pane-empty-sub"))}E()}},typeof window.subscribeI18n=="function"&&window.subscribeI18n("bank-recon",window._rerenderBankRecon),window._openBankSession=async function(y){y&&(i||await r(),await p(y))}})();(function(){const e=document.getElementById("page-clients");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        <div class="page-head">
            <div class="page-head-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){let e=[],n=null,a="",o="seller";const s={page:0,pageSize:12,keyword:""},i=new Set;let r=[];const u={keyword:""};let l=null;function p(){return{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}}async function f(b,z={}){const F=await fetch(b,{...z,headers:{"Content-Type":"application/json",...p(),...z.headers||{}}});if(!F.ok){const K=await F.json().catch(()=>({}));throw new Error(K.detail||"HTTP "+F.status)}return F.json()}async function c(){try{e=(await f("/api/clients")).clients||[],window._clientsCache=e}catch(b){console.error("loadClientsCache fail",b),e=[]}try{typeof window._refreshExcClientFilter=="function"&&window._refreshExcClientFilter()}catch{}try{typeof window._refreshClientSwitcher=="function"&&window._refreshClientSwitcher()}catch{}return e}function d(b){o=b==="buyer"?"buyer":"seller",document.querySelectorAll("[data-cust-tab]").forEach(K=>K.classList.toggle("active",K.dataset.custTab===o));const z=document.getElementById("cust-pane-seller"),F=document.getElementById("cust-pane-buyer");z&&z.classList.toggle("active",o==="seller"),F&&F.classList.toggle("active",o==="buyer")}function v(){const b=window._userInfo||{},z=String(b.role||"").toLowerCase(),F=String(b.tenant_role||"").toLowerCase();return b.is_super_admin===!0||b.is_owner===!0||z==="owner"||z==="admin"||F==="owner"||F==="admin"}function w(){window._workspaceClientsCache=r,typeof window.renderWorkspaceControl=="function"&&window.renderWorkspaceControl()}async function C(){try{const b=await f("/api/workspace/clients");r=b&&(b.clients||b.items)||[],window._workspaceClientsCache=r}catch(b){console.error("loadSellerCache fail",b),r=[]}return r}function E(){const b=u.keyword.trim().toLowerCase();return b?r.filter(z=>(z.name||"").toLowerCase().includes(b)||(z.tax_id||"").toLowerCase().includes(b)):r}function m(){const b=document.getElementById("seller-tbody");if(!b)return;const z=v(),F=document.getElementById("btn-seller-new");F&&(F.style.display=z?"":"none");const K=E(),Z=typeof window.getActiveWorkspaceClientId=="function"?window.getActiveWorkspaceClientId():null;if(!K.length){b.innerHTML=`<div class="cust-empty">${escapeHtml(t(u.keyword?"cust-no-match":"seller-empty"))}</div>`;return}b.innerHTML=K.map(le=>{const U=Z!=null&&Number(Z)===Number(le.id)?`<span class="cust-badge-current"><svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 7.5l3.2 3.2L12 4"/></svg>${escapeHtml(t("seller-current"))}</span>`:`<button class="cust-row-btn primary" data-saction="activate" data-wid="${le.id}">${escapeHtml(t("seller-set-current"))}</button>`,Q=z?`
            ...
            </div>`}).join("")}function h(b){l=b?b.id:null,document.getElementById("wsclient-modal-title").textContent=t(b?"wsclient-modal-edit":"wsclient-modal-new"),document.getElementById("wsclient-input-name").value=b&&b.name||"",document.getElementById("wsclient-input-tax").value=b&&b.tax_id||"",document.getElementById("wsclient-modal-archive").style.display=b?"":"none",document.getElementById("wsclient-modal-mask").style.display="flex",setTimeout(()=>document.getElementById("wsclient-input-name").focus(),50)}function S(){document.getElementById("wsclient-modal-mask").style.display="none",l=null}async function L(){const b=document.getElementById("wsclient-input-name").value.trim(),z=document.getElementById("wsclient-input-tax").value.trim();if(!b){showToast(t("client-msg-name-required"),"fail");return}try{l?(await f("/api/workspace/clients/"+l,{method:"PATCH",body:JSON.stringify({name:b,tax_id:z})}),showToast(t("client-msg-updated"),"success")):(await f("/api/workspace/clients",{method:"POST",body:JSON.stringify({name:b,tax_id:z||null})}),showToast(t("client-msg-created"),"success")),S(),await C(),m(),w()}catch(F){const K=F&&F.message?F.message:t("client-msg-save-fail");showToast(t("client-msg-save-fail")+" · "+K,"fail")}}async function D(){if(!l)return;const b=r.find(F=>Number(F.id)===Number(l));if(await showConfirm(t("wsclient-archive-confirm").replace("{name}",b?b.name:""),{danger:!0}))try{const F=l;await f("/api/workspace/clients/"+F,{method:"DELETE"}),showToast(t("wsclient-archived"),"success"),typeof window.getActiveWorkspaceClientId=="function"&&Number(window.getActiveWorkspaceClientId())===Number(F)&&typeof window.enterPersonalMode=="function"&&window.enterPersonalMode(),S(),await C(),m(),w()}catch{showToast(t("client-msg-save-fail"),"fail")}}function H(){const b=s.keyword.trim().toLowerCase();return b?e.filter(z=>(z.name||"").toLowerCase().includes(b)||(z.short_name||"").toLowerCase().includes(b)||(z.tax_id||"").toLowerCase().includes(b)):e}function _(){const b=H(),z=s.pageSize,F=Math.max(0,Math.ceil(b.length/z)-1);s.page>F&&(s.page=F);const K=s.page*z;return{all:b,items:b.slice(K,K+z),start:K,ps:z,total:b.length,maxPage:F}}function g(){const b=document.getElementById("buyer-tbody");if(!b)return;const{items:z,start:F,ps:K,total:Z,maxPage:le}=_();Z?b.innerHTML=z.map(te=>{const ne=i.has(te.id);return`<div class="cust-row buyer-grid${ne?" selected":""}" data-cid="${te.id}">
                ...
                </div>`}).join(""):b.innerHTML=`<div class="cust-empty">${escapeHtml(t(s.keyword?"cust-no-match":"clients-empty"))}</div>`;const re=document.getElementById("buyer-pager-info");re&&(re.textContent=Z?`${F+1}–${Math.min(F+K,Z)} / ${Z}`:"0");const U=document.getElementById("buyer-prev");U&&(U.disabled=s.page<=0);const Q=document.getElementById("buyer-next");Q&&(Q.disabled=s.page>=le),I()}function I(){const b=i.size,z=document.getElementById("buyer-batch-bar");z&&(z.style.display=b?"flex":"none");const F=document.getElementById("buyer-batch-count");F&&(F.textContent=t("cust-selected-n").replace("{n}",b));const K=document.getElementById("buyer-check-all");if(K){const{items:Z}=_(),le=Z.map(U=>U.id),re=le.filter(U=>i.has(U)).length;K.checked=le.length>0&&re===le.length,K.indeterminate=re>0&&re<le.length}}function M(){i.clear(),g()}async function P(){const b=Array.from(i);if(!(!b.length||!await showConfirm(t("cust-batch-del-confirm").replace("{n}",b.length),{danger:!0})))try{await f("/api/clients/batch-delete",{method:"POST",body:JSON.stringify({ids:b})}),showToast(t("client-msg-deleted"),"success"),i.clear(),await c(),g(),ee(),x()}catch{showToast(t("client-msg-save-fail"),"fail")}}window.loadClientsPage=async function(){const b=document.getElementById("seller-tbody");b&&(b.innerHTML=`<div class="cust-loading">${escapeHtml(t("clients-loading"))}</div>`);const z=document.getElementById("buyer-tbody");z&&(z.innerHTML=`<div class="cust-loading">${escapeHtml(t("clients-loading"))}</div>`),await Promise.all([C(),c()]),m(),g()},window.addEventListener("pearnly:workspace-changed",function(){typeof currentRoute<"u"&&currentRoute==="clients"&&m()});function R(b){n=b?b.id:null;const z=!!b;document.getElementById("client-modal-title").textContent=t(z?"client-modal-edit":"client-modal-new"),document.getElementById("client-input-name").value=b&&b.name||"",document.getElementById("client-input-short").value=b&&b.short_name||"",document.getElementById("client-input-tax").value=b&&b.tax_id||"",document.getElementById("client-input-address").value=b&&b.address||"",document.getElementById("client-input-contact").value=b&&b.contact_person||"",document.getElementById("client-input-phone").value=b&&b.contact_phone||"",document.getElementById("client-input-email").value=b&&b.contact_email||"",document.getElementById("client-input-notes").value=b&&b.notes||"";const F=b&&b.color||"#111111";document.querySelectorAll("#client-color-picker .color-swatch").forEach(K=>{K.classList.toggle("active",K.dataset.color===F)}),document.getElementById("client-modal-delete").style.display=z?"":"none",document.getElementById("client-modal-mask").style.display="flex",setTimeout(()=>document.getElementById("client-input-name").focus(),50)}function B(){document.getElementById("client-modal-mask").style.display="none",n=null}function j(){const b=document.querySelector("#client-color-picker .color-swatch.active");return b?b.dataset.color:"#111111"}async function Y(){const b=document.getElementById("client-input-name").value.trim();if(!b){showToast(t("client-msg-name-required"),"fail");return}const z={name:b,short_name:document.getElementById("client-input-short").value.trim()||null,tax_id:document.getElementById("client-input-tax").value.trim()||null,address:document.getElementById("client-input-address").value.trim()||null,contact_person:document.getElementById("client-input-contact").value.trim()||null,contact_phone:document.getElementById("client-input-phone").value.trim()||null,contact_email:document.getElementById("client-input-email").value.trim()||null,notes:document.getElementById("client-input-notes").value.trim()||null,color:j()};try{n?(await f(`/api/clients/${n}`,{method:"PATCH",body:JSON.stringify(z)}),showToast(t("client-msg-updated"),"success")):(await f("/api/clients",{method:"POST",body:JSON.stringify(z)}),showToast(t("client-msg-created"),"success")),B(),await c(),currentRoute==="clients"&&g(),ee(),x()}catch(F){console.error("saveClient fail",F);const K=F&&F.message?F.message:t("client-msg-save-fail");showToast(t("client-msg-save-fail")+" · "+K,"fail")}}async function A(){if(!n)return;const b=e.find(K=>K.id===n);if(!b)return;const z=t("client-delete-confirm").replace("{name}",b.name);if(await showConfirm(z,{danger:!0}))try{await f(`/api/clients/${n}`,{method:"DELETE"}),showToast(t("client-msg-deleted"),"success"),B(),await c(),currentRoute==="clients"&&g(),ee(),x()}catch(K){console.error(K),showToast(t("client-msg-save-fail"),"fail")}}async function W(b){const z=e.find(F=>F.id===b);if(typeof window.openClientExportModal=="function"){window.openClientExportModal(b,z?z.name:"");return}try{const F=localStorage.getItem("mrpilot_token"),K=await fetch(`/api/clients/${b}/export?month=all`,{headers:{Authorization:"Bearer "+F}});if(!K.ok){let Q="HTTP "+K.status;try{const te=await K.json();te&&te.detail&&(Q=te.detail)}catch{}throw new Error(Q)}const Z=await K.blob();if(Z.size<200){showToast(t("client-export-month-empty"),"info");return}const le=z&&z.name?z.name.replace(/[^a-zA-Z0-9\u0e00-\u0e7f\u4e00-\u9fff]/g,"_").slice(0,40):"client",re=URL.createObjectURL(Z),U=document.createElement("a");U.href=re,U.download=`${le}_export.csv`,U.click(),URL.revokeObjectURL(re)}catch(F){console.error("exportClient fail",F),showToast(t("client-msg-save-fail")+" · "+(F.message||""),"fail")}}function ee(){const b=document.getElementById("drawer-client-select");if(!b)return;const z=b.value;b.innerHTML=`<option value="">${escapeHtml(t("drawer-client-none"))}</option>`+e.map(F=>`<option value="${F.id}">${escapeHtml(F.name)}</option>`).join(""),b.value=z||""}window.bindDrawerClient=function(b,z){const F=document.getElementById("drawer-client-select");if(!F)return;if(ee(),F.value=z?String(z):"",!b){F.onchange=null;const Z=document.getElementById("drawer-client-add");Z&&(Z.onclick=()=>R(null));return}F.onchange=async()=>{const Z=F.value?parseInt(F.value,10):null;try{await f(`/api/history/${b}/assign_client`,{method:"POST",body:JSON.stringify({client_id:Z})}),showToast(t("client-msg-updated"),"success");const le=_results[_drawerIdx];le&&(le.client_id=Z),await c()}catch(le){console.error(le),showToast(t("client-msg-save-fail"),"fail"),F.value=z?String(z):""}};const K=document.getElementById("drawer-client-add");K&&(K.onclick=()=>R(null))};let oe={fetched:0,items:[],supplier_count:0};window.fillCategoryDatalist=async function(){try{const b=document.getElementById("drawer-cat-datalist"),z=Date.now();if(z-oe.fetched<300*1e3){b&&(b.innerHTML=oe.items.map(K=>`<option value="${escapeHtml(K)}">`).join("")),$(oe.supplier_count);return}const F=await f("/api/categories",{method:"GET"});oe.fetched=z,oe.items=F&&F.categories||[],oe.supplier_count=F&&F.supplier_count||0,b&&(b.innerHTML=oe.items.map(K=>`<option value="${escapeHtml(K)}">`).join("")),$(oe.supplier_count)}catch(b){console.warn("fillCategoryDatalist failed",b)}};function $(b){const z=document.getElementById("drawer-cat-learned-tag");z&&b>0&&(z.textContent=(t("drawer-suggest-learned-with-count")||"已学 {n}").replace("{n}",b))}function x(){const b=document.getElementById("history-client-filter");if(!b)return;const z=b.value;b.innerHTML=`<option value="">${escapeHtml(t("history-client-all"))}</option>`+e.map(F=>`<option value="${F.id}">${escapeHtml(F.name)}</option>`).join(""),b.value=z||""}window.getHistoryClientFilter=function(){return a},document.addEventListener("DOMContentLoaded",()=>{const b=document.querySelector(".cust-tab-bar");b&&b.addEventListener("click",ae=>{const pe=ae.target.closest("[data-cust-tab]");pe&&d(pe.dataset.custTab)});const z=document.getElementById("btn-buyer-new");z&&z.addEventListener("click",()=>R(null));const F=document.getElementById("buyer-tbody");F&&F.addEventListener("click",ae=>{const pe=ae.target.closest(".buyer-row-check");if(pe){const ge=parseInt(pe.dataset.cid,10);pe.checked?i.add(ge):i.delete(ge);const ye=pe.closest(".cust-row");ye&&ye.classList.toggle("selected",pe.checked),I();return}const fe=ae.target.closest(".cust-row-btn");if(fe){ae.stopPropagation();const ge=parseInt(fe.dataset.cid,10);if(fe.dataset.action==="edit"){const ye=e.find(xe=>xe.id===ge);ye&&R(ye)}else fe.dataset.action==="export"&&W(ge);return}const me=ae.target.closest(".cust-row");if(me&&!ae.target.closest(".cust-cell-check")){const ge=e.find(ye=>ye.id===parseInt(me.dataset.cid,10));ge&&R(ge)}});const K=document.getElementById("buyer-check-all");K&&K.addEventListener("change",()=>{const{items:ae}=_();ae.forEach(pe=>{K.checked?i.add(pe.id):i.delete(pe.id)}),g()});const Z=document.getElementById("buyer-batch-cancel");Z&&Z.addEventListener("click",M);const le=document.getElementById("buyer-batch-delete");le&&le.addEventListener("click",P);const re=document.getElementById("buyer-prev");re&&re.addEventListener("click",()=>{s.page>0&&(s.page--,g())});const U=document.getElementById("buyer-next");U&&U.addEventListener("click",()=>{s.page++,g()});const Q=document.getElementById("buyer-search");if(Q){let ae;Q.addEventListener("input",()=>{clearTimeout(ae),ae=setTimeout(()=>{s.keyword=Q.value,s.page=0;const pe=document.getElementById("buyer-search-clear");pe&&(pe.style.display=Q.value?"":"none"),g()},200)})}const te=document.getElementById("buyer-search-clear");te&&te.addEventListener("click",()=>{const ae=document.getElementById("buyer-search");ae&&(ae.value=""),s.keyword="",s.page=0,te.style.display="none",g()});const ne=document.getElementById("btn-seller-new");ne&&ne.addEventListener("click",()=>h(null));const se=document.getElementById("seller-tbody");se&&se.addEventListener("click",ae=>{const pe=ae.target.closest("[data-saction]");if(!pe)return;ae.stopPropagation();const fe=parseInt(pe.dataset.wid,10),me=pe.dataset.saction,ge=r.find(ye=>Number(ye.id)===fe);me==="activate"?(typeof window.setActiveWorkspaceClientId=="function"&&window.setActiveWorkspaceClientId(fe),m(),typeof window.renderWorkspaceControl=="function"&&window.renderWorkspaceControl(),showToast(t("seller-activated").replace("{name}",ge?ge.name:""),"success")):me==="edit"?ge&&h(ge):me==="archive"&&(l=fe,D())});const ue=document.getElementById("seller-search");if(ue){let ae;ue.addEventListener("input",()=>{clearTimeout(ae),ae=setTimeout(()=>{u.keyword=ue.value;const pe=document.getElementById("seller-search-clear");pe&&(pe.style.display=ue.value?"":"none"),m()},200)})}const k=document.getElementById("seller-search-clear");k&&k.addEventListener("click",()=>{const ae=document.getElementById("seller-search");ae&&(ae.value=""),u.keyword="",k.style.display="none",m()});const T=document.getElementById("wsclient-modal-close");T&&T.addEventListener("click",S);const q=document.getElementById("wsclient-modal-cancel");q&&q.addEventListener("click",S);const G=document.getElementById("wsclient-modal-save");G&&G.addEventListener("click",L);const O=document.getElementById("wsclient-modal-archive");O&&O.addEventListener("click",D);const y=document.getElementById("wsclient-modal-mask");y&&y.addEventListener("click",ae=>{ae.target===y&&S()});const N=document.getElementById("client-modal-close");N&&N.addEventListener("click",B);const V=document.getElementById("client-modal-cancel");V&&V.addEventListener("click",B);const J=document.getElementById("client-modal-save");J&&J.addEventListener("click",Y);const X=document.getElementById("client-modal-delete");X&&X.addEventListener("click",A);const de=document.getElementById("client-modal-mask");de&&de.addEventListener("click",ae=>{ae.target===de&&B()});const ie=document.getElementById("client-color-picker");ie&&ie.addEventListener("click",ae=>{const pe=ae.target.closest(".color-swatch");pe&&(ie.querySelectorAll(".color-swatch").forEach(fe=>fe.classList.remove("active")),pe.classList.add("active"))});const ce=document.getElementById("history-client-filter");ce&&ce.addEventListener("change",()=>{a=ce.value,typeof renderHistoryList=="function"&&renderHistoryList()})}),setTimeout(()=>c(),1e3)})();(function(){const e=document.getElementById("page-exceptions");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        <div class="page-head">
            <div class="page-head-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function n($,x){let b=t($)||$;if(x)for(const z in x)b=b.replace(new RegExp("\\{"+z+"\\}","g"),String(x[z]));return b}async function a(){try{const $=e.currentClient||"",x="/api/exceptions/stats?status=pending"+($?"&client_id="+encodeURIComponent($):""),b=await fetch(x,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!b.ok)return;const z=await b.json(),F=document.getElementById("nav-exc-badge");if(!F)return;const K=parseInt(z.pending||0,10);K>0?(F.textContent=K>99?"99+":String(K),F.style.display=""):F.style.display="none"}catch{}}function o($){return $==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        ...
        </svg>`}function i($){if($==null)return"—";const x=parseFloat($);return isNaN(x)?"—":"฿ "+x.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})}function r($){return $?$.slice(0,10):"—"}function u($){document.getElementById("exc-kpi-pending").textContent=$.pending||0,document.getElementById("exc-kpi-high").textContent=$.high_severity||0,document.getElementById("exc-kpi-resolved").textContent=$.resolved||0,document.getElementById("exc-kpi-learned").textContent=$.learned_rules||0;const x=document.getElementById("exc-status-tab-count-pending"),b=document.getElementById("exc-status-tab-count-resolved"),z=document.getElementById("exc-status-tab-count-ignored");x&&(x.textContent=$.pending||0),b&&(b.textContent=$.resolved||0),z&&(z.textContent=$.ignored||0),document.querySelectorAll("#exc-status-tabs .exc-status-tab").forEach(K=>{K.classList.toggle("active",K.dataset.status===(e.currentStatus||"pending"))})}function l($){const x=document.getElementById("exc-chips");if(!x)return;const b=$.by_rule||{},z=["confidence_low","duplicate","amount_missing","math_mismatch","tax_id_format_invalid"];let K=`<button class="exc-chip ${!e.currentRule?"active":""}" data-rule="">
            <span>${escapeHtml(t("exc-chip-all"))}</span>
            <span class="exc-chip-count">${$.pending||0}</span>
        </button>`;for(const Z of z){const le=b[Z]||0;if(le===0&&e.currentRule!==Z)continue;const re=e.currentRule===Z;K+=`<button class="exc-chip ${re?"active":""}" data-rule="${escapeHtml(Z)}">
                <span>${escapeHtml(t("exc-chip-"+Z))}</span>
                <span class="exc-chip-count">${le}</span>
            </button>`}x.innerHTML=K,x.querySelectorAll(".exc-chip").forEach(Z=>{Z.addEventListener("click",()=>{const le=Z.dataset.rule||null;e.currentRule=le,w()})})}function p($){const x=document.getElementById("exc-list");if(!x)return;if(!$||$.length===0){x.innerHTML=`<div class="exc-empty">
                ${s()}
                <div class="exc-empty-title">${escapeHtml(t("exc-empty-title"))}</div>
                <div>${escapeHtml(t("exc-empty-desc"))}</div>
            </div>`,c();return}const b='<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7l3 3 5-5"/></svg>',z=(e.currentStatus||"pending")==="pending";x.innerHTML=$.map(F=>{const K=F.severity||"medium",Z=t("exc-rule-"+F.rule_code)||F.rule_code,le=F.seller_name&&F.seller_name.trim()?F.seller_name:t("exc-no-seller"),re=F.filename||"—",U=r(F.invoice_date||F.created_at),Q=F.status==="pending",te=e.selectedIds.has(F.id),ne=z&&Q;return`
            ...
            `}).join(""),x.querySelectorAll(".exc-row").forEach(F=>{F.addEventListener("click",K=>{if(K.target.closest(".exc-row-check"))return;const Z=F.dataset.excId;Z&&L(parseInt(Z,10))})}),x.querySelectorAll(".exc-row-check").forEach(F=>{F.addEventListener("click",K=>{K.stopPropagation();const Z=parseInt(F.dataset.checkId,10);Z&&(e.selectedIds.has(Z)?(e.selectedIds.delete(Z),F.classList.remove("checked"),F.closest(".exc-row").classList.remove("selected")):(e.selectedIds.add(Z),F.classList.add("checked"),F.closest(".exc-row").classList.add("selected")),f())})}),f(),c()}function f(){const $=document.getElementById("exc-batch-bar"),x=document.getElementById("exc-batch-count");if(!$||!x)return;const b=e.selectedIds.size;b===0?$.style.display="none":($.style.display="",x.textContent=n("exc-batch-count",{n:b}))}function c(){const $=document.getElementById("exc-list-foot"),x=document.getElementById("exc-list-count"),b=document.getElementById("exc-loadmore");if(!$||!x||!b)return;const z=e.listCache.length;if(z===0){$.style.display="none";return}$.style.display="";let F=z;const K=e.statsCache;K&&(e.currentRule?F=(K.by_rule||{})[e.currentRule]||z:F=K.pending||z),e.total=F,x.textContent=n("exc-list-count",{shown:z,total:F});const Z=z<F&&z<500;b.style.display=Z?"":"none"}async function d(){try{if(navigator.onLine===!1)throw new Error("offline");const $=e.currentClient||"",x=e.currentStatus||"pending",b=new URLSearchParams;b.set("status",x),$&&b.set("client_id",$);const z="/api/exceptions/stats?"+b.toString(),F=await fetch(z,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!F.ok)throw new Error("http "+F.status);const K=await F.json();return e.statsCache=K,u(K),l(K),K}catch($){return console.warn("loadExceptionsStats fail",$),null}}function v($){const x=document.getElementById("exc-list");if(!x)return;const b=`<svg class="exc-error-icon" viewBox="0 0 36 36" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="18" cy="18" r="14"/>
            <line x1="18" y1="11" x2="18" y2="19"/>
            <circle cx="18" cy="23" r="0.8" fill="currentColor"/>
        </svg>`,z=$?t("exc-offline"):t("exc-error-retry-title"),F=$?"":t("exc-error-retry-desc");x.innerHTML=`
            <div class="exc-error">
                ${b}
                <div class="exc-error-msg">${escapeHtml(z)}${F?" · "+escapeHtml(F):""}</div>
                <button class="btn btn-sm btn-secondary" id="exc-retry-btn" type="button">${escapeHtml(t("exc-retry-btn"))}</button>
            </div>`;const K=document.getElementById("exc-retry-btn");K&&K.addEventListener("click",()=>window.loadExceptionsPage&&window.loadExceptionsPage())}async function w($){$=$||{};const x=!!$.append,b=document.getElementById("exc-list");!x&&b&&e.listCache.length===0&&(b.innerHTML=`<div class="exc-loading">${escapeHtml(t("exc-loading"))}</div>`);const z=new URLSearchParams;z.set("status",e.currentStatus||"pending"),e.currentRule&&z.set("rule_code",e.currentRule),e.currentClient&&z.set("client_id",e.currentClient);const F=x?e.listCache.length:0;z.set("limit",String(e.pageSize)),z.set("offset",String(F));try{if(navigator.onLine===!1)throw new Error("offline");const K=await fetch("/api/exceptions/list?"+z.toString(),{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!K.ok)throw new Error("http "+K.status);const le=(await K.json()).items||[];x?e.listCache=e.listCache.concat(le):(e.listCache=le,e.selectedIds.clear()),e.loadFailed=!1,p(e.listCache),e.statsCache&&l(e.statsCache)}catch(K){console.warn("loadExceptionsList fail",K),e.loadFailed=!0;const Z=navigator.onLine===!1||String(K.message||"").includes("offline");x?showToast(t("exc-toast-load-fail"),"error"):(v(Z),showToast(Z?t("exc-offline"):t("exc-toast-load-fail"),"error"))}}async function C(){if(!e.loading&&!(e.listCache.length>=500)){e.loading=!0;try{await w({append:!0})}finally{e.loading=!1}}}function E(){const $=document.getElementById("exc-client-filter");if(!$)return;const x=window._clientsCache||[],b=e.currentClient||"",z=typeof t=="function"?t("history-client-all"):"全部客户";$.innerHTML=`<option value="">${escapeHtml(z)}</option>`+x.map(F=>`<option value="${F.id}">${escapeHtml(F.name)}</option>`).join(""),$.value=b}window.loadExceptionsPage=async function(){if(!e.loading){e.loading=!0;try{E(),typeof window.loadErpExceptions=="function"&&window.loadErpExceptions(),await d(),await w()}finally{e.loading=!1}}},window.refreshExcBadge=a,window._refreshExcClientFilter=E,window._excState=e,window._rerenderExceptions=function(){try{E()}catch{}e.statsCache&&(u(e.statsCache),l(e.statsCache)),e.listCache&&e.listCache.length&&p(e.listCache);try{window._erpExcState&&window._erpExcState.items&&window._erpExcState.items.length&&typeof window._rerenderErpExceptions=="function"&&window._rerenderErpExceptions()}catch{}m.openExcId&&M()};let m={openExcId:null,excRow:null,history:null,loading:!1,pdfUrl:null,pdfStatus:"idle",editing:!1,editFields:null};function h(){if(m.pdfUrl){try{URL.revokeObjectURL(m.pdfUrl)}catch{}m.pdfUrl=null}m.pdfStatus="idle"}async function S($,x){m.pdfStatus="loading",M();try{const b=await fetch("/api/history/"+encodeURIComponent($)+"/pdf",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(m.openExcId!==x)return;if(b.status===404){m.pdfStatus="empty",M();return}if(!b.ok)throw new Error("http "+b.status);const z=await b.blob();if(m.openExcId!==x)return;h(),m.pdfUrl=URL.createObjectURL(z),m.pdfStatus="ready",M()}catch(b){if(m.openExcId!==x)return;console.warn("loadDrawerPdf fail",b),m.pdfStatus="error",M()}}function L($){const x=(e.listCache||[]).find(b=>b.id===$);if(!x){showToast(t("exc-drawer-error"),"error");return}e.listScrollY=window.scrollY||document.documentElement.scrollTop||0,h(),m.editing=!1,m.editFields=null,m.openExcId=$,m.excRow=x,m.history=null,document.getElementById("exc-drawer-mask").classList.add("show"),document.getElementById("exc-drawer").classList.add("show"),M(),H(x.history_id),S(x.history_id,$)}function D(){h(),m.editing=!1,m.editFields=null,m.openExcId=null,m.excRow=null,m.history=null,document.getElementById("exc-drawer-mask").classList.remove("show"),document.getElementById("exc-drawer").classList.remove("show");const $=e.listScrollY||0;$>0&&requestAnimationFrame(()=>window.scrollTo(0,$))}async function H($){try{const x=await fetch("/api/history/"+encodeURIComponent($),{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);m.history=await x.json()}catch(x){console.warn("loadHistoryDetail fail",x),m.history={_err:!0}}m.excRow&&M()}function _($){if(!$||!$.pages)return{};const x=$.pages,b=x.find(z=>!z.is_duplicate&&!z.is_copy)||x[0];return b&&b.fields||{}}function g($){if($==null)return"—";const x=typeof $=="number"?$:parseFloat(String($).replace(/,/g,""));return isNaN(x)?escapeHtml(String($)):"฿ "+x.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})}function I($,x){if(x=x||{},$==="math_mismatch")return`
            ...
            `:$==="amount_missing"?`<div class="exc-why-detail-row" style="justify-content:center;color:var(--danger);"><span>${escapeHtml(t("exc-detail-missing"))}</span></div>`:`<div class="exc-why-detail-row"><span style="font-size:11px;">${escapeHtml(JSON.stringify(x))}</span></div>`}function M(){const $=m.excRow;if(!$)return;const x=$.seller_name&&$.seller_name.trim()?$.seller_name:t("exc-no-seller"),b=$.filename||"—";document.getElementById("exc-drawer-title").textContent=b;const z="exc-status-"+($.status||"pending"),F=t(z)||$.status,K="s-"+($.status||"pending"),Z=($.invoice_date||$.created_at||"").slice(0,10);document.getElementById("exc-drawer-sub").innerHTML=`
        ...
        `;const le=$.severity||"medium",re=t("exc-rule-"+$.rule_code)||$.rule_code,U=I($.rule_code,$.detail||{}),Q=_(m.history),te=m.history===null,ne=m.history&&m.history._err,se=new Set;$.rule_code==="math_mismatch"?(se.add("subtotal"),se.add("vat"),se.add("total_amount")):$.rule_code==="tax_id_format_invalid"?se.add("seller_tax"):$.rule_code==="amount_missing"&&(se.add("total_amount"),se.add("invoice_number"));const ue=!!m.editing,k=m.editFields||{},T=(ae,pe,fe)=>{if(te)return`<div class="exc-field-row"><label>${escapeHtml(t(pe))}</label><span class="val empty">…</span></div>`;const me=ue?k[ae]!==void 0?k[ae]:Q[ae]!==void 0&&Q[ae]!==null?Q[ae]:"":Q[ae],ge=se.has(ae)?"flagged":"";if(ue){const Oe=fe?"number":"text",Ce=fe?' step="0.01" inputmode="decimal"':"",He=me==null?"":String(me).replace(/"/g,"&quot;");return`<div class="exc-field-row ${ge} editing">
                    <label>${escapeHtml(t(pe))}</label>
                    <input class="exc-field-input" type="${Oe}"${Ce} data-edit-key="${escapeHtml(ae)}" value="${He}">
                </div>`}const ye=fe?g(me):me||"",xe=me==null||me===""?`<span class="val empty">${escapeHtml(t("exc-empty-val"))}</span>`:`<span class="val">${escapeHtml(ye)}</span>`;return`<div class="exc-field-row ${ge}"><label>${escapeHtml(t(pe))}</label>${xe}</div>`};let q="";ne?q=`<div class="exc-drawer-error">${escapeHtml(t("exc-drawer-error"))}</div>`:q=`
        ...
        `;const O=document.getElementById("exc-fld-edit");O&&O.addEventListener("click",()=>{m.editing=!0,m.editFields={..._(m.history)},M()});const y=document.getElementById("exc-fld-cancel");y&&y.addEventListener("click",()=>{m.editing=!1,m.editFields=null,M()});const N=document.getElementById("exc-fld-save");N&&N.addEventListener("click",()=>P()),document.querySelectorAll(".exc-field-input").forEach(ae=>{ae.addEventListener("input",()=>{m.editFields||(m.editFields={}),m.editFields[ae.dataset.editKey]=ae.value})});const J=document.getElementById("exc-pdf-retry");J&&m.openExcId&&J.addEventListener("click",()=>{m.excRow&&S(m.excRow.history_id,m.openExcId)});const X=$.status==="pending",de=!!($.seller_name&&$.seller_name.trim()),ie=document.getElementById("exc-btn-resolve"),ce=document.getElementById("exc-btn-ignore");ie.disabled=!X,ce.disabled=!X||!de,ce.title=de?t("exc-ignore-hint"):t("exc-ignore-no-seller")}async function P(){if(!m.openExcId||!m.history||!m.history.pages||m.loading)return;m.loading=!0;const $=showToast(t("exc-fld-saving"),"loading",0);try{const x=JSON.parse(JSON.stringify(m.history.pages||[]));let b=x.findIndex(re=>!re.is_duplicate&&!re.is_copy);b<0&&(b=0),x[b]||(x[b]={fields:{}});const z=x[b].fields||{},F=m.editFields||{},K=new Set(["subtotal","vat","total_amount"]),Z={...z};for(const re in F){let U=F[re];if((U===""||U===void 0)&&(U=null),K.has(re)&&U!==null){const Q=parseFloat(U);U=isNaN(Q)?null:Q}Z[re]=U}x[b].fields=Z;const le=await fetch("/api/history/"+encodeURIComponent(m.history.id),{method:"PUT",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({pages:x})});if(!le.ok)throw new Error("http "+le.status);$(),showToast(t("exc-fld-save-ok"),"success"),D(),await d(),await w(),a()}catch(x){$(),console.warn("save fields fail",x),showToast(t("exc-fld-save-fail"),"error")}finally{m.loading=!1}}async function R(){if(!(!m.openExcId||m.loading)){m.loading=!0;try{const $=await fetch("/api/exceptions/"+m.openExcId+"/resolve",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!$.ok)throw new Error("http "+$.status);showToast(t("exc-toast-resolved"),"success"),D(),await d(),await w(),a()}catch($){console.warn("resolve fail",$),showToast(t("exc-toast-action-fail"),"error")}finally{m.loading=!1}}}async function B(){if(!(!m.openExcId||m.loading)){m.loading=!0;try{const $=await fetch("/api/exceptions/"+m.openExcId+"/ignore",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!$.ok)throw new Error("http "+$.status);showToast(t("exc-toast-ignored"),"success"),D(),await d(),await w(),a()}catch($){console.warn("ignore fail",$),showToast(t("exc-toast-action-fail"),"error")}finally{m.loading=!1}}}let j=!1;async function Y(){if(j)return;const $=Array.from(e.selectedIds);if($.length===0||!await showConfirm(n("exc-batch-confirm-resolve",{n:$.length})))return;j=!0;const b=showToast(n("exc-batch-count",{n:$.length})+" …","loading",0);try{const z=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:$,action:"resolve"})});if(!z.ok)throw new Error("http "+z.status);const F=await z.json();b(),showToast(n("exc-toast-batch-resolved",{n:F.processed||0}),"success"),e.selectedIds.clear(),await d(),await w(),a()}catch(z){b(),console.warn("batch resolve fail",z),showToast(t("exc-toast-batch-fail"),"error")}finally{j=!1}}async function A(){if(j)return;const $=Array.from(e.selectedIds);if($.length===0||!await showConfirm(n("exc-batch-confirm-ignore",{n:$.length}),{danger:!1}))return;j=!0;const b=showToast(n("exc-batch-count",{n:$.length})+" …","loading",0);try{const z=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:$,action:"ignore"})});if(!z.ok)throw new Error("http "+z.status);const F=await z.json();b(),showToast(n("exc-toast-batch-ignored",{n:F.processed||0,wl:F.whitelist_added||0}),"success"),e.selectedIds.clear(),await d(),await w(),a()}catch(z){b(),console.warn("batch ignore fail",z),showToast(t("exc-toast-batch-fail"),"error")}finally{j=!1}}function W(){e.selectedIds.clear(),p(e.listCache)}document.addEventListener("click",$=>{$.target.closest("#exc-drawer-close")&&D(),$.target.closest("#exc-drawer-mask")&&D(),$.target.closest("#exc-btn-resolve")&&R(),$.target.closest("#exc-btn-ignore")&&B(),$.target.closest("#exc-batch-resolve")&&Y(),$.target.closest("#exc-batch-ignore")&&A(),$.target.closest("#exc-batch-clear")&&W(),$.target.closest("#exc-loadmore")&&C()}),document.addEventListener("keydown",$=>{$.key==="Escape"&&m.openExcId&&D()}),document.addEventListener("click",$=>{$.target.closest("#btn-exc-refresh")&&(typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a())}),document.addEventListener("change",$=>{if(!$.target.closest("#exc-client-filter"))return;const x=$.target;e.currentClient=x.value||"",e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a()}),document.addEventListener("click",$=>{const x=$.target.closest("#exc-status-tabs .exc-status-tab");if(!x)return;const b=x.dataset.status||"pending";b!==e.currentStatus&&(e.currentStatus=b,e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage())}),window.addEventListener("online",()=>{e.loadFailed&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage&&window.loadExceptionsPage()}),setTimeout(a,1500),setInterval(a,6e4);function ee($){if(!$)return"—";try{const x=new Date($),b=z=>String(z).padStart(2,"0");return`${x.getFullYear()}-${b(x.getMonth()+1)}-${b(x.getDate())} ${b(x.getHours())}:${b(x.getMinutes())}`}catch{return $.slice(0,16).replace("T"," ")}}async function oe(){const $=document.getElementById("learned-list");if($){$.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-loading"))}</div>`;try{const x=await fetch("/api/exception-whitelist",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);const z=(await x.json()).items||[];if(z.length===0){$.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-empty"))}</div>`;return}const F=`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                <path d="M3 4h8M5.5 4V2.5h3V4M4 4l0.6 8.5h4.8L10 4"/>
            </svg>`;$.innerHTML=z.map(K=>{const Z=t("exc-rule-"+K.rule_code)||K.rule_code;return`
                ...
                `}).join("")}catch(x){console.warn("loadLearnedRules fail",x),$.innerHTML=`<div class="learned-empty">${escapeHtml(t("exc-toast-load-fail"))}</div>`}}}window.loadLearnedRules=oe,document.addEventListener("click",async $=>{const x=$.target.closest("[data-del-wl]");if(!x)return;const b=parseInt(x.dataset.delWl,10);if(!b)return;const z=x.closest(".learned-row"),F=z&&z.querySelector(".learned-seller"),K=F?F.textContent.trim():"",Z=t("set-learned-del-confirm").replace("{seller}",K);if(await showConfirm(Z,{danger:!0}))try{const re=await fetch("/api/exception-whitelist/"+b,{method:"DELETE",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!re.ok)throw new Error("http "+re.status);showToast(t("set-learned-del-ok"),"success"),oe(),typeof window.loadExceptionsPage=="function"&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage()}catch(re){console.warn("delete whitelist fail",re),showToast(t("set-learned-del-fail"),"error")}})})();(function(){let e={items:[],q:"",cat:"",adapter:"",selected:new Set,total:0,categories:{},pageSize:30,loading:!1,focusSearch:!1,searchCaret:0},n=null;function a(){return localStorage.getItem("mrpilot_token")||""}function o(c){const d=typeof currentLang=="string"&&currentLang||window._currentLang||"th",v=c.error_friendly&&c.error_friendly[d];if(v)return v;if(typeof humanizeError=="function"&&c.error_msg)try{return humanizeError(c.error_msg)}catch{}return t("erp-exc-reason-"+(c.category||"other"))}function s(){const c=document.getElementById("erp-exc-batch");if(!c)return;const d=e.selected.size;c.hidden=d===0;const v=c.querySelector(".erp-exc-batch-count");v&&(v.textContent=String(d))}function i(){const c=document.getElementById("erp-exc-block");if(!c)return;const d=e;if(!(d.total>0||!!d.q||!!d.cat)){c.hidden=!0,c.innerHTML="";return}c.hidden=!1;const w=d.categories||{},C=Object.keys(w).reduce((A,W)=>A+w[W],0);let E=`<button class="erp-exc-chip ${d.cat===""?"active":""}" data-erpexc-cat=""><span>${escapeHtml(t("erp-exc-cat-all"))}</span><span class="erp-exc-chip-count">${C}</span></button>`;Object.keys(w).forEach(A=>{E+=`<button class="erp-exc-chip ${d.cat===A?"active":""}" data-erpexc-cat="${escapeHtml(A)}"><span>${escapeHtml(t("erp-exc-cat-"+A))}</span><span class="erp-exc-chip-count">${w[A]}</span></button>`});const m=d.items||[],h=m.length>0&&m.every(A=>d.selected.has(A.id)),S=m.map(A=>{const W=A.state==="needs_action"?"needs":A.state==="retrying"?"retry":"fail",ee=t("erp-exc-state-"+(A.state||"failed")),oe=o(A),$=d.selected.has(A.id)?"checked":"",x=A.push_type==="id_card",b=x?`<span class="erp-exc-type-idcard">${escapeHtml(t("erp-log-type-idcard"))}</span> `:"",z=x?`<span class="ex-inv" title="${escapeHtml(t("erp-log-col-booking"))}">${b}${escapeHtml(A.invoice_no||"—")}</span>`:`<span class="ex-inv" title="${escapeHtml(A.invoice_no||"")}">${escapeHtml(A.invoice_no||"—")}</span>`,F=x?`<span class="ex-seller" title="${escapeHtml(t("erp-log-col-customer"))}">${escapeHtml(A.seller_name||"—")}</span>`:`<span class="ex-seller" title="${escapeHtml(A.seller_name||"")}">${escapeHtml(A.seller_name||"—")}</span>`,K=x?'<span class="ex-buyer">—</span>':`<span class="ex-buyer" title="${escapeHtml(A.ocr_buyer_name||"")}">${escapeHtml(A.ocr_buyer_name||"—")}</span>`;return`<div class="erp-exc-row" data-erpexc-id="${escapeHtml(A.id)}">
                <span class="ex-cb"><input type="checkbox" class="erp-exc-cb" data-erpexc-cb="${escapeHtml(A.id)}" ${$}></span>
                ${z}
                ${F}
                ${K}
                <span class="ex-state"><span class="erp-exc-state ${W}">${escapeHtml(ee)}</span></span>
                <span class="ex-reason" title="${escapeHtml(oe)}">${escapeHtml(oe)}${A.error_code?` <span class="erp-exc-code">${escapeHtml(A.error_code)}</span>`:""}</span>
            ...
            </div>`}).join(""),L=m.length===0?`<div class="erp-exc-empty">${escapeHtml(t("erp-exc-empty"))}</div>`:"",D=m.length<d.total?`<button class="erp-exc-more" type="button" id="erp-exc-more">${escapeHtml(t("erp-exc-load-more"))} (${m.length}/${d.total})</button>`:d.total>0?`<div class="erp-exc-count">${escapeHtml(t("erp-exc-shown",{n:m.length,total:d.total}))}</div>`:"",H=d.adapter==="mrerp_dms",_=Array.isArray(window._erpEndpoints)?window._erpEndpoints:[],g=new Set;let I=`<option value="">${escapeHtml(t("erp-logs-erp-all"))}</option>`;_.forEach(A=>{const W=(A&&A.adapter||"").toLowerCase();!W||g.has(W)||(g.add(W),I+=`<option value="${escapeHtml(W)}"${W===d.adapter?" selected":""}>${escapeHtml(A&&A.name||W)}</option>`)});const M=H?t("erp-log-col-booking"):t("erp-exc-f-invoice"),P=H?t("erp-log-col-customer"):t("erp-exc-f-seller");c.innerHTML=`
            ...
            <div class="erp-exc-rows">
                <div class="erp-exc-row erp-exc-row-head">
                    ...
                    <span class="ex-seller">${escapeHtml(P)}</span>
            ...
            <div class="erp-exc-foot">${D}</div>`;const R=document.getElementById("erp-exc-search");if(R){if(d.focusSearch){R.focus();try{R.setSelectionRange(d.searchCaret,d.searchCaret)}catch{}}R.addEventListener("input",()=>{d.q=R.value,d.focusSearch=!0,d.searchCaret=R.selectionStart||R.value.length,clearTimeout(n),n=setTimeout(()=>l(!1),350)}),R.addEventListener("blur",()=>{d.focusSearch=!1})}c.querySelectorAll(".erp-exc-chip").forEach(A=>{A.addEventListener("click",()=>{d.cat=A.dataset.erpexcCat||"",l(!1)})});const B=document.getElementById("erp-exc-erp-select");B&&B.addEventListener("change",()=>{d.adapter=B.value||"",l(!1)}),c.querySelectorAll("[data-erpexc-retry]").forEach(A=>{A.addEventListener("click",W=>{W.stopPropagation(),r(A.dataset.erpexcRetry,A)})}),c.querySelectorAll(".erp-exc-cb").forEach(A=>{A.addEventListener("change",()=>{const W=A.dataset.erpexcCb;A.checked?d.selected.add(W):d.selected.delete(W);const ee=document.getElementById("erp-exc-cb-all");ee&&(ee.checked=m.length>0&&m.every(oe=>d.selected.has(oe.id))),s()})});const j=document.getElementById("erp-exc-cb-all");j&&j.addEventListener("change",()=>{m.forEach(A=>{j.checked?d.selected.add(A.id):d.selected.delete(A.id)}),c.querySelectorAll(".erp-exc-cb").forEach(A=>{A.checked=j.checked}),s()}),c.querySelectorAll("[data-erpexc-batch]").forEach(A=>{A.addEventListener("click",()=>u(A.dataset.erpexcBatch))});const Y=document.getElementById("erp-exc-more");Y&&Y.addEventListener("click",()=>l(!0)),c.querySelectorAll(".erp-exc-row:not(.erp-exc-row-head)").forEach(A=>{A.addEventListener("click",W=>{W.target.closest("input,button")||typeof window._erpExcOpenEdit=="function"&&window._erpExcOpenEdit(A.dataset.erpexcId)})})}async function r(c,d){if(c){d&&(d.disabled=!0,d.textContent=t("erp-exc-retrying"));try{const v=await fetch("/api/erp/logs/"+encodeURIComponent(c)+"/retry",{method:"POST",headers:{Authorization:"Bearer "+a()}}),w=await v.json().catch(()=>({}));showToast(v.ok&&w.ok?t("erp-exc-retry-ok"):t("erp-exc-retry-fail"),v.ok&&w.ok?"success":"error")}catch{showToast(t("erp-exc-retry-fail"),"error")}if(e.selected.delete(c),l(!1),typeof window.refreshExcBadge=="function")try{window.refreshExcBadge()}catch{}}}async function u(c){const d=Array.from(e.selected);if(c==="clear"){e.selected.clear(),i();return}if(d.length!==0){if(c==="delete"){if(!await showConfirm(t("erp-exc-batch-delete-confirm",{n:d.length}),{danger:!0}))return;try{const w=await fetch("/api/erp/logs/batch-delete",{method:"POST",headers:{Authorization:"Bearer "+a(),"Content-Type":"application/json"},body:JSON.stringify({log_ids:d.slice(0,200)})}),C=await w.json().catch(()=>({}));showToast(w.ok?t("erp-exc-batch-delete-ok",{n:C.deleted||0}):t("erp-exc-retry-fail"),w.ok?"success":"error")}catch{showToast(t("erp-exc-retry-fail"),"error")}}else if(c==="retry")try{const v=await fetch("/api/erp/logs/batch-retry",{method:"POST",headers:{Authorization:"Bearer "+a(),"Content-Type":"application/json"},body:JSON.stringify({log_ids:d.slice(0,50)})}),w=await v.json().catch(()=>({}));showToast(v.ok?t("erp-exc-batch-retry-ok",{ok:w.succeeded||0,fail:(w.failed||0)+(w.skipped||0)}):t("erp-exc-retry-fail"),v.ok?"success":"error")}catch{showToast(t("erp-exc-retry-fail"),"error")}if(e.selected.clear(),l(!1),typeof window.refreshExcBadge=="function")try{window.refreshExcBadge()}catch{}}}async function l(c){const d=document.getElementById("erp-exc-block");if(!(!d||e.loading)){e.loading=!0;try{if(!Array.isArray(window._erpEndpoints)||!window._erpEndpoints.length)try{const m=await fetch("/api/erp/endpoints",{headers:{Authorization:"Bearer "+a()}});if(m.ok){const h=await m.json();window._erpEndpoints=h&&(h.items||h)||[]}}catch{}const v=new URLSearchParams;e.q&&v.set("q",e.q),e.cat&&v.set("category",e.cat),e.adapter&&v.set("adapter",e.adapter),v.set("limit",String(e.pageSize)),v.set("offset",String(c?e.items.length:0));const w=await fetch("/api/erp/exceptions?"+v.toString(),{headers:{Authorization:"Bearer "+a()}});if(!w.ok){c||(d.hidden=!0);return}const C=await w.json(),E=C.items||[];e.items=c?e.items.concat(E):E,e.total=C.total||0,e.categories=C.categories||{},i()}catch{c||(d.hidden=!0)}finally{e.loading=!1}}}let p={};function f(){const c=document.getElementById("erp-exc-modal");c&&c.remove()}window._erpExcOpenEdit=function(c){const d=(e.items||[]).find(L=>String(L.id)===String(c));if(!d)return;const v=!!d.history_client_id&&d.category==="customer_mismatch",w=d.category==="product_mismatch"&&!!d.history_id&&!!d.endpoint_id,C=o(d),E=d.state==="needs_action"?"needs":d.state==="retrying"?"retry":"fail",m=(L,D)=>`<div class="erp-exc-m-row"><span class="erp-exc-m-k">${escapeHtml(L)}</span><span class="erp-exc-m-v">${escapeHtml(D||"—")}</span></div>`;let h="";if(v)h=`
                ...
                </div>`;else{const L="erp-exc-edit-hint-"+(d.category||"other");let D=t(L);(!D||D===L)&&(D=C),h=`<div class="erp-exc-m-hint">${escapeHtml(D)}</div>`}const S=document.createElement("div");if(S.id="erp-exc-modal",S.className="erp-exc-modal-overlay",S.innerHTML=`
            <div class="erp-exc-modal" role="dialog" aria-modal="true">
                ...
                <div class="erp-exc-m-body">
                    <div class="erp-exc-m-reason"><span class="erp-exc-state ${E}">${escapeHtml(t("erp-exc-state-"+(d.state||"failed")))}</span> ${escapeHtml(C)}${d.error_code?` <span class="erp-exc-code">${escapeHtml(d.error_code)}</span>`:""}</div>
                ...
                <div class="erp-exc-m-foot">
            ...
            </div>`,document.body.appendChild(S),S.addEventListener("click",L=>{L.target===S&&f()}),document.getElementById("erp-exc-m-close").addEventListener("click",f),document.getElementById("erp-exc-m-cancel").addEventListener("click",f),document.getElementById("erp-exc-m-retry").addEventListener("click",()=>{f(),r(d.id,null)}),v){let L="";const D=document.getElementById("erp-exc-m-bind"),H=document.getElementById("erp-exc-m-custlist"),_=document.getElementById("erp-exc-m-search"),g=(M,P)=>{const R=(P||"").trim().toLowerCase(),B=R?M.filter(j=>(j.code||"").toLowerCase().includes(R)||(j.name||"").toLowerCase().includes(R)):M;if(B.length===0){H.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-pick-empty"))}</div>`;return}H.innerHTML=B.slice(0,100).map(j=>`<div class="erp-exc-m-cust" data-cust-code="${escapeHtml(j.code||"")}">
                        <span class="erp-exc-m-cust-name">${escapeHtml(j.name||"")}</span>
                        <span class="erp-exc-m-cust-code">${escapeHtml(j.code||"")}</span>
                    </div>`).join(""),H.querySelectorAll(".erp-exc-m-cust").forEach(j=>{j.addEventListener("click",()=>{L=j.dataset.custCode||"",H.querySelectorAll(".erp-exc-m-cust").forEach(Y=>Y.classList.remove("sel")),j.classList.add("sel"),D&&(D.disabled=!L)})})},I=async()=>{const M=d.endpoint_id;if(p[M]){g(p[M],"");return}try{const P=await fetch("/api/erp/endpoints/"+encodeURIComponent(M)+"/customers",{headers:{Authorization:"Bearer "+a()}}),R=await P.json().catch(()=>({}));if(!P.ok||!R.ok){H.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-pick-fail"))}</div>`;return}const B=R.customers||[];p[M]=B,g(B,"")}catch{H.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-pick-fail"))}</div>`}};_&&_.addEventListener("input",()=>g(p[d.endpoint_id]||[],_.value)),I(),D&&D.addEventListener("click",async()=>{if(L){D.disabled=!0,D.textContent=t("erp-exc-retrying");try{if(!(await fetch("/api/erp/mappings/clients",{method:"POST",headers:{Authorization:"Bearer "+a(),"Content-Type":"application/json"},body:JSON.stringify({client_id:d.history_client_id,erp_type:d.endpoint_adapter,erp_code:L})})).ok){showToast(t("erp-exc-retry-fail"),"error"),D.disabled=!1,D.textContent=t("erp-exc-edit-bind-retry");return}showToast(t("erp-exc-edit-bound-ok"),"success"),f(),await r(d.id,null)}catch{showToast(t("erp-exc-retry-fail"),"error"),D.disabled=!1,D.textContent=t("erp-exc-edit-bind-retry")}}})}if(w){const L=document.getElementById("erp-exc-m-bind-prod"),D=document.getElementById("erp-exc-m-prodlist"),H={};let _=[];const g=()=>'<option value="">'+escapeHtml(t("erp-exc-edit-prod-choose"))+"</option>"+_.slice(0,500).map(P=>`<option value="${escapeHtml(P.code||"")}" data-pname="${escapeHtml(P.name||"")}">`+escapeHtml((P.name||"")+" · "+(P.code||""))+"</option>").join(""),I=P=>{if(!P.length){D.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-prod-noitems"))}</div>`;return}D.innerHTML=P.map(R=>`<div class="erp-exc-m-cust" style="cursor:default">
                    ...
                    </div>`).join(""),D.querySelectorAll(".erp-exc-m-prod-sel").forEach(R=>{R.addEventListener("change",()=>{const B=R.dataset.item,j=R.options[R.selectedIndex];R.value?H[B]={code:R.value,name:j&&j.dataset.pname||""}:delete H[B],L&&(L.disabled=Object.keys(H).length===0)})})};(async()=>{try{const R=await(await fetch("/api/history/"+encodeURIComponent(d.history_id),{headers:{Authorization:"Bearer "+a()}})).json().catch(()=>({})),B=R&&R.pages||[],j=[],Y={};(Array.isArray(B)?B:[]).forEach(ee=>{const oe=ee&&ee.fields&&ee.fields.items||[];(Array.isArray(oe)?oe:[]).forEach($=>{const x=($&&($.name||$.description)||"").trim();x&&!Y[x]&&(Y[x]=1,j.push(x))})});const A=await fetch("/api/erp/endpoints/"+encodeURIComponent(d.endpoint_id)+"/products",{headers:{Authorization:"Bearer "+a()}}),W=await A.json().catch(()=>({}));if(!A.ok||!W.ok){D.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-prod-fail"))}</div>`;return}_=W.products||[],I(j)}catch{D.innerHTML=`<div class="erp-exc-m-empty">${escapeHtml(t("erp-exc-edit-prod-fail"))}</div>`}})(),L&&L.addEventListener("click",async()=>{const P=Object.entries(H);if(P.length){L.disabled=!0,L.textContent=t("erp-exc-retrying");try{for(const[R,B]of P)if(!(await fetch("/api/erp/mappings/products",{method:"POST",headers:{Authorization:"Bearer "+a(),"Content-Type":"application/json"},body:JSON.stringify({erp_type:d.endpoint_adapter,item_name:R,erp_code:B.code,erp_name:B.name})})).ok){showToast(t("erp-exc-retry-fail"),"error"),L.disabled=!1,L.textContent=t("erp-exc-edit-bind-prod-retry");return}showToast(t("erp-exc-edit-prod-bound-ok"),"success"),f(),await r(d.id,null)}catch{showToast(t("erp-exc-retry-fail"),"error"),L.disabled=!1,L.textContent=t("erp-exc-edit-bind-prod-retry")}}})}},window._rerenderErpExceptions=i,window.loadErpExceptions=l,window._erpExcState=e})();(function(){function e(c){try{if(location.search.indexOf("test_center=1")>=0||localStorage.getItem("pearnly_test_mode")==="1"||c&&c.id&&String(c.id)==="468b50c1-5593-4fd6-990d-515ce8085563")return!0}catch{}return!1}window.applyRoleVisibility=function(){var d=window._userInfo,v=!1,w=!0,C=!1,E=!1;d&&(v=typeof canManageTeam=="function"?canManageTeam(d):!!(d.role==="owner"||d.is_super_admin),w=typeof shouldHideMoney=="function"?shouldHideMoney(d):d.role==="member"&&!d.is_super_admin,C=typeof isSuperAdmin=="function"?isSuperAdmin(d):!!d.is_super_admin,E=e(d)),document.querySelectorAll("[data-show-if-team]").forEach(function(h){h.style.display=v?"":"none"}),document.querySelectorAll("[data-show-if-money]").forEach(function(h){h.style.display=w?"none":""}),document.querySelectorAll("[data-show-if-admin]").forEach(function(h){h.style.display=C?"":"none"}),document.querySelectorAll("[data-show-if-test]").forEach(function(h){h.style.display=E?"":"none"});var m=C||E;document.querySelectorAll("[data-show-if-special]").forEach(function(h){h.style.display=m?"":"none"})},window.renderAvatarMenu=function(d){if(d){var v=document.getElementById("avatar-btn"),w=document.getElementById("avatar-popup-name"),C=document.getElementById("avatar-popup-email");if(!(!v||!w||!C)){var E=(d.username||"").trim(),m=E.split("@")[0]||E||"—",h=(E.charAt(0)||"?").toUpperCase(),S=(d.avatar_url||"").trim();if(S){var L=S.replace(/"/g,"&quot;"),D=h.replace(/'/g,"\\'");v.innerHTML='<img src="'+L+'" alt="'+h+`" referrerpolicy="no-referrer" onerror="this.parentNode.textContent='`+D+`'">`}else v.textContent=h;w.textContent=m,C.textContent=E||"—",v.setAttribute("title",E||"")}}};function n(){var c=document.getElementById("avatar-wrap"),d=document.getElementById("avatar-btn"),v=document.getElementById("avatar-popup");if(!c||!d||!v)return;function w(){v.classList.remove("show"),d.setAttribute("aria-expanded","false")}function C(){v.classList.add("show"),d.setAttribute("aria-expanded","true")}d.addEventListener("click",function(E){E.stopPropagation(),v.classList.contains("show")?w():C()}),document.addEventListener("click",function(E){v.classList.contains("show")&&!c.contains(E.target)&&w()}),v.addEventListener("click",function(E){var m=E.target.closest(".avatar-popup-item");if(m){var h=m.dataset.action;switch(w(),h){case"settings":typeof openSettingsModal=="function"?openSettingsModal():typeof routeTo=="function"&&routeTo("settings");break;case"team":typeof openSettingsModal=="function"?openSettingsModal():typeof routeTo=="function"&&routeTo("settings"),setTimeout(function(){typeof switchSettingsTab=="function"&&switchSettingsTab("team")},50);break;case"billing":typeof openSettingsModal=="function"?openSettingsModal():typeof routeTo=="function"&&routeTo("settings"),setTimeout(function(){typeof switchSettingsTab=="function"&&switchSettingsTab("plan")},50);break;case"shortcuts":if(typeof showToast=="function"){var S=typeof t=="function"?t("feature-coming-soon"):"即将上线";showToast(S||"即将上线","info")}break;case"admin":window.location.href="/admin/cost";break;case"test-center":typeof routeTo=="function"&&routeTo("test-center");break;case"help":var L=document.getElementById("help-modal");L&&(L.style.display="flex");break;case"logout":try{localStorage.removeItem("mrpilot_token")}catch{}try{localStorage.removeItem("mrpilot_user")}catch{}window.location.href="/";break}}}),window._closeAvatarPopup=w}function a(){return[].slice.call(document.querySelectorAll(".cmdk-item")).filter(function(c){return c.style.display!=="none"})}function o(c){var d=a();d.forEach(function(v){v.classList.remove("focus")}),d[c]&&(d[c].classList.add("focus"),d[c].scrollIntoView({block:"nearest"}))}function s(c){var d=a();if(d.length){var v=d.findIndex(function(C){return C.classList.contains("focus")});v<0&&(v=0);var w=(v+c+d.length)%d.length;o(w)}}function i(c){c=(c||"").toLowerCase().trim();var d=0,v=window._userInfo,w=typeof isSuperAdmin=="function"?isSuperAdmin(v):!!(v&&v.is_super_admin),C=e(v);document.querySelectorAll(".cmdk-item").forEach(function(m){if(m.dataset.showIfAdmin==="1"&&!w){m.style.display="none";return}if(m.dataset.showIfTest==="1"&&!C){m.style.display="none";return}var h=(m.dataset.cmdkText||m.textContent||"").toLowerCase(),S=!c||h.indexOf(c)>=0;m.style.display=S?"":"none",m.classList.remove("focus"),S&&d++}),document.querySelectorAll("[data-cmdk-section]").forEach(function(m){for(var h=m.nextElementSibling,S=!1;h&&!h.hasAttribute("data-cmdk-section");){if(h.classList&&h.classList.contains("cmdk-item")&&h.style.display!=="none"){S=!0;break}h=h.nextElementSibling}m.style.display=S?"":"none"});var E=document.getElementById("cmdk-empty");E&&(E.style.display=d===0?"flex":"none"),o(0)}window.openCmdk=function(){var d=document.getElementById("cmdk-mask");d&&(typeof window._closeAvatarPopup=="function"&&window._closeAvatarPopup(),d.classList.add("show"),typeof window.applyRoleVisibility=="function"&&window.applyRoleVisibility(),setTimeout(function(){var v=document.getElementById("cmdk-input");v&&(v.value="",i(""),v.focus(),o(0))},50))},window.closeCmdk=function(){var d=document.getElementById("cmdk-mask");d&&d.classList.remove("show")};function r(c){if(c){if(c.classList.contains("cmdk-item-locked")){if(typeof showToast=="function"){var d=typeof t=="function"?t("feature-coming-soon"):"即将上线";showToast(d||"即将上线","info")}return}var v=c.dataset.cmdkRoute,w=c.dataset.cmdkAction;if(window.closeCmdk(),v){typeof routeTo=="function"&&routeTo(v);return}if(w){if(w==="open-admin"){window.location.href="/admin/cost";return}if(w.indexOf("lang-")===0){var C=w.slice(5);typeof applyLang=="function"&&applyLang(C)}}}}function u(){var c=document.getElementById("cmdk-mask"),d=document.getElementById("cmdk-input"),v=document.getElementById("cmdk-body");if(!(!c||!d||!v)){c.addEventListener("click",function(E){E.target===c&&window.closeCmdk()});var w=document.getElementById("cmdk-esc-btn");w&&w.addEventListener("click",function(){window.closeCmdk()}),d.addEventListener("input",function(E){i(E.target.value)}),d.addEventListener("keydown",function(E){E.key==="ArrowDown"?(E.preventDefault(),s(1)):E.key==="ArrowUp"?(E.preventDefault(),s(-1)):E.key==="Enter"?(E.preventDefault(),r(c.querySelector(".cmdk-item.focus"))):E.key==="Escape"&&(E.preventDefault(),window.closeCmdk())}),v.addEventListener("click",function(E){var m=E.target.closest(".cmdk-item");m&&r(m)}),v.addEventListener("mousemove",function(E){var m=E.target.closest(".cmdk-item");!m||m.style.display==="none"||m.classList.contains("cmdk-item-locked")||(a().forEach(function(h){h.classList.remove("focus")}),m.classList.add("focus"))});var C=document.getElementById("topbar-search");C&&(C.addEventListener("click",function(){window.openCmdk()}),C.addEventListener("keydown",function(E){(E.key==="Enter"||E.key===" ")&&(E.preventDefault(),window.openCmdk())}))}}document.addEventListener("keydown",function(c){if((c.metaKey||c.ctrlKey)&&(c.key==="k"||c.key==="K")){c.preventDefault(),window.openCmdk();return}if(c.key==="Escape"){var d=document.getElementById("cmdk-mask");if(d&&d.classList.contains("show")){window.closeCmdk();return}var v=document.getElementById("avatar-popup");v&&v.classList.contains("show")&&typeof window._closeAvatarPopup=="function"&&window._closeAvatarPopup()}});try{var l=(navigator.userAgent||"").toLowerCase(),p=l.indexOf("mac")>=0||l.indexOf("iphone")>=0||l.indexOf("ipad")>=0;p||document.body.classList.add("is-windows")}catch{}function f(){n(),u(),typeof window.subscribeI18n=="function"&&window.subscribeI18n("nav-ia-phase1-role",function(){try{typeof window.applyRoleVisibility=="function"&&window.applyRoleVisibility()}catch{}})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",f):f()})();(function(){function n(w){return String(w??"").replace(/[&<>"']/g,function(C){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[C]})}function a(w){if(!w||isNaN(w))return"";var C=Number(w);return C<1024?C+" B":C<1024*1024?(C/1024).toFixed(1)+" KB":(C/1024/1024).toFixed(1)+" MB"}document.addEventListener("click",function(w){var C=w.target.closest&&w.target.closest(".recon-collapse-head");if(C&&!(w.target.closest("button")||w.target.closest("a"))){var E=C.closest(".recon-collapse");if(E){var m=E.getAttribute("data-collapsed")==="true";E.setAttribute("data-collapsed",m?"false":"true"),m&&(E.id==="vex-summary-collapse"&&f(),E.id==="vex-detail-collapse"&&c())}}}),document.addEventListener("keydown",function(w){if(!(w.key!=="Enter"&&w.key!==" ")){var C=w.target.closest&&w.target.closest(".recon-collapse-head");C&&(w.preventDefault(),C.click())}});var o={vat:"",gl:""};window._glvClearPreviewSearch=function(){o.vat="",o.gl=""};var s='<svg class="vex-pp-fi-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#111111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>',i='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';function r(){l("vat"),l("gl")}function u(w){try{if(typeof window._glvPreviewFiles=="function")return window._glvPreviewFiles(w)||[]}catch{}var C=document.getElementById(w==="vat"?"glv-vat-input":"glv-gl-input");return C&&C.files?Array.from(C.files):[]}function l(w){var C=document.getElementById(w==="vat"?"glv-pp-vat-col":"glv-pp-gl-col");if(C){var E=u(w),m=w==="vat"?"glv-up-vat-title":"glv-up-gl-title",h=w==="vat"?"① 销项税报告":"② 总账 GL",S=window.t&&window.t(m)||h,L=n(window.t&&window.t("vex-preview-search")||"搜索文件名..."),D=n(window.t&&window.t("vex-preview-clear-all")||"全清"),H=o[w]||"",_=E.length;C.innerHTML='<div class="vex-pp-col-title"><span class="vex-pp-col-name">'+n(S)+' <span class="vex-pp-col-count">'+_+'</span></span></div><div class="vex-pp-search-row"><input class="vex-pp-search" id="glv-pp-search-'+w+'" type="text" placeholder="'+L+'" value="'+n(H)+'" autocomplete="off"><button class="vex-pp-clear-btn" id="glv-pp-clearall-'+w+'" type="button">'+D+'</button></div><div class="vex-pp-file-list" id="glv-pp-'+w+'-list"></div><div class="vex-pp-pagination" id="glv-pp-'+w+'-pg"></div>';var g=document.getElementById("glv-pp-search-"+w);g&&g.addEventListener("input",function(M){o[w]=M.target.value,p(w)});var I=document.getElementById("glv-pp-clearall-"+w);I&&I.addEventListener("click",function(){window._glvRemoveFile&&window._glvRemoveFile(w)}),p(w)}}function p(w){var C=document.getElementById("glv-pp-"+w+"-list"),E=document.getElementById("glv-pp-"+w+"-pg");if(C){var m=u(w),h=(o[w]||"").toLowerCase(),S=m.map(function(H,_){return{f:H,i:_}}),L=h?S.filter(function(H){return H.f.name.toLowerCase().indexOf(h)>=0}):S;if(C.innerHTML=L.map(function(H){var _=H.f;return'<div class="vex-pp-file-row">'+s+'<span class="vex-pp-fi-name" title="'+n(_.name)+'">'+n(_.name)+'</span><span class="vex-pp-fi-size">'+a(_.size)+'</span><button class="vex-pp-fi-del" type="button" data-kind="'+w+'" data-idx="'+H.i+'" aria-label="remove">'+i+"</button></div>"}).join(""),C.querySelectorAll(".vex-pp-fi-del").forEach(function(H){H.addEventListener("click",function(){var _=H.dataset.kind,g=parseInt(H.dataset.idx,10);window._glvRemoveFile&&window._glvRemoveFile(_,isNaN(g)?null:g)})}),E){var D=window.t&&window.t("vex-preview-count")||"显示前 {n} / 共 {m}";E.textContent=D.replace("{n}",L.length).replace("{m}",L.length)}}}function f(){var w=function(E,m){var h=document.getElementById(E);h&&(h.textContent=m==null?"—":String(m))},C=window._vexLastTask||{};w("vex-sum-total",C.total),w("vex-sum-matched",C.matched),w("vex-sum-diff",C.diff),w("vex-sum-incomplete",C.incomplete),w("vex-sum-cash",C.cash),document.getElementById("vex-summary-sub")}function c(){var w=window._vexLastTask&&window._vexLastTask.diff_rows||[],C=document.getElementById("vex-detail-tbody"),E=document.getElementById("vex-detail-table"),m=document.getElementById("vex-detail-empty");if(!(!C||!E||!m)){if(w.length===0){E.style.display="none",m.style.display="";return}m.style.display="none",E.style.display="";var h=w.map(function(L){return'<tr><td class="recon-detail-cell-mono">'+n(L.invoice_no||"")+"</td><td>"+n(L.field||"")+"</td><td>"+n(L.report_value||"")+"</td><td>"+n(L.invoice_value||"")+"</td><td>"+n(L.kind||"")+"</td></tr>"}).join("");C.innerHTML=h;var S=document.getElementById("vex-detail-sub");S&&(S.textContent=String(w.length))}}function d(){var w=document.getElementById("glv-toggle-preview");w&&!w._reconBound&&(w._reconBound=!0,w.addEventListener("click",function(){var C=document.getElementById("glv-preview-panel"),E=document.getElementById("glv-toggle-preview-label"),m=C&&C.style.display!=="none";C&&(C.style.display=m?"none":""),w.classList.toggle("open",!m),E&&(E.textContent=m?window.t&&window.t("vex-toggle-preview-open")||"查看清单":window.t&&window.t("vex-toggle-preview-close")||"收起清单"),m||r()})),["glv-vat-input","glv-gl-input"].forEach(function(C){var E=document.getElementById(C);!E||E._reconWatched||(E._reconWatched=!0,E.addEventListener("change",function(){var m=document.getElementById("glv-preview-panel");m&&m.style.display!=="none"&&r()}))})}function v(){var w=document.getElementById("vex-summary-collapse"),C=document.getElementById("vex-detail-collapse");w&&(w.style.display=""),C&&(C.style.display=""),f(),c()}window._fillVexSummary=f,window._fillVexDetail=c,window._onVexResultShown=v,document.addEventListener("DOMContentLoaded",function(){d()}),setTimeout(d,1500),typeof window.subscribeI18n=="function"&&window.subscribeI18n("recon-collapse",function(){var w=document.getElementById("glv-preview-panel");w&&w.style.display!=="none"&&r();var C=document.getElementById("glv-toggle-preview-label"),E=document.getElementById("glv-toggle-preview");C&&E&&(C.textContent=E.classList.contains("open")?window.t&&window.t("vex-toggle-preview-close")||"收起清单":window.t&&window.t("vex-toggle-preview-open")||"查看清单")}),window._reconCollapse={renderGlvPreview:r,fillVexSummary:f,fillVexDetail:c}})();(function(){function e(i){}function n(){const i=document.querySelectorAll("[data-recon-tab]");i.forEach(u=>{u.addEventListener("click",()=>{i.forEach(d=>d.classList.remove("active")),u.classList.add("active");const l=u.dataset.reconTab,p=document.getElementById("recon-pane-bank"),f=document.getElementById("recon-pane-sale-vat"),c=document.getElementById("recon-pane-gl-vat");p&&(p.style.display=l==="bank"?"":"none"),f&&(f.style.display=l==="sale-vat"?"":"none"),c&&(c.style.display=l==="gl-vat"?"":"none"),l==="gl-vat"&&window.GlVatRecon&&window.GlVatRecon.ensureInit(),l==="bank"&&typeof window._bankReconV2Init=="function"&&window._bankReconV2Init()})});const r=document.querySelector("[data-recon-tab].active");r&&(r.dataset.reconTab,void 0)}function a(){const i=document.getElementById("page-settings");if(!i)return null;let r=document.getElementById("settings-modal-overlay");if(r)return r;r=document.createElement("div"),r.id="settings-modal-overlay",r.className="settings-modal-overlay",r.style.display="none",i.parentElement.insertBefore(r,i),r.appendChild(i);const u=document.createElement("button");return u.id="settings-modal-close",u.className="settings-modal-close",u.setAttribute("aria-label","close"),u.innerHTML='<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 5l10 10M15 5L5 15"/></svg>',i.insertBefore(u,i.firstChild),u.addEventListener("click",s),r.addEventListener("click",l=>{l.target===r&&s()}),r}function o(){const i=a();if(!i)return;i.style.display="flex",document.body.classList.add("settings-modal-open");const r=document.getElementById("page-settings");r&&(r.style.display="block"),setTimeout(()=>{try{typeof renderSettings=="function"&&renderSettings()}catch(l){console.warn("renderSettings:",l)}let u=document.querySelector(".settings-tab.active")||document.querySelector('.settings-tab[data-tab="profile"]');u&&u.click()},50)}function s(){const i=document.getElementById("settings-modal-overlay");i&&(i.style.display="none"),document.body.classList.remove("settings-modal-open")}window.openSettingsModal=o,window.closeSettingsModal=s,document.addEventListener("keydown",i=>{if(i.key==="Escape"){const r=document.getElementById("settings-modal-overlay");r&&r.style.display==="flex"&&s()}}),window.addEventListener("hashchange",()=>{location.hash==="#/settings"&&o()}),window.addEventListener("DOMContentLoaded",()=>{location.hash==="#/settings"&&o()}),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",n):n()})();(function(){const a=/\.(pdf|jpe?g|png|webp|tiff?|xlsx?|xlsm|csv|tsv|docx?)$/i,o=U=>document.getElementById(U);function s(){return{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}}function i(U){return String(U??"").replace(/[&<>"']/g,Q=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[Q])}function r(U){return U<1024?U+" B":U<1024*1024?(U/1024).toFixed(1)+" KB":(U/1024/1024).toFixed(1)+" MB"}let u=[],l=[],p=!1,f=[],c=50,d=50,v="",w="";async function C(){try{const U=await fetch("/api/vat_excel/tasks?page=1&page_size=1",{headers:s()});if(!U.ok)return;const te=(await U.json()).kpi||{};[["vex-kpi-month-val",te.this_month],["vex-kpi-running-val",te.running],["vex-kpi-done-val",te.done],["vex-kpi-failed-val",te.failed]].forEach(([ne,se])=>{const ue=document.getElementById(ne);ue&&(ue.textContent=se??0)})}catch{}}async function E(){try{const U=await fetch("/api/vat_excel/tasks?page=1&page_size=100",{headers:s()});if(!U.ok)return;const Q=await U.json();L(Q.rows||[])}catch{}}const m=10;var h=1;function S(){var U=((document.getElementById("vex-task-search")||{}).value||"").trim().toLowerCase();if(h=1,L(f),!!U){var Q=document.getElementById("vex-task-tbody");Q&&Q.querySelectorAll("tr").forEach(function(te){te.dataset.taskId&&(te.style.display=te.textContent.toLowerCase().indexOf(U)>=0?"":"none")})}}function L(U){f=U||f;const Q=document.getElementById("vex-task-tbody");if(!Q)return;if(!f.length){Q.innerHTML='<tr><td colspan="9" class="vex-task-empty">'+(t("sv-empty-title")||"还没有对账任务")+"</td></tr>",D(0);return}const te=Math.ceil(f.length/m);h>te&&(h=te);const ne=(h-1)*m;H(f.slice(ne,ne+m)),D(f.length)}function D(U){const Q=document.getElementById("vex-task-pager"),te=document.getElementById("vex-task-pager-info"),ne=document.getElementById("vex-task-prev"),se=document.getElementById("vex-task-next");if(!Q)return;if(U<=m){Q.style.display="none";return}Q.style.display="";const ue=Math.ceil(U/m);te&&(te.textContent=h+" / "+ue),ne&&(ne.disabled=h<=1),se&&(se.disabled=h>=ue)}function H(U){const Q=document.getElementById("vex-task-tbody");if(!Q)return;const te={pending:t("vex-status-pending")||"待处理",running:t("vex-status-running")||"处理中",done:t("vex-status-done")||"已完成",failed:t("vex-status-failed")||"失败"},ne={pending:"badge-gray",running:"badge-blue",done:"badge-green",failed:"badge-red"},se='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v9M4 7l4 4 4-4M3 14h10"/></svg>',ue='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 4h10M6 4V3h4v1M5 4v8a1 1 0 001 1h4a1 1 0 001-1V4"/></svg>';Q.innerHTML=U.map(k=>{const T=k.created_at?new Date(k.created_at).toLocaleString([],{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}):"—",q=k.period||"—",G=k.matched_count!=null?k.matched_count+" ✓ · "+k.mismatched_count+" ⚠":"—",O=k.mismatch_amount!=null?"฿ "+Number(k.mismatch_amount).toLocaleString():"—",y=k.elapsed_seconds!=null?k.elapsed_seconds.toFixed(1)+" s":"—",N=k.status||"pending",V=k.client_name&&k.client_name!=="client"?k.client_name:t("vex-client-all")||"全部客户";return`<tr class="vex-task-row" data-task-id="${i(k.id)}" style="cursor:pointer">
                <td>${T}</td>
                <td>${i(V)}</td>
                <td>${i(q)}</td>
            ...
            </tr>`}).join(""),Q.querySelectorAll(".vex-task-dl-btn").forEach(k=>{k.addEventListener("click",async T=>{T.stopPropagation();const q=k.dataset.taskId;try{const G=await fetch("/api/vat_excel/tasks/"+encodeURIComponent(q)+"/download",{credentials:"include",headers:s()});if(G.status===410){showToast(t("vex-toast-expired")||"数据已过期 · 请重新对账","warn");return}if(!G.ok){showToast(t("vex-toast-dl-fail")||"下载失败 · 请重试","error");return}const O=await G.blob(),N=(G.headers.get("Content-Disposition")||"").match(/filename\*?=(?:UTF-8''|")?([^";]+)/i),V=N?decodeURIComponent(N[1]):"vat_recon_"+q+".xlsx",J=URL.createObjectURL(O),X=document.createElement("a");X.href=J,X.download=V,X.click(),setTimeout(()=>URL.revokeObjectURL(J),1e3)}catch{showToast(t("vex-toast-dl-fail")||"下载失败 · 请重试","error")}})}),Q.querySelectorAll(".vex-task-del-btn").forEach(k=>{k.addEventListener("click",T=>{T.stopPropagation(),g(k.dataset.taskId)})}),S()}function _(){var U=document.getElementById("vex-task-prev"),Q=document.getElementById("vex-task-next");U&&!U._vexBound&&(U._vexBound=!0,U.addEventListener("click",function(){h>1&&(h--,L())})),Q&&!Q._vexBound&&(Q._vexBound=!0,Q.addEventListener("click",function(){var te=Math.ceil(f.length/m);h<te&&(h++,L())}))}async function g(U){const Q=t("vex-task-delete-confirm-title")||"删除对账任务?",te=t("vex-task-delete-confirm-body")||"同时清掉对应的发票识别缓存 · 不可恢复";if(await showConfirm(te,{title:Q,danger:!0,okText:t("vex-task-delete-confirm-title")||"确认删除"}))try{const se=await fetch("/api/vat_excel/tasks/"+encodeURIComponent(U),{method:"DELETE",credentials:"include",headers:s()});if(!se.ok)throw new Error(se.status);showToast(t("vex-task-delete-ok")||"已删除","success"),E(),C()}catch{showToast(t("vex-task-delete-fail")||"删除失败","error")}}function I(U){const Q=window._currentLang||"th",te={zh:`已忽略 ${U} 个不支持的文件 · 仅支持 PDF / 图片 / Excel / CSV / Word`,th:`ข้ามไฟล์ที่ไม่รองรับ ${U} ไฟล์ · รองรับเฉพาะ PDF / รูปภาพ / Excel / CSV / Word`,en:`Ignored ${U} unsupported file(s) · only PDF / image / Excel / CSV / Word are supported`,ja:`非対応ファイル ${U} 件をスキップ · 対応形式は PDF / 画像 / Excel / CSV / Word のみ`};showToast(te[Q]||te.th,"warn")}function M(U){const Q=new Set(u.map(ne=>ne.name+"|"+ne.size));let te=0;for(const ne of U){if(!a.test(ne.name)){te++;continue}const se=ne.name+"|"+ne.size;if(!Q.has(se)&&(Q.add(se),u.push(ne),u.length>=1e3))break}te>0&&I(te),u.length>1e3&&(u=u.slice(0,1e3),showToast(t("vex-toast-cap-inv"),"warn")),j()}function P(U){const Q=new Set(l.map(ne=>ne.name+"|"+ne.size));let te=0;for(const ne of U){if(!a.test(ne.name)){te++;continue}const se=ne.name+"|"+ne.size;if(!Q.has(se)&&(Q.add(se),l.push(ne),l.length>=30))break}te>0&&I(te),l.length>30&&(l=l.slice(0,30),showToast(t("vex-toast-cap-rep"),"warn")),j()}function R(U){u.splice(U,1),j()}function B(U){l.splice(U,1),j()}function j(){const U=o("vex-list-invoice"),Q=o("vex-list-report"),te=o("vex-count-invoice"),ne=o("vex-count-report");te&&(te.textContent=u.length),ne&&(ne.textContent=l.length);const se=(T,q,G)=>`<div class="vex-fi">
            ...
            <button class="vex-fi-x" type="button" data-vex-kind="${G}" data-vex-idx="${q}" aria-label="remove">×</button>
        </div>`;U&&(U.innerHTML=u.map((T,q)=>se(T,q,"inv")).join("")||'<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>'),Q&&(Q.innerHTML=l.map((T,q)=>se(T,q,"rep")).join("")||'<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>'),document.querySelectorAll(".vex-fi-x").forEach(T=>{T.addEventListener("click",q=>{const G=T.dataset.vexKind,O=parseInt(T.dataset.vexIdx,10);G==="inv"?R(O):B(O)})});const ue=u.length>0&&l.length>0;o("vex-build").disabled=!ue||p;const k=o("vex-action-info");k&&(!u.length||!l.length?(k.textContent=t("vex-need-both")||"需要至少 1 张发票 + 1 份 VAT 报告",k.className="vex-action-info muted"):(k.textContent=(t("vex-ready")||"已就绪 · {a} 张发票 · {b} 份报告").replace("{a}",u.length).replace("{b}",l.length),k.className="vex-action-info ok")),ee()}const Y='<svg class="vex-pp-fi-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6B7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>',A='<svg class="vex-pp-fi-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#111111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>',W='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';function ee(){const U=o("vex-preview-panel");if(!U||U.style.display==="none")return;oe("inv"),oe("rep");const Q=o("vex-pp-guide");Q&&(Q.style.display=u.length>100?"flex":"none")}function oe(U){const Q=o(U==="inv"?"vex-pp-invoice-col":"vex-pp-report-col");if(!Q)return;const te=U==="inv"?u:l,ne=U==="inv"?v:w,se=t(U==="inv"?"vex-preview-invoice":"vex-preview-report")||(U==="inv"?"销售发票":"VAT 报告"),ue=i(t("vex-preview-search")||"搜索文件名..."),k=i(t("vex-preview-clear-all")||"全清");Q.innerHTML=`
            ...
            <div class="vex-pp-search-row">
                ...
                <button class="vex-pp-clear-btn" id="vex-pp-clearall-${U}" type="button">${k}</button>
            </div>
            <div class="vex-pp-file-list" id="vex-pp-${U}-list"></div>
            <div class="vex-pp-pagination" id="vex-pp-${U}-pg"></div>`;const T=o("vex-pp-search-"+U);T&&T.addEventListener("input",G=>{U==="inv"?(v=G.target.value,c=50):(w=G.target.value,d=50),$(U)});const q=o("vex-pp-clearall-"+U);q&&q.addEventListener("click",()=>{U==="inv"?(u=[],v="",c=50):(l=[],w="",d=50),j()}),$(U)}function $(U){const Q=o("vex-pp-"+U+"-list"),te=o("vex-pp-"+U+"-pg");if(!Q)return;const ne=U==="inv"?u:l,se=U==="inv"?v:w,ue=U==="inv"?c:d,k=U==="inv"?Y:A,T=ne.map((O,y)=>({f:O,i:y})),q=se?T.filter(({f:O})=>O.name.toLowerCase().includes(se.toLowerCase())):T,G=q.slice(0,ue);if(Q.innerHTML=G.map(({f:O,i:y})=>`
            ...
            </div>`).join("")+`<div id="vex-pp-sentinel-${U}" style="height:1px;flex-shrink:0"></div>`,Q.querySelectorAll(".vex-pp-fi-del").forEach(O=>{O.addEventListener("click",()=>{const y=parseInt(O.dataset.ridx,10);O.dataset.kind==="inv"?R(y):B(y)})}),te){const O=t("vex-preview-count")||"显示前 {n} / 共 {m}";te.textContent=O.replace("{n}",G.length).replace("{m}",q.length)}x(U,q.length)}function x(U,Q){if((U==="inv"?c:d)>=Q)return;const ne=o("vex-pp-sentinel-"+U),se=o("vex-pp-"+U+"-list");if(!ne||!se)return;const ue=new IntersectionObserver(k=>{k[0].isIntersecting&&(ue.disconnect(),U==="inv"?c+=50:d+=50,$(U))},{root:se,threshold:.8});ue.observe(ne)}function b(U,Q,te,ne){const se=o(U),ue=o(Q);!se||!ue||(se.addEventListener("click",()=>ue.click()),se.addEventListener("keydown",k=>{(k.key==="Enter"||k.key===" ")&&(k.preventDefault(),ue.click())}),se.addEventListener("dragover",k=>{k.preventDefault(),se.classList.add("drag-over")}),se.addEventListener("dragleave",()=>se.classList.remove("drag-over")),se.addEventListener("drop",k=>{k.preventDefault(),se.classList.remove("drag-over");const q=Array.from(k.dataTransfer.files).filter(G=>a.test(G.name));if(!q.length){showToast(t("vex-toast-bad-ext"),"error");return}te(q)}),ue.addEventListener("change",()=>{const k=Array.from(ue.files);te(k),ue.value=""}))}async function z(){if(p||!u.length||!l.length)return;p=!0,o("vex-build").disabled=!0,o("vex-progress").style.display="flex";var U=document.getElementById("vex-download");U&&(U.style.display="none"),["vex-summary-collapse","vex-detail-collapse"].forEach(function(se){var ue=document.getElementById(se);ue&&(ue.style.display="none")});const Q=Date.now();o("vex-progress-title").textContent=t("vex-progress-running")||"AI 抽取中",o("vex-progress-sub").textContent=(t("vex-progress-sub")||"{a} 张发票 + {b} 份报告 · 并行处理").replace("{a}",u.length).replace("{b}",l.length);const te=setInterval(()=>{const se=Math.floor((Date.now()-Q)/1e3);o("vex-progress-sub").textContent=(t("vex-progress-elapsed")||"已 {s} 秒 · {a} 张发票 + {b} 份报告").replace("{s}",se).replace("{a}",u.length).replace("{b}",l.length)},1e3);try{const se=new FormData;for(const pe of u)se.append("invoices",pe);for(const pe of l)se.append("reports",pe);const ue=typeof currentLang=="string"&&currentLang||localStorage.getItem("mrpilot_lang")||"th";se.append("lang",ue);const k=localStorage.getItem("mrpilot_token")||"",T=await fetch("/api/vat_excel/submit",{method:"POST",headers:s(),body:se});let q=null;try{q=await T.json()}catch{q=null}if(!T.ok||!q||!q.ok||!q.job_id)throw clearInterval(te),new Error(q&&q.detail||"HTTP "+T.status);const G=o("vex-progress-sub"),O=await window._reconPollJob(q.job_id,k,{onProgress:pe=>{G&&(G.textContent=window._reconProgressText(pe,ue))}});if(clearInterval(te),!O||O.status!=="done"||!O.result_id)throw new Error(t("vex-toast-fail")||"生成失败");const y=O.result_id;let N=0;const V=await fetch("/api/vat_excel/tasks/"+encodeURIComponent(y)+"/download",{headers:s()});if(!V.ok)throw new Error("HTTP "+V.status);const X=(V.headers.get("Content-Disposition")||"").match(/filename="([^"]+)"/),de=X&&X[1]||"vat_recon_"+Date.now()+".xlsx",ie=await V.blob(),ce=URL.createObjectURL(ie),ae=o("vex-download");ae.href=ce,ae.download=de;try{const pe=document.createElement("a");pe.href=ce,pe.download=de,document.body.appendChild(pe),pe.click(),setTimeout(()=>pe.remove(),100)}catch{}o("vex-progress").style.display="none";var ne=document.getElementById("vex-download");ne&&(ne.style.display=""),y&&(N=await Z(y)),window._onVexResultShown&&window._onVexResultShown(),N>0?showToast((t("vex-toast-some-fail")||"有 {n} 张发票 OCR 失败").replace("{n}",N),"warn"):showToast(t("vex-toast-done")||"Excel 已生成","success"),C(),setTimeout(E,800)}catch(se){clearInterval(te),o("vex-progress").style.display="none";const ue=(t("vex-toast-fail")||"生成失败")+": "+(se.message||se);showToast(ue,"error")}finally{p=!1,o("vex-build").disabled=!1}}function F(){u=[],l=[];var U=document.getElementById("vex-download");U&&(U.style.display="none"),j()}function K(U){if(U==null)return"—";var Q=parseFloat(U);return isNaN(Q)?"—":Q.toLocaleString("th-TH",{minimumFractionDigits:2,maximumFractionDigits:2})}async function Z(U){try{var Q=await fetch("/api/vat_excel/tasks/"+encodeURIComponent(U),{headers:s()});if(!Q.ok)throw new Error(Q.status);var te=await Q.json(),ne=te.raw_data_json;if(typeof ne=="string")try{ne=JSON.parse(ne)}catch{ne={}}ne=ne||{};var se=ne.rows||[],ue=[];se.forEach(function(q){q.kind==="invoice_orphan"?ue.push({invoice_no:q.invoice_no||"",field:"仅发票有",report_value:"—",invoice_value:K(q.amount_inv),kind:q.kind}):q.kind==="report_orphan"?ue.push({invoice_no:q.invoice_no||"",field:"仅报告有",report_value:K(q.amount_rep),invoice_value:"—",kind:q.kind}):q.dims&&Object.keys(q.dims).length>0&&Object.keys(q.dims).forEach(function(G){var O=String(q.dims[G]||""),y=O.split(" ≠ ");ue.push({invoice_no:q.invoice_no||"",field:G,report_value:y[0]||O,invoice_value:y.length>1?y[1]:"—",kind:"diff"})})});var k=se.filter(function(q){return q.kind==="matched_cash"}).length,T=Math.max(0,parseInt(ne.invoice_ocr_failed_count||0,10));return window._vexLastTask={total:ne.n_total||0,matched:ne.n_ok||0,diff:ne.n_diff||0,incomplete:T,cash:k,diff_rows:ue,task_id:U},window._fillVexSummary&&window._fillVexSummary(),window._fillVexDetail&&window._fillVexDetail(),T}catch{return 0}}function le(){const U=document.getElementById("vex-pane");U&&U.querySelectorAll("[data-i18n]").forEach(Q=>{const te=t(Q.dataset.i18n);te&&(Q.textContent=te)}),j(),E()}function re(){b("vex-drop-invoice","vex-input-invoice",M),b("vex-drop-report","vex-input-report",P);const U=o("vex-build"),Q=o("vex-reset");U&&U.addEventListener("click",z),Q&&Q.addEventListener("click",F),document.querySelectorAll('[data-recon-tab="sale-vat"]').forEach(se=>{se.addEventListener("click",()=>{C(),E()})}),_();const te=document.getElementById("vex-task-search");te&&te.addEventListener("input",S);const ne=document.getElementById("vex-toggle-preview");ne&&ne.addEventListener("click",()=>{const se=o("vex-preview-panel"),ue=o("vex-toggle-preview-label"),k=se&&se.style.display!=="none";se&&(se.style.display=k?"none":""),ne&&ne.classList.toggle("open",!k),ue&&(ue.textContent=k?t("vex-toggle-preview-open")||"查看清单":t("vex-toggle-preview-close")||"收起清单"),k||ee()}),j(),C()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",re):re(),typeof window.subscribeI18n=="function"&&(window.subscribeI18n("vex-excel",le),window.subscribeI18n("vex-preview-panel",ee))})();(function(){const e=x=>document.getElementById(x),n=()=>localStorage.getItem("mrpilot_token")||"",a=()=>typeof window.currentLang=="string"&&window.currentLang?window.currentLang:localStorage.getItem("mrpilot_lang")||"th",o=()=>({Authorization:"Bearer "+n()}),s={inited:!1,glFile:[],vatFile:[],running:!1,currentTaskId:null,lastDetail:[],lastSummary:null},i={th:{not_found:"ไม่พบข้อมูล",running:"กำลังกระทบยอด…",error:"เกิดข้อผิดพลาด",need_files:"กรุณาเลือกไฟล์ทั้งสอง",done:"เสร็จสิ้น",hint_need_both:"อัปโหลด ① รายงานภาษีขาย + ② GL",hint_need_one_more:"อัปโหลดอีก 1 ไฟล์",hint_ready:"พร้อมแล้ว · กดเริ่มกระทบยอด",hist_load:"โหลด",hist_export:"ส่งออก",hist_delete:"ลบ",confirm_delete:"ยืนยันการลบงานนี้?",s_gl_total:"ยอดรวมตามบัญชีแยกประเภท",s_minus_gl_cr:"หัก : รายการเครดิตที่ไม่มีในรายงานภาษีขาย",s_plus_gl_dr:"บวก : รายการเดบิตที่ไม่มีในรายงานภาษีขาย",s_plus_vat_p:"บวก : รายการยอดขายที่ไม่มีในบัญชีแยกประเภท",s_minus_vat_n:"หัก : รายการลดหนี้ที่ไม่มีในบัญชีแยกประเภท",s_vat_total:"ยอดรวมตามรายงานภาษีขาย"},zh:{not_found:"未找到数据",running:"正在对账中...",error:"出错了",need_files:"请先选择两个文件",done:"完成",hint_need_both:"请上传① 销项税报告 + ② 总账 GL",hint_need_one_more:"还需上传 1 份文件",hint_ready:"已就绪 · 点击开始对账",hist_load:"加载",hist_export:"导出",hist_delete:"删除",confirm_delete:"确认删除此任务？",s_gl_total:"总账金额合计",s_minus_gl_cr:"减：销项税报告中未列的贷方记录",s_plus_gl_dr:"加：销项税报告中未列的借方记录",s_plus_vat_p:"加：总账中未列的销售记录",s_minus_vat_n:"减：总账中未列的贷项凭单(credit note)记录",s_vat_total:"销项税报告金额合计"},en:{not_found:"Not found",running:"Reconciling...",error:"Error",need_files:"Please select both files",done:"Done",hint_need_both:"Upload ① Output VAT report + ② GL file",hint_need_one_more:"1 more file required",hint_ready:"Ready · click Run to start",hist_load:"Load",hist_export:"Export",hist_delete:"Delete",confirm_delete:"Delete this task?",s_gl_total:"Total per General Ledger",s_minus_gl_cr:"Less: GL credits not in VAT Report",s_plus_gl_dr:"Add: GL debits not in VAT Report",s_plus_vat_p:"Add: Sales in VAT Report not in GL",s_minus_vat_n:"Less: Credit notes in VAT Report not in GL",s_vat_total:"Total per VAT Sales Report"},ja:{not_found:"データなし",running:"照合中…",error:"エラー",need_files:"両方のファイルを選択してください",done:"完了",hint_need_both:"① 売上税報告 + ② GL をアップロード",hint_need_one_more:"あと 1 ファイル必要",hint_ready:"準備完了 · 「開始」をクリック",hist_load:"読込",hist_export:"出力",hist_delete:"削除",confirm_delete:"このタスクを削除しますか?",s_gl_total:"総勘定元帳合計",s_minus_gl_cr:"減：売上税報告にないGL貸方記録",s_plus_gl_dr:"加：売上税報告にないGL借方記録",s_plus_vat_p:"加：GLにない売上記録",s_minus_vat_n:"減：GLにない赤伝記録",s_vat_total:"売上税報告合計"}},r=x=>(i[a()]||i.th)[x]||x;function u(x){const b=a(),F={gl_no_revenue_rows:{zh:"GL 中未找到收入科目行。请确认「收入科目前缀」是否正确(收入科目通常以 4 开头),改对后重试。",th:"ไม่พบรายการบัญชีรายได้ใน GL · ตรวจสอบ «คำนำหน้าบัญชีรายได้» (รายได้มักขึ้นต้นด้วย 4) แล้วลองใหม่",en:"No revenue-account rows found in the GL. Check the “revenue account prefix” (revenue usually starts with 4) and retry.",ja:"GL に収益科目の行が見つかりません。「収益科目プレフィックス」(通常 4 で始まる)を確認して再試行してください。"},gl_parse_failed:{zh:"GL 文件解析失败。请确认文件含日期/科目/借贷列,或换清晰的 Excel/CSV 重传。",th:"อ่านไฟล์ GL ไม่สำเร็จ · ตรวจสอบว่ามีคอลัมน์ วันที่/บัญชี/เดบิต-เครดิต หรืออัปโหลด Excel/CSV ที่ชัดเจน",en:"Failed to parse the GL file. Ensure it has date/account/debit-credit columns, or re-upload a clean Excel/CSV.",ja:"GL ファイルの解析に失敗しました。日付/科目/借方貸方列を確認するか、Excel/CSV を再アップロードしてください。"},vat_no_rows:{zh:"销项税报告里没有可对账的数据行。请确认上传了正确的销项税报告。",th:"ไม่พบแถวข้อมูลในรายงานภาษีขาย · ตรวจสอบว่าอัปโหลดรายงานที่ถูกต้อง",en:"No rows found in the sales-VAT report. Please check you uploaded the correct report.",ja:"売上VATレポートに行が見つかりません。正しいレポートをアップロードしたか確認してください。"},vat_parse_failed:{zh:"销项税报告解析失败。请换更清晰的版本,或转成 Excel/PDF 重传。",th:"อ่านรายงานภาษีขายไม่สำเร็จ · ลองเวอร์ชันที่ชัดเจนกว่า หรือแปลงเป็น Excel/PDF",en:"Failed to parse the sales-VAT report. Try a clearer version, or convert to Excel/PDF.",ja:"売上VATレポートの解析に失敗しました。より鮮明な版か、Excel/PDF に変換してください。"}}[x];return F?F[b]||F.th||F.en:r("error")||"Error"}const l=x=>x==null||isNaN(x)?"":Number(x).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});function p(x,b,z,F){const K=e(x),Z=e(b),le=e(z);if(!K||!Z||!le)return;const re=U=>{if(!U||!U.length)return;const Q=Array.isArray(s[F])?s[F].slice():[],te=new Set(Q.map(ne=>ne.name+"|"+ne.size));for(const ne of U){if(!ne)continue;const se=ne.name+"|"+ne.size;te.has(se)||(Q.push(ne),te.add(se))}s[F]=Q,f(le,Q),d(),v(),window._reconCollapse&&window._reconCollapse.renderGlvPreview&&window._reconCollapse.renderGlvPreview()};K.addEventListener("click",()=>Z.click()),K.addEventListener("keydown",U=>{(U.key==="Enter"||U.key===" ")&&(U.preventDefault(),Z.click())}),Z.addEventListener("change",()=>{re(Array.from(Z.files||[])),Z.value=""}),K.addEventListener("dragover",U=>{U.preventDefault(),K.classList.add("drag-over")}),K.addEventListener("dragleave",()=>K.classList.remove("drag-over")),K.addEventListener("drop",U=>{U.preventDefault(),K.classList.remove("drag-over");const Q=U.dataTransfer&&U.dataTransfer.files?Array.from(U.dataTransfer.files):[];re(Q)})}function f(x,b){if(!x)return;if(!b||b.length===0){x.textContent="";return}const z=b.reduce((F,K)=>F+Math.round(K.size/1024),0);if(b.length===1)x.textContent=b[0].name+"  ("+z+" KB)";else{const F=window.t&&window.t("glv-files-count")||"{n} 个文件";x.textContent=F.replace("{n}",b.length)+"  ("+z+" KB)"}}function c(x){const b=s[x];return Array.isArray(b)?b:b?[b]:[]}function d(){const x=e("btn-glv-run");if(!x)return;const b=c("glFile").length>0&&c("vatFile").length>0;x.disabled=!b||s.running}function v(){const x=e("glv-status");if(!x||s.running)return;const b=c("vatFile").length,z=c("glFile").length;b===0&&z===0?(x.className="vex-action-info muted",x.innerHTML="<span>"+r("hint_need_both")+"</span>"):b>0&&z>0?(x.className="vex-action-info ok",x.innerHTML="<span>"+r("hint_ready")+"</span>"):(x.className="vex-action-info muted",x.innerHTML="<span>"+r("hint_need_one_more")+"</span>")}function w(x,b){const z=x==="vat"?"vatFile":"glFile",F=x==="vat"?"glv-vat-input":"glv-gl-input",K=x==="vat"?"glv-vat-name":"glv-gl-name",Z=c(z);b==null?s[z]=[]:s[z]=Z.filter((re,U)=>U!==b);const le=e(F);le&&(le.value=""),f(e(K),c(z)),d(),v(),window._reconCollapse&&window._reconCollapse.renderGlvPreview&&window._reconCollapse.renderGlvPreview()}window._glvRemoveFile=w;function C(){s.glFile=[],s.vatFile=[],s.currentTaskId=null,s.lastDetail=[],s.lastSummary=null;const x=e("glv-vat-input");x&&(x.value="");const b=e("glv-gl-input");b&&(b.value="");const z=e("glv-vat-name");z&&(z.textContent="");const F=e("glv-gl-name");F&&(F.textContent="");const K=e("glv-result");K&&(K.style.display="none");const Z=e("glv-kpi-strip");Z&&(Z.style.display="none"),d(),v(),window._glvClearPreviewSearch&&window._glvClearPreviewSearch(),window._reconCollapse&&window._reconCollapse.renderGlvPreview&&window._reconCollapse.renderGlvPreview()}function E(x){const b=e("glv-tbody");if(!b)return;oe(x.length),b.innerHTML="";const z=r("not_found"),F=document.createDocumentFragment();x.forEach(K=>{const Z=document.createElement("tr"),le=(ne,se)=>{const ue=document.createElement("td");return se&&(ue.className=se),ue.textContent=ne,ue},re=K.gl_amount===null||K.gl_amount===void 0,U=K.diff;let Q="glv-num",te="glv-num";re?(te+=" glv-cell-missing",Q+=" glv-cell-missing"):Math.abs(U||0)<.005?Q+=" glv-cell-ok":Q+=" glv-cell-diff",Z.appendChild(le(K.doc_no||"","glv-doc")),Z.appendChild(le(K.date||"","")),Z.appendChild(le(K.customer_name||"","")),Z.appendChild(le(l(K.vat_amount),"glv-num")),Z.appendChild(le(re?z:l(K.gl_amount),te)),Z.appendChild(le(re?z:l(K.diff),Q)),Z.appendChild(le(K.account_codes||"","glv-doc")),F.appendChild(Z)}),b.appendChild(F)}function m(x){const b=e("glv-summary-table")&&e("glv-summary-table").querySelector("tbody");if(!b)return;b.innerHTML="",[{label:r("s_gl_total"),amount:x.gl_total,emph:!0,items:[],negate:!1},{label:r("s_minus_gl_cr"),amount:-(x.gl_only_credit||0),emph:!1,items:x.gl_only_credit_items||[],negate:!0},{label:r("s_plus_gl_dr"),amount:x.gl_only_debit||0,emph:!1,items:x.gl_only_debit_items||[],negate:!1},{label:r("s_plus_vat_p"),amount:x.vat_only_positive||0,emph:!1,items:x.vat_only_positive_items||[],negate:!1},{label:r("s_minus_vat_n"),amount:x.vat_only_negative||0,emph:!1,items:x.vat_only_negative_items||[],negate:!1},{label:r("s_vat_total"),amount:x.vat_total,emph:!0,items:[],negate:!1}].forEach(({label:F,amount:K,emph:Z,items:le,negate:re})=>{const U=document.createElement("tr");U.className=Z?"glv-summary-total":"glv-summary-sect";const Q=document.createElement("td"),te=document.createElement("td");Q.textContent=F,te.textContent=Z?l(K):"",U.appendChild(Q),U.appendChild(te),b.appendChild(U),(le||[]).forEach(ne=>{const se=document.createElement("tr");se.className="glv-summary-item";const ue=document.createElement("td"),k=document.createElement("td"),T=[ne.doc_no,ne.date,ne.name].filter(Boolean);ue.textContent="· "+T.join("  ·  ");const q=re?-(ne.amount||0):ne.amount||0;k.textContent=l(q),se.appendChild(ue),se.appendChild(k),b.appendChild(se)})})}function h(x){e("glv-kpi-matched")&&(e("glv-kpi-matched").textContent=x&&x.matched!=null?x.matched:"—"),e("glv-kpi-diff")&&(e("glv-kpi-diff").textContent=x&&x.diff!=null?x.diff:"—"),e("glv-kpi-unmatched")&&(e("glv-kpi-unmatched").textContent=x&&x.unmatched!=null?x.unmatched:"—")}function S(x){if(!x)return"";try{const b=new Date(x);if(isNaN(b.getTime()))return x;const z=F=>String(F).padStart(2,"0");return b.getFullYear()+"-"+z(b.getMonth()+1)+"-"+z(b.getDate())+" "+z(b.getHours())+":"+z(b.getMinutes())}catch{return x}}const L=10;var D=[],H=1;function _(){H=1,g();var x=((e("glv-hist-search")||{}).value||"").trim().toLowerCase();if(x){var b=e("glv-history-tbody");b&&b.querySelectorAll("tr").forEach(function(z){z.dataset.taskId&&(z.style.display=z.textContent.toLowerCase().indexOf(x)>=0?"":"none")})}}function g(){const x=e("glv-history-table-wrap"),b=e("glv-history-empty"),z=e("glv-history-tbody"),F=e("glv-history-pager"),K=e("glv-history-pager-info"),Z=e("glv-history-prev"),le=e("glv-history-next");if(!z)return;if(z.innerHTML="",!D.length){x&&(x.style.display="none"),b&&(b.style.display=""),F&&(F.style.display="none");return}x&&(x.style.display=""),b&&(b.style.display="none");const re=Math.ceil(D.length/L);H>re&&(H=re);const U=(H-1)*L,Q=D.slice(U,U+L);F&&(F.style.display=D.length>L?"":"none",K&&(K.textContent=H+" / "+re),Z&&(Z.disabled=H<=1),le&&(le.disabled=H>=re)),Q.forEach(ne=>{const se=document.createElement("tr");se.dataset.taskId=ne.id;const ue=document.createElement("td");ue.textContent=S(ne.created_at);const k=document.createElement("td");k.className="glv-history-file",k.title=(ne.vat_filename||"")+" + "+(ne.gl_filename||""),k.textContent=(ne.vat_filename||"?")+" + "+(ne.gl_filename||"?");const T=document.createElement("td");T.className="glv-num",T.textContent=(ne.vat_row_count||0)+" / "+(ne.gl_row_count||0);const q=document.createElement("td");q.className="glv-num",q.textContent=ne.matched_count||0;const G=document.createElement("td");G.className="glv-num",G.textContent=ne.diff_count||0;const O=document.createElement("td");O.className="glv-num",O.textContent=ne.unmatched_count||0;const y=document.createElement("td");y.className="glv-history-actions";const N=(de,ie,ce,ae)=>{const pe=document.createElement("button");return pe.type="button",ce&&(pe.className=ce),pe.title=ie,pe.setAttribute("aria-label",ie),pe.innerHTML=de,pe.onclick=ae,pe},V='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><path d="M2 8a6 6 0 1 0 12 0A6 6 0 0 0 2 8z"/><polyline points="6 8 8 10 10 8"/><line x1="8" y1="4" x2="8" y2="10"/></svg>',J='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><path d="M8 2v9M4 7l4 4 4-4M3 14h10"/></svg>',X='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><polyline points="3 4 13 4"/><path d="M6 4V2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V4"/><path d="M5 4l1 9a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1l1-9"/></svg>';y.appendChild(N(V,r("hist_load"),"",()=>P(ne.id))),y.appendChild(N(J,r("hist_export"),"",()=>R(ne.id))),y.appendChild(N(X,r("hist_delete"),"glv-del",()=>B(ne.id))),[ue,k,T,q,G,O,y].forEach(de=>se.appendChild(de)),z.appendChild(se)})}function I(){var x=e("glv-history-prev"),b=e("glv-history-next");x&&!x._glvBound&&(x._glvBound=!0,x.addEventListener("click",function(){H>1&&(H--,g())})),b&&!b._glvBound&&(b._glvBound=!0,b.addEventListener("click",function(){var z=Math.ceil(D.length/L);H<z&&(H++,g())}))}async function M(){try{const b=await(await fetch("/api/recon/gl-vat/tasks",{headers:o()})).json();D=b&&b.tasks||[],H=1,g(),I()}catch(x){console.error("[gl-vat] history load failed:",x)}}async function P(x){try{const z=await(await fetch("/api/recon/gl-vat/"+x,{headers:o()})).json();if(!z||!z.ok)throw new Error("load_failed");s.currentTaskId=x,s.lastDetail=z.detail||[],s.lastSummary=z.summary||{},h(z.stats||{}),E(s.lastDetail),m(s.lastSummary);const F=e("glv-result");F&&(F.style.display=""),W(),window.scrollTo({top:F?F.offsetTop-80:0,behavior:"smooth"})}catch(b){console.error("[gl-vat] load task failed:",b),alert(r("error")+": "+(b.message||b))}}async function R(x){try{const b="/api/recon/gl-vat/"+x+"/export?lang="+encodeURIComponent(a()),z=await fetch(b,{headers:o()});if(!z.ok)throw new Error("HTTP "+z.status);const F=await z.blob(),K=document.createElement("a");K.href=URL.createObjectURL(F),K.download="GL_VAT_recon_"+x+".xlsx",document.body.appendChild(K),K.click(),setTimeout(()=>{URL.revokeObjectURL(K.href),K.remove()},200)}catch(b){console.error("[gl-vat] exportTask failed:",b),typeof showToast=="function"&&showToast(r("error")+": "+(b.message||b),"error")}}async function B(x){let b;if(typeof window.showConfirm=="function"?b=await window.showConfirm(r("confirm_delete"),{danger:!0}):b=confirm(r("confirm_delete")),!!b)try{const z=await fetch("/api/recon/gl-vat/"+x,{method:"DELETE",headers:o()});if(!z.ok)throw new Error("HTTP "+z.status);M()}catch(z){console.error("[gl-vat] delete failed:",z),typeof showToast=="function"&&showToast(r("error")+": "+(z.message||z),"error")}}async function j(){if(!s.glFile||!s.vatFile){typeof showToast=="function"&&showToast(r("need_files"),"warn");return}s.running=!0,d();const x=e("glv-status"),b=e("glv-progress"),z=e("glv-progress-sub");x&&(x.className="vex-action-info muted",x.style.color="",x.innerHTML="<span>"+r("running")+"</span>"),b&&(b.style.display=""),z&&(z.textContent=(s.vatFile.name||"VAT")+" + "+(s.glFile.name||"GL"));const F=new FormData,K=c("vatFile"),Z=c("glFile");for(const re of K)F.append("vat_files",re,re.name);for(const re of Z)F.append("gl_files",re,re.name);const le=(e("glv-prefix")&&e("glv-prefix").value||"4").trim()||"4";F.append("revenue_prefix",le),F.append("lang",a());try{const re=await fetch("/api/recon/gl-vat/submit",{method:"POST",headers:o(),body:F});let U=null;try{U=await re.json()}catch{U=null}if(!re.ok||!U||!U.ok||!U.job_id)throw new Error(U&&U.detail||U&&U.error||"HTTP "+re.status);const Q=e("glv-progress-sub"),te=await window._reconPollJob(U.job_id,n(),{onProgress:k=>{Q&&(Q.textContent=window._reconProgressText(k,a()))}});if(!te||te.status!=="done"||!te.result_id)throw te&&te.status==="failed"&&te.error_code?new Error(u(te.error_code)):new Error(r("error")||"Error");const ne=await fetch("/api/recon/gl-vat/"+encodeURIComponent(te.result_id),{headers:o()});let se=null;try{se=await ne.json()}catch{se=null}if(!ne.ok||!se||!se.ok)throw new Error(se&&se.detail||se&&se.error||"HTTP "+ne.status);s.currentTaskId=se.task_id,s.lastDetail=se.detail||[],s.lastSummary=se.summary||{},h(se.stats||{}),E(s.lastDetail),m(s.lastSummary);const ue=e("glv-result");ue&&(ue.style.display=""),W(),x&&(x.className="vex-action-info ok",x.style.color="",x.innerHTML="<span>"+r("done")+" · GL "+(se.gl_row_count||0)+" · VAT "+(se.vat_row_count||0)+"</span>"),M()}catch(re){console.error("[gl-vat] run failed:",re),x&&(x.className="vex-action-info",x.style.color="#ef4444",x.innerHTML="<span>"+r("error")+": "+(re.message||re)+"</span>")}finally{s.running=!1,b&&(b.style.display="none"),d()}}async function Y(){if(s.currentTaskId)try{const x="/api/recon/gl-vat/"+s.currentTaskId+"/export?lang="+encodeURIComponent(a()),b=await fetch(x,{headers:o()});if(!b.ok)throw new Error("HTTP "+b.status);const z=await b.blob(),F=document.createElement("a");F.href=URL.createObjectURL(z),F.download="GL_VAT_recon_"+s.currentTaskId+".xlsx",document.body.appendChild(F),F.click(),setTimeout(()=>{URL.revokeObjectURL(F.href),F.remove()},200)}catch(x){console.error("[gl-vat] export failed:",x),typeof showToast=="function"&&showToast(r("error")+": "+(x.message||x),"error")}}function A(){s.running||v(),M(),s.lastDetail&&s.lastDetail.length&&E(s.lastDetail),s.lastSummary&&m(s.lastSummary)}function W(){var x=e("glv-kpi-strip");x&&(x.style.display="");var b=e("glv-section-summary");b&&b.setAttribute("data-collapsed","false");var z=e("glv-section-detail");z&&z.setAttribute("data-collapsed","false")}function ee(){document.querySelectorAll(".glv-section-head[data-toggle]").forEach(x=>{const b=x.getAttribute("data-toggle"),z=document.getElementById(b);if(!z)return;const F=K=>{if(K.target&&K.target.closest("button")!==null&&!K.target.classList.contains("glv-section-head"))return;const Z=z.getAttribute("data-collapsed")==="true";z.setAttribute("data-collapsed",Z?"false":"true")};x.addEventListener("click",F),x.addEventListener("keydown",K=>{(K.key==="Enter"||K.key===" ")&&(K.preventDefault(),F(K))})})}function oe(x){const b=e("glv-detail-count");b&&(b.textContent=x!=null?String(x):"")}function $(){if(s.inited){M();return}s.inited=!0,p("glv-drop-gl","glv-gl-input","glv-gl-name","glFile"),p("glv-drop-vat","glv-vat-input","glv-vat-name","vatFile");const x=e("btn-glv-run");x&&x.addEventListener("click",j);const b=e("btn-glv-export");b&&b.addEventListener("click",Y);const z=e("btn-glv-reset");z&&z.addEventListener("click",C);const F=e("glv-hist-search");F&&F.addEventListener("input",_),ee(),h(null),v(),window._loadGlvHistory=M,M(),typeof window.subscribeI18n=="function"&&window.subscribeI18n("gl-vat-recon",A)}window.GlVatRecon={ensureInit:$},window._glvPreviewFiles=function(x){return c(x==="vat"?"vatFile":"glFile")}})();(function(){const e=["flowaccount","peak","xero","quickbooks","express"],n={flowaccount:"FlowAccount",peak:"PEAK",xero:"Xero",quickbooks:"QuickBooks",express:"Express"},a=["expense_office","expense_travel","expense_utility","asset_inventory","asset_fixed","liability_ap","revenue_sales","revenue_service","other"],o=["vat_7","vat_0","vat_exempt","wht_1","wht_3","wht_5","non_vat"],s="468b50c1-5593-4fd6-990d-515ce8085563";let i={sub:"clients",loaded:{clients:!1,accounts:!1,taxes:!1,products:!1},items:{clients:[],accounts:[],taxes:[],products:[]},clientList:[],clientLoaded:!1,addingNew:{clients:!1,accounts:!1,taxes:!1,products:!1},bound:!1};function r(){const P=typeof _userInfo<"u"?_userInfo:null;return!!(P&&(P.role==="owner"||P.is_super_admin))}function u(){const P=typeof _userInfo<"u"?_userInfo:null;return!!(P&&P.id===s)}function l(P){return typeof escapeHtml=="function"?escapeHtml(P==null?"":String(P)):String(P??"")}function p(P,R){try{typeof showToast=="function"&&showToast(P,R||"info")}catch{}}async function f(P,R){const B=localStorage.getItem("mrpilot_token");if(B&&!(i.loaded[P]&&!R))try{const j=await fetch("/api/erp/mappings/"+P,{headers:{Authorization:"Bearer "+B}});if(!j.ok)throw new Error("http_"+j.status);const Y=await j.json();i.items[P]=Y.items||[],i.loaded[P]=!0}catch{i.items[P]=[],i.loaded[P]=!1}}async function c(P){if(i.clientLoaded)return;const R=localStorage.getItem("mrpilot_token");if(R)try{const B=await fetch("/api/clients?include_inactive=false",{headers:{Authorization:"Bearer "+R}});if(!B.ok)throw new Error("http_"+B.status);const j=await B.json();i.clientList=(j.clients||j.items||[]).filter(Y=>Y.is_active!==!1),i.clientLoaded=!0}catch{i.clientList=[]}}function d(){const P=document.getElementById("erp-map-pane-wrap");if(!P)return;const R=!r();let B="";R&&(B+='<div class="erp-map-readonly-banner"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="10" cy="10" r="8"/><path d="M10 6v4M10 13v0.01"/></svg>'+l(t("erp-map-readonly-tip"))+"</div>"),B+='<div class="erp-map-toolbar">',!R&&i.sub!=="products"&&(B+='<button class="btn btn-primary" type="button" id="erp-map-add-btn" data-i18n="erp-map-add-row">'+l(t("erp-map-add-row"))+"</button>"),B+="</div>",B+='<div class="erp-map-table" id="erp-map-table-host"></div>',P.innerHTML=B,v();const j=document.getElementById("erp-map-dev-bar");j&&(j.style.display=r()&&u()?"":"none")}function v(){const P=document.getElementById("erp-map-table-host");if(!P)return;const R=i.sub,B=i.items[R]||[],j=i.addingNew[R],Y=!r();if(!B.length&&!j){P.innerHTML='<div class="erp-map-empty"><strong>'+l(t("erp-map-empty-"+R))+"</strong>"+l(t("erp-map-empty-"+R+"-sub"))+"</div>";return}let A="";A+=w(R),j&&!Y&&(A+=S(R)),B.forEach(function(W){A+=L(R,W,Y)}),P.innerHTML=A}function w(P){return P==="clients"?'<div class="erp-map-row erp-map-head row-clients"><div>'+l(t("erp-map-col-client"))+"</div><div>"+l(t("erp-map-col-erp"))+"</div><div>"+l(t("erp-map-col-erp-code"))+"</div><div>"+l(t("erp-map-col-notes"))+"</div><div>"+l(t("erp-map-col-actions"))+"</div></div>":P==="accounts"?'<div class="erp-map-row erp-map-head row-accounts"><div>'+l(t("erp-map-col-erp"))+"</div><div>"+l(t("erp-map-col-category"))+"</div><div>"+l(t("erp-map-col-erp-code"))+"</div><div>"+l(t("erp-map-col-erp-name"))+"</div><div>"+l(t("erp-map-col-notes"))+"</div><div>"+l(t("erp-map-col-actions"))+"</div></div>":P==="products"?'<div class="erp-map-row erp-map-head row-products"><div>'+l(t("erp-map-col-item-name"))+"</div><div>"+l(t("erp-map-col-erp-product-code"))+"</div><div>"+l(t("erp-map-col-erp-name"))+"</div><div>"+l(t("erp-map-col-notes"))+"</div><div>"+l(t("erp-map-col-actions"))+"</div></div>":'<div class="erp-map-row erp-map-head row-taxes"><div>'+l(t("erp-map-col-erp"))+"</div><div>"+l(t("erp-map-col-tax"))+"</div><div>"+l(t("erp-map-col-erp-tax-code"))+"</div><div>"+l(t("erp-map-col-notes"))+"</div><div>"+l(t("erp-map-col-actions"))+"</div></div>"}function C(P,R){let B='<select class="form-input" data-erp-field="'+R+'">';return B+='<option value="">'+l(t("erp-map-pick-erp"))+"</option>",e.forEach(function(j){const Y=j===P?" selected":"";B+='<option value="'+j+'"'+Y+">"+l(n[j])+"</option>"}),B+="</select>",B}function E(P){let R='<select class="form-input" data-erp-field="client_id">';return R+='<option value="">'+l(t("erp-map-pick-client"))+"</option>",(i.clientList||[]).forEach(function(B){const j=String(B.id)===String(P)?" selected":"";R+='<option value="'+B.id+'"'+j+">"+l(B.name||"#"+B.id)+"</option>"}),R+="</select>",R}function m(P){let R='<select class="form-input" data-erp-field="pearnly_category">';return R+='<option value="">'+l(t("erp-map-pick-cat"))+"</option>",a.forEach(function(B){const j=B===P?" selected":"";R+='<option value="'+B+'"'+j+">"+l(t("erp-map-cat-"+B))+"</option>"}),R+="</select>",R}function h(P){let R='<select class="form-input" data-erp-field="pearnly_tax_kind">';return R+='<option value="">'+l(t("erp-map-pick-tax"))+"</option>",o.forEach(function(B){const j=B===P?" selected":"";R+='<option value="'+B+'"'+j+">"+l(t("erp-map-tax-"+B))+"</option>"}),R+="</select>",R}function S(P){const R='<button class="btn btn-primary" type="button" data-erp-save="new" style="padding:6px 12px;height:32px;">'+l(t("erp-map-save"))+"</button>";return P==="clients"?'<div class="erp-map-row erp-map-row-add row-clients" data-erp-row="new"><div data-label="'+l(t("erp-map-col-client"))+'">'+E("")+'</div><div data-label="'+l(t("erp-map-col-erp"))+'">'+C("","erp_type")+'</div><div data-label="'+l(t("erp-map-col-erp-code"))+'"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="'+l(t("erp-map-ph-erp-code"))+'"></div><div data-label="'+l(t("erp-map-col-notes"))+'"><input type="text" class="form-input" data-erp-field="notes" placeholder="'+l(t("erp-map-ph-notes"))+'"></div><div>'+R+"</div></div>":P==="accounts"?'<div class="erp-map-row erp-map-row-add row-accounts" data-erp-row="new"><div data-label="'+l(t("erp-map-col-erp"))+'">'+C("","erp_type")+'</div><div data-label="'+l(t("erp-map-col-category"))+'">'+m("")+'</div><div data-label="'+l(t("erp-map-col-erp-code"))+'"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="'+l(t("erp-map-ph-acc-code"))+'"></div><div data-label="'+l(t("erp-map-col-erp-name"))+'"><input type="text" class="form-input" data-erp-field="erp_name" placeholder="'+l(t("erp-map-ph-acc-name"))+'"></div><div data-label="'+l(t("erp-map-col-notes"))+'"><input type="text" class="form-input" data-erp-field="notes" placeholder="'+l(t("erp-map-ph-notes"))+'"></div><div>'+R+"</div></div>":'<div class="erp-map-row erp-map-row-add row-taxes" data-erp-row="new"><div data-label="'+l(t("erp-map-col-erp"))+'">'+C("","erp_type")+'</div><div data-label="'+l(t("erp-map-col-tax"))+'">'+h("")+'</div><div data-label="'+l(t("erp-map-col-erp-tax-code"))+'"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="'+l(t("erp-map-ph-tax-code"))+'"></div><div data-label="'+l(t("erp-map-col-notes"))+'"><input type="text" class="form-input" data-erp-field="notes" placeholder="'+l(t("erp-map-ph-notes"))+'"></div><div>'+R+"</div></div>"}function L(P,R,B){const j=B?"":'<button class="erp-map-del-btn" type="button" data-erp-del="'+l(R.id)+'" title="'+l(t("erp-map-delete"))+'"><svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 6h12M8 6V4h4v2M6 6l1 11h6l1-11"/></svg></button>',Y='<span class="erp-map-erp-badge">'+l(n[R.erp_type]||R.erp_type)+"</span>";if(P==="clients")return'<div class="erp-map-row row-clients"><div data-label="'+l(t("erp-map-col-client"))+'" class="erp-map-cell-name">'+l(R.client_name||"#"+R.client_id)+'</div><div data-label="'+l(t("erp-map-col-erp"))+'">'+Y+'</div><div data-label="'+l(t("erp-map-col-erp-code"))+'" class="erp-map-code">'+l(R.erp_code||"")+'</div><div data-label="'+l(t("erp-map-col-notes"))+'">'+l(R.notes||"")+"</div><div>"+j+"</div></div>";if(P==="accounts"){const W=t("erp-map-cat-"+(R.pearnly_category||"other"))||R.pearnly_category;return'<div class="erp-map-row row-accounts"><div data-label="'+l(t("erp-map-col-erp"))+'">'+Y+'</div><div data-label="'+l(t("erp-map-col-category"))+'" class="erp-map-cell-name">'+l(W)+'</div><div data-label="'+l(t("erp-map-col-erp-code"))+'" class="erp-map-code">'+l(R.erp_code||"")+'</div><div data-label="'+l(t("erp-map-col-erp-name"))+'">'+l(R.erp_name||"")+'</div><div data-label="'+l(t("erp-map-col-notes"))+'">'+l(R.notes||"")+"</div><div>"+j+"</div></div>"}if(P==="products")return'<div class="erp-map-row row-products"><div data-label="'+l(t("erp-map-col-item-name"))+'" class="erp-map-cell-name">'+l(R.item_name||"")+'</div><div data-label="'+l(t("erp-map-col-erp-product-code"))+'" class="erp-map-code">'+l(R.erp_code||"")+'</div><div data-label="'+l(t("erp-map-col-erp-name"))+'">'+l(R.erp_name||"")+'</div><div data-label="'+l(t("erp-map-col-notes"))+'">'+l(R.notes||"")+"</div><div>"+j+"</div></div>";const A=t("erp-map-tax-"+(R.pearnly_tax_kind||""))||R.pearnly_tax_kind;return'<div class="erp-map-row row-taxes"><div data-label="'+l(t("erp-map-col-erp"))+'">'+Y+'</div><div data-label="'+l(t("erp-map-col-tax"))+'" class="erp-map-cell-name"><span class="erp-map-tax-badge">'+l(A)+'</span></div><div data-label="'+l(t("erp-map-col-erp-tax-code"))+'" class="erp-map-code">'+l(R.erp_code||"")+'</div><div data-label="'+l(t("erp-map-col-notes"))+'">'+l(R.notes||"")+"</div><div>"+j+"</div></div>"}async function D(P){const R=i.sub,B={};P.querySelectorAll("[data-erp-field]").forEach(function(W){B[W.dataset.erpField]=(W.value||"").trim()});const j=localStorage.getItem("mrpilot_token");if(!j)return;let Y={},A="/api/erp/mappings/"+R;if(R==="clients"){if(!B.client_id||!B.erp_type||!B.erp_code){p(t("erp-map-save-fail"),"error");return}Y={client_id:parseInt(B.client_id,10),erp_type:B.erp_type,erp_code:B.erp_code,notes:B.notes||""}}else if(R==="accounts"){if(!B.erp_type||!B.pearnly_category||!B.erp_code){p(t("erp-map-save-fail"),"error");return}Y={erp_type:B.erp_type,pearnly_category:B.pearnly_category,erp_code:B.erp_code,erp_name:B.erp_name||"",notes:B.notes||""}}else{if(!B.erp_type||!B.pearnly_tax_kind||!B.erp_code){p(t("erp-map-save-fail"),"error");return}Y={erp_type:B.erp_type,pearnly_tax_kind:B.pearnly_tax_kind,erp_code:B.erp_code,notes:B.notes||""}}try{const W=await fetch(A,{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+j},body:JSON.stringify(Y)});if(!W.ok)throw new Error("http_"+W.status);i.addingNew[R]=!1,await f(R,!0),v(),p(t("erp-map-saved-toast"),"success")}catch{p(t("erp-map-save-fail"),"error")}}async function H(P){if(!await window.pearnlyConfirm(t("erp-map-confirm-delete")))return;const B=i.sub,j=localStorage.getItem("mrpilot_token");try{const Y=await fetch("/api/erp/mappings/"+B+"/"+encodeURIComponent(P),{method:"DELETE",headers:{Authorization:"Bearer "+j}});if(!Y.ok)throw new Error("http_"+Y.status);await f(B,!0),v(),p(t("erp-map-deleted-toast"),"success")}catch{p(t("erp-map-delete-fail"),"error")}}async function _(){await c(),await f(i.sub,!1),d()}function g(P){P!==i.sub&&(i.sub=P,i.addingNew[P]=!1,["clients","accounts","taxes","products"].forEach(function(R){R!==P&&(i.addingNew[R]=!1)}),document.querySelectorAll(".erp-map-subtab").forEach(function(R){R.classList.toggle("active",R.dataset.erpSubtab===P)}),f(P,!1).then(function(){d()}))}function I(){i.bound||(i.bound=!0,document.addEventListener("click",function(P){const R=P.target.closest(".erp-subtab[data-erp-subtab]");if(R){P.preventDefault();const W=R.dataset.erpSubtab;document.querySelectorAll(".erp-subtab").forEach(function(ee){ee.classList.toggle("active",ee.dataset.erpSubtab===W)}),document.querySelectorAll(".erp-subpanel").forEach(function(ee){ee.classList.toggle("active",ee.dataset.erpSubpanel===W)}),W==="mappings"&&setTimeout(_,50);return}const B=P.target.closest(".erp-map-subtab[data-erp-subtab]");if(B){P.preventDefault(),g(B.dataset.erpSubtab);return}if(P.target.closest("#erp-map-add-btn")){if(P.preventDefault(),!r())return;i.addingNew[i.sub]=!0,v();return}const Y=P.target.closest('[data-erp-save="new"]');if(Y){P.preventDefault();const W=Y.closest('[data-erp-row="new"]');W&&D(W);return}const A=P.target.closest("[data-erp-del]");if(A){P.preventDefault(),H(A.dataset.erpDel);return}}))}function M(){const P=document.getElementById("erp-map-pane-wrap");P&&P.children.length>0&&d(),document.querySelectorAll(".erp-map-subtab").forEach(function(R){const B="erp-map-subtab-"+R.dataset.erpSubtab,j=t(B);j&&j!==B&&(R.textContent=j)})}I(),typeof window.subscribeI18n=="function"&&window.subscribeI18n("erp-mappings",M)})();(function(){let e=null,n=0,a=!1;function o(_){return typeof escapeHtml=="function"?escapeHtml(_==null?"":String(_)):String(_??"")}function s(_,g){try{typeof showToast=="function"&&showToast(_,g||"info")}catch{}}async function i(_){const g=Date.now();if(e&&g-n<3e4)return e;const I=localStorage.getItem("mrpilot_token");if(!I)return[];try{const M=await fetch("/api/erp/connectors/status",{headers:{Authorization:"Bearer "+I}});if(!M.ok)return[];const P=await M.json();return e=P&&P.connectors||[],n=g,e}catch{return[]}}function r(){try{return localStorage.getItem("pn_push_default_connector")||""}catch{return""}}function u(_){try{localStorage.setItem("pn_push_default_connector",_||"")}catch{}}function l(_){if(!_||!_.length)return null;const g=r();if(g){const M=_.find(P=>P.id===g);if(M)return M}const I=_.find(M=>M.is_default);return I||_[0]}function p(_){if(!_)return!1;const g=String(_.status||"").toLowerCase();return g==="exception"||g==="exception_pending"||g==="rejected"}function f(){try{return(typeof _results<"u"?_results:[])[typeof _drawerIdx<"u"?_drawerIdx:-1]||null}catch{return null}}function c(_){const g=_&&(_.type||_.id);return g==="xero"?'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M5.5 8l2 2 3-3.5"/></svg>':g==="webhook"?'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="11.5" r="1.8"/><circle cx="11" cy="4.5" r="1.8"/><path d="M6.4 10l3.2-4M5 9.6V5.5a3 3 0 016 0"/></svg>':'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M2 8h9M8 5l3 3-3 3"/><rect x="11" y="3" width="3" height="10" rx="1"/></svg>'}async function d(_,g){if(!_||!g)return!1;const I=document.getElementById("btn-push-default");I&&(I.disabled=!0,I.classList.add("loading"));const M=localStorage.getItem("mrpilot_token");try{let P,R={method:"POST",headers:{Authorization:"Bearer "+M}};_.type==="xero"?P="/api/erp/xero/push/"+encodeURIComponent(g):(P="/api/erp/push",R.headers["Content-Type"]="application/json",R.body=JSON.stringify({history_id:g,endpoint_id:_.endpoint_id||void 0}));const B=await fetch(P,R);let j={};try{j=await B.json()}catch{}if(!B.ok){let Y=j&&j.detail||"unknown";typeof Y=="object"&&(Y=Y.code||JSON.stringify(Y));let A=String(Y||"unknown");if(_.type==="xero"){const W=A.replace(/^xero\./,"").toLowerCase(),ee=t("xero-"+W);ee&&ee!=="xero-"+W&&(A=ee)}return s(t("unified-push-fail").replace("{name}",_.name).replace("{err}",A),"error"),!1}if(j&&j.ok===!1){let Y=j.error_msg||j.error_code||"unknown";return Y=String(Y).slice(0,200),s(t("unified-push-fail").replace("{name}",_.name).replace("{err}",Y),"error"),!1}return s(t("unified-push-ok").replace("{name}",_.name),"success"),!0}catch(P){return s(t("unified-push-fail").replace("{name}",_.name).replace("{err}",P.message||"network"),"error"),!1}finally{I&&(I.disabled=!1,I.classList.remove("loading"))}}async function v(_,g){for(const I of _)await d(I,g)}function w(_,g){const I=document.createElement("div");I.className="pn-push-dropdown",I.id="pn-push-dropdown";const M=(_||[]).map(R=>{const B=!!(g&&R.id===g.id),j=R.method==="download"?t("unified-push-tag-download"):B?t("unified-push-tag-default"):"";return'<div class="pn-pd-item" data-cid="'+o(R.id)+'"><span class="pn-pd-icon">'+c(R)+'</span><span class="pn-pd-name">'+o(R.name)+"</span>"+(j?'<span class="pn-pd-tag">'+o(j)+"</span>":"")+(B?'<span class="pn-pd-check">✓</span>':"")+"</div>"}).join(""),P=_&&_.length>1?'<div class="pn-pd-divider"></div><div class="pn-pd-item pn-pd-all" data-cid="__all__"><span class="pn-pd-icon"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h10M3 10h10M3 13.5h6"/></svg></span><span class="pn-pd-name">'+o(t("unified-push-all").replace("{n}",_.length))+"</span></div>":"";return I.innerHTML=M+P,I}function C(){const _=document.getElementById("pn-push-dropdown");_&&_.remove()}async function E(){if(document.getElementById("pn-push-dropdown")){C();return}const _=await i()||[],g=l(_),I=w(_,g),M=document.getElementById("pn-push-wrap");M&&M.appendChild(I)}async function m(){const _=await i()||[],g=l(_);if(!g)return;const I=f(),M=I&&(I._historyId||I.history_id);if(M){if(p(I)){s(t("unified-push-disabled-exc"),"warn");return}await d(g,M)}}async function h(_){C();const g=await i()||[],I=f(),M=I&&(I._historyId||I.history_id);if(!M)return;if(p(I)){s(t("unified-push-disabled-exc"),"warn");return}if(_==="__all__"){await v(g,M);return}const P=g.find(R=>R.id===_);P&&(u(_),await d(P,M),L())}async function S(){const _=document.getElementById("drawer-history-save");if(!_||_.querySelector("#pn-push-wrap"))return;const g=document.createElement("div");g.id="pn-push-wrap",g.className="pn-push-wrap",g.dataset.loading="1",_.insertBefore(g,_.firstChild),["btn-push-erp","btn-xero-push"].forEach(j=>{_.querySelectorAll("#"+j).forEach(Y=>{Y.style.display="none"})});const I=await i()||[],M=l(I),P=I.length>0;if(!P)g.innerHTML='<button type="button" class="btn btn-ghost pn-push-empty" id="btn-push-default" disabled title="'+o(t("unified-push-empty-tip"))+'"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M2 8h9M8 5l3 3-3 3"/></svg><span style="margin-left:4px;">'+o(t("unified-push-empty"))+"</span></button>";else{const j=I.length>1;g.innerHTML='<div class="pn-push-split"><button type="button" class="pn-push-main" id="btn-push-default" title="'+o(t("unified-push-tip"))+'">'+c(M)+"<span>"+o(t("unified-push-to").replace("{name}",M?M.name:""))+"</span></button>"+(j?'<button type="button" class="pn-push-arrow" id="btn-push-arrow" title="'+o(t("unified-push-other"))+'"><svg viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 5l3 3 3-3"/></svg></button>':"")+"</div>"}delete g.dataset.loading;const R=g.querySelector("#btn-push-default");R&&P&&R.addEventListener("click",m);const B=g.querySelector("#btn-push-arrow");B&&B.addEventListener("click",function(j){j.stopPropagation(),E()}),a||(a=!0,document.addEventListener("click",function(j){const Y=j.target.closest(".pn-pd-item");if(Y){const A=Y.getAttribute("data-cid");h(A);return}j.target.closest("#btn-push-arrow")||C()}),typeof window.subscribeI18n=="function"&&window.subscribeI18n("unified-push",L))}function L(){const _=document.getElementById("pn-push-wrap");_&&(_.remove(),e=null,n=0,S())}function D(){const _=document.getElementById("drawer-history-save");if(!_||!_.querySelector("#pn-push-wrap"))return;["btn-push-erp","btn-xero-push"].forEach(I=>{_.querySelectorAll("#"+I).forEach(M=>{M.style.display!=="none"&&(M.style.display="none")})});const g=_.querySelectorAll("#pn-push-wrap");if(g.length>1)for(let I=1;I<g.length;I++)g[I].remove()}function H(){try{const _=function(){return document.getElementById("drawer-body")},g=new MutationObserver(function(){document.getElementById("drawer-history-save")&&!document.getElementById("pn-push-wrap")&&S(),D()}),I=_();if(I)g.observe(I,{childList:!0,subtree:!0});else{const M=new MutationObserver(function(){const P=_();P&&(g.observe(P,{childList:!0,subtree:!0}),M.disconnect())});M.observe(document.body,{childList:!0,subtree:!0})}setTimeout(function(){document.getElementById("drawer-history-save")&&!document.getElementById("pn-push-wrap")&&S(),D()},200)}catch{}}H()})();(function(){function e(){const a=document.getElementById("erp-map-show-advanced-btn");if(!a)return;const o=document.getElementById("erp-map-subtabs");if(!o)return;const s=o.classList.contains("show-advanced"),i=a.querySelector(".erp-map-adv-btn-label");if(i&&typeof t=="function"){const r=s?"erp-map-hide-advanced":"erp-map-show-advanced",u=t(r);u&&u!==r&&(i.textContent=u)}a.setAttribute("aria-pressed",s?"true":"false")}document.addEventListener("click",function(a){if(!a.target.closest("#erp-map-show-advanced-btn"))return;a.preventDefault();const s=document.getElementById("erp-map-subtabs");if(s&&(s.classList.toggle("show-advanced"),e(),!s.classList.contains("show-advanced")&&s.querySelector(".erp-map-subtab.active.erp-map-subtab-advanced"))){const r=s.querySelector('.erp-map-subtab[data-erp-subtab="clients"]');r&&r.click()}});function n(){e()}typeof window.subscribeI18n=="function"&&window.subscribeI18n("erp-map-advanced-toggle",n)})();(function(){const e="pearnly_erp_onboard_shown";let n=!1;function a(){try{return Array.isArray(window._erpEndpoints)&&window._erpEndpoints.length>0}catch{return!1}}function o(){if(document.getElementById("erp-onboard-mask"))return;const i=document.createElement("div");i.id="erp-onboard-mask",i.className="erp-onboard-mask",i.innerHTML='<div class="erp-onboard-modal" role="dialog" aria-modal="true"><div class="erp-onboard-icon"><svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="6" width="24" height="20" rx="3"/><path d="M9 13h14M9 18h10"/><path d="M22 22l3 3 5-5"/></svg></div><div class="erp-onboard-title" id="erp-onboard-title"></div><div class="erp-onboard-body" id="erp-onboard-body"></div><div class="erp-onboard-btns"><button type="button" class="btn btn-secondary" id="erp-onboard-later"></button><button type="button" class="btn btn-primary" id="erp-onboard-ok"></button></div></div>',document.body.appendChild(i);function r(){const l=document.getElementById("erp-onboard-title"),p=document.getElementById("erp-onboard-body"),f=document.getElementById("erp-onboard-ok"),c=document.getElementById("erp-onboard-later");l&&(l.textContent=t("erp-onboard-title")),p&&(p.textContent=t("erp-onboard-body")),f&&(f.textContent=t("erp-onboard-ok")),c&&(c.textContent=t("erp-onboard-later"))}r();function u(){i.style.display="none"}document.getElementById("erp-onboard-ok").addEventListener("click",function(){try{localStorage.setItem(e,"1")}catch{}u();try{const l=document.querySelector('#btn-add-endpoint, [data-action="erp-add-endpoint"]');l&&l.scrollIntoView({behavior:"smooth",block:"center"})}catch{}}),document.getElementById("erp-onboard-later").addEventListener("click",function(){try{localStorage.setItem(e,"1")}catch{}u()}),i.addEventListener("click",function(l){l.target===i&&u()}),typeof window.subscribeI18n=="function"&&window.subscribeI18n("erp-onboard-modal",function(){i.style.display!=="none"&&r()})}function s(){if(!n){try{if(localStorage.getItem(e)==="1")return}catch{}a()||(n=!0,o(),requestAnimationFrame(function(){requestAnimationFrame(function(){if(a())return;const i=document.getElementById("erp-onboard-mask");i&&(i.style.display="flex")})}))}}document.addEventListener("click",function(i){const r=i.target.closest('.auto-nav-item[data-auto-tab="erp"]'),u=i.target.closest('.erp-subtab[data-erp-subtab="connect"]');(r||u)&&setTimeout(s,700)})})();(function(){const e={parse:{zh:"解析文件中",th:"กำลังอ่านไฟล์",en:"Parsing files",ja:"ファイル解析中"},report:{zh:"读取报告中",th:"กำลังอ่านรายงาน",en:"Reading report",ja:"レポート読込中"},reconcile:{zh:"对账中",th:"กำลังกระทบยอด",en:"Reconciling",ja:"照合中"},build:{zh:"生成中",th:"กำลังสร้างไฟล์",en:"Building",ja:"作成中"},persist:{zh:"保存中",th:"กำลังบันทึก",en:"Saving",ja:"保存中"},done:{zh:"完成",th:"เสร็จสิ้น",en:"Done",ja:"完了"}};window._reconProgressText=function(n,a){n=n||{},a=window._currentLang||a||localStorage.getItem("mrpilot_lang")||"th",e.parse[a]||(a="th");const o=n.stage||"parse",s=e[o]||e.parse,i=s[a]||s.th||s.en,r=n.stage_total,u=n.stage_done;if(o==="parse"&&Number.isFinite(r)&&r>0){const l={zh:"共 {d}/{t} 个文件",th:"{d}/{t} ไฟล์",en:"{d}/{t} files",ja:"{d}/{t} ファイル"}[a]||"{d}/{t} files";return i+" · "+l.replace("{d}",u||0).replace("{t}",r)}return i},window._reconPollJob=async function(n,a,o){o=o||{};const s=o.intervalMs||1500,i=o.maxMs||1200*1e3,r=Date.now();let u=0;for(;;){let l=null;try{const p=await fetch("/api/recon/jobs/"+encodeURIComponent(n),{headers:{Authorization:"Bearer "+a}});try{l=await p.json()}catch{l=null}(!p.ok||!l||!l.ok)&&(l=null)}catch{l=null}if(l){if(u=0,o.onProgress)try{o.onProgress(l.progress||{},l)}catch{}if(l.status==="done"||l.status==="failed"||l.status==="needs_review"||l.status==="needs_mapping")return l}else if(++u>=10)return{ok:!1,status:"failed",error_code:"poll_unreachable"};if(Date.now()-r>i)return{ok:!1,status:"timeout",error_code:"timeout"};await new Promise(p=>setTimeout(p,s))}}})();(function(){let e=!1,n=[],a=[],o=null,s="all",i=[],r={stmt:"",gl:""},u=[];const l=k=>document.getElementById(k);function p(k){if(k==null)return"—";const T=Number(k);return isNaN(T)?"—":T.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function f(k){return k?String(k).slice(0,10).split("-").reverse().join("/"):"—"}function c(k){return String(k||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function d(k,T){T=window._currentLang||T||"th";const q={stmt_headers_not_found:{zh:"认不出银行账单表头 · 请确认文件含日期/金额/余额列,或转成清晰的 Excel/CSV 重传",th:"หาหัวตารางบัญชีธนาคารไม่เจอ · ตรวจสอบว่ามีคอลัมน์ วันที่/จำนวนเงิน/ยอดคงเหลือ หรือแปลงเป็น Excel/CSV แล้วอัปโหลดใหม่",en:"Cannot detect bank statement headers · ensure the file has date/amount/balance columns, or re-upload as a clean Excel/CSV",ja:"銀行明細のヘッダーを認識できません · 日付/金額/残高列を確認するか、Excel/CSVに変換して再アップロードしてください"},gl_headers_not_found:{zh:"认不出总账表头 · 请确认文件含日期/科目/借方/贷方列,或转成清晰的 Excel/CSV 重传",th:"หาหัวตาราง GL ไม่เจอ · ตรวจสอบว่ามีคอลัมน์ วันที่/บัญชี/เดบิต/เครดิต หรือแปลงเป็น Excel/CSV แล้วอัปโหลดใหม่",en:"Cannot detect GL headers · ensure the file has date/account/debit/credit columns, or re-upload as a clean Excel/CSV",ja:"GLのヘッダーを認識できません · 日付/科目/借方/貸方列を確認するか、Excel/CSVに変換して再アップロードしてください"},stmt_no_rows:{zh:"文件里没有交易数据 · 请确认上传了正确的银行流水,或换更清晰的版本重传",th:"ไม่พบรายการธุรกรรมในไฟล์ · ตรวจสอบว่าอัปโหลด statement ที่ถูกต้อง หรือใช้เวอร์ชันที่ชัดเจนกว่า",en:"No transaction rows found · please upload the correct statement, or try a clearer version",ja:"取引データが見つかりません · 正しい明細をアップロードするか、より鮮明なファイルでお試しください"},no_rows:{zh:"解析后没有可对账的数据行 · 请确认文件内容完整,或换清晰版本重传",th:"ไม่มีแถวข้อมูลให้กระทบยอดหลังการอ่าน · ตรวจสอบความสมบูรณ์ของไฟล์ หรืออัปโหลดใหม่",en:"No reconcilable rows after parsing · check the file is complete, or re-upload a clearer version",ja:"解析後に照合可能な行がありません · ファイルの完全性を確認するか再アップロードしてください"},file_unreadable:{zh:"文件无法读取 · 可能已损坏或被加密 · 请换文件或转 PDF/Excel 重传",th:"อ่านไฟล์ไม่ได้ · อาจเสียหายหรือถูกเข้ารหัส · ลองไฟล์อื่นหรือแปลงเป็น PDF/Excel",en:"File cannot be read · may be corrupted or encrypted · try another file or convert to PDF/Excel",ja:"ファイルを読み取れません · 破損または暗号化の可能性 · 別ファイルまたはPDF/Excelに変換してください"},file_not_supported:{zh:"不支持此文件类型 · 请上传 PDF / 图片 / Excel / CSV",th:"ไม่รองรับไฟล์ประเภทนี้ · กรุณาอัปโหลด PDF / รูปภาพ / Excel / CSV",en:"File type not supported · please upload PDF / image / Excel / CSV",ja:"このファイル形式は非対応 · PDF / 画像 / Excel / CSV をアップロードしてください"},ocr_failed:{zh:"文件识别失败 · 请尝试更清晰的版本,或转成 PDF / Excel / CSV 重传",th:"อ่านไฟล์ไม่ออก · ลองเวอร์ชันที่ชัดเจนกว่า หรือแปลงเป็น PDF / Excel / CSV",en:"Could not read the file · try a clearer version, or convert to PDF / Excel / CSV",ja:"読み取りに失敗 · より鮮明なファイルか、PDF / Excel / CSV に変換して再試行してください"}},G={zh:"解析失败 · 请换更清晰的文件,或转成 Excel / CSV 后重新上传",th:"อ่านไฟล์ไม่สำเร็จ · ลองไฟล์ที่ชัดเจนกว่า หรือแปลงเป็น Excel / CSV แล้วอัปโหลดใหม่",en:"Parsing failed · try a clearer file, or convert to Excel / CSV and re-upload",ja:"解析に失敗しました · より鮮明なファイルか、Excel / CSV に変換して再アップロードしてください"},O=q[k]||G;return O[T]||O.th||O.en}function v(k){const T=k==="stmt"?n:a,q=l(`brv2-${k}-name`);if(q)if(T.length===0)q.textContent="";else{const O=window._currentLang||"zh",y={zh:"个文件",th:" ไฟล์",en:" file(s)",ja:" ファイル"};q.textContent=T.length+(y[O]||" 个文件")}const G=l("brv2-preview-panel");G&&G.style.display!=="none"&&E(k),w()}function w(){const k=l("brv2-toggle-preview"),T=l("brv2-preview-panel"),q=n.length+a.length>0;k&&(k.style.display=q?"":"none"),!q&&T&&(T.style.display="none",k&&k.classList.remove("open"))}function C(){E("stmt"),E("gl")}function E(k){const T=l(k==="stmt"?"brv2-pp-stmt-col":"brv2-pp-gl-col");if(!T)return;const q=k==="stmt"?n:a,G=window._currentLang||"zh",O={stmt:{zh:"① 银行账单",th:"① บัญชีธนาคาร",en:"① Bank Stmt",ja:"① 銀行明細"},gl:{zh:"② 总账 GL",th:"② GL รายงาน",en:"② GL Report",ja:"② GL帳簿"}},y=(O[k]||{})[G]||O[k].zh,N=c(window.t&&window.t("vex-preview-search")||"搜索文件名..."),V=c(window.t&&window.t("vex-preview-clear-all")||"全清"),J=r[k]||"";T.innerHTML='<div class="vex-pp-col-title"><span class="vex-pp-col-name">'+c(y)+' <span class="vex-pp-col-count">'+q.length+'</span></span></div><div class="vex-pp-search-row"><input class="vex-pp-search" id="brv2-pp-search-'+k+'" type="text" placeholder="'+N+'" value="'+c(J)+'" autocomplete="off"><button class="vex-pp-clear-btn" id="brv2-pp-clearall-'+k+'" type="button">'+V+'</button></div><div class="vex-pp-file-list" id="brv2-pp-'+k+'-list"></div><div class="vex-pp-pagination" id="brv2-pp-'+k+'-pg"></div>';const X=l("brv2-pp-search-"+k);X&&X.addEventListener("input",function(ie){r[k]=ie.target.value,m(k)});const de=l("brv2-pp-clearall-"+k);de&&de.addEventListener("click",function(){k==="stmt"?n.length=0:a.length=0,v(k),j()}),m(k)}function m(k){const T=l("brv2-pp-"+k+"-list"),q=l("brv2-pp-"+k+"-pg");if(!T)return;const G=k==="stmt"?n:a,O=(r[k]||"").toLowerCase(),y=O?G.filter(J=>J.name.toLowerCase().includes(O)):G.slice(),N='<svg class="vex-pp-fi-ico" viewBox="0 0 14 16" fill="none" stroke="currentColor" stroke-width="1.4" width="12" height="14"><path d="M3 1h6l3 3v11H3V1z"/><path d="M9 1v3h3"/></svg>',V='<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" width="11" height="11"><path d="M2 4h10M5 4V2h4v2M5.5 7v4M8.5 7v4M3 4l1 8h6l1-8"/></svg>';if(T.innerHTML=y.map((J,X)=>'<div class="vex-pp-file-row">'+N+'<span class="vex-pp-fi-name" title="'+c(J.name)+'">'+c(J.name)+'</span><span class="vex-pp-fi-size">'+h(J.size)+'</span><button class="vex-pp-fi-del" type="button" data-zone="'+k+'" data-idx="'+G.indexOf(J)+'" aria-label="remove">'+V+"</button></div>").join(""),T.querySelectorAll(".vex-pp-fi-del").forEach(function(J){J.addEventListener("click",function(){const X=parseInt(J.dataset.idx,10);J.dataset.zone==="stmt"?n.splice(X,1):a.splice(X,1),v(J.dataset.zone),j()})}),q){const J=window.t&&window.t("vex-preview-count")||"显示 {n} / 共 {m}";q.textContent=J.replace("{n}",y.length).replace("{m}",G.length)}}function h(k){return k?k<1024?k+" B":k<1048576?(k/1024).toFixed(1)+" KB":(k/1048576).toFixed(1)+" MB":""}var S="pearnly.brv2.lastAnchorOcr";function L(k){try{var T=k&&k._anchor_ocr;if(!T||typeof T!="object")return;var q={stmt_opening:Number.isFinite(+T.stmt_opening)?+T.stmt_opening:null,gl_opening:Number.isFinite(+T.gl_opening)?+T.gl_opening:null,gl_closing:Number.isFinite(+T.gl_closing)?+T.gl_closing:null,stmt_closing:Number.isFinite(+T.stmt_closing)?+T.stmt_closing:null,ts:Date.now()};localStorage.setItem(S,JSON.stringify(q))}catch{}}function D(){try{var k=localStorage.getItem(S);if(!k)return null;var T=JSON.parse(k);return!T||typeof T!="object"?null:T}catch{return null}}function H(){var k=D();if(k){var T={"brv2-anchor-stmt-opening":k.stmt_opening,"brv2-anchor-gl-opening":k.gl_opening,"brv2-anchor-gl-closing":k.gl_closing,"brv2-anchor-stmt-closing":k.stmt_closing},q=0;Object.keys(T).forEach(function(V){var J=document.getElementById(V);if(J&&J.value===""){var X=T[V];if(Number.isFinite(X)){J.value=X.toFixed(2);var de=J.closest&&J.closest(".brv2-anchor-cell");de&&de.classList.add("is-prefilled"),q+=1}}});var G=document.getElementById("brv2-anchor-eq"),O=document.getElementById("brv2-anchor-eq-val");if(G&&O&&Number.isFinite(k.stmt_opening)&&Number.isFinite(k.gl_opening)){var y=k.stmt_opening-k.gl_opening;O.textContent=y.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}),G.style.display=""}if(q>0){var N=document.getElementById("brv2-anchor-prefill-banner");N&&N.classList.add("show")}}}function _(){var k=document.getElementById("brv2-anchor-prefill-banner");if(k){var T=!1;["brv2-anchor-gl-closing","brv2-anchor-stmt-closing","brv2-anchor-stmt-opening","brv2-anchor-gl-opening"].forEach(function(q){var G=document.getElementById(q);if(G){var O=G.closest&&G.closest(".brv2-anchor-cell");O&&O.classList.contains("is-prefilled")&&(T=!0)}}),k.classList.toggle("show",T)}}var g=[["stmt_opening","brv2-anchor-stmt-opening"],["gl_opening","brv2-anchor-gl-opening"],["gl_closing","brv2-anchor-gl-closing"],["stmt_closing","brv2-anchor-stmt-closing"]];function I(k,T){return window.t&&window.t(k)||T}function M(k){return String(k??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function P(k){return Number.isFinite(+k)?(+k).toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}):"—"}function R(k){var T=document.getElementById("brv2-summary-collapse");if(!(!T||!T.parentNode)){var q=document.getElementById("brv2-anchor-audit"),G=k&&k._anchor_overrides;if(!G||typeof G!="object"||Object.keys(G).length===0){q&&q.parentNode&&q.parentNode.removeChild(q);return}q||(q=document.createElement("div"),q.id="brv2-anchor-audit",q.style.cssText="margin-top:14px;background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:14px 16px;",T.parentNode.insertBefore(q,T.nextSibling));var O=g.map(function(y){var N=G[y[0]];if(!N)return"";var V=+N.ocr||0,J=+N.user||0,X=J-V,de=X>0?"+":(X<0,""),ie=Math.abs(X)<.005?"#6b7280":X>0?"#16a34a":"#dc2626";return'<tr><td style="padding:6px 10px;color:#111827;font-size:13px">'+M(I(y[1],y[0]))+'</td><td style="padding:6px 10px;color:#6b7280;font-size:13px;text-align:right;font-variant-numeric:tabular-nums">'+M(P(V))+'</td><td style="padding:6px 10px;background:#fef08a;color:#92400e;font-weight:600;font-size:13px;text-align:right;font-variant-numeric:tabular-nums">'+M(P(J))+'</td><td style="padding:6px 10px;color:'+ie+';font-weight:500;font-size:13px;text-align:right;font-variant-numeric:tabular-nums">'+M(de+P(X))+"</td></tr>"}).join("");q.innerHTML='<div style="font-weight:600;color:#92400e;font-size:13px;margin-bottom:10px">'+M(I("brv2-anchor-audit-title","⚠ This run contains manually entered anchors"))+'</div><table style="width:100%;border-collapse:collapse;font-family:inherit"><thead><tr><th style="padding:6px 10px;text-align:left;color:#6b7280;font-size:11px;font-weight:500;border-bottom:1px solid #fed7aa">'+M(I("brv2-anchor-audit-col-field","Field"))+'</th><th style="padding:6px 10px;text-align:right;color:#6b7280;font-size:11px;font-weight:500;border-bottom:1px solid #fed7aa">'+M(I("brv2-anchor-audit-col-ocr","OCR"))+'</th><th style="padding:6px 10px;text-align:right;color:#6b7280;font-size:11px;font-weight:500;border-bottom:1px solid #fed7aa">'+M(I("brv2-anchor-audit-col-user","User"))+'</th><th style="padding:6px 10px;text-align:right;color:#6b7280;font-size:11px;font-weight:500;border-bottom:1px solid #fed7aa">'+M(I("brv2-anchor-audit-col-diff","Diff"))+"</th></tr></thead><tbody>"+O+"</tbody></table>"}}function B(){const k=l("brv2-toggle-preview");k&&!k._reconBound&&(k._reconBound=!0,k.addEventListener("click",function(){const T=l("brv2-preview-panel"),q=l("brv2-toggle-preview-label"),G=T&&T.style.display!=="none";T&&(T.style.display=G?"none":""),k.classList.toggle("open",!G),q&&(q.textContent=G?window.t&&window.t("vex-toggle-preview-open")||"查看清单":window.t&&window.t("vex-toggle-preview-close")||"收起清单"),G||C()}))}function j(){const k=l("brv2-run-btn"),T=l("brv2-status"),q=n.length>0,G=a.length>0;if(k&&(k.disabled=!(q&&G)),T){const O=window._currentLang||"zh";if(!q&&!G){const y={zh:"请上传银行账单和 GL 文件",th:"กรุณาอัปโหลดบัญชีธนาคารและ GL",en:"Upload bank statement and GL files",ja:"銀行明細と GL を両方アップロードしてください"};T.textContent=y[O]||y.zh}else if(q)if(G){const y={zh:"两份文件已就绪",th:"พร้อมสอบทาน",en:"Ready to reconcile",ja:"照合を開始できます"};T.textContent=y[O]||y.zh}else{const y={zh:"还缺 GL 文件",th:"ยังขาดไฟล์ GL",en:"Missing GL file",ja:"GL ファイルが未アップロード"};T.textContent=y[O]||y.zh}else{const y={zh:"还缺银行账单 PDF",th:"ยังขาดไฟล์บัญชีธนาคาร PDF",en:"Missing bank statement PDF",ja:"銀行明細 PDF が未アップロード"};T.textContent=y[O]||y.zh}}}function Y(k,T,q){const G=l(k),O=l(T);!G||!O||(G.addEventListener("click",()=>O.click()),G.addEventListener("keydown",y=>{(y.key==="Enter"||y.key===" ")&&(y.preventDefault(),O.click())}),G.addEventListener("dragover",y=>{y.preventDefault(),G.classList.add("drag-over")}),G.addEventListener("dragleave",()=>G.classList.remove("drag-over")),G.addEventListener("drop",y=>{y.preventDefault(),G.classList.remove("drag-over");const N=Array.from(y.dataTransfer.files||[]);q==="stmt"?n.push(...N):a.push(...N),v(q),j()}),O.addEventListener("change",()=>{const y=Array.from(O.files||[]);q==="stmt"?n.push(...y):a.push(...y),O.value="",v(q),j()}))}function A(k){const T=l("brv2-progress"),q=l("brv2-run-btn"),G=l("brv2-error");T&&(T.style.display=k?"":"none"),q&&(q.disabled=k),G&&(G.style.display="none")}function W(k){const T=l("brv2-error");T&&(T.textContent=k,T.style.display="",T.scrollIntoView({behavior:"smooth",block:"nearest"})),A(!1),j(),window.showToast&&window.showToast(k,"error")}async function ee(){if(n.length===0||a.length===0)return;const k=localStorage.getItem("mrpilot_token")||"",T=window._currentLang||"zh",q=(l("brv2-acct-select")||{}).value||"";$(!1),A(!0);try{const G=new FormData;n.forEach(ae=>G.append("stmt_files",ae)),a.forEach(ae=>G.append("gl_files",ae)),G.append("gl_account",q),G.append("lang",T);const O=parseFloat((l("brv2-anchor-gl-closing")||{}).value),y=parseFloat((l("brv2-anchor-stmt-closing")||{}).value),N=parseFloat((l("brv2-anchor-stmt-opening")||{}).value),V=parseFloat((l("brv2-anchor-gl-opening")||{}).value);Number.isFinite(O)&&G.append("gl_closing_override",O),Number.isFinite(y)&&G.append("stmt_closing_override",y),Number.isFinite(N)&&G.append("stmt_opening_override",N),Number.isFinite(V)&&G.append("gl_opening_override",V);const J=await fetch("/api/recon/bank-v2/submit",{method:"POST",headers:{Authorization:"Bearer "+k},body:G});let X=null;try{X=await J.json()}catch{X=null}if(X&&X.needs_mapping){A(!1),window.ReconMapping?window.ReconMapping.show(X,{token:k,lang:T,onConfirmed:function(){ee()}}):W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}if(!J.ok||!X||!X.ok||!X.job_id){A(!1),X&&(X.detail||X.error)?W(_humanizeBackendError(X.detail||X.error,"Error "+J.status)):W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}const de=l("brv2-progress-sub"),ie=await window._reconPollJob(X.job_id,k,{onProgress:ae=>{de&&(de.textContent=window._reconProgressText(ae,T))}});if(ie&&ie.status==="needs_mapping"&&ie.mapping){A(!1),window.ReconMapping?window.ReconMapping.show(ie.mapping,{token:k,lang:T,onConfirmed:function(){ee()}}):W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}if(ie&&ie.status==="needs_review"&&ie.review){A(!1),window.ReconReview?window.ReconReview.show(ie.review,{token:k,lang:T,jobId:X.job_id,onConfirmed:async function(ae){A(!0);const pe=await window._reconPollJob(ae,k,{onProgress:fe=>{de&&(de.textContent=window._reconProgressText(fe,T))}});await ce(pe)}}):W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}if(ie&&ie.status==="failed"){A(!1),W(d(ie.error_code,T));return}await ce(ie);async function ce(ae){try{if(!ae||ae.status!=="done"||!ae.result_id){A(!1),W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}const pe=await fetch("/api/recon/bank-v2/"+encodeURIComponent(ae.result_id),{headers:{Authorization:"Bearer "+k}});let fe=null;try{fe=await pe.json()}catch{fe=null}if(!pe.ok||fe===null||!fe.ok){A(!1),W(t("brv2-err-server")||"服务器繁忙,请稍后重试");return}(fe.gl_accounts||[]).length>1&&oe(fe.gl_accounts),o=fe,i=fe.detail||[],s="all",document.querySelectorAll(".brv2-filter-btn").forEach(ge=>ge.classList.toggle("active",ge.dataset.filter==="all")),L(fe&&fe.summary),A(!1),F(fe),Z();const me=l("brv2-summary-collapse");me&&me.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(pe){A(!1),W(pe.message||"Network error")}}}catch(G){W(G.message||"Network error")}}function oe(k){const T=l("brv2-acct-select");if(!T)return;const q=window._currentLang||"zh",G={zh:"全部账户",th:"ทุกบัญชี",en:"All Accounts",ja:"すべての口座"}[q]||"全部账户";T.innerHTML=`<option value="">${G}</option>`+k.map(O=>`<option value="${c(O)}">${c(O)}</option>`).join(""),T.style.display=""}function $(k){const T=l("brv2-summary-collapse"),q=l("brv2-detail-collapse"),G=l("brv2-export-btn"),O=l("brv2-new-btn"),y=l("brv2-parse-info-wrap");T&&(T.style.display=k?"":"none"),q&&(q.style.display=k?"":"none"),G&&(G.style.display=k?"":"none"),O&&(O.style.display=k?"":"none"),!k&&y&&(y.style.display="none");const N=l("brv2-warnings");!k&&N&&(N.style.display="none",N.innerHTML="")}function x(k){const T=l("brv2-parse-info-wrap"),q=l("brv2-parse-info-body");if(!T||!q)return;const G=k.parse_info;if(!G){T.style.display="none";return}const O=window._currentLang||"zh",y={title:{zh:"文件解析状态",th:"สถานะการอ่านไฟล์",en:"File Parse Status",ja:"ファイル解析状態"},type:{zh:"类型",th:"ประเภท",en:"Type",ja:"種別"},file:{zh:"文件名",th:"ชื่อไฟล์",en:"File",ja:"ファイル"},rows:{zh:"解析行数",th:"แถวที่พบ",en:"Rows Found",ja:"解析行数"},bank:{zh:"银行/科目",th:"ธนาคาร/บัญชี",en:"Bank/Account",ja:"銀行/科目"},status:{zh:"状态",th:"สถานะ",en:"Status",ja:"状態"},stmt:{zh:"账单",th:"บัญชีธนาคาร",en:"Stmt",ja:"明細"},gl:{zh:"总账GL",th:"GL",en:"GL",ja:"GL"},ok:{zh:"✓ 成功",th:"✓ สำเร็จ",en:"✓ OK",ja:"✓ 成功"},warn:{zh:"⚠ 0行",th:"⚠ 0 แถว",en:"⚠ 0 rows",ja:"⚠ 0行"},fail:{zh:"✗ 失败",th:"✗ ล้มเหลว",en:"✗ Failed",ja:"✗ 失敗"}},N=ce=>(y[ce]||{})[O]||(y[ce]||{}).zh||ce,V=[...(G.stmt_files||[]).map(ce=>({...ce,_type:"stmt",_extra:ce.bank_code||""})),...(G.gl_files||[]).map(ce=>({...ce,_type:"gl",_extra:(ce.accounts||[]).join(", ")}))],J={stmt_headers_not_found:{zh:"认不出表头列 · 请确认文件含日期/金额/余额列",th:"หาคอลัมน์หัวตารางไม่เจอ · ตรวจสอบไฟล์มีวันที่/จำนวนเงิน/ยอดคงเหลือ",en:"Cannot detect column headers · ensure file has date/amount/balance columns",ja:"列ヘッダーが認識できません · 日付/金額/残高列を確認してください"},stmt_no_rows:{zh:"文件里没有交易数据 · 请确认上传了正确的银行流水",th:"ไม่พบรายการธุรกรรมในไฟล์ · ตรวจสอบว่าอัปโหลด statement ที่ถูกต้อง",en:"No transaction rows found · please check the file",ja:"取引データが見つかりません · ファイルを確認してください"},file_not_supported:{zh:"不支持此文件类型 · 请上传 PDF / 图片 / Excel / CSV",th:"ไม่รองรับไฟล์ประเภทนี้ · กรุณาอัปโหลด PDF / รูปภาพ / Excel / CSV",en:"File type not supported · please upload PDF / image / Excel / CSV",ja:"このファイル形式は非対応 · PDF / 画像 / Excel / CSV をアップロード"},file_unreadable:{zh:"文件无法读取 · 可能已损坏或被加密",th:"อ่านไฟล์ไม่ได้ · อาจเสียหายหรือถูกเข้ารหัส",en:"File cannot be read · may be corrupted or encrypted",ja:"ファイルを読み取れません · 破損または暗号化の可能性"},ocr_failed:{zh:"文件识别失败 · 请尝试更清晰的版本或换 PDF 格式重传",th:"อ่านไฟล์ไม่ออก · ลองเวอร์ชันที่ชัดเจนกว่าหรือเปลี่ยนเป็น PDF",en:"Could not read file · try a clearer version or upload as PDF",ja:"読み取り失敗 · より鮮明なファイルまたは PDF 形式で再試行"},gl_headers_not_found:{zh:"认不出总账表头 · 请确认文件含科目/借方/贷方列",th:"หาหัวคอลัมน์ GL ไม่เจอ · ตรวจสอบมีคอลัมน์บัญชี/เดบิต/เครดิต",en:"Cannot detect GL column headers · ensure account/debit/credit columns exist",ja:"GL 列ヘッダー認識不可 · 科目/借方/貸方列を確認してください"}},X=ce=>{const ae=String(ce||"");return/Cannot detect bank statement column headers/i.test(ae)?"stmt_headers_not_found":/Cannot detect GL column headers/i.test(ae)?"gl_headers_not_found":/No transaction rows found|no pages parsed/i.test(ae)?"stmt_no_rows":/unsupported format/i.test(ae)?"file_not_supported":/Cannot read Excel|file_unreadable/i.test(ae)?"file_unreadable":/Gemini.*invalid JSON|Gemini.*parsed but failed|validation errors|BankStatementDocument schema|layer2:|layer1:/i.test(ae)?"ocr_failed":null},de=ce=>{const ae=ce.error_code||X(ce.error);if(ae&&J[ae]){const pe=window._currentLang||"zh";return J[ae][pe]||J[ae].zh}return String(ce.error||"").slice(0,80)},ie=ce=>!ce.ok&&ce.error?`<span style="color:#dc2626">${N("fail")} — ${c(de(ce))}</span>`:ce.rows?`<span style="color:#059669">${N("ok")} (${ce.rows})</span>`:`<span style="color:#d97706">${N("warn")}</span>`;q.innerHTML=`
            ...
            <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:4px">
            ...
            </table>`,T.style.display=""}async function b(k){const T=localStorage.getItem("mrpilot_token")||"",q=window._currentLang||"zh";try{const G=await fetch("/api/recon/bank-v2/"+k+"/export?lang="+q,{headers:{Authorization:"Bearer "+T}});if(!G.ok){const de=await G.json().catch(()=>({}));window.showToast&&window.showToast(de.detail||"Export failed","error");return}const O=await G.blob(),N=(G.headers.get("content-disposition")||"").match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/),V=N?N[1].replace(/['"]/g,""):"reconciliation.xlsx",J=URL.createObjectURL(O),X=document.createElement("a");X.href=J,X.download=V,document.body.appendChild(X),X.click(),document.body.removeChild(X),URL.revokeObjectURL(J)}catch(G){window.showToast&&window.showToast("Export error: "+G.message,"error")}}function z(k,T){const q=l("brv2-summary-collapse");let G=l("brv2-warnings");const O=window._currentLang||"zh",y={zh:"⏭ 已跳过无法识别的文件:",th:"⏭ ข้ามไฟล์ที่อ่านไม่ได้:",en:"⏭ Skipped unreadable file:",ja:"⏭ 読み取れないファイルをスキップ:"}[O]||"⏭ ",N=[];if((T||[]).forEach(V=>N.push(y+" "+V)),(k||[]).forEach(V=>N.push(V)),!N.length){G&&(G.style.display="none");return}if(!G)if(G=document.createElement("div"),G.id="brv2-warnings",q&&q.parentNode)q.parentNode.insertBefore(G,q);else return;G.style.cssText="display:block;margin:10px 0;padding:10px 14px;background:#FEF3C7;border:1px solid #FCD34D;border-radius:8px;color:#92400E;font-size:13px;line-height:1.6",G.innerHTML=N.map(V=>"<div>"+c(V)+"</div>").join("")}function F(k){x(k),z(k.warnings||[],k.skipped_files||[]),!k.ok&&k.error&&window.showToast&&window.showToast(k.error,"error");const T=k.stats||{},q=k.summary||{},G=T.matched||0,O=(T.gl_debit_only||0)+(T.gl_credit_only||0),y=(T.stmt_withdrawal_only||0)+(T.stmt_deposit_only||0),N=Number(q.formula_diff||0),V=Math.abs(N)<.05;l("brv2-kpi-matched")&&(l("brv2-kpi-matched").textContent=G),l("brv2-kpi-diff")&&(l("brv2-kpi-diff").textContent=p(N)),l("brv2-kpi-unmatched")&&(l("brv2-kpi-unmatched").textContent=O+y);const J=l("brv2-kpi-diff-icon");J&&(J.style.background=V?"#d1fae5":"#fee2e2",J.style.color=V?"#065f46":"#b91c1c");const X=l("brv2-formula-sub");if(X){const pe=window._currentLang||"zh";X.textContent=V?{zh:"✓ 平衡",th:"✓ สมดุล",en:"✓ Balanced",ja:"✓ 一致"}[pe]||"✓ 平衡":({zh:"差 ",th:"ต่าง ",en:"Diff ",ja:"差 "}[pe]||"差 ")+p(N)}const de=l("brv2-detail-sub");if(de){const pe=window._currentLang||"zh",fe={zh:"共 {n} 行",th:"ทั้งหมด {n} แถว",en:"{n} rows",ja:"計 {n} 行"}[pe]||"共 {n} 行";de.textContent=fe.replace("{n}",i.length)}function ie(pe,fe,me){const ge=l(pe);ge&&(ge.textContent=(me&&fe>0?"(":"")+p(me?-fe:fe)+(me&&fe>0?")":""))}ie("brf-gl-close",q.gl_closing||0),ie("brf-open-diff",q.opening_diff||0),ie("brf-gl-debit-only",q.gl_debit_only_amount||0,!0),ie("brf-gl-credit-only",q.gl_credit_only_amount||0),ie("brf-stmt-wd-only",q.stmt_withdrawal_only_amount||0,!0),ie("brf-stmt-dep-only",q.stmt_deposit_only_amount||0),ie("brf-calc-close",q.formula_stmt_closing||0),ie("brf-stmt-close",q.stmt_closing||0),l("brf-diff")&&(l("brf-diff").textContent=p(N));const ce=l("brv2-fcell-diff");ce&&ce.classList.toggle("brv2-fcell-diff-ok",V);const ae=l("brv2-export-btn");ae&&(ae.onclick=()=>{o&&b(o.task_id)}),R(q),$(!0),K()}function K(){const k=l("brv2-tbody");if(!k)return;const T=i.filter(y=>s==="all"?!0:s==="matched"?y.match_status==="matched":s==="gl_only"?y.match_status.startsWith("gl_"):s==="stmt_only"?y.match_status.startsWith("stmt_"):!0);if(T.length===0){const y={zh:"无记录",th:"ไม่มีรายการ",en:"No rows",ja:"行なし"}[window._currentLang||"zh"]||"无记录";k.innerHTML=`<tr><td colspan="10" style="text-align:center;padding:20px;color:var(--ink-3)">${y}</td></tr>`;return}const q=window._currentLang||"zh",G={zh:"OCR 余额验证未通过 · 上一行余额 ± 金额 ≠ 本行余额，请核对原 PDF",th:"การตรวจสอบยอดคงเหลือไม่ผ่าน · ยอดก่อนหน้า ± จำนวน ≠ ยอดบรรทัดนี้ โปรดตรวจสอบ PDF ต้นฉบับ",en:"Balance check FAILED · prev_balance ± amount ≠ this row balance — verify against the original PDF",ja:"残高検証エラー · 前残高 ± 金額 ≠ この行残高 — 元のPDFと照合してください"}[q],O={zh:"OCR 低置信度 · 数字模糊或难以辨认，请核对原 PDF",th:"OCR ความมั่นใจต่ำ · ตัวเลขเบลอหรืออ่านยาก โปรดตรวจสอบ PDF ต้นฉบับ",en:"OCR low confidence · digit was blurry or hard to read — verify against the original PDF",ja:"OCR信頼度低 · 数字がぼやけている — 元のPDFと照合してください"}[q];k.innerHTML=T.map(y=>{const N=y.match_status,V=y.match_layer;let J="",X="";N==="matched"?(V===1&&(J="matched",X='<span class="brv2-status-badge brv2-badge-matched">L1 ✓</span>'),V===2&&(J="matched-l2",X='<span class="brv2-status-badge brv2-badge-matched-l2">L2 ~</span>'),V===3&&(J="matched-l3",X='<span class="brv2-status-badge brv2-badge-matched-l3">L3 ?</span>')):N==="gl_debit_only"||N==="gl_credit_only"?(J="gl-only",X='<span class="brv2-status-badge brv2-badge-gl-only">GL</span>'):(J="stmt-only",X=`<span class="brv2-status-badge brv2-badge-stmt-only">${{zh:"账单",th:"บัญชี",en:"Stmt",ja:"明細"}[q]||"账单"}</span>`);let de="";return y.stmt_balance_ok===!1&&(de+=`<span class="brv2-ocr-warn brv2-ocr-warn-bal" title="${c(G)}">⚠</span>`,J+=" brv2-row-warn"),y.stmt_confidence==="low"&&(de+=`<span class="brv2-ocr-warn brv2-ocr-warn-conf" title="${c(O)}">◌</span>`,J.includes("brv2-row-warn")||(J+=" brv2-row-warn-soft")),`<tr class="${J.trim()}">
              <td>${X}${de}</td>
              ...
              <td>${V?"L"+V:"—"}</td>
            </tr>`}).join("")}async function Z(){const k=localStorage.getItem("mrpilot_token")||"";try{const q=await(await fetch("/api/recon/bank-v2/tasks",{headers:{Authorization:"Bearer "+k}})).json();te(q.tasks||[])}catch{const q=l("brv2-history-empty"),G=window._currentLang||"zh",O={zh:"加载失败",th:"โหลดประวัติไม่ได้",en:"Load failed",ja:"読み込み失敗"}[G]||"加载失败";q&&(q.textContent=O,q.style.display="");const y=l("brv2-history-table-wrap");y&&(y.style.display="none")}}const le=10;let re=1;function U(){const k=l("brv2-history-pager"),T=l("brv2-history-pager-info"),q=l("brv2-history-prev"),G=l("brv2-history-next");if(!k)return;if(u.length<=le){k.style.display="none";return}k.style.display="";const O=Math.ceil(u.length/le);T&&(T.textContent=re+" / "+O),q&&(q.disabled=re<=1),G&&(G.disabled=re>=O)}function Q(){const k=l("brv2-history-prev"),T=l("brv2-history-next");k&&!k._brv2Bound&&(k._brv2Bound=!0,k.addEventListener("click",()=>{re>1&&(re--,te(u))})),T&&!T._brv2Bound&&(T._brv2Bound=!0,T.addEventListener("click",()=>{const q=Math.ceil(u.length/le);re<q&&(re++,te(u))}))}function te(k){k!==void 0&&(u=k||[],re=1);const T=u,q=l("brv2-history-empty"),G=l("brv2-history-table-wrap"),O=l("brv2-history-tbody");if(!O)return;const y=window._currentLang||"zh";if(!T.length){const ae={zh:"暂无对账记录",th:"ยังไม่มีประวัติ",en:"No records yet",ja:"記録なし"}[y]||"暂无对账记录";q&&(q.textContent=ae,q.style.display=""),G&&(G.style.display="none"),U();return}q&&(q.style.display="none"),G&&(G.style.display="");const N=Math.ceil(T.length/le);re>N&&(re=N);const V=(re-1)*le,J=T.slice(V,V+le),X=localStorage.getItem("mrpilot_token")||"",de='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><circle cx="8" cy="8" r="6"/><polyline points="6 8 8 10 10 8"/><line x1="8" y1="4" x2="8" y2="10"/></svg>',ie='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><path d="M8 2v9M4 7l4 4 4-4M3 14h10"/></svg>',ce='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><polyline points="3 4 13 4"/><path d="M6 4V2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V4"/><path d="M5 4l1 9a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1l1-9"/></svg>';O.innerHTML="",J.forEach(ae=>{const pe=Number(ae.formula_diff||0),fe=Math.abs(pe)<.05,me=(ae.stmt_files||"").split(";").map(ke=>ke.trim().split(/[/\\]/).pop()).filter(Boolean).join(", "),ge=(ae.gl_files||"").split(";").map(ke=>ke.trim().split(/[/\\]/).pop()).filter(Boolean).join(", "),ye=ae.created_at?String(ae.created_at).slice(0,16).replace("T"," "):"",xe=document.createElement("tr");xe.dataset.taskId=ae.id;const Oe=document.createElement("td");Oe.textContent=ye;const Ce=document.createElement("td");Ce.className="glv-history-file",Ce.title=me+" + "+ge,Ce.textContent=me+" + "+ge;const He=document.createElement("td");He.className="glv-num",He.textContent=(ae.stmt_row_count||0)+" / "+(ae.gl_row_count||0);const rt=document.createElement("td");rt.className="glv-num",rt.textContent=ae.matched_count||0;const lt=document.createElement("td");lt.className="glv-num",lt.textContent=ae.unmatched_gl||0;const ct=document.createElement("td");ct.className="glv-num",ct.textContent=ae.unmatched_stmt||0;const Ve=document.createElement("td");Ve.className="glv-num",Ve.style.color=fe?"#059669":"#dc2626",Ve.textContent=fe?"✓":p(pe);const Ae=document.createElement("td");Ae.className="glv-history-actions";const dt=(ke,Tt,Mt,En)=>{const Ie=document.createElement("button");return Ie.type="button",Ie.title=Tt,Ie.setAttribute("aria-label",Tt),Mt&&(Ie.className=Mt),Ie.innerHTML=ke,Ie.onclick=In=>{In.stopPropagation(),En()},Ie},wn={zh:"删除这条记录?",th:"ลบรายการนี้?",en:"Delete this record?",ja:"この記録を削除しますか?"}[y]||"删除?",_n={zh:"加载",th:"โหลด",en:"Load",ja:"読込"}[y]||"加载",kn={zh:"导出",th:"ส่งออก",en:"Export",ja:"エクスポート"}[y]||"导出",xn={zh:"删除",th:"ลบ",en:"Delete",ja:"削除"}[y]||"删除";Ae.appendChild(dt(de,_n,"",()=>se(ae.id,X))),Ae.appendChild(dt(ie,kn,"",()=>b(ae.id))),Ae.appendChild(dt(ce,xn,"glv-del",async()=>{await showConfirm(wn,{danger:!0})&&(await fetch("/api/recon/bank-v2/"+ae.id,{method:"DELETE",headers:{Authorization:"Bearer "+X}}),Z())})),[Oe,Ce,He,rt,lt,ct,Ve,Ae].forEach(ke=>xe.appendChild(ke)),xe.style.cursor="pointer",xe.addEventListener("click",async ke=>{ke.target.closest(".glv-del")||ke.target.closest("button")||await se(ae.id,X)}),O.appendChild(xe)}),U(),ne()}function ne(){const k=((l("brv2-hist-search")||{}).value||"").trim().toLowerCase(),T=l("brv2-history-tbody");T&&T.querySelectorAll("tr").forEach(q=>{q.dataset.taskId&&(q.style.display=!k||q.textContent.toLowerCase().includes(k)?"":"none")})}async function se(k,T){try{const G=await(await fetch("/api/recon/bank-v2/"+k,{headers:{Authorization:"Bearer "+T}})).json();if(!G.ok)return;o={task_id:G.task_id,...G},i=G.detail||[],s="all",document.querySelectorAll(".brv2-filter-btn").forEach(O=>O.classList.toggle("active",O.dataset.filter==="all")),F(o)}catch{}}function ue(){if(e){Z();return}e=!0,Y("brv2-stmt-zone","brv2-stmt-input","stmt"),Y("brv2-gl-zone","brv2-gl-input","gl");const k=["brv2-anchor-gl-closing","brv2-anchor-stmt-closing","brv2-anchor-stmt-opening","brv2-anchor-gl-opening"];function T(){const V=parseFloat((l("brv2-anchor-stmt-opening")||{}).value),J=parseFloat((l("brv2-anchor-gl-opening")||{}).value),X=l("brv2-anchor-eq"),de=l("brv2-anchor-eq-val");if(!(!X||!de))if(Number.isFinite(V)&&Number.isFinite(J)){const ie=V-J;de.textContent=ie.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}),X.style.display=""}else X.style.display="none"}k.forEach(V=>{const J=l(V);J&&(J.addEventListener("input",T),J.addEventListener("input",()=>{const X=J.closest(".brv2-anchor-cell");X&&X.classList.remove("is-prefilled"),_()}))}),H();const q=l("brv2-run-btn");q&&q.addEventListener("click",ee);const G=l("brv2-reset-btn");G&&G.addEventListener("click",()=>{o=null,i=[],n=[],a=[],v("stmt"),v("gl"),j(),$(!1);const V=l("brv2-acct-select");V&&(V.style.display="none"),k.forEach(de=>{const ie=l(de);if(ie){ie.value="";const ce=ie.closest&&ie.closest(".brv2-anchor-cell");ce&&ce.classList.remove("is-prefilled")}});const J=l("brv2-anchor-eq");J&&(J.style.display="none");const X=l("brv2-anchor-prefill-banner");X&&X.classList.remove("show")});const O=l("brv2-new-btn");O&&O.addEventListener("click",()=>{o=null,i=[],n=[],a=[],v("stmt"),v("gl"),j(),$(!1)});const y=l("brv2-filter-tabs");y&&y.addEventListener("click",V=>{V.stopPropagation();const J=V.target.closest(".brv2-filter-btn");J&&(s=J.dataset.filter,y.querySelectorAll(".brv2-filter-btn").forEach(X=>X.classList.toggle("active",X===J)),K())}),B(),Q();const N=l("brv2-hist-search");N&&N.addEventListener("input",ne),Z(),j(),window._brv2LoadHistory=Z,Array.isArray(window.__i18nSubs)||(window.__i18nSubs=[]),window.__i18nSubs=window.__i18nSubs.filter(V=>V.name!=="brv2"),window.__i18nSubs.push({name:"brv2",fn:function(){j(),v("stmt"),v("gl"),o&&F(o),te()}})}window._loadBankReconV2Panel=function(k){const T=k?document.getElementById(k):null;T&&T.id!=="recon-pane-bank"&&(T.innerHTML=`<div style="padding:16px;font-size:13px;color:var(--ink-3)">
    ...
    `,document.body.appendChild(n),n.addEventListener("click",async a=>{if(a.target===n||a.target.classList.contains("log-detail-close")){n.remove();return}const o=a.target.closest("[data-receipt-copy]");if(o){vn(o.dataset.receiptCopy);return}const s=a.target.closest("[data-receipt-action]");if(s){const i=s.dataset.receiptAction;i==="retry"?window.retryPushLog(s.dataset.logId):i==="exceptions"?typeof routeTo=="function"&&routeTo("exceptions"):i==="mappings"&&typeof routeTo=="function"&&routeTo("integrations"),n.remove();return}});try{const a=await fetch(`/api/erp/logs/${encodeURIComponent(e)}`,{headers:{Authorization:"Bearer "+token}});if(!a.ok){n.remove();return}const o=await a.json(),s=window._erpEndpoints.find(R=>R.id===o.endpoint_id),i=o.endpoint_name||(s?s.name:o.endpoint_id?t("erp-log-endpoint-deleted"):"-"),r=(o.endpoint_adapter||s&&s.adapter||"").toLowerCase(),u=new Date(o.created_at).toLocaleString(),l=o.trigger==="auto"?t("log-tag-auto"):o.trigger==="retry"?t("log-tag-retry"):t("log-tag-manual"),p=o.request_body?JSON.stringify(o.request_body,null,2):t("erp-receipt-no-tech"),f=o.response_body||t("erp-receipt-no-tech"),c=o.status==="success";let d=typeof f=="string"?f:JSON.stringify(f,null,2);if(c)try{const R=typeof o.response_body=="string"?JSON.parse(o.response_body):o.response_body||{},B=R.row_count||(Array.isArray(R.imported_rows)?R.imported_rows.length:0);B>0&&(d=t("log-push-rows").replace("{n}",String(B)))}catch{}const v=(o.external_doc_no||"").trim(),w=(o.external_url||"").trim(),C=(o.external_doc_hint||"").trim(),E=(o.ocr_buyer_name||"").trim()||o.client_name||"-",m=o.seller_name||"-",h=o.push_type==="id_card";let S="-";const L=Number(o.total_amount);o.total_amount!=null&&o.total_amount!==""&&!isNaN(L)&&(S=L.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2}));const D=c?t("erp-receipt-title-ok"):t("erp-receipt-title-fail"),H=c?"✓":"✗",_=[],g=(R,B)=>{_.push(`
                <div class="erp-receipt-row">
                    <span class="erp-receipt-key">${escapeHtml(R)}</span>
                    <span class="erp-receipt-val">${B}</span>
                </div>`)};if(g(h?t("erp-log-col-booking"):t("erp-receipt-invoice-no"),`<strong>${escapeHtml(o.invoice_no||"-")}</strong>`),g(t("erp-receipt-erp-name"),escapeHtml(i)),c){let R;v?R=`<strong class="erp-receipt-docno">${escapeHtml(v)}</strong><button class="erp-receipt-copy-btn" type="button" data-receipt-copy="${escapeHtml(v)}" title="${escapeHtml(t("erp-doc-copy-tip"))}">${escapeHtml(t("erp-receipt-copy-btn"))}</button>`:R=`<span class="erp-receipt-docno-missing">${escapeHtml(t("erp-log-doc-missing"))}</span>`,g(t("erp-receipt-doc-no"),R)}h||g(t("erp-receipt-client"),escapeHtml(E)),g(h?t("erp-log-col-customer"):t("erp-receipt-seller"),escapeHtml(m)),c&&g(t("erp-receipt-amount"),escapeHtml(S)),g(t("erp-receipt-time"),escapeHtml(u)),g(t("erp-receipt-elapsed"),escapeHtml((o.elapsed_ms!=null?o.elapsed_ms:"-")+"ms"));let I="";c&&w?I=`<a class="erp-receipt-primary-btn" href="${escapeHtml(w)}" target="_blank" rel="noopener">${escapeHtml(t("erp-receipt-open-erp"))}</a>`:c&&v&&(I=`<button class="erp-receipt-primary-btn" type="button" data-receipt-copy="${escapeHtml(v)}">${escapeHtml(t("erp-receipt-copy-docno"))}</button>`);let M="";if(c&&v&&C){const R="erp-receipt-hint-"+C,B=t(R);B&&B!==R&&(M=`<div class="erp-receipt-hint"><svg class="erp-receipt-hint-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><span>${escapeHtml(B)}</span></div>`)}let P="";if(!c){const R=(o.error_msg||"").match(/ERR_[A-Z0-9_]+/),B=R?R[0]:"",j=typeof currentLang=="string"&&currentLang||window._currentLang||"th",A=o.error_friendly&&o.error_friendly[j]||(o.error_msg?humanizeError(o.error_msg):t("erp-receipt-no-error")),W=/ERR_NO_CUSTOMER_MAPPING|ERR_NO_CLIENT|ERR_NO_SEED_CUSTOMER|ERR_NO_SEED_PRODUCT/.test(o.error_msg||""),ee=!!(o.history_id&&o.endpoint_id),oe=[];oe.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="exceptions">${escapeHtml(t("erp-receipt-act-exceptions"))}</button>`),W&&oe.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="mappings">${escapeHtml(t("erp-receipt-act-mapping"))}</button>`),ee&&oe.push(`<button class="erp-receipt-action-btn primary" type="button" data-receipt-action="retry" data-log-id="${escapeHtml(o.id)}">${escapeHtml(t("erp-receipt-act-retry"))}</button>`),P=`
            ...
            <div class="log-detail-head">
                <div class="log-detail-title">
                    <span class="log-detail-status-icon ${c?"ok":"fail"}">${H}</span>
                    ${escapeHtml(D)}
        ...
        `}catch(a){console.error(a),n.remove()}}window.copyErpDocNo=vn;window.showLogDetail=Ka;let De={key:"all",val:""},ze="",ft=!1,Ee=new Set;window._erpSelected=Ee;async function Ja(){const e=document.getElementById("erp-logs-erp-select");if(!(!e||ft)){ft=!0;try{let n=window._erpEndpoints;if(!Array.isArray(n)||n.length===0){const s=await fetch("/api/erp/endpoints",{headers:{Authorization:"Bearer "+token}});if(s.ok){const i=await s.json();n=i&&(i.items||i)||[]}}Array.isArray(n)||(n=[]);const a=new Set,o=[];n.forEach(s=>{const i=(s&&s.adapter||"").toLowerCase();!i||a.has(i)||(a.add(i),o.push({val:i,label:s&&s.name||i}))}),e.innerHTML=`<option value="">${escapeHtml(t("erp-logs-erp-all"))}</option>`+o.map(s=>`<option value="${escapeHtml(s.val)}"${s.val===ze?" selected":""}>${escapeHtml(s.label)}</option>`).join("")}catch{ft=!1}}}async function Me(e){const n=document.getElementById("erp-logs-list");if(n){e||(n.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-loading"))}</div>`),window.loadErpTodayStats(),Ja();try{const a=new URLSearchParams({limit:"30"});De.key==="status"&&a.set("status",De.val),De.key==="trigger"&&a.set("trigger",De.val),ze&&a.set("adapter",ze);const o=await fetch(`/api/erp/logs?${a}`,{headers:{Authorization:"Bearer "+token}});if(!o.ok){n.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-error"))}</div>`;return}const i=(await o.json()).items||[];if(window._erpLogPollTimer&&(clearTimeout(window._erpLogPollTimer),window._erpLogPollTimer=null),i.some(function(d){return d.status==="pending"})&&(window._erpLogPollTimer=setTimeout(function(){Me(!0)},4e3)),i.length===0){n.innerHTML=`<div class="erp-logs-empty">${escapeHtml(t("erp-logs-empty"))}</div>`;return}const r=i.filter(function(d){var v=d.status==="failed"&&d.next_retry_at&&new Date(d.next_retry_at).getTime()>Date.now()-6e4;return!v}).map(function(d){return d.id}),u=ze==="mrerp_dms",l=u?t("erp-log-col-booking"):t("erp-log-col-invoice"),p=u?t("erp-log-col-customer"):t("erp-log-col-seller"),f='<div class="erp-log-row erp-log-row-header" data-log-header>'+(r.length>0?`<input type="checkbox" class="erp-log-cb erp-log-cb-all" data-log-select-all title="${escapeHtml(t("erp-log-select-all-tip"))}">`:'<span class="erp-log-cb-spacer"></span>')+`<span class="log-time">${escapeHtml(t("erp-log-col-time"))}</span><span class="log-status">${escapeHtml(t("erp-log-col-status"))}</span><span class="log-tag-header">${escapeHtml(t("erp-log-col-trigger"))}</span><span class="log-invoice">${escapeHtml(l)}</span><span class="log-workspace">${escapeHtml(t("erp-log-col-workspace"))}</span><span class="log-client">${escapeHtml(t("erp-log-col-client"))}</span><span class="log-seller">${escapeHtml(p)}</span><span class="log-erp">${escapeHtml(t("erp-log-col-erp"))}</span><span class="log-doc">${escapeHtml(t("erp-log-col-doc"))}</span><span class="log-http">${escapeHtml(t("erp-log-col-http"))}</span><span class="log-elapsed">${escapeHtml(t("erp-log-col-elapsed"))}</span><span class="log-actions"></span></div>`;n.innerHTML=f+i.map(d=>{const v=new Date(d.created_at),w=`${String(v.getMonth()+1).padStart(2,"0")}-${String(v.getDate()).padStart(2,"0")} ${String(v.getHours()).padStart(2,"0")}:${String(v.getMinutes()).padStart(2,"0")}`,C=d.status==="failed"&&d.next_retry_at&&new Date(d.next_retry_at).getTime()>Date.now()-6e4;let E,m,h;d.status==="pending"?(E="retrying",m="⟳",h=t("erp-status-pending")):d.status==="success"?(E="ok",m="✓",h=t("erp-status-success")):d.status==="skipped_dup"?(E="skipped",m="⏭",h=t("erp-status-skipped")):C?(E="retrying",m="↻",h=t("erp-status-retrying")):(E="fail",m="✗",h=t("erp-status-failed"));let S;d.trigger==="auto"?S=`<span class="log-tag auto">${escapeHtml(t("log-tag-auto"))}</span>`:d.trigger==="retry"?S=`<span class="log-tag retry">${escapeHtml(t("log-tag-retry"))}</span>`:S=`<span class="log-tag manual">${escapeHtml(t("log-tag-manual"))}</span>`;const L=d.push_type==="id_card",D=L?`<span class="log-tag log-type-idcard">${escapeHtml(t("erp-log-type-idcard"))}</span>`:"",H=d.error_friendly&&(d.error_friendly[currentLang]||d.error_friendly.en)||"";let _="";const g=d.retry_count||0,I=d.max_retries||3;if(C){const x=new Date(d.next_retry_at).getTime()-Date.now(),b=Math.max(0,Math.round(x/6e4)),z=b<=0?t("erp-retry-next-soon"):t("erp-retry-next-min",{n:b});_=`${t("erp-retry-attempt",{n:g,max:I})} · ${z}`}else d.status==="failed"&&g>=I&&!d.next_retry_at&&(_=t("erp-retry-exhausted",{n:g}));const M=d.status==="failed"&&!C?`<button class="log-retry-btn btn btn-icon" data-log-retry="${escapeHtml(d.id)}" title="${escapeHtml(t("log-retry-title"))}">↻</button>`:"",P=!C,R=Ee.has(d.id)?"checked":"",B=P?`<input type="checkbox" class="erp-log-cb" data-log-cb="${escapeHtml(d.id)}" ${R}>`:'<span class="erp-log-cb-spacer"></span>',j=(d.ocr_buyer_name||"").trim()||(d.client_name||"").trim(),Y=L?'<span class="log-client log-client-empty">—</span>':j?`<span class="log-client" title="${escapeHtml(j)}">${escapeHtml(j.substring(0,18))}</span>`:`<span class="log-client log-client-empty" title="${escapeHtml(t("erp-log-client-unassigned-tip"))}">${escapeHtml(t("erp-log-client-unassigned"))}</span>`,A=L?'<span class="log-workspace log-workspace-unresolved">—</span>':d.workspace_name?`<span class="log-workspace">${escapeHtml((d.workspace_name||"").substring(0,16))}</span>`:`<span class="log-workspace log-workspace-unresolved" title="${escapeHtml(t("erp-log-ws-unresolved-tip"))}">${escapeHtml(t("erp-log-ws-unresolved"))}</span>`,W=d.endpoint_name?`<span class="log-erp">${escapeHtml((d.endpoint_name||"").substring(0,14))}</span>`:`<span class="log-erp log-erp-deleted">${escapeHtml(t("erp-log-endpoint-deleted"))}</span>`,ee=(d.external_doc_no||"").trim(),oe=(d.external_url||"").trim();let $;return oe?$=`<span class="log-doc"><a href="${escapeHtml(oe)}" target="_blank" rel="noopener" class="log-doc-open" title="${escapeHtml(ee||"")}">${escapeHtml(t("erp-log-doc-open"))}</a></span>`:ee?$=`<span class="log-doc log-doc-copy" data-copy-doc="${escapeHtml(ee)}" title="${escapeHtml(t("erp-log-doc-copy-tip"))}">${escapeHtml(ee.substring(0,18))}</span>`:d.status==="success"?$=`<span class="log-doc log-doc-missing" title="${escapeHtml(t("erp-log-doc-missing-tip"))}">${escapeHtml(t("erp-log-doc-missing"))}</span>`:$='<span class="log-doc log-doc-empty">-</span>',`
                <div class="erp-log-row ${E}" data-log-detail="${escapeHtml(d.id)}">
                    ...
                    ${S}${D}
                    <span class="log-invoice"${L?` title="${escapeHtml(t("erp-log-col-booking"))}"`:""}>${escapeHtml(d.invoice_no||"-")}</span>
                    ${A}
                    ${Y}
                    <span class="log-seller"${L?` title="${escapeHtml(t("erp-log-col-customer"))}"`:""}>${escapeHtml((d.seller_name||"").substring(0,20))}</span>
                    ${W}
                    ${$}
