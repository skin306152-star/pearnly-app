"""Membership-keyed Cowork LINE identity and one-time connection tokens."""
...
TOKEN_PREFIX = "clc_"
DEFAULT_CONNECT_TTL_MINUTES = 10
...
def issue_connect_token(
    *, user_id: str, tenant_id: str, ttl_minutes: int = DEFAULT_CONNECT_TTL_MINUTES
) -> dict[str, str]:
...
def consume_connect_token(raw_token: str) -> dict[str, str] | None:
    ...
    with db.get_cursor(commit=True) as cur:
        ...
        if expires_at <= now:
            ...
            raise CoworkLineIdentityError("token_expired")
        cur.execute(
            "UPDATE cowork_line_connect_tokens SET used_at = NOW() WHERE id = %s",
            (row["id"],),
        )
    ...
    *,
    ...
    try:
        with db.get_cursor(commit=True) as cur:
            ...
            owner = cur.fetchone()
            if owner and str(owner["membership_id"]) != str(membership_id):
