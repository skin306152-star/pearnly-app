...
signoff_projection(P0-1)同域顺放于此(读同一批 review_signoff 事件的零依赖纯投影):
api.order_detail 与 review_feed 两个读模型共用它点亮前端签批态,避免各写一份判定漂移。
"""
...
_EVT_REVIEW_REJECTED = "review_rejected"
_EVT_RUN_FINISHED = "run_finished"
...
def signoff_projection(events: list[dict]) -> Optional[dict]:
    """复核签批投影(P0-1 · 签批状态单一事实源),返回 {actor, at, note, stale} 或 None。

    按事件落库序(id 升序,store.list_events / review_feed 均以此序取)扫描:记录最新一条
    review_signoff;其后出现 review_rejected → 签批随工单打回作废(None);其后出现 run_finished
    → stale=True(复核后引擎重跑过、数字已重生成,需再次复核)。前端据 stale 保持复核钮可点
    (一键重签)并提示,不再靠点击后的易失本地态点亮——正常重跑后签批态能回读。
    """
    signoff: Optional[dict] = None
    stale = False
    for e in events:
        etype = e.get("event_type")
        if etype == _EVT_REVIEW_SIGNOFF:
            ...
            if etype == _EVT_REVIEW_REJECTED:
                signoff = None
                stale = False
            elif etype == _EVT_RUN_FINISHED:
                stale = True
    if signoff is None:
        return None
    signoff["stale"] = stale
