# 归档 · 旧 CLAUDE.md 全文(2026-05-13 → 2026-07-25 · 已冻结)

> **不要按这里的内容干活。** 本文件是 2026-07-25 重写项目宪法前的 1470 行全文快照,只为查"当年为什么这么定"而留。
> 活的宪法 = `CLAUDE.md/CLAUDE.md`(轻量版)· 干活做法 = `.claude/skills/*` · 逐段处置对照表 = `docs/context-engineering/2026-07-25-claude-md-simplify.md`。
>
> 已知本文件内的过期/错误内容(别照抄):本地路径写 `D:\Users\Skin\Desktop\pearnly_project\`(真路径 `C:\Users\skin3\Desktop\pearnly-app`)· 部署写 `git push origin main`(真分支 master)· scp 备用打东京 `45.76.53.194`(现役新加坡 `66.42.49.213`)· i18n 说改 `home.js translations`(真源 `static/i18n-data.js` 的 `window.I18N`)· 版本号 `v118.x` 体系已停用 · 铁律 #18/#19/#20/#21/#23 是整顿封锁期专属(2026-07-01 已解除)。
...
> **代码质量 Canon(2026-07-08 拍板)**:代码质量约束(依赖倒置/幂等/Fail Fast/SOLID/KISS/YAGNI + 可机械化的闸)统一收在 [`docs/CODE_QUALITY_CANON.md`](../docs/CODE_QUALITY_CANON.md)。**能机械化→闸(Q1~Q5)· 机械不了→评审铁律(CQ-1~8)**。已上线:闸-Q4「换大脑不写死」(WARN)。钱路径盯死四命门:依赖倒置 / 幂等 / Fail Fast / 高内聚<500。
...
- 路径:`gh` 已在 PATH(2026-07-14 起为 WinGet 安装,旧 `C:\Program Files\GitHub CLI\gh.exe` 路径已失效;bash/PowerShell 直接 `gh` 即可)
...
gh run view <RUN_ID> --repo skin306152-star/pearnly-app

# 3. 看 failed step 的具体错误输出
gh run view <RUN_ID> --repo skin306152-star/pearnly-app --log-failed | grep -E "FAIL|ERROR|AssertionError" | head -20

# 4. 列所有 open PR + CI 状态(查 dependabot 等堆积)
gh pr list --repo skin306152-star/pearnly-app --state open --json number,title,statusCheckRollup

# 5. 关 PR(如 dependabot 红 PR · 加 ignore 规则后清理)
gh pr close <NUMBER> --repo skin306152-star/pearnly-app --comment "<reason>"
...
**bash 调用**:git bash 里直接 `gh ...` 即可(PATH 已有)。
