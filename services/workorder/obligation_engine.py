...
算法(§3.1 并集 + 数据覆盖,§3.2 九义务映射表):
  ...
  - 截止日顺延(G3 · MC2-B 件2):due_paper/due_efiling 本模块只算裸日历日,不在生成时
    顺延——顺延是纯读侧现算,单一权威源 = services.workorder.due_date_rules.defer_due_date
    (本模块 re-export,消费方一律 import 这里,不许另起判据)。存量快照因此保持原始事实
    不被顺延表未来增补悄悄改写;读侧(义务清单/矩阵)序列化时另加 *_deferred 字段展示。
