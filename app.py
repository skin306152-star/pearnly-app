...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.35 · 对账中心 4 语文案大改 + 2 个 UI bug 修复:\n• 对账中心 78 条术语统一(银行对账/销项税核查/收入对账 × 4 语):泰文 สอบทาน→กระทบยอด(对账动词统一)· 中文 账单→对账单(避免信用卡账单歧义)· 英文 Stmt→Statement / Sales VAT→Output VAT(RD 官方术语)· 日文 消込→照合(消込 是 AR/AP 冲销不是 bank reconciliation)+ 公式→計算式(公式 在日语意为 official)\n• Excel 导出 sheet 名 รายละเอียดบัญชี → รายละเอียดSTATEMENT(强对比 GL 明细)\n• adm-engine-vex 4 语补全(en/ja 之前缺导致 admin 页面显示 raw key)\n• Bug 修:设置弹窗浏览器缩放后无滚动条 — settings-layout 加 align-items:stretch + min-height:0 让 side-nav 正确触发自身 overflow-y 滚动\n• Bug 修:ERP 集成页 — 看推送日志按钮从顶部移到底部(将来加新 ERP 卡片时按钮自然在最下)· Xero 卡假按钮改真启用/停用 toggle(对齐 MR.ERP)· MR.ERP 卡删冗余的「看推送日志」按钮 · FlowAccount 空卡片移除\n• 修 home.js 引用了 6 个 i18n key 但没在主 I18N 字典定义的 pre-existing bug · 补 4 语",
