...
(function () {
    ...
    function render() {
        ...
        if (hostEl) hostEl.textContent = `${_emailAccount.imap_host || '-'}:${_emailAccount.imap_port || 993}`;
    ...
    function setStatus(kind) {
        ...
        if (kind === 'none')       { key = 'email-status-none';  pill.classList.add('none'); }
        else if (kind === 'on')    { key = 'email-status-on';    pill.classList.add('active'); }
        else if (kind === 'off')   { key = 'email-status-off';   pill.classList.add('coming'); }
        else if (kind === 'error') { key = 'email-status-error'; pill.classList.add('none'); }
    ...
    function humanizeEmailError(raw) {
        if (!raw) return '';
        const r = String(raw);
        if (/auth|AUTHENTICATIONFAILED|invalid credentials/i.test(r)) return t('email-test-auth-fail');
    ...
    function openModal(mode) {
        ...
        const labels = { gmail: 'Gmail', outlook: 'Outlook / Office365', yahoo: 'Yahoo Mail',
                         icloud: 'iCloud', qq: 'QQ 邮箱', '163': '网易 163' };
        order.forEach(k => {
            if (!presets[k]) return;
    ...
    function readModalForm() {
        ...
        const activeIntervalBtn = document.querySelector('#email-interval-options .email-interval-btn.active');
        ...
        return {
            ...
            filter_subject: (document.getElementById('email-filter-subject').value || '').trim() || null,
        };
    }
    ...
    function showTestResult(kind, text) {
        ...
        el.style.display = '';
        el.textContent = text;
        el.className = 'form-test-result ' + (kind === 'ok' ? 'ok' : kind === 'running' ? 'running' : 'fail');
    }

    async function testFromModal() {
        const form = readModalForm();
        if (!form.email_address) { showTestResult('fail', t('email-addr-required')); return; }
        if (!form.password)      { showTestResult('fail', t('email-password-required')); return; }
        if (!form.imap_host)     { showTestResult('fail', t('email-host-required')); return; }
        ...
        try {
            ...
            if (resp.ok && data.success) {
                showTestResult('ok', t('email-test-ok', { folder: form.folder, n: data.folder_count ?? '?' }));
            } else {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                if (scanned === 0 && ok === 0 && fail === 0) {
                    ...
                    showToast(t('email-trigger-result', { scanned, ok, fail }),
                              fail > 0 ? 'warn' : 'success');
                }
            }
            await load();
    ...
    function wire() {
        ...
        if (addrEl && !addrEl.dataset.autoBound) {
            ...
            addrEl.addEventListener('blur', (e) => autoDetectPresetByAddress((e.target.value || '').trim()));
...
(function folderWatcher() {
    ...
    function renderActive() {
        ...
        document.getElementById('folder-stat-last').textContent = _lastScanAt ? fmtTime(_lastScanAt) : '-';
        ...
        if (statusTextEl) {
            statusTextEl.setAttribute('data-i18n', _paused ? 'folder-status-paused' : 'folder-status-running');
            statusTextEl.textContent = t(_paused ? 'folder-status-paused' : 'folder-status-running');
        }
    ...
    function renderRecent() {
        ...
        listEl.innerHTML = _recent.slice(0, 20).map(r => {
            ...
            const subTxt = (r.status === 'fail' && r.error) ? r.error
                         : (r.status === 'dup'  && r.reason) ? r.reason
                         : (r.status === 'skip' && r.reason) ? r.reason
                         : '';
            const subHtml = subTxt ? `<div class="folder-recent-err">${escapeHtml(subTxt)}</div>` : '';
            return `
    ...
    async function uploadOne(file) {
        ...
        if (!resp.ok) {
            ...
            try {
                ...
                errCode = (j && j.detail) ? (typeof j.detail === 'string' ? j.detail : (j.detail.code || JSON.stringify(j.detail))) : errCode;
            } catch {}
    ...
    async function walkDir(handle, prefix, candidates, depth) {
        ...
        for await (const entry of handle.values()) {
            ...
            if (entry.kind === 'file') {
                ...
                try { await walkDir(entry, relPath, candidates, depth + 1); } catch (e) { /* 单个子目录失败不影响整体 */ }
            }
        }
    }
    ...
    async function scanOnce() {
        ...
        try {
            ...
            for (const c of candidates) {
                ...
                try {
                    let freshFile;
                    try { freshFile = await c.entry.getFile(); } catch { freshFile = c.file; }
                    const result = await uploadOne(freshFile);
                    await idbPut('seen', c.seenKey, { name: freshFile.name, relPath: c.relPath, size: freshFile.size, lastModified: freshFile.lastModified, processed_at: Date.now() });
                    // v101 · Bug 1 修复 · 按真实新增数计数
                    const newIds = result.history_ids || (result.history_id ? [result.history_id] : []);
                    ...
                    if (newIds.length > 0) {
                        ...
                        pushRecent({ name: displayName, status: 'ok', time: new Date(), history_id: newIds[0], count: newIds.length });
                        await idbPut('config', 'processed_count', _processedCount);
                    } else if (dupWarnings.length > 0) {
                        pushRecent({ name: displayName, status: 'dup', time: new Date(), reason: t('folder-recent-dup-reason') });
                    } else {
                        pushRecent({ name: displayName, status: 'skip', time: new Date(), reason: t('folder-recent-skip-reason') });
                    }
                } catch (err) {
                    _failedCount++;
                    pushRecent({ name: c.relPath || c.file.name, status: 'fail', time: new Date(), error: String(err.message || err) });
                    // 不写 seen · 下次重试
    ...
    function _unloadHandler(e) {
        ...
        const msg = (typeof t === 'function') ? t('folder-unload-warn') : 'Folder watcher running · close anyway?';
        e.preventDefault();
    ...
    async function pickFolder() {
        try {
            ...
            _processedCount = 0; _failedCount = 0; _queueCount = 0; _recent = [];
    ...
    async function clearRecent() {
        ...
        try { await idbDel('config', 'recent_list'); } catch {}
        renderRecent();
    }
    ...
    async function load() {
        ...
        try { savedHandle = await idbGet('handles', 'main'); } catch {}
        if (!savedHandle) {
            showOnly('folder-empty');
            setStatus('folder-status-empty', '');
            return;
        ...
        try { _processedCount = (await idbGet('config', 'processed_count')) || 0; } catch {}
        try { _failedCount = (await idbGet('config', 'failed_count')) || 0; } catch {}
        // v101 · Bug 3 修复 · 恢复最近处理列表 · 切页/刷新不丢
        try {
            const saved = await idbGet('config', 'recent_list');
            if (Array.isArray(saved)) {
                // 反序列化:time 字段可能是字符串 · 转回 Date
                _recent = saved.map(r => ({ ...r, time: r.time ? new Date(r.time) : new Date() }));
            }
        } catch {}
        start();
    }
        ...
        document.querySelectorAll('[data-auto-panel="folder"] [data-tab-jump]').forEach(btn => {
            ...
            btn.addEventListener('click', (e) => {
                ...
                if (target === 'email') {
                    ...
                    const navUpload = document.querySelector('[data-page="recognize"]') || document.querySelector('[data-page="upload"]');
    ...
    function _isChromium() {
        try {
            if (navigator.userAgentData && Array.isArray(navigator.userAgentData.brands)) {
                return navigator.userAgentData.brands.some(b => /chromium|google chrome|microsoft edge/i.test(b.brand || ''));
            }
        } catch {}
        ...
        try {
            ...
            if (closeBtn && !closeBtn.dataset.bound) {
                ...
                closeBtn.addEventListener('click', () => {
                    el.style.display = 'none';
                    try { localStorage.setItem('pearnly_chrome_banner_dismissed', '1'); } catch {}
                });
            }
        } catch (e) { /* 静默 */ }
    }
...
(function () {
    ...
    async function refreshSessions() {
        try {
            const resp = await fetch('/api/bank-recon/sessions?limit=30', { headers: { 'Authorization': 'Bearer ' + token } });
    ...
    async function loadSessionDetail(sessionId) {
        try {
            const url = '/api/bank-recon/sessions/' + encodeURIComponent(sessionId)
    ...
    function _qid() { _qSeq += 1; return 'q' + _qSeq + '_' + Date.now(); }
    ...
    async function handleFilePick(e) {
        ...
        // 校验 + 入队
        for (const f of files) {
            const item = { id: _qid(), file: f, name: f.name, size: f.size,
                           status: 'pending', progress: 0, error_code: null,
                           tx_count: 0, session_id: null };
    ...
    function renderQueue() {
        ...
        // summary 文案
        let nDone = 0, nFail = 0, nRun = 0, nWait = 0;
    ...
    function _rowHtml(it) {
        ...
        if (it.status === 'pending') {
            ...
            actHtml = '<button data-q-act="remove" data-q-id="' + esc(it.id) + '" class="bq-act">×</button>';
        } else if (it.status === 'uploading') {
            statusHtml = '<span class="bq-stat bq-run">' + t('bank-queue-status-uploading') + '</span>'
                       + '<div class="bq-bar"><div class="bq-bar-fill" style="width:' + (it.progress || 0) + '%"></div></div>';
        } else if (it.status === 'parsing') {
            statusHtml = '<span class="bq-stat bq-run">' + t('bank-queue-status-parsing') + '</span>'
                       + '<div class="bq-bar"><div class="bq-bar-fill bq-bar-indet"></div></div>';
        } else if (it.status === 'ok') {
            statusHtml = '<span class="bq-stat bq-ok">' + t('bank-queue-status-ok').replace('{n}', it.tx_count || 0) + '</span>';
            actHtml = '<button data-q-act="remove" data-q-id="' + esc(it.id) + '" class="bq-act">×</button>';
        } else if (it.status === 'failed') {
            const msg = formatUploadError(it.error_code || 'unknown');
            statusHtml = '<span class="bq-stat bq-fail" title="' + esc(msg) + '">' + esc(msg) + '</span>';
            actHtml = '<button data-q-act="retry" data-q-id="' + esc(it.id) + '" class="bq-act bq-act-retry">' + t('bank-queue-retry') + '</button>'
                    + '<button data-q-act="remove" data-q-id="' + esc(it.id) + '" class="bq-act">×</button>';
        }
        return '<div class="bq-row" data-q-row="' + esc(it.id) + '">'
             +   '<div class="bq-name" title="' + esc(it.name) + '">' + esc(it.name) + '</div>'
             +   '<div class="bq-size">' + sizeKb + '</div>'
             +   '<div class="bq-status">' + statusHtml + '</div>'
             +   '<div class="bq-actions">' + actHtml + '</div>'
             + '</div>';
    }
    ...
    async function _drainQueue() {
        while (true) {
            const running = _queue.filter(x => x.status === 'uploading' || x.status === 'parsing').length;
    ...
    async function _runOne(it) {
        ...
        try {
            ...
            let body = {};
            try { body = JSON.parse(respText.text || '{}'); } catch (_) { body = {}; }
            ...
            if (body.parse_status === 'parse_failed') {
                it.status = 'failed';
                it.error_code = body.error === 'scanned_pdf_not_yet' ? 'bank_recon.scanned' : 'bank_recon.no_tx';
                renderQueue();
                return;
            }
            it.status = 'ok';
    ...
    async function handleRunMatch() {
        ...
        try {
            const resp = await fetch('/api/bank-recon/sessions/'
                + encodeURIComponent(_currentSession.id) + '/match', {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token },
            });
    ...
    async function handleDeleteSession() {
        ...
        try {
            const resp = await fetch('/api/bank-recon/sessions/'
                + encodeURIComponent(_currentSession.id), {
                method: 'DELETE',
                headers: { 'Authorization': 'Bearer ' + token },
            });
    ...
    async function handleIgnoreTx() {
        ...
        try {
            const resp = await fetch('/api/bank-recon/tx/'
                + encodeURIComponent(_currentTxForDrawer.id) + '/override', {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
    ...
    function refreshSummary() {
        ...
        pill.textContent = pending > 0
            ? t('bank-pill-pending').replace('{n}', pending)
            : t('bank-pill-ok');
    }
    ...
    function renderSessionList() {
        ...
        if (!_sessions || _sessions.length === 0) {
            list.innerHTML = '<div class="bank-empty" data-i18n="bank-sessions-empty">' + t('bank-sessions-empty') + '</div>';
            return;
        }
        if (visible.length === 0) {
    ...
    async function handleDeleteSessionFromList(sessionId, fname) {
        if (!sessionId) return;
        const msg = (t('bank-session-delete-confirm') || '确定删除这条对账记录吗?').replace('{name}', fname || '');
    ...
    function renderSessionCounts(s) {
        ...
        if (matched > 0) parts.push(`<span class="bank-session-count cnt-matched">${matched} ${t('bank-count-matched')}</span>`);
        if (unmatched > 0) parts.push(`<span class="bank-session-count cnt-unmatched">${unmatched} ${t('bank-count-unmatched')}</span>`);
        return parts.join('');
    }
    ...
    function _openClientPicker() {
        ...
        _pickerSelected = _currentSession.client_id != null
            ? Number(_currentSession.client_id) : null;
    ...
    function _renderClientPickerList() {
        ...
        if (!list) return;
        const clients = (window._clientsCache || []).filter(c =>
            c && (c.is_active === true || c.is_active === undefined));
        const rows = [];
        // 「不绑定」一行
        rows.push(
            '<div class="bank-client-picker-row is-none' + (_pickerSelected == null ? ' is-selected' : '') + '" data-cid="">'
            + '<span class="bank-cp-dot"></span>'
            + '<span>' + esc(t('bank-client-picker-none')) + '</span>'
            + '</div>'
        );
        clients.forEach(c => {
    ...
    async function _saveClientPicker() {
        ...
        try {
            const resp = await fetch('/api/bank-recon/sessions/'
                + encodeURIComponent(_currentSession.id) + '/client', {
                method: 'PATCH',
                headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
    ...
    function renderDetailMeta() {
        ...
        document.getElementById('bank-detail-title').textContent =
            (s.bank_code || '-') + (s.account_last4 ? ' ···' + s.account_last4 : '') + ' · ' + (s.source_filename || '');
        document.getElementById('bank-meta-period').textContent = formatPeriod(s.period_start, s.period_end) || '-';
        ...
        let nMatched = 0, nSuggested = 0, nUnmatched = 0;
        for (const x of txs) {
    ...
    function renderTxTable() {
        ...
        if (_currentFilter !== 'all') {
            txs = txs.filter(x => {
                ...
                if (_currentFilter === 'unmatched') return x.match_status === 'unmatched' || x.match_status === 'ignored';
                return true;
            });
        }
        if (txs.length === 0) {
    ...
    function renderTxRow(tx) {
        ...
        const channel = tx.channel
            ? `<span class="bank-tx-channel">${esc(tx.channel)}</span>`
            : '';
        return `
    ...
    async function openCandDrawer(tx) {
        ...
        try {
            const resp = await fetch('/api/bank-recon/tx/'
                + encodeURIComponent(tx.id) + '/candidates', {
                headers: { 'Authorization': 'Bearer ' + token },
            });
    ...
    function _scoreBadge(score) {
        ...
        return '<span class="bank-cand-score ' + cls + '">' + n.toFixed(0) + ' ' + t('bank-cand-score-unit') + '</span>';
    }
    ...
    function _candCard(tx, c, isCurrentMatched) {
        ...
        const amt = c.amount_total !== null && c.amount_total !== undefined ? fmtAmt(c.amount_total) : '-';
        ...
        const cardCls = 'bank-cand-card'
            + (c.is_auto_picked ? ' is-auto' : '')
            + (isPicked ? ' is-picked' : '');

        let actHtml = '';
        if (isPicked) {
            actHtml = '<button class="btn btn-ghost btn-small" data-act="unmatch">'
                    +   '<span>' + t('bank-cand-unmatch') + '</span>'
                    + '</button>';
        } else {
            actHtml = '<button class="btn btn-primary btn-small" data-act="pick" data-hid="' + esc(hid) + '">'
                    +   '<span>' + t(c.is_auto_picked ? 'bank-cand-confirm' : 'bank-cand-pick-this') + '</span>'
                    + '</button>';
        }

        return '<div class="' + cardCls + '" data-hid="' + esc(hid) + '">'
             +   '<div class="bank-cand-card-head">'
             +     '<div class="bank-cand-card-vendor">' + esc(vendor) + '</div>'
             +     _scoreBadge(c.score)
             +   '</div>'
             +   '<div class="bank-cand-card-fields">'
             +     '<span class="bank-cand-field"><span class="bank-cand-flabel">' + t('bank-cand-fld-invoice-no') + '</span> ' + esc(inv) + '</span>'
             +     '<span class="bank-cand-field"><span class="bank-cand-flabel">' + t('bank-cand-fld-amount') + '</span> ' + amt + '</span>'
             +     '<span class="bank-cand-field"><span class="bank-cand-flabel">' + t('bank-cand-fld-date') + '</span> ' + esc(idate) + '</span>'
             +   '</div>'
             +   (fname ? '<div class="bank-cand-card-file" title="' + esc(fname) + '">' + esc(fname) + '</div>' : '')
             +   (c.reason ? '<div class="bank-cand-card-reason">' + esc(c.reason) + '</div>' : '')
             +   '<div class="bank-cand-card-actions">' + actHtml + '</div>'
             + '</div>';
    }
    ...
    function renderCandBody(tx, candidates) {
        ...
        if (tx.match_status === 'matched') {
            headHint = '<div class="bank-cand-hint hint-matched">'
                     + t('bank-cand-hint-matched').replace('{n}', list.length) + '</div>';
        } else if (tx.match_status === 'suggested') {
            headHint = '<div class="bank-cand-hint hint-suggested">'
                     + t('bank-cand-hint-suggested').replace('{n}', list.length) + '</div>';
        } else if (list.length > 0) {
            headHint = '<div class="bank-cand-hint hint-low">'
                     + t('bank-cand-hint-low').replace('{n}', list.length) + '</div>';
        } else {
            // 0 候选 · 空态卡片
    ...
    function closeCandDrawer() {
        ...
        if (body) {
            body.innerHTML = '<div class="bank-cand-pane-empty">'
                ...
                + '<div>' + t('bank-cand-pane-empty-hint') + '</div></div>';
        }
        const tbody = document.getElementById('bank-tx-tbody');
        if (tbody) {
            tbody.querySelectorAll('tr.is-selected').forEach(r => r.classList.remove('is-selected'));
...
(function () {
    ...
    function switchCustTab(tab) {
        ...
        document.querySelectorAll('[data-cust-tab]').forEach(b =>
            b.classList.toggle('active', b.dataset.custTab === _custTab));
    ...
    function _isWsOwner() {
        ...
        return u.is_super_admin === true || u.is_owner === true ||
            r === 'owner' || r === 'admin' || tr === 'owner' || tr === 'admin';
    }
    ...
    function _sellerFiltered() {
        ...
        return _sellerClients.filter(c =>
    ...
    function renderSellerList() {
        ...
        const activeId = (typeof window.getActiveWorkspaceClientId === 'function')
            ? window.getActiveWorkspaceClientId() : null;
        if (!items.length) {
    ...
    function openWsClientModal(ws) {
        ...
        document.getElementById('wsclient-modal-title').textContent = t(ws ? 'wsclient-modal-edit' : 'wsclient-modal-new');
    ...
    async function saveWsClient() {
        ...
        if (!name) { showToast(t('client-msg-name-required'), 'fail'); return; }
        try {
            if (_editingWsClientId) {
                await apiClient('/api/workspace/clients/' + _editingWsClientId, { method: 'PATCH', body: JSON.stringify({ name, tax_id: tax }) });
                showToast(t('client-msg-updated'), 'success');
            } else {
                await apiClient('/api/workspace/clients', { method: 'POST', body: JSON.stringify({ name, tax_id: tax || null }) });
    ...
    async function archiveWsClient() {
        ...
        const ok = await showConfirm(t('wsclient-archive-confirm').replace('{name}', ws ? ws.name : ''), { danger: true });
        if (!ok) return;
        try {
    ...
    function renderBuyerList() {
        ...
        if (info) info.textContent = total ? `${start + 1}–${Math.min(start + ps, total)} / ${total}` : '0';
        const prev = document.getElementById('buyer-prev'); if (prev) prev.disabled = _buyerState.page <= 0;
        const next = document.getElementById('buyer-next'); if (next) next.disabled = _buyerState.page >= maxPage;
    ...
    async function buyerBatchDelete() {
        ...
        const ok = await showConfirm(t('cust-batch-del-confirm').replace('{n}', ids.length), { danger: true });
        if (!ok) return;
        try {
            await apiClient('/api/clients/batch-delete', { method: 'POST', body: JSON.stringify({ ids }) });
    ...
    function openClientModal(client) {
        ...
        const isEdit = !!client;
        document.getElementById('client-modal-title').textContent = t(isEdit ? 'client-modal-edit' : 'client-modal-new');
    ...
    async function saveClient() {
        ...
        try {
            if (_editingClientId) {
                await apiClient(`/api/clients/${_editingClientId}`, { method: 'PATCH', body: JSON.stringify(payload) });
                showToast(t('client-msg-updated'), 'success');
            } else {
    ...
    async function exportClient(clientId) {
        ...
        try {
            ...
            const cname = (c && c.name) ? c.name.replace(/[^a-zA-Z0-9\u0e00-\u0e7f\u4e00-\u9fff]/g, '_').slice(0, 40) : 'client';
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
    ...
    function refreshDrawerClientSelect() {
        ...
        if (!sel) return;
        const cur = sel.value;
        sel.innerHTML = `<option value="">${escapeHtml(t('drawer-client-none'))}</option>` +
    ...
    window.fillCategoryDatalist = async function () {
        try {
            ...
            if (now - _catCache.fetched < 5 * 60 * 1000) {
                if (dl) {
                    dl.innerHTML = _catCache.items.map(c => `<option value="${escapeHtml(c)}">`).join('');
                }
    ...
    function refreshHistoryClientFilter() {
        ...
        if (!sel) return;
        const cur = sel.value;
        sel.innerHTML = `<option value="">${escapeHtml(t('history-client-all'))}</option>` +
            _clients.map(c => `<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');
        sel.value = cur || '';
    }

    window.getHistoryClientFilter = function () { return _historyClientFilter; };
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (buyerTbody) {
            buyerTbody.addEventListener('click', (e) => {
                ...
                if (cb) {
                    ...
                    if (cb.checked) _buyerSelected.add(cid); else _buyerSelected.delete(cid);
                    const row = cb.closest('.cust-row'); if (row) row.classList.toggle('selected', cb.checked);
                    _updateBuyerBatchBar();
                    return;
                }
        ...
        if (buyerCheckAll) {
            buyerCheckAll.addEventListener('change', () => {
                const { items } = _buyerPageItems();
                items.forEach(c => { if (buyerCheckAll.checked) _buyerSelected.add(c.id); else _buyerSelected.delete(c.id); });
                renderBuyerList();
            });
        }
        ...
        if (buyerPrev) buyerPrev.addEventListener('click', () => { if (_buyerState.page > 0) { _buyerState.page--; renderBuyerList(); } });
        const buyerNext = document.getElementById('buyer-next');
        if (buyerNext) buyerNext.addEventListener('click', () => { _buyerState.page++; renderBuyerList(); });
        ...
        if (buyerSearchClear) buyerSearchClear.addEventListener('click', () => {
            const s = document.getElementById('buyer-search'); if (s) s.value = '';
            _buyerState.keyword = ''; _buyerState.page = 0;
        ...
        if (sellerTbody) {
            sellerTbody.addEventListener('click', (e) => {
                ...
                if (act === 'activate') {
                    if (typeof window.setActiveWorkspaceClientId === 'function') window.setActiveWorkspaceClientId(wid);
                    renderSellerList();
                    if (typeof window.renderWorkspaceControl === 'function') window.renderWorkspaceControl();
        ...
        if (sellerSearchClear) sellerSearchClear.addEventListener('click', () => {
            const s = document.getElementById('seller-search'); if (s) s.value = '';
        ...
        if (wsMask) wsMask.addEventListener('click', (e) => { if (e.target === wsMask) closeWsClientModal(); });
        ...
        if (mask) mask.addEventListener('click', (e) => { if (e.target === mask) closeClientModal(); });
...
(function() {
    ...
    async function fetchTemplates(lang, ctxMode) {
        ...
        try {
            ...
            result = [
                { code: 'input_vat', name: t('tpl-input-vat'), desc: t('tpl-input-vat-desc'), recommended: true },
                { code: 'standard',  name: t('tpl-standard'),  desc: t('tpl-standard-desc'),  recommended: false },
    ...
    function renderTemplates(templates) {
        ...
        const itemsHtml = templates.map((tpl, idx) => `
        ...
        try {
            ...
            if (ctx.mode === 'records') {
                ...
                resp = await fetch('/api/reports/export', {
                    ...
                    body: JSON.stringify({
                        template, lang: currentLang, records: ctx.records || [],
                        meta: ctx.meta || {},
                    }),
                });
...
(function() {
    ...
    async function refreshExcBadge() {
        try {
            ...
            const url = '/api/exceptions/stats?status=pending' + (cid ? ('&client_id=' + encodeURIComponent(cid)) : '');
            const resp = await fetch(url, {
    ...
    function renderChips(stats) {
        ...
        const rules = ['confidence_low', 'duplicate', 'amount_missing', 'math_mismatch', 'tax_id_format_invalid'];
    ...
    function renderList(items) {
        ...
        wrap.innerHTML = items.map(it => {
            ...
            const seller = (it.seller_name && it.seller_name.trim()) ? it.seller_name : t('exc-no-seller');
    ...
    function _renderListError(isOffline) {
        ...
        if (btn) btn.addEventListener('click', () => window.loadExceptionsPage && window.loadExceptionsPage());
    }
    ...
    function renderWhyDetail(rule, detail) {
        ...
        if (rule === 'duplicate') {
            const lvl = detail.level === 'exact' ? t('exc-detail-dup-level-exact') : t('exc-detail-dup-level-likely');
            return `
    ...
    function renderDrawer() {
        ...
        if (row.rule_code === 'math_mismatch') {
            flagSet.add('subtotal'); flagSet.add('vat'); flagSet.add('total_amount');
        } else if (row.rule_code === 'tax_id_format_invalid') {
            flagSet.add('seller_tax');
        } else if (row.rule_code === 'amount_missing') {
            flagSet.add('total_amount'); flagSet.add('invoice_number');
        }
        ...
        const fld = (key, labelKey, isMoney) => {
            ...
            const v = editing ? (editF[key] !== undefined ? editF[key] : (f[key] !== undefined && f[key] !== null ? f[key] : '')) : f[key];
            ...
            const valHtml = (v === null || v === undefined || v === '')
    ...
    async function actionBatchIgnore() {
        ...
        const ids = Array.from(_excState.selectedIds);
        if (ids.length === 0) return;
        const ok = await showConfirm(_tn('exc-batch-confirm-ignore', { n: ids.length }), { danger: false });
        if (!ok) return;
        _batchLoading = true;
        const dismiss = showToast(_tn('exc-batch-count', { n: ids.length }) + ' …', 'loading', 0);
        try {
            ...
            const data = await resp.json();
            dismiss();
            showToast(_tn('exc-toast-batch-ignored', {
                n: data.processed || 0,
    ...
    window.addEventListener('online', () => {
        if (_excState.loadFailed && document.getElementById('page-exceptions')?.classList.contains('show')) {
    ...
    document.addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (typeof window.loadExceptionsPage === 'function' && document.getElementById('page-exceptions')?.classList.contains('show')) {
...
(function () {
    ...
    const CATEGORY_OPTIONS = [
        'expense_office', 'expense_travel', 'expense_utility',
        'asset_inventory', 'asset_fixed',
        'liability_ap',
        'revenue_sales', 'revenue_service',
        'other',
    ];
    ...
    async function _fetchClients(force) {
        ...
        try {
            ...
            if (!r.ok) throw new Error('http_' + r.status);
            const data = await r.json();
            _state.clientList = (data.clients || data.items || []).filter(c => c.is_active !== false);
    ...
    function _renderRoot() {
        ...
        let html = '';
        if (readonly) {
            html += '<div class="erp-map-readonly-banner">' +
    ...
    function _renderTable() {
        ...
        if (!items.length && !adding) {
            host.innerHTML = '<div class="erp-map-empty">' +
                '<strong>' + _esc(t('erp-map-empty-' + sub)) + '</strong>' +
    ...
    function _renderHead(sub) {
        if (sub === 'clients') {
            return '<div class="erp-map-row erp-map-head row-clients">' +
                '<div>' + _esc(t('erp-map-col-client')) + '</div>' +
        ...
        if (sub === 'accounts') {
            return '<div class="erp-map-row erp-map-head row-accounts">' +
                '<div>' + _esc(t('erp-map-col-erp')) + '</div>' +
        ...
        // taxes
        return '<div class="erp-map-row erp-map-head row-taxes">' +
            '<div>' + _esc(t('erp-map-col-erp')) + '</div>' +
            '<div>' + _esc(t('erp-map-col-tax')) + '</div>' +
            '<div>' + _esc(t('erp-map-col-erp-tax-code')) + '</div>' +
            '<div>' + _esc(t('erp-map-col-notes')) + '</div>' +
            '<div>' + _esc(t('erp-map-col-actions')) + '</div>' +
        '</div>';
    }
    ...
    function _renderAddRow(sub) {
        const saveBtn = '<button class="btn btn-primary" type="button" data-erp-save="new" style="padding:6px 12px;height:32px;">' + _esc(t('erp-map-save')) + '</button>';
        if (sub === 'clients') {
            return '<div class="erp-map-row erp-map-row-add row-clients" data-erp-row="new">' +
                '<div data-label="' + _esc(t('erp-map-col-client')) + '">' + _clientSelect('') + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + _erpSelect('', 'erp_type') + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp-code')) + '"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="' + _esc(t('erp-map-ph-erp-code')) + '"></div>' +
                '<div data-label="' + _esc(t('erp-map-col-notes')) + '"><input type="text" class="form-input" data-erp-field="notes" placeholder="' + _esc(t('erp-map-ph-notes')) + '"></div>' +
                '<div>' + saveBtn + '</div>' +
            '</div>';
        }
        if (sub === 'accounts') {
            return '<div class="erp-map-row erp-map-row-add row-accounts" data-erp-row="new">' +
                '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + _erpSelect('', 'erp_type') + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-category')) + '">' + _categorySelect('') + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp-code')) + '"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="' + _esc(t('erp-map-ph-acc-code')) + '"></div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp-name')) + '"><input type="text" class="form-input" data-erp-field="erp_name" placeholder="' + _esc(t('erp-map-ph-acc-name')) + '"></div>' +
                '<div data-label="' + _esc(t('erp-map-col-notes')) + '"><input type="text" class="form-input" data-erp-field="notes" placeholder="' + _esc(t('erp-map-ph-notes')) + '"></div>' +
                '<div>' + saveBtn + '</div>' +
            '</div>';
        }
        // taxes
        return '<div class="erp-map-row erp-map-row-add row-taxes" data-erp-row="new">' +
            '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + _erpSelect('', 'erp_type') + '</div>' +
            '<div data-label="' + _esc(t('erp-map-col-tax')) + '">' + _taxKindSelect('') + '</div>' +
            '<div data-label="' + _esc(t('erp-map-col-erp-tax-code')) + '"><input type="text" class="form-input" data-erp-field="erp_code" placeholder="' + _esc(t('erp-map-ph-tax-code')) + '"></div>' +
            '<div data-label="' + _esc(t('erp-map-col-notes')) + '"><input type="text" class="form-input" data-erp-field="notes" placeholder="' + _esc(t('erp-map-ph-notes')) + '"></div>' +
            '<div>' + saveBtn + '</div>' +
        '</div>';
    }
    ...
    function _renderItemRow(sub, it, readonly) {
        const delBtn = readonly ? '' :
            '<button class="erp-map-del-btn" type="button" data-erp-del="' + _esc(it.id) + '" title="' + _esc(t('erp-map-delete')) + '">' +
            ...
            '</button>';
        const erpBadge = '<span class="erp-map-erp-badge">' + _esc(ERP_LABELS[it.erp_type] || it.erp_type) + '</span>';
        if (sub === 'clients') {
            return '<div class="erp-map-row row-clients">' +
                '<div data-label="' + _esc(t('erp-map-col-client')) + '" class="erp-map-cell-name">' + _esc(it.client_name || '#' + it.client_id) + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + erpBadge + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp-code')) + '" class="erp-map-code">' + _esc(it.erp_code || '') + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-notes')) + '">' + _esc(it.notes || '') + '</div>' +
                '<div>' + delBtn + '</div>' +
            '</div>';
        }
        if (sub === 'accounts') {
            const catLabel = t('erp-map-cat-' + (it.pearnly_category || 'other')) || it.pearnly_category;
            return '<div class="erp-map-row row-accounts">' +
                '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + erpBadge + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-category')) + '" class="erp-map-cell-name">' + _esc(catLabel) + '</div>' +
                '<div data-label="' + _esc(t('erp-map-col-erp-code')) + '" class="erp-map-code">' + _esc(it.erp_code || '') + '</div>' +
        ...
        return '<div class="erp-map-row row-taxes">' +
            '<div data-label="' + _esc(t('erp-map-col-erp')) + '">' + erpBadge + '</div>' +
            '<div data-label="' + _esc(t('erp-map-col-tax')) + '" class="erp-map-cell-name">' +
                '<span class="erp-map-tax-badge">' + _esc(taxLabel) + '</span></div>' +
            '<div data-label="' + _esc(t('erp-map-col-erp-tax-code')) + '" class="erp-map-code">' + _esc(it.erp_code || '') + '</div>' +
            '<div data-label="' + _esc(t('erp-map-col-notes')) + '">' + _esc(it.notes || '') + '</div>' +
            '<div>' + delBtn + '</div>' +
        '</div>';
    }
    ...
    async function _save(rowEl) {
        ...
        if (sub === 'clients') {
            if (!fields.client_id || !fields.erp_type || !fields.erp_code) {
                _toast(t('erp-map-save-fail'), 'error'); return;
            }
            payload = { client_id: parseInt(fields.client_id, 10), erp_type: fields.erp_type, erp_code: fields.erp_code, notes: fields.notes || '' };
        } else if (sub === 'accounts') {
            if (!fields.erp_type || !fields.pearnly_category || !fields.erp_code) {
                _toast(t('erp-map-save-fail'), 'error'); return;
            }
            payload = { erp_type: fields.erp_type, pearnly_category: fields.pearnly_category, erp_code: fields.erp_code, erp_name: fields.erp_name || '', notes: fields.notes || '' };
        } else {
            if (!fields.erp_type || !fields.pearnly_tax_kind || !fields.erp_code) {
                _toast(t('erp-map-save-fail'), 'error'); return;
            }
            payload = { erp_type: fields.erp_type, pearnly_tax_kind: fields.pearnly_tax_kind, erp_code: fields.erp_code, notes: fields.notes || '' };
        }
        try {
    ...
    function _switchSub(sub) {
        ...
        _fetchSub(sub, false).then(function () { _renderRoot(); });
    }
...
(function () {
    ...
    async function _onConnect() {
        ...
        try {
            ...
            if (!r.ok) {
                ...
                const errKey = String(detail).replace(/^xero\./, '').toLowerCase();
                _toast(t('xero-push-fail').replace('{err}',
                    t('xero-err-' + errKey) || detail), 'error');
                return;
            }
            const data = await r.json();
    ...
    async function _injectPushBtn() {
        ...
        if (!_status || !_status.configured)        { disabled = true; titleKey = 'xero-err-not_configured'; }
        else if (!_status.connected)                { disabled = true; titleKey = 'xero-push-disabled-no-conn'; }
        else if (_isHistoryExceptional(r))          { disabled = true; titleKey = 'xero-push-disabled-exc'; }
    ...
    async function _onPush() {
        ...
        try {
            ...
            if (!resp.ok) {
                let detail = 'unknown';
                try { detail = (await resp.json()).detail || 'unknown'; } catch (e) {}
                const errKey = String(detail).replace(/^xero\./, '').toLowerCase();
    ...
    function _bind() {
        ...
        document.addEventListener('click', function (ev) {
            ...
            if (subBtn) { setTimeout(_onEnterConnectSubtab, 50); return; }
            // 进入 ERP 对接 panel(自动化 nav)
            const erpAutoTab = ev.target.closest('.auto-nav-item[data-auto-tab="erp"]');
            if (erpAutoTab) { setTimeout(_onEnterConnectSubtab, 80); return; }
            // 卡片按钮
            if (ev.target.closest('#btn-xero-connect')) { ev.preventDefault(); _onConnect(); return; }
            if (ev.target.closest('#btn-xero-disconnect')) { ev.preventDefault(); _onDisconnect(); return; }
        });
...
(function() {
    ...
    function _getDefaultConnectorId() {
        try { return localStorage.getItem('pn_push_default_connector') || ''; }
        catch (e) { return ''; }
    }
    function _setDefaultConnectorId(id) {
        try { localStorage.setItem('pn_push_default_connector', id || ''); } catch (e) {}
    }
    ...
    async function _pushOne(connector, historyId) {
        ...
        try {
            ...
            _toast(t('unified-push-fail').replace('{name}', connector.name).replace('{err}', e.message || 'network'), 'error');
            return false;
        } finally {
            if (btn) { btn.disabled = false; btn.classList.remove('loading'); }
        }
    ...
    function _renderDropdown(connectors, def) {
        ...
        const items = (connectors || []).map(c => {
            ...
            const tag = (c.method === 'download') ? t('unified-push-tag-download') : (isDef ? t('unified-push-tag-default') : '');
            return ''
                + '<div class="pn-pd-item" data-cid="' + _esc(c.id) + '">'
                +   '<span class="pn-pd-icon">' + _connectorIcon(c) + '</span>'
                +   '<span class="pn-pd-name">' + _esc(c.name) + '</span>'
                +   (tag ? '<span class="pn-pd-tag">' + _esc(tag) + '</span>' : '')
                +   (isDef ? '<span class="pn-pd-check">✓</span>' : '')
                + '</div>';
        }).join('');
        const allItem = (connectors && connectors.length > 1)
            ? '<div class="pn-pd-divider"></div>'
              ...
              +   '<span class="pn-pd-name">' + _esc(t('unified-push-all').replace('{n}', connectors.length)) + '</span>'
              + '</div>'
            : '';
    ...
    async function _inject() {
        ...
        if (!hasAny) {
            wrap.innerHTML = ''
                + '<button type="button" class="btn btn-ghost pn-push-empty" id="btn-push-default" disabled title="' + _esc(t('unified-push-empty-tip')) + '">'
                ...
                +   '<span style="margin-left:4px;">' + _esc(t('unified-push-empty')) + '</span>'
                + '</button>';
        } else {
            const showArrow = connectors.length > 1;
            wrap.innerHTML = ''
                + '<div class="pn-push-split">'
                +   '<button type="button" class="pn-push-main" id="btn-push-default" title="' + _esc(t('unified-push-tip')) + '">'
                +     _connectorIcon(def)
                +     '<span>' + _esc(t('unified-push-to').replace('{name}', def ? def.name : '')) + '</span>'
                +   '</button>'
                +   (showArrow
                        ? '<button type="button" class="pn-push-arrow" id="btn-push-arrow" title="' + _esc(t('unified-push-other')) + '">'
        ...
        if (arrowBtn) arrowBtn.addEventListener('click', function (e) {
            e.stopPropagation();
            _toggleDropdown();
        });
...
(function () {
    ...
    document.addEventListener('click', function (ev) {
        ...
        if (!subtabs.classList.contains('show-advanced')) {
            const activeBtn = subtabs.querySelector('.erp-map-subtab.active.erp-map-subtab-advanced');
            if (activeBtn) {
                const clientBtn = subtabs.querySelector('.erp-map-subtab[data-erp-subtab="clients"]');
...
(function () {
    ...
    function _unloadHandler(e) {
        const msg = (typeof t === 'function') ? t('big-batch-unload-warn')
    ...
    function _injectBar() {
        ...
        if (txt) txt.textContent = (typeof t === 'function') ? t('big-batch-progress-init') : 'Starting...';
    }
    function _removeBar() {
...
(function () {
    ...
    function _injectModal() {
        ...
        function _close() { mask.style.display = 'none'; }
        ...
        document.getElementById('erp-onboard-ok').addEventListener('click', function () {
            ...
            try {
                const btnAddEp = document.querySelector('#btn-add-endpoint, [data-action="erp-add-endpoint"]');
        ...
        mask.addEventListener('click', function (ev) { if (ev.target === mask) _close(); });
...
function _wrapSettingsAsModal() {
    ...
    closeBtn.innerHTML = '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 5l10 10M15 5L5 15"/></svg>';
...
function openSettingsModal() {
    ...
    setTimeout(() => {
        ...
        } catch (e) { console.warn('renderSettings:', e); }
        // 找当前 active tab · 没有就 fallback profile
        let activeTab = document.querySelector('.settings-tab.active') ||
...
(function () {
    ...
    async function _loadVexKpi() {
        try {
            const r = await fetch('/api/vat_excel/tasks?page=1&page_size=1', { headers: _authHeader() });
            if (!r.ok) return;
            const d = await r.json();
            const k = d.kpi || {};
            [
    ...
    function _renderFiles() {
        ...
        if (il) il.innerHTML = _invoiceFiles.map((f, i) => _row(f, i, 'inv')).join('') || `<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>`;
        if (rl) rl.innerHTML = _reportFiles.map((f, i) => _row(f, i, 'rep')).join('') || `<div class="vex-fi-empty" data-i18n="vex-no-files">还没文件</div>`;
        ...
        if (info) {
            if (!_invoiceFiles.length || !_reportFiles.length) {
                ...
                info.textContent = (t('vex-ready') || '已就绪 · {a} 张发票 · {b} 份报告')
                    .replace('{a}', _invoiceFiles.length).replace('{b}', _reportFiles.length);
    ...
    async function _onBuild() {
        ...
        const _tick = setInterval(() => {
            ...
            $('vex-progress-sub').textContent =
                (t('vex-progress-elapsed') || '已 {s} 秒 · {a} 张发票 + {b} 份报告')
                    .replace('{s}', s).replace('{a}', _invoiceFiles.length).replace('{b}', _reportFiles.length);
        }, 1000);
        ...
        try {
            ...
            const _curLang = (typeof currentLang === 'string' && currentLang)
                || localStorage.getItem('mrpilot_lang') || 'th';
    ...
    function _rerenderAll() {
        ...
        if (pane) pane.querySelectorAll('[data-i18n]').forEach(el => {
