...
- **★整单折扣(2026-09-01 真票复现)**:含税明细 4,006、整单折扣 200、净额/实收 3,806 若同时把 200 写进表头 `หักส่วนลด`,MR.ERP 拒绝 `"รวมรับชำระ" ต้องเท่ากับ "ยอดสุทธิ"`。TEST2019 两组受控副本确认:表头折扣归零、把折扣分摊到明细 `จำนวนเงิน` 后成功;`ราคา/หน่วย` 可保留票面单价,无需改价。生成器只在「明细合计−税基/含税净额=折扣」勾稽成立时转换,不成立则保留原值交给服务端拒绝,不猜数;Pearnly 原 OCR/审核数据不改。
