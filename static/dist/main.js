...
`,e.dataset.wbInjected="1";try{const a=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",n=window.I18N;n&&n[a]&&(e.querySelectorAll("[data-i18n]").forEach(s=>{const i=s.getAttribute("data-i18n");n[a][i]&&(s.textContent=n[a][i])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(s=>{const i=s.getAttribute("data-i18n-placeholder");n[a][i]&&(s.placeholder=n[a][i])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function a(S,g){let c=t(S)||S;if(g)for(const $ in g)c=c.replace(new RegExp("\\{"+$+"\\}","g"),String(g[$]));return c}async function n(){try{const S=e.currentClient||"",g="/api/exceptions/stats?status=pending"+(S?"&client_id="+encodeURIComponent(S):""),c=await fetch(g,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!c.ok)return;const $=await c.json(),P=document.getElementById("nav-exc-badge");if(!P)return;const Y=parseInt($.pending||0,10);Y>0?(P.textContent=Y>99?"99+":String(Y),P.style.display=""):P.style.display="none"}catch{}}function s(S){return S==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        ...
        </svg>`}function h(S){if(S==null)return"—";const g=parseFloat(S);return isNaN(g)?"—":"฿ "+g.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})}function b(S){return S?S.slice(0,10):"—"}function _(S){document.getElementById("exc-kpi-pending").textContent=S.pending||0,document.getElementById("exc-kpi-high").textContent=S.high_severity||0,document.getElementById("exc-kpi-resolved").textContent=S.resolved||0,document.getElementById("exc-kpi-learned").textContent=S.learned_rules||0;const g=document.getElementById("exc-status-tab-count-pending"),c=document.getElementById("exc-status-tab-count-resolved"),$=document.getElementById("exc-status-tab-count-ignored");g&&(g.textContent=S.pending||0),c&&(c.textContent=S.resolved||0),$&&($.textContent=S.ignored||0),document.querySelectorAll("#exc-status-tabs .exc-status-tab").forEach(Y=>{Y.classList.toggle("active",Y.dataset.status===(e.currentStatus||"pending"))})}function o(S){const g=document.getElementById("exc-chips");if(!g)return;const c=S.by_rule||{},$=["confidence_low","duplicate","amount_missing","math_mismatch","tax_id_format_invalid"];let Y=`<button class="exc-chip ${!e.currentRule?"active":""}" data-rule="">
            ...
            `:S==="amount_missing"?`<div class="exc-why-detail-row" style="justify-content:center;color:var(--danger);"><span>${escapeHtml(t("exc-detail-missing"))}</span></div>`:`<div class="exc-why-detail-row"><span style="font-size:11px;">${escapeHtml(JSON.stringify(g))}</span></div>`}function I(){const S=y.excRow;if(!S)return;const g=S.seller_name&&S.seller_name.trim()?S.seller_name:t("exc-no-seller"),c=S.filename||"—";document.getElementById("exc-drawer-title").textContent=c;const $="exc-status-"+(S.status||"pending"),P=t($)||S.status,Y="s-"+(S.status||"pending"),Z=(S.invoice_date||S.created_at||"").slice(0,10);document.getElementById("exc-drawer-sub").innerHTML=`
