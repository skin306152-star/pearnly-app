...
S.mount(
    ...
    HTTPAdapter(
        max_retries=Retry(
            total=4, backoff_factor=1.5, status_forcelist=[502, 503, 504], allowed_methods=None
        )
    ),
)
...
def login(u, p):
    return S.post(f"{BASE}/api/login", json={"username": u, "password": p}, timeout=40).json()[
        "token"
    ]
...
try:
    ...
    check(
        "owner 64 码全集(②加 field.cost/payroll)",
        len(d["permissions"]) == 64,
        len(d["permissions"]),
    )
    ...
    for role in ("admin", "accountant", "clerk", "viewer"):
        ...
        ra = S.post(
            f"{BASE}/api/invitations/{itok}/accept",
    ...
    MATRIX = [
        ...
        (
            ...
            "GET",
            "/api/team/security-events/export",
            None,
            {"owner": 200, "admin": 200, "accountant": 403, "clerk": 403, "viewer": 403},
        ),
        (
    ...
    for name, method, path, body, expect in MATRIX:
        ...
        for role in ("owner", "admin", "accountant", "clerk", "viewer"):
            st[role] = S.request(
                method, f"{BASE}{path}", json=body, headers=H(tokens[role]), timeout=40
            ).status_code
    ...
    if tokens.get("owner"):
        for role in ("admin", "accountant", "clerk", "viewer"):
            ...
            if uid:
                try:
                    S.delete(
                        f"{BASE}/api/team/members/{uid}", headers=H(tokens["owner"]), timeout=40
                    )
                except Exception as e:
                    print(f"  清理 {role} 失败: {e}")
        rem = [
            x["username"]
            for x in S.get(f"{BASE}/api/team/members", headers=H(tokens["owner"]), timeout=40)
            .json()
            .get("members", [])
        ]
        print("  剩余成员:", rem)
