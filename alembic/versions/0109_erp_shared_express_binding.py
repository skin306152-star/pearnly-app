"""Add typed identity fields for one shared Express profile."""

from alembic import op

revision = "0109_erp_shared_express_binding"
down_revision = "0108_erp_shared_express_foundation"
branch_labels = None
depends_on = None
...
_COLUMN_DDL = tuple(
    ...
    for name, add_type, _catalog_type, _not_null, _default in _COLUMN_CONTRACTS
)
_COLUMN_CONTRACT_DDL = _column_contract_ddl(_COLUMN_CONTRACTS)
...
def downgrade() -> None:
    """Expand-only archive: production startup also depends on this additive shape."""
