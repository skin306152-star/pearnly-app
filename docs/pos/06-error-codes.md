...
| pos.month_invalid | 422 | รูปแบบเดือนไม่ถูกต้อง (ต้องเป็น YYYY-MM) | Invalid month format (expected YYYY-MM) | 月份格式不正确(应为 YYYY-MM) | 月の形式が正しくありません(YYYY-MM 形式で指定してください) |
