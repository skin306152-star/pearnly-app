...
| 16 | OCR 余额 / 充值 | 充值弹窗三步流 + 余额实时轮询;`src/home/billing.ts`、`billing-records.ts` | 界面 | **已做**(2026-07-27 B5):/ai 设置页计费区 = 余额三格(余额/本月用量/费率)+ 三步充值弹窗(金额→转账→凭证,同 /home、/dms 范式)+ 充值记录 + 30s 轮询;`static/ai/ai-billing{,-render}.js`、`ai-api-billing.js`,后端复用 `/api/me/credits`、`/api/credits/topup/*` 零新增钱逻辑;「全局入口不随壳走」已裁掉——住设置页不另造壳。员工视角不露钱。node 单测 `test_ai_billing_render.py` + 真浏览器 E2E `_b5_billing_local.spec.js`(本地 stub 走真构建产物);prod 真机流程未验(充值收尾态依赖 SlipOK 真凭证,收官 push 后补一条真链) | 老站在用 |
