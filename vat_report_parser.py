...
_VAT_HEAD_KW = ("ใบกำกับภาษี", "ใบกํากับภาษี", "ลำดับ", "ลําดับ",
                "ภาษีมูลค่า", "ภาษีมูลคา", "เลขที่ออก", "เลขประจำตัว",
                "เลขประจําตัว", "หมายเหตุ", "ผู้ซื้อ", "ผูซื้อ",
                "ผู้รับบริการ", "ผูรับบริการ", "หรือบริการ",
                "ชื่อสถาน", "ชื่อผู้", "ชื่อผู")
...
def parse_excel(file_bytes: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        for raw in ws.iter_rows(min_row=header_idx + 1, values_only=True):
            cells = list(raw)
            if not any(c for c in cells if c not in (None, "")): continue
        ...
        return {"ok": True, "rows": rows, "meta": meta, "warnings": [],
                "parser_version": PARSER_VERSION, "row_count": len(rows),
                "method": "excel"}
...
def _map_columns(headers) -> Dict[str, int]:
    ...
    for i, h in enumerate(headers):
        h = (str(h) or "").strip()
        if not h: continue
        if _hit(h, _DATE_H)     and "date"           not in col_map: col_map["date"]           = i
        # v118.32.5 · 参考单据号优先检测（关键词更specific，避免被 _NO_H 抢走）
        elif _hit(h, _REF_H)    and "ref_no"         not in col_map: col_map["ref_no"]         = i
        elif _hit(h, _NO_H)     and "invoice_no"     not in col_map: col_map["invoice_no"]     = i
        elif _hit(h, _NAME_H)   and "buyer_name"     not in col_map: col_map["buyer_name"]     = i
        elif _hit(h, _TAX_H)    and "buyer_tax_id"   not in col_map: col_map["buyer_tax_id"]   = i
        elif _hit(h, _BRANCH_H) and "buyer_branch"   not in col_map: col_map["buyer_branch"]   = i
        elif _hit(h, _NET_H)    and "amount_pre_vat" not in col_map: col_map["amount_pre_vat"] = i
        elif _hit(h, _VAT_H)    and "vat_amount"     not in col_map: col_map["vat_amount"]     = i
        elif _hit(h, _TOTAL_H)  and "total_amount"   not in col_map: col_map["total_amount"]   = i
    return col_map
...
def _build_row(row_no: int, cells: list, col_map: Dict[str, int]) -> Dict[str, Any]:
    ...
    for field, ci in col_map.items():
        ...
        if field == "date":
            d = parse_date(val); parsed["report_date"] = d.isoformat() if d else val
        elif field == "invoice_no":     parsed["report_invoice_no"]   = val
        elif field == "ref_no":         parsed["report_ref_no"]       = val  # v118.32.5 · GL对账匹配键
        elif field == "buyer_name":     parsed["report_buyer_name"]   = val
        elif field == "buyer_tax_id":   parsed["report_buyer_tax_id"] = normalize_tax_id(val)
        elif field == "buyer_branch":   parsed["report_buyer_branch"] = normalize_branch(val)
        elif field == "amount_pre_vat": parsed["report_amount_pre_vat"] = _to_float(val)
        elif field == "vat_amount":     parsed["report_vat_amount"]     = _to_float(val)
        elif field == "total_amount":   parsed["report_amount"]         = _to_float(val)
    ...
    try:
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    for body in table[hi + 1:]:
                        if not body or not any(c for c in body if c): continue
                        first = str(body[0] or "").strip().lower()
                        if any(k in first for k in _SKIP_H):
                            if "amount_pre_vat" in col_map and col_map["amount_pre_vat"] < len(body):
                                meta["total_amount_pre_vat"] = _to_float(body[col_map["amount_pre_vat"]])
        ...
        if not rows:
            ...
            if text_rows:
                ...
                return {"ok": True, "rows": text_rows, "meta": {}, "warnings": [],
                        "parser_version": PARSER_VERSION, "row_count": len(text_rows),
...
def parse_with_gemini_paged(file_bytes: bytes,
    ...
    logger.info(f"[vat_paged] PDF {n_pages} 页 → {len(batches)} 批 × {_BATCH_PAGES}p · 并行 {_BATCH_WORKERS}")
    ...
    return {"ok": True, "rows": rows_all, "meta": meta_all,
            "warnings": warnings, "parser_version": PARSER_VERSION,
...
def parse_with_gemini_image_smart(file_bytes: bytes, ext: str,
    ...
    try:
        ...
        if succeeded == 0:
            return {"ok": False, "rows": [],
                    "error": f"两块均失败: {'; '.join(failed_msgs[:2])}"}

        # 重叠区可能重复行 · 按 (date, invoice_no, amount) 去重
        seen = set()
        dedup = []
        for r in rows_all:
            key = (r.get("report_date"), r.get("report_invoice_no"),
        ...
        return {"ok": True, "rows": dedup, "meta": meta_all,
                "warnings": warnings, "parser_version": PARSER_VERSION,
                "row_count": len(dedup), "method": "gemini_img_split",
...
def parse_with_gemini(file_bytes: bytes, mime_type: str,
                       api_key: Optional[str] = None) -> Dict[str, Any]:
    try:
    ...
    try:
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0 and ("timeout" in err_msg or "deadline" in err_msg
        ...
        for i, r in enumerate(raw_rows, 1):
            parsed = {
                ...
                "report_ref_no":         str(r.get("report_ref_no") or "").strip(),  # v118.32.5 · GL对账匹配键
            ...
            _is_garbage = (
                not re.search(r"[A-Za-z0-9]{2,}", inv) or          # 发票号必须含 ≥2 字母数字
                "ใบกำกับภาษี" in inv or "เลขที่" == inv or         # 页眉关键词
                "ลำดับ" in inv or                                  # 表头"序号"
                (amt == 0 and parsed["report_vat_amount"] == 0
        ...
        return {"ok": True, "rows": rows, "meta": data.get("meta", {}) or {},
                "warnings": [],
    ...
    if ext in ("xlsx", "xls"):
        ...
        if result is None:
            ...
            if len(regex_cleaned) >= 3:
                ...
                result = {"ok": True, "rows": regex_cleaned, "meta": {}, "warnings": [],
                          "parser_version": PARSER_VERSION,
