...
- **产出**：
  - `AGENTS.md` 铁律 #17(加在铁律 #16 后)· 4 不许 + 例外条款 + 自检清单 + 历史溯源
...
### Task 8.1 · 文档导航整理（P3-02）✅ 2026-05-22 完成
- **状态**：✅ completed · 阶段 8 开张第一炮
- **类型**：纯文档 · 0 代码 · `docs/README.md` 索引
- **产出**：`docs/README.md`(165 行 · 8 类索引 · 40 个 markdown 文件全列出 + 1-2 句说明 + 现状标签 ✅/📚/⚠️)
- **8 类**:
  1. 架构当前状态(architecture/ · 3 文件 · DB ensure 盘点 + Alembic 设计 + 多租户矩阵)
  2. 审计报告(audits/ + 根 · 3 文件)
  3. 屎山清理报告(cleanup/ · 11 文件 · 全部归档)
  4. 集成资料(integrations/ · 7 文件 · MR.ERP 真接入)
  5. OCR 迁移(ocr-migration/ · 10 文件 · 完整 4 阶段方案)
  6. UI 设计稿(ui/ · 1 文件 · gated 实施)
  7. 运维 / 部署手册(根 · 2 文件)
  8. 历史 STATE 快照(根 · 1 文件 · `STATE_2026_05_19.md`)
- **链接验证**:Python 脚本验 40 个本地 markdown 链接 → 0 broken
- **新 doc 入库规则**:同步写入本 README 一行索引(铁律 #17 配套延伸)
- **完成判定**:✅ 40 个 docs/*.md 文件全索引 · ✅ 5 个子目录全 cover · ✅ 接力 agent 进窗口能快速找到 task 相关 doc
- **真实工作量**:≈30 分钟(原估 1h · 因为已知文件结构 + 标题来源 grep 高效)
...
| **8 · 治理收尾** | 🟡 部分完成 | **1/2** | Task 8.1 ✅(2026-05-22)· `docs/README.md` 索引 40 文件 |
...
- 铁律 16（全档位 push 授权）：`AGENTS.md` 第 247 行起
