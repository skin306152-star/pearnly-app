...
- **下一步(按序)**:① **wave3 3a 外围收口**:enroll 后裸 get_cursor 非核心点仍 owner 绕过(未隔离·安全)——sales get_buyer(漏 user)、workspace seller_routing/list_stats、routes(clients CSV/auth_me/history PDF 计数)穿上下文;erp/push_log_queries+mappings_store 的 JOIN(只收 user_id·强转会丢富化)归 **wave4** 与 erp_push_logs 一起迁;超管(admin_users_query/auth_admin_risk/membership/usage cleanup/owner_users)显式 bypass。② **wave3 3b/3c/3d** exceptions/notification/archive/billing(charge.py 钱禁 bypass·credits/cost 超管聚合必 bypass)③ **wave4** erp/email/import ④ **P4** `12-rls` `passed===5`(重写 harness 测真 policy)+ready 域 force=True。
