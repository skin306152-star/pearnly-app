...
def modeA(raw, fn, pdt, cat):
    ...
    return {"actual": actual, "cost": cost, "ms": ms, "l3": l3, "chain": chain, "review": review}
...
def modeB(raw, fn, prompt, cat):
    ...
    with engine_context("invoice"):  # economy → flash_lite=3.1-lite 覆写生效
        o = timed(
            vertex.multimodal_to_json,
            prompt + IMG_PROMPT_HINT,
            [(imgb, mime)],
            tier="flash_lite",
            temperature=0.0,
            max_tokens=8192,
...
def _flatten_inv(d):
    ...
    if not isinstance(d, dict):
        return {}
    if "items" in d and "total_amount" in d:
        return d
...
def main():
    ...
    for i, (path, gt, cat) in enumerate(corpus, 1):
        ...
        if cat == "invoice":
            ...
            scoreA = sA.get("weighted_score")
            scoreB = sB.get("weighted_score")
            missA = sA.get("critical_misses")
            missB = sB.get("critical_misses")
        else:
            gt_agg = {**gt, **_recon_agg(cat, gt)}  # GT 也派生汇总键(与抽取同算法)
            sA = score_recon(cat, gt_agg, A["actual"])
            sB = score_recon(cat, gt_agg, B["actual"])
            scoreA, scoreB = sA["score"], sB["score"]
            missA, missB = sA["miss"], sB["miss"]
        row = {
            "n": i,
            "file": fn[:24],
            "cat": cat,
            ...
            "traps": gt.get("traps"),
            "deg": gt.get("degradation"),
            "A_score": scoreA,
            "B_score": scoreB,
            "A_cost": round(A["cost"], 4),
            "B_cost": round(B["cost"], 4),
            "A_ms": A["ms"],
            "B_ms": B["ms"],
            "A_l3": A.get("l3"),
            "A_chain": A.get("chain"),
    ...
    json.dump(
        out,
        open("/tmp/vision_ablation_results.json", "w", encoding="utf-8"),
        ensure_ascii=False,
        indent=1,
    )
    ...
    def agg(rows, label):
        ...
        cA = sum(r["A_cost"] for r in rows)
        cB = sum(r["B_cost"] for r in rows)
        print(
            "  %-28s n=%-3d A=%.3f B=%.3f | ฿A=%.2f ฿B=%.2f"
            % (label, len(rows), sum(va) / max(1, len(va)), sum(vb) / max(1, len(vb)), cA, cB),
            flush=True,
        )
