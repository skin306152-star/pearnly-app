    ...
    items: [
        {
            ...
            seller_name: 'บริษัท ซินเซียร์ไอซ์ จำกัด',
            total_amount: 941.6,
            status: 'manual',
            http_status: 0,
    ...
    await page.route('**/api/**', async (route) => {
        ...
        if (u.includes('/api/erp/stats/today'))
            return route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ total: 1, success: 0, failed: 0, auto_cnt: 0 }),
            });
        return route.fulfill({
            status: 200,
            contentType: 'application/json',
...
async function run() {
    ...
    await chk(
        '提到要先建库存商品 / 录进货或期初',
        cardText.includes('ใบซื้อ') || cardText.includes('ยอดยกมา')
    );
    await chk(
        '不裸露英文码 stock_no_master_in_account_set',
        !cardText.includes('stock_no_master_in_account_set')
    );
    await chk(
        '原因文字真的画出来了(非零高度)',
        page.evaluate(() => {
    ...
    await page.waitForSelector('#erp-logs-batch-bar:not([style*="display: none"])', {
        timeout: 5000,
    });
    ...
    const dlg = await page.evaluate(() => {
        ...
        return {
            text: body.innerText,
            boxBg: getComputedStyle(document.querySelector('#confirm-modal .modal'))
                .backgroundColor,
    ...
    await chk(
        '弹窗写明「可以再送一次 ERP」(ซ้ำ/ERP)',
        dlg.text.includes('ERP') && dlg.text.includes('ซ้ำ')
    );
