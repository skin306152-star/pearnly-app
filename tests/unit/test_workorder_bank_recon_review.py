...
脱库:patch store.last_step_done_payload/append_event,验证四件——①合法 accept 落 human_decision
...
def _step_done_payload(tx_id="tx-1", candidates=("it-2",)):
    """reconcile 步 step_done 的 payload(record_bank_decision 现走窄读 last_step_done_payload
    直取此 payload,不再全量 list_events 回放)。None 模拟未跑到 reconcile / 无 step_done。"""
    recon = {
        "auto_matched": [],
        "review": [
            {
    ...
    return {"gates": {"r3_bank": {"recon": recon}}}
...
class RecordBankDecisionTests(unittest.TestCase):
    def _run(self, payload, **kw):
        ...
        with (
            mock.patch.object(target.store, "last_step_done_payload", return_value=payload),
    ...
    def test_accept_records_candidate_and_decision_verb(self):
        out, appended = self._run(
            _step_done_payload(), statement_tx_id="tx-1", action="accept", candidate_id="it-2"
        )
    ...
    def test_invalid_action_rejected(self):
        with self.assertRaises(api.WorkOrderApiError) as ctx:
            self._run(
                _step_done_payload(), statement_tx_id="tx-1", action="bogus", candidate_id=None
            )
    ...
    def test_accept_with_candidate_outside_review_set_rejected(self):
        with self.assertRaises(api.WorkOrderApiError) as ctx:
            self._run(
                _step_done_payload(candidates=("it-2",)),
                statement_tx_id="tx-1",
                action="accept",
    ...
    def test_accept_without_candidate_id_rejected(self):
        with self.assertRaises(api.WorkOrderApiError) as ctx:
            self._run(
                _step_done_payload(), statement_tx_id="tx-1", action="accept", candidate_id=None
            )
        self.assertEqual(ctx.exception.code, "workorder.bank_recon_candidate_invalid")

    def test_no_recon_at_all_rejected(self):
        with self.assertRaises(api.WorkOrderApiError) as ctx:
            self._run(None, statement_tx_id="tx-1", action="reject", candidate_id=None)
        self.assertEqual(ctx.exception.code, "workorder.bank_recon_tx_not_found")
