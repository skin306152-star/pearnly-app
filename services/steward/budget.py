...
三级判据:任务 ฿2 / 会话 ฿12 / 租户 24 小时 ฿150(各自 env 可配,≤0 关闭该级)。
...
# 单次调用的预留额:占坑用,结算时改成真实成本;拿不到成本就留它当最终值,低了=系统性漏记。
# ฿0.30 卡在实测上沿与理论边界之间:62 次真调用 ฿0.13-0.34(p95 ฿0.20),再高 6 步全额占坑撞任务上限。
_DEFAULT_CALL_RESERVE = "0.30"
# 识别类工具(file_convert / vat_report_check)一次跑完可能是几十次多模态调用(50 页 PDF 逐页
# 栅格化),拿单次预留额给它占坑等于封顶名存实亡:真花 ฿5 只记 ฿0.30,租户日额十几倍放水。
