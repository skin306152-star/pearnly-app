#!/usr/bin/env python3
"""取词调用点的字面量键扫描 —— /ai 与 /home 两道词典引用闸共用的那半截。

两棵树的取词函数名、键的字符集、词典结构都不同(见各自的闸),但"从一个调用点里挑出
哪些字面量是键"这件事逐字相同:按括号深度和引号扫过去,只认第一个实参、只认深度 0 的
字面量,再用左右邻居判它站没站在键位上。这段是全闸最容易写错的部分(正则版会把
`at(x ? 'a' : 'b')` 和 `at('pre_' + s)` 一起认错),抄两份等于以后每个 bug 修两遍。

调用方需要提供的只有一样:键的字符集正则(/ai 是 `[A-Za-z_]\\w*`,/home 的键带连字符
