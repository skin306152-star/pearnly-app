...
> **当前数字(master 全绿·工作树干净)**:home.js **7006**(第四十二会话抽 admin · 8941→7006)· app.py 4839 · db.py 3371 · home.css 0 · home.html 6658 · 单测 **1516** · E2E **10**。`gh.exe` 路径 `C:\Program Files\GitHub CLI\gh.exe` · repo `skin306152-star/pearnly-app`。
> **🅱 计费模型已变(重要)**:产品现在**只有「充值+按量扣费」** · 老月订/年订/体验(trial)/免费(free)套餐**全下线** → home.js+后端大量老套餐 UI/PLG 试用闸/升级弹窗 = **死码可清**(I2)。
> **下一个任务(第四十二会话已抽 admin 后半 · 见下方【第四十二会话】块)**:大 IIFE 现**只剩前半**(home.js ~L5687-6441 · 套餐/PLG/试用横幅/LINE 绑定 + `window.fetch` 429 拦截 + `loadPlan` 60s 轮询)。**🅱 计费已改只剩充值 → 这前半大半是死码**(trial/free/monthly/yearly banner 都 hideBanner · 升级弹窗早删)。下一步=**结合「只剩充值」清前半套餐死码**:先核实 `loadPlan`/`/api/me/plan`/429 v109. code 后端是否还活、`renderTrialBanner` 是否永远 hide → 能删则删(它写 `_userInfo`/`_quota` + 裸调 5 render,删时小心)· 留下真活的(429 配额拦截+LINE modal 若还用)。**已抽出的 `src/home/admin-users.js` = 超管 home.html 用户管理(实测超管 Earn 登录直接弹到独立 /admin SPA · /home#/admin-users 被重定向回 /admin · home.html admin-users 路由对超管不可达 = 基本死码)→ 若确认全死可整模块删(rm 文件 + 删 main.js import)**,这是更干净的下一刀。一步一 E2E·别 rush。
...
> ════════════════════════════════════════════════════════════
> **【第四十二会话 · 交接 · 2026-05-27】home.js 抽 v109.3 IIFE 后半(超管用户管理)→ src/home/admin-users.js · home.js 8941→7006(-1935)· 6 门全绿 + CI 绿 + 生产 E2E 10/10**
> ════════════════════════════════════════════════════════════
...
> - **⛔ 给下窗口**:① 现在 admin 半已隔离成独立模块 → **若确认 home.html admin-users 对所有角色都不可达,可整模块物理删**(rm `src/home/admin-users.js` + 删 main.js import + 删 home.html `#page-admin-users` section + routeTo 里 admin-users 分支)· 是最干净的减重(再 -1900 行级)· 但删前要核实没有别的角色/入口能到(skin?老板?grep `routeTo('admin-users')`/侧栏 nav-admin)。② 大 IIFE 前半(套餐死码)按上方「下一个任务」清。两者都一步一 E2E。
>
> ════════════════════════════════════════════════════════════
