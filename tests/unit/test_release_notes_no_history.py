...
class ReleaseNotesContractTests(unittest.TestCase):
    ...
    def test_4_lang_all_present_with_reasonable_length(self):
        ...
        for lang in ("zh", "th", "en", "ja"):
            ...
            self.assertGreaterEqual(len(text), 50,
                f"{lang} 太短({len(text)} chars)· 至少 50 字符")
    ...
    def test_no_history_prepend_chain(self):
        ...
        for lang in ("zh", "th", "en", "ja"):
            ...
            self.assertLessEqual(version_count, 1,
                f"{lang} 含 {version_count} 个版本号 · 覆盖式应至多 1 个")

    def test_uses_official_language_no_emoji_no_kouyu(self):
        """契约 3 · 官方语言 · 不含 🚨 / emoji / 口语化卖萌词"""
        kouyu_markers = [
            "🚨", "🔥", "💥", "🎉",  # emoji 表情
            "客户反馈", "客户说", "用户反馈我们",  # 内部叙事
            "我们修了", "我们发现", "我们决定",
            "大白话", "口语", "通俗",
            "hotfix",  # 开发术语
            "BUG-FIX", "P0", "P1", "P2",  # commit/task 编号
        ]
        for lang in ("zh", "th", "en", "ja"):
            text = self.release_notes[lang]
    ...
    def test_no_developer_jargon(self):
        ...
        dev_terms = [
            ...
            "ต้นเหตุ", "วิธีแก้",  # th 开发术语
            "根本原因", "修正:",  # ja
            # 直接代码引用
            "datetime", "openpyxl", "split(", "regex",
            "JSONB", "Alembic", "schema",
        ...
        for lang in ("zh", "th", "en", "ja"):
            ...
            for term in dev_terms:
                self.assertNotIn(term, text,
                    f"{lang} 含开发术语『{term}』· 违反 CLAUDE.md §6 用户能懂规则")
