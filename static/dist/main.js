...
`))}</pre>
    ...
    `;const r=document.getElementById("history-tbody");a.length===0?r.innerHTML=`<div class="history-row-empty">${escapeHtml(t("history-empty-title"))}</div>`:r.innerHTML=a.map(d=>{const c=new Date(d.created_at),p=String(c.getMonth()+1).padStart(2,"0"),u=String(c.getDate()).padStart(2,"0"),g=String(c.getHours()).padStart(2,"0"),w=String(c.getMinutes()).padStart(2,"0"),$=`${p}-${u} ${g}:${w}`,E=escapeHtml(d.filename||""),_=E.length>50?E.substring(0,50)+"…":E,B=d.invoice_no?escapeHtml(d.invoice_no):_,q=[];d.seller_name&&q.push(escapeHtml(d.seller_name)),d.invoice_no&&d.filename&&q.push(_);const H=q.join(" · ")||"-",F=d.category_tag?`<span class="history-badge category">${escapeHtml(d.category_tag)}</span>`:"",A=d.source_total&&d.source_total>1?`<span class="history-badge multi">${escapeHtml(t("invoice-part-of",{i:d.source_index||1,n:d.source_total}))}</span>`:"",y=d.total_amount!==null&&d.total_amount!==void 0?Number(d.total_amount).toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}):'<span class="history-cell-amount-empty">—</span>',v=[];(d.total_amount===null||d.total_amount===void 0)&&v.push(t("field-amount")),d.invoice_no||v.push(t("field-invoice-no")),d.invoice_date||v.push(t("field-invoice-date")),d.seller_name||v.push(t("field-seller-name")),v.length>0&&`${escapeHtml(d.id)}${escapeHtml(t("history-needs-review-tip")+" · "+v.join(" · "))}${escapeHtml(t("history-needs-review-tip"))}${svgIcon("alert",14)}`,d.edited&&`${escapeHtml(t("history-edited",{n:d.edit_count||1}))}`;const b=d.smart_assigned_flag?`<span class="history-badge smart-assigned" title="${escapeHtml(t("history-smart-assigned"))}">${svgIcon("sparkle",11)}</span>`:"",L=d.confidence==="high"?"high":d.confidence==="medium"?"mid":"low",C=d.confidence==="high"?t("conf-high"):d.confidence==="medium"?t("conf-medium"):t("conf-low"),P=`<span class="history-badge conf-${L}">${escapeHtml(C)}</span>`;let I="";const j=d.source||"manual";return j==="email"?I=`<span class="history-badge source source-email" title="${escapeHtml(t("history-source-email"))}">${svgIcon("mail",11)}<span>${escapeHtml(t("history-source-email"))}</span></span>`:j==="folder"?I=`<span class="history-badge source source-folder" title="${escapeHtml(t("history-source-folder"))}">${svgIcon("folder",11)}<span>${escapeHtml(t("history-source-folder"))}</span></span>`:j==="api"&&(I=`<span class="history-badge source source-api" title="${escapeHtml(t("history-source-api"))}">${svgIcon("api",11)}<span>${escapeHtml(t("history-source-api"))}</span></span>`),`
    ...
    `,r.style.top=a.bottom+4+"px",r.style.left=a.right-160+"px",document.body.appendChild(r);const f=()=>{r.remove(),document.removeEventListener("click",i,!0)},i=d=>{!r.contains(d.target)&&d.target!==n&&f()};setTimeout(()=>document.addEventListener("click",i,!0),0),r.addEventListener("click",async d=>{const c=d.target.closest("[data-act]");if(!c||c.disabled)return;const p=c.dataset.act;if(f(),p==="copy-invno"){if(!s)return;try{await navigator.clipboard.writeText(s),showToast(t("history-copy-invno-ok",{no:s}),"success")}catch{try{const g=document.createElement("textarea");g.value=s,g.style.position="fixed",g.style.opacity="0",document.body.appendChild(g),g.select(),document.execCommand("copy"),document.body.removeChild(g),showToast(t("history-copy-invno-ok",{no:s}),"success")}catch{showToast(t("history-copy-invno-fail"),"error")}}}else if(p==="download-pdf"){const u=showToast(t("history-download-pdf-loading"),"loading",0);try{const g=await fetch(`/api/history/${encodeURIComponent(e)}/pdf`,{headers:{Authorization:"Bearer "+token}});if(!g.ok)throw new Error("download failed");const w=await g.blob(),$=URL.createObjectURL(w),E=document.createElement("a");E.href=$,E.download=o&&o.filename?o.filename.endsWith(".pdf")?o.filename:o.filename+".pdf":"invoice.pdf",document.body.appendChild(E),E.click(),document.body.removeChild(E),setTimeout(()=>URL.revokeObjectURL($),5e3),u(),showToast(t("history-download-pdf-ok"),"success")}catch{u(),showToast(t("history-download-pdf-fail"),"error")}}else if(p==="delete"){if(!await showConfirm(t("history-confirm-delete"),{danger:!0}))return;try{if(!(await fetch(`/api/history/${encodeURIComponent(e)}`,{method:"DELETE",headers:{Authorization:"Bearer "+token}})).ok)throw new Error;showAlert("info",t("history-delete-ok")),setTimeout(hideAlerts,1500),loadHistoryPage()}catch{showAlert("error",t("history-delete-fail"))}}})}(function(){document.addEventListener("click",r=>{const f=r.target.closest(".history-row"),i=r.target.closest("[data-hmenu]");if(i){r.stopPropagation(),Xn(i.dataset.hmenu,i);return}const d=r.target.closest("[data-review]");if(d){r.stopPropagation(),Ke(d.dataset.review);return}const c=r.target.closest("[data-fill-amount]");if(c){r.stopPropagation(),Gn(c.dataset.fillAmount);return}r.target.closest(".history-row-check")||r.target.closest(".history-cell-check")||f&&!r.target.closest("[data-hmenu]")&&Ke(f.dataset.hid)});const n=document.getElementById("history-tbody");n&&n.addEventListener("change",r=>{const f=r.target.closest(".history-row-check");if(!f)return;const i=f.dataset.hid;f.checked?_historySelected.add(i):_historySelected.delete(i),updateHistoryBatchBar()});const a=document.getElementById("history-check-all");a&&a.addEventListener("change",r=>{const f=r.target.checked;for(const i of _historyState.items)f?_historySelected.add(i.id):_historySelected.delete(i.id);document.querySelectorAll(".history-row-check").forEach(i=>{i.checked=f}),updateHistoryBatchBar()});const o=document.getElementById("history-batch-cancel");o&&o.addEventListener("click",()=>{clearHistorySelection(),document.querySelectorAll(".history-row-check").forEach(r=>{r.checked=!1})});const s=document.getElementById("history-batch-delete");s&&s.addEventListener("click",async()=>{const r=_historySelected.size;if(r===0||!await showConfirm(t("history-batch-confirm",{n:r}),{danger:!0}))return;const i=Array.from(_historySelected);try{const d=await fetch("/api/history/batch-delete",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({ids:i})});if(!d.ok)throw new Error("batch delete failed");const c=await d.json();showToast(t("history-batch-done",{n:c.deleted||0}),"success"),clearHistorySelection(),loadHistoryPage()}catch(d){console.error("batch delete",d),showToast(t("history-batch-fail"),"error")}});let l=null;document.getElementById("history-search").addEventListener("input",r=>{const f=r.target.value;document.getElementById("history-search-clear").style.display=f?"":"none",clearTimeout(l),l=setTimeout(()=>{_historyState.keyword=f.trim(),_historyState.page=0,clearHistorySelection(),loadHistoryPage()},300)}),document.getElementById("history-search-clear").addEventListener("click",()=>{const r=document.getElementById("history-search");r.value="",_historyState.keyword="",_historyState.page=0,clearHistorySelection(),document.getElementById("history-search-clear").style.display="none",loadHistoryPage(),r.focus()}),document.getElementById("history-range").addEventListener("change",r=>{_historyState.range=parseInt(r.target.value,10),_historyState.page=0,clearHistorySelection(),loadHistoryPage()}),document.getElementById("history-prev").addEventListener("click",()=>{_historyState.page>0&&(_historyState.page--,clearHistorySelection(),loadHistoryPage())}),document.getElementById("history-next").addEventListener("click",()=>{(_historyState.page+1)*_historyState.pageSize<_historyState.total&&(_historyState.page++,clearHistorySelection(),loadHistoryPage())})})();window.openHistoryDrawer=Ke;const Me=document.getElementById("drop-zone"),vt=document.getElementById("file-input");Me.addEventListener("click",()=>vt.click());vt.addEventListener("change",e=>Nt(e.target.files));["dragover","dragenter"].forEach(e=>{Me.addEventListener(e,n=>{n.preventDefault(),Me.classList.add("drag-over")})});["dragleave","drop"].forEach(e=>{Me.addEventListener(e,n=>{n.preventDefault(),Me.classList.remove("drag-over")})});Me.addEventListener("drop",e=>{e.preventDefault(),Nt(e.dataTransfer.files)});const Qn=/\.(pdf|png|jpe?g|webp|tiff?|bmp|gif|xlsx?|xlsm|csv|tsv|docx?|txt)$/i;function ct(e){return e.type&&e.type.startsWith("image/")||/\.(png|jpe?g|webp|tiff?|bmp|gif)$/i.test(e.name)}function Zn(e){return e.type==="application/pdf"||/\.pdf$/i.test(e.name)}function ea(e){return Zn(e)||ct(e)||Qn.test(e.name)}function Nt(e){hideAlerts();const n=Array.from(e),a=n.filter(ea);a.length!==n.length&&showAlert("warn",t("alert-unsupported-format"));const o=a.filter(f=>!ct(f)),s=a.filter(ct),l=new Set(_selectedFiles.map(f=>f.name+"_"+f.size));for(const f of o){const i=f.name+"_"+f.size;l.has(i)||(_selectedFiles.push({file:f,name:f.name,size:f.size,status:"waiting",errorKey:null,errorParams:null}),l.add(i))}if(s.length>0)try{handleCameraImages(s,"gallery")}catch(f){console.error("[upload] image route failed",f)}const r=getMaxFiles();_selectedFiles.length>r&&(showAlert("warn",t("alert-file-count",{n:r})),_selectedFiles=_selectedFiles.slice(0,r)),Qe(),ht(),vt.value=""}let Ve=!1;function Qe(){const e=document.getElementById("file-list");if(!e)return;if(_selectedFiles.length===0){e.classList.remove("has-files"),e.innerHTML="";return}e.classList.add("has-files");const n=_selectedFiles.length,a=_selectedFiles.filter(p=>p.status==="processing"||p.status==="retrying").length,o=_selectedFiles.filter(p=>p.status==="success").length,s=_selectedFiles.filter(p=>p.status==="error").length;let l=`<span class="count">${escapeHtml(t("file-list-total",{n}))}</span>`;const r=[];a&&r.push(`<span style="color: var(--accent, #111111);">${a} ${escapeHtml(t("status-processing"))}</span>`),o&&r.push(`<span style="color: var(--success, #059669);">${o} ${escapeHtml(t("status-success"))}</span>`),s&&r.push(`<span style="color: var(--danger, #dc2626);">${s} ${escapeHtml(t("status-error"))}</span>`),r.length&&(l+=" · "+r.join(" · "));const f=Ve?t("file-list-collapse"):t("file-list-expand"),i=_selectedFiles.map((p,u)=>{let g=t("status-"+p.status);p.status==="retrying"&&(g=t("status-retrying")),p.status==="error"&&p.errorKey&&(g=t(p.errorKey,p.errorParams||{}));const w=p.status==="processing"||p.status==="retrying"?'<span class="spinner"></span>':"",$=p.status==="error"&&p.canRetry?`<button class="file-retry-btn" data-retry-idx="${u}" title="${escapeHtml(t("upload-retry-btn"))}">${svgIcon("refresh",12)}<span>${escapeHtml(t("upload-retry-btn"))}</span></button>`:"",E=p.status==="success"&&p.fromCache?`<span class="file-cache-badge">${svgIcon("cache",11)}<span>${escapeHtml(t("cache-hit-badge"))}</span></span>`:"";return`
            <li class="file-item">
                ...
                ${E}
                <span class="file-status ${p.status}">${w}${g}</span>
                ${$}
            </li>
        `}).join("");e.innerHTML=`
    ...
    <div class="modal" style="max-width:480px">
        ...
        <div class="modal-foot">
            ...
            <div class="access-log-row access-log-head">
                ...
                </div>`}).join("");c.innerHTML=u+g}function s(){const c=document.getElementById("access-log-pager");if(!c)return;const p=e.total||0;if(!p){c.innerHTML="";return}const u=e.page||1,g=e.per_page,w=Math.max(1,Math.ceil(p/g)),$=(t("set-access-log-pager-total")||"Total {n}").replace("{n}",p),E=(t("set-access-log-pager-page")||"Page {p} / {t}").replace("{p}",u).replace("{t}",w);c.innerHTML=`
            ...
            </div>`}async function l(c){const p=localStorage.getItem("mrpilot_token");if(p){e.page=c||1,a(t("set-access-log-loading"));try{const u="/api/me/access_log?page="+e.page+"&per_page="+e.per_page+"&q="+encodeURIComponent(e.q||""),g=await fetch(u,{headers:{Authorization:"Bearer "+p}});if(g.status===403){a(t("set-access-log-empty"));return}if(!g.ok)throw new Error("http_"+g.status);const w=await g.json();e.rows=w.logs||[],e.total=w.total||0,e.loaded=!0,o(),s()}catch{a(t("set-access-log-fail"))}}}async function r(){const c=localStorage.getItem("mrpilot_token");if(c)try{const p="/api/me/access_log.csv?q="+encodeURIComponent(e.q||""),u=await fetch(p,{headers:{Authorization:"Bearer "+c}});if(!u.ok){showToast(t("set-access-log-csv-fail")||"Export failed","error");return}const g=await u.blob(),w=document.createElement("a"),$=URL.createObjectURL(g);w.href=$,w.download="pearnly_access_log.csv",document.body.appendChild(w),w.click(),setTimeout(function(){URL.revokeObjectURL($),w.remove()},100),showToast(t("set-access-log-csv-ok")||"Exported","success")}catch{showToast(t("set-access-log-csv-fail")||"Export failed","error")}}function f(){const c=document.querySelectorAll(".set-tab-owner-only"),p=!!(_userInfo&&(_userInfo.role==="owner"||_userInfo.is_super_admin));c.forEach(function(u){u.style.display=p?"":"none"})}document.addEventListener("click",function(c){if(c.target.closest('.settings-tab[data-tab="access-log"]')){setTimeout(function(){(!e.loaded||e.page!==1)&&l(1)},50);return}if(c.target.closest("#access-log-csv-btn")){c.preventDefault(),r();return}const g=c.target.closest(".access-log-pager-btn[data-access-log-page]");if(g&&!g.disabled){const w=parseInt(g.dataset.accessLogPage,10);l(w)}}),document.addEventListener("input",function(c){c.target&&c.target.id==="access-log-search"&&(clearTimeout(window.__accessLogSearchTimer),window.__accessLogSearchTimer=setTimeout(function(){e.q=(c.target.value||"").trim(),l(1)},350))});let i=0;const d=setInterval(function(){i++,_userInfo&&(f(),clearInterval(d)),i>60&&clearInterval(d)},500);typeof window.subscribeI18n=="function"&&window.subscribeI18n("me-access-log",function(){f(),e.loaded&&(o(),s())})})();(function(){const e=document.getElementById("notif-new-modal");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){const e=document.getElementById("archive-token-modal");if(!(!e||e.dataset.wbInjected==="1")){e.innerHTML=`
        ...
        `}async function c(){try{const y=await fetch("/api/settings/dup-check",{headers:{Authorization:"Bearer "+token}});if(!y.ok)return;const v=await y.json(),b=document.getElementById("pref-dup-check");b&&(b.checked=!!v.enabled)}catch(y){console.warn("load prefs failed",y)}}const p=document.getElementById("pref-dup-check");p&&!p.dataset.bound&&(p.dataset.bound="1",p.addEventListener("change",async y=>{const v=y.target.checked;try{(await fetch("/api/settings/dup-check",{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({enabled:v})})).ok?showToast(v?t("pref-dup-check-on-toast"):t("pref-dup-check-off-toast"),"success"):(y.target.checked=!v,showToast(t("pref-save-failed"),"error"))}catch{y.target.checked=!v,showToast(t("pref-save-failed"),"error")}}));async function u(){const y=!!(_userInfo&&_userInfo.can_customize_archive);o=!y;const v=document.getElementById("archive-upgrade-banner");v&&(v.style.display=y?"none":"");const b=document.getElementById("archive-plus-badge");b&&(b.style.display=y?"none":"");try{const L=await fetch("/api/archive/settings",{headers:{Authorization:"Bearer "+token}});if(!L.ok)throw new Error("load failed");const C=await L.json();e=Array.isArray(C.name_template)?C.name_template:[],n=C.folder_strategy||"by_month_seller"}catch(L){console.error("load archive settings failed",L),showToast(t("archive-load-failed"),"error");return}g(),w(),$(),E()}function g(){const y=document.getElementById("archive-rule-canvas");if(y){if(e.length===0){y.innerHTML=`<div class="archive-empty">${escapeHtml(t("archive-rule-empty"))}</div>`;return}y.innerHTML=e.map((v,b)=>{const L=l[v.type]||{label:v.type},C=s[v.type]||"",P=v.type==="sep"?`"${escapeHtml(v.val||"_")}"`:escapeHtml(t(L.label));return`
        ...
        </div>`,c=document.createElement("div");c.innerHTML=d,document.body.appendChild(c.firstElementChild),document.getElementById("report-modal-close-x").addEventListener("click",a),document.getElementById("report-modal-cancel").addEventListener("click",a),document.getElementById("report-modal").addEventListener("click",p=>{p.target.id==="report-modal"&&a()})}function a(){const d=document.getElementById("report-modal");d&&(d.style.display="none"),o=null}let o=null;async function s(d,c){const p=d+":"+(c||"");if(e[p])return e[p];let u;try{const g=localStorage.getItem("mrpilot_token"),w=await fetch(`/api/reports/templates?lang=${encodeURIComponent(d)}`,{headers:{Authorization:"Bearer "+g}});if(!w.ok)throw new Error("templates fetch failed");u=(await w.json()).templates||[]}catch(g){console.error("fetchTemplates fail",g),u=[{code:"input_vat",name:t("tpl-input-vat"),desc:t("tpl-input-vat-desc"),recommended:!0},{code:"standard",name:t("tpl-standard"),desc:t("tpl-standard-desc"),recommended:!1},{code:"print",name:t("tpl-print"),desc:t("tpl-print-desc"),recommended:!1}]}if(u=u.filter(g=>g.code!=="erp"),c==="history-batch"){const g=u.findIndex($=>$.code==="standard"),w=g>=0?g+1:u.length;u.splice(w,0,{code:"sales_detail_th",name:t("export-tpl-sales-detail"),desc:t("export-tpl-sales-detail-desc"),recommended:!1,is_new:!0})}return e[p]=u,u}function l(d){const c=document.getElementById("report-tpl-list"),p=d.map((g,w)=>`
...
`,e.dataset.wbInjected="1";try{const n=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",a=window.I18N;a&&a[n]&&(e.querySelectorAll("[data-i18n]").forEach(o=>{const s=o.getAttribute("data-i18n");a[n][s]&&(o.textContent=a[n][s])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(o=>{const s=o.getAttribute("data-i18n-placeholder");a[n][s]&&(o.placeholder=a[n][s])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function n(T,x){let h=t(T)||T;if(x)for(const D in x)h=h.replace(new RegExp("\\{"+D+"\\}","g"),String(x[D]));return h}async function a(){try{const T=e.currentClient||"",x="/api/exceptions/stats?status=pending"+(T?"&client_id="+encodeURIComponent(T):""),h=await fetch(x,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!h.ok)return;const D=await h.json(),N=document.getElementById("nav-exc-badge");if(!N)return;const K=parseInt(D.pending||0,10);K>0?(N.textContent=K>99?"99+":String(K),N.style.display=""):N.style.display="none"}catch{}}function o(T){return T==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            </button>`}x.innerHTML=K,x.querySelectorAll(".exc-chip").forEach(Q=>{Q.addEventListener("click",()=>{const le=Q.dataset.rule||null;e.currentRule=le,w()})})}function d(T){const x=document.getElementById("exc-list");if(!x)return;if(!T||T.length===0){x.innerHTML=`<div class="exc-empty">
                ${s()}
        ...
        `;const z=document.getElementById("exc-fld-edit");z&&z.addEventListener("click",()=>{_.editing=!0,_.editFields={...y(_.history)},L()});const m=document.getElementById("exc-fld-cancel");m&&m.addEventListener("click",()=>{_.editing=!1,_.editFields=null,L()});const R=document.getElementById("exc-fld-save");R&&R.addEventListener("click",()=>C()),document.querySelectorAll(".exc-field-input").forEach(ae=>{ae.addEventListener("input",()=>{_.editFields||(_.editFields={}),_.editFields[ae.dataset.editKey]=ae.value})});const J=document.getElementById("exc-pdf-retry");J&&_.openExcId&&J.addEventListener("click",()=>{_.excRow&&q(_.excRow.history_id,_.openExcId)});const X=T.status==="pending",de=!!(T.seller_name&&T.seller_name.trim()),se=document.getElementById("exc-btn-resolve"),ce=document.getElementById("exc-btn-ignore");se.disabled=!X,ce.disabled=!X||!de,ce.title=de?t("exc-ignore-hint"):t("exc-ignore-no-seller")}async function C(){if(!_.openExcId||!_.history||!_.history.pages||_.loading)return;_.loading=!0;const T=showToast(t("exc-fld-saving"),"loading",0);try{const x=JSON.parse(JSON.stringify(_.history.pages||[]));let h=x.findIndex(ie=>!ie.is_duplicate&&!ie.is_copy);h<0&&(h=0),x[h]||(x[h]={fields:{}});const D=x[h].fields||{},N=_.editFields||{},K=new Set(["subtotal","vat","total_amount"]),Q={...D};for(const ie in N){let V=N[ie];if((V===""||V===void 0)&&(V=null),K.has(ie)&&V!==null){const Z=parseFloat(V);V=isNaN(Z)?null:Z}Q[ie]=V}x[h].fields=Q;const le=await fetch("/api/history/"+encodeURIComponent(_.history.id),{method:"PUT",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({pages:x})});if(!le.ok)throw new Error("http "+le.status);T(),showToast(t("exc-fld-save-ok"),"success"),F(),await u(),await w(),a()}catch(x){T(),console.warn("save fields fail",x),showToast(t("exc-fld-save-fail"),"error")}finally{_.loading=!1}}async function P(){if(!(!_.openExcId||_.loading)){_.loading=!0;try{const T=await fetch("/api/exceptions/"+_.openExcId+"/resolve",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!T.ok)throw new Error("http "+T.status);showToast(t("exc-toast-resolved"),"success"),F(),await u(),await w(),a()}catch(T){console.warn("resolve fail",T),showToast(t("exc-toast-action-fail"),"error")}finally{_.loading=!1}}}async function I(){if(!(!_.openExcId||_.loading)){_.loading=!0;try{const T=await fetch("/api/exceptions/"+_.openExcId+"/ignore",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!T.ok)throw new Error("http "+T.status);showToast(t("exc-toast-ignored"),"success"),F(),await u(),await w(),a()}catch(T){console.warn("ignore fail",T),showToast(t("exc-toast-action-fail"),"error")}finally{_.loading=!1}}}let j=!1;async function Y(){if(j)return;const T=Array.from(e.selectedIds);if(T.length===0||!await showConfirm(n("exc-batch-confirm-resolve",{n:T.length})))return;j=!0;const h=showToast(n("exc-batch-count",{n:T.length})+" …","loading",0);try{const D=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:T,action:"resolve"})});if(!D.ok)throw new Error("http "+D.status);const N=await D.json();h(),showToast(n("exc-toast-batch-resolved",{n:N.processed||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(D){h(),console.warn("batch resolve fail",D),showToast(t("exc-toast-batch-fail"),"error")}finally{j=!1}}async function G(){if(j)return;const T=Array.from(e.selectedIds);if(T.length===0||!await showConfirm(n("exc-batch-confirm-ignore",{n:T.length}),{danger:!1}))return;j=!0;const h=showToast(n("exc-batch-count",{n:T.length})+" …","loading",0);try{const D=await fetch("/api/exceptions/batch",{method:"POST",headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||""),"Content-Type":"application/json"},body:JSON.stringify({ids:T,action:"ignore"})});if(!D.ok)throw new Error("http "+D.status);const N=await D.json();h(),showToast(n("exc-toast-batch-ignored",{n:N.processed||0,wl:N.whitelist_added||0}),"success"),e.selectedIds.clear(),await u(),await w(),a()}catch(D){h(),console.warn("batch ignore fail",D),showToast(t("exc-toast-batch-fail"),"error")}finally{j=!1}}function W(){e.selectedIds.clear(),d(e.listCache)}document.addEventListener("click",T=>{T.target.closest("#exc-drawer-close")&&F(),T.target.closest("#exc-drawer-mask")&&F(),T.target.closest("#exc-btn-resolve")&&P(),T.target.closest("#exc-btn-ignore")&&I(),T.target.closest("#exc-batch-resolve")&&Y(),T.target.closest("#exc-batch-ignore")&&G(),T.target.closest("#exc-batch-clear")&&W(),T.target.closest("#exc-loadmore")&&$()}),document.addEventListener("keydown",T=>{T.key==="Escape"&&_.openExcId&&F()}),document.addEventListener("click",T=>{T.target.closest("#btn-exc-refresh")&&(typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a())}),document.addEventListener("change",T=>{if(!T.target.closest("#exc-client-filter"))return;const x=T.target;e.currentClient=x.value||"",e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage(),a()}),document.addEventListener("click",T=>{const x=T.target.closest("#exc-status-tabs .exc-status-tab");if(!x)return;const h=x.dataset.status||"pending";h!==e.currentStatus&&(e.currentStatus=h,e.currentRule=null,e.selectedIds.clear(),e.listCache=[],typeof window.loadExceptionsPage=="function"&&window.loadExceptionsPage())}),window.addEventListener("online",()=>{e.loadFailed&&document.getElementById("page-exceptions")?.classList.contains("show")&&window.loadExceptionsPage&&window.loadExceptionsPage()}),setTimeout(a,1500),setInterval(a,6e4);function te(T){if(!T)return"—";try{const x=new Date(T),h=D=>String(D).padStart(2,"0");return`${x.getFullYear()}-${h(x.getMonth()+1)}-${h(x.getDate())} ${h(x.getHours())}:${h(x.getMinutes())}`}catch{return T.slice(0,16).replace("T"," ")}}async function re(){const T=document.getElementById("learned-list");if(T){T.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-loading"))}</div>`;try{const x=await fetch("/api/exception-whitelist",{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!x.ok)throw new Error("http "+x.status);const D=(await x.json()).items||[];if(D.length===0){T.innerHTML=`<div class="learned-empty">${escapeHtml(t("set-learned-empty"))}</div>`;return}const N=`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span class="vex-fi-n" title="${l(S.name)}">${l(S.name)}</span>
            ...
            <div class="vex-pp-pagination" id="vex-pp-${V}-pg"></div>`;const S=o("vex-pp-search-"+V);S&&S.addEventListener("input",U=>{V==="inv"?(g=U.target.value,p=50):(w=U.target.value,u=50),T(V)});const M=o("vex-pp-clearall-"+V);M&&M.addEventListener("click",()=>{V==="inv"?(f=[],g="",p=50):(i=[],w="",u=50),j()}),T(V)}function T(V){const Z=o("vex-pp-"+V+"-list"),ee=o("vex-pp-"+V+"-pg");if(!Z)return;const ne=V==="inv"?f:i,oe=V==="inv"?g:w,ue=V==="inv"?p:u,k=V==="inv"?Y:G,S=ne.map((z,m)=>({f:z,i:m})),M=oe?S.filter(({f:z})=>z.name.toLowerCase().includes(oe.toLowerCase())):S,U=M.slice(0,ue);if(Z.innerHTML=U.map(({f:z,i:m})=>`
    ...
    `,document.body.appendChild(n),n.addEventListener("click",async a=>{if(a.target===n||a.target.classList.contains("log-detail-close")){n.remove();return}const o=a.target.closest("[data-receipt-copy]");if(o){Qt(o.dataset.receiptCopy);return}const s=a.target.closest("[data-receipt-action]");if(s){const l=s.dataset.receiptAction;l==="retry"?window.retryPushLog(s.dataset.logId):l==="exceptions"?typeof routeTo=="function"&&routeTo("exceptions"):l==="mappings"&&typeof routeTo=="function"&&routeTo("integrations"),n.remove();return}});try{const a=await fetch(`/api/erp/logs/${encodeURIComponent(e)}`,{headers:{Authorization:"Bearer "+token}});if(!a.ok){n.remove();return}const o=await a.json(),s=window._erpEndpoints.find(C=>C.id===o.endpoint_id),l=o.endpoint_name||(s?s.name:o.endpoint_id?t("erp-log-endpoint-deleted"):"-"),r=(o.endpoint_adapter||s&&s.adapter||"").toLowerCase(),f=new Date(o.created_at).toLocaleString(),i=o.trigger==="auto"?t("log-tag-auto"):o.trigger==="retry"?t("log-tag-retry"):t("log-tag-manual"),d=o.request_body?JSON.stringify(o.request_body,null,2):t("erp-receipt-no-tech"),c=o.response_body||t("erp-receipt-no-tech"),p=o.status==="success";let u=typeof c=="string"?c:JSON.stringify(c,null,2);if(p)try{const C=typeof o.response_body=="string"?JSON.parse(o.response_body):o.response_body||{},P=C.row_count||(Array.isArray(C.imported_rows)?C.imported_rows.length:0);P>0&&(u=t("log-push-rows").replace("{n}",String(P)))}catch{}const g=(o.external_doc_no||"").trim(),w=(o.external_url||"").trim(),$=(o.external_doc_hint||"").trim(),E=(o.ocr_buyer_name||"").trim()||o.client_name||"-",_=o.seller_name||"-";let B="-";const q=Number(o.total_amount);o.total_amount!=null&&o.total_amount!==""&&!isNaN(q)&&(B=q.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2}));const H=p?t("erp-receipt-title-ok"):t("erp-receipt-title-fail"),F=p?"✓":"✗",A=[],y=(C,P)=>{A.push(`
                <div class="erp-receipt-row">
                    <span class="erp-receipt-key">${escapeHtml(C)}</span>
                    <span class="erp-receipt-val">${P}</span>
                </div>`)};if(y(t("erp-receipt-invoice-no"),`<strong>${escapeHtml(o.invoice_no||"-")}</strong>`),y(t("erp-receipt-erp-name"),escapeHtml(l)),p){let C;g?C=`<strong class="erp-receipt-docno">${escapeHtml(g)}</strong><button class="erp-receipt-copy-btn" type="button" data-receipt-copy="${escapeHtml(g)}" title="${escapeHtml(t("erp-doc-copy-tip"))}">${escapeHtml(t("erp-receipt-copy-btn"))}</button>`:C=`<span class="erp-receipt-docno-missing">${escapeHtml(t("erp-log-doc-missing"))}</span>`,y(t("erp-receipt-doc-no"),C)}y(t("erp-receipt-client"),escapeHtml(E)),y(t("erp-receipt-seller"),escapeHtml(_)),p&&y(t("erp-receipt-amount"),escapeHtml(B)),y(t("erp-receipt-time"),escapeHtml(f)),y(t("erp-receipt-elapsed"),escapeHtml((o.elapsed_ms!=null?o.elapsed_ms:"-")+"ms"));let v="";p&&w?v=`<a class="erp-receipt-primary-btn" href="${escapeHtml(w)}" target="_blank" rel="noopener">${escapeHtml(t("erp-receipt-open-erp"))}</a>`:p&&g&&(v=`<button class="erp-receipt-primary-btn" type="button" data-receipt-copy="${escapeHtml(g)}">${escapeHtml(t("erp-receipt-copy-docno"))}</button>`);let b="";if(p&&g&&$){const C="erp-receipt-hint-"+$,P=t(C);P&&P!==C&&(b=`<div class="erp-receipt-hint"><svg class="erp-receipt-hint-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><span>${escapeHtml(P)}</span></div>`)}let L="";if(!p){const C=(o.error_msg||"").match(/ERR_[A-Z0-9_]+/),P=C?C[0]:"",I=typeof currentLang=="string"&&currentLang||window._currentLang||"th",Y=o.error_friendly&&o.error_friendly[I]||(o.error_msg?humanizeError(o.error_msg):t("erp-receipt-no-error")),G=/ERR_NO_CUSTOMER_MAPPING|ERR_NO_CLIENT|ERR_NO_SEED_CUSTOMER|ERR_NO_SEED_PRODUCT/.test(o.error_msg||""),W=!!(o.history_id&&o.endpoint_id),te=[];te.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="exceptions">${escapeHtml(t("erp-receipt-act-exceptions"))}</button>`),G&&te.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="mappings">${escapeHtml(t("erp-receipt-act-mapping"))}</button>`),W&&te.push(`<button class="erp-receipt-action-btn primary" type="button" data-receipt-action="retry" data-log-id="${escapeHtml(o.id)}">${escapeHtml(t("erp-receipt-act-retry"))}</button>`),L=`
