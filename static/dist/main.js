...
`,e.dataset.wbInjected="1";try{const a=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",n=window.I18N;n&&n[a]&&(e.querySelectorAll("[data-i18n]").forEach(s=>{const i=s.getAttribute("data-i18n");n[a][i]&&(s.textContent=n[a][i])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(s=>{const i=s.getAttribute("data-i18n-placeholder");n[a][i]&&(s.placeholder=n[a][i])}))}catch{}}})();(function(){let e=[],a=null,n="",s="seller";const i={page:0,pageSize:12,keyword:""},h=new Set;let b=[];const _={keyword:""};let o=null;function v(){return{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}}async function w(c,H={}){const P=await fetch(c,{...H,headers:{"Content-Type":"application/json",...v(),...H.headers||{}}});if(!P.ok){const Y=await P.json().catch(()=>({}));throw new Error(Y.detail||"HTTP "+P.status)}return P.json()}async function u(){try{e=(await w("/api/clients")).clients||[],window._clientsCache=e}catch(c){console.error("loadClientsCache fail",c),e=[]}try{typeof window._refreshExcClientFilter=="function"&&window._refreshExcClientFilter()}catch{}try{typeof window._refreshClientSwitcher=="function"&&window._refreshClientSwitcher()}catch{}return e}function d(c){s=c==="buyer"?"buyer":"seller",document.querySelectorAll("[data-cust-tab]").forEach(Y=>Y.classList.toggle("active",Y.dataset.custTab===s));const H=document.getElementById("cust-pane-seller"),P=document.getElementById("cust-pane-buyer");H&&H.classList.toggle("active",s==="seller"),P&&P.classList.toggle("active",s==="buyer")}function E(){const c=window._userInfo||{},H=String(c.role||"").toLowerCase(),P=String(c.tenant_role||"").toLowerCase();return c.is_super_admin===!0||c.is_owner===!0||H==="owner"||H==="admin"||P==="owner"||P==="admin"}function k(){window._workspaceClientsCache=b,typeof window.renderWorkspaceControl=="function"&&window.renderWorkspaceControl()}async function q(){try{const c=await w("/api/workspace/clients");b=c&&(c.clients||c.items)||[],window._workspaceClientsCache=b}catch(c){console.error("loadSellerCache fail",c),b=[]}return b}function R(){const c=_.keyword.trim().toLowerCase();return c?b.filter(H=>(H.name||"").toLowerCase().includes(c)||(H.tax_id||"").toLowerCase().includes(c)):b}function y(){const c=document.getElementById("seller-tbody");if(!c)return;const H=E(),P=document.getElementById("btn-seller-new");P&&(P.style.display=H?"":"none");const Y=R(),Z=typeof window.getActiveWorkspaceClientId=="function"?window.getActiveWorkspaceClientId():null;if(!Y.length){c.innerHTML=`<div class="cust-empty">${escapeHtml(t(_.keyword?"cust-no-match":"seller-empty"))}</div>`;return}c.innerHTML=Y.map(le=>{const V=Z!=null&&Number(Z)===Number(le.id)?`<span class="cust-badge-current"><svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 7.5l3.2 3.2L12 4"/></svg>${escapeHtml(t("seller-current"))}</span>`:`<button class="cust-row-btn primary" data-saction="activate" data-wid="${le.id}">${escapeHtml(t("seller-set-current"))}</button>`,Q=H?`
            ...
            </div>`}).join("")}function B(c){o=c?c.id:null,document.getElementById("wsclient-modal-title").textContent=t(c?"wsclient-modal-edit":"wsclient-modal-new"),document.getElementById("wsclient-input-name").value=c&&c.name||"",document.getElementById("wsclient-input-tax").value=c&&c.tax_id||"",document.getElementById("wsclient-modal-archive").style.display=c?"":"none",document.getElementById("wsclient-modal-mask").style.display="flex",setTimeout(()=>document.getElementById("wsclient-input-name").focus(),50)}function O(){document.getElementById("wsclient-modal-mask").style.display="none",o=null}async function D(){const c=document.getElementById("wsclient-input-name").value.trim(),H=document.getElementById("wsclient-input-tax").value.trim();if(!c){showToast(t("client-msg-name-required"),"fail");return}try{o?(await w("/api/workspace/clients/"+o,{method:"PATCH",body:JSON.stringify({name:c,tax_id:H})}),showToast(t("client-msg-updated"),"success")):(await w("/api/workspace/clients",{method:"POST",body:JSON.stringify({name:c,tax_id:H||null})}),showToast(t("client-msg-created"),"success")),O(),await q(),y(),k()}catch(P){const Y=P&&P.message?P.message:t("client-msg-save-fail");showToast(t("client-msg-save-fail")+" · "+Y,"fail")}}async function F(){if(!o)return;const c=b.find(P=>Number(P.id)===Number(o));if(await showConfirm(t("wsclient-archive-confirm").replace("{name}",c?c.name:""),{danger:!0}))try{const P=o;await w("/api/workspace/clients/"+P,{method:"DELETE"}),showToast(t("wsclient-archived"),"success"),typeof window.getActiveWorkspaceClientId=="function"&&Number(window.getActiveWorkspaceClientId())===Number(P)&&typeof window.enterPersonalMode=="function"&&window.enterPersonalMode(),O(),await q(),y(),k()}catch{showToast(t("client-msg-save-fail"),"fail")}}function A(){const c=i.keyword.trim().toLowerCase();return c?e.filter(H=>(H.name||"").toLowerCase().includes(c)||(H.short_name||"").toLowerCase().includes(c)||(H.tax_id||"").toLowerCase().includes(c)):e}function f(){const c=A(),H=i.pageSize,P=Math.max(0,Math.ceil(c.length/H)-1);i.page>P&&(i.page=P);const Y=i.page*H;return{all:c,items:c.slice(Y,Y+H),start:Y,ps:H,total:c.length,maxPage:P}}function l(){const c=document.getElementById("buyer-tbody");if(!c)return;const{items:H,start:P,ps:Y,total:Z,maxPage:le}=f();Z?c.innerHTML=H.map(ee=>{const ne=h.has(ee.id);return`<div class="cust-row buyer-grid${ne?" selected":""}" data-cid="${ee.id}">
...
`,e.dataset.wbInjected="1";try{const a=window._currentLang||localStorage.getItem("mrpilot_lang")||"th",n=window.I18N;n&&n[a]&&(e.querySelectorAll("[data-i18n]").forEach(s=>{const i=s.getAttribute("data-i18n");n[a][i]&&(s.textContent=n[a][i])}),e.querySelectorAll("[data-i18n-placeholder]").forEach(s=>{const i=s.getAttribute("data-i18n-placeholder");n[a][i]&&(s.placeholder=n[a][i])}))}catch{}}})();(function(){let e={statsCache:null,listCache:[],currentRule:null,currentClient:"",currentStatus:"pending",loading:!1,selectedIds:new Set,offset:0,pageSize:50,total:0,loadFailed:!1,listScrollY:0};function a(S,g){let c=t(S)||S;if(g)for(const H in g)c=c.replace(new RegExp("\\{"+H+"\\}","g"),String(g[H]));return c}async function n(){try{const S=e.currentClient||"",g="/api/exceptions/stats?status=pending"+(S?"&client_id="+encodeURIComponent(S):""),c=await fetch(g,{headers:{Authorization:"Bearer "+(localStorage.getItem("mrpilot_token")||"")}});if(!c.ok)return;const H=await c.json(),P=document.getElementById("nav-exc-badge");if(!P)return;const Y=parseInt(H.pending||0,10);Y>0?(P.textContent=Y>99?"99+":String(Y),P.style.display=""):P.style.display="none"}catch{}}function s(S){return S==="high"?`<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            </button>`}g.innerHTML=Y,g.querySelectorAll(".exc-chip").forEach(Z=>{Z.addEventListener("click",()=>{const le=Z.dataset.rule||null;e.currentRule=le,k()})})}function v(S){const g=document.getElementById("exc-list");if(!g)return;if(!S||S.length===0){g.innerHTML=`<div class="exc-empty">
                ${i()}
        ...
        `}).join(""),_userInfo&&_userInfo.endpoints_limit!==-1){const i=ye.length,h=_userInfo.endpoints_limit,b=_userInfo.plan,_=document.createElement("div");_.className="erp-limit-hint",b==="free"?_.innerHTML=`${escapeHtml(t("ep-free-limit-hint",{used:i,limit:h}))} <a data-upgrade="plus">${escapeHtml(t("upgrade-to-plus"))}</a>`:_.textContent=t("ep-plus-limit-hint",{used:i,limit:h}),e.appendChild(_)}}function Ct(e){Ae=e||null;const a=document.getElementById("endpoint-modal"),n=document.getElementById("endpoint-modal-title");n.textContent=e?t("ep-modal-title-edit"):t("ep-modal-title-new");const s=document.getElementById("ep-name"),i=document.getElementById("ep-url"),h=document.getElementById("ep-token"),b=document.getElementById("ep-is-default"),_=document.getElementById("ep-auto-push"),o=document.getElementById("ep-test-result");o.style.display="none",o.textContent="";const v=document.getElementById("ep-save-error");if(v&&v.remove(),e){const u=ye.find(d=>d.id===e);if(!u)return;s.value=u.name||"",i.value=(u.config||{}).url||"",h.value=(u.config||{})._token_set&&u.config.token||"",h.placeholder=(u.config||{})._token_set?"（已保存 · 留空保持不变）":t("ep-token-ph"),b.checked=!!u.is_default,_.checked=!!u.auto_push}else s.value="",i.value="",h.value="",h.placeholder=t("ep-token-ph"),b.checked=ye.length===0,_.checked=!0;const w=_.closest(".form-switch-row");if(_.disabled=!1,w){w.classList.remove("disabled-plus"),w.title="",w.style.cursor="",w.onclick=null;const u=w.querySelector(".plus-badge");u&&u.remove()}a.style.display="",setTimeout(()=>s.focus(),50)}function it(){document.getElementById("endpoint-modal").style.display="none",Ae=null;const e=document.getElementById("ep-save-error");e&&e.remove()}function rt(e){if(!e)return"";let a=e.trim();const n=a.search(/\s/);return n>=0&&(a=a.slice(0,n)),a}function lt(){const e=document.getElementById("ep-name").value.trim(),a=rt(document.getElementById("ep-url").value),n=document.getElementById("ep-token").value,s=document.getElementById("ep-is-default").checked,i=document.getElementById("ep-auto-push").checked,h={url:a};return n&&(h.token=n),{name:e,url:a,tokenVal:n,isDefault:s,autoPush:i,config:h}}async function St(){const{url:e,config:a}=lt(),n=document.getElementById("ep-test-result");if(!e){n.style.display="",n.className="form-test-result fail",n.textContent=t("ep-required");return}n.style.display="",n.className="form-test-result running",n.textContent=t("ep-test-running");try{const i=await(await fetch("/api/erp/test-connection",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({adapter:"webhook",config:a})})).json();i.success?(n.className="form-test-result ok",n.textContent=t("ep-test-ok",{status:i.http_status,ms:i.elapsed_ms})):(n.className="form-test-result fail",n.textContent=t("ep-test-fail",{err:i.error_msg||"unknown"}))}catch(s){n.className="form-test-result fail",n.textContent=t("ep-test-fail",{err:s.message})}}async function Tt(){const e=lt(),a=document.getElementById("ep-save-error");if(a&&(a.style.display="none"),!e.name||!e.url){et(t("ep-required"));return}const n={name:e.name,adapter:"webhook",config:e.config,is_default:e.isDefault,auto_push:e.autoPush},s=document.getElementById("btn-ep-save"),i=s.innerHTML;s.disabled=!0,s.classList.add("loading");try{let h;if(Ae?h=await fetch(`/api/erp/endpoints/${encodeURIComponent(Ae)}`,{method:"PATCH",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify(n)}):h=await fetch("/api/erp/endpoints",{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify(n)}),!h.ok){const _=(await h.json().catch(()=>({}))).detail||`HTTP ${h.status}`;throw new Error(typeof _=="string"?_:JSON.stringify(_))}it(),showToast(t("ep-save-ok")),Ne()}catch(h){et(`${t("ep-save-fail")} · ${h.message||"unknown"}`)}finally{s.disabled=!1,s.classList.remove("loading"),s.innerHTML=i}}function et(e){let a=document.getElementById("ep-save-error");if(!a){const n=document.querySelector("#endpoint-modal .modal-foot");if(!n)return;a=document.createElement("div"),a.id="ep-save-error",a.className="ep-inline-error",n.parentNode.insertBefore(a,n)}a.textContent=e,a.style.display=""}async function Mt(e){const a=ye.find(s=>s.id===e);if(!(!a||!await showConfirm(t("ep-delete-confirm",{name:a.name}),{danger:!0})))try{if(!(await fetch(`/api/erp/endpoints/${encodeURIComponent(e)}`,{method:"DELETE",headers:{Authorization:"Bearer "+token}})).ok)throw new Error;showToast(t("ep-delete-ok")),Ne()}catch{showToast(t("ep-save-fail"),"fail")}}window.loadErpEndpoints=Ne;window.loadErpTodayStats=ot;window.renderErpEndpointsList=st;window.openEndpointModal=Ct;window.closeEndpointModal=it;window.saveEndpoint=Tt;window.deleteEndpoint=Mt;window.testEndpointConnection=St;window._sanitizeUrl=rt;async function ct(e){if(e=(e||"").trim(),!!e)try{await navigator.clipboard.writeText(e),showToast(t("erp-doc-copy-ok",{no:e}),"success")}catch{try{const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),n.remove(),showToast(t("erp-doc-copy-ok",{no:e}),"success")}catch{showToast(t("erp-doc-copy-fail"),"error")}}}async function Ht(e){const a=document.createElement("div");a.className="log-detail-modal",a.innerHTML=`
        <div class="log-detail-box">
            <div class="log-detail-loading">${escapeHtml(t("log-detail-loading"))}</div>
        </div>
    `,document.body.appendChild(a),a.addEventListener("click",async n=>{if(n.target===a||n.target.classList.contains("log-detail-close")){a.remove();return}const s=n.target.closest("[data-receipt-copy]");if(s){ct(s.dataset.receiptCopy);return}const i=n.target.closest("[data-receipt-action]");if(i){const h=i.dataset.receiptAction;h==="retry"?window.retryPushLog(i.dataset.logId):h==="exceptions"?typeof routeTo=="function"&&routeTo("exceptions"):h==="mappings"&&typeof routeTo=="function"&&routeTo("integrations"),a.remove();return}});try{const n=await fetch(`/api/erp/logs/${encodeURIComponent(e)}`,{headers:{Authorization:"Bearer "+token}});if(!n.ok){a.remove();return}const s=await n.json(),i=window._erpEndpoints.find(L=>L.id===s.endpoint_id),h=s.endpoint_name||(i?i.name:s.endpoint_id?t("erp-log-endpoint-deleted"):"-"),b=(s.endpoint_adapter||i&&i.adapter||"").toLowerCase(),_=new Date(s.created_at).toLocaleString(),o=s.trigger==="auto"?t("log-tag-auto"):s.trigger==="retry"?t("log-tag-retry"):t("log-tag-manual"),v=s.request_body?JSON.stringify(s.request_body,null,2):t("erp-receipt-no-tech"),w=s.response_body||t("erp-receipt-no-tech"),u=s.status==="success";let d=typeof w=="string"?w:JSON.stringify(w,null,2);if(u)try{const L=typeof s.response_body=="string"?JSON.parse(s.response_body):s.response_body||{},$=L.row_count||(Array.isArray(L.imported_rows)?L.imported_rows.length:0);$>0&&(d=t("log-push-rows").replace("{n}",String($)))}catch{}const E=(s.external_doc_no||"").trim(),k=(s.external_url||"").trim(),q=(s.external_doc_hint||"").trim(),R=(s.ocr_buyer_name||"").trim()||s.client_name||"-",y=s.seller_name||"-";let B="-";const O=Number(s.total_amount);s.total_amount!=null&&s.total_amount!==""&&!isNaN(O)&&(B=O.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2}));const D=u?t("erp-receipt-title-ok"):t("erp-receipt-title-fail"),F=u?"✓":"✗",A=[],f=(L,$)=>{A.push(`
                <div class="erp-receipt-row">
                    <span class="erp-receipt-key">${escapeHtml(L)}</span>
                    <span class="erp-receipt-val">${$}</span>
                </div>`)};if(f(t("erp-receipt-invoice-no"),`<strong>${escapeHtml(s.invoice_no||"-")}</strong>`),f(t("erp-receipt-erp-name"),escapeHtml(h)),u){let L;E?L=`<strong class="erp-receipt-docno">${escapeHtml(E)}</strong><button class="erp-receipt-copy-btn" type="button" data-receipt-copy="${escapeHtml(E)}" title="${escapeHtml(t("erp-doc-copy-tip"))}">${escapeHtml(t("erp-receipt-copy-btn"))}</button>`:L=`<span class="erp-receipt-docno-missing">${escapeHtml(t("erp-log-doc-missing"))}</span>`,f(t("erp-receipt-doc-no"),L)}f(t("erp-receipt-client"),escapeHtml(R)),f(t("erp-receipt-seller"),escapeHtml(y)),u&&f(t("erp-receipt-amount"),escapeHtml(B)),f(t("erp-receipt-time"),escapeHtml(_)),f(t("erp-receipt-elapsed"),escapeHtml((s.elapsed_ms!=null?s.elapsed_ms:"-")+"ms"));let l="";u&&k?l=`<a class="erp-receipt-primary-btn" href="${escapeHtml(k)}" target="_blank" rel="noopener">${escapeHtml(t("erp-receipt-open-erp"))}</a>`:u&&E&&(l=`<button class="erp-receipt-primary-btn" type="button" data-receipt-copy="${escapeHtml(E)}">${escapeHtml(t("erp-receipt-copy-docno"))}</button>`);let p="";if(u&&E&&q){const L="erp-receipt-hint-"+q,$=t(L);$&&$!==L&&(p=`<div class="erp-receipt-hint"><svg class="erp-receipt-hint-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><span>${escapeHtml($)}</span></div>`)}let I="";if(!u){const L=(s.error_msg||"").match(/ERR_[A-Z0-9_]+/),$=L?L[0]:"",x=typeof currentLang=="string"&&currentLang||window._currentLang||"th",W=s.error_friendly&&s.error_friendly[x]||(s.error_msg?humanizeError(s.error_msg):t("erp-receipt-no-error")),G=/ERR_NO_CUSTOMER_MAPPING|ERR_NO_CLIENT|ERR_NO_SEED_CUSTOMER|ERR_NO_SEED_PRODUCT/.test(s.error_msg||""),K=!!(s.history_id&&s.endpoint_id),te=[];te.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="exceptions">${escapeHtml(t("erp-receipt-act-exceptions"))}</button>`),G&&te.push(`<button class="erp-receipt-action-btn" type="button" data-receipt-action="mappings">${escapeHtml(t("erp-receipt-act-mapping"))}</button>`),K&&te.push(`<button class="erp-receipt-action-btn primary" type="button" data-receipt-action="retry" data-log-id="${escapeHtml(s.id)}">${escapeHtml(t("erp-receipt-act-retry"))}</button>`),I=`
        ...
        `,document.body.appendChild(w);const u=()=>w.remove();w.querySelector("#cpw-forgot-close").addEventListener("click",u),w.querySelector("#cpw-forgot-cancel").addEventListener("click",u),w.addEventListener("click",d=>{d.target===w&&u()}),w.querySelector("#cpw-forgot-send").addEventListener("click",async()=>{const d=w.querySelector("#cpw-forgot-send"),E=w.querySelector("#cpw-forgot-msg");d.disabled=!0;const k=d.textContent;d.textContent=n("cpw-forgot-sending");try{const q=await fetch("/api/auth/forgot_password",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:o})}),R=await q.json().catch(()=>({}));q.ok?(E.textContent=n("cpw-forgot-success"),E.className="cpw-forgot-msg success",d.style.display="none",w.querySelector("#cpw-forgot-cancel").textContent=n("cpw-forgot-close-btn")):(E.textContent=R.detail||n("cpw-forgot-fail"),E.className="cpw-forgot-msg error",d.disabled=!1,d.textContent=k)}catch{E.textContent=n("cpw-forgot-fail"),E.className="cpw-forgot-msg error",d.disabled=!1,d.textContent=k}})}function h(_){if(!_||!_.includes("@"))return _||"";const[o,v]=_.split("@");return o.length<=2?o+"****@"+v:o.slice(0,2)+"****@"+v}function b(_){return _==null?"":String(_).replace(/[&<>"']/g,o=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[o])}document.readyState==="complete"||document.readyState==="interactive"?s():document.addEventListener("DOMContentLoaded",s)})();(function(){let e=null,a=!1;async function n(){if(a)return;const i=localStorage.getItem("mrpilot_token");if(i){a=!0;try{const h=await fetch("/api/me",{headers:{Authorization:"Bearer "+i},cache:"no-store"});if(h.status===401){const b=await h.json().catch(()=>({})),_=b&&b.detail;let o="";if(typeof _=="string"?o=_:_&&typeof _=="object"&&(o=_.code||""),console.warn("[heartbeat] session revoked",o),localStorage.removeItem("mrpilot_token"),e&&(clearInterval(e),e=null),o==="auth.session_revoked")try{_showSessionRevokedModal()}catch{window.location.href="/"}else{const v=o==="auth.password_changed_relogin"?"alert-password-changed-relogin":"alert-session";try{typeof showToast=="function"&&typeof t=="function"?showToast(t(v),"error"):alert("Session expired")}catch{}setTimeout(()=>{window.location.href="/"},1500)}}}catch{}finally{a=!1}}}function s(){e&&clearInterval(e),e=setInterval(n,15e3)}localStorage.getItem("mrpilot_token")&&s(),window.addEventListener("focus",n),document.addEventListener("visibilitychange",function(){document.hidden||n()}),window._sessionCheck=n})();async function ze(){const e=document.getElementById("team-loading"),a=document.getElementById("team-list"),n=document.getElementById("team-empty"),s=document.getElementById("team-count");if(a){e&&(e.style.display=""),a.style.display="none",n&&(n.style.display="none");try{const i=await apiGet("/api/team/employees"),h=i&&i.employees||[];if(e&&(e.style.display="none"),s&&(s.textContent=(t("team-count")||"共 {n} 名员工").replace("{n}",h.length)),h.length===0){n&&(n.style.display="");return}a.style.display="",a.innerHTML=h.map(b=>{const _=b.last_login_at?new Date(b.last_login_at).toLocaleDateString():t("team-never-login")||"从未登录",o=b.is_active===!1?"team-status-off":"team-status-on",v=b.is_active===!1?t("team-status-disabled")||"已禁用":t("team-status-active")||"正常",w=b.email?`<span class="team-meta-sep">·</span><span>${escapeHtml(b.email)}</span>`:"";return`
            ...
            </div>`}).join("")}catch(i){console.error("loadTeamList failed:",i),e&&(e.textContent=t("team-load-failed")||"加载失败")}}}async function $t(){document.querySelectorAll(".add-emp-overlay").forEach(s=>s.remove());const e=document.createElement("div");e.className="add-emp-overlay",e.innerHTML=`
