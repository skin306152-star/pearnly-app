...
async def upload_report(
    ...
    if not report_id:
        raise HTTPException(500, "报告存储失败")
    return {
        "ok": True,
        "report_id": report_id,
...
async def batch_classify(request: Request, files: List[UploadFile] = File(...)):
    ...
    async def _classify_one(f):
        async with sem:
            ...
            try:
                ...
                logger.info(
                    f"[batch_classify] OK {f.filename} · {elapsed:.1f}s · type={info.get('type')}"
                )
                return {
                    "filename": f.filename,
    ...
    logger.info(
        f"[batch_classify] 完成 · 总耗时={time.time()-t_start:.1f}s · 成功={sum(1 for r in classified if r['type']!='unknown')}/{len(classified)}"
    )
    # v118.32.4.5 · 诊断:每个文件识别出的 type/tax_id/year/month
    for _r in classified:
        logger.info(
            f"[batch_classify] · {_r.get('filename','?')[:30]} type={_r.get('type')} tax={_r.get('tax_id')} year={_r.get('year')} month={_r.get('month')}"
        )
    ...
    if single_report:
        logger.info(
            f"[batch_classify] 单一报告模式 · 强制所有发票归 "
    ...
    for _u in list(unassigned_extras):
        if _u.get("reason_code") != "no_seller_tax_id":
            continue
        if _u.get("type") != "invoice":
            continue
        _yr, _mo = _u.get("year"), _u.get("month")
        if not _yr or not _mo:
            continue
        _candidates = [
            g for g in groups_map.values() if g.get("year") == _yr and g.get("month") == _mo
        ]
        if len(_candidates) != 1:
            continue
        _g = _candidates[0]
        _g["invoices"].append({"filename": _u["filename"], "confidence": _u.get("confidence", 0)})
        _g["min_conf"] = min(_g["min_conf"], _u.get("confidence", 0) or 0)
        _rescued.append(_u)
        logger.info(
            f"[batch_classify] 空税号发票兜底归入 {_u['filename'][:30]} → {_g['tax_id']}_{_yr}_{_mo}"
        )
    for _u in _rescued:
