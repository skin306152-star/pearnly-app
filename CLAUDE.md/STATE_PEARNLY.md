...
> **最近更新**:2026-05-24(**第十三会话**)· **🟢 通用模板学习层 ADR-006 S1–S5 全部完成并上线(v118.35.0.70 · home.js?v=11835070)· 新格式 Excel 银行账单不再"解析失败" → 弹"确认列对应"面板确认一次 → 保存 → 自动重跑出结果 → 下次同格式自动 · 真站点端到端 UI 验证通过(面板正确识别表头+预选列+保存重跑出结果)· KTB 1332行零回归 · 全程不烧 Gemini。剩 S6(CSV账单+GL总账)→ S7(AI低信心建议)→ S8(PDF/图片扫描件)。单一权威源 docs/refactor/adr-006-universal-importer.md**
>
