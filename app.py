...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "系统已同步优化『收入对账』模块。\n\n此前部分会计软件导出的 GL 文件中,日期字段可能显示为带时间格式(如 2568-12-31 00:00:00),影响阅读体验。本次更新已统一处理,日期将以公历(2025-12-31)清晰展示,与界面其他位置保持一致。\n\n本次同步检查了 4 个对账模块(发票上传、销售税对账、收入对账、银行对账)的日期处理与导出语言一致性,确保跨模块体验统一。导出的 Excel 文件标题始终跟随当前界面语言。\n\n即日生效。",
            "th": "ระบบได้ปรับปรุงโมดูล『กระทบยอดรายได้』ให้สอดคล้องกับการอัปเดตที่ผ่านมา\n\nก่อนหน้านี้ ฟิลด์วันที่ในไฟล์ GL ที่ส่งออกจากซอฟต์แวร์บัญชีบางประเภทอาจแสดงเป็นรูปแบบที่มีเวลา (เช่น 2568-12-31 00:00:00) ทำให้อ่านยาก การอัปเดตครั้งนี้ได้จัดการให้แสดงเป็นวันที่ ค.ศ. (2025-12-31) อย่างชัดเจน สอดคล้องกับส่วนอื่นของหน้าจอ\n\nการอัปเดตครั้งนี้ตรวจสอบโมดูลกระทบยอดทั้ง 4 โมดูล (อัปโหลดใบกำกับ, กระทบยอดภาษีขาย, กระทบยอดรายได้, กระทบยอดบัญชีธนาคาร) ให้การจัดการวันที่และภาษาส่งออกสอดคล้องกัน\n\nหัวข้อไฟล์ Excel ที่ส่งออก จะตรงกับภาษาที่เลือกในหน้าจอเสมอ\n\nมีผลทันที",
            "en": "The『Income Reconciliation』module has been synchronized with recent improvements.\n\nPreviously, the date field in GL files exported from certain accounting software could display in a date-and-time format (e.g. 2568-12-31 00:00:00), making it harder to read. This update standardizes the display to a clean date format (e.g. 2025-12-31), consistent with other parts of the interface.\n\nThis release also includes a synchronized audit across all 4 reconciliation modules (invoice upload, VAT-output reconciliation, income reconciliation, bank reconciliation) to ensure consistent date handling and export language behavior.\n\nExported Excel file titles consistently follow the current interface language.\n\nEffective immediately.",
            "ja": "『収益照合』モジュールを最近の改善に同期して更新しました。\n\nこれまで一部の会計ソフトウェアから出力された GL ファイルの日付欄が時刻付き形式(2568-12-31 00:00:00 等)で表示され、読みづらい場合がありました。本更新では西暦のクリーンな形式(2025-12-31 等)に統一表示し、画面の他の箇所と一貫した表示となります。\n\nまた、4 つの照合モジュール(請求書アップロード、売上税照合、収益照合、銀行照合)について、日付処理と出力言語の動作の整合性を一括点検しました。\n\n出力される Excel ファイルのタイトルは、常に現在のインターフェース言語に従います。\n\n即日有効。"
