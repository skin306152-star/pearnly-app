...
function renderDrawer() {
    ...
    const pdfPaneHtml = (() => {
        ...
        return `
            ...
            ${imageViewerHtml({ hint: t('imgv-hint'), noimg: t('imgv-noimg'), loading: t('imgv-loading') })}
        `;
    })();
    ...
    // 原图就绪后挂共用查看器(采购同款 · page.png · 拖拽/缩放/旋转/全屏)· 每次重渲先清旧实例
    if (_excViewerCleanup) {
        _excViewerCleanup();
        _excViewerCleanup = null;
    }
    const excPane = document.querySelector('.exc-pdf-pane') as HTMLElement | null;
    if (excPane && excPane.querySelector('.pv-viewer'))
        _excViewerCleanup = mountImageViewer(excPane, row.history_id || null);
