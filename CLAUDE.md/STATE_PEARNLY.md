...
- **待做 backlog(按价值排序)**:① **Agent 放量决策(Zihao)** + M2(数据层补齐 → 回填 `ok.history/summary/usage` 三键富版 · 大脑"几张"语义调优)② OCR 异步**开灯验证**(高敏计费 · 开前报方案:只扣一次 + 结果一致 + 秒回退)③ 续步升级(只记步号 → 后端 staged 草稿恢复 · 跨硬刷新 · **设计-gated**)④ 对账中心视觉对齐录入(rcx 独立一套 · 加右侧面板 · **设计-gated** · 见 `docs/TODO-录入入口统一与向导续步.md`)⑤ 整顿全局剩余:D 测试覆盖率 ~22%→70% · H 合规(隐私政策/真删数据)。
