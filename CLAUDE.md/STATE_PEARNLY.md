...
## 🎯 状态卡(2026-06-01 · **整顿主线 C3 进行中** · R15 `#bank-cand-drawer` + R16 客户/账套编辑弹窗 · home.html 1495→1375 上线 prod 11850055)

- **模式**:整顿封锁期 · **整顿主线 C3 进行中**(Zihao 2026-06-01 拍板:DMS 全链路收尾闭环 → 回整顿)。**当前 task = C3 拆 home.html → <1000**:本窗口 R15+R16 后 **1375**(还差 ~375)。下个候选(由易到难):剩余 on-demand modal(`endpoint-modal`~80〔erp-integration.js eval IIFE **无守卫**绑·注入须严格 import 在前〕/ `billing-modal-mask` / `camera-tips-modal` / `bank-client-picker-modal` 等·逐个查消费模块绑定时机)/ page-ocr(~150·识别主页高耦合·留最后)。**app shell(顶栏~128/侧栏~143)boot 关键勿轻动**。`email-modal` 牵涉 `line-email-modal.js`(🔴高敏)·跳过。
...
- **C3 进度封存**:home.html 1730→**1375**(R13 `#page-history`→`page-history.js` `4d3b9e0` + R14 `#onboarding-modal`→`welcome-wizard-html.js` `99c5da4` + **R15 `#bank-cand-drawer` inner→`bank-cand-drawer-html.js` `277b377`(-22)** + **R16 `#client-modal-mask`+`#wsclient-modal-mask` inner→`modal-client-edit.js` `bf42884`(-98)**·均字节等价+prod E2E 验〔R16 含真浏览器开两弹窗验注入 inner 可见+已译〕)。范式见记忆 [[wb-src-home-c3-split-playbook]](留空壳 + `*-html.js`/`modal-*.js` eval 期注入 inner + 子树初译 + import 置消费模块前;消费模块 load()/_bindOnce/DOMContentLoaded 守卫绑定→eval 注入恒早于首次绑定·非竞态)。app.py 1731→500 仍卡 auth 专窗口(剩高敏 @app·需 Zihao 在场)。
...
- **最后 commit**:`bf42884`(C3 R16 抽客户/账套主体编辑弹窗 inner→运行期注入 · REFACTOR-WB-C3 · bump `?v=` 11850054→11850055)· `git log --oneline -8` 查最新。**均已 push 上线·prod 11850055 健康**。**整顿轮 cache-bust:仍 bump `?v=` 保缓存正确,但不重写 release_notes(Zihao 2026-06-01:纯重构忽视新版本横幅·见记忆 [[refactor-skip-version-banner]])**。home.js 拆迁全收官 33768→0(范式 [[home-js-batch-split-bridge]])。
