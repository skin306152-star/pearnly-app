...
# 因此对不齐(泰语一直是中性的「ทำความเข้าใจคำสั่ง」)。
_STEP_UNDERSTAND = {"zh": "理解指令", "th": "ทำความเข้าใจคำสั่ง"}
...
_ASK = {
    ...
    "keyword": {
        "zh": "找哪一张票?给店名或单号。",
        "th": "หาใบไหน บอกชื่อร้านหรือเลขที่ใบ",
    },
    "period": {
        "zh": "哪一期?例如「上个月」或「2569-06」。",
        "th": "งวดไหน เช่น「เดือนที่แล้ว」หรือ「2569-06」",
    },
    "amount": {
        "zh": "要算哪个金额?",
        "th": "คำนวณจากยอดเงินไหน",
    },
    # 方向缺省会算出一个看不出错的假数,所以宁可多问一句(见 registry 的 basis 槽)。
    "basis": {
        "zh": "这个数含税了吗?",
        "th": "ตัวเลขนี้รวม VAT แล้วหรือยัง",
    },
}

# 不写「左侧」:390px 下是单栏,执行状态在这条消息上面而不是左边 —— 方位词一到手机就是错的。
_TASK_ACK = {
    "zh": "「{title}」已开始,结果回到这里。",
    "th": "เริ่ม「{title}」แล้ว ผลลัพธ์จะแจ้งในแชทนี้",
}

# 写工具入队的应承(confirm-first):活没开跑,先出授权卡等人批 —— 措辞不冒领「开跑了」。
_AUTHZ_ACK = {
    "zh": "「{title}」会改数据,已出授权卡。批准后才执行,拒绝则不执行任何操作。",
    "th": "「{title}」จะแก้ข้อมูล ออกการ์ดขออนุมัติแล้ว กดอนุมัติแล้วจึงจะทำ ถ้าปฏิเสธจะไม่ทำอะไรเลย",
}
...
_FAIL_REASON = {
    "steward.timeout": {
        "zh": "跑了超过 {seconds} 秒没有结果,已停止。稍后再说一次。",
        "th": "ทำงานเกิน {seconds} วินาทีแล้วไม่มีผล จึงหยุด สั่งใหม่ภายหลัง",
    },
    "steward.worker_lost": {
        "zh": "执行中断(服务重启或异常),这条没跑完。再说一次即可重跑。",
        "th": "งานถูกขัดจังหวะ (ระบบรีสตาร์ต) ยังทำไม่เสร็จ สั่งใหม่เพื่อทำอีกครั้ง",
    },
    "steward.queue_stalled": {
        "zh": "排队很久仍未开始(后台任务未运行),已标记失败。再说一次即可重跑。",
        "th": "รอคิวนานแต่ยังไม่เริ่มทำ (ระบบประมวลผลไม่ทำงาน) จึงบันทึกว่าล้มเหลว สั่งใหม่เพื่อทำอีกครั้ง",
    },
    "steward.cancelled": {
        "zh": "你取消了这条任务,后面的步骤没有执行。",
        "th": "คุณยกเลิกงานนี้แล้ว ขั้นตอนที่เหลือไม่ได้ทำ",
    },
    "steward.task_crashed": {
        "zh": "执行时出错,这条没跑完。再说一次即可重跑。",
        "th": "เกิดข้อผิดพลาดระหว่างทำงาน ยังทำไม่เสร็จ สั่งใหม่เพื่อทำอีกครั้ง",
    },
    "steward.context_lost": {
        "zh": "下单时的账号或权限已变更,这条无法继续。重新说一次即可。",
        "th": "บัญชีหรือสิทธิ์ที่ใช้สั่งงานเปลี่ยนไป งานนี้ทำต่อไม่ได้ สั่งใหม่อีกครั้ง",
    },
    "steward.authz_rejected": {
        "zh": "这个操作被拒绝,未执行任何步骤。要做的话重新说一次,批准后才会执行。",
        "th": "คำขอนี้ถูกปฏิเสธ ยังไม่ได้ทำอะไรเลย ถ้าต้องการทำ สั่งใหม่แล้วกดอนุมัติ",
    },
    "steward.budget_task_exceeded": {
        "zh": "这条任务的 AI 花费已达上限(฿{cap}),已停止。缩小范围再试,或让管理员调高上限。",
        "th": "งานนี้ใช้ค่า AI ถึงเพดานแล้ว (฿{cap}) จึงหยุด สั่งให้แคบลง หรือให้ผู้ดูแลปรับเพดาน",
    },
    "steward.budget_session_exceeded": {
        "zh": "这个会话的 AI 花费已达上限(฿{cap}),已停止。开新会话继续,或让管理员调高上限。",
        "th": "แชตนี้ใช้ค่า AI ถึงเพดานแล้ว (฿{cap}) จึงหยุด เปิดแชตใหม่เพื่อทำต่อ หรือให้ผู้ดูแลปรับเพดาน",
    },
    "steward.budget_tenant_exceeded": {
        "zh": "全所 24 小时内的 AI 花费已达上限(฿{cap}),已停止。稍后自动恢复,或让管理员调高上限。",
        "th": "สำนักงานใช้ค่า AI ใน 24 ชม. ถึงเพดานแล้ว (฿{cap}) จึงหยุด รอสักพักจะใช้ได้อีก หรือให้ผู้ดูแลปรับเพดาน",
    },
}

_OUT_OF_SCOPE = {
    "zh": (
        "这个还做不了。能查:今天从哪下手 · 本期矩阵 · 到期义务 · 待审队列 · 客户进度 · "
        "能否签批 · 交付物清单 · 工单清单 · 应交税额与全所税额表 · 本期票推完没 · 银行对账 · "
        "推送成败 · 识别记录与单票详情 · 客户名录 · VAT 试算。"
        "能改数据的只有一件:把已识别的票推进 Express(需你批准)。"
    ),
    "th": (
        "เรื่องนี้ยังทำไม่ได้ ที่ค้นได้: งานวันนี้ · ภาพรวมงวด · รายการใกล้ครบกำหนด · "
        "คิวรอตรวจ · ความคืบหน้าลูกค้า · ปิดงวดได้หรือยัง · ชุดส่งมอบ · รายการงาน · "
        "ยอดภาษีที่ต้องชำระและตารางภาษีทุกราย · ใบกำกับในงวดส่งครบหรือยัง · "
        "กระทบยอดธนาคาร · ผลส่งเข้า ERP · เอกสารที่สแกนและรายละเอียดใบเดี่ยว · "
        "รายชื่อลูกค้า · คำนวณ VAT; "
        "ที่แก้ข้อมูลได้มีอย่างเดียว: ส่งใบที่สแกนแล้วเข้า Express (ต้องให้คุณอนุมัติก่อน)"
    ),
}
_DEGRADED = {
    "zh": "暂时无法处理指令,这句没查成。稍后再说一次,或去「工作台」看期间矩阵。",
    "th": "ตอนนี้ประมวลผลคำสั่งไม่ได้ คำถามนี้จึงยังไม่ได้ค้น ลองใหม่อีกครั้ง หรือดูตารางงวดที่「แดชบอร์ด」",
}
_ERROR = {
    "steward.client_not_found": {
        "zh": "名录里没有「{keyword}」这家客户。换个说法,或先查客户名录。",
        "th": 'ไม่พบลูกค้าชื่อ "{keyword}" ในรายชื่อ พิมพ์ชื่ออื่น หรือค้นรายชื่อลูกค้าก่อน',
    },
    "steward.client_ambiguous": {
        "zh": "叫「{keyword}」的有 {n} 家:{names}。指明是哪一家。",
        "th": 'ชื่อ "{keyword}" มี {n} ราย: {names} ระบุว่ารายไหน',
    },
    "steward.history_forbidden": {
        "zh": "你的套餐看不了识别记录。",
        "th": "แพ็กเกจของคุณดูประวัติเอกสารไม่ได้",
    },
}
_ERROR_FALLBACK = {
    "zh": "这条没查成(错误码 {code})。再说一次,或直接在页面上查看。",
    "th": "ค้นไม่สำเร็จ (รหัส {code}) พิมพ์ใหม่ หรือดูในหน้าจอโดยตรง",
}
...
_CLIENT_STATUS_WITH_ORDER = {
    "zh": "{client_name} · {period}:{status}(当前 {step}),已收 {material_count} 件资料,{needs}。",
...
def authz_arg_rows(tool: str, args: dict, lang: str) -> list[dict]:
    """授权卡的明细行([{label, value}])。执行参数本身是机器形态(direction=sales、
    history_id=<uuid>),原样摆给会计签字等于让她闭眼签 —— 这里翻成她读得懂的行。
    没有登记翻译的写工具返空,前端据此回落原始键值,不静默丢参数。"""
    if tool == registry.ERP_PUSH:
        return copy_erp_push.arg_rows(args or {}, lang)
    return []
