...
租户开通路(不建号 · 不碰凭据 · 不回显密码);③初始密码满足强度(≥8 · 含字母和数字)且
【绝不落日志】;④非法邮箱早拒不碰 DB;⑤自定义密码三分支(过校验用它 / 不合格拒 / 留空随机);
⑥重置严格限发放制账号(租户持 pos_entitlement + 成员归属),主站普通用户/超管/不存在邮箱
一律 not_in_scope(路由翻 404 防枚举)。"""
...
class ProvisionOrchestrationTests(unittest.TestCase):
    ...
    def test_custom_password_used_when_valid(self):
        mock.patch.object(provision, "find_login_user", return_value=None).start()
        res = provision.provision_pos_account(
            object(), email="new@example.com", password="Custom1234"
        )
        # 自定义密码生效:回显它 + 哈希它
    ...
    def test_custom_password_policy_rejected(self):
        ...
        with self.assertRaises(ValueError) as short:
            provision.provision_pos_account(object(), email="new@example.com", password="Ab1")
        ...
        with self.assertRaises(ValueError) as weak:
            provision.provision_pos_account(
                object(), email="new@example.com", password="onlyletters"
            )
    ...
    def test_blank_password_falls_back_to_random(self):
        mock.patch.object(provision, "find_login_user", return_value=None).start()
        res = provision.provision_pos_account(object(), email="new@example.com", password=None)
        pw = res["initial_password"]
        self.assertTrue(pw and len(pw) >= 8)
        self.assertNotEqual(pw, "")

    def test_existing_email_never_touches_credentials(self):
        # 安全自查①:已存在邮箱分支不改动该账号任何凭据(不哈希 · 不写 password)。
        mock.patch.object(
            provision,
            "find_login_user",
            return_value={"id": "uid-old", "tenant_id": "tid-old", "username": "old"},
        ...
        provision.provision_pos_account(object(), email="old@example.com", password="Custom1234")
    ...
    def test_new_account_lands_only_in_fresh_tenant(self):
        ...
        mock.patch.object(provision, "find_login_user", return_value=None).start()
        provision.provision_pos_account(object(), email="new@example.com")
...
class ResetPosAccountPasswordTests(unittest.TestCase):
    ...
    def test_entitled_member_reset_ok_random(self):
        self._user()
        res = provision.reset_pos_account_password(object(), email="shop@example.com")
        pw = res["new_password"]
        self.assertTrue(pw and len(pw) >= 8)
        self.hash.assert_called_once_with(pw)
    ...
    def test_custom_password_three_branches(self):
        ...
        res = provision.reset_pos_account_password(
            object(), email="shop@example.com", password="Custom1234"
        )
        ...
        for bad, code in (("Ab1", "password_too_short"), ("onlyletters", "password_too_weak")):
            with self.assertRaises(ValueError) as e:
                provision.reset_pos_account_password(
                    object(), email="shop@example.com", password=bad
                )
            self.assertEqual(str(e.exception), code)
        # ③留空走随机
        res2 = provision.reset_pos_account_password(
            object(), email="shop@example.com", password=None
        )
    ...
    def test_out_of_scope_plain_user_without_entitlement(self):
        ...
        with self.assertRaises(ValueError) as e:
            provision.reset_pos_account_password(object(), email="free@example.com")
    ...
    def test_out_of_scope_super_admin(self):
        ...
        with self.assertRaises(ValueError) as e:
            provision.reset_pos_account_password(object(), email="admin@example.com")
        self.assertEqual(str(e.exception), "not_in_scope")
        self.write.assert_not_called()

    def test_out_of_scope_unknown_email(self):
        # 安全自查③:不存在邮箱 → 同一个 not_in_scope(不区分缘由 · 防枚举)。
        mock.patch.object(provision, "find_login_user", return_value=None).start()
        with self.assertRaises(ValueError) as e:
            provision.reset_pos_account_password(object(), email="ghost@example.com")
        self.assertEqual(str(e.exception), "not_in_scope")
        self.write.assert_not_called()
    ...
    def test_out_of_scope_tenantless_user(self):
        ...
        with self.assertRaises(ValueError) as e:
            provision.reset_pos_account_password(object(), email="orphan@example.com")
        self.assertEqual(str(e.exception), "not_in_scope")
        self.write.assert_not_called()
