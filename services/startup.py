...
from services.cost.ai_usage_store import ensure_ai_usage_table
...
def _boot_schema_ddl() -> None:
    ...
    try:
        from services.erp.shared_express_schema import ensure_shared_express_foundation

        ensure_shared_express_foundation()
    except Exception as e:
        logger.warning(f"启动 shared Express 数据底座失败(等 alembic 0108): {e}")
    ...
    try:
        ...
        logger.warning(f"启动 shared Express Profile 底座失败(等 alembic 0109): {e}")

    try:
        from services.erp.shared_express_managed_schema import (
            ensure_shared_express_managed_foundation,
        )

        ensure_shared_express_managed_foundation()
    except Exception as e:
        logger.warning(f"启动 shared Express managed ownership 失败(等 alembic 0110): {e}")

    try:
        from services.erp.shared_express_enrollment_schema import (
            ensure_shared_express_enrollment_rls,
        )
    ...
    try:
        from services.erp.shared_express_live_schema import ensure_shared_express_live_schema

        ensure_shared_express_live_schema()
    except Exception as e:
        logger.warning(f"启动 shared Express managed live profile 失败(等 alembic 0113): {e}")

    try:
        from services.erp.target_projection_schema import ensure_target_projection_schema

        ensure_target_projection_schema()
    except Exception as e:
        logger.warning(f"启动 ERP target projection 失败(等 alembic 0119): {e}")
...
async def run_startup() -> dict:
    ...
    if os.environ.get("PEARNLY_SKIP_HEAVY_INIT", "").lower() not in ("1", "true", "yes"):
        try:
            ...
            # B3 · 管家任务异步工人(默认开 · STEWARD_ASYNC=0 急停 · 表由 worker/首用自愈)
            from services.steward import worker as _steward_worker

            _steward_worker.start_embedded()
...
async def run_shutdown(tasks: dict):
    ...
    try:
        ...
        from services.steward import worker as _steward_worker  # B3

        await _recon_worker.stop_embedded()
        await _ocr_jobs_worker.stop_embedded()
        await _steward_worker.stop_embedded()
    except Exception:
        pass
    # 关闭时停定时
