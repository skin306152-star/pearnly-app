...
# 旧杂牌按钮类(应被收编/删除)
LEGACY_BTN_CLASSES = [
    "tc-btn", "tc-tool-btn", "bank-filter-btn", "email-interval-btn",
    "dash-quick-btn", "ob-btn-skip", "ob-btn-next", "int-btn-view-logs",
    "erp-map-show-advanced-btn", "erp-map-adv-btn-label", "folder-alt-btn",
    "hist-batch-icon-btn", "history-pager-btns",
]
...
def main():
    ...
    for rule, items in results.items():
        ...
        print(f"  {flag} {rule}: {n}")
    print("-" * 64)
    print(f"  违规合计:{total_violations}  →  {'✅ 全站统一达标' if total_violations == 0 else '🔴 未达标(以上即真实待办)'}")
    print("=" * 64)
