...
2026-07-22:3.6-flash 全线试过又全线退回——35 张带真值金标各两轮打平、79 张大样本互有
胜负,唯一收益是输出降价约 ฿15/月,却在银行长表上把余额链断点从 2 抬到 7。留在 3.5。
(3.6 在 Vertex 只有 global 端点,就近区域 404;那条区域规则留在 vertex provider 备查。)
"""
...
def best() -> str:
    """最强可用模型(3.5-flash 兜底档;关闭兜底时安全回落主力档)。
...
def tier_for_model(model_name: str) -> str:
    ...
    多档共用同一模型名时(默认全 3.5)映到最中性的 "flash"——
