...
解析优先级:显式传参 > 票上的声明 > None。None = 没人声明过,不是「选了服务」——下游
sales_mapper 据此走既有画像路径,未声明的票行为与本模块引入前逐字一致。
...
**故意没有账套级默认这一层。** sales_mapper 把「有 posting_kind」当作用户对本批的显式
决定,据此绕过「永续客户 + 库存路未开 → 交会计」的 escalate;一个常驻的账套级默认不是
本批决定,配上等于把那道安全网对该端点长期关闭。邮件收料 / LINE 这类没有向导会话的入口
因此解析出 None,维持既有 escalate 不变 —— 在采购侧库存入库(C6)落地前,那才是对的。
"""
