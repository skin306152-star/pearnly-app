...
G1 小票合规/画像派生已统一只读该表,这里改为读写代理而非另存一份,消掉两处零同步
的登记态(见 services/pos/sale.py _receipt_doc_kind 注)。tax_settings.vat_registered
列保留不迁移(零 schema 变更),代码里不再读写,待后续 schema 清理批一并退役该列。
"""
...
def _read_vat_registered(cur, *, tenant_id: str, workspace_client_id: int) -> bool:
    cur.execute(
        "SELECT vat_registered FROM workspace_clients WHERE tenant_id = %s AND id = %s",
        (tenant_id, workspace_client_id),
    )
    row = cur.fetchone()
    return bool(row["vat_registered"]) if row is not None else DEFAULTS["vat_registered"]


def _write_vat_registered(cur, *, tenant_id: str, workspace_client_id: int, value: bool) -> None:
    cur.execute(
        "UPDATE workspace_clients SET vat_registered = %s, updated_at = NOW() "
        "WHERE tenant_id = %s AND id = %s",
        (bool(value), tenant_id, workspace_client_id),
    )
...
def get_settings(cur, *, tenant_id: str, workspace_client_id: int) -> dict:
    ...
    out["vat_registered"] = _read_vat_registered(
...
def update_settings(cur, *, tenant_id: str, workspace_client_id: int, data: dict) -> dict:
    ...
    _write_vat_registered(
        cur,
        tenant_id=tenant_id,
        workspace_client_id=workspace_client_id,
        value=bool(merged["vat_registered"]),
    )
    cur.execute(
