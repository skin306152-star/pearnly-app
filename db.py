...
def seed_erp_test_mappings(user_id, tenant_id):
    """灌一份 mrerp 全量演示映射 · 客户取 tenant 内前 3 个真实客户
    v118.27.0.1 · 优化:单事务 + executemany · 从 ~4s 压到 < 500ms
    """
    if not tenant_id:
    ...
    try:
        with get_cursor(commit=True) as cur:
            ...
            cur.execute("""
                ...
                JOIN users u ON u.id = c.user_id
                WHERE u.tenant_id = %s AND c.is_active = TRUE
                ORDER BY c.id ASC LIMIT 3
            """, (str(tenant_id),))
            clients = list(cur.fetchall() or [])

            # 2) 客户映射 · executemany
            cli_rows = []
            for i, row in enumerate(clients):
                cid = int(row["id"] if isinstance(row, dict) else row[0])
                cname = row["name"] if isinstance(row, dict) else row[1]
                cli_rows.append((
                    str(tenant_id), cid, "mrerp",
                    f"01-TEST-{(i + 1):03d}",
                    f"DEMO {cname}"[:500],
                    str(user_id) if user_id else None,
                ))
            if cli_rows:
                cur.executemany("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON CONFLICT (tenant_id, client_id, erp_type)
                    DO UPDATE SET erp_code = EXCLUDED.erp_code,
                                  notes = EXCLUDED.notes,
                                  updated_at = NOW()
                """, cli_rows)
            cli_n = len(cli_rows)

            # 3) 科目映射(5 条)· executemany
            acc_data = [
                ("expense_office",  "5410", "ค่าใช้จ่ายสำนักงาน"),
                ("expense_travel",  "5420", "ค่าเดินทาง"),
                ("asset_inventory", "1310", "สินค้าคงเหลือ"),
                ("liability_ap",    "2110", "เจ้าหนี้การค้า"),
                ("revenue_sales",   "4110", "รายได้จากการขาย"),
            ]
            acc_rows = [
                (str(tenant_id), "mrerp", cat, code, name, "DEMO",
                 str(user_id) if user_id else None)
                for cat, code, name in acc_data
            ]
            cur.executemany("""
                INSERT INTO erp_account_mappings
                    (tenant_id, erp_type, pearnly_category, erp_code, erp_name, notes, created_by)
            ...
            tax_rows = [
                (str(tenant_id), "mrerp", kind, code, "DEMO",
                 str(user_id) if user_id else None)
                for kind, code in tax_data
            ]
            cur.executemany("""
                INSERT INTO erp_tax_mappings
                    (tenant_id, erp_type, pearnly_tax_kind, erp_code, notes, created_by)
        ...
        return {"ok": True, "clients": cli_n, "accounts": acc_n, "taxes": tax_n}
    except Exception as e:
        logger.error(f"seed_erp_test_mappings failed: {e}")
