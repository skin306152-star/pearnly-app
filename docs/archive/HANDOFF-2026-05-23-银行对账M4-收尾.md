# 交接备忘 · 银行对账 M4(第八会话 · 2026-05-23 · 已归档)

> **归档(2026-08-27)**:本文原为根目录 `HANDOVER_TO_NEXT_WINDOW.md`,因持有"push 用户授权后
> 让用户自己 `! git push origin master`"这条与 `AGENTS.md` 现行口径冲突的旧规,且正文早已是
> 2026-05-23 银行对账 M4 的历史存档,故挪来 `docs/archive/` 冻结。**查历史可看,勿当现状读。**
> **唯一活文档 = `docs/project/STATE_PEARNLY.md` 顶部状态卡 + `AGENTS.md`;真数字跑
> `python3 scripts/refactor_progress.py`。** 部署闭环以 `AGENTS.md §5` + `docs/RUNBOOK.md`
> 为准:自验 → commit → git push origin master → 盯本人 SHA 的 CI 全绿 → deploy job → 生产
> HEAD == 该 SHA 且重启时间 ≥ 部署。

> ⚠️ **已归档 / 陈旧(2026-06-02 标记)**:本文档停在 2026-05-23 第八会话(v118.35 改善计划期),数字/进度全过期。**活文档 = `docs/project/STATE_PEARNLY.md` 顶部状态卡 + `AGENTS.md`**。本文件仅留银行对账 M4 历史存档,**勿当现状读**。

> **最后更新**:2026-05-23(第八会话) · 当前版本 **v118.35.0.67** · cache bust **home.js?v=11835067**
> **🟢 改善计划已收尾(Zihao 2026-05-23 拍板)· 下窗口回归工程整顿 `docs/project/REFACTOR_MASTER_PLAN.md`(从 A5 续)· 不再以本文档/审计为主线。** 本文档留作银行对账(M4)的成果与代码地图存档。
...
> **下窗口第一步建议**:**改善计划已收尾 · 回归工程整顿**。读 `AGENTS.md`(铁律 18-22)+ `docs/project/STATE_PEARNLY.md`(第八会话段)+ `docs/project/REFACTOR_MASTER_PLAN.md`(进度看板 · 从 A5 续)。银行对账(M4)若用户报新问题再回来翻本文件;学习机制/M1-M3 设计见审计文档,等用户反馈再做。**始终用数字说话。**
