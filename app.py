...
async def get_frontend_version():
    ...
    return {
        ...
        "release_notes": {
            "zh": "v118.34.16 · MR.ERP 推送终于能真把发票发出去了:\n• 之前抽屉点「推送到 MR.ERP」秒报 'mrerp push is not wired'(stub 没接 · 上窗口漏了)\n• 现在 push_to_endpoint 真接 MRERPAdapter · 走批量上传 + 自动建客户/商品 + 解析 report.php 报错\n• 推送结果带回 mrerp_bill_no(SI 单号)· 推送日志和发票记录抽屉同步显示\n• 新增 2 个守门测试:一个 sync 验路由真接 adapter · 一个 async 验跑在 worker thread 不占事件循环\n• stub 函数留着不删 · 作为 ADAPTER_REGISTRY 兜底防回归\n\n配好 MR.ERP 后点抽屉「推送」· 应该能在 MR.ERP 后台 artran/allview.php 看到发票",
            "en": "v118.34.16 · MR.ERP push finally actually pushes invoices:\n• Previously clicking 'Push to MR.ERP' in the invoice drawer instantly failed with 'mrerp push is not wired into push_to_endpoint yet' — last window left the wiring as a stub.\n• push_to_endpoint now routes adapter='mrerp' through push_mrerp_history → MRERPAdapter.upload_invoice_batch (batch upload + auto-create customer/product from seed + parse report.php หมายเหตุ rejections).\n• Returns the SI bill number to the caller; push log + invoice drawer show consistent status.\n• Added two guard tests: a sync test that pins the route to call the real adapter (not the stub), and an async test that uses a tripwire MRERPAdapter to prove the work runs in a worker thread.\n• Kept push_mrerp stub in ADAPTER_REGISTRY as a regression tripwire.\n\nConfigure an MR.ERP connection, click Push in any invoice drawer — the invoice should appear in MR.ERP's artran/allview.php listing.",
            "th": "v118.34.16 · ปุ่ม 'ส่งไป MR.ERP' ทำงานได้จริงแล้ว:\n• ก่อนหน้านี้คลิกแล้วเด้งทันทีว่า 'mrerp push is not wired' (วินโดวก่อนหน้าทิ้ง stub ไว้ ลืมต่อจริง)\n• ตอนนี้ push_to_endpoint ต่อเข้ากับ MRERPAdapter จริง: อัปโหลด xlsx + auto-create ลูกค้า/สินค้า จาก seed + อ่านสาเหตุปฏิเสธจาก report.php\n• คืนเลขที่บิล (SI...) กลับมา ทำให้ log การส่ง + drawer ใบกำกับแสดงสถานะตรงกัน\n• เพิ่มเทสต์เฝ้าระวัง 2 ตัว: sync (เช็คเส้นทาง) + async (เช็คว่าทำงานบน worker thread ไม่ใช่ event loop)\n• stub function เก็บไว้ใน ADAPTER_REGISTRY เป็น regression tripwire\n\nตั้งค่าเชื่อม MR.ERP แล้วคลิกส่ง — ใบกำกับควรปรากฏใน artran/allview.php",
            "ja": "v118.34.16 · MR.ERP プッシュがやっと実際に請求書を送信するようになりました:\n• 以前は請求書ドロワーで「MR.ERPに送信」をクリックすると即座に「mrerp push is not wired」エラー（前のウィンドウがstubのまま残してました）\n• push_to_endpointが本物のMRERPAdapterに接続: バッチアップロード + seedから顧客/商品の自動作成 + report.phpのหมายเหตุ拒否解析\n• 請求書番号(SI...)を呼び出し元に返却し、プッシュログとドロワーの状態が一致\n• ガードテスト2本追加: sync（ルートが本物のadapterを呼ぶか）+ async（ワーカースレッドで実行されイベントループを占有しないか）\n• 回帰防止のため、push_mrerp stubはADAPTER_REGISTRYに残置\n\nMR.ERP接続を設定し、ドロワーの「送信」をクリック → MR.ERPのartran/allview.phpに請求書が表示されるはず"
        }
    }
