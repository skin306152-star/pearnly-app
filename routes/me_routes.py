...
class UserInfo(BaseModel):
    ...
    # 缺口④ · 网页 OCR 异步上传灰度开关(服务端全局 OCR_ASYNC_WEB · 前端据此走 submit 或同步老路)
    ocr_async_web: bool = False
    # 兼容旧字段(不再使用但前端可能仍引用)
...
def _build_user_info(user, ip_used=None, ip_limit=None) -> dict:
    ...
    return {
        ...
        # 缺口④ · 网页 OCR 异步灰度(服务端全局 flag · 与套餐无关 · worker 同闸)
        "ocr_async_web": os.environ.get("OCR_ASYNC_WEB", "0") == "1",
        # 次级
