...
async def get_frontend_version():
    ...
    return {
        ...
        "ts": int(_t.time()),
        "release_notes": {
            "zh": "系统已优化『收入对账』与『银行对账』模块的 GL(分类账)Excel 上传处理。\n\n此前部分会计软件导出的 GL 文件(日期采用佛历年、期初余额位于『余额』列)可能显示『0 行』。本次更新已支持该格式,文件可正常读取,期初余额、期末余额、明细行均显示准确。\n\n如需手动核对锚点数据,可在对账页面填写 3 个余额录入框(GL 期末 / Statement 期初 / GL 期初)以覆盖系统识别值。\n\n即日生效。",
            "th": "ระบบได้ปรับปรุงการอัปโหลดไฟล์ GL (บัญชีแยกประเภท) Excel ในโมดูล『กระทบยอดรายได้』และ『กระทบยอดบัญชีธนาคาร』\n\nก่อนหน้านี้ไฟล์ GL ที่ส่งออกจากซอฟต์แวร์บัญชีบางประเภท (ใช้ปี พ.ศ. และยอดยกมาอยู่ในคอลัมน์『คงเหลือ』) อาจแสดง『0 แถว』 การอัปเดตครั้งนี้รองรับรูปแบบดังกล่าวแล้ว ไฟล์อ่านได้ตามปกติ ยอดยกมา ยอดยกไป และรายการละเอียดแสดงถูกต้องครบถ้วน\n\nหากต้องการตรวจสอบข้อมูลแอนเคอร์ด้วยตนเอง สามารถกรอกในช่องยอด 3 ช่อง (ยอดยกไป GL / ยอดยกมา Statement / ยอดยกมา GL) บนหน้ากระทบยอด เพื่อแทนที่ค่าที่ระบบตรวจพบ\n\nมีผลทันที",
            "en": "The GL (general ledger) Excel upload in the『Income Reconciliation』and『Bank Reconciliation』modules has been improved.\n\nFiles exported from certain accounting software (using Buddhist-era dates, opening balance placed in the『Balance』column) that previously displayed『0 rows』can now be read normally. Opening balance, closing balance, and detail rows are all displayed accurately.\n\nTo manually verify anchor values, you may fill in the 3 balance input fields (GL Closing / Statement Opening / GL Opening) on the reconciliation page to override system-detected values.\n\nEffective immediately.",
            "ja": "『収益照合』および『銀行照合』モジュールにおける GL(総勘定元帳)Excel アップロード処理を改善しました。\n\nこれまで一部の会計ソフトウェアから出力された GL ファイル(仏暦年使用、期首残高が『残高』列に配置)が『0 行』と表示されることがありましたが、当該形式に対応済みです。ファイルは正常に読み込まれ、期首残高、期末残高、明細行が正確に表示されます。\n\nアンカー値を手動で確認する場合は、照合ページの 3 つの残高入力欄(GL 期末 / Statement 期首 / GL 期首)に入力することで、システム検出値を上書きできます。\n\n即日有効。"
