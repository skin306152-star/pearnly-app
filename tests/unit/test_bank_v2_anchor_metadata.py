...
class ApplyAnchorOverridesContractTests(unittest.TestCase):
    ...
    def test_no_override_returns_ocr_snapshot_and_empty_overrides(self):
        ...
        final_stmt, final_gl_open, final_gl_close, ocr_snap, overrides = _apply_anchor_overrides(
            stmt_opening=1000.0, gl_opening=1010.0, gl_closing=2000.0,
    ...
    def test_partial_override_only_replaces_specified_anchors(self):
        """P0.1 契约 · 只 override stmt_opening · 其它 2 个仍走 OCR · overrides 只记 stmt"""
        final_stmt, final_gl_open, final_gl_close, ocr_snap, overrides = _apply_anchor_overrides(
            stmt_opening=1000.0, gl_opening=1010.0, gl_closing=2000.0,
        ...
        self.assertEqual(final_gl_open, 1010.0)
        self.assertEqual(final_gl_close, 2000.0)
        # OCR snapshot 仍保留所有 3 个 OCR 原值(给 P0.3 历史对照)
        self.assertEqual(ocr_snap, {
            "stmt_opening": 1000.0,
            "gl_opening": 1010.0,
            "gl_closing": 2000.0,
    ...
    def test_all_three_overrides_preserve_ocr_in_snapshot_and_overrides(self):
        ...
        # OCR snapshot 仍是 OCR 原值(关键 · 不能被 override 污染)
        self.assertEqual(ocr_snap, {
            "stmt_opening": 1000.0,
            "gl_opening": 1010.0,
            "gl_closing": 2000.0,
        })
        # overrides 3 个全记 · ocr 字段必须是 OCR 原值 · user 字段是用户输入
        self.assertEqual(overrides, {
