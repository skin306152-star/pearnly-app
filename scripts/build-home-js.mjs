...
const SCAN_RESIDENT = [
    'scan/scan-loader.js',
    'scan/scan-feedback.js',
    'scan/scan-success-visual.js',
    'scan/scan-wedge.js',
];
...
const BUNDLES = [
    ...
    // 画像卡智能判断批次 · 2026-08-06)。
    {
        ...
        files: [
            ...
            // ai-tabs-scroll.js(横滚 tab 条的「当前项看得见」· 零依赖纯 DOM 工具)只需排在
            // ai-client.js(renderTabs 每次调 AI.tabsScroll.reveal)之前。
            'ai/ai-tabs-scroll.js',
            'ai/ai-client.js',
    ...
    // 连接向导(读 DXAPI)→ 套餐与余额(波1 · 模板 dms-billing-html 先于逻辑 dms-billing ·
    // 读 DXAPI)→ 记录页 →
    ...
    // ai-i18n.js),不并入本 bundle。
    {
        ...
        files: [
            // 与 /ai 共用同一份复制反馈(零依赖·挂 root.CopyFlash)。两个壳各自打包,
            // 共用只是在两张清单里各列一行 —— 不列这行,充值弹窗点复制就是 undefined。
            'shared/copy-flash.js',
            'dms/dms-util.js',
            'dms/dms-api.js',
            'dms/dms-gate.js',
