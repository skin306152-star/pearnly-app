# -*- coding: utf-8 -*-
"""qwen 档的三段提示词(2026-08-11 实测稿,51 张金标 49 中,逐字照搬不改写)。
...
{"seller_tax","buyer_tax","invoice_number","date","subtotal","vat","total_amount","currency","cash","change","discount"}
Rules:
...
- invoice_number: the document number (เลขที่/No./Rcpt/Slip/R#/Tax Invoice No). Copy EXACTLY as printed, keep all prefix letters and symbols.
...
Schema: {"seller_tax","buyer_tax","invoice_number","date","subtotal","vat","total_amount","currency"}
...
- Tax IDs exactly 13 digits from Tax ID labels (device/member numbers excluded). invoice_number = document number copied EXACTLY with prefixes; device IDs (POS/TID/MID/PERMIT) excluded.
