...
class ErpReceiptFieldsTests(unittest.TestCase):
    def test_erp_dates_use_printed_year_not_current_year_or_model_guess(self):
        with context(True):
    ...
    def test_erp_prompt_context_does_not_change_cowork(self):
    ...
    def test_explicit_receipt_payments(self):
        for direction in ("purchase", "sales"):
            with self.subTest(direction=direction):
                self.check_explicit_receipt_payments(direction)

    def check_explicit_receipt_payments(self, direction):
        assert (
            payment(
        ...
        assert (
            payment({"total_amount": "3806", "payment_method": "card"}, direction)["status"]
            == "unpaid"
        )
        f = {
            "total_amount": "1125",
        ...
        assert payment(f, direction)["paid_amount"] == Decimal("1125.00")
        assert (
            payment({"total_amount": "100", "payment_received": "40"}, direction)["status"]
            == "partial"
        )
