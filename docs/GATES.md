...
| 全量 unittest(分片并行 · `scripts/run_unit_sharded.py`) | 改 .py | 改一处崩别处 | `PYTHONUTF8=1 python scripts/run_unit_sharded.py` | 无;**新文件≥1测试**是另一条家规;**必须 `PYTHONUTF8=1`**(只设 `PYTHONIOENCODING` 会让 `test_file_crypto` 假红,什么都不设会让 `test_agent_capability_audit` 假红 · 见 `tests/unit/test_pre_push_hook_env.py`);2026-08-02 起分片并行跑(1089 模块切 6 段连续区间 · 段内保字母序防顺序耦合 · 覆盖面与单进程 discover 逐一相同,`tests/unit/test_run_unit_sharded.py` 钉住)· 单进程 ~9.5 分钟 → 2-3 分钟 |
