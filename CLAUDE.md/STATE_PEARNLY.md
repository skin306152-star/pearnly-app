...
> **最近更新**:2026-05-25(**第二十二会话 · B2 db.py→services 长跑开张**)· **🟢 全部 push + CI 5 连绿 + 生产验证。**
> **第二十二会话 · B2 启动(Zihao "继续 · 授权 push 长跑 · 达到目标行数")**:**db.py 10663→8439(-2224)** · 抽 5 个 cohesive 域 DAL 到 `services/`:① `email_ingest/store.py`(10 函数 `435ece6`)② `erp/oauth_store.py`(12 函数 + _b64 私有 `78e9667`)③ `erp/mappings_store.py`(15 函数 + ERP_TYPES_VALID/PEARNLY_TAX_KINDS_VALID 常量随域搬 `f62d0d9`)④ `notification/store.py`(9 函数 `9de1baa`)⑤ `erp/push_store.py`(25 函数 + ERP_MAX_RETRIES/USER_DATA_ERROR_CODES/THAI_PATTERNS 常量 + 重试调度 + is_user_data_error 分类器 `7edb3c3`)。**范式确立**:cohesive 域整片搬 service 模块(`import db` + 运行时 `db.get_cursor()` → `patch("db.get_cursor")` 仍生效 + 防循环 import)· db.py 文件尾 `from services.X import a as a` 显式 re-export(pyflakes 不报 F401)→ 所有 `db.xxx()`/`from db import xxx` 调用点**零改动** · 私有 `_helper`/`_常量` 不外露 · 校验常量随域搬。中途 `09a3e73` 把前 3 个 store 从 `from db import get_cursor` by-value 统一改 `import db`(notification_rules 是 tenant 隔离矩阵表 · by-value 让 patch 失效 → 9 测试红 · 已修 · 确立统一范式)。**守门**:每域带契约测试(函数在 service / re-export 同一对象防漂移)· unit 656→**676** · 既有 tenant 隔离/分类器/dedup/adapter filter 测试全绿 · LF 干净无 BOM。**全 push master + CI 5 连绿 + 生产逐域 401 验证**(/api/email-ingest/account · /api/erp/{mappings/clients,connectors/status,endpoints,logs} · /api/notifications/rules 全返 401 非 500/404 = 路由+re-export 函数加载正常)。services/*.py 40→**47**。⚠️ **下窗口续 B2**(剩余域见 `REFACTOR_MASTER_PLAN.md` "下一个 task" · 由易到难 · **抽前必 re-grep 当前行号**——每次删除后行号下移 · credits/charge_ocr(钱)/auth(登录)/ocr_history(OCR 热路径)高敏后置 · 建议 Zihao 在场)。
