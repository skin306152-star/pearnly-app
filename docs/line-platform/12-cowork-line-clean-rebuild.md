...
Pearnly Cowork LINE 不是聊天机器人。它只完成票据识别、复核、留档和按用户选择推送 ERP：

`成员自行绑定 → 选择 ERP/账套/采购或销售/过账方式 → 上传票据 → OCR → 编辑 → 确认 → 写入 Cowork 识别记录和推送记录`

完成分支：

- 选 MR.ERP 或 Express：先保存 Cowork 识别记录和推送意图，再推送所选目标。
...
- 手机优先，主路径按菜单逐步选择，采购/销售方向由用户明确确认。
